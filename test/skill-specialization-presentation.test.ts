...
for (const tier of ['基础', '下位', '中位']) test(`${tier} 首点相同，下一级按倍率比显示`, () => {
  ...
  assert.equal(lines.overcharge[0], '升至Lv.2：威力+8%；蓝耗+16%；冷却/吟唱基数+8%。');
  assert.equal(lines.efficient[0], '升至Lv.2：蓝耗−32%（乘算当前蓝耗）。');
  assert.ok(lines.potent[2].includes('可成长时长+8%'));
  assert.ok(passiveSpecializationPerLevelLine(tier).includes('+1.5%'));
  assert.ok(specializationPerLevelLines(tier, { overcharge: 2 }).overcharge[0].includes('威力+7.2%；蓝耗+16%'));
...
test('总体变化使用公式合算值，不相加每级展示的舍入值', () => {
  ...
  assert.ok(lines.includes('威力：150 → 155.52（+3.68%）'));
  assert.ok(lines.includes('蓝耗：300 → 275（−8.33%）'));
  assert.ok(lines.includes('冷却：3 → 3回合｜吟唱：0 → 0回合'));
  assert.ok(lines.some(line => line.includes('时间基数：+7.31%')));
});
