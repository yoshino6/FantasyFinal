...
test('已接取环展示目标和品质不限的材料库存，异地只能导航回导师',async()=>{
  const tasks=await setup([row()]).run();assert.equal(tasks.length,1);assert.match(tasks[0].title,/魔学者 1\/10/);assert.match(tasks[0].description,/微愈液：3\/2/);assert.match(tasks[0].description,/晴儿.*百纳镇/);assert.equal(tasks[0].action.command,'/前往 12 34 0');assert.doesNotMatch(JSON.stringify(tasks),/委托操作|隐藏二转 magical_scholar/);
});
