  ...
  try{
    ...
    await t.test('四家店固定三级：普通玩家可代工，实际扣料不加熟练度，离店及跨来源确认拒绝',async()=>{
      ...
      try{
        ...
        const affinity=async(shop:string)=>{const [rows]=await pool.execute<any[]>('SELECT affinity,daily_craft_count FROM player_npc_affinity WHERE character_id=1 AND npc_code=?',[shop]);return rows[0];};
        assert.equal(Number((await affinity('alchemy_sweetshop')).affinity),10);
        ...
        assert.equal(Number((await affinity('alchemy_sweetshop')).affinity),10,'重放提纯票据不再次奖励');
        ...
        assert.equal(Number((await affinity('oddworkshop')).affinity),10);
        for(let i=0;i<3;i++)await run('oddworkshop',()=>deconstruction.deconstructItems('automaton_test_1',Number(bone.item_id),1));
        assert.equal(Number((await affinity('oddworkshop')).affinity),30,'同店每日最多三次');
        assert.equal(Number((await affinity('oddworkshop')).daily_craft_count),3);
