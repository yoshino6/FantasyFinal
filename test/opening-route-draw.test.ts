...
const all:OpeningRouteCandidate[]=openingRoutes.map(route=>({code:route.code,regionCode:route.region,tier:openingSpawnRegions[route.region as keyof typeof openingSpawnRegions].tier}));
const memory=()=>{
...
test('八条路线按原档位和地图权重分配，女神路线维持独立旧概率',()=>{
...
test('普通七路线一轮不重样，抽尽才换轮，女神连续命中不消耗轮次',async()=>{
  const db=memory(),results=[];
  // .99 不命中女神；每次从剩余加权池取尾部。
  for(let i=0;i<7;i++)results.push(await drawOpeningRoute(db.connection,all,()=>.99));
  assert.equal(new Set(results).size,7);assert.ok(!results.includes('A01'));assert.equal(db.state.cycle_no,1);
  const before=db.state;
  for(let i=0;i<3;i++)assert.equal(await drawOpeningRoute(db.connection,all,()=>0),'A01');
  assert.deepEqual(db.state,before);assert.equal(db.writes,7);
...
test('女神概率不随普通剩余池变化；新地图加入和关闭不清空已抽记录',async()=>{
  ...
  const withoutGoddess=all.filter(c=>c.code!=='A01');
  const next=await drawOpeningRoute(db.connection,withoutGoddess,()=>.5);assert.notEqual(next,first);assert.equal(db.state.cycle_no,1);
});
...
test('完整降临流程使用不放回路线对应的地图，并保留合法坐标和安全终点',async()=>{
  ...
  const hubs=[{id:5,code:'baina_town',guild_code:'guild_counter',pos_x:80,pos_y:0,pos_z:0},{id:6,code:'world_tree',guild_code:'world_tree_adventurer_guild',pos_x:100,pos_y:0,pos_z:0}];
  ...
  for(let i=0;i<5;i++){
    ...
    assert.equal(spawn.region.code,spawn.route.region);assert.ok(['world_tree','baina_town'].includes(spawn.route.destination));
