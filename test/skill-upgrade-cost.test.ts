...
test('全部天赋拒绝学习、升级与四类专精，不扣SP、不写流水或专精记录',async()=>{
  for(const talent of talentDefinitions){
    const f=fixture({code:talent.code,category:'bound',level:1,max_level:99},1000);
    await assert.rejects(f.service.learnSkill('test',7),/天赋/);
    await assert.rejects(f.service.upgradeSkill('test',7),/天赋/);
    for(const direction of ['overcharge','potent','instant','efficient'])await assert.rejects(f.service.upgradeSkillSpecialization('test',7,direction),/天赋/);
    assert.deepEqual(f.writes,[]);assert.deepEqual(f.logs,[]);
    assert.equal(canSpecializePassive(talent.code,{damageBonusPct:50}),false);
    const base={code:talent.code,category:'bound',power:100,mana_cost:100,cooldown_turns:5};
    assert.deepEqual(specializationOptions(base,true),[]);
    assert.deepEqual(skillSpecialization(base,{overcharge:40,potent:40,instant:40,efficient:40}),skillSpecialization(base));
  }
});
