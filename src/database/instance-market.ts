...
export const initializeInstanceMarket=async(pool:Pool)=>{
  ...
  for(const [name,table] of [['equipment_market_lock_v1','player_equipment'],['device_market_lock_v1','player_active_devices']])if(!names.has(name))await pool.query(`CREATE TRIGGER ${name} BEFORE INSERT ON ${table} FOR EACH ROW BEGIN
  ...
  END`);
  if(!names.has('equipment_binding_update_v1'))await pool.query(`CREATE TRIGGER equipment_binding_update_v1 BEFORE UPDATE ON player_equipment FOR EACH ROW BEGIN
    IF EXISTS(SELECT 1 FROM player_item_instances WHERE id=NEW.instance_id AND market_listing_id IS NOT NULL) THEN SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='item is in market escrow'; END IF;
    UPDATE player_item_instances SET bound_kind=IF(bound_kind='personal','personal','trade'),bound_at=COALESCE(bound_at,NOW()),bound_reason=COALESCE(bound_reason,'used') WHERE id=NEW.instance_id;
  END`);
