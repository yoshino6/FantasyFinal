...
appGroup.use({ path: '升级鉴识', schema: { usage: '/升级鉴识 <等级差|信息深化>', args: [{ name: 'direction', rules: [{ required: true, type: 'enum', enum: ['等级差', '信息深化'] }] }] } }, () => import('./response/skill-list').then(module => ({ default: module.upgradeAppraisalHandler })))
