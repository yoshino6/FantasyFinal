/** 专精只改变有界数值；不增长行动次数、控制时长、资源返还、驱散层数或机制权限。 */
...
export const specializationProgress = (level: unknown, tier?: string) => Math.max(0, Math.min(1, (finite(level, 1) - 1) / (specializationMaximum(tier) - 1)));
...
export const specializationOptions = (base: SpecializationBase, hasOrdinaryEffect = false): Specialization[] => {
  ...
  if (base.power > 0 || supportGrowth.has(base.code)) options.push('overcharge');
  if (skillSpecialization(base, { instant: specializationMaximum(base.tier) }).cooldown < base.cooldown_turns || Number(base.chant_turns) > 1) options.push('instant');
  if (base.mana_cost > 0) options.push('efficient');
  if (base.power > 0 || supportGrowth.has(base.code) || ordinaryGrowth.has(base.code) || hasOrdinaryEffect) options.push('potent');
  return options;
};
/** 只有普通数值效果参与强效；不碰标记、硬控、法镜、回蓝和冷却/行动经济。 */
export const specializeEffectValue = (code: string, value: number, factor = 1) => {
  const caps: Record<string, number> = { beat: 30, refraction: 50, blind_resist: 60, accuracy_down: 50, evasion: 50, evasion_down: 50, alchemy_evasion: 50, alchemy_guard: 50, shield_guard: 60, imbalance: 50, next_damage: 50, damage: 50, life_shield: 40, attack: 50, magic: 50, defense: 50, magic_defense: 50, speed: 50, accuracy: 50, attack_down: 50, magic_down: 50, slow: 50, armor_shatter: 50, magic_shatter: 50, reduction: 60, physical_reduction: 60, magic_reduction: 60, barrier: 60, exposed: 50, vulnerability: 50, battle_cry: 50, sprint: 50, precision: 50, poison: 15, burn: 15, bleeding: 15, bleed: 15, regeneration: 20 };
  const cap = caps[code];
  // 已高于上限的 Boss 固定机制不被专精函数反向削弱。
