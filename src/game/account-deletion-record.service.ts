...
const databaseValue = (value: unknown): unknown => {
...
const foreignKeys = async (connection: PoolConnection) => {
  ...
  return rows.filter(row => !excludedTables.has(row.tableName) && !excludedTables.has(row.referencedTable));
};
...
const tableOrder = (tables: Record<string, SnapshotRow[]>, keys: ForeignKey[]) => {
...
export const restoreDeletedAccount = async (recordId: number, operatorQqUserId: string, overwrite = false) => withTransaction(async connection => {
  ...
  for (const table of snapshot.tableOrder) {
    ...
    for (const row of rows) {
      ...
      await connection.execute(sql, columns.map(column => databaseValue(row[column])) as any);
    }
  }
