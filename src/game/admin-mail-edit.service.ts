...
type EditRow = RowDataPacket & { id: number; recipient_scope: Scope; content: string; status: 'editing' | 'draft' };
...
export type MailEdit = { scope: Scope; status: 'editing' | 'draft'; content: string; recipients: Array<{ qqUserId: string; nickname: string }>; attachments: Array<{ itemId: number; name: string; quantity: number }> };
...
const editRow = async (connection: Connection, adminQqUserId: string, lock = false) => {
  const [rows] = await connection.execute<EditRow[]>(`SELECT id,recipient_scope,content,status FROM admin_mail_edits WHERE admin_qq_user_id=? LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [adminQqUserId]);
  return rows[0] ?? null;
};
...
const loadEdit = async (connection: Connection, edit: EditRow): Promise<MailEdit> => {
  ...
  return { scope: edit.recipient_scope, status: edit.status, content: edit.content, recipients: recipients.map(row => ({ qqUserId: row.qq_user_id, nickname: row.nickname })), attachments: attachments.map(row => ({ itemId: Number(row.item_id), name: row.name, quantity: Number(row.quantity) })) };
};
...
const validateEdit = (edit: MailEdit) => {
  ...
  if (!edit.content.trim()) throw new Error('请先编辑邮件内容。');
  if (!edit.attachments.length) throw new Error('请至少添加一项附件。');
};
...
export const openMailEdit = async (adminQqUserId: string, scope: Scope) => withTransaction(async connection => {
  ...
  if (!edit) {
    const [result] = await connection.execute<ResultSetHeader>('INSERT INTO admin_mail_edits (admin_qq_user_id,recipient_scope,content) VALUES (?,?,?)', [adminQqUserId, scope, '']);
    edit = { id: Number(result.insertId), recipient_scope: scope, content: '', status: 'editing' } as EditRow;
  } else if (edit.status === 'draft') {
    await connection.execute("UPDATE admin_mail_edits SET status='editing' WHERE id=?", [edit.id]);
    edit.status = 'editing';
  }
...
export const updateMailContent = async (adminQqUserId: string, content: string) => withTransaction(async connection => {
  await requireAdministrator(adminQqUserId, connection); const edit = await requireEdit(connection, adminQqUserId);
  const text = content.trim(); if (!text) throw new Error('邮件内容不能为空。'); if (text.length > 2000) throw new Error('邮件内容不能超过 2000 字。');
...
export const sendMailEdit = async (adminQqUserId: string) => withTransaction(async connection => {
  ...
  for (const characterId of targets) {
    const [mail] = await connection.execute<ResultSetHeader>('INSERT INTO player_mails (character_id,title,content) VALUES (?,?,?)', [characterId, '管理员邮件', data.content]);
