...
const writeStage = (connection: PoolConnection, characterId: number, stage: number) => connection.execute('INSERT INTO player_main_quest_progress (character_id,quest_code,stage) VALUES (?,?,?) ON DUPLICATE KEY UPDATE stage=VALUES(stage)', [characterId, questCode, stage]);
...
export const teleportToWorldTree = (qqUserId: string) => withTransaction(async connection => {
  ...
  if (!regions[0]) throw new Error('世界树尚未开放。');
...
export const returnToBainaTown = (qqUserId: string) => withTransaction(async connection => {
  const character = await characterFor(connection, qqUserId, true);
  await requireAt(connection, character, worldTreeGateCode);
  const [regions] = await connection.execute<(RowDataPacket & { id: number })[]>('SELECT id FROM map_regions WHERE code=? LIMIT 1 FOR UPDATE', ['baina_town']);
  if (!regions[0]) throw new Error('百纳镇尚未开放。');
