import { retiredAchievementIds } from './achievement-retired.config';
import { MessageDirect, logger } from 'alemonjs';
import type { RowDataPacket } from 'mysql2/promise';
import { getPool } from '../database/pool';
import { achievementAnnouncementFormat } from './achievement-message';
import { achievementById } from './achievement-rules';
import { loadBossAchievementDefinitions } from './achievement-boss';
import { achievementDeliveryOutcome } from './achievement-delivery-rules';

let running=false;
export const deliverAchievementAnnouncements=async()=>{
  ...
  try{
    ...
    // 已开始发送但进程退出：结果未知，禁止自动重发造成重复全群公告。
    ...
    const [rows]=await db.query<RowDataPacket[]>(`SELECT d.*,a.name_snapshot,a.rarity FROM achievement_deliveries d JOIN achievement_announcements a ON a.achievement_id=d.achievement_id WHERE d.status='pending' AND d.attempts<5 AND a.created_at<=DATE_SUB(NOW(),INTERVAL 15 SECOND) ORDER BY a.created_at,d.bot_id,d.group_id LIMIT 20`);
    ...
    for(const row of rows){
      ...
      if(!definition){logger.warn({achievementId:row.achievement_id},'成就首位公告缺少定义，保留待发送');continue;}
      const [claimed]=await db.execute<any>("UPDATE achievement_deliveries SET status='sending',attempts=attempts+1,updated_at=NOW() WHERE achievement_id=? AND bot_id=? AND group_id=? AND status='pending'",[row.achievement_id,row.bot_id,row.group_id]);
