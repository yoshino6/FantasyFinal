...
type BountyRow = RowDataPacket & { id: number; title: string; target_name: string; required_count: number; copper_reward: number; progress: number | null; status: 'accepted' | 'completed' | 'claimed' | null };
...
export const bountyBoard = async (qqUserId: string) => {
  ...
  const [rows] = await pool.execute<BountyRow[]>(`SELECT b.id,b.title,t.name AS target_name,b.required_count,b.copper_reward,pb.progress,pb.status
  ...
  return { bounties: rows.map(row => ({ id: Number(row.id), title: row.title, targetName: row.target_name, requiredCount: Number(row.required_count), copperReward: Number(row.copper_reward), progress: row.progress === null ? 0 : Number(row.progress), status: row.status })), activeCount: Number(activeRows[0]?.total ?? 0) };
};
...
export const playerBounties = async (qqUserId: string) => {
  ...
  return rows.map(row => ({ id: Number(row.id), title: row.title, targetName: row.target_name, requiredCount: Number(row.required_count), copperReward: Number(row.copper_reward), progress: Number(row.progress), status: row.status! }));
};
...
export const advanceBountyProgress = async (connection: PoolConnection, characterId: number, templateIds: number[]) => {
  const counts = new Map<number, number>();
  templateIds.forEach(id => counts.set(id, Number(counts.get(id) ?? 0) + 1));
  for (const [templateId, count] of counts) await connection.execute(`UPDATE player_bounties pb JOIN bounty_notices b ON b.id=pb.bounty_id
    ...
    WHERE pb.character_id=? AND b.target_template_id=? AND pb.status='accepted'`, [count, count, count, characterId, templateId]);
};
