...
import { purifiedCraftMaterialDisplayName, purifiedCraftMaterialValue, purifiedMaterialForArmor } from './monster-crafting-material.service';
...
const armorClassEffects: Record<string, string> = {
...
export const armorClassEffectText = (subtype: string | null | undefined) => armorClassEffects[subtype ?? ''] ?? '';
const forgeMaterialValue: Record<string, number> = {
...
const tierForgeMaterial = (level: number) => level <= 20 ? 'living_wood' : 'ridge_core';
const weaponCraftMaterial = (subtype: string | null | undefined) => subtype === '匕首' ? 'tanned_spirit_leather' : subtype === '法杖' || subtype === '法书' || subtype === '法球' ? 'spellcloth_bolt' : subtype === '盾牌' ? 'cast_shell_plate' : 'bone_steel_plate';
const forgeRequirements = (category: string, subtype: string | null | undefined, level: number): ForgeRequirement[] => {
  ...
  if (category === '武器') {
    const quantity = ({ 15: 8, 20: 12, 25: 10, 30: 14 } as Record<number, number>)[level];
    if (!quantity) throw new Error('当前仅开放 Lv.15、20、25、30 的常规打造。');
    return [{ code: base, quantity }, { code: weaponCraftMaterial(subtype), quantity: ({ 15: 2, 20: 3, 25: 3, 30: 4 } as Record<number, number>)[level] }];
  }
  const armorType = ['轻甲', '皮甲', '重甲', '布甲', '板甲'].includes(subtype ?? '') ? subtype! : '轻甲';
  const weights: Record<string, [number, number]> = {
    '头肩': [({ 15: 3, 20: 4, 25: 4, 30: 5 } as Record<number, number>)[level] ?? 0, ({ 15: 1, 20: 1, 25: 1, 30: 2 } as Record<number, number>)[level] ?? 0],
    '上装': [({ 15: 6, 20: 8, 25: 7, 30: 9 } as Record<number, number>)[level] ?? 0, ({ 15: 2, 20: 2, 25: 2, 30: 3 } as Record<number, number>)[level] ?? 0],
    '腰部': [({ 15: 3, 20: 3, 25: 3, 30: 4 } as Record<number, number>)[level] ?? 0, 1],
    '下装': [({ 15: 5, 20: 7, 25: 6, 30: 8 } as Record<number, number>)[level] ?? 0, ({ 15: 2, 20: 2, 25: 2, 30: 3 } as Record<number, number>)[level] ?? 0],
    '脚部': [({ 15: 3, 20: 4, 25: 4, 30: 5 } as Record<number, number>)[level] ?? 0, ({ 15: 1, 20: 1, 25: 1, 30: 2 } as Record<number, number>)[level] ?? 0]
  };
  const [mainQuantity, craftQuantity] = weights[category] ?? [0, 0];
  if (!mainQuantity || !craftQuantity) throw new Error('当前仅开放 Lv.15、20、25、30 的常规打造。');
...
const baseForgeEffect = (category: string, subtype: string, level: number, rarity: string): Record<string, number> => {
  ...
  if (category === '武器') {
    if (subtype === '盾牌') return { physicalDefense: weaponBase, magicDefense: weaponBase * .5 };
    ...
    if (subtype === '拳刃') return { physicalAttack: weaponBase };
...
const equipmentKind = (category: string, subtype?: string | null): '武器' | '防具' => category === '武器' && subtype !== '盾牌' ? '武器' : '防具';
...
export const legendaryTestForgeDraft = (category: '武器' | '防具', subtype: string, level: number) => {
...
export const craftForgeEquipment = async (qqUserId: string, _confirmed = false) => withTransaction(async connection => {
  ...
  const requirements = forgeRequirements(session.equipment_category, session.subtype, Number(session.target_level)); const requiredCodes = requirements.map(item => item.code);
  ...
  const missingRequirement = requirements.find(item => Number(requiredByCode.get(item.code)?.quantity ?? 0) < item.quantity); if (missingRequirement) throw new Error(`必备材料不足：${forgeMaterialName(missingRequirement.code)} 还需要 ${missingRequirement.quantity - Number(requiredByCode.get(missingRequirement.code)?.quantity ?? 0)} 份。`);
  const materials = requirements.map(requirement => ({ ...requiredByCode.get(requirement.code)!, selected_quantity: requirement.quantity })); const shortage = materials.find(item => Number(item.quantity) < Number(item.selected_quantity)); if (shortage) throw new Error(`材料不足：${shortage.name}。`);
  const success = 100;
  ...
  const mainMaterial = tierForgeMaterial(level); const name = forgeName(session.subtype, level, effect, mainMaterial); const code = `crafted_${characterId}_${Date.now()}_${Math.floor(Math.random() * 100000)}`; const quality = randomForgeQuality();
  const armorText = armorClassEffectText(session.subtype); const description = `由铁匠铺以${forgeMaterialName(mainMaterial)}为主体、按固定配方打造的 Lv.${level}${session.subtype}。随机词条的原始数值即为精炼至 100% 时的上限。${armorText}`;
  ...
  const proficiencyGain = forgeProficiencyGain(session.equipment_category, level, rarity); const progress = await addBlacksmithProficiency(connection, characterId, proficiencyGain);
...
type EpicRecipeMaterialView = { code: string; name: string; required: number; owned: number };
...
const loadEpicRecipeMaterials = async (connection: PoolConnection | Awaited<ReturnType<typeof getPool>>, characterId: number, recipe: EpicForgeRecipe, lock = false): Promise<EpicRecipeMaterialView[]> => {
  ...
  const [rows] = await connection.execute<(RowDataPacket & { code: string; name: string; quantity: number | null })[]>(`SELECT i.code,i.name,pi.quantity FROM item_definitions i
...
export const craftEpicForgeEquipment = async (qqUserId: string, blueprintCode: string) => withTransaction(async connection => {
  ...
  for (const material of materials) await connection.execute(`UPDATE player_inventory pi JOIN item_definitions i ON i.id=pi.item_id
