...
export const automatonRuleUnit=(id:number,battle:AutomatonBattleState):RuleUnit=>{
  ...
  const unit:RuleUnit={key:`automaton:${id}`,side:'member',name:`${battle.pet.name}（机巧）`,level:battle.pet.level,boss:false,
...
const heal=async(rules:CombatRules,p:AutomatonCombatant,target:RuleUnit,amount:number,normal=true)=>{
  ...
  if(normal&&actual>0&&equipped(p,'S014')){const memory=p.unit.state.memory;memory.healCount=Number(memory.healCount??0)+1;if(Number(memory.healCount)%3===0){const echo=Math.min(target.hpMax-target.hp,Math.floor(actual*.4));target.hp+=echo;if(echo)rules.log.push(`　&修护回响&【${target.name}】恢复 ${echo} HP。`);}}
  return actual>0;
};
...
const dot=(rules:CombatRules,p:AutomatonCombatant,target:RuleUnit,id:string,amount:number,element:string,duration:number)=>{
  ...
  rules.add(target,code,amount*(equipped(p,'N083')?1.1:1),duration,p.unit,true,JSON.stringify({element}));
};
export const installAutomatonRules=(rules:CombatRules,pets:AutomatonCombatant[])=>{
  ...
  rules.hooks.afterDamage=async(unit,damage,broken)=>{
    ...
    if(pet){
      ...
      if(broken&&equipped(pet,'S013')&&once(rules,pet,'brokenMana'))await rules.restore(unit,unit,0,unit.mpMax*.04,true);
    }
  ...
  rules.hooks.beforeHpDamage=async(unit,damage)=>{
    ...
    const pet=pets.find(p=>`member:${p.ownerId}`===unit.key&&!p.battle.exited&&p.unit.hp>0&&p.battle.pet.guard);
    ...
    rules.log.push(`　&挡刀&【${pet.unit.name}】替主人承受 ${transferred} 点伤害。`);
...
export const chooseAutomatonAction=(_rules:CombatRules,p:AutomatonCombatant,owner:RuleUnit)=>{
  ...
  if(p.battle.manual){const action=p.battle.manual;delete p.battle.manual;return ['防御','待机','普攻'].includes(action)||available.includes(action)?action:'普攻';}
  const selfCare=p.unit.hp/p.unit.hpMax*100<p.battle.pet.personality.selfCareHp,ownerCare=owner.hp/owner.hpMax*100<p.battle.pet.personality.ownerCareHp;
  const strategy=p.battle.pet.strategy;
  if(strategy==='节能'&&p.unit.mp/p.unit.mpMax<.25)return available.includes('N076')?'N076':'普攻';
  const scored=available.map((id,index)=>({id,score:(skillById.get(id)!.kind==='ULT'?40:0)+(defensive.includes(id)?(selfCare||ownerCare?70:strategy==='守护'?15:-20):strategy==='进攻'?30:20)+(skillById.get(id)!.domains.some(d=>p.battle.pet.personality.aligned.includes(d))?8:0)-index*.01}));
  return scored.sort((a,b)=>b.score-a.score)[0]?.id??(selfCare&&strategy==='守护'?'防御':'普攻');
};
export const actAutomaton=async(rules:CombatRules,p:AutomatonCombatant)=>{
  ...
  p.unit.name=`${owner.name}的${p.battle.pet.name}（机巧）`;
  ...
  if(skill&&!channel){paid=manaCost(p,id);if(p.unit.mp<paid)return;p.unit.mp-=paid;if(skill.kind==='ULT'){p.battle.sync=0;p.battle.ultimateUsed=true;}else p.battle.cooldowns[id]=Number(skill.cost.split('/')[1]);
    if(['S025','S026','S031'].includes(id)){p.battle.channel=id;rules.log.push(`➤【${p.unit.name}】预备【${skill.name}】，被打断不退还消耗。`);return;}}
  delete p.battle.channel;
  rules.log.push(`➤【${p.unit.name}】${skill?`使用【${skill.name}】`:'普通攻击'}`);
  ...
  if(definition){
    ...
    const oldCrit=self.crit;if(equipped(p,'S006'))self.crit*=1.15;
    for(const enemy of affected)for(let segment=0;segment<segments&&enemy.hp>0;segment++){
      ...
      const last=self.state.memory.lastAttackType,current=magic?'magic':'physical';if(equipped(p,'S009')&&last&&last!==current)bonus*=1.25;
      if(equipped(p,'N024')&&self.state.memory.element===element&&Number(self.state.memory.elementCount)>=2)bonus*=1.2;
      const didHit=await rules.strike(self,enemy,(id==='S031'&&enemy!==target?2.1:power)*100,element,magic,false,false,1,{skill:Boolean(skill),single:area===1,penetration:Math.min(50,penetration+(equipped(p,'S012')?20:0)),hitPenalty:-(id==='N016'?8:id==='N054'?15:id==='N057'?25:0)-(equipped(p,'S006')?10:0),finalMultiplier:bonus});
      ...
      if(id==='N006'&&didHit&&shieldBefore>0)await rules.drainShield(enemy,Math.max(0,shieldBefore-rules.shieldValue(enemy))*.4);
      if(didHit){
        const weaken=equipped(p,'N070')?1.1:1;
        if(id==='N005')rules.add(enemy,'armor_shatter',12*weaken,2,self,true);if(id==='N020')rules.add(enemy,'magic_shatter',12*weaken,2,self,true);
        if(['N015','N061','S020'].includes(id)&&(id!=='S020'||rules.random()<.5))rules.add(enemy,'slow',id==='N015'?8:id==='N061'?15:25,id==='S020'?2:1,self,true);
        if(id==='N019')rules.add(enemy,'alchemy_antiheal',20,2,self,true);
    ...
    if(hit&&equipped(p,'S007'))await heal(rules,p,self,Math.min(totalHpDamage*.1,self.hpMax*.08),false);
    if(hit&&target.hp<=0&&skill?.kind==='A'&&equipped(p,'N095')&&once(rules,p,'killMana'))await rules.restore(self,self,0,self.mpMax*.03,true);
    if(!hit&&skill?.kind==='A'&&equipped(p,'S010'))p.battle.cooldowns[id]=Math.max(0,p.battle.cooldowns[id]!-2);
  ...
  if(id==='N027')await shield(rules,p,support,.6*D,2);if(id==='N031'){await shield(rules,p,self,.8*D,2);rules.add(self,'automaton_mirror',1,2,self);}if(id==='N032')rules.add(support,'automaton_buffer',self.hpMax*.15,2,self);
  if(['N037','N041','N044','N045','N080','N092','S023','S027'].includes(id)){
    ...
    if(id!=='N092'||self.hp/self.hpMax<.5)for(const recipient of recipients)await heal(rules,p,recipient,factor*self.magic,skill?.kind==='A');else await shield(rules,p,owner,.65*D,2);
  }
  ...
  if(id==='N039'||id==='N044')await rules.dispel(self,support,true,1);
  if(id==='N040')await shield(rules,p,support,.55*self.magic,2);
  ...
  if(['N062','N063','N064','N067'].includes(id)&&rules.random()<.7){const values:Record<string,string[]>={N062:['accuracy_down'],N063:['evasion_down'],N064:['armor_shatter','magic_shatter'],N067:['attack_down','magic_down']};for(const code of values[id]!)rules.add(target,code,id==='N064'?8*(equipped(p,'N070')?1.1:1):id==='N067'?12:15,['N062','N063'].includes(id)?1:2,self,true);}
  if(id==='N076'||id==='N080')await rules.restore(self,self,0,self.mpMax*(id==='N076'?.08:.04),true);
  if(id==='N077'){dot(rules,p,target,id,.2*self.magic,'水',2);rules.add(target,'armor_shatter',8*(equipped(p,'N070')?1.1:1),2,self,true);}
  ...
  if(id==='N088'){await rules.dispel(self,self,true,1,e=>['accuracy_down','evasion_down'].includes(e.code));await shield(rules,p,self,.3*D,1);}
  ...
  if(['S022','S028'].includes(id))for(const recipient of [self,owner])await shield(rules,p,recipient,(id==='S022'?1.5:2)*D,3);
  ...
  if(id==='S030'){await rules.dispel(self,self,true,2,e=>hard.includes(e.code)||e.code==='silence');for(const code of Object.keys(p.battle.cooldowns))if(skillById.get(code)?.kind==='A')p.battle.cooldowns[code]=Math.max(0,p.battle.cooldowns[code]!-2);await shield(rules,p,self,1.2*D,2);}
  ...
  if(skill?.kind==='A'&&changed&&!['N042','N076','N080','N091'].includes(id)&&equipped(p,'S016')){self.state.memory.activeCount=Number(self.state.memory.activeCount??0)+1;if(Number(self.state.memory.activeCount)%3===0){p.battle.cooldowns[id]=0;self.mp=Math.min(self.mpMax,self.mp+Math.floor(paid*.3));}}
