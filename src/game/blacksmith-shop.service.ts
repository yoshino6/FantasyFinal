...
type StockRow = RowDataPacket & { id: number; codex_id: string; name: string; item_category: string; required_level: number; description: string; buy_price: number; owned_quantity: number };
...
export const blacksmithShopCatalog = async (qqUserId: string, page = 1, category = '全部', keyword = '') => {
  ...
  const [rows] = await pool.execute<StockRow[]>(`SELECT i.id,i.codex_id,i.name,i.item_category,i.required_level,i.description,si.buy_price,COUNT(ii.id) AS owned_quantity
    ...
    GROUP BY i.id,i.codex_id,i.name,i.item_category,i.required_level,i.description,si.buy_price
    ORDER BY i.required_level,i.item_category,i.id LIMIT ? OFFSET ?`, [character.id, ...values, PAGE_SIZE, (paging.page - 1) * PAGE_SIZE]);
  return { items: rows.map(row => ({ id: Number(row.id), codexId: row.codex_id, name: row.name, category: row.item_category, level: Number(row.required_level), description: row.description, price: Number(row.buy_price), ownedQuantity: Number(row.owned_quantity) })), ...paging, category: selected, keyword: keyword.trim(), copper: Number(character.copper_coins) };
};
...
export const blacksmithSellCatalog = async (qqUserId: string, page = 1, keyword = '') => {
  ...
  const [countRows] = await pool.execute<(RowDataPacket & { total: number })[]>(`SELECT COUNT(*) AS total FROM player_item_instances ii JOIN item_definitions i ON i.id=ii.item_id LEFT JOIN player_equipment pe ON pe.character_id=ii.character_id AND pe.instance_id=ii.id WHERE ii.character_id=? AND pe.instance_id IS NULL AND i.is_tradeable=1 AND i.name LIKE ?`, [character.id, term]);
  const paging = pageInfo(page, Number(countRows[0]?.total ?? 0));
  const [rows] = await pool.execute<SellRow[]>(`SELECT ii.id AS instance_id,i.name,i.item_category,i.required_level,ii.quality,COALESCE(si.sell_price,GREATEST(1,FLOOR(i.required_level*20))) AS sell_price
    ...
    WHERE ii.character_id=? AND pe.instance_id IS NULL AND i.is_tradeable=1 AND i.name LIKE ?
    ORDER BY ii.acquired_at DESC,ii.id DESC LIMIT ? OFFSET ?`, [character.id, term, PAGE_SIZE, (paging.page - 1) * PAGE_SIZE]);
  return { items: rows.map(row => ({ instanceId: Number(row.instance_id), name: row.name, category: row.item_category, level: Number(row.required_level), quality: Number(row.quality), price: Number(row.sell_price) })), ...paging, keyword: keyword.trim(), copper: Number(character.copper_coins) };
};
...
export const buyBlacksmithEquipment = async (qqUserId: string, itemId: number, quantity = 1) => withTransaction(async connection => {
  ...
  const [rows] = await connection.execute<(RowDataPacket & { id: number; name: string; buy_price: number })[]>(`SELECT i.id,i.name,si.buy_price FROM blacksmith_shop_items si JOIN item_definitions i ON i.id=si.item_id WHERE si.item_id=? AND si.is_active=1 AND i.item_type='equipment' FOR UPDATE`, [itemId]);
