...
/** 从当前级与下一级的实际公式取倍率比；展示对当前值的变化，组合结果在总体区展示。 */
export const specializationPerLevelLines = (tier?: string, levels: SkillSpecializations = {}): Record<Specialization, string[]> => {
  ...
  return Object.fromEntries((['overcharge', 'potent', 'instant', 'efficient'] as const).map(key => {
    ...
    if (key === 'overcharge') return [key, [`${heading}威力${change('powerFactor')}；蓝耗${change('manaFactor')}；冷却/吟唱基数${change('timeFactor')}。`]];
    if (key === 'potent') return [key, [
      `${heading}普通效果${change('effectFactor')}、控制概率系数${change('controlChanceFactor')}。`,
      `蓝耗${change('manaFactor')}；冷却/吟唱基数${change('timeFactor')}。`,
      `可成长时长${percentage(((1 + after.durationChange) / (1 + before.durationChange) - 1) * 100)}（先乘算、再取整）。`
    ]];
    if (key === 'instant') return [key, [`${heading}冷却/吟唱基数${change('timeFactor')}；威力、效果、控制概率系数${change('powerFactor')}。`]];
    return [key, [`${heading}蓝耗${change('manaFactor')}（乘算当前蓝耗）。`]];
