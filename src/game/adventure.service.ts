...
export const nearbyPoints = async (qqUserId: string) => {
  ...
  const points = [...monsters.map(item => point('怪物', item)), ...npcs.map(item => point('NPC', item)), ...objects.map(item => point('地标', item))]
    .sort((a, b) => a.distance - b.distance || a.name.localeCompare(b.name, 'zh-CN')).slice(0, 12);
  return { character, range, points };
};

const moveToPosition = async (connection: PoolConnection, qqUserId: string, x: number, y: number, restrictToPerception: boolean) => {
  const character = await characterFor(qqUserId);
  ...
  if (restrictToPerception && Math.max(Math.abs(x - Number(character.pos_x)), Math.abs(y - Number(character.pos_y))) > perceptionRange(Number(character.perception))) throw new Error('该位置超出你的感知范围。');
...
export const moveTo = async (qqUserId: string, x: number, y: number) => {
  ...
  return withTransaction(async connection => moveToPosition(connection, qqUserId, x, y, true));
};
