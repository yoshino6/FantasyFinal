...
export const adventurerCardImage = async (qqUserId: string, avatarUrl?: string) => {
  ...
  const [profiles] = await pool.execute<(RowDataPacket & CardProfile)[]>(`SELECT c.id,c.game_id AS gameId,c.name,c.gender,c.level,c.adventurer_rank AS rank,p.name AS profession,
