...
export const createCraftRequest = async (connection: PoolConnection, characterId: number, kind: string, snapshot: unknown, minutes = 2) => {
  ...
  await connection.execute('INSERT INTO player_craft_requests (token,character_id,kind,snapshot_json,expires_at) VALUES (?,?,?,?,DATE_ADD(NOW(),INTERVAL ? MINUTE))', [token, characterId, kind, JSON.stringify(snapshot), minutes]); return token;
};
export const craftRequestFor = async <T>(connection: PoolConnection, characterId: number, kind: string, token: string) => {
  ...
  return { snapshot: craftJson<T>(row.snapshot_json), result: row.state === 'complete' ? craftJson<Record<string, any>>(row.result_json) : null };
};
