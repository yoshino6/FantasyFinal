...
export const useInventoryItem=async(user:string,itemId:number,token:string)=>withTransaction(async connection=>{
  ...
  if(effect.experienceBonusPct||effect.partyDropBonusPct)result={...await applyBattleElixir(connection,id,effect),name:String(item.name)};
    ...
    else{await connection.execute('DELETE FROM player_food_buffs WHERE character_id=?',[id]);await connection.execute('INSERT INTO player_food_buffs (character_id,item_id,buff_json,expires_at) VALUES (?,?,?,DATE_ADD(NOW(),INTERVAL ? MINUTE))',[id,itemId,JSON.stringify(craftJson(meal.buff_json)),Number(meal.duration_minutes)]);await recalculateCharacterStats(connection,id);result={consumed:true,message:`已食用，餐食增益持续 ${meal.duration_minutes} 分钟。`,name:String(item.name)};}
  }else{
