...
export const bookshopHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try { await requireBookshop(event.current.UserId); await message.send({ format: bookshopFormat() }); } catch (error) { await message.send({ format: messageFormat('无法进入书屋', error instanceof Error ? error.message : '请稍后重试。') }); } };
...
export const bookshopChatHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try { await requireBookshop(event.current.UserId); const affinity = await addNpcAffinity(event.current.UserId, code, 'chat'); await message.send({ format: bookshopFormat(npcChatDialogue('bookshop', affinity.affinity), true) }); } catch (error) { await message.send({ format: messageFormat('无法闲聊', error instanceof Error ? error.message : '请稍后重试。') }); } };
