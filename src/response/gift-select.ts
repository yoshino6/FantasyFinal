...
export default async () => {
  ...
  try {
    ...
    await message.send({ format: Format.create().addMarkdown(Format.createMarkdown().addTitle('选择完成').addText(`\n\n你获得了【${character.giftName}】\n一个光柱将你笼罩，你缓缓升空。光芒散去时，你已经进入幽暗密林。\n\n地图尚未解锁，脚下的地面只会显示为未知之地。向任意方向移动，便会立即触发遭遇或奇遇。`)).addButtonGroup(Format.createButtonGroup().addRow().addButton('上', '/移动 上', { type: 'command', autoEnter: true, style: 'blue' }).addRow().addButton('左', '/移动 左', { type: 'command', autoEnter: true }).addButton('右', '/移动 右', { type: 'command', autoEnter: true }).addRow().addButton('下', '/移动 下', { type: 'command', autoEnter: true, style: 'blue' }).addButton('面板', '/面板', { type: 'command', autoEnter: true })) });
  } catch (error) {
