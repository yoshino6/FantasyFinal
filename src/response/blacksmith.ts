...
const requireBlacksmith = async (qqUserId: string) => {
  if ((await blacksmithQuest(qqUserId)).status === 'claimed') return;
...
export const secondaryProfessionHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try { const { secondaryProfessionCode } = await import('../game/alchemist.service'); if (await secondaryProfessionCode(event.current.UserId) === 'alchemist') { const { alchemistProfessionFormat } = await import('./alchemist'); await message.send({ format: await alchemistProfessionFormat(event.current.UserId) }); return; } const quest = await blacksmithQuest(event.current.UserId); const claimed = quest.status === 'claimed'; const progress = claimed ? await blacksmithProgress(event.current.UserId) : null; const markdown = Format.createMarkdown().addTitle(claimed ? '副职业·锻造师' : '副职业').addNewline().addNewline(); if (claimed && progress) markdown.addText(`等级：Lv.${progress.level}\n熟练度：${progress.proficiency}/${progress.required}\n${proficiencyBar(progress.proficiency, progress.required)}`).addNewline().addNewline().addBlockquote(`打造成功率+${progress.bonus}%`).addNewline().addBlockquote(`精炼大成功率+${progress.bonus}%`).addNewline().addBlockquote(`熔铸成功率+${progress.bonus}%`); else markdown.addText('尚未获得副职业。'); await message.send({ format: Format.create().addMarkdown(markdown).addButtonGroup(claimed ? professionButtons() : Format.createButtonGroup().addRow().addButton('前往铁匠铺', '/前往 -17 -123', { type: 'command', autoEnter: true, style: 'blue' })) }); } catch (error) { await message.send({ format: messageFormat('无法查看副职业', error instanceof Error ? error.message : '请稍后重试。') }); } };
