...
export default async () => {
  ...
  try {
    ...
    const instances = result.instances.map(item => `[${item.item_category}·${item.name}] #${item.id}\n品质 ${Number(item.quality).toFixed(2)}%｜耐久 ${item.durability}/${item.durability_max}\n${item.description}`).join('\n\n');
    const stacked = result.stacked.map(item => `[${item.item_category}·${item.name}] ×${item.quantity}\n${item.description}`).join('\n\n');
    await message.send({ format: Format.create().addMarkdown(Format.createMarkdown().addTitle('背包').addText(`最近获得：${recent}\n\n${instances || stacked || '该分类暂无物品。'}`)).addButtonGroup(Format.createButtonGroup().addRow()
