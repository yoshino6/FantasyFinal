...
import { addMailAttachment, addMailRecipientByName, addMailRecipientByQq, discardMailEdit, getMailEdit, openMailEdit, previewMailEdit, removeMailAttachment, removeMailRecipient, sendMailEdit, stashMailEdit, updateMailAttachmentQuantity, updateMailContent, updateMailTitle, type MailEdit } from '../game/admin-mail-edit.service';
...
export const mailEditFormat = (edit: MailEdit) => {
  ...
  markdown.addNewline().addText('标题：').addButton('[编辑]', textButton('编辑标题', '管理员邮件 编辑标题 ')).addText(' ').addButton('[清空]', textButton('清空标题', '管理员邮件 清空标题')).addNewline().addBlockquote(edit.title || '无').addNewline().addNewline()
