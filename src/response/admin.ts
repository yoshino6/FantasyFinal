...
import { addMailAttachment, addMailRecipientByName, addMailRecipientByQq, discardMailEdit, getMailEdit, openMailEdit, previewMailEdit, removeMailAttachment, removeMailRecipient, sendMailEdit, stashMailEdit, updateMailContent, type MailEdit } from '../game/admin-mail-edit.service';
import { grantAdministrator, loginAsOwner, permissionFor, permissionList, requireOwner, revokeAdministrator, type PermissionRole } from '../game/permission.service';
...
export const mailEditFormat = (edit: MailEdit) => {
  ...
  markdown.addNewline().addText('内容：').addButton('[编辑]', textButton('编辑内容', '管理员邮件 编辑内容 ')).addNewline().addBlockquote(edit.content || '尚未编辑内容。').addNewline().addNewline()
...
export const confirmEditHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try { const result = await sendMailEdit(event.current.UserId); await message.send({ format: messageFormat('邮件发放成功', `已向 ${result.recipientCount} 名玩家发放邮件\n附件：${result.attachments.map(attachment => `【${attachment.name}】× ${attachment.quantity}`).join('、')}`) }); } catch (error) { await message.send({ format: messageFormat('邮件发放失败', error instanceof Error ? error.message : '请稍后重试。') }); } };
