...
export const guildShopFormat = (text = '柜台后的商人正在归类药剂、兽材与卷轴。木架上陈列着实用的旅途用品，价签清晰，空气里混着羊皮纸与草药的淡香。') => {
  const markdown = Format.createMarkdown().addTitle('冒险者公会·商店').addNewline().addNewline().addBlockquote(text);
...
const buyFormat = async (qqUserId: string, page = 1, keyword = '') => {
  ...
  shop.items.forEach((item, index) => {
    const label = item.category === '地图' ? `【${item.name}】` : `【${item.category}】${item.name}`;
    const ownership = item.ownedQuantity > 0 ? `已拥有${item.ownedQuantity}` : '未拥有';
    markdown.addText(`${sequence[index]}${label} `).addButton('[购买]', { data: `/购买商品 ${item.id} `, autoEnter: false }).addNewline().addBlockquote(`价格：铜币×${item.price}｜${ownership}`).addNewline().addBlockquote(`简介：${item.description}`).addNewline().addNewline();
  });
...
export const shopPurchaseHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try { await requireGuildShop(event.current.UserId); const requested = String(route.param('quantity') ?? '').trim(); const result = await buyShopItem(event.current.UserId, Number(route.param('id')), requested ? Number(requested) : 1); await message.send({ format: messageFormat('购买成功', `获得【${result.name}】×${result.quantity}\n消耗铜币×${result.price}`) }); await message.send({ format: await buyFormat(event.current.UserId) }); } catch (error) { await message.send({ format: messageFormat('购买失败', error instanceof Error ? error.message : '请稍后重试。') }); } };
...
export const shopSellHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try { await requireGuildShop(event.current.UserId); const requested = String(route.param('quantity') ?? '').trim(); const result = await sellShopItem(event.current.UserId, Number(route.param('id')), requested ? Number(requested) : 1); await message.send({ format: messageFormat('出售成功', `出售【${result.name}】×${result.quantity}\n获得铜币×${result.price}`) }); await message.send({ format: await sellFormat(event.current.UserId) }); } catch (error) { await message.send({ format: messageFormat('出售失败', error instanceof Error ? error.message : '请稍后重试。') }); } };
export const shopChatHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try { await requireGuildShop(event.current.UserId); await message.send({ format: guildShopFormat('商人抬起头，笑着擦了擦柜台。“东西会用完，冒险的见闻却不会。等你从密林回来，记得和我说说那里又有什么新鲜事。”') }); } catch (error) { await message.send({ format: messageFormat('无法闲聊', error instanceof Error ? error.message : '请稍后重试。') }); } };
