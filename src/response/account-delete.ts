...
export default async () => {
  ...
  try {
    ...
    .addBlockquote('此操作无法恢复。确认后仍可发送“注册”重新开始。').addNewline().addNewline()
    .addText(`验证码：${request.code}`).addNewline()
    .addBlockquote(`请在 ${request.expiresMinutes} 分钟内发送“确认注销 ${request.code}”完成注销。`);
    const buttons = Format.createButtonGroup().addRow()
    .addButton('取消', '/菜单', { type: 'command', autoEnter: true })
    .addButton('确认注销', `/确认注销 ${request.code}`, { type: 'command', autoEnter: false, style: 'blue' });
