...
export const ambushHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try { await chooseTarget(event.current.UserId, Number(route.param('id')), true); const battle = await battleStatus(event.current.UserId); await message.send({ format: battleFormat('偷袭成功', '偷袭成功！全队第一回合伤害提高 50%。', battle) }); } catch (error) { await fail(message, error, '无法发动偷袭'); } };
