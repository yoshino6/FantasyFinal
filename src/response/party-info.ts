...
const partyFormat = async (qqUserId: string) => {
  ...
  if (!party) return Format.create().addMarkdown(Format.createMarkdown().addTitle('我的队伍').addNewline().addNewline().addText('你不在任何队伍中')).addButtonGroup(buttons.addRow().addButton('创建队伍', '/组队 创建', { type: 'command', autoEnter: true, style: 'blue' }).addButton('队伍列表', '/队伍列表', { type: 'command', autoEnter: true }));
  ...
  buttons.addRow().addButton('退出队伍', '/退出队伍', { type: 'command', autoEnter: true }).addButton('加入队伍', '/队伍列表', { type: 'command', autoEnter: true });
...
export const leaveHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try { await leaveParty(event.current.UserId); await message.send({ format: await partyFormat(event.current.UserId) }); } catch (error) { await message.send({ format: messageFormat('无法退出队伍', error instanceof Error ? error.message : '请稍后重试。') }); } };
