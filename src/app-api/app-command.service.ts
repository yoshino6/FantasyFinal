...
import { formatToAppMessage, plainAppMessage, type AppMessage } from './app-format';
...
const inventoryText = async (qqUserId: string): Promise<AppMessage> => {
...
const exploreText = async (qqUserId: string): Promise<AppMessage> => {
...
const moveText = async (qqUserId: string, direction: string): Promise<AppMessage> => {
  const result = await move(qqUserId, direction);
  const text = result.text ?? '你移动了一步。';
  return plainAppMessage(text, [], '我跟着你走。');
};

const mapTravelText = async (qqUserId: string, mapCode: string): Promise<AppMessage> => {
...
const statusText = async (qqUserId: string): Promise<AppMessage> => {
...
const restText = async (qqUserId: string, command: string): Promise<AppMessage> => {
...
const battleText = async (qqUserId: string, args: string[]): Promise<AppMessage> => {
...
const equipmentText = async (qqUserId: string): Promise<AppMessage> => {
  const items = await equipment(qqUserId);
  if (!items.length) return plainAppMessage('你还没有穿戴任何装备。', [{ label: '背包', command: '/背包 装备' }], '空荡荡的，去背包看看吧。');
...
const equipmentDetailText = async (qqUserId: string, id: number): Promise<AppMessage> => {
...
const equipText = async (qqUserId: string, slot: string, id: number): Promise<AppMessage> => {
  const item = await equip(qqUserId, slot, id);
  return plainAppMessage(`已装备【${item.name}】。`, [{ label: '装备', command: '/装备' }], '穿好了。');
};

const unequipText = async (qqUserId: string, slot: string): Promise<AppMessage> => {
  const item = await unequip(qqUserId, slot);
  return plainAppMessage(`已卸下【${item.name}】。`, [{ label: '装备', command: '/装备' }], '收好了。');
};

const skillsText = async (qqUserId: string, view: string, id?: number): Promise<AppMessage> => {
  if (id) {
...
const learnSkillText = async (qqUserId: string, id: number): Promise<AppMessage> => {
...
const upgradeSkillText = async (qqUserId: string, id: number): Promise<AppMessage> => {
...
const toggleShortcutText = async (qqUserId: string, id: number): Promise<AppMessage> => {
...
const partyText = async (qqUserId: string): Promise<AppMessage> => {
...
const mailText = async (qqUserId: string, page = 1): Promise<AppMessage> => {
  const { playerMails } = await import('../game/mail.service');
...
const mailDetailText = async (qqUserId: string, id: number): Promise<AppMessage> => {
  const { mailDetail } = await import('../game/mail.service');
...
const mailClaimText = async (qqUserId: string, id?: number): Promise<AppMessage> => {
  const { claimMail, claimAllMails } = await import('../game/mail.service');
  if (!id) {
...
const useItemText = async (qqUserId: string, id: number): Promise<AppMessage> => {
  const { randomUUID } = await import('node:crypto');
  const { useInventoryItem } = await import('../game/item-use.service');
...
const giftText = async (qqUserId: string, code: string): Promise<AppMessage> => {
  const { chooseGift } = await import('../game/character.service');
...
const registerText = async (qqUserId: string, command: string): Promise<AppMessage> => {
  const args = command.replace(/^注册\s*/, '').trim().split(/\s+/).filter(Boolean);
  if (args[0] === '继续') {
    // 剧情按钮命令形如「注册 继续 story」：第一个参数是「继续」，
    // 第二个是按钮生成时的预期 stage，用于跳过过期/重复点击。
    const { continueRegistration } = await import('../game/character.service');
    const next = await continueRegistration(qqUserId, args[1] ?? undefined);
    if (next === 'completed') return plainAppMessage('你已经完成注册，可以开始冒险了。', [{ label: '角色', command: '/角色' }], '欢迎回来！');
    return formatToAppMessage(await registrationScene(next, qqUserId), '我在这里等你。');
  }
  const { beginRegistration } = await import('../game/character.service');
  const result = await beginRegistration(qqUserId);
  if (result.alreadyRegistered) return plainAppMessage('你已经创建过角色，直接继续冒险吧。', [{ label: '角色', command: '/角色' }], '老伙伴了。');
  return formatToAppMessage(await registrationScene(result.stage!, qqUserId), '欢迎来到猫拉瑞亚。');
};

const unsupportedText = (command: string): AppMessage =>
  plainAppMessage(`命令“${command}”暂未接入 App 网关。当前支持：角色、背包、探索、移动、地图前往、状态、休息、战斗、注册。`, [
    { label: '角色', command: '/角色' },
    { label: '背包', command: '/背包' },
    { label: '状态', command: '/状态' }
  ], '这条命令我还没学会。');

export const executeAppCommand = async (input: AppCommandInput): Promise<AppMessage> => {
...
export const appQuickPanel = (): AppMessage => plainAppMessage('桌宠待命中。', [
  { label: '角色', command: '/角色' },
  { label: '背包', command: '/背包' },
  { label: '状态', command: '/状态' },
  { label: '探索', command: '/探索' }
