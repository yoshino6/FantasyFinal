...
export const registerAppApiRoutes = (router: koaRouter) => {
  router.get('/app-api/v1/health', async (ctx: Context) => {
    ctx.type = 'application/json';
    ctx.body = { ok: true, service: 'fantasy-final-game-core', time: new Date().toISOString() };
  });

  router.post('/app-api/v1/register', async (ctx: Context) => {
    try {
      if (!secureRequest(ctx)) {
        apiError(ctx, 404, '未找到接口。');
        return;
  ...
  router.post('/app-api/v1/bind', async (ctx: Context) => {
    try {
      if (!secureRequest(ctx)) {
        apiError(ctx, 404, '未找到接口。');
        return;
  ...
  router.post('/app-api/v1/command', async (ctx: Context) => {
  ...
  router.get('/app-api/v1/panel', async (ctx: Context) => {
    const session = await requireSession(ctx);
    if (!session) return;
    ctx.type = 'application/json';
