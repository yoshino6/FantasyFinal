...
| 2000～2999 | 同频 | 彼此已经形成稳定的配合节奏，可解锁更丰富的互动文案 |
| 3000～4999 | 知己 | 熟悉彼此的习惯与选择，好友资料显示专属称号 |
| 5000～7999 | 灵犀 | 交流与赠礼剧情进入高阶文案池 |
| 8000～11999 | 星伴 | 星誓关系可展示长期同行称号 |
| 12000～19999 | 共鸣 | 可作为纪念剧情与后续双人内容的高阶前置 |
| 20000 及以上 | 星誓共鸣 | 关系阶段的最高展示级别，仍不提供直接战斗数值加成 |

阶段计算约定：好感值继续累加，不设置低于当前值的回退；达到 `20000` 后阶段显示为“星誓共鸣”，但数据库仍保留实际好感值。星誓最低要求仍为 `500`，不会因为新增高阶阶段而提高。服务层 `relationshipStage(affinity)` 必须返回当前阶段、下一阶段门槛和距离下一阶段所需好感；最高阶段的下一阶段门槛返回 `null`。
...
CREATE TABLE IF NOT EXISTS player_relationships (
  ...
  status ENUM('friend','oath','ended') NOT NULL DEFAULT 'friend',
...
CREATE TABLE IF NOT EXISTS player_oaths (
  ...
  status ENUM('active','release_pending','released') NOT NULL DEFAULT 'active',
...
`player_oaths` 是关系仪式和解除状态的唯一来源；`player_relationships.status='oath'` 是好友面板的快速展示状态，但写入必须和 `player_oaths` 在同一事务完成。查询任意一名玩家是否已有 active 星誓时，必须按 `character_low_id` 和 `character_high_id` 两个方向查询。

### 4.5 祈福与限时增益

每天一次的限制必须落库，不能只依赖消息上下文或内存缓存。新增两张表：

```sql
CREATE TABLE IF NOT EXISTS player_daily_blessings (
  character_id BIGINT UNSIGNED NOT NULL,
  business_date DATE NOT NULL,
  blessed_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  reward_bouquet_quantity INT UNSIGNED NOT NULL DEFAULT 0,
  reward_fruit_quantity INT UNSIGNED NOT NULL DEFAULT 0,
  PRIMARY KEY (character_id, business_date),
  CONSTRAINT fk_daily_blessing_character FOREIGN KEY (character_id)
    REFERENCES characters(id) ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE IF NOT EXISTS player_timed_buffs (
  character_id BIGINT UNSIGNED NOT NULL,
  buff_code VARCHAR(64) NOT NULL,
  expires_at DATETIME NOT NULL,
  experience_multiplier DECIMAL(6,3) NOT NULL DEFAULT 1.000,
  all_core_attributes_multiplier DECIMAL(6,3) NOT NULL DEFAULT 1.000,
  source VARCHAR(64) NOT NULL DEFAULT 'church_blessing',
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (character_id, buff_code),
  KEY idx_timed_buff_expiry (expires_at),
  CONSTRAINT fk_timed_buff_character FOREIGN KEY (character_id)
    REFERENCES characters(id) ON DELETE CASCADE
) ENGINE=InnoDB;
```

固定配置：`buff_code='church_blessing'`、`experience_multiplier=1.250`、`all_core_attributes_multiplier=1.050`、`expires_at=当前时间+1小时`。属性加成仅在读取当前玩家战斗/成长结算时应用，不写入基础属性；队友、好友和星誓对象不会共享这份增益。
...
social.blessing.claimed
social.oath.requested
social.oath.completed
social.oath.release_requested
social.oath.released
```
...
```text
src/game/social.constants.ts       # 阶段、门槛、每日上限、礼物规则、祈福常量
src/game/social.service.ts         # 好友、好感、赠礼、星誓、解除星誓事务
src/game/blessing.service.ts       # 每日祈福、个人限时增益、教堂奖励事务
src/game/social-message.ts         # 好友/星誓面板与星誓仪式剧情 Format 构建器
src/game/blessing-message.ts      # 祈福结果与修女奖励剧情文案
src/response/social.ts             # 好友、申请、好感、星誓命令响应
src/response/blessing.ts           # `/祈福` 响应
```
...
claimChurchBlessing(qqUserId, eventContext)

oathStatus(qqUserId)
requestOath(qqUserId, targetGameId, eventContext)
oathRequests(qqUserId)
acceptOath(qqUserId, requestId, eventContext)
rejectOath(qqUserId, requestId)
requestOathRelease(qqUserId)
acceptOathRelease(qqUserId, requestId)
...
`claimChurchBlessing` 事务与读取流程：

1. 通过 `useEvent/useRoute/useMessage` 获取当前玩家和事件上下文，校验玩家已注册且当前位置是 `saint_church`；不接受远程调用、家园内调用或战斗中调用。
2. 按 `Asia/Shanghai` 计算 `business_date`，锁定 `(character_id, business_date)` 祈福记录；已有记录直接返回“今日已祈福”，不重复发放奖励。
3. 插入当天记录，写入 `player_timed_buffs` 的 `church_blessing`：经验倍率 `1.250`、全六项核心属性倍率 `1.050`、过期时间为当前时间后 1 小时。
4. 查询当前玩家是否存在 `player_oaths.status='active'`。若存在，向背包写入 `心意花束 ×1`，再用安全随机数以 10% 概率写入 `共鸣果实 ×1`；非 active 星誓玩家不发放这两件道具。
5. 将实际奖励数量写入祈福记录，并记录 `social.blessing.claimed` 事件；所有步骤成功后一次提交。
6. 提交后输出对应剧情分支：普通祈福、获得花束、花束与果实同时获得。奖励写入成功但消息发送失败时，不回滚已提交的祈福结果。

经验和属性结算读取 `player_timed_buffs` 时只接受 `expires_at > 当前时间` 的记录；过期记录可懒清理。该表的属性倍率只作用于当前玩家，不向队友、好友或星誓对象传播。
...
3. 锁定规范化关系，确认 `status IN ('friend','oath')`。
...
发起成功只代表“提出星誓仪式邀请”，不能把关系直接改为 oath，也不能先扣环。
...
8. 将 `player_relationships.status` 更新为 `oath`，申请更新为 accepted。
...
1. `/解除星誓` 只创建 `release_pending` 申请，不立即解除。
2. 另一方通过 `/同意解除星誓` 完成状态更新。
3. 解除时 `player_oaths.status='released'`、`player_relationships.status='friend'`，保留 affinity。
...
| `/祈福` | 无 | 在圣恩教堂每日一次，为自己获得 1 小时个人增益与星誓奖励 |
...
appGroup.use({
  path: '祈福',
  schema: { usage: '/祈福' }
}, () => import('./response/blessing').then(module => ({ default: module.churchBlessingHandler })));
```
...
- “星誓”按钮改为 `/星誓`；
- “祈福”按钮改为 `/祈福`；
- 教堂不显示商品列表，不注册任何购买类命令；
- `/星誓` 不在教堂外直接发起申请，只能查看已有状态并提示“请前往圣恩教堂”；
- `/祈福` 必须在圣恩教堂执行，每名玩家每天一次；
...
4. 双方先通过世界树任务、限时活动或剧情奖励取得各自的星誓之环。
5. 双方前往百纳镇·圣恩教堂，各自点击“祈福”；当天首次祈福获得 1 小时个人增益，已有 active 星誓的玩家额外获得花束，并有小概率获得果实。
6. A 点击“发起星誓”，B 的 `/好友` 和 `/星誓申请` 出现申请；若支持主动私聊通知，则同时收到通知。
7. B 也来到教堂，点击“接受星誓”。系统在一个事务中同时扣除两枚戒指、写入星誓关系并完成剧情。
8. 星誓仪式结束后，双方 `/星誓` 都能看到对方、星誓日期、好感和关系称号；同队 `/队伍` 时显示“星誓同行”。
...
`social-message.ts` 中按 `friend / oath` 和时间段维护文案池，不要把长文案散落在 handler：
...
你在彩窗投下的星光前合拢双手。
修女伊芙琳没有询问你的愿望，只将一缕温柔的神辉引向你的旅装。

“愿你今天走过的路，都留下可以平安归来的光。”
...
已有 active 星誓的玩家祈福并获得花束：

```text
# 圣恩教堂·同行祈福

神辉掠过你的指尖，祭台旁的白花忽然轻轻摇曳。
修女伊芙琳取下一束缀着世界树嫩叶的花束，递到你手中。

“有人在远方与你并肩，所以这份光也有了可以抵达的方向。”

获得：心意花束 ×1
祈福增益：经验获取 +25%、全六项核心属性 +5%，持续 1 小时
以上增益仅对你自己生效。
```
...
花束落入掌心的那一刻，教堂穹顶映出一圈细小的树影。
一枚带着星点露光的果实从枝影间坠下，被修女稳稳接住。
...
教堂闲聊继续使用现有 NPC 好感 `saint_church`；玩家好感和修女好感必须在显示层明确区分。祈福奖励由玩家是否拥有 active 星誓决定，不读取修女好感数值。
...
1. 在 `bootstrap.ts/schemaStatements` 加入六张表：好友申请、玩家关系、星誓申请、星誓、每日祈福、玩家限时增益；不加入教堂商店表。
...
4. 配置三件物品的来源与效果：花束、果实只能由祈福奖励发放；星誓之环由任务、活动或剧情奖励发放；不写入任何商店报价表。
...
8. 增加 `social-message.ts`、`response/social.ts`、`blessing.service.ts`、`blessing-message.ts` 和 `response/blessing.ts`。
...
11. 修改 `church.ts` 中当前空的“星誓”“祈福”按钮，并保留修女 NPC 闲聊逻辑。
12. 修改 `help.ts` 的“队伍与社交”区，加入好友、星誓、祈福说明。
...
- 好感达到每个阶段边界时准确切换称号；`20000` 及以上显示“星誓共鸣”，并保留实际好感值。
- 好感为 `499` 时仍显示“交心”，达到 `500` 后显示“莫逆”；阶段展示与星誓资格判断使用同一份数值。
...
- 教堂面板只出现 `[祈福]`，点击后调用 `/祈福`，不出现商品列表或购买入口。
- 同一玩家同一上海业务日只能成功祈福 1 次；重复点击不会重复发放增益或道具。
- 首次成功祈福后，当前玩家获得经验获取 `+25%` 与全六项核心属性 `+5%`，持续 1 小时。
- 增益只作用于本人；队友、好友和星誓对象的经验与属性不发生变化。
- active 星誓玩家必得心意花束；共鸣果实掉落概率为 10%，实际数量与事件记录一致。
- 花束和果实只能由祈福获得；普通玩家祈福不获得，非教堂位置、战斗中或未注册玩家不能领取。
- 跨上海时区午夜后可再次祈福；跨午夜不提前重置限时增益，增益仍按自身过期时间结束。
...
阶段边界：49/50、199/200、499/500、999/1000、1999/2000、2999/3000、4999/5000、7999/8000、11999/12000、19999/20000、20000+
赠礼：花束、果实、普通物品、数量不足、目标离开、并发赠礼、背包归零
祈福：普通玩家、active 星誓玩家、每日重复、跨日重置、非教堂位置、战斗中、增益过期、果实概率与奖励记录
