...
try{const [rows]=await db.query("SELECT a.achievement_id,a.name_snapshot,a.created_at,d.bot_id,d.group_id,d.status,d.attempts,d.updated_at FROM achievement_announcements a JOIN achievement_deliveries d ON d.achievement_id=a.achievement_id ORDER BY a.created_at DESC LIMIT 80");writeFileSync('.data/achievement-design-20260909/announcement-audit.json',JSON.stringify(rows,null,2));console.log(JSON.stringify(rows));}finally{await db.end();}
