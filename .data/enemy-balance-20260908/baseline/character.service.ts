...
const finalAttributes = (row: Record<string, unknown>) => Object.fromEntries(attributes.map(key => [key, Number(row[key] ?? 0) + Number(row[`${key}_growth`] ?? row[`${key}Growth`] ?? 0) * (row.npc_code ? Math.max(0, Number(row.level ?? 1) - 1) : playerGrowthShares(Number(row.level ?? 1)))])) as Allocation;
...
export const recalculateCharacterStats = async (connection: Pool | PoolConnection, characterId: number) => {
  ...
  const migrating = Number(character.stat_formula_version ?? 2) < STAT_BALANCE_VERSION;
  ...
  await connection.execute('UPDATE characters SET stat_formula_version=3,hp_max=?,mp_max=?,current_hp=?,current_mp=?,physical_attack=?,magic_attack=?,physical_defense=?,magic_defense=?,accuracy=?,evasion=?,crit_rate_bp=?,crit_damage_bp=?,crit_resist_bp=?,crit_damage_reduction_bp=?,tenacity=?,tenacity_pierce=?,speed=?,element_mastery_json=?,element_resistance_json=? WHERE id=?', [stats.hpMax, stats.mpMax, hp, mp, stats.physicalAttack, stats.magicAttack, stats.physicalDefense, stats.magicDefense, stats.accuracy, stats.evasion, stats.critRateBp, stats.critDamageBp, stats.critResistBp, stats.critDamageReductionBp, stats.tenacity, stats.tenacityPierce, stats.speed, JSON.stringify(elemental.mastery), JSON.stringify(elemental.resistance), characterId]);
};
...
export const chooseGift = async (qqUserId: string, giftCode: string, nickname?: string): Promise<CharacterView | null> => withTransaction(async connection => {
  giftCode = /^G\d{2}$/i.test(giftCode) ? `divine_${giftCode.toLowerCase()}` : giftCode;
  const gift = divineSkillDefinitions.find(skill => skill.code === giftCode);
  if (!gift) throw new Error('神器已经散布世界各地。请从当前神技目录选择一项恩赐。');
...
export const getCharacter = async (qqUserId: string): Promise<CharacterView | null> => {
  ...
  return {
    ...row,
    giftName: divineSkillDefinitions.find(skill => skill.code === row.giftName)?.name ?? row.giftName,
