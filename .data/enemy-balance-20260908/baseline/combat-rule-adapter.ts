...
export const createCombatRules = async (connection: PoolConnection, sessionId: string, turn: number, members: CombatRow[], targets: CombatRow[],
  ...
  const make = (row: CombatRow, kind: 'member' | 'target'): RuleUnit => {
    ...
    return {
      ...
      armorSet: kind === 'member' ? armorSets.get(Number(row.id)) : (Array.isArray(traits) ? traits.find(trait => trait.code === 'advanced_mentor_build')?.build?.armorSet : undefined),
  ...
  const takeDamage = async (kind: 'member' | 'target', id: number, incoming: number, areaHit = false) => {
    const unit = get(kind, id); const ownBefore = rule.value(unit, 'shield');
    const result = await rule.takeHit(unit, incoming, 1, areaHit); incoming = result.damage;
