...
import { strikeCorrections } from '../../src/game/combat-math';
import { armorSetFromRows, type ArmorSet } from '../../src/game/armor-set';
import { grantInventory } from '../../src/game/inventory-binding';
import { runNegotiation, readNegotiationReplay, assertNoNegotiation, assertMonsterNotNegotiating, closeNegotiationSession, NegotiationCombatError, type NegotiationCommand, type NegotiationDrop, type NegotiationResult } from '../../src/game/negotiation.service';
import { hiddenAttributesFor } from '../../src/game/hidden-attributes.service';
import { negotiationReferenceValue } from '../../src/game/negotiation-item-policy';
import { teamLuckMultiplier, weightedRecipient, scaledDropEntries, moodDropMultiplier, negotiationVersion } from '../../src/game/negotiation-rules';
import { executeHiddenCombat, HiddenBattleError, hiddenEndTurn, hiddenReorder, hiddenDeviceSnapshot, hiddenNativeDevice, hiddenAfterHit } from '../../src/game/hidden-combat';
import { hiddenBattleContext, submitHiddenDraft, initializeHiddenBattleUnits, type HiddenTicket } from '../../src/game/hidden-battle.service';
import { isHiddenSkill } from '../../src/game/hidden-profession.config';
import { hiddenResourceView, hiddenResourceShortage, gainHiddenResource, type HiddenChoice } from '../../src/game/hidden-combat-state';
...
import { validWorldSitePoint, type WorldArea } from '../../src/game/world-site-geometry';
import { installAutomatonRules, actAutomaton } from '../../src/game/automaton-combat';
import { loadCombatAutomatons, saveCombatAutomatons, finishCombatAutomatons, appendAutomatonBattleQuotes, applyEnemySkillToAutomaton } from '../../src/game/automaton-combat.service';
import { useAlchemyCombat, alchemyEndTurn, consumeAlchemyChant } from '../../src/game/alchemy-combat';
import { randomUUID } from 'node:crypto';
import { skillSpecialization, manaTransferCost, specializationUpgradeCost, specializationMaximum, specializationOptions, specializeEffectValue, specializeEffectDuration, specializeControlChance, type SkillSpecializations, type SkillSpecializationResult } from '../../src/game/skill-specialization';
import { canDispelCombatEffect, nativeCleanseLimit } from '../../src/game/combat-dispel-policy';
import { canSpecializePassive, passiveSpecializationFactor, specializedPassiveValue } from '../../src/game/passive-specialization';
import { finishNpcSparring } from '../../src/game/npc-sparring.service';
import { createCombatRules } from '../../src/game/combat-rule-adapter';
import { maskRuleBattleLog, readRuleState, ruleManaCost, ruleStatusSummary, displayedRuleName, visibleResidentBuff, type CombatRules } from '../../src/game/combat-rule-registry';
import { residentSkillByCode } from '../../src/game/resident-skill.config';
import type { Pool, PoolConnection, RowDataPacket } from 'mysql2/promise';
import { getPool, withTransaction } from '../../src/database/pool';
import { sortMapMarkers } from '../../src/game/map-marker.service';
import { calculateDerivedStats, experienceRequiredForLevel, forgedEquipmentBase, realmEnergyDissipationText, realmLevelCap, virtualEquipmentStats, type VirtualEquipmentTier } from '../../src/game/constants';
import { effectiveCharacterAttributes, recalculateCharacterStats, refreshCharacterStamina } from '../../src/game/character.service';
import { encumbrance } from '../../src/game/encumbrance';
import { advanceBountyProgress, refreshBounties } from '../../src/game/bounty.service';
import { attributes, type Allocation } from '../../src/game/types';
import { recordSkillPointChange } from '../../src/game/skill-point-ledger.service';
import { awardOmniscientProficiency, recordOmniscientObservation } from '../../src/game/omniscient.service';
import {applyBattleElixir} from '../../src/game/battle-elixir.service';
import {combatItemEffect} from '../../src/game/item-use-policy';
import { closeDungeonForBossSpawns, dungeonArrivalEvent, dungeonCellAt, dungeonEntranceAt, dungeonPlayersInRange } from '../../src/game/dungeon.service';
import { completeDungeonSecretForLeader, discoverDungeonEntrance } from '../../src/game/dungeon-quest.service';
import { completeEvolutionQuest, completeGoblinKingQuest, goblinKingArrival } from '../../src/game/main-quest.service';
import { collectCityDebts, recordWarrantSighting, settleCityPursuitDefeat } from '../../src/game/pvp.service';
import { worldSurfaceMonsters } from '../../src/config/world-surface';
import { detentionMessage } from '../../src/game/time-format';
import { homeRestExperiencePerMinute, homeRestRecoveryBonus, isInHome } from '../../src/game/home.service';
import { bossControlChanceMultiplier, directDamageVariance, resolveStrike, tenacityContest } from '../../src/game/combat-math';
import { hasCompatibleSkillWeapon, skillWeaponRequirementMessage } from '../../src/game/skill-weapon.service';
import { resolvedMonsterMaterialDropCode } from '../../src/game/monster-crafting-material.service';
import { secondaryProfessionBonus } from '../../src/game/secondary-profession';
import { globalCopperMultiplier, globalDropMultiplier, globalExperienceMultiplier } from '../../src/game/global-management.service';
import { advanceEvolutionObservationBattle, advanceEvolutionObservationMining, evolutionStatBonuses, repairEvolutionProgress, symbiosisTraits, type SymbiosisTraitCode } from '../../src/game/evolution.service';
import { completeAdvancedProfessionTrial, recordAdvancedProfessionKills } from '../../src/game/advanced-profession.service';
import { spiritDefinitionBySkill, type SpiritDefinition } from '../../src/game/spirit-summoner.config';
import { inheritancePassivesFor } from '../../src/game/advanced-passive.service';
import { advancedResourceForProfession, advancedResourceRequirementForSkill } from '../../src/game/advanced-resource.config';
import { registeredAdvancedProfessionByCode as advancedProfessionByCode, isAdvancedProfessionSkillCode, isCachedAdvancedPassiveEffect } from '../../src/game/advanced-profession.config';
import { epicLoadoutFor, hasEpicWeaponEffect, type EpicLoadout } from '../../src/game/epic-equipment.service';
import { combatEnvironmentFor, snapshotCombatEnvironment, weatherElementMultiplier as dynamicWeatherElementMultiplier } from '../../src/game/world-dynamics.service';
import { dynamicNpcProfile } from '../../src/game/dynamic-npc-dialogue.service';
import { activeDeviceSkillByCode, combatDeviceSlotsFor, initializeCombatDeviceEnergy, restoreCombatDeviceEnergy } from '../../src/game/device.service';
import { assertCombatLoadoutMutable } from '../../src/game/combat-loadout-lock.service';
import { regionalBossComponentByKey, regionalBossComponentsFor, type RegionalBossComponentKey } from '../../src/game/regional-boss-components.config';
...
type PendingAction = { hidden?: HiddenChoice; type: 'attack' | 'skill' | 'item' | 'escape' | 'device' | 'device_charge'; chantRelease?: boolean; slot?: number; skillId?: number; itemId?: number; deviceSkillCode?: string; deviceInstanceId?: number; targetKind?: 'member' | 'target'; targetId?: number };
...
export type VictorySettlement = { kind: 'victory'; members: { name: string; experience: number; staminaSpent?: number; staminaInsufficient?: boolean; realmLocked?: boolean; realmCapReached?: boolean; realmStage?: number; levelText?: string; drops: { name: string; quantity: number; itemType: string; codexId: string | null; instanceId?: number }[]; learned: { id: number; name: string }[] }[]; advancedProfessionCompleted?: { name: string; code: string; profession: string; passive: string; refundedSkillPoints: number; availableSkillPoints: number }[]; arrivalPending?: boolean; dungeonSecretCompleted?: boolean; goblinKingCompleted?: boolean; evolutionCompleted?: boolean; pursuitCooldownMinutes?: number };
...
export const awardRealmExperience = async (connection: PoolConnection, character: Pick<CharacterRow, 'id' | 'level' | 'experience' | 'realm_stage'>, rawExperience: number, options: { fixed?: boolean } = {}) => {
  ...
  const divineExperience = !options.fixed && await (await import('../../src/game/divine-effects')).hasDivine(connection,Number(character.id),'divine_g12') ? 1.25 : 1;
  let level = currentLevel; let experience = repaired.corrected ? 0 : Number(character.experience); let remaining = Math.max(0, Math.floor(rawExperience * (options.fixed ? 1 : await globalExperienceMultiplier(connection)) * divineExperience));
...
const normalizeRegionalBossTraits = async (pool: Pool) => {
  ...
  const [rows] = await pool.execute<(RowDataPacket & MonsterAttributes & { id: number; level: number; traits_json: unknown })[]>(`SELECT DISTINCT s.id,COALESCE(s.level,t.level) AS level,
...
export const decayWorldBossTraits = async (pool: Pool) => {
  const [rows] = await pool.execute<(RowDataPacket & MonsterAttributes & { id: number; level: number; current_hp: number; traits_json: unknown })[]>(`SELECT DISTINCT s.id,COALESCE(s.level,t.level) AS level,s.current_hp,
    COALESCE(s.constitution,t.constitution) AS constitution,COALESCE(s.spirit,t.spirit) AS spirit,COALESCE(s.strength,t.strength) AS strength,
    COALESCE(s.intelligence,t.intelligence) AS intelligence,COALESCE(s.agility,t.agility) AS agility,COALESCE(s.perception,t.perception) AS perception,
    t.constitution_growth,t.spirit_growth,t.strength_growth,t.intelligence_growth,t.agility_growth,t.perception_growth,s.traits_json
    FROM monster_spawns s JOIN monster_templates t ON t.id=s.template_id
...
const finalAttribute = (character: Pick<CharacterRow, keyof Allocation | `${keyof Allocation}_growth` | 'level'>, attribute: keyof Allocation) => Number(character[attribute]) + Number(character[`${attribute}_growth`]) * ('npc_code' in character && character.npc_code ? Math.max(0, Number(character.level) - 1) : playerGrowthShares(Number(character.level)));
...
const characterFor = async (qqUserId: string, connection?: PoolConnection): Promise<CharacterRow> => {
  ...
  if ((character.activity_status === 'resting' || character.activity_status === 'unconscious') && character.rest_started_at) {
    ...
    if (seconds > 0) {
      ...
      const recoveryMultiplier = (1 + await homeRestRecoveryBonus(pool, Number(character.id)) / 100) * (await (await import('../../src/game/divine-effects')).hasDivine(pool,Number(character.id),'divine_g01') ? 1.3 : 1) * (epicRest.setCode==='crimson_crown'&&epicRest.setCount>=5?1.1:1) * (character.activity_status==='resting'&&await(await import('../../src/game/companion.service')).activeCompanionSpecialty(pool,Number(character.id),'watch')?1.1:1);
...
export const startRest = async (qqUserId: string) => withTransaction(async connection => {
  ...
  const openingRest=(await(await import('../../src/game/divine-effects')).hasDivine(connection,Number(character.id),'divine_g01')?1.3:1)*(restEpic.setCode==='crimson_crown'&&restEpic.setCount>=5?1.1:1)*(await(await import('../../src/game/companion.service')).activeCompanionSpecialty(connection,Number(character.id),'watch')?1.1:1);
  ...
  return { resting: true, message: `你开始休息，每秒恢复 ${Number(((1 + homeBonus / 100)*openingRest).toFixed(3))}% 的生命与魔力。` };
});
...
export const equipmentDetail = async (qqUserId: string, instanceId: number) => {
  const character = await characterFor(qqUserId);
  const [rows] = await (await getPool()).execute<(RowDataPacket & { name: string; item_category: string; weapon_type: string | null; rarity: string; required_level: number; quality: number; durability: number; durability_max: number; effect_json: unknown; forge_primary_json: unknown; description: string })[]>(`
...
const moveToPosition = async (connection: PoolConnection, qqUserId: string, x: number, y: number, restrictToPerception: boolean, speedLimit?: number, destinationRegionId?: number, allowRestingArrival = false) => {
  ...
  const find=distance>0&&Number(findRegion[0]?.danger_level)>0&&findRegion[0]?.is_spawn_enabled&&!findRegion[0]?.is_owner_only?await(await import('../../src/game/companion.service')).companionFind(connection,Number(character.id)):'';
...
export const moveTo = async (qqUserId: string, x: number, y: number, options: { destinationKind?: 'normal' | 'home'; destinationRegionId?: number } = {}) => {
  ...
  return withTransaction(async connection => {
    ...
    const seconds = Math.max(1, Math.ceil(distance / carry.movementSpeed * (await (await import('../../src/game/divine-effects')).hasDivine(connection,Number(character.id),'divine_g02') ? .8 : 1)));
...
export const completeTravel = async (qqUserId: string) => {
  ...
  const { enterHome } = await import('../../src/game/home.service');
...
const chooseTargetInTransaction = async (connection: PoolConnection, qqUserId: string, spawnId: number, ambush = false, retreatPosition?: CombatRetreatPosition, prepaid?: Record<string, boolean>) => {
  ...
  for (const member of members) await assertNoNegotiation(connection, Number(member.id));
...
export const observeHiddenQuestMonster = async (qqUserId: string, spawnId: number) => withTransaction(async connection => {
  const { hiddenQuestCharacter, recordHiddenQuestObservation } = await import('../../src/game/hidden-quest.service');
...
const applySkillEffects = async (connection: PoolConnection, sessionId: string, skillId: number, caster: { id: number; level?: number; tenacity_pierce?: number }, casterKind: 'member' | 'target', target: { id: number; level?: number; tenacity?: number }, targetKind: 'member' | 'target', timing: 'on_hit' | 'on_cast', log: string[], beneficialEffectBonusPct = 0, controlChanceMultiplier = 1, ruleBridge?: CombatRules) => {
  ...
  for (const effect of effects) {
    ...
    if (effect.stackable) {
      ...
      if (currentStacks < maxStacks) await connection.execute('INSERT INTO combat_status_effects (session_id,target_kind,target_id,effect_id,effect_level,value,remaining_turns) VALUES (?,?,?,?,?,?,?)', [sessionId, effectTargetKind, targetId, effect.id, effect.effect_level, appliedValue, appliedDuration]);
...
const processTurnEffects = async (connection: PoolConnection, sessionId: string, targetKind: 'member' | 'target', targetId: number, members: CombatMemberRow[], targets: CombatTargetRow[], log: string[], absorb: typeof absorbLifeShield = absorbLifeShield) => {
...
const spawnRegionalBossComponents = async (connection: PoolConnection, sessionId: string, bodies: SpawnRow[], members: Array<Pick<CharacterRow, 'id'>>) => {
  for (const body of bodies) {
    ...
    for (let index = 0; index < definitions.length; index += 1) {
      ...
      const stats: BossComponentStats = {
...
const refreshCombatSpiritEffect = async (connection: PoolConnection, sessionId: string, targetKind: 'member' | 'target', targetId: number, effectCode: string, value: number, turns: number) => {
  ...
  if (existing[0]) await connection.execute('UPDATE combat_status_effects SET value=GREATEST(value,?),remaining_turns=GREATEST(remaining_turns,?) WHERE id=?', [value, turns, existing[0].id]);
  else await connection.execute('INSERT INTO combat_status_effects (session_id,target_kind,target_id,effect_id,effect_level,value,remaining_turns) VALUES (?,?,?,?,1,?,?)', [sessionId, targetKind, targetId, definition.id, value, turns]);
};
    ...
    if (spirit.spirit_code === 'ember') {
      ...
      const strikeTarget = async (target: CombatTargetRow) => {
        ...
        if (!strike.hit) log.push(`&灵契&〖${spirit.spirit_name}〗追击${combatUnitLabel(target)}，但攻击被闪避。`);
          ...
          let damage = directDamageVariance(Math.max(1, Math.floor(strike.damage * elemental * weather * partAoeMultiplier * (isBossComponent(target) ? 1 : bodyMultiplier))));
...
const finishPartyVictory = async (connection: PoolConnection, sessionId: string, members: CombatMemberRow[], targets: CombatTargetRow[]) => {
  ...
  const luckyMembers=new Set<number>();
  for(const member of playerMembers)if(await(await import('../../src/game/divine-effects')).hasDivine(connection,Number(member.id),'divine_g13'))luckyMembers.add(Number(member.id));
  for (const member of playerMembers) {
    ...
    if(experience>0){await(await import('../../src/game/companion.service')).grantCompanionExperience(connection,Number(member.id),totalExperience);await(await import('../../src/game/aqua.service')).advanceAquaPermit(connection,Number(member.id));}
  ...
  for (const target of rewardTargets) {
    ...
    const generated = scaledDropEntries(rawDrops, drop => {
      const reference = rewardMembers[random(0, rewardMembers.length - 1)]!;
      const modifiers = modifiersByMemberId.get(Number(reference.id))!;
      const traitDropBonus = percentBonus(traitList(target.traits_json), 'dropPct') / 100;
      const chance=effectiveGoblinMaterialDropChance(target, drop);
      const divineChance=luckyMembers.has(Number(reference.id))&&Number(drop.chance??1)>0&&Number(drop.chance??1)<1?Math.min(1,chance*1.2):chance;
      return divineChance * globalDrop * (1 + traitDropBonus + partyDropBonus + omniscientDropBonus + elixirDropBonus) + modifiers.dropBonus;
    }, luckMultiplier);
...
const persistBattleMembers = async (connection: PoolConnection, members: CombatMemberRow[], forceRest = false) => {
  for (const member of members) {
    ...
    if(defeated&&!member.npc_code){
      const divine=await import('../../src/game/divine-effects');
      if(await divine.hasDivine(connection,Number(member.id),'divine_g01')&&await divine.spendDivineUse(connection,Number(member.id),'g01_recovery'))hp=Math.max(1,Math.floor(Number(member.hp_max)*.5));
...
const combatActionInTransaction = async (connection: PoolConnection, qqUserId: string, action: Exclude<PendingAction['type'], 'device_charge'>, slot?: number, skillId?: number, itemId?: number, deviceSkillCode?: string, targetKind?: 'member' | 'target', targetId?: number, automaticChant = false, hiddenTicket?: HiddenTicket, hiddenAutomatic = false, skipPlayerTurn = false) => {
  ...
  const specializedNativeHealing = async (caster: CombatMemberRow, recipient: CombatMemberRow, amount: number, fixedPercent=false) => {
    ...
    if(!fixedPercent)multiplier*=(await import('../../src/game/opening-combat')).openingSpellHealingFactor(source);
  ...
  const triggerEpicEffectiveHeal = async (caster: CombatMemberRow, recipient: CombatMemberRow, hpBefore: number, lowBeforeHeal: boolean) => {
    ...
    (await import('../../src/game/opening-combat')).openingEffectiveHeal(rules,ruleUnit('member',Number(caster.id)),ruleUnit('member',Number(recipient.id)),healed);
  ...
  hiddenReorder(rules, turns, entry => entry.kind === 'automaton' ? entry.pet.unit : entry.kind === 'spirit' ? ({ key: `spirit:${entry.id}`, hp: 1 } as import('../../src/game/combat-rule-registry').RuleUnit) : ruleUnit(entry.kind, entry.id), bonusPhase);
  ...
  for (const turn of turns) {
    ...
    if (turn.kind === 'member') {
      ...
      if(!extraTurn&&session.mode!=='spar'&&character.region_name!=='首领测试场'){await(await import('../../src/game/companion.service')).companionSupport(connection,rules,ruleUnit('member',Number(member.id)),`${session.combat_id}:${session.turn_no}`);await(await import('../../src/game/aqua.service')).aquaSupport(connection,rules,ruleUnit('member',Number(member.id)),`${session.combat_id}:${session.turn_no}`);if(targets.every(target=>target.is_defeated))continue;}
      if (choice.type === 'item') {
      ...
      if (choice.type === 'device') {
        ...
        const applyDamage = async (enemy: CombatTargetRow, magic: boolean, multiplier: number, element = '') => {
          ...
          const oldHp = Number(enemy.current_hp); const shield = await ruleTakeDamage('target', Number(enemy.id), damage, deviceSkill.targetScope === 'all_enemies'); damage = shield.incoming;
      ...
      if (skillCode === 'summoner_contract_spirit' || skillCode === 'summoner_spirit_tether' || skillCode === 'summoner_star_pact') {
        ...
        if (!spirits.length) {
          ...
          const requirement = advancedResourceRequirementForSkill(skillCode); if (requirement) await gainResource(member, requirement.amount, '灵体未响应，返还消耗');
      ...
      await rules.areaDamage(affectedTargets.map(row => ruleUnit('target', Number(row.id))), async struckUnit => {
        ...
        const oldHp = Number(struckTarget.current_hp); const shield = await ruleTakeDamage('target', Number(struckTarget.id), damage, skillTargetScope === '全体'); damage = shield.incoming; const hpDamage = damage - shield.absorbed;
      ...
      for (const affectedVictim of affected) {
        ...
        const split = await splitAdvancedGuard(affectedVictim, await rules.incoming(ruleUnit('target', Number(monsterTarget.id)), ruleUnit('member', Number(affectedVictim.id)), dealt, String(skill?.element ?? '无'), skill?.category === 'magic', Boolean(skill), affected.length === 1, true), affected.length === 1); const finalDealt = split.recipientDamage; const shield = await ruleTakeDamage('member', Number(affectedVictim.id), finalDealt); const hpDamage = finalDealt - shield.absorbed;
        ...
        if(skill?.target_scope==='全体'&&session.mode!=='spar'&&character.region_name!=='首领测试场')await(await import('../../src/game/companion.service')).companionAreaHit(connection,rules,ruleUnit('member',Number(affectedVictim.id)),ruleUnit('target',Number(monsterTarget.id)),skill?.category==='magic');
  ...
  if (session.mode === 'spar' && (targets.every(target => target.is_defeated) || members.every(member => member.is_defeated) || aliveMembers.every(member => (jsonObject(member.pending_action) as unknown as PendingAction).type === 'escape'))) {
...
const settleForcedCombatDefeat = async (connection: PoolConnection, sessionId: string) => {
  ...
  if (sessionRows[0]?.mode === 'spar') return { ambushWaiting: false, settlement: await finishNpcSparring(connection, sessionId, 'timeout') };
