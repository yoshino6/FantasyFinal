...
test('器阵实例仅在所属职业战斗中锁定，非登记物品与战后仍可操作',async()=>{
  for (const profession of ['weapon_master','inventor']) {
    let fighting=true;
    const connection={execute:async(sql:string)=>[sql.includes('negotiation_participants')?[]:sql.includes('player_advanced_professions')?[{profession_code:profession}]:sql.includes('player_hidden_profession_loadouts')?[{config_json:JSON.stringify({weapons:[7],devices:[7]})}]:fighting?[{found:1}]:[]]};
