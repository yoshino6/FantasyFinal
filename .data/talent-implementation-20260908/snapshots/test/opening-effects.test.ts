...
test('G15/G17只进入敌对PVE直击；额外来源、PVP和80%减伤上限隔离',async()=>{
  const a=unit(),b=unit('target:2'),r=engine(a,b);a.opening!.divines=['divine_g15'];
  assert.equal(await r.incoming(a,b,1000,'无',false,false),1100);
  ...
  a.opening!.pve=true;a.opening!.divines=['divine_g17'];assert.equal(await r.incoming(b,a,1000,'无',false,true),920);
...
test('G16以实付MP折减并向上取整，原零耗维持零；晨枝只返还首个付费技能',async()=>{
  const a=unit(),r=engine(a);a.opening!.divines=['divine_g16'];a.opening!.weapons=['dawn_staff'];
  assert.equal(r.manaCost(a,0),0);assert.equal(r.manaCost(a,1),1);assert.equal(r.manaCost(a,11),9);
...
test('掌心微日只跟随有效治疗，溢出不转盾，追加不形成治疗链',()=>{
  const a=unit(),b=unit('member:2'),r=engine(a,b);a.opening!.divines=['divine_g14'];a.opening!.weapons=['dawn_orb'];
  assert.equal(openingSpellHealingFactor(a),1.15);openingEffectiveHeal(r,a,b,0);assert.equal(a.state.memory.opening_dawn_orb,undefined);
...
test('G10目录报价与扣款共用额度：优先礼包，不叠加，超过日上限只折剩余部分',async()=>{
  let credit=0,used=198;const c={execute:async(sql:string)=>sql.startsWith('SELECT')&&sql.includes('player_opening_services')?[[{uses:credit}]]:sql.startsWith('SELECT used')?[[{used}]]:sql.startsWith('SELECT 1')?[[{}]]:[{affectedRows:1}]} as any;
