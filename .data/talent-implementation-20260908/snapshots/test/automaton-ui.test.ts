...
const load=(path:string,mocks:Record<string,unknown>)=>{
  ...
  new Function('require','module','exports',code)((id:string)=>mocks[id]??(id==='./secondary-shop-format'?{secondaryShopFormatSource:(source:unknown)=>source}:id==='node:async_hooks'?{AsyncLocalStorage}:{}),module,module.exports);return module.exports;
};
...
test('自动战斗仅发战报和结算，不发旧对白、不触发每日问候，手动回复不受影响',async()=>{
  const sends:any[]=[];let dailyCalls=0,acks=0;
  const wrapper=load('src/game/use-game-message.ts',{
    alemonjs:{Format,ResultCode:{Ok:2000},logger:log,useEvent:()=>[{current:{UserId:'u'}}],useRoute:()=>[{matched:true}],useMessage:()=>[{send:async(params:any)=>{await Promise.resolve();sends.push(params.format);return[{code:2000}];}}]},
    './automaton-dialogue':{automatonInteractionText,automatonBattleInteractionText},
    './automaton-dialogue.service':{reserveDailyAutomaton:async()=>{dailyCalls++;return null;},acknowledgeAutomatonBattleText:async()=>{acks++;}}
  });
  const sender=wrapper.useGameMessage()[0],automatic=wrapper.withoutAutomatonInteractions(sender);
  const process=Format.create().addMarkdown(Format.createMarkdown().addTitle('自动战斗过程').addCode('伤害 100\n\u2063〖小光〗「我会保护你。」',{language:'text'}));
  await automatic.send({format:process});
  await automatic.send({format:Format.create().addMarkdown(Format.createMarkdown().addTitle('战斗胜利').addText('获得材料×1'))});
  assert.equal(sends.length,2);assert.equal(dailyCalls,0);assert.equal(acks,0);
  assert(!text(sends[0]).includes('我会保护你'));assert(text(sends[0]).includes('伤害 100'));assert(text(sends[1]).includes('获得材料'));
  const direct=wrapper.withoutAutomatonInteractions({send:async(params:any)=>{assert(!text(params.format).includes('我会保护你'));return [];}});
  await direct.send({format:process});
  await Promise.all([automatic.send({format:process}),sender.send({format:Format.create().addText('角色面板')})]);
  assert.equal(dailyCalls,1);
  await sender.send({format:process});assert.equal(acks,1);assert.equal(dailyCalls,2);
});

test('自动玩家的人偶不生成战斗语录、不占语录配额；手动队友和关闭自动后正常触发',async()=>{
  let enabled=true;const prepared:number[]=[];
  const service=load('src/game/automaton-combat.service.ts',{
    './combat-unit-label':{combatUnitLabel},
    './automaton-dialogue.service':{prepareAutomatonQuote:async(_c:unknown,id:number)=>{prepared.push(id);return {text:'准备好了。'};}}
  });
  const connection={execute:async(sql:string)=>sql.includes('player_auto_battle_settings')?[enabled?[{character_id:1}]:[]]:[[]]};
  const pet=(id:number,ownerId:number)=>({id,ownerId,unit:{key:`automaton:${id}`,name:`人偶${id}`,hp:100,hpMax:100,state:{memory:{}}},battle:{actionCount:0,pet:{}}});
  const log:string[]=[];
  await service.appendAutomatonBattleQuotes(connection,'battle',[pet(1,1)],log,false);
  assert.deepEqual(prepared,[]);assert.deepEqual(log,[]);
  await service.appendAutomatonBattleQuotes(connection,'battle',[pet(1,1),pet(2,2)],log,false);
  assert.deepEqual(prepared,[2]);assert.equal(log.length,1);
  enabled=false;
  await service.appendAutomatonBattleQuotes(connection,'battle',[pet(1,1)],log,false);
  assert.deepEqual(prepared,[2,1]);assert.equal(log.length,2);
});
