...
const names = ['jsonRecord', 'withEquipmentStats'];
...
const withEquipmentStats = new Function('equipmentQualityMultiplier', 'calculatePanelStats', 'panelPercentKeys', 'armorSetFromRows', 'armorPanelPercent', `${compiled}; return withEquipmentStats;`)(equipmentQualityMultiplier, calculatePanelStats, panelPercentKeys, armorSetFromRows, armorPanelPercent);
...
test('板甲双防与其他甲类相同，衣服单件速度按品质独立乘算', async () => {
  const result = await withEquipmentStats(connection([
    { effect_json: { physicalDefense: 100, magicDefense: 100, speed: 100 }, quality: 100, weapon_type: '板甲', slot:'upper' }
  ]), 1, { ...base, physicalDefense: 100, magicDefense: 100, speed: 100 }, { physicalDefensePct: 50 });
  assert.equal(result.physicalDefense, 300);
  assert.equal(result.magicDefense, 200);
