...
const message=load('src/game/opening-message.ts',{alemonjs:{Format}});
...
test('先点击其他功能只重发待开始页，开始按钮有效；相似命令前缀不能绕过保护',async()=>{
  for(const key of ['工会商店','随从操作','物品出售','前往商店']){
    ...
    assert.ok(JSON.stringify(group.value).includes('/继续剧情'));assert.ok(!JSON.stringify(group.value).includes('/初行选择 0 next'));
  }
});
