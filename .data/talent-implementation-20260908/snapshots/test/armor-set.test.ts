...
test('轻甲只加生命魔力；重甲和板甲生命档位独立于品质',()=>{
  ...
  for (const [count,pct] of [[3,8],[5,15]]) {
    const set=armorSetFromRows(rows('轻甲',count))!;
    assert.deepEqual(Object.keys(set.panelPercent).sort(),['hpPct','mpPct']);
    const result=calculatePanelStats(base,{}, {},[set.panelPercent]);
    for(const key of Object.keys(base) as Array<keyof typeof base>) assert.equal(result[key],Math.floor(base[key]*(1+(['hpMax','mpMax'].includes(key)?pct:0)/100)),key);
  }
  for (const name of ['板甲','重甲']) for(const count of [3,5]) {
    const set=armorSetFromRows(rows(name,count).map(row=>({...row,quality:1,rarity:'普通'})))!;
    assert.equal(set.panelPercent.hpPct,count===5?25:12);
    assert.equal(set.critAvoidanceCorrectionPct,name==='板甲'?12:(count===5?16:8));
  }
});
...
test('概率按用户例子计算：补足未命中部分、削减最终命中、只削减额外暴伤',()=>{
  for(const [count,rate] of [[3,.16],[5,.33]]) {
    ...
    close(correctedCritChance(.5,strikeCorrections(undefined,plate)),.5*.88);
    close(1+correctedCritBonus(.5,strikeCorrections(undefined,plate)),1+.5*.88);
  }
});
...
test('共享战斗引擎确实读取攻守双方套装，未命中不扣血，暴抗仅削减暴击奖励',async()=>{
  ...
  source.armorSet=armorSetFromRows(rows('皮甲',5));
...
test('板甲套装减伤只计一次，与其他减伤共用80%上限',async()=>{
  const [source,,target]=newHiddenTrial('tactician').units;
  for(const unit of [source,target]){unit.passives=[];unit.modifiers={};unit.state.statuses=[];unit.state.memory={};unit.cooldowns={};unit.opening=undefined;}
  const rules=new CombatRules([source,target],1,[],{absorb:async()=>0,legacyEffects:()=>[],removeLegacy:async()=>{},extraAction:()=>{},swapThreat:async()=>{}},'');
  target.armorSet=armorSetFromRows(rows('板甲',5));
  assert.equal(await rules.incoming(source,target,1000,'',false,false),960);
  rules.add(target,'reduction',79,2,target);
  assert.equal(await rules.incoming(source,target,1000,'',false,false),199);
  assert.equal(await rules.incoming(source,target,1000,'',false,false,true,true),199);
});
...
test('装备摘要同时显示甲类与史诗；5件甲类只显示高档，史诗保留两档；未激活不展示',()=>{
  ...
  assert.match(summary[0].effects[0],/12%/);assert.match(summary[0].effects[0],/25%/);
