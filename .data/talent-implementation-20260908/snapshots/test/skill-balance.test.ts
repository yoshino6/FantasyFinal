...
test('零值时间以1为基数，累计整回合才生效，无等阶上限', () => {
  ...
  assert.equal(both.cooldown, 0); assert.equal(both.chant, 0);
  const lower = skillSpecialization({ ...base, tier: '下位', cooldown_turns: 3, chant_turns: 0 }, { overcharge: 20, potent: 20 });
  assert.equal(lower.cooldown, 3); assert.equal(lower.chant, 0);
});

test('四项专精首点乘算与合算数值', () => {
  const over = skillSpecialization(base, { overcharge: 2 });
  assert.equal(over.power, 159); assert.equal(over.effectFactor, 1); assert.equal(over.mana, 336);
  assert.equal(over.cooldown, 5); assert.equal(over.chant, 1);
  const potent = skillSpecialization(base, { potent: 2 });
  assert.equal(potent.power, 150); assert.equal(potent.effectFactor, 1.06); assert.ok(Math.abs(potent.durationChange - .06) < 1e-10);
  const instant = skillSpecialization(base, { instant: 2 });
  assert.equal(instant.power, 150); assert.equal(instant.effectFactor, 1); assert.equal(instant.timeFactor, .94);
  assert.equal(skillSpecialization(base, { efficient: 2 }).mana, 264);
  const all = skillSpecialization(base, { overcharge: 2, potent: 2, instant: 2, efficient: 2 });
  assert.equal(all.powerFactor, 1.06); assert.equal(all.effectFactor, 1.06); assert.equal(all.mana, 371);
  assert.equal(all.cooldown, 5); assert.equal(all.chant, 1);
});

test('瞬息保留普通效果，强效延时不改机制和硬控', async () => {
  ...
  await cast(f, 'F02', f.friend); assert.ok(Math.abs(f.rules.value(f.friend, 'physical_reduction') - 24) < 1e-8);
...
test('控制强效概率有界，瞬息不降低控制概率，强效不额外增伤', async () => {
...
test('石化强效概率实际生效，但不延长硬控时长', async () => {
  ...
  const reduced = fixture(() => .01); reduced.source.castSpecialization = skillSpecialization(base, { instant: 40 });
  await cast(reduced, 'B02'); assert.ok(reduced.rules.status(reduced.target, 'petrify'));
});
...
test('专精10/20/40限制点数，非法等级钳制，不限制合法点数的乘算惩罚', () => {
  ...
  for (const tier of ['基础', '下位', '中位']) {
    ...
    assert.equal(maximum.powerFactor, 1.06 ** (limit - 1)); assert.equal(maximum.supportFactor, 1.06 ** (limit - 1)); assert.equal(maximum.damageFactor, 1);
...
for (const skill of [...nativeSkillBalance, ...residentSkills]) test(`分阶与满专精边界 ${skill.code}`, () => {
  ...
  for (const levels of Array.from({ length: 16 }, (_, mask) => Object.fromEntries(['overcharge', 'potent', 'instant', 'efficient'].map((key, index) => [key, mask & (1 << index) ? 100 : 1])))) {
    ...
    assert.ok(result.cooldown <= skill.cooldown);
    assert.ok(result.chant <= skill.chant);
...
test('无数值落点不出售强效，纯机制不虚设成长', () => {
  ...
  assert.ok(!specializationOptions({ ...base, cooldown_turns: 0, chant_turns: 0 }).includes('instant'));
...
test('过充增加直伤，强效只增加治疗护盾，普攻不吃主动专精', async () => {
  ...
  await f.rules.restore(f.source, f.friend, 1000); assert.equal(f.friend.hp, 10000);
