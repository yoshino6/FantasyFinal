...
test('隔离MySQL：113分支从注册到安全公会，关闭改道、保护与重试', {skip:process.env.FF_OPENING_DB_TEST!=='1'},async t=>{
  ...
  const tables=['players','characters','registration_sessions','registration_scene_records','character_hidden_attributes','player_skill_point_ledger',
    'map_regions','map_region_areas','map_npcs','item_definitions','skill_definitions','profession_definitions','player_inventory','player_quick_items','player_skills','player_appraisal_progress','player_blessings',
    ...
    'combat_sessions','combat_members','player_pvp_battle_sessions','negotiation_sessions','negotiation_participants','player_travels'];
  const summaries:any[]=[];
  try{
    ...
    for(const table of ['map_regions','map_region_areas','map_npcs','item_definitions','skill_definitions','profession_definitions'])await c.query(`INSERT INTO ${quoted(table)} SELECT * FROM ${quoted(db.database)}.${quoted(table)}`);
    ...
    let random=()=>.5;
    const deps:Record<string,any>={'alemonjs':{},'node:crypto':{randomUUID},'../database/pool':database,'./constants':constants,'./types':{attributes},
      ...
      './opening-keepsakes.config':keepsakeConfig,'./opening-guild.config':guildConfig,'./opening-replay':replay,'./encumbrance':{encumbrance},
    ...
    deps['./companion.service']=load('src/game/companion.service.ts',['grantGoldenRabbit','openingFeedRabbit']);
    ...
    const registration=load('src/game/character.service.ts',['elements','randomBalancedElements','randomInRange','distribute','getPlayer','getSession','completedRegistration','beginRegistration','continueRegistration','askWhereAmI','chooseDestination','requireChoiceSession','chooseGift']);
    let sequence=0;
    const prepare=async(routeCode:string)=>{
      ...
      random=()=> (Number(route.code.at(-1))-.5)/3;
      ...
      await registration.chooseGift(user,'G16');
      ...
      assert.ok(skills.some(s=>s.code==='appraisal'&&s.level===1));assert.ok(skills.some(s=>s.code==='divine_g16'));assert.equal(character.skill_points,1);
      assert.equal(await registration.chooseGift(user,'G15'),null);
    ...
    const toBranchEnd=async(run:Awaited<ReturnType<typeof prepare>>,branch:string)=>{
      ...
      while(view.state==='reading')view=await story.advanceOpening(run.user,view.revision,'next');
    ...
    await t.test('女神被另一位玩家先带走时重新选择厄里斯返还，不抢归属或重发恩赐',async()=>{
      ...
      const [gifts]=await c.execute<RowDataPacket[]>('SELECT code FROM player_blessings WHERE character_id=?',[run.id]);assert.deepEqual(gifts.map(g=>g.code),['divine_g16']);
    });
    await t.test('旧armed翻页按钮可开始；同按钮重试不跳页',async()=>{
      const run=await prepare('F01'),first=await story.advanceOpening(run.user,0,'next');assert.equal(first.state,'reading');
      const second=await story.advanceOpening(run.user,0,'next');assert.deepEqual(second,JSON.parse(JSON.stringify(first)));
    });
    ...
    await t.test('缺公会或地图物品时不作为安全终点；全无有效终点时拒绝生成新角色',async()=>{
      ...
      await registration.continueRegistration(user,'story');await registration.askWhereAmI(user);await registration.continueRegistration(user,'question');
      await registration.chooseDestination(user,'异世界');await registration.continueRegistration(user,'danger');
      await assert.rejects(registration.chooseGift(user,'G16'),/安全接引点/);
