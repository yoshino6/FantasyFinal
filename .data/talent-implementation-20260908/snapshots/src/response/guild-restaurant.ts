...
export default async () => { const [event] = useEvent(); const [message] = useMessage(); try { const context=await requireRestaurant(event.current.UserId); if(context.code==='world_tree'){
  ...
  await message.send({format:Format.create().addMarkdown(markdown).addButtonGroup(Format.createButtonGroup().addRow().addButton('查看 菜单','/餐厅菜单',{type:'command',autoEnter:true,style:'blue'}).addButton('返回 公会大厅','/初行公会',{type:'command',autoEnter:true}))});return;
} await message.send({ format: restaurantFormat() }); } catch (error) { await message.send({ format: messageFormat('无法进入餐厅', error instanceof Error ? error.message : '请稍后重试。') }); } };
