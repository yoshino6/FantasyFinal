...
export default async () => {
  ...
  if (category === 'artifact') { await message.send({ format: messageFormat('神器已经远行', '所有神器已经散布世界各地，请从神技目录选择恩赐。') }); }
...
export const divineDetailHandler=async()=>{const[route]=useRoute();const[message]=useMessage();try{await message.send({format:divineDetail(String(route.param('code')))});}catch(error){await message.send({format:messageFormat('神技',error instanceof Error?error.message:'请重新选择。')});}};
