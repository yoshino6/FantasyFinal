...
import { chestTables, dawnWeapons } from '../game/opening-chest.config';

const contents='每只黄金宝箱固定获得微光草药 ×2，另独立抽取一组奖励：\n\n初晓系列 Lv.1 史诗武器：20%\nLv.1 稀有武器：30%\nLv.1 稀有防具：20%\n随行急救补给：15%\n公会补给券 ×2：15%\n\n武器类型等概率抽取；稀有防具五个部位等概率抽取。每只宝箱单独开奖，奖励均为个人绑定。';
export const openingChestHandler=(mode:'preview'|'confirm'|'result'|'contents')=>async()=>{
  ...
  try{
    const code=String(route.param('code')??'opening_golden_chest');
    if(mode==='contents'){
      if(!chestTables[code])throw new Error('这件物品尚未配置宝箱内容。');
      const md=Format.createMarkdown().addTitle('黄金宝箱·内容').addNewline().addNewline().addText(contents).addNewline().addNewline().addText(dawnWeapons.map(([,type,name])=>`${type}：${name}`).join('\n'));
    ...
    if(mode==='preview'){
      ...
      const md=Format.createMarkdown().addTitle('黄金宝箱·确认开启').addNewline().addNewline().addText(`本次开启：${result.quantity} 只\n\n${contents}\n\n确认后扣除整批宝箱。承载不足时整批保留，可整理背包后继续此页。`);
    ...
    const md=Format.createMarkdown().addTitle('黄金宝箱·开箱结果').addNewline().addNewline().addText(`已开启 ${result.quantity} 只\n\n`);
