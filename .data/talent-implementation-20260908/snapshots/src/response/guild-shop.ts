...
export const guildShopFormat = (text?: string, speaker?: string, continuingChat = false) => {
  ...
  const buttons = Format.createButtonGroup().addRow()
    ...
    .addRow().addButton('切磋', '/切磋 guild_merchant', { type: 'command', autoEnter: true, style: 'blue' }).addButton('闲聊', '/商店闲聊', { type: 'command', autoEnter: true, style: 'blue' }).addButton('返回公会大厅', '/初行公会', { type: 'command', autoEnter: true });
