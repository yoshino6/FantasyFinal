  ...
  if (view === '已学习') {
    ...
    if (boundSkills.length) {
      ...
      for (const skill of boundSkills) {
        ...
        markdown.addBlockquote(`【${skill.name}】${isAdvancedProfessionSkillCode(skill.code) ? '' : `Lv.${skill.level} `}`).addButton('[详情]', { data: `/技能详情 ${skill.id}`, autoEnter: false }).addText(bound ? ` [绑定·${bound.kind}]` : ' [绑定]').addNewline();
      }
    }
