...
export const openingGuildView=async(user:string)=>{
  ...
  const[maps]=await pool.execute<RowDataPacket[]>(`SELECT i.code,i.name FROM item_definitions i JOIN map_regions r ON JSON_UNQUOTE(JSON_EXTRACT(i.effect_json,'$.map'))=r.code WHERE i.item_category='地图' AND r.is_enabled=1 AND r.is_owner_only=0 AND r.danger_level<=10 AND r.code<>'floating_leaf_town' ORDER BY r.id`);
...
export const openingGuildAction=async(user:string,action:string,value='')=>withTransaction(async c=>{
  ...
  if(action==='map_exchange'){
    const[items]=await c.execute<RowDataPacket[]>(`SELECT i.id,i.code,i.name FROM item_definitions i JOIN map_regions r ON JSON_UNQUOTE(JSON_EXTRACT(i.effect_json,'$.map'))=r.code WHERE i.code=? AND i.item_category='地图' AND r.is_enabled=1 AND r.is_owner_only=0 AND r.danger_level<=10 AND (r.code<>'floating_leaf_town' OR EXISTS(SELECT 1 FROM opening_world WHERE id=1 AND leaf_route_open=1)) FOR UPDATE`,[value]);
    const item=items[0];if(!item)throw new Error('请选择兑换清单中已经开放的普通低危地图。');
  ...
  if(action==='meal'){
    ...
    const[divine]=await c.execute<RowDataPacket[]>("SELECT 1 FROM player_blessings WHERE character_id=? AND code='divine_g06'",[id]);
...
export const openingKeepsakes=async(user:string)=>{
  ...
  return{items:rows.map(r=>{const item=parse(r.record_json);return{name:String(item.name),use:String(item.use),future:String(item.future),code:String(r.code),used:Boolean(r.used),archived:Boolean(r.archived)};}),events};
};
