...
export const adminPortraitReviews=async(query:{page?:unknown;keyword?:unknown;status?:unknown})=>{
  ...
  const [rows]=await pool.execute<RowDataPacket[]>("SELECT r.id,r.automaton_id,r.status,r.reason,r.reviewer,r.created_at,r.reviewed_at,c.name player_name,p.qq_user_id,JSON_UNQUOTE(JSON_EXTRACT(a.state_json,'$.name')) automaton_name "+from+where+' ORDER BY r.id DESC LIMIT 20 OFFSET ?',[...values,(page-1)*20]);
