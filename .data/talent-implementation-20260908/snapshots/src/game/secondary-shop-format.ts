...
export const secondaryShopFormatSource=(source:unknown):unknown=>{
  ...
  if(Array.isArray(result))result.push({type:'BT.group',value:[{type:'BT.row',value:[{type:'Button',value:'返回店铺',options:{data:shopReturnCommands[context.shop],type:'command',autoEnter:true}}]}]});
  return result;
};
