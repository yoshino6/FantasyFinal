...
const weights: Record<string, number[]> = { warrior: [25, 12, 25, 8, 14, 16], mage: [12, 25, 8, 27, 12, 16], rogue: [13, 10, 20, 10, 25, 22], priest: [22, 27, 8, 21, 10, 12] };
const professionGrowth: Record<string, string[]> = { warrior: ['constitution', 'strength'], mage: ['spirit', 'intelligence'], rogue: ['agility', 'perception'], priest: ['constitution', 'spirit'] };
...
export const recalculateNpcSparProfileStats = (profile: NpcNumericInput) => {
  ...
  const armorType=({warrior:'重甲',mage:'布甲',rogue:'皮甲',priest:'轻甲'} as Record<string,string>)[archetype]!;
...
export const buildNpcSparProfile = (npc: { code: string; name: string; description: string; region_code: string }, playerLevel: number, worldStage: number) => {
  ...
  const baseNameToCode: Record<string, string> = { 战士: 'warrior', 法师: 'mage', 盗贼: 'rogue', 牧师: 'priest' };
  ...
  const offense = active.filter(skill => skill.category !== 'utility'); const support = active.filter(skill => skill.category === 'utility');
