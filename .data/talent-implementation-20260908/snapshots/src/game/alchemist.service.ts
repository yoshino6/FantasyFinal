...
export const executeBulkPurification = async (qqUserId: string, requestToken?:string) => withTransaction(async connection => {
  ...
  const earnedProficiency = shopProficiency(Math.round(proficiencyGain)); const next = await addAlchemistProficiency(connection, qqUserId, characterId, earnedProficiency);
  const token = requestToken;
