...
export const craftForgeEquipment = async (qqUserId: string, _confirmed = false) => withTransaction(async connection => {
  ...
  const proficiencyGain = shopProficiency(forgeProficiencyGain(session.equipment_category, level, rarity)); const progress = await addBlacksmithProficiency(connection, characterId, proficiencyGain);
