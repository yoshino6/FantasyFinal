...
import { correctedHitChance, correctedCritChance, correctedCritBonus, strikeCorrections, type StrikeCorrections, bossControlChanceMultiplier, opposedChance, tenacityContest } from './combat-math';
...
import { hasOpeningDivine, hasOpeningWeapon, openingManaCost, openingPaid, openingAfterHit, type OpeningCombatEffects } from './opening-combat';
...
export class CombatRules {
  ...
  healingMultiplier(source: RuleUnit, target: RuleUnit, equipment = true) {
    ...
    return healing * alchemyHealingFactor(this, target) * hiddenHealingFactor(this, target);
  }
  ...
  async secondary(source: RuleUnit, target: RuleUnit, damage: number, label: string, element = '无', areaHit = false) {
    ...
    const { damage: dealt } = await this.takeHit(target, incoming, 1, areaHit); this.log.push(`　&${label}&${combatUnitLabel(target)}受到 ${dealt} 点${element === '无' ? '' : element}伤害。`);
  }
  async take(target: RuleUnit, damage: number, shieldMultiplier=1) {
    return (await this.takeHit(target, damage, shieldMultiplier)).absorbed;
  }
  async takeHit(target: RuleUnit, damage: number, shieldMultiplier = 1, areaHit = false): Promise<TakenDamage> {
    const apply = async (amount: number) => ({ damage: amount, absorbed: await this.takeUnlinked(target, amount, shieldMultiplier) });
  ...
  async takeUnlinked(target: RuleUnit, damage: number, shieldMultiplier=1) {
    ...
    const hpDamage = await this.hooks.beforeHpDamage?.(target, pendingHpDamage) ?? pendingHpDamage;
  ...
  async incoming(source: RuleUnit, target: RuleUnit, raw: number, element: string, magic: boolean, skill: boolean, single = true, legacyResolved = false, playerDirect = true) {
    ...
    if(source.side!==target.side&&target.opening?.pve&&hasOpeningDivine(target,'divine_g17'))reduction=100-(100-reduction)*.92;
    if(source.side!==target.side&&source.opening?.pve&&playerDirect){
      if(hasOpeningDivine(source,'divine_g15'))raw*=1.1;
  ...
  async strike(source: RuleUnit, original: RuleUnit, power: number, element: string, magic: boolean, extra = false, forceHit = false, secondaryScale = 1,
    ...
    const { damage: dealt, absorbed } = await this.takeHit(target, Math.floor(bounded*(1-(options.deferFraction??0))), options.shieldMultiplier ?? 1, !single);
