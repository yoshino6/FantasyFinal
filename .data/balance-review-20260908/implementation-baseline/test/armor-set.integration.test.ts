...
test('数据库事务：临时角色穿卸甲、15项属性重算、双战斗适配器与装备消息（全部回滚）',{skip:!enabled},async()=>{
  ...
  try {
    ...
    for(const name of ['重甲','板甲']) for(const count of [2,3,5]) {
      const panel=await wear(name,count),ratio=count===5?1.2:count===3?1.1:1;
    ...
    for(const count of [3,5]) {
      const panel=await wear('轻甲',count),ratio=count===5?1.05:1.03;
      for(const [key,column] of Object.entries(map)) {
        const mobility=['accuracy','evasion','speed'].includes(key)?1+count*characterService.armorClassMobilityModifier('轻甲',key)/100:1;
        assert.ok(Math.abs(Number(panel[column])-Number(baseline[column])*ratio*mobility)<=2,`${count}轻甲 ${column}`);
      }
    ...
    assert.equal(pve.get('member',id).armorSet?.critAvoidanceCorrectionPct,33);assert.equal(pve.get('target',-1).armorSet,undefined);
    const pvp=await createPvpCombatRules(c,[member,foe],[{},{}],1,[],()=>{});assert.equal(pvp.get(id).armorSet?.critDamageCorrectionPct,33);
    ...
    const rendered=strings.join('\n');assert.ok(rendered.indexOf('已生效套装效果')>rendered.indexOf('【戒指】'));assert.match(rendered,/板甲 5\/5件/);assert.match(rendered,/断层壁障/);assert.match(rendered,/33%/);
