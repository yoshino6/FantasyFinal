...
import { openingRoutes, divineSkillDefinitions } from '../game/opening-content';
...
export const initializeOpening = async (pool: Pool) => {
  ...
  for (const skill of divineSkillDefinitions) await pool.execute(`INSERT INTO skill_definitions (code,name,description,category,learn_cost,upgrade_cost,max_level,power_per_level,skill_kind,range_type,passive_effect_json)
    VALUES (?,?,?,'bound',99,99,1,0,'绑定','自身',?) ON DUPLICATE KEY UPDATE name=VALUES(name),description=VALUES(description),passive_effect_json=VALUES(passive_effect_json)`,
  [skill.code, skill.name, skill.description, JSON.stringify({ openingDivine: skill.number })]);
  ...
  for (const route of openingRoutes) for (const choice of route.choices) if (!choice.pack) await item(choice.rewardCode, choice.rewardName, `${choice.rewardUse}\n\n未解之事：${choice.future}`, '特殊', { openingKeepsake: choice.rewardCode, personalBound: true });
  ...
  for (const [code,name,category,weapon,effect] of [
    ['opening_staff','旅人短杖','武器','法杖',{ physicalAttack: 3, magicAttack: 3 }],
    ['opening_clothes','普通旅衣','防具','布甲',{ slot:'上装', physicalDefense: 2, magicDefense: 2 }]
