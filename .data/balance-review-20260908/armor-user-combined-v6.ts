...
assert.ok(Math.abs(full[4].withUserSet.hit-8/33)<1e-12);
assert.ok(Math.abs(full[4].withUserSet.incoming-25/33)<1e-12);
...
const out={at:new Date().toISOString(),profiles,slots,weights,userSets:sets,full,partial,qualityRows,bestMixes:mixes.slice(0,10),rescue,
 checks:{passed:true,loadouts:mixes.length},scope:'Exact newly supplied per-piece table; Exact newly supplied 3/5-piece sets; 5-piece replaces 3-piece. Equal pre-armor opposition stats, identical flat defense, direct expectation only; no skills, mana, initiative or live DB.'};
writeFileSync('.data/balance-review-20260908/armor-user-combined-v6.json',JSON.stringify(out,null,2));
