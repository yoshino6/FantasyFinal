...
const profiles:Record<string,number[]>={
 布甲:[0,3,0,0,0,2],皮甲:[0,2,4,4,2,1],轻甲:[0,0,8,8,4,-1],重甲:[0,-.5,12,12,6,-2],板甲:[0,-1,16,16,8,-3]
};
...
const sets:Record<string,Record<number,SetEffect>>={
 布甲:{3:{hit:10,evade:4},5:{hit:20,evade:8}},
 皮甲:{3:{hit:12,mp:15},5:{hit:24,mp:30}},
 轻甲:{3:{hp:12.5,mp:10},5:{hp:25,mp:20}},
 重甲:{3:{hp:12.5,critAvoid:8,critReduction:8},5:{hp:25,critAvoid:16,critReduction:16}},
 板甲:{3:{hp:10,critAvoid:8,critReduction:8,reduction:2},5:{hp:20,critAvoid:16,critReduction:16,reduction:4}}
};
...
for(const n of names)assert.equal(profiles[n][0],0,'Armor alone does not improve aim');
for(const n of ['轻甲','重甲','板甲']){assert.ok(profiles[n][1]<=0);assert.ok(profiles[n][5]<0);}
for(let i=1;i<names.length;i++){
 assert.ok(profiles[names[i]][1]<=profiles[names[i-1]][1]);
 assert.ok(profiles[names[i]][5]<profiles[names[i-1]][5]);
 for(const k of [2,3,4])assert.ok(profiles[names[i]][k]>profiles[names[i-1]][k]);
}
...
writeFileSync('.data/balance-review-20260908/armor-v4.json',JSON.stringify(data,null,2));
