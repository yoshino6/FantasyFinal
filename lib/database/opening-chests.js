...
const initializeOpeningChests = async (pool) => {
    ...
    await pool.execute(`INSERT INTO item_definitions (code,name,description,obtain_source,item_type,item_category,weapon_type,rarity,required_level,weight,trade_price,stackable,is_tradeable,effect_json) VALUES (?,?,?,'公会选职','equipment','武器',?,'普通',1,0.5,0,0,0,?) ON DUPLICATE KEY UPDATE effect_json=VALUES(effect_json)`, ['opening_normal_bow_crossbow', '公会练习弓弩', '登记职业时领取的普通练习武器。', '弓弩', JSON.stringify({ ...forgedPrimaryStats('武器', '弓弩', 1, '普通'), balanceVersion: 3 })]);
    await pool.execute(`INSERT INTO item_definitions (code,name,description,obtain_source,item_type,item_category,weapon_type,rarity,required_level,weight,trade_price,stackable,is_tradeable,effect_json) VALUES (?,?,?,'公会选职','equipment','武器',?,'普通',1,0.5,0,0,0,?) ON DUPLICATE KEY UPDATE effect_json=VALUES(effect_json)`, ['opening_normal_gun', '公会练习枪炮', '登记职业时领取的普通练习武器。', '枪炮', JSON.stringify({ ...forgedPrimaryStats('武器', '枪炮', 1, '普通'), balanceVersion: 3 })]);
