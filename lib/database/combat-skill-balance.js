...
import { nativeSkillBalance, balancedSkillDescription } from '../game/combat-skill-balance.config.js';
...
const initializeCombatSkillBalance = async (pool) => {
    ...
    for (const skill of nativeSkillBalance)
        await pool.execute(`UPDATE skill_definitions SET
    ...
    for (const [code, area] of Object.entries(aoeDamageProfiles))
        if (!nativeCodes.has(code) && currentDescriptions.has(code)) {
            await pool.execute("UPDATE skill_definitions SET power=?,target_scope='全体',description=? WHERE code=?", [area.power, aoeDescription(code, currentDescriptions.get(code) ?? ''), code]);
        }
