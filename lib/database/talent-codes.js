import { retiredTalentCodes } from './talent-code-map.js';

const identifier = (value) => `\`${value.replaceAll('`', '``')}\``;
const skillReferences = async (connection) => {
    const [rows] = await connection.execute(`SELECT TABLE_NAME,COLUMN_NAME
...
const migrateTalentCodesWithConnection = async (connection) => {
    let migrated = 0;
    let references;
    for (const [oldCode, code] of Object.entries(retiredTalentCodes)) {
        const [definitions] = await connection.execute('SELECT * FROM skill_definitions WHERE code IN (?,?) ORDER BY id FOR UPDATE', [oldCode, code]);
        ...
        if (old && current && Number(old.id) !== Number(current.id)) {
            ...
            for (const reference of references) {
                const [used] = await connection.execute(`SELECT 1 FROM ${identifier(reference.TABLE_NAME)} WHERE ${identifier(reference.COLUMN_NAME)}=? LIMIT 1 FOR UPDATE`, [current.id]);
                if (used.length)
                    throw new Error(`天赋编号迁移冲突：${code}在${reference.TABLE_NAME}已有引用，未合并或删除，请核查。`);
            }
            await connection.execute('DELETE FROM skill_definitions WHERE id=?', [current.id]);
        }
        if (old)
            await connection.execute('UPDATE skill_definitions SET code=? WHERE id=?', [code, old.id]);
        const [holders] = await connection.execute('SELECT character_id FROM player_blessings WHERE code=? FOR UPDATE', [oldCode]);
...
const migrateTalentCodes = async (pool) => {
    const connection = await pool.getConnection();
    try {
        await connection.beginTransaction();
        const migrated = await migrateTalentCodesWithConnection(connection);
...
export { migrateTalentCodes, migrateTalentCodesWithConnection };
