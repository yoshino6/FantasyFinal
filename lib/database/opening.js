import { forgedPrimaryStats } from '../game/constants.js';
import { openingRouteVersions } from '../game/opening-content.js';
import { openingHubs, openingSpawnRegions, openingStartRouteCodes } from '../game/opening-world.config.js';
import { talentSchema } from '../game/talent-data.js';
import { openingRouteDrawSchema } from '../game/opening-route-draw.js';
import { talentDefinitions } from '../game/talent.config.js';
...
const releaseOpeningRouteMaps = async (pool) => {
    const owned = 'getConnection' in pool;
    const connection = owned ? await pool.getConnection() : pool;
    try {
        await connection.beginTransaction();
        const [created] = await connection.execute("INSERT IGNORE INTO game_data_migrations (code) VALUES ('opening_four_route_regions_v1')");
        ...
        const [additional] = await connection.execute("INSERT IGNORE INTO game_data_migrations (code) VALUES ('opening_additional_three_route_regions_v1')");
...
const initializeOpening = async (pool) => {
    for (const sql of openingSchema)
        await pool.query(sql);
    await pool.execute('INSERT IGNORE INTO opening_world (id) VALUES (1)');
    const [columns] = await pool.query("SHOW COLUMNS FROM map_regions LIKE 'newbie_spawn_enabled'");
    if (!columns.length) {
    ...
    for (const skill of talentDefinitions)
        await pool.execute(`INSERT INTO skill_definitions (code,name,description,category,learn_cost,upgrade_cost,max_level,power_per_level,skill_kind,range_type,passive_effect_json)
    VALUES (?,?,?,'bound',99,99,1,0,'绑定','自身',?) ON DUPLICATE KEY UPDATE name=VALUES(name),description=VALUES(description),category=VALUES(category),learn_cost=VALUES(learn_cost),upgrade_cost=VALUES(upgrade_cost),max_level=VALUES(max_level),power_per_level=VALUES(power_per_level),skill_kind=VALUES(skill_kind),range_type=VALUES(range_type),passive_effect_json=VALUES(passive_effect_json)`, [skill.code, skill.name, skill.description, JSON.stringify({ openingTalent: skill.number })]);
    const item = async (code, name, description, category, effect) => pool.execute(`INSERT INTO item_definitions (code,name,description,obtain_source,item_type,item_category,weight,trade_price,is_tradeable,effect_json)
    ...
    for (const route of openingRouteVersions)
        for (const choice of route.choices)
            if (!choice.pack && !choice.rewardKind && !directRewards.has(`${route.code}-${choice.code}`))
                await item(choice.rewardCode, choice.rewardName, `${choice.rewardUse}\n\n未解之事：${choice.future}`, '特殊', { openingKeepsake: choice.rewardCode, personalBound: true });
    ...
    for (const [code, name, description, x, y] of leafLandmarks)
        await pool.execute(`INSERT INTO map_special_objects (region_id,code,name,description,pos_x,pos_y,pos_z)
    ...
    const [forestPoints] = await pool.execute(`SELECT r.id,a.min_x AS pos_x,a.min_y AS pos_y,a.min_z AS pos_z FROM map_regions r
...
export { initializeOpening, openingSchema, releaseOpeningRouteMaps };
