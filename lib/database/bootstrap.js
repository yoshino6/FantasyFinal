...
import { rareForgeMaterials, regionalForgeMaterials, epicForgeRecipes, epicBlueprintDropChance } from '../config/epic-forging.js';
...
import { regionalV2Resistance, regionalV2Skills, regionalV2SkillProfiles } from '../game/regional-boss-v2.config.js';
...
const seedWorldSurfaceContent = async (pool) => {
    ...
    for (const [code, resistance] of Object.entries(regionalV2Resistance))
        await pool.execute('UPDATE monster_templates SET element_resistance_json=? WHERE code=?', [JSON.stringify(resistance), code]);
    for (const [code, name, category, element] of regionalV2Skills) {
      ...
      VALUES (?,?,?,?,?,?,?,?,0,0,?,99,1,?) ON DUPLICATE KEY UPDATE name=VALUES(name),category=VALUES(category),damage_type=VALUES(damage_type),element=VALUES(element),target_scope=VALUES(target_scope),power=VALUES(power),description=VALUES(description)`, [code, name, category, category === 'physical' ? '打击' : element, category === 'utility' ? '强化' : category === 'magic' ? '元素' : '武技', element, '远程', profile && profile.ratio < 1 ? '全体' : '单体', profile?.power ?? 0, `区域首领专属招式「${name}」。随本场状态进入对应工序，群攻按防御后比例结算。`]);
    }
...
const seedEpicForgeContent = async (pool) => {
    ...
    for (const [bossCode, recipes] of recipesByBoss) {
        ...
        for (const recipe of recipes)
            filtered.push({ code: recipe.blueprintCode, chance: epicBlueprintDropChance[recipe.category], min_quantity: 1, max_quantity: 1 });
...
const initializeSchema = async (pool) => {
    ...
    const [playerEventColumns] = await pool.execute('SHOW COLUMNS FROM player_events');
    ...
    for (const column of [
        ...
        if (!existingPlayerEventColumns.has(name))
            await pool.query(`ALTER TABLE player_events ADD COLUMN ${column}`);
    }
    const [playerEventIndexes] = await pool.execute('SHOW INDEX FROM player_events');
    ...
    for (const index of [
        ...
        if (!name)
            throw new Error(`无法识别玩家事件索引：${index}`);
        if (!existingPlayerEventIndexes.has(name))
            await pool.query(`ALTER TABLE player_events ADD ${index}`);
    }
    const [playerEventConstraints] = await pool.execute("SELECT CONSTRAINT_NAME FROM information_schema.TABLE_CONSTRAINTS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='player_events' AND CONSTRAINT_NAME='fk_player_event_character' AND CONSTRAINT_TYPE='FOREIGN KEY'");
    if (!playerEventConstraints.length)
        await pool.query('ALTER TABLE player_events ADD CONSTRAINT fk_player_event_character FOREIGN KEY (character_id) REFERENCES characters(id) ON DELETE CASCADE');
    const [bountyColumns] = await pool.execute('SHOW COLUMNS FROM player_bounties');
    if (!bountyColumns.some(row => row.Field === 'instance_key'))
        await pool.query("ALTER TABLE player_bounties ADD COLUMN instance_key CHAR(36) NOT NULL DEFAULT '' AFTER bounty_id");
    ...
    await (await import('./monster-cards.js')).initializeMonsterCards(pool);
    ...
    try {
        await pool.query('ALTER TABLE characters ADD COLUMN guild_contribution BIGINT UNSIGNED NOT NULL DEFAULT 0');
    }
    catch (error) {
        if (error?.code !== 'ER_DUP_FIELDNAME')
            throw error;
  ...
  WHERE code IN ('armor_break','fireball','toxic_edge','bloodletting','frost_bind','jump_strike','thunder_lance','goblin_player_rockfall','goblin_player_soulscorch','war_cry','blessing_aegis')`);
    ...
    const [dualMasteryMigration] = await pool.query("SELECT 1 FROM game_data_migrations WHERE code='dual_weapon_mastery_panel_recalculation_v1' LIMIT 1");
    if (!dualMasteryMigration.length) {
        const [activeBattles] = await pool.query("SELECT 1 FROM combat_sessions WHERE state='active' UNION ALL SELECT 1 FROM player_pvp_battle_sessions WHERE state='active' LIMIT 1");
        ...
        WHERE c.npc_code IS NULL AND primary_item.weapon_type IS NOT NULL AND primary_item.weapon_type=secondary_item.weapon_type`);
            for (const character of characters)
                await recalculateCharacterStats(pool, Number(character.id));
    ...
    await (await import('./skill-access.js')).initializeSkillAccess(pool);
    await (await import('./active-folio-skills.js')).initializeActiveFolioSkills(pool);
    await (await import('./leaf-route.js')).initializeLeafRoute(pool);
    await refreshShopStocks(pool);
    await (await import('./finance.js')).initializeFinance(pool);
    await (await import('./map-descriptions.js')).initializeMapDescriptions(pool);
    await (await import('./travel-routes.js')).initializeTravelRoutes(pool);
    await (await import('./equipment-workshop.js')).initializeEquipmentWorkshop(pool);
};
