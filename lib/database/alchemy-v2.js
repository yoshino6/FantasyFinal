const initializeAlchemyV2 = async (pool) => {
    ...
    for (const column of ['main_quantity TINYINT UNSIGNED NOT NULL DEFAULT 1', 'auxiliary_quantity TINYINT UNSIGNED NOT NULL DEFAULT 1', 'reagent_quantity TINYINT UNSIGNED NOT NULL DEFAULT 1']) {
        try {
            await pool.query(`ALTER TABLE player_alchemy_formulas ADD COLUMN ${column}`);
        }
        catch (error) {
    ...
    try {
        await pool.query('ALTER TABLE player_skill_point_ledger ADD COLUMN refund_token VARCHAR(64) NULL');
    }
    catch (error) {
        if (error?.code !== 'ER_DUP_FIELDNAME')
            throw error;
