import { achievementRewardItems } from '../game/achievement-rewards.config.js';
...
const initializeAchievements = async (pool) => {
    for (const sql of achievementSchema)
        await pool.query(sql);
};
const seedAchievementProfiles = async (pool) => {
    ...
    await (await import('../game/achievement-boss.js')).loadBossAchievementDefinitions(pool);
    await (await import('../game/achievement-boss-reward-migration.js')).initializeBossAchievementRewards(pool);
    for (const item of achievementRewardItems)
    ...
    await (await import('../game/achievement.service.js')).backfillAchievementBoxRewards(pool);
};

export { achievementSchema, initializeAchievements, seedAchievementProfiles };
