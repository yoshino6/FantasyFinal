...
import { reservePortraitUpload, portraitScope } from '../game/automaton-portrait.service.js';
import { acceptPortraitImage } from '../game/automaton-portrait-upload.js';
...
var automatonPortraitUpload = async (_event, next) => {
    ...
    if (pendingHeart) {
        ...
        await message.send({ format: heartQuestionFormat(pendingHeart) });
        return;
    }
    const [message] = useMessage();
    let handled = false, passed = false, submitted = false;
    try {
        ...
        handled = true;
        if ('expired' in upload) {
            await message.send({ format: Format.create().addMarkdown(Format.createMarkdown().addText('上传已超时，请重新点击“更换形象”。')) });
            return;
        }
        if ('busy' in upload) {
            await message.send({ format: Format.create().addMarkdown(Format.createMarkdown().addText('上一张图片正在处理，请稍等。')) });
            return;
        }
        const result = await acceptPortraitImage(current.UserId, upload, media);
        submitted = true;
...
export { automatonPortraitUpload as default };
