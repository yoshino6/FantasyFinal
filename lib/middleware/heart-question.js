...
var heartQuestion = async (_event, next) => {
    const [event] = useEvent(), [route] = useRoute();
    if (!route.matched || isPriorityCommand(route.key) || /^(?:窥尘问心|问心选择|问心跳过)(?:$| )/.test(route.key)) {
        await next();
        return;
    }
    ...
    if (after)
        await message.send({ format: heartQuestionFormat(after) });
};

export { heartQuestion as default };
