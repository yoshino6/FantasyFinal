...
import { achievementBoxRewardForRarity, achievementBoxes, achievementRewardItemByKey, achievementBoxLoot } from './achievement-rewards.config.js';
...
import { achievementById, achievementThreshold, achievementAttributeKeys, completionPercent } from './achievement-rules.js';
...
const flushAchievements = async (connection, events) => {
    ...
    for (const actor of [...actors].sort((a, b) => String(a.qq_user_id).localeCompare(String(b.qq_user_id)))) {
        ...
        for (const event of events.filter(e => e.characterId === Number(actor.id))) {
            ...
            for (const fact of event.facts) {
                ...
                if (!definition || /^ACH_L2[1-5]$/.test(definition.id))
                    continue;
                const life = fact.life || definition.scope === '世' ? String(actor.id) : '';
                const [rows] = await connection.execute('SELECT value_json FROM achievement_progress WHERE identity_key=? AND life_key=? AND metric=? FOR UPDATE', [identity, life, fact.metric]);
                const state = json(rows[0]?.value_json) ?? { count: 0, seen: [] };
                if (fact.distinct !== undefined) {
                ...
                if (state.count >= achievementThreshold(definition.id) && !unlocked.has(definition.id)) {
                    ...
                    if (fact.cooperationKey && isCooperativeAchievement(definition.id)) {
                        const group = cooperation.get(definition.id) ?? new Map();
    ...
    for (const id of [...new Set([...candidates.values()].flatMap(s => [...s]))].sort()) {
        ...
        for (const identity of identities) {
            ...
            const [rewardReceipt] = await connection.execute('INSERT IGNORE INTO achievement_box_rewards(identity_key,achievement_id,reward_key,quantity) VALUES (?,?,?,?)', [identity, id, baseBox.key, baseBox.quantity]);
            if (rewardReceipt.affectedRows)
                await connection.execute('INSERT INTO achievement_rewards(identity_key,reward_key,quantity) VALUES (?,?,?) ON DUPLICATE KEY UPDATE quantity=quantity+VALUES(quantity)', [identity, baseBox.key, baseBox.quantity]);
            if (first)
            ...
            if (first)
                await connection.execute('INSERT INTO achievement_first_members(achievement_id,identity_key,name_snapshot) VALUES (?,?,?)', [id, identity, actor.name]);
            if (rank === 1) {
...
const unlockAccountAchievement = async (connection, identity, name, achievementId, eventKey) => {
    ...
    if (!definition || retiredAchievementIdSet.has(achievementId))
        throw new Error('成就配置尚未完成。');
    await connection.execute('INSERT IGNORE INTO achievement_profiles(identity_key) VALUES (?)', [identity]);
    await connection.execute('SELECT identity_key FROM achievement_profiles WHERE identity_key=? FOR UPDATE', [identity]);
    ...
    if (existing.length || !eventReceipt.affectedRows)
    ...
    const [rewardReceipt] = await connection.execute('INSERT IGNORE INTO achievement_box_rewards(identity_key,achievement_id,reward_key,quantity) VALUES (?,?,?,?)', [identity, achievementId, reward.key, reward.quantity]);
    if (rewardReceipt.affectedRows)
        await connection.execute('INSERT INTO achievement_rewards(identity_key,reward_key,quantity) VALUES (?,?,?) ON DUPLICATE KEY UPDATE quantity=quantity+VALUES(quantity)', [identity, reward.key, reward.quantity]);
    if (ordinal === 1) {
        await connection.execute("INSERT INTO achievement_rewards(identity_key,reward_key,quantity) VALUES (?,'rare_box',1) ON DUPLICATE KEY UPDATE quantity=quantity+1", [identity]);
...
const achievementListInDatabase = async (db, identity, category = '全部', requestedPage = 1) => {
    if (!achievementCategories.includes(category))
        throw new Error('未知成就分类。');
    if (!Number.isSafeInteger(requestedPage) || requestedPage < 1)
        throw new Error('页码须为正整数。');
    ...
    const entries = rows.flatMap(r => { const d = achievementById.get(String(r.achievement_id)); return d && (category === '全部' || d.category === category) ? [{ id: d.id, name: d.name, category: d.category, description: d.description, rarity: String(r.rarity), attribute: Object.keys(achievementAttributeKeys).find(k => achievementAttributeKeys[k] === r.reward_attribute) + '+' + r.reward_points, rank: Number(r.ordinal), percentage: completionPercent(Number(r.completed_count), Number(r.population)), completedAt: new Date(r.completed_at).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai', hour12: false }) }] : []; });
...
const achievementDetail = async (identity, id) => {
    ...
    await (await import('./achievement-boss.js')).loadBossAchievementDefinitions(db);
    if (!owned.length || !achievementById.has(id))
    ...
    for (let page = 1; page <= first.totalPages; page++) {
        const data = page === 1 ? first : await achievementList(identity, definition.category, page);
...
const syncAchievementRewardInventory = async (connection, identity) => {
    ...
    for (const key of keys) {
        ...
        ON DUPLICATE KEY UPDATE quantity=VALUES(quantity),trade_bound_quantity=0,personal_bound_quantity=VALUES(personal_bound_quantity),binding_revision=binding_revision+1,acquired_at=IF(quantity<VALUES(quantity),NOW(),acquired_at)`, [characterId, itemId, quantity, 0, quantity]);
            ...
            await connection.execute('DELETE FROM player_inventory WHERE character_id=? AND item_id=?', [characterId, itemId]);
    }
};
const achievementRewards = async (identity) => withTransaction(async (connection) => { await syncAchievementRewardInventory(connection, identity); return achievementRewardsInDatabase(connection, identity); });
const consumeAchievementRewardItem = async (connection, characterId, itemCode, quantity = 1) => {
    if (!achievementRewardItemByKey.has(itemCode))
        return;
    ...
    if (!paid.affectedRows)
        throw new Error('足迹中的永久道具库存不足，已取消使用。');
};
...
const openAchievementBoxInDatabase = async (connection, identity, boxKey, token, quantity = 1) => {
    ...
    if (!profile.length)
        throw new Error(`没有可打开的${box.name}。`);
    if (!Number.isSafeInteger(quantity) || quantity < 1 || quantity > 100)
        throw new Error('每次可打开1～100个，请输入整数。');
    ...
    if (receipt.length) {
        ...
        const result = old.items ? { ...old, boxKey, boxName: box.name } : { boxKey, boxName: box.name, quantity: 1, items: [{ ...old, quantity: 1 }] };
        if (result.quantity !== quantity)
            throw new Error('此凭据已用于其他数量，请重新查看背包获取新的打开按钮。');
        return result;
    }
    const [paid] = await connection.execute('UPDATE achievement_rewards SET quantity=quantity-? WHERE identity_key=? AND reward_key=? AND quantity>=?', [quantity, identity, boxKey, quantity]);
    if (!paid.affectedRows)
        throw new Error(`没有可打开的${box.name}。`);
    const [characters] = await connection.execute('SELECT c.id FROM players p JOIN characters c ON c.player_id=p.id WHERE p.qq_user_id=? AND c.npc_code IS NULL FOR UPDATE', [identity]);
    if (!characters[0])
        throw new Error('当前没有可接收道具的角色。');
    ...
    for (const item of items) {
        ...
        if (!definitions[0])
            throw new Error(`道具【${item.name}】尚未完成初始化，未消耗道具匣。`);
...
const backfillAchievementBoxRewards = async (pool) => {
    ...
    try {
        ...
        if (marker.affectedRows) {
            const [rows] = await connection.execute('SELECT identity_key,achievement_id,rarity FROM achievement_completions ORDER BY identity_key,achievement_id FOR UPDATE');
            for (const row of rows) {
                const reward = achievementBoxRewardForRarity(String(row.rarity));
                await connection.execute('INSERT IGNORE INTO achievement_box_rewards(identity_key,achievement_id,reward_key,quantity) VALUES (?,?,?,?)', [row.identity_key, row.achievement_id, reward.key, reward.quantity]);
                await connection.execute('INSERT INTO achievement_rewards(identity_key,reward_key,quantity) VALUES (?,?,?) ON DUPLICATE KEY UPDATE quantity=quantity+VALUES(quantity)', [row.identity_key, reward.key, reward.quantity]);
            }
        }
