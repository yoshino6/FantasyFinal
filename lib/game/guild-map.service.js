...
const guildMapRisk = (maxLevel) => maxLevel === null || !Number.isFinite(maxLevel) || maxLevel < 1 ? '待勘测' : maxLevel <= 30 ? '低危' : maxLevel <= 50 ? '中危' : '高危';
const guildMapContributionPrice = (risk) => risk === '安全区' || risk === '低危' ? 200 : risk === '中危' ? 1000 : risk === '高危' ? 10000 : null;
const guildMapCatalog = async (connection) => {
    const [rows] = await connection.execute(`SELECT r.code AS region_code,r.name AS region_name,r.description,
...
const ensureRegistrationMapExchange = async (connection, characterId) => {
...
export { ensureRegistrationMapExchange, guildMapCatalog, guildMapContributionPrice, guildMapRisk };
