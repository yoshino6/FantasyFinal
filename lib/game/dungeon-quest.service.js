...
import { constructionBlueprintCodes, blindBoxBlueprints, blueprintRecipeCode } from './deconstructor-catalog.js';
...
const characterFor = async (connection, qqUserId, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.id,c.name,c.level,c.copper_coins,c.current_region_id,c.pos_x,c.pos_y,c.pos_z,c.secondary_profession_code
...
const progressFor = async (connection, characterId, lock = false) => {
    const [rows] = await connection.execute(`SELECT p.stage,p.dungeon_id,q.status FROM player_dungeon_secret_progress p
...
const ownsItem = async (connection, characterId, code) => {
...
const syncOwnedPassProgress = async (connection, characterId, progress) => {
    if (!progress || Number(progress.stage) !== 3 || !await ownsItem(connection, characterId, passCode))
        return progress;
...
const requireAtNpc = async (connection, character, code) => {
    ...
    if (!rows[0])
        throw new Error('请先前往对应地点，再继续询问。');
};
const affinityFor = async (connection, characterId, npcCode) => {
...
const discoverDungeonEntrance = async (connection, characterId, dungeonId, regionId, posX, posY) => {
    const mapped = await ownsItem(connection, characterId, 'map_dark_forest');
    const [mark] = await connection.execute('INSERT IGNORE INTO player_dungeon_entrance_marks (character_id,dungeon_id,region_id,pos_x,pos_y) VALUES (?,?,?,?,?)', [characterId, dungeonId, regionId, posX, posY]);
    ...
    if (!progress) {
        ...
        if (mapped && Number(mark.affectedRows) > 0)
            await recordCharacterOperation(connection, { characterId, kind: 'exploration.dungeon_entrance_marked', source: { system: 'dungeon_entrance', id: `${characterId}:${dungeonId}`, step: 'marked' }, outcome: '标记', summary: '在地图上标记地下迷宫入口', detail: { dungeonId, regionId, posX, posY } });
    ...
    if (progress.dungeon_id && Number(progress.dungeon_id) !== dungeonId) {
        const [bound] = await connection.execute('SELECT state FROM dungeon_instances WHERE id=? LIMIT 1 FOR UPDATE', [progress.dungeon_id]);
...
const dungeonSecretProgress = async (qqUserId) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const progress = await syncOwnedPassProgress(pool, character.id, await progressFor(pool, character.id));
...
const secondaryProfessionGuide = async (qqUserId) => {
    const pool = await getPool();
    const [rows] = await pool.execute('SELECT c.level,c.secondary_profession_code FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1', [qqUserId]);
    return Number(rows[0]?.level ?? 0) >= 8 && !rows[0]?.secondary_profession_code;
};
const consultDungeonAtGuild = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    await requireAtNpc(connection, character, 'guild_counter');
    const progress = await progressFor(connection, character.id, true);
    if (!progress || Number(progress.stage) !== 1)
        throw new Error('你暂时没有需要向公会询问的地下秘密。');
...
const consultDungeonAtWorkshop = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    await requireAtNpc(connection, character, 'oddworkshop');
    const progress = await progressFor(connection, character.id, true);
    if (!progress || Number(progress.stage) !== 2)
        throw new Error('你暂时不需要向唯薇安询问迷宫。');
...
const completeDungeonSecretPurchase = async (connection, characterId) => {
    const progress = await progressFor(connection, characterId, true);
    if (progress && Number(progress.stage) === 3) {
        await connection.execute('UPDATE player_dungeon_secret_progress SET stage=4 WHERE character_id=?', [characterId]);
        await recordCharacterOperation(connection, { characterId, kind: 'quest.dungeon_secret_stage', source: { system: 'dungeon_secret', id: characterId, step: 'stage_4' }, outcome: '推进', summary: '取得破魔传送器', detail: { questCode, stage: 4 } });
    }
};
const studyWorkshopBlueprint = async (qqUserId, code) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    await requireAtNpc(connection, character, 'oddworkshop');
    if (character.secondary_profession_code !== 'deconstructor' || !constructionBlueprintCodes.has(code) && !blindBoxBlueprints.some(box => box.code === code))
        throw new Error('请在个人解构师研习面板选择可研习图纸。');
    const [items] = await connection.execute(`SELECT i.id,i.name,oi.buy_price,oi.stock_quantity FROM oddworkshop_items oi JOIN item_definitions i ON i.id=oi.item_id WHERE i.code=? AND oi.is_active=1 FOR UPDATE`, [code]);
    const item = items[0];
    if (!item)
        throw new Error('这件商品暂时没有摆上货架。');
    if (Number(item.stock_quantity) < 1)
        throw new Error('这件商品暂时售罄。');
    if (Number(character.copper_coins) < Number(item.buy_price))
        throw new Error(`铜币不足，还需要 ${Number(item.buy_price) - Number(character.copper_coins)} 铜币。`);
    if (code === passCode && await ownsItem(connection, character.id, passCode))
        throw new Error('你已经持有破魔传送器，无需重复购买。');
    const box = blindBoxBlueprints.find(entry => entry.code === code);
    if (box) {
        if (character.secondary_profession_code !== 'deconstructor')
            throw new Error('异械盲盒仅向解构师开放。');
        const [professionRows] = await connection.execute('SELECT level FROM player_secondary_professions WHERE character_id=? AND profession_code=\'deconstructor\' LIMIT 1', [character.id]);
        const deconstructorLevel = Number(professionRows[0]?.level ?? 0);
        if (deconstructorLevel < box.requiredLevel)
            throw new Error(`需要解构师等级 ${box.requiredLevel} 才能开启这只盲盒。`);
        const candidates = [];
        for (const recipeCode of box.outputs)
            if (!await ownsItem(connection, character.id, `${recipeCode}_blueprint`))
                candidates.push(recipeCode);
        if (!candidates.length)
            throw new Error('这档盲盒内的图纸你都已拥有，无需重复购买。');
        const rewardCode = candidates[Math.floor(Math.random() * candidates.length)];
        const [rewardRows] = await connection.execute('SELECT id,name FROM item_definitions WHERE code=? LIMIT 1', [`${rewardCode}_blueprint`]);
        if (!rewardRows[0])
            throw new Error('盲盒图纸定义尚未初始化，请稍后再试。');
    ...
    if (constructionBlueprintCodes.has(code) && await ownsItem(connection, character.id, code))
        throw new Error('这张图纸已经在你的背包中。');
    await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=?', [item.buy_price, character.id]);
    await connection.execute('UPDATE oddworkshop_items SET stock_quantity=stock_quantity-1 WHERE item_id=?', [item.id]);
    ...
    recordAchievement(connection, Number(character.id), [{ metric: 'ACH_K08', value: Number(item.buy_price), life: true }]);
    if (code === passCode)
        await completeDungeonSecretPurchase(connection, character.id);
...
const oddWorkshopDungeonCatalog = async (qqUserId) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const [rows] = await pool.execute(`SELECT i.code,i.name,i.description,i.item_category,oi.buy_price,oi.stock_quantity,COALESCE(pi.quantity,0) AS quantity
...
const entranceStory = async (qqUserId, dungeonId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const progress = await syncOwnedPassProgress(connection, character.id, await progressFor(connection, character.id, true));
    if (!progress || Number(progress.dungeon_id) !== dungeonId)
        return { stage: 0, text: '楼梯下的石门覆着一层淡银色结界。你的手才刚靠近，便被柔和却无法撼动的力量推开。' };
    const stage = Number(progress.stage);
    if (stage < 2)
        return { stage, text: '石门上的结界轻轻泛起波纹，像在无声拒绝来客。你想起公会的档案或许会知道这里的来历。' };
    if (stage < 4)
        return { stage, text: '结界仍横在石门前。唯薇安提过，只有破魔传送器才能让人安全穿过它。' };
    if (!await ownsItem(connection, character.id, passCode))
        return { stage, text: '你曾拿到过破魔传送器的线索，但背包里还没有那台装置。结界安静地挡在石门之外。' };
    if (stage === 4) {
...
const authorizeDungeonEntry = async (connection, characterId, dungeonId) => {
    const progress = await progressFor(connection, characterId, true);
    if (!progress || Number(progress.dungeon_id) !== dungeonId || Number(progress.stage) < 5)
        throw new Error('石门上的结界尚未解除。先完成支线【地下的秘密】的引导。');
    if (!await ownsItem(connection, characterId, passCode))
        throw new Error('缺少破魔传送器，无法穿过石门结界。');
    if (Number(progress.stage) === 5) {
        await connection.execute('UPDATE player_dungeon_secret_progress SET stage=6 WHERE character_id=?', [characterId]);
        await recordCharacterOperation(connection, { characterId, kind: 'quest.dungeon_secret_stage', source: { system: 'dungeon_secret', id: characterId, step: 'stage_6' }, outcome: '推进', summary: '进入地下迷宫', detail: { questCode, stage: 6, dungeonId } });
    }
};
const completeDungeonSecretForLeader = async (connection, characterIds, spawnIds) => {
    if (!characterIds.length || !spawnIds.length)
        return false;
    const [leaders] = await connection.execute(`SELECT dm.dungeon_id FROM dungeon_monsters dm WHERE dm.is_floor_leader=1 AND dm.spawn_id IN (${spawnIds.map(() => '?').join(',')}) LIMIT 1`, spawnIds);
    const dungeonId = Number(leaders[0]?.dungeon_id ?? 0);
    if (!dungeonId)
        return false;
    const [eligible] = await connection.execute(`SELECT p.character_id FROM player_dungeon_secret_progress p JOIN player_side_quests q ON q.character_id=p.character_id AND q.quest_code=? WHERE p.dungeon_id=? AND p.stage=6 AND p.character_id IN (${characterIds.map(() => '?').join(',')}) FOR UPDATE`, [questCode, dungeonId, ...characterIds]);
    ...
    for (const row of eligible)
        await recordCharacterOperation(connection, { characterId: Number(row.character_id), kind: 'quest.dungeon_secret_completed', source: { system: 'dungeon_secret', id: row.character_id, step: 'completed' }, actorRole: 'system', outcome: '完成', summary: '击败迷宫层主，解开地下的秘密', detail: { questCode, dungeonId, spawnIds } });
    return true;
};
const markedDungeonEntrances = async (qqUserId, regionCode) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    if (regionCode !== 'dark_forest')
        return [];
    const progress = await progressFor(pool, character.id);
    const [rows] = await pool.execute(`SELECT d.id,m.pos_x AS x,m.pos_y AS y FROM player_dungeon_entrance_marks m
...
const buyOddWorkshopItem = async (user, code) => {
    const { buySecondaryFinished } = await import('./secondary-shop.service.js');
    const pool = await getPool();
    const [rows] = await pool.execute('SELECT id FROM item_definitions WHERE code=?', [code]);
    if (!rows[0])
        throw new Error('该成品不存在。');
    const result = await buySecondaryFinished(user, 'oddworkshop', Number(rows[0].id));
    return { name: result.name, price: result.price, rewardName: undefined };
};

export { authorizeDungeonEntry, buyOddWorkshopItem, completeDungeonSecretForLeader, completeDungeonSecretPurchase, consultDungeonAtGuild, consultDungeonAtWorkshop, discoverDungeonEntrance, dungeonSecretProgress, entranceStory, markedDungeonEntrances, oddWorkshopDungeonCatalog, secondaryProfessionGuide, studyWorkshopBlueprint };
