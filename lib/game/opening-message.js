...
const openingFormat = (story) => {
    ...
    if (story.state === 'choice') {
        for (const choice of story.choices) {
            md.addNewline().addNewline().addBold(`${choice.code} · ${choice.label}`);
            buttons.addRow().addButton(`选择 ${choice.code}`, `/初行选择 ${story.revision} ${choice.code}`, { type: 'command', autoEnter: true, style: 'blue' });
        }
    }
    else if (story.state === 'armed')
        buttons.addRow().addButton('打开面板', '/面板', { type: 'command', autoEnter: true, style: 'blue' });
    else if (story.state === 'completed')
        buttons.addRow().addButton('进入公会', '/初行公会', { type: 'command', autoEnter: true, style: 'blue' }).addButton('未解之事', '/初行见闻', { type: 'command', autoEnter: true });
    else {
