...
import { financeFactions, financeFaction } from './finance-content.js';
import { financePeriodKey, nextFinancePeriod, previousFinancePeriod, financeBusinessDate, stableFinancePrices, tryConstrainedFinancePrices } from './finance-math.js';
import { npcTargetShares } from './finance-npc.js';
import { publicFinanceNews, financeNewsPriceNote, financeRedironEncounterEligible, financePriceDirectionMatched, financeRumorPairs, financeRumorPairIndex } from './finance-policy.js';
...
const assertFinanceShareSupply = async (connection, code) => {
    const [rows] = await connection.execute(`SELECT i.total_shares,i.treasury_shares,n.shares npc_shares,
    ...
    const row = rows[0];
    if (!row || numeric(row.total_shares) !== numeric(row.treasury_shares) + numeric(row.npc_shares) + numeric(row.player_shares))
        throw new Error(`${code}的发行份额与持仓账不平，交易已回滚。`);
};
const refreshFinanceMissions = async () => {
    const pool = await getPool();
    const date = financeBusinessDate();
    for (const faction of financeFactions)
        await pool.execute(`INSERT IGNORE INTO finance_missions
...
const recordFinanceSignal = async (connection, input) => {
    ...
    if (!score)
        return;
    const [gate] = await connection.execute("SELECT copper FROM finance_pools WHERE code='signal_gate' FOR UPDATE");
    if (!gate.length)
        throw new Error('证券经营信号闸门尚未初始化。');
    ...
    if (input.characterId !== null) {
        ...
        if (score > 0 && input.sourceType === faction.source) {
            const [accepted] = await connection.execute('SELECT 1 FROM finance_mission_acceptances WHERE business_date=? AND instrument_code=? AND character_id=? LIMIT 1', [date, faction.code, input.characterId]);
            if (accepted.length) {
                const [done] = await connection.execute('INSERT IGNORE INTO finance_mission_completions (business_date,instrument_code,character_id,source_key) VALUES (?,?,?,?)', [date, faction.code, input.characterId, sourceKey]);
        ...
        const [count] = await connection.execute(`SELECT COUNT(*) n FROM finance_signals
      WHERE character_id=? AND instrument_code=? AND business_date=? AND score ${score > 0 ? '>0' : '<0'} FOR UPDATE`, [input.characterId, faction.code, date]);
        if (numeric(count[0]?.n) >= (score > 0 ? 1 : 3))
            return;
    }
    const [insert] = await connection.execute(`INSERT IGNORE INTO finance_signals
    (source_key,instrument_code,character_id,business_date,period_key,event_type,source_type,score) VALUES (?,?,?,?,?,?,?,?)`, [sourceKey, faction.code, input.characterId, date, period, input.eventType, input.sourceType ?? null, score]);
    if (!insert.affectedRows)
        return;
};
const recordFinanceMarketTrade = async (connection, tradeId, sellerId, itemId) => {
    const [rows] = await connection.execute('SELECT item_category FROM item_definitions WHERE id=?', [itemId]);
    ...
    if (faction)
        await recordFinanceSignal(connection, { sourceKey, factionCode: faction[0], characterId: sellerId, eventType: 'market.trade', sourceType: faction[1], score: 2 });
};
const recordFinanceWorldEvent = async (connection, ledgerId, eventType, outcome, actorId, regionId, payload) => {
    if (actorId === null || regionId === null)
        return;
    ...
    if (!siteCommission && !encounter)
        return;
    const [regions] = await connection.execute('SELECT code FROM map_regions WHERE id=?', [regionId]);
    const region = String(regions[0]?.code ?? '');
    const sourceKey = `world:${ledgerId}`;
    if (encounter) {
        if (region === 'rediron_pass' && financeRedironEncounterEligible(payload))
            await recordFinanceSignal(connection, { sourceKey, factionCode: 'rediron_caravan', characterId: actorId, eventType, sourceType: 'rediron_encounter', score: 2 });
        return;
    }
    ...
    if (region === 'world_tree' || region === 'worldtree_meadow')
        await recordFinanceSignal(connection, { sourceKey, factionCode: 'worldtree_covenant', characterId: actorId, eventType, sourceType: 'worldtree_commission', score: 2 });
    if (region === 'mistalgae_marsh')
        await recordFinanceSignal(connection, { sourceKey, factionCode: 'mistalgae_ferrymen', characterId: actorId, eventType, sourceType: 'mistalgae_commission', score: 2 });
};
const recordFinanceCombatOutcome = async (connection, sessionId, outcome, members) => {
    for (const member of members) {
        ...
        const [regions] = await connection.execute('SELECT code FROM map_regions WHERE id=?', [member.current_region_id]);
        const region = String(regions[0]?.code ?? '');
        ...
        if (regional)
            await recordFinanceSignal(connection, { sourceKey, factionCode: regional, characterId: Number(member.id), eventType: `combat.${outcome}`, score });
    }
};
const planNews = async (connection, period) => {
    const [existing] = await connection.execute('SELECT 1 FROM finance_news WHERE period_key=? LIMIT 1', [period]);
    if (existing.length)
        return;
    ...
    const [prices] = await connection.execute("SELECT code,total_shares,price_milli FROM finance_instruments WHERE status='open' ORDER BY code");
    const [anchors] = await connection.execute('SELECT instrument_code,price_milli FROM finance_daily_anchors WHERE business_date=?', [nextPeriod.slice(0, 8)]);
    const anchorByCode = new Map(anchors.map(row => [row.instrument_code, numeric(row.price_milli)]));
    const [capitalRows] = await connection.execute("SELECT copper FROM finance_pools WHERE code IN ('clearing','npc_capital')");
    const capital = capitalRows.reduce((sum, row) => sum + numeric(row.copper), 0);
    const [holdings] = await connection.execute('SELECT instrument_code,SUM(shares) shares FROM finance_holdings WHERE shares>0 GROUP BY instrument_code');
    ...
    const chosenPair = Array.from({ length: pairs.length }, (_, offset) => pairs[(start + offset) % pairs.length]).find(pair => {
        const effects = pair.map((code, index) => [code, (directions[index] * (outcome === 'fulfilled' ? 1 : -1))]);
...
const rebalanceNpcPortfolio = async (connection, period, items) => {
    const [pools] = await connection.execute("SELECT code,copper FROM finance_pools WHERE code IN ('clearing','npc_capital') ORDER BY code FOR UPDATE");
    if (pools.length !== 2)
        throw new Error('域民投资资本或清算准备金尚未初始化。');
    ...
    for (const item of items) {
        const [positions] = await connection.execute('SELECT shares FROM finance_npc_portfolio WHERE instrument_code=? FOR UPDATE', [item.code]);
        ...
        if (delta > 0) {
            const [inventory] = await connection.execute('SELECT treasury_shares FROM finance_instruments WHERE code=?', [item.code]);
...
const settleOneFinancePeriod = async (period) => {
    const previous = previousFinancePeriod(period);
    await withTransaction(async (connection) => {
        const [job] = await connection.execute('INSERT IGNORE INTO finance_jobs (job_key) VALUES (?)', [`price:${period}`]);
        if (!job.affectedRows)
            return;
        const [instruments] = await connection.execute("SELECT code,total_shares,price_milli FROM finance_instruments WHERE status='open' ORDER BY code FOR UPDATE");
        const [gate] = await connection.execute("SELECT copper FROM finance_pools WHERE code='signal_gate' FOR UPDATE");
        if (!gate.length)
            throw new Error('证券经营信号闸门尚未初始化。');
        const [signals] = await connection.execute('SELECT instrument_code,COALESCE(SUM(score),0) score FROM finance_signals WHERE period_key=? GROUP BY instrument_code', [previous]);
        const [news] = await connection.execute('SELECT id,instrument_code,direction,outcome FROM finance_news WHERE period_key=?', [previous]);
        const [signalFactors] = await connection.execute(`SELECT instrument_code,event_type,source_type,COUNT(*) count,SUM(score) score
      FROM finance_signals WHERE period_key=? GROUP BY instrument_code,event_type,source_type`, [previous]);
        ...
        const [opening] = await connection.execute('SELECT instrument_code,price_milli FROM finance_price_history WHERE period_key=?', [openingPeriod]);
        ...
        const [anchors] = await connection.execute('SELECT instrument_code,price_milli FROM finance_daily_anchors WHERE business_date=?', [date]);
        const anchorByCode = new Map(anchors.map(row => [row.instrument_code, numeric(row.price_milli)]));
        const scores = new Map();
        for (const row of signals)
            scores.set(row.instrument_code, Math.min(10, Math.max(-10, numeric(row.score))));
        for (const row of news)
            scores.set(row.instrument_code, (scores.get(row.instrument_code) ?? 0) + numeric(row.direction) * (row.outcome === 'fulfilled' ? 3 : -2));
        ...
        const directions = new Map(news.map(row => [row.instrument_code, (numeric(row.direction) * (row.outcome === 'fulfilled' ? 1 : -1))]));
        const constrained = news.length ? tryConstrainedFinancePrices(priceInputs, directions) : base;
        const [money] = await connection.execute("SELECT code,copper FROM finance_pools WHERE code IN ('clearing','npc_capital') ORDER BY code FOR UPDATE");
        if (money.length !== 2)
            throw new Error('域民投资资本或清算准备金尚未初始化。');
        const [holdings] = await connection.execute(`SELECT h.instrument_code,SUM(h.shares) shares,i.price_milli
      FROM finance_holdings h JOIN finance_instruments i ON i.code=h.instrument_code WHERE h.shares>0 GROUP BY h.instrument_code,i.price_milli`);
        ...
        for (const row of news)
            if (!instruments.some(item => item.code === row.instrument_code))
...
const settleFinancePeriod = async () => {
    const current = financePeriodKey();
    const pool = await getPool();
    const [jobs] = await pool.execute("SELECT job_key FROM finance_jobs WHERE job_key LIKE 'price:%' ORDER BY job_key DESC LIMIT 1");
    let next = jobs[0] ? nextFinancePeriod(jobs[0].job_key.slice(6)) : current;
    let earliest = current;
    for (let i = 0; i < 41; i++)
        earliest = previousFinancePeriod(earliest);
    if (next < earliest) {
        await pool.execute('INSERT IGNORE INTO finance_jobs (job_key) VALUES (?)', [`catchup-skip:${next}:${earliest}`]);
        next = earliest;
    }
    for (; next <= current; next = nextFinancePeriod(next))
        await settleOneFinancePeriod(next);
    await refreshFinanceMissions();
};
const financeNewsBoard = async (qqUserId) => {
    const period = financePeriodKey(), previous = previousFinancePeriod(period), pool = await getPool();
    const [characters] = await pool.execute(`SELECT c.id,c.current_region_id,c.pos_x,c.pos_y,c.pos_z FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1`, [qqUserId]);
    const character = characters[0];
    if (!character)
        throw new Error('请先创建角色。');
    const [atExchange] = await pool.execute('SELECT 1 FROM map_npcs WHERE code=\'canopy_exchange\' AND region_id=? AND pos_x=? AND pos_y=? AND pos_z=? LIMIT 1', [character.current_region_id, character.pos_x, character.pos_y, character.pos_z]);
    if (!atExchange.length)
        throw new Error('请到万叶联市的闲聊区听取消息。');
    const [oldNews] = await pool.execute('SELECT id,instrument_code,direction,scenario_no,outcome,price_status,price_reason FROM finance_news WHERE period_key=? ORDER BY id', [previous]);
    const [priceRows] = await pool.execute('SELECT instrument_code,price_milli,previous_price_milli,factors_text FROM finance_price_history WHERE period_key=?', [period]);
    ...
    const history = publicFinanceNews(previous, oldNews).map(row => {
        const faction = financeFaction(row.instrument_code);
        const scene = row.direction > 0 ? faction?.rise[numeric(row.scenario_no)] : faction?.fall[numeric(row.scenario_no)];
    ...
    const heard = await withTransaction(async (connection) => {
        const [rows] = await connection.execute('SELECT news_id FROM finance_news_heard WHERE character_id=? AND period_key=? FOR UPDATE', [character.id, period]);
        const [candidates] = await connection.execute('SELECT id FROM finance_news WHERE period_key=? ORDER BY id', [period]);
        const publicCandidates = publicFinanceNews(period, candidates);
        if (rows.length)
            return publicCandidates.some(row => numeric(row.id) === numeric(rows[0].news_id)) ? rows[0].news_id : null;
    ...
    let prophecy = null;
    if (heard) {
        const [rows] = await pool.execute('SELECT instrument_code,direction,scenario_no FROM finance_news WHERE id=? AND period_key=?', [heard, period]);
        const row = rows[0];
        if (row) {
            const faction = financeFaction(row.instrument_code);
            const scene = row.direction > 0 ? faction?.rise[numeric(row.scenario_no)] : faction?.fall[numeric(row.scenario_no)];
            if (scene)
                prophecy = { faction: faction.name, text: scene.prophecy };
        }
    }
    return { period, history, prophecy, moves, warnings };
};

export { assertFinanceShareSupply, financeBusinessDate, financeNewsBoard, financePeriodKey, nextFinancePeriod, previousFinancePeriod, recordFinanceCombatOutcome, recordFinanceMarketTrade, recordFinanceSignal, recordFinanceWorldEvent, refreshFinanceMissions, settleFinancePeriod, stableFinancePrices };
