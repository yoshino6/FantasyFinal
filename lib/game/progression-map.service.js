...
const progressionMapRegions = (character) => {
    if (!character.adventurer_registered || character.opening_state && character.opening_state !== 'completed')
        return [];
    const regions = new Set(['world_tree', 'worldtree_meadow', 'dark_forest', 'baina_town']);
    for (const code of [character.region_code, character.destination_code])
        if (code && Object.hasOwn(openingHubs, code))
            regions.add(code);
    if (character.goblin_stage > 0 || character.realm_stage >= 3)
        regions.add('dark_forest_deep');
    if (character.advanced_trial && character.advanced_profession_code) {
        ...
        for (const code of profession?.route.maps ?? [])
            regions.add(code);
    }
    return [...regions];
};
const ensureMapRegions = async (connection, characterId, regions) => {
    const result = { granted: [], stored: [], unavailable: [] };
    if (!regions.length)
        return result;
    await connection.execute('SELECT id FROM characters WHERE id=? FOR UPDATE', [characterId]);
    for (const code of new Set(regions)) {
        const [items] = await connection.execute(`SELECT i.id,i.name,r.is_enabled,r.is_owner_only FROM map_regions r
      ...
      WHERE r.code=? ORDER BY i.id LIMIT 1`, [code]);
        const item = items[0];
        if (!item?.id || !item.is_enabled || item.is_owner_only) {
            result.unavailable.push(String(item?.name ?? code));
            continue;
        }
        const [owned] = await connection.execute(`SELECT 1 FROM player_inventory WHERE character_id=? AND item_id=? AND quantity>0`, [characterId, item.id]);
        if (owned.length)
            continue;
        const [stored] = await connection.execute(`SELECT 1 FROM player_home_storage_items s JOIN player_homes h ON h.id=s.home_id
      WHERE h.character_id=? AND s.item_id=? AND s.quantity>0 LIMIT 1`, [characterId, item.id]);
        if (stored.length) {
            result.stored.push(String(item.name));
            continue;
        }
...
const ensureProgressionMaps = async (connection, characterId) => {
    const [rows] = await connection.execute(`SELECT c.adventurer_registered,c.realm_stage,r.code AS region_code,
...
const repairProgressionMaps = async (user) => {
    const [rows] = await (await getPool()).execute('SELECT c.id FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? AND c.npc_id IS NULL LIMIT 1', [user]);
    if (rows[0])
        return withTransaction(connection => ensureProgressionMaps(connection, Number(rows[0].id)));
    return { granted: [], stored: [], unavailable: [] };
};
const progressionMapReceipt = (result) => [
...
export { ensureMapRegions, ensureProgressionMaps, progressionMapReceipt, progressionMapRegions, repairProgressionMaps };
