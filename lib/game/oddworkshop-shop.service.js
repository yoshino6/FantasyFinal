...
const oddWorkshopSellCatalog = async (qqUserId, page = 1, keyword = '') => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const term = `%${keyword.trim()}%`;
    const [counts] = await pool.execute(`SELECT COUNT(*) AS total FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE ${workshopSellable}`, [character.id, term]);
    const paging = pageInfo(page, Number(counts[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT i.id,i.name,i.item_category,pi.quantity,CEIL(i.trade_price*1.25) AS sell_price
...
const sellOddWorkshopItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = validQuantity(quantity);
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT i.id,i.name,i.item_category,pi.quantity,CEIL(i.trade_price*1.25) AS sell_price
    ...
    const item = rows[0];
    if (!item)
        throw new Error('唯薇安只收购粒子、基材、构件与异械。');
...
export { oddWorkshopSellCatalog, sellOddWorkshopItem };
