...
import { openingReplay, saveOpeningReplay } from './opening-replay.js';
import { recordCharacterOperation } from './character-operation.service.js';

const openingExperienceShares = () => {
...
const grantOpeningExperience = async (c, id, stage) => {
    const [marked] = await c.execute('INSERT IGNORE INTO player_opening_services (character_id,code,uses) VALUES (?,?,0)', [id, `xp_${stage}`]);
    if (!marked.affectedRows)
        return 0;
    const [rows] = await c.execute('SELECT id,level,experience,realm_stage FROM characters WHERE id=? FOR UPDATE', [id]);
    const row = rows[0];
    const result = await (await import('./adventure.service.js')).awardRealmExperience(c, { id, level: Number(row.level), experience: Number(row.experience), realm_stage: Number(row.realm_stage) }, openingExperienceShares()[stage], { fixed: true });
    await (await import('./character.service.js')).recalculateCharacterStats(c, id);
    return result.experience;
};
const openingFollowupQuest = async (user) => {
    const pool = await getPool();
    const character = await openingCharacter(pool, user);
    const [stories] = await pool.execute("SELECT * FROM player_opening_stories WHERE character_id=? AND state='completed'", [character.id]);
    if (!stories[0])
        return null;
    ...
    const [keepsakes] = await pool.execute('SELECT code,record_json FROM player_opening_keepsakes WHERE character_id=?', [character.id]);
    const pending = keepsakes.find(r => !(typeof r.record_json === 'string' ? JSON.parse(r.record_json) : r.record_json)?.registered);
    const [world] = await pool.execute('SELECT aqua_stage,aqua_character_id FROM opening_world WHERE id=1');
    ...
    if (pending || aquaPending || !character.profession_code || Number(character.level) < 5) {
        const guild = await (await import('./opening-guild.service.js')).openingGuildView(user);
        if (!guild.at || !guild.inside)
            return { title: `【主线·${choice.quest}】`, description: `你已完成安全护送。接下来先进入${guild.hub.name}的公会，再与接待员办理交接与职业登记。`, action: { label: guild.at ? '[进入公会]' : '[前往当地公会]', command: guild.at ? '/初行入会' : '/初行公会' } };
    }
    if (pending)
        return { title: `【主线·${choice.quest}】`, description: `${choice.task}\n\n带着这次相遇留下的凭物，到安全公会的对应联络窗口完成交接。`, action: { label: '[办理剧情凭物]', command: `/初行凭物 ${pending.code}` } };
    if (aquaPending)
        return { title: '【主线·女神也要在地上生活】', description: '同行凭据已登记，陪阿库娅完成她在世界树的第一份工作。', action: { label: '[女神办事桌]', command: '/女神' } };
    if (!character.profession_code)
        return { title: '【主线·选择今后的手艺】', description: `${choice.quest}已经留下完整记录。先在当地公会选择主职业，领取适合自己的普通武器，再安排下一段旅途。`, action: { label: '[当地公会]', command: '/初行公会' } };
    if (Number(character.level) < 5)
        return { title: `【主线·${choice.quest}之后】`, description: '公会为你安排了安全区内的低级委托。检查目标、完成实际操作，再回来结算。无需回到出生的高危野外。', action: { label: '[查看初行委托]', command: '/初行委托' } };
    return null;
};
const newcomerJobs = [
...
const openingJob = async (user, revision) => withTransaction(async (c) => {
    if (revision !== undefined && (!Number.isSafeInteger(revision) || revision < 0))
        throw new Error('委托页码无效。');
    const character = await openingCharacter(c, user, true);
    const id = Number(character.id);
    if (revision !== undefined) {
        const replay = await openingReplay(c, id, 'job', revision);
        if (replay)
            return replay.result;
    }
    await requireGuildService(c, id);
    const [stories] = await c.execute("SELECT route_code FROM player_opening_stories WHERE character_id=? AND state='completed'", [id]);
    if (!stories[0])
        throw new Error('这份初行委托安排给已完成安全交接的新旅人。');
    if (!character.profession_code)
        throw new Error('先在公会选择主职业，再领取初行委托。');
    await c.execute("INSERT IGNORE INTO player_opening_services (character_id,code,uses) VALUES (?,'job_progress',0)", [id]);
    const [rows] = await c.execute("SELECT uses FROM player_opening_services WHERE character_id=? AND code='job_progress' FOR UPDATE", [id]);
    ...
    if (revision === step && Number(character.level) < 5) {
        if (step % 2 === 1) {
            ...
            await (await import('./adventure.service.js')).awardRealmExperience(c, { id, level: Number(character.level), experience: Number(character.experience), realm_stage: Number(character.realm_stage) }, Math.max(1, remaining), { fixed: true });
            await c.execute('UPDATE characters SET copper_coins=copper_coins+40 WHERE id=?', [id]);
            await (await import('./character.service.js')).recalculateCharacterStats(c, id);
            settled = true;
        }
        await c.execute("UPDATE player_opening_services SET uses=uses+1 WHERE character_id=? AND code='job_progress'", [id]);
        step++;
    }
    const [current] = await c.execute('SELECT level FROM characters WHERE id=?', [id]);
    const result = settled ? { title: job.title, text: `${job.done}\n\n【结算】经验已到账，铜币 ×40。${Number(current[0].level) >= 5 ? '你已经可以继续常规旅途，初行委托圆满结束。' : '可继续查看下一份初行委托。'}`, revision: step, action: undefined }
    ...
    if (step !== startingStep) {
        await saveOpeningReplay(c, id, 'job', startingStep, 'next', result);
        await recordCharacterOperation(c, { characterId: id, kind: settled ? 'opening.job_settled' : 'opening.job_accepted', source: { system: 'opening_job', id: id, step: String(startingStep) }, outcome: settled ? '结算' : '推进', summary: `初行委托「${job.title}」${settled ? '完成结算' : '进入下一阶段'}`, detail: { jobTitle: job.title, stepBefore: startingStep, stepAfter: step, settled, rewardCopper: settled ? 40 : 0 } });
    }
    return result;
});

export { grantOpeningExperience, newcomerJobs, openingExperienceShares, openingFollowupQuest, openingJob };
