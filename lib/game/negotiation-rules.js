...
const resolveNegotiationMove = (before, move, random = secureRandom, options = {}) => {
    ...
    if (move.type === 'talk') {
        ...
        if (move.preference === 'neutral')
            aggression *= 1 - clamp(Number(options.neutralGiftAggressionReductionPct ?? 0), 0, 100) / 100;
    }
    ...
    if (aggression > 0 && random() < aggression) {
        ...
        if (aggressionRetried && random() >= aggression)
            return { state, result: 'ongoing', protected: false, earned, refused: false, aggressionRetried, ...(failureEscalationIgnored ? { failureEscalationIgnored } : {}) };
        if (state.protection > 0) {
            state.protection--;
            return { state, result: 'ongoing', protected: true, earned, refused: false, ...(aggressionRetried ? { aggressionRetried } : {}), ...(failureEscalationIgnored ? { failureEscalationIgnored } : {}) };
        }
