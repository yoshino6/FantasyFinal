import { lowForgeMaterials, allocateLowMaterials, armorWorkshopNames } from './equipment-workshop-rules.js';
...
import { assertEquipmentEnchantmentTransferCompatible, transferEquipmentEnchantment } from './equipment-enchantment-lifecycle.js';
...
const rejectRetiredWorkshop = () => { throw new Error('此旧版执行指令已停用，请重新从锻造面板预览并确认。'); };
...
const learnXiaobeiCraftsmanship = async (qqUserId) => withTransaction(async (connection) => {
    ...
    const [skills] = await connection.execute('SELECT id,name,tier,learn_cost FROM skill_definitions WHERE code=\'craftsmanship\' LIMIT 1 FOR UPDATE');
    ...
    if (Number(character.skill_points) < cost)
        throw new Error(`技能点不足，学习「匠心」需要 ${cost} 点。`);
...
const fusionMaterials = async (qqUserId, equipmentCategory = '武器') => {
    const pool = await getPool();
    const characterId = await characterIdFor(pool, qqUserId);
    const [rows] = await pool.execute(`SELECT i.id,i.code,i.name,i.item_category,i.trade_price,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.quantity>0 AND i.item_type='material' AND i.item_category NOT IN ('货币','怪物卡片') ORDER BY i.item_category,i.name`, [characterId]);
...
const fuseWeapon = async (qqUserId, instanceId, materialId) => withTransaction(async (connection) => {
    ...
    const [materials] = await connection.execute(`SELECT i.id,i.code,i.name,i.item_category,i.trade_price,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.id=? AND i.item_type='material' AND i.item_category NOT IN ('货币','怪物卡片') FOR UPDATE`, [characterId, materialId]);
    const material = materials[0];
    if (!material || Number(material.quantity) < 1)
...
const weaponTypes = new Set(['长剑', '法杖', '法书', '法球', '匕首', '拳刃', '盾牌', '弓弩', '枪炮']);
...
const forgeMaterialTitle = (materialCode, level) => materialCode === 'root_heart' ? '根灵' : materialCode === 'river_shell' ? '河纹' : materialCode === 'tide_shell' ? '潮纹' : materialCode === 'living_wood' ? '活纹'
...
const forgeName = (subtype, level, effect, materialCode, category = '武器', rarity = '普通', budgetLevel = level) => {
...
const forgeState = async (qqUserId) => {
    ...
    if (requirements[0]?.code === 'living_wood') {
        const { result, remaining } = allocateLowMaterials(materials.map(m => ({ code: m.code, quantity: Number(m.quantity), selected: Number(m.selected_quantity ?? 0) })), requirements[0].quantity);
        requirements = remaining ? [{ ...requirements[0], name: '低级主材（活纹木胚、根心、河壳、潮壳可混用）' }, ...requirements.slice(1)] : [...result.map(r => ({ ...r, name: materials.find(m => m.code === r.code).name })), ...requirements.slice(1)];
    }
...
const addForgeMaterial = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => { if (!Number.isInteger(quantity) || quantity < 1)
    ...
    throw new Error('请选择背包中的材料。'); if (!lowForgeMaterials.includes(item.code))
    throw new Error('这里只能指定低级通用主材，提纯辅材按配方消耗。'); const [selected] = await connection.execute('SELECT quantity FROM player_forge_materials WHERE character_id=? AND item_id=? FOR UPDATE', [characterId, itemId]); if (Number(selected[0]?.quantity ?? 0) + quantity > Number(item.quantity))
...
const setForgeMaterial = async (qqUserId, itemId, quantity) => withTransaction(async (connection) => { if (!Number.isInteger(quantity) || quantity < 0)
    ...
    return;
} if (!lowForgeMaterials.includes(item.code))
    throw new Error('这里只能指定低级通用主材，提纯辅材按配方消耗。'); if (quantity > Number(item.quantity))
...
const craftForgeEquipment = async (qqUserId, _confirmed = false) => withTransaction(async (connection) => {
    ...
    if (requirements[0]?.code === 'living_wood') {
        const [choices] = await connection.execute('SELECT i.code,fm.quantity FROM player_forge_materials fm JOIN item_definitions i ON i.id=fm.item_id WHERE fm.character_id=?', [characterId]);
        const main = await (await import('./equipment-workshop.service.js')).workshopCosts(connection, characterId, [requirements[0]], choices.map(r => ({ code: r.code, quantity: Number(r.quantity) })));
        requirements = [...main.map(({ code, quantity }) => ({ code, quantity })), ...requirements.slice(1)];
    }
    const materials = await fixedTalentMaterials(connection, characterId, requirements, requirements.filter(r => r.code !== tierForgeMaterial(session.equipment_category, session.subtype, Number(session.target_level)) && !lowForgeMaterials.includes(r.code)).map(r => r.code), !currentSecondaryShop());
    ...
    if (risk && Math.random() * 100 >= success) {
        ...
        await recordCharacterOperation(connection, { characterId, kind: 'craft.forge_failed', source: { system: 'forge_attempt', id: randomUUID(), step: 'settled' }, outcome: '失败', summary: `打造${session.subtype}失败，材料已消耗`, detail: { category: session.equipment_category, subtype: session.subtype, targetLevel: Number(session.target_level), feeCopper: fee, materials: requirements } });
    ...
    const mainMaterial = level <= 20 ? requirements.filter(r => lowForgeMaterials.includes(r.code)).sort((a, b) => b.quantity - a.quantity + (b.quantity === a.quantity ? (lowForgeMaterials.indexOf(a.code) - lowForgeMaterials.indexOf(b.code)) : 0))[0].code : tierForgeMaterial(session.equipment_category, session.subtype, level);
    const name = forgeName(session.subtype, level, effect, mainMaterial, session.equipment_category, rarity);
    ...
    await (await import('./finance-settlement.js')).recordFinanceSignal(connection, { sourceKey: `forge:${instance.insertId}`, factionCode: 'smiths_association', characterId: Number(characterId), eventType: 'smith.forged', sourceType: 'smith_forge', score: 2 });
...
const reforgeEquipment = async (qqUserId, instanceId) => withTransaction(async (connection) => {
    rejectRetiredWorkshop();
    const characterId = await characterIdFor(connection, qqUserId, true);
    const profession = await blacksmithProgressFor(connection, characterId, true);
    ...
    const [coins] = await connection.execute('SELECT copper_coins FROM characters WHERE id=? FOR UPDATE', [characterId]);
    if (Number(coins[0]?.copper_coins ?? 0) < fee)
        throw new Error(`铜币不足，重铸手续费需要 ${fee} 铜币。`);
    for (const material of materials)
        await consumeTalentMaterial(connection, characterId, Number(material.item.id), material.quantity, 'craft', !currentSecondaryShop());
    await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=?', [fee, characterId]);
    ...
    const [instance] = await connection.execute('INSERT INTO player_item_instances (character_id,item_id,quality,durability,durability_max,effect_json,forge_primary_json) VALUES (?,?,?,100,100,?,?)', [characterId, definition.insertId, randomForgeQuality(), JSON.stringify(effect), JSON.stringify(primaryKeys)]);
    const enchantment = await transferEquipmentEnchantment(connection, instanceId, Number(instance.insertId));
    const [removed] = await connection.execute('DELETE FROM player_item_instances WHERE id=? AND character_id=?', [instanceId, characterId]);
    if (Number(removed.affectedRows) !== 1)
        throw new Error('原装备状态已经变化，本次重铸已回滚。');
    await recalculateCharacterStats(connection, characterId);
...
const blacksmithQuestFor = async (connection, characterId) => {
    const [rows] = await connection.execute("SELECT q.status,c.secondary_profession_code FROM characters c LEFT JOIN player_side_quests q ON q.character_id=c.id AND q.quest_code='blacksmith_apprentice' WHERE c.id=? FOR UPDATE", [characterId]);
    const row = rows[0];
    const [items] = await connection.execute("SELECT i.code,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code IN ('living_wood','beast_core') FOR UPDATE", [characterId]);
...
const epicTestForgeDraft = (recipe) => {
...
const blacksmithQuest = (qqUserId) => withTransaction(async (connection) => blacksmithQuestFor(connection, await characterIdFor(connection, qqUserId, true)));
...
const claimBlacksmithQuest = async (qqUserId) => withTransaction(async (connection) => {
    ...
    await recordCharacterOperation(connection, { characterId, kind: 'profession.blacksmith_claimed', source: { system: 'player_side_quests', id: characterId, step: 'blacksmith_apprentice_claimed' }, outcome: '转职', summary: '完成锻造师转职并领取小北的赠礼', detail: { questCode: 'blacksmith_apprentice', consumedItemIds: [Number(wood[0].item_id), Number(core[0].item_id)], giftItemId: Number(gift[0].id) } });
    return { name: '锻造师', characterName: character[0]?.name ?? '冒险者', giftName: '小北的赠礼' };
});

export { acceptBlacksmithQuest, addBlacksmithProficiency, addForgeMaterial, armorClassEffectText, blacksmithFusionEquipment, blacksmithMaxLevel, blacksmithProgress, blacksmithProgressFor, blacksmithQuest, blacksmithWeapons, characterIdFor, claimBlacksmithQuest, clearForgeMaterial, craftEpicForgeEquipment, craftForgeEquipment, craftsmanshipDurabilityLoss, craftsmanshipEffect, craftsmanshipPotionMultiplier, epicForgeBlueprints, epicForgePreview, epicTestForgeDraft, equipmentKind, forgeEquipmentCapsFor, forgeFee, forgeName, forgePrimaryKeys, forgeRarityWeights, forgeRequirements, forgeState, fuseWeapon, fusionMaterials, learnXiaobeiCraftsmanship, legendaryTestForgeDraft, pickRandomSecondaryAffix, randomForgeValue, randomSecondaryAffixPool, refineWeapon, refinementMaterials, reforgeEquipment, reforgeEquipmentList, reforgePreview, removeForgeMaterial, resetForgeSession, secondaryAffixCount, selectForgeCategory, selectForgeLevel, selectForgeSubtype, setForgeMaterial, specialAffixChance, tierForgeMaterial, xiaobeiCraftsmanshipStatus };
