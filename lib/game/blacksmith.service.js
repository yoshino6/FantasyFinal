...
import { forgedAffixCap, forgedEquipmentCaps, forgedEquipmentBase, forgeRarityMultiplier } from './constants.js';
import { attributes } from './types.js';
import { purifiedCraftMaterialValue, purifiedMaterialForArmor, purifiedCraftMaterialDisplayName } from './monster-crafting-material.service.js';
...
const armorClassEffects = {
    '布甲': '甲类影响：防御极低，极大提升命中、闪避与速度。',
    '皮甲': '甲类影响：防御很低，明显提升命中与速度。',
    '轻甲': '甲类影响：防御适中，属性较为平衡。',
    '重甲': '甲类影响：物理防御很高、魔法防御较高，略微降低闪避与速度。',
    '板甲': '甲类影响：双防极高，大幅降低命中、闪避与速度。'
};
const armorClassEffectText = (subtype) => armorClassEffects[subtype ?? ''] ?? '';
const forgeMaterialValue = {
    living_wood: 20, meteor_iron: 24, star_copper: 29, moon_silver: 35, sun_gold: 41,
    beast_bone: 4, beast_hide: 6, beast_tendon: 9, beast_core: 25, magic_wool: 12, magic_scale: 14,
    tide_shell: 20, goblin_scrap_iron: 18, goblin_drumhide: 26, goblin_shadowcloth: 35, goblin_blast_core: 32, goblin_earth_crystal: 42,
    ridge_core: 40, fire_crystal: 50, marsh_heart: 60, duskvein_crystal: 55, star_mud_core: 65, frost_crystal: 70, thunder_core: 70, eclipse_core: 70,
    spellcloth_bolt: 32, tanned_spirit_leather: 32, bone_steel_plate: 34, cast_shell_plate: 36, laminated_scale_plate: 38,
...
const blacksmithProgressFor = async (connection, characterId, lock = false) => {
    ...
    if (characters[0]?.secondary_profession_code !== 'blacksmith')
        return { isBlacksmith: false, level: 1, proficiency: 0, required: proficiencyRequired(1), bonus: 0 };
...
const craftsmanshipEffect = async (qqUserId) => {
    ...
    if (row.profession === 'alchemist')
        return { profession: '炼金师', level, percent, kind: 'potion', text: `[炼金师Lv.${level}]你产出的药品实际效果提升${percent}%` };
...
const fusionMaterials = async (qqUserId, _equipmentCategory = '武器') => {
    const pool = await getPool();
    const characterId = await characterIdFor(pool, qqUserId);
    const [rows] = await pool.execute(`SELECT i.id,i.code,i.name,i.item_category,pi.quantity,fe.effect_json,fe.description FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id LEFT JOIN blacksmith_fusion_material_effects fe ON fe.item_id=i.id WHERE pi.character_id=? AND pi.quantity>0 AND i.item_type='material' AND i.item_category<>'货币' ORDER BY i.item_category,i.name`, [characterId]);
    return rows.flatMap(row => {
        const profile = forgeMaterialProfiles[row.code];
        if (!profile)
            return [];
        ...
        return [{ id: Number(row.id), name: row.name, category: row.item_category, quantity: Number(row.quantity), effect: Object.fromEntries(keys.map(key => [key, upper])), description: `本次增量上限 +${upper}，实际值按正态分布抽取。${elemental ? '单元素上限每 10 级 +11。' : ''}` }];
    });
};
...
const fuseWeapon = async (qqUserId, instanceId, materialId) => withTransaction(async (connection) => {
    ...
    const [materials] = await connection.execute(`SELECT i.id,i.code,i.name,i.item_category,pi.quantity,fe.effect_json,fe.description FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id LEFT JOIN blacksmith_fusion_material_effects fe ON fe.item_id=i.id WHERE pi.character_id=? AND i.id=? AND i.item_type='material' AND i.item_category<>'货币' FOR UPDATE`, [characterId, materialId]);
    const material = materials[0];
    if (!material || Number(material.quantity) < 1)
        throw new Error('请选择背包中的熔铸材料。');
    const profile = forgeMaterialProfiles[material.code];
    if (!profile)
        throw new Error(`熔铸材料【${material.name}】尚未配置锻造倾向。`);
    if (material.code === 'goblin_colonel_insignia') {
        const [usedRows] = await connection.execute('SELECT COUNT(*) AS total FROM equipment_fusions WHERE instance_id=? AND material_item_id=?', [instanceId, material.id]);
        if (Number(usedRows[0]?.total ?? 0) > 0)
            throw new Error('同一件装备最多熔铸一枚上校军徽。');
    }
    const primaryKeys = forgePrimaryKeys(weapon.item_category, weapon.weapon_type);
    const current = jsonRecord(weapon.effect_json);
    const existingSecondary = Object.entries(current).filter(([key, value]) => !primaryKeys.includes(key) && typeof value === 'number' && Number(value) !== 0).length;
    ...
    for (const [key, value] of Object.entries(added))
        merged[key] = Math.round((Number(merged[key] ?? 0) + Number(value)) * 100) / 100;
...
const tierForgeMaterial = (level) => level <= 20 ? 'living_wood' : 'ridge_core';
const weaponCraftMaterial = (subtype) => subtype === '匕首' ? 'tanned_spirit_leather' : subtype === '法杖' || subtype === '法书' || subtype === '法球' ? 'spellcloth_bolt' : subtype === '盾牌' ? 'cast_shell_plate' : 'bone_steel_plate';
const forgeRequirements = (category, subtype, level) => {
    const base = tierForgeMaterial(level);
    if (category === '武器') {
        const quantity = { 15: 8, 20: 12, 25: 10, 30: 14 }[level];
        if (!quantity)
            throw new Error('当前仅开放 Lv.15、20、25、30 的常规打造。');
        return [{ code: base, quantity }, { code: weaponCraftMaterial(subtype), quantity: { 15: 2, 20: 3, 25: 3, 30: 4 }[level] }];
    }
    const armorType = ['轻甲', '皮甲', '重甲', '布甲', '板甲'].includes(subtype ?? '') ? subtype : '轻甲';
    const weights = {
        '头肩': [{ 15: 3, 20: 4, 25: 4, 30: 5 }[level] ?? 0, { 15: 1, 20: 1, 25: 1, 30: 2 }[level] ?? 0],
        '上装': [{ 15: 6, 20: 8, 25: 7, 30: 9 }[level] ?? 0, { 15: 2, 20: 2, 25: 2, 30: 3 }[level] ?? 0],
        '腰部': [{ 15: 3, 20: 3, 25: 3, 30: 4 }[level] ?? 0, 1],
        '下装': [{ 15: 5, 20: 7, 25: 6, 30: 8 }[level] ?? 0, { 15: 2, 20: 2, 25: 2, 30: 3 }[level] ?? 0],
        '脚部': [{ 15: 3, 20: 4, 25: 4, 30: 5 }[level] ?? 0, { 15: 1, 20: 1, 25: 1, 30: 2 }[level] ?? 0]
    };
    const [mainQuantity, craftQuantity] = weights[category] ?? [0, 0];
    if (!mainQuantity || !craftQuantity)
        throw new Error('当前仅开放 Lv.15、20、25、30 的常规打造。');
    return [{ code: base, quantity: mainQuantity }, { code: purifiedMaterialForArmor(armorType), quantity: craftQuantity }];
};
...
const baseForgeEffect = (category, subtype, level, rarity) => {
    const multiplier = forgeRarityMultiplier[rarity] ?? 1;
    const weaponBase = forgedEquipmentBase(level, '武器') * multiplier;
    const armorBase = forgedEquipmentBase(level, '防具') * multiplier;
    if (category === '武器') {
        if (subtype === '盾牌')
            return { physicalDefense: weaponBase, magicDefense: weaponBase * .5 };
        if (subtype === '法杖' || subtype === '法书' || subtype === '法球')
            return { magicAttack: weaponBase };
        if (subtype === '匕首')
            return { physicalAttack: weaponBase * .9, magicAttack: weaponBase * .9 };
        if (subtype === '拳刃')
...
const usableMaterialKeys = (profile, category, subtype, primaryKeys, level, rarity) => profileKeys(profile).filter(key => {
    ...
    return !offTypeWeaponAttack && (coreAttribute || primaryKeys.includes(key) || forgedAffixCap(equipmentKind(category, subtype), key, level, rarity) > 0);
});
...
const materialGain = (profile) => {
    if (profile.fixedValue !== undefined)
        return profile.fixedValue;
...
const equipmentKind = (category, subtype) => category === '武器' && subtype !== '盾牌' ? '武器' : '防具';
...
const forgeEquipmentCapsFor = (category, subtype, level, rarity, primaryKeys) => {
    const caps = forgedEquipmentCaps(equipmentKind(category, subtype), level, rarity, primaryKeys);
    if (category === '武器' && subtype === '盾牌') {
...
const selectForgeLevel = async (qqUserId, level) => withTransaction(async (connection) => { if (![15, 20, 25, 30].includes(level))
    throw new Error('当前仅开放 Lv.15、20、25、30 的常规打造。'); const characterId = await characterIdFor(connection, qqUserId, true); const [rows] = await connection.execute('SELECT subtype FROM player_forge_sessions WHERE character_id=? FOR UPDATE', [characterId]); if (!rows[0]?.subtype)
...
const clearForgeMaterial = async (qqUserId, itemId) => withTransaction(async (connection) => { const characterId = await characterIdFor(connection, qqUserId, true); await connection.execute('DELETE FROM player_forge_materials WHERE character_id=? AND item_id=?', [characterId, itemId]); });
const craftForgeEquipment = async (qqUserId, _confirmed = false) => withTransaction(async (connection) => {
    const characterId = await characterIdFor(connection, qqUserId, true);
    const profession = await blacksmithProgressFor(connection, characterId, true);
    const [sessions] = await connection.execute('SELECT equipment_category,subtype,target_level FROM player_forge_sessions WHERE character_id=? FOR UPDATE', [characterId]);
    ...
    const requiredCodes = requirements.map(item => item.code);
    const [requiredInventory] = await connection.execute(`SELECT i.id,i.code,i.name,i.item_category,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=?${requiredCodes.length ? ` AND i.code IN (${requiredCodes.map(() => '?').join(',')})` : ''} FOR UPDATE`, [characterId, ...requiredCodes]);
    const requiredByCode = new Map(requiredInventory.map(item => [item.code, item]));
    const missingRequirement = requirements.find(item => Number(requiredByCode.get(item.code)?.quantity ?? 0) < item.quantity);
    if (missingRequirement)
        throw new Error(`必备材料不足：${forgeMaterialName(missingRequirement.code)} 还需要 ${missingRequirement.quantity - Number(requiredByCode.get(missingRequirement.code)?.quantity ?? 0)} 份。`);
    const materials = requirements.map(requirement => ({ ...requiredByCode.get(requirement.code), selected_quantity: requirement.quantity }));
    const shortage = materials.find(item => Number(item.quantity) < Number(item.selected_quantity));
    if (shortage)
        throw new Error(`材料不足：${shortage.name}。`);
    const success = 100;
    ...
    for (const material of materials)
        await connection.execute('UPDATE player_inventory SET quantity=quantity-? WHERE character_id=? AND item_id=?', [material.selected_quantity, characterId, material.id]);
    ...
    const mainMaterial = tierForgeMaterial(level);
    ...
    const armorText = armorClassEffectText(session.subtype);
    ...
    const proficiencyGain = forgeProficiencyGain(session.equipment_category, level, rarity);
...
const loadEpicRecipeMaterials = async (connection, characterId, recipe, lock = false) => {
    ...
    const [rows] = await connection.execute(`SELECT i.code,i.name,pi.quantity FROM item_definitions i
    ...
    const inventory = new Map(rows.map(row => [row.code, { name: row.name, quantity: Number(row.quantity ?? 0) }]));
    return [{ code: recipe.blueprintCode, name: inventory.get(recipe.blueprintCode)?.name ?? `${recipe.name}图纸`, required: 1, owned: inventory.get(recipe.blueprintCode)?.quantity ?? 0 }, ...recipe.materials.map(material => ({ code: material.code, name: inventory.get(material.code)?.name ?? forgeMaterialName(material.code), required: material.quantity, owned: inventory.get(material.code)?.quantity ?? 0 }))];
};
...
const craftEpicForgeEquipment = async (qqUserId, blueprintCode) => withTransaction(async (connection) => {
    ...
    for (const material of materials)
        await connection.execute(`UPDATE player_inventory pi JOIN item_definitions i ON i.id=pi.item_id
    SET pi.quantity=pi.quantity-? WHERE pi.character_id=? AND i.code=?`, [material.required, characterId, material.code]);
    await connection.execute('DELETE FROM player_inventory WHERE character_id=? AND quantity<=0', [characterId]);
    await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=?', [fee, characterId]);
    const rarity = '史诗';
    const level = 30;
...
const claimBlacksmithQuest = async (qqUserId) => withTransaction(async (connection) => { const characterId = await characterIdFor(connection, qqUserId, true); const status = await blacksmithQuest(qqUserId); if (status.status !== 'completed')
    throw new Error('任务尚未完成。'); const [wood] = await connection.execute('SELECT pi.item_id FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code=\'living_wood\' FOR UPDATE', [characterId]); const [core] = await connection.execute('SELECT pi.item_id FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code=\'beast_core\' FOR UPDATE', [characterId]); if (!wood[0] || !core[0])
    throw new Error('任务材料已不在背包中。'); const [gift] = await connection.execute('SELECT id FROM item_definitions WHERE code=\'xiaobei_gift\' LIMIT 1', []); if (!gift[0])
