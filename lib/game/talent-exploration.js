...
import { ownedTalent, readTalentData, saveTalentData, talentWhole, talentDay } from './talent-data.js';
...
const scavengeNames = { beast_bone: '兽骨', beast_hide: '兽皮', beast_tendon: '兽筋', magic_wool: '魔绒', blood_residue: '血肉残渣' };
...
const talentScavenge = async (connection, characterId, source, destination) => {
    const talent = await ownedTalent(connection, characterId);
    ...
    if (source === 'move')
        ledger.moveTriggers = Number(ledger.moveTriggers ?? 0) + 1;
    else
        ledger.combatTriggers = Number(ledger.combatTriggers ?? 0) + 1;
    data.counters.scavengeTotal = Number(data.counters.scavengeTotal ?? 0) + quantity;
    await saveTalentData(connection, characterId, data);
    return [{ code, name: scavengeNames[code], quantity }];
};
