...
import { hiddenSkill, hiddenWeaponAttackPower } from './hidden-profession.config.js';
...
const twoTurnReady = (last, turn) => last === undefined || turn - last >= 2;
const recordInventorCapabilities = (state, turn, capabilities) => {
    ...
    if (state.inventorActions.length === 2 && recent.length >= 2)
        state.inventorLink = { capabilities: recent, until: turn + 2 };
};
const inherit = (r, u, options = {}) => {
    const state = hiddenState(u);
    if (state.profession === 'magical_scholar') {
        if (state.reviewTurn === r.turn)
            return;
        ...
        if (!twoTurnReady(state.weaponInheritanceTurn, r.turn))
            return;
        state.weaponInheritanceTurn = r.turn;
        const lastType = state.lastType ?? '';
        if (lastType)
            state.weaponSheath = { lastType, trait: options.trait, until: r.turn + 2 };
        reward(r, u, 15);
        r.log.push(`　➥归鞘余响：${u.name}恢复15器鸣，下一次更换器类可继承器性。`);
    }
    else if (state.profession === 'inventor') {
        if (!twoTurnReady(state.inventorInheritanceTurn, r.turn))
            return;
...
const planSucceeded = async (r, owner, target, plan, mode) => {
    ...
    if (!twoTurnReady(state.tacticianInheritanceTurn, r.turn))
        return;
    ...
    if (mode === 'guard') {
        ...
        if (removed.length)
            r.log.push(`　➥余策·守势：为【${target.name}】清除一项普通减益。`);
    }
    else if (mode === 'rescue') {
        const shield = r.status(target, 'shield');
        if (shield)
            shield.until = Math.max(shield.until, r.turn + 2);
...
const hiddenNativeDevice = (r, source, skill, before) => {
    ...
    if (effective) {
        ...
        recordInventorCapabilities(state, r.turn, [capability.primary]);
    }
};
const hiddenBeforeAction = async (r, u) => {
    ...
    if (state.observation && state.observation.until < r.turn)
        delete state.observation;
    if (state.review && state.review.until < r.turn)
        delete state.review;
    if (state.weaponSheath && state.weaponSheath.until < r.turn)
        delete state.weaponSheath;
    if (state.inventorLink && state.inventorLink.until < r.turn)
        delete state.inventorLink;
    if (state.inventorAcceptance && state.inventorAcceptance.until < r.turn)
        delete state.inventorAcceptance;
    if (state.inventorStandby && state.inventorStandby.until < r.turn)
        delete state.inventorStandby;
    if (state.freePlan && state.freePlan.until < r.turn)
        delete state.freePlan;
    if (state.remainder && state.remainder.until < r.turn)
...
const interrupt = async (r, source, target, plan) => {
    ...
    if (!immune && r.random() < .5 * (target.boss ? .4 : 1)) {
        ...
        reward(r, source, 20);
        await planSucceeded(r, source, target, plan, 'interrupt');
    ...
    if (direct) {
        ...
        if (plan && metadata(plan).mode === 'guard') {
            ...
            if (owner) {
                ...
                await r.removeEffect(target, plan);
                reward(r, owner, 20);
                await planSucceeded(r, owner, target, plan, 'guard');
            }
        }
    ...
    if (plan && metadata(plan).mode === 'rescue' && source && source.side !== target.side && damage > 0 && target.hp - damage <= target.hpMax * .4) {
        ...
        if (owner) {
            ...
            if (gained > 0 || absorbed > 0) {
                reward(r, owner, 20);
                await planSucceeded(r, owner, target, plan, 'rescue');
            }
        }
    }
    return damage;
...
const hiddenReorder = (r, queue, unit, bonus = false) => {
    ...
    const ordered = queue.map(entry => r.units.find(candidate => candidate.key === unit(entry).key)).filter((candidate) => Boolean(candidate));
...
const executeHiddenCombat = async (r, u, code, choice, ctx) => {
    ...
    const shortage = resourceCost === definition.resource ? hiddenResourceShortage(code, u.cooldowns) : state.resource < resourceCost ? `专属资源不足：需要 ${resourceCost}，当前 ${state.resource}/100。` : null;
    if (shortage)
    ...
    if (code === 'hidden_plan' && ownedPlans.length && !freeReplace)
        throw new HiddenBattleError('你已有尚未触发的预案。');
    if (code === 'hidden_plan' && r.effects(choice.mode === 'interrupt' ? enemy ?? u : ally).some(e => e.code === 'hidden_plan' && (!freeReplace || e.source !== u.key)))
    ...
    const acceptanceReady = (device) => !['hidden_transfer', 'hidden_debug', 'hidden_synergy'].includes(code) && selected.length === 1 && state.inventorAcceptance?.until !== undefined && state.inventorAcceptance.until >= r.turn && state.inventorAcceptance.devices.includes(device.id);
    const deviceEnergyCost = (device, skill) => (acceptanceReady(device) || state.inventorStandby?.device === device.id && state.inventorStandby.until >= r.turn)
    ...
    if (definition.profession === 'inventor') {
        ...
        for (const { device, skill } of selected) {
            ...
            if (device.energy + bonus < deviceEnergyCost(device, skill))
    ...
    if (freeReplace)
        for (const existing of ownedPlans)
            await r.removeEffect(existing.target, existing.item);
    ...
    state.active = code;
    state.consumes = resourceCost > 0;
    if (freeReplace)
        delete state.freePlan;
    if (remainderDiscount)
        delete state.remainder;
    ...
    r.log.push(`➤【${u.name}】施放「${definition.name}」 · MP −${mana}${resourceCost ? ` · 专属资源 −${resourceCost}` : ''}`);
    if (mix) {
        ...
        for (let index = 0; index < chosen.length; index++) {
            ...
            if (aoe)
                await r.areaDamage(targets, hitTarget);
            else
        ...
        if (effective && code === 'hidden_weapon_combo') {
            if (allEffective) {
                ...
                if (new Set(chosen.map(weapon => weapon.type)).size >= 2)
                    inherit(r, u, { trait: lastWeaponTrait });
            }
        }
    }
        ...
        for (const selection of selected) {
            ...
            for (const fragment of capability.fragments) {
                ...
                for (const target of targets.slice(0, limit)) {
                    if (fragment.kind === 'damage')
                        ...
                        await r.restore(u, target, amount, 0, true, code === 'hidden_synergy' ? .2 * target.hpMax : Infinity, true);
                        ...
                        if (!projected && state.inventorLink && state.inventorLink.until >= r.turn && !state.inventorLink.capabilities.includes(capability.primary) && target.side === u.side) {
                            ...
                            if (other) {
                                const beforeOther = other.hp;
                                await r.restore(u, other, amount * .6, 0, true);
                                projected = other.hp > beforeOther;
                            }
                        }
                    }
                        ...
                        effect(r, target, fragment.code, specializeEffectValue(fragment.code, fragment.value, stateScale * potency), duration, u, fragment.debuff, { untilHit: fragment.untilHit });
                        effective = true;
                        if (!projected && state.inventorLink && state.inventorLink.until >= r.turn && !state.inventorLink.capabilities.includes(capability.primary) && !fragment.untilHit && !['slow', 'bind', 'stun', 'freeze', 'sleep', 'petrify', 'charm', 'fear'].includes(fragment.code)) {
                            ...
                            if (projectionTarget) {
                                effect(r, projectionTarget, fragment.code, specializeEffectValue(fragment.code, fragment.value, stateScale * potency * .6), duration, u, fragment.debuff, { projected: true });
                                projected = true;
                            }
                        }
                    }
            ...
            if (effective) {
                ...
                state.lastCapability = capability.primary;
                if (acceptanceReady(device))
                    acceptedDevice = device.id;
            }
        }
        if (projected) {
            delete state.inventorLink;
            r.log.push(`　➥主脑链路：第三种能力的附属效果以60%投影。`);
        }
        if (acceptedDevice !== undefined) {
            const other = state.inventorAcceptance?.devices.find(deviceId => deviceId !== acceptedDevice);
            delete state.inventorAcceptance;
            if (other !== undefined) {
                state.inventorStandby = { device: other, until: r.turn + 2 };
                r.log.push(`　➥验收合格：异械${other}进入待机，下次能耗降低25%。`);
            }
        }
        if (code === 'hidden_synergy' && results.length === 2 && results.every(Boolean)) {
            ...
            if (lowest) {
                lowest.energy = Math.min(lowest.max, lowest.energy + 20);
                r.log.push(`　➥验收合格：${lowest.name}恢复20点能量。`);
            }
            for (const selection of selected) {
...
const weaponTraitDefinition = (weapon, potency) => {
    const duration = Math.min(3, Math.floor(2 * potency));
    if (['剑', '长剑', '单手剑', '双手剑', 'sword'].includes(weapon.type))
        return { code: 'armor_shatter', value: Math.min(20, 10 * potency), duration };
    if (['拳刃', '拳套', 'fist'].includes(weapon.type))
        return { code: 'hidden_outgoing', value: Math.min(16, 8 * potency), duration: 1 };
    if (['法书', '魔导书', 'book'].includes(weapon.type))
        return { code: 'magic_shatter', value: Math.min(20, 10 * potency), duration };
    if (['法球', 'orb'].includes(weapon.type))
        return { code: 'shield', value: Math.min(.08, .04 * potency), duration: 2, self: true };
    return undefined;
};
const applyInheritedWeaponTrait = async (r, u, target, trait) => {
    const value = trait.value * .6;
    if (trait.code === 'shield') {
        hiddenShield(r, u, u, u.hpMax * value, trait.duration);
        return;
    }
...
const weaponTrait = async (r, u, target, weapon, potency) => {
    const trait = weaponTraitDefinition(weapon, potency);
    if (!trait)
        return undefined;
    if (trait.code === 'shield')
        return hiddenShield(r, u, u, u.hpMax * trait.value, trait.duration) > 0 ? trait : undefined;
    if (!await r.control(u, target, trait.code, 100, trait.duration))
        return undefined;
...
const executeMix = async (r, u, mix, ally, enemy) => {
    ...
    const reviewedDuration = (duration) => { if (reviewReady && !reviewUsed) {
        reviewUsed = true;
        return duration + 1;
    } return duration; };
    ...
    for (const branch of mix.branches) {
        ...
        for (const target of friends) {
            ...
            if (branch.regeneration)
                queue(target, 'heal', total, reviewedDuration(mix.duration), '木相再生', great && mix.primary === branch.code ? allocation(target, 'shield', .06 * target.hpMax * branch.stateScale * common) : undefined);
            else if (total) {
                const immediate = total * (mix.counts.energy_ember ? .6 : 1), overflow = Math.max(0, immediate * r.healingMultiplier(u, target, true, true) - (target.hpMax - target.hp));
                await r.restore(u, target, immediate, 0, true, immediate * r.healingMultiplier(u, target, true, true), true);
            ...
            if (branch.shield) {
                ...
                if (!wasReviewed && reviewUsed && applied <= 0)
                    reviewUsed = false;
            }
    ...
    if (great) {
        ...
        if (mix.primary === 'light_element_dust')
            effect(r, ally, 'hidden_light', 1, reviewedDuration(mix.duration), u);
    ...
    if (reviewUsed) {
        delete state.review;
        if (great) {
            const gained = reward(r, u, 10);
            if (gained > 0)
                r.log.push(`　➥复盘回收：大成功额外获得${gained}实验值。`);
        }
    }
};
