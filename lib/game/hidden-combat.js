...
const inherit = (r, u) => {
    ...
    if (state.profession === 'magical_scholar') {
        ...
        if (lastType)
            state.weaponSheath = { lastType, until: r.turn + 2 };
...
const executeHiddenCombat = async (r, u, code, choice, ctx) => {
    ...
    const deviceEnergyCost = (device, skill) => state.inventorStandby?.device === device.id && state.inventorStandby.until >= r.turn
            ...
            for (const fragment of capability.fragments) {
                ...
                for (const target of targets.slice(0, limit)) {
                    if (fragment.kind === 'damage')
                        ...
                        await r.restore(u, target, Math.min(target.hpMax * fragment.percent / 100 * numeric * support, code === 'hidden_synergy' ? .2 * target.hpMax / Math.max(.01, r.healingMultiplier(u, target, true, true)) : Infinity), 0, true, code === 'hidden_synergy' ? .2 * target.hpMax : Infinity, true);
