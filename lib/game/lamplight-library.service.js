import { ownedTalent } from './talent-data.js';
import { experienceRequiredForLevel } from './constants.js';

const lamplightLibraryRooms = [
...
const lamplightLibraryAction = async (c, character, state, flags, action) => {
    if (state.phase !== 'join' || state.node_index !== 3 || Number(character.level) < 20)
        throw Error('先接到联合调查的图书馆任务，再来核验馆藏。');
    const [progress] = await c.execute("SELECT stage FROM player_main_quest_progress WHERE character_id=? AND quest_code='evolution_barrier'", [character.id]);
    const old = Number(progress[0]?.stage ?? 0);
    const index = Math.max(Number(flags.libraryRoom ?? 0), Math.min(4, Math.max(0, old - 1)));
    if (index >= 5)
        throw Error('五处调查已经完成，请回到主线提交记录。');
    ...
    const p = places[0];
    if (!p)
        throw Error('世界图书馆暂未开放，研究进度保留。');
    const [room, text] = lamplightLibraryRooms[index];
    if (action === 'library_go') {
        await (await import('./lamplight.service.js')).assertLamplightBoarding(c, Number(character.current_region_id));
        ...
        flags.libraryAt = room;
        flags.message = `你通过已核验的安全接驳抵达世界图书馆，沿导览进入${room}。下一步请调查这间房的线索。`;
        return;
    }
    if (Number(character.current_region_id) !== Number(p.region_id) || Number(character.pos_x) !== Number(p.pos_x) || Number(character.pos_y) !== Number(p.pos_y) || Number(character.pos_z) !== Number(p.pos_z) || flags.libraryAt !== room)
        throw Error(`请先前往世界图书馆·${room}。`);
    flags.libraryRoom = index + 1;
    delete flags.libraryAt;
    flags.message = text;
...
export { lamplightLibraryAction, lamplightLibraryRooms };
