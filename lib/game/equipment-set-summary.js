import { armorSetFromRows, armorSetDescription } from './armor-set.js';
import { epicLoadoutFromRows } from './epic-equipment.service.js';
import { epicSetProfile } from '../config/epic-forging.js';
...
const equipmentSetSummary = (items) => {
    const entries = [], armor = armorSetFromRows(items), epic = epicLoadoutFromRows(items);
    if (armor)
        entries.push({ title: `${armor.name} ${armor.count}/5件 · ${armor.tier}件效果`, effects: [armorSetDescription(armor)] });
