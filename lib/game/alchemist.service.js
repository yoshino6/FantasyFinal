...
const alchemistQuest = async (qqUserId) => withTransaction(async (connection) => {
    ...
    const [items] = await connection.execute(`SELECT pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code='healing_herb' FOR UPDATE`, [characterId]);
...
const acceptAlchemistQuest = async (qqUserId) => withTransaction(async (connection) => {
    ...
    if (!['accepted', 'claimed'].includes(before[0]?.status ?? ''))
        await recordCharacterOperation(connection, { characterId, kind: 'profession.alchemist_quest_accepted', source: { system: 'side_quest_change', id: randomUUID(), step: 'accepted' }, outcome: '接取', summary: '接取炼金师转职委托', detail: { questCode } });
});
...
const purificationMaterials = async (qqUserId) => {
    ...
    const [rows] = await pool.execute(`SELECT i.id,i.code,i.name,i.item_category,i.effect_json,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.quantity>0 AND i.item_type='material' AND i.item_category<>'怪物卡片' ORDER BY i.id`, [characterId]);
...
const tagSetFor = (item) => {
    ...
    if (/wood_element|木元素/.test(source))
        add('生机');
    if (/metal_element|土元素|土相/.test(source))
        add('韧护', '锋锐');
...
const alchemyMaterials = async (qqUserId) => {
    ...
    const characterId = await characterIdFor(pool, qqUserId);
    await activeAlchemistProgressFor(pool, qqUserId, characterId);
    const [rows] = await pool.execute('SELECT i.id,i.code,i.name,i.item_category,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.quantity>0 AND i.item_type=\'material\' AND i.item_category<>\'怪物卡片\' ORDER BY i.item_category,i.name,i.id', [characterId]);
...
const renameAlchemyFormula = async (qqUserId, formulaId, name) => withTransaction(async (connection) => {
    ...
    const [formulas] = await connection.execute('SELECT name FROM player_alchemy_formulas WHERE id=? AND character_id=? FOR UPDATE', [formulaId, characterId]);
    if (!formulas[0])
        throw new Error('未找到该快捷配方。');
    if (formulas[0].name === trimmed)
        return trimmed;
...
const executeAlchemy = async (qqUserId, token, origin = 'manual', note) => withTransaction(async (connection) => {
    ...
    if (successes > 0)
        await (await import('./finance-settlement.js')).recordFinanceSignal(connection, { sourceKey: `alchemy:${result.journalId}`, factionCode: 'alchemists_association', characterId: Number(characterId), eventType: 'alchemy.crafted', sourceType: 'alchemy_craft', score: 2 });
    await awardSecondaryShopCraftAffinity(connection, characterId);
    await completeCraftRequest(connection, characterId, token, result);
    if (!currentSecondaryShop())
