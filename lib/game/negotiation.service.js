...
import { monsterNegotiationProfile, monsterItemPreference } from '../config/monster-negotiation.js';
import { negotiationDialogue } from '../config/monster-negotiation-dialogues.js';
import { negotiationInventoryPage, classifyNegotiationItem } from './negotiation-item-policy.js';
import { moodBand, negotiationVersion, initialNegotiationState, synchronizeNegotiation, resolveNegotiationMove } from './negotiation-rules.js';
import { consumeInventory, consumeBinding } from './inventory-binding.js';
...
class NegotiationCombatError extends Error {
...
const readNegotiationReplay = async (connection, actorId, command) => {
    if (!command.sessionId || command.revision === undefined || command.type === 'view')
        return;
    ...
    if (!rows.length)
        return;
    if (Number(rows[0].actor_id) !== actorId || rows[0].request_key !== requestKey(actorId, command))
        throw new Error('交涉状态已改变，请刷新页面后重试。');
...
const assertNoNegotiation = async (connection, characterId, forestSpawnId) => {
    ...
    if (rows.length)
        throw new Error('你正在交涉中，请先回到交涉页面结束交涉。');
};
const assertMonsterNotNegotiating = async (connection, spawnIds) => {
    ...
    if (rows.length)
        throw new Error('该怪物正在与另一支队伍交涉，请稍后再试。');
};
...
const runNegotiation = async (connection, ctx, command, hooks) => {
    ...
    const life = lives[0];
    if (life.resolved)
        throw new Error('该怪物已经结束遭遇，不能重复交涉。');
    ...
    if (active && !parse(active.members_json).includes(ctx.actorId))
        throw new Error('对方正在应付另一队人，请稍后再试。');
    ...
    if (session && (session.battle_id ?? null) !== (ctx.battleId ?? null)) {
        if (session.state === 'active' || command.sessionId)
            throw new Error('遭遇已进入另一场战斗，请重新打开交涉页面。');
        await closeNegotiationSession(connection, session.id);
        session = undefined;
    }
    if (command.sessionId && !session)
        throw new Error('这张交涉页面已经失效，请重新打开交涉。');
    if (session && active && active.id !== session.id)
        throw new Error('队友已开始交涉，请刷新到当前会话。');
    if (!session) {
        if (command.type !== 'view')
            throw new Error('请先打开交涉页面。');
        const intro = negotiationDialogue(frozenProfile.family, state.mood, 'approach');
        if (await (await import('./divine-effects.js')).hasDivine(connection, ctx.actorId, 'talent_social_01'))
    ...
    if (!parse(session.members_json).includes(ctx.actorId))
        throw new Error('请打开你自己的交涉页面。');
    if (session.state === 'active' && JSON.stringify(parse(session.members_json)) !== JSON.stringify(ctx.memberIds))
        throw new Error('参与队伍已改变，请先结束原交涉。');
    ...
    if (blocked) {
        ...
        const eligibility = session.state === 'active' ? parse(session.stamina_json) : undefined;
        await closeNegotiationSession(connection, session.id, 'combat');
        for (const id of ctx.memberIds)
            await connection.execute(`INSERT INTO monster_negotiation_memories (spawn_id,character_id,state_json,combat_failed,first_failure_at) VALUES (?,?,?,1,NOW())
    ...
    if (command.type === 'view')
        return readNegotiationView(connection, ctx, session, state, command);
    if (Number(command.revision) !== Number(session.revision))
        throw new Error('交涉状态已改变，请刷新后重试。');
    if (['leave', 'fight'].includes(command.type) && ctx.actorId !== ctx.leaderId)
        throw new Error('只有队长可以结束交涉或主动开战。');
    if (session.state === 'preview' && ctx.actorId !== ctx.leaderId && !blocked)
        throw new Error('请先由队长发起第一项交涉动作。');
    if (!blocked && command.type === 'gift' && state.mood === 1_000_000) {
        ...
        if (policy?.usable && monsterItemPreference(frozenProfile, policy.subtype, items[0].code) === 'like')
            throw new Error('对方已充分接受你们的诚意，不再收取这份礼物。请点击「交谈」说定协议。');
    }
    if (session.state === 'preview' && command.type === 'leave' && !blocked) {
        await closeNegotiationSession(connection, session.id);
        const result = { kind: 'closed', spawnId: ctx.target.id, text: '你们暂时收起提议，对方仍保留先前的心情。' };
    ...
    if (session.state === 'preview' && !['leave'].includes(command.type)) {
        for (const id of ctx.memberIds)
            await assertNoNegotiation(connection, id);
        ...
        for (const id of ctx.memberIds)
            await connection.execute('INSERT INTO negotiation_participants (character_id,session_id) VALUES (?,?)', [id, session.id]);
    ...
    if (command.type === 'leave' || command.type === 'fight') {
        ...
        if (command.type === 'gift') {
            if (!Number.isSafeInteger(command.itemId) || !Number.isSafeInteger(command.quantity) || Number(command.quantity) < 1 || Number(command.quantity) > 999999)
                throw new Error('交付数量必须是 1～999999 的整数。');
            ...
            if (preference === 'like' && state.mood < 1_000_000)
                await consumeInventory(connection, ctx.actorId, Number(command.itemId), Number(command.quantity), true);
            else {
            ...
            move = { type: 'talk', charm: (await hiddenAttributesFor(connection, ctx.actorId)).charm };
        const before = state;
        ...
        if (outcome.aggressionRetried)
            Object.assign(usageAfter, command.type === 'talk' ? { talk: true } : { neutralGift: true });
        if (outcome.failureEscalationIgnored)
            usageAfter.firstProbe = true;
        if (outcome.aggressionRetried || outcome.failureEscalationIgnored)
        ...
        if (command.type === 'gift' && (outcome.refused || preference !== 'like')) {
            ...
            if (!refused.includes(ctx.actorId))
                refused.push(ctx.actorId);
        }
    ...
    for (const id of ctx.memberIds)
        await connection.execute(`INSERT INTO monster_negotiation_memories (spawn_id,character_id,state_json,combat_failed,first_failure_at) VALUES (?,?,?,?,${failed ? 'NOW()' : 'NULL'})
    ...
    if (kind === 'success') {
        if (state.achievementGiftRefusedBy?.includes(ctx.actorId))
            recordAchievement(connection, ctx.actorId, ['ACH_F06'], 'negotiation-refused:' + session.id);
        ...
        result = { kind: 'success', spawnId: ctx.target.id, text };
    }
    else if (kind === 'combat') {
        await closeNegotiationSession(connection, session.id, 'combat');
        ...
        await closeNegotiationSession(connection, session.id);
        result = { kind: 'closed', spawnId: ctx.target.id, text };
    }
    else {
    ...
    if (['talk', 'gift'].includes(command.type))
        recordAchievement(connection, ctx.actorId, ['ACH_F01']);
    await connection.execute('INSERT INTO negotiation_actions (session_id,revision,actor_id,request_key,result_json) VALUES (?,?,?,?,?)', [session.id, Number(command.revision), ctx.actorId, requestKey(ctx.actorId, command), JSON.stringify(result)]);
