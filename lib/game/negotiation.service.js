...
const assertNoNegotiation = async (connection, characterId) => {
    await (await import('./opening-state.js')).assertOpeningFree(connection, characterId);
...
const runNegotiation = async (connection, ctx, command, hooks) => {
    ...
    if (command.type === 'view' && !blocked)
        return readNegotiationView(connection, ctx, session, state, command);
    if (command.type !== 'view' && Number(command.revision) !== Number(session.revision))
    ...
    let failed = false;
    if (blocked) {
        const quote = negotiationDialogue(frozenProfile.family, state.mood, 'blocked', '', '', key);
        text = quote.text;
        key = quote.key;
        kind = 'combat';
        failed = true;
