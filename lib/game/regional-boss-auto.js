...
const planRegionalAuto = (state, members, turn) => {
    ...
    for (const member of automatic.filter(member => member.healingSuppressed))
        result.set(member.key, member.cleanse ?? (member.hp / member.hpMax < .45 ? { type: 'defend' } : { type: 'attack' }));
    ...
    for (const member of automatic) {
        ...
        if (!order)
            continue;
        if (member.healingSuppressed && member.cleanse) {
            result.set(member.key, member.cleanse);
            continue;
        }
