...
const characterFor = async (connection, qqUserId, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.id,c.adventurer_registered FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [qqUserId]);
    if (!rows[0])
...
const syncBountyBoard = async (connection) => {
    ...
    const [slots] = await connection.execute('SELECT slot_no FROM bounty_board_slots ORDER BY slot_no');
    ...
    const [candidates] = await connection.execute(`SELECT b.id FROM bounty_notices b
...
const refreshBounties = async (connection, _rollBossBounties = false) => {
...
const postBossBounty = async (bossCode) => {
    const pool = await getPool();
    const [result] = await pool.execute(`INSERT INTO bounty_notices (refresh_key,title,target_template_id,source_spawn_id,required_count,copper_reward,is_active,expires_at)
    SELECT CONCAT('boss-',s.id),CONCAT('紧急：讨伐',t.name),s.template_id,s.id,1,300+COALESCE(s.level,t.level)*60,1,DATE_ADD(NOW(),INTERVAL 3650 DAY)
    FROM monster_spawns s JOIN monster_templates t ON t.id=s.template_id
    ...
    if (!Number(result.affectedRows))
        throw new Error('当前 Boss 未刷新，无法上悬赏板。');
    ...
    const [rows] = await pool.execute(`SELECT b.id,b.title FROM bounty_notices b JOIN monster_templates t ON t.id=b.target_template_id
    WHERE t.code=? AND b.is_active=1 AND b.expires_at>NOW() ORDER BY b.id DESC LIMIT 1`, [bossCode]);
    return rows[0];
};
const bountyBoard = async (qqUserId, _requestedPage = 1, keyword = '') => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    await refreshBounties(pool);
    await syncBountyBoard(pool);
    const search = String(keyword).trim();
    const pageSize = 10;
    const searchSql = search ? ' AND (b.title LIKE CONCAT(\'%\',?,\'%\') OR t.name LIKE CONCAT(\'%\',?,\'%\'))' : '';
    const [rows] = await pool.execute(`SELECT b.id,bs.slot_no AS board_no,b.title,t.name AS target_name,b.required_count,b.copper_reward,b.source_spawn_id,r.name AS region_name,s.pos_x,s.pos_y,s.pos_z,pb.progress,pb.status
    ...
    const [activeRows] = await pool.execute(`SELECT COUNT(*) AS total FROM player_bounties pb
...
const playerBounties = async (qqUserId) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const [rows] = await pool.execute(`SELECT b.id,b.title,t.name AS target_name,b.required_count,b.copper_reward,b.source_spawn_id,r.name AS region_name,s.pos_x,s.pos_y,s.pos_z,pb.progress,pb.status,
...
const clearInvalidBounty = async (qqUserId, bountyId) => withTransaction(async (connection) => {
    ...
    if (!rows[0])
        throw new Error('没有找到这份悬赏。');
    if (!Number(rows[0].is_invalid))
        throw new Error('这份悬赏仍在进行，无法清除。');
    await connection.execute('DELETE FROM player_bounties WHERE character_id=? AND bounty_id=?', [character.id, bountyId]);
});
const abandonBounty = async (qqUserId, bountyId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT b.title,pb.instance_key FROM player_bounties pb JOIN bounty_notices b ON b.id=pb.bounty_id
    WHERE pb.character_id=? AND pb.bounty_id=? AND pb.status IN ('accepted','completed') FOR UPDATE`, [character.id, bountyId]);
    const bounty = rows[0];
    if (!bounty)
        throw new Error('没有找到可放弃的悬赏。');
    await connection.execute("DELETE FROM player_bounties WHERE character_id=? AND bounty_id=?", [character.id, bountyId]);
...
const abandonSecondaryQuest = async (qqUserId, questCode) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const [result] = await connection.execute("DELETE FROM player_side_quests WHERE character_id=? AND quest_code=? AND status IN ('accepted','completed')", [character.id, questCode]);
    if (!Number(result.affectedRows))
        throw new Error('没有找到可放弃的支线任务。');
...
const acceptBounty = async (qqUserId, bountyId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    if (!character.adventurer_registered)
        throw new Error('完成冒险者注册后才能接受悬赏。');
    await (await import('./guild-context.js')).requireGuildService(connection, Number(character.id));
    await refreshBounties(connection);
    await syncBountyBoard(connection);
    const [counts] = await connection.execute(`SELECT COUNT(*) AS total FROM player_bounties pb
    JOIN bounty_notices b ON b.id=pb.bounty_id LEFT JOIN monster_spawns s ON s.id=b.source_spawn_id
    WHERE pb.character_id=? AND ${occupiedBountySlot} FOR UPDATE`, [character.id]);
    if (Number(counts[0]?.total ?? 0) >= 3)
        throw new Error('同时最多接受三个悬赏，请先完成并领取现有悬赏。');
    const [notices] = await connection.execute(`SELECT b.id,b.title FROM bounty_board_slots bs JOIN bounty_notices b ON b.id=bs.bounty_id
    ...
    if (!notice)
        throw new Error('该悬赏已经刷新，请查看最新悬赏板。');
    const [existing] = await connection.execute('SELECT 1 FROM player_bounties WHERE character_id=? AND bounty_id=? FOR UPDATE', [character.id, bountyId]);
    if (existing[0])
        throw new Error('你已经接受过这份悬赏。');
...
const claimBounty = async (qqUserId, bountyId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    await (await import('./guild-context.js')).requireGuildService(connection, Number(character.id));
    const [rows] = await connection.execute(`SELECT b.title,b.copper_reward,pb.status,pb.instance_key FROM player_bounties pb JOIN bounty_notices b ON b.id=pb.bounty_id
    WHERE pb.character_id=? AND pb.bounty_id=? FOR UPDATE`, [character.id, bountyId]);
    const bounty = rows[0];
    if (!bounty)
        throw new Error('没有找到这份已接受的悬赏。');
    if (bounty.status !== 'completed')
        throw new Error('讨伐目标尚未全部完成。');
    ...
    await (await import('./finance-settlement.js')).recordFinanceSignal(connection, { sourceKey: `bounty:${character.id}:${bountyId}`, factionCode: 'adventurer_guild', characterId: Number(character.id), eventType: 'bounty.claimed', sourceType: 'bounty_claim', score: 2 });
    recordAchievement(connection, Number(character.id), [{ metric: 'ACH_L15' }, { metric: 'ACH_L16', distinct: String(bountyId) }, { metric: 'ACH_K09', value: Number(bounty.copper_reward), life: true }], 'bounty:' + bountyId + ':' + character.id);
...
const advanceBountyProgress = async (connection, characterId, targets) => {
    for (const target of targets) {
        const [finishing] = await connection.execute(`SELECT pb.bounty_id,pb.instance_key,b.title,b.required_count FROM player_bounties pb JOIN bounty_notices b ON b.id=pb.bounty_id
...
export { abandonBounty, abandonSecondaryQuest, acceptBounty, advanceBountyProgress, bountyBoard, claimBounty, clearInvalidBounty, playerBounties, postBossBounty, refreshBounties };
