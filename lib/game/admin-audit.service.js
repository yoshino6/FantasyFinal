...
import { ensureSkillPointLedger, skillPointLedgerSummary } from './skill-point-ledger.service.js';
import { forgeEquipmentCapsFor, forgePrimaryKeys } from './blacksmith.service.js';
...
const hasLegacyPercentAffix = (value, crafted = false, category = '', subtype = null) => Object.entries(jsonRecord(value)).some(([key, amount]) => {
    if (!key.endsWith('Pct') || Number(amount) === 0)
        return false;
...
const invalidForgedEquipment = (category, subtype, level, rarity, effectJson, primaryJson) => {
    ...
    if (damageBonus && (category !== '武器' || subtype === '盾牌' || damageBonus < 0 || damageBonus > 5))
        return true;
    const damageReduction = Number(effect.damageReductionPct ?? 0);
    if (damageReduction && ((category === '武器' && subtype !== '盾牌') || damageReduction < 0 || damageReduction > 5))
        return true;
    if (subtype === '盾牌' && (Number(effect.physicalAttack ?? 0) !== 0 || Number(effect.magicAttack ?? 0) !== 0))
        return true;
    if (markedMain.length && (markedMain.length !== expectedMain.length || markedMain.some(key => !expectedMain.includes(key))))
        return true;
    if (!main.every(key => Number(effect[key] ?? 0) > 0))
        return true;
    ...
    if (secondary.length > allowedSecondary)
        return true;
...
const characterIdFor = async (connection, qqUserId) => {
    const [rows] = await connection.execute('SELECT c.id,c.name,c.level,c.skill_points FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1 FOR UPDATE', [qqUserId]);
    if (!rows[0])
        throw new Error('该用户尚未注册角色。');
    return rows[0];
};
const auditCharacter = async (qqUserId) => withTransaction(async (connection) => {
    ...
    const [beforeRows] = await connection.execute(`SELECT ${columns.join(',')} FROM characters WHERE id=? FOR UPDATE`, [character.id]);
    ...
    const [afterRows] = await connection.execute(`SELECT ${columns.join(',')} FROM characters WHERE id=?`, [character.id]);
...
const auditInventory = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterIdFor(connection, qqUserId);
    const [removed] = await connection.execute('DELETE pi FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND (pi.quantity<=0 OR (i.stackable=0 AND pi.quantity>1))', [character.id]);
    const [quickRemoved] = await connection.execute('DELETE qi FROM player_quick_items qi LEFT JOIN player_inventory pi ON pi.character_id=qi.character_id AND pi.item_id=qi.item_id WHERE qi.character_id=? AND pi.item_id IS NULL', [character.id]);
    const [artifacts] = await connection.execute(`SELECT pe.slot,pe.item_id,pe.instance_id FROM player_equipment pe
    ...
    const [forgedRows] = await connection.execute(`SELECT ii.id,i.item_category,i.weapon_type,i.required_level,i.rarity,COALESCE(ii.effect_json,i.effect_json) AS effect_json,ii.forge_primary_json
    ...
    const [equipmentRows] = await connection.execute(`SELECT ii.id,i.code,i.item_category,i.weapon_type,COALESCE(ii.effect_json,i.effect_json) AS effect_json FROM player_item_instances ii
    ...
    if (invalidInstances.length || recycledInstances.length)
        await recalculateCharacterStats(connection, Number(character.id));
    const removedCount = Number(removed.affectedRows);
    const quickRemovedCount = Number(quickRemoved.affectedRows);
    const artifactCount = excessArtifacts.length;
    const forgedCount = invalidInstances.length;
    const percentCount = percentInstances.filter(id => !invalidInstances.includes(id)).length;
    const shieldCount = invalidShieldInstances.filter(id => !invalidInstances.includes(id) && !percentInstances.includes(id)).length;
    const recycledCount = recycledInstances.length;
...
const clearPlayerBackpack = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterIdFor(connection, qqUserId);
    const [stacked] = await connection.execute('DELETE FROM player_inventory WHERE character_id=?', [character.id]);
    ...
    const [instances] = await connection.execute(`DELETE ii FROM player_item_instances ii
...
const auditPlayerState = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterIdFor(connection, qqUserId);
    const [storyRows] = await connection.execute('SELECT status FROM player_story_progress WHERE character_id=? AND story_code=\'forest_guide\' FOR UPDATE', [character.id]);
    const storyStatus = storyRows[0]?.status;
    const [sessions] = await connection.execute(`SELECT cs.id,
    ...
    for (const session of sessions) {
        ...
        if (!invalidForestStoryCombat && !staleCombat)
            continue;
    ...
    const [travel] = await connection.execute('SELECT 1 FROM player_travels WHERE character_id=? AND arrival_at<=NOW(6) LIMIT 1 FOR UPDATE', [character.id]);
    const [activity] = await connection.execute('UPDATE characters SET activity_status=\'active\',rest_started_at=NULL WHERE id=? AND activity_status IN (\'resting\',\'unconscious\') AND current_hp>=hp_max AND current_mp>=mp_max', [character.id]);
...
const auditSkills = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterIdFor(connection, qqUserId);
    const migrated = await ensureSkillPointLedger(connection, character.id, Number(character.skill_points));
    const [skills] = await connection.execute('SELECT ps.skill_id,ps.level,s.max_level FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id WHERE ps.character_id=? FOR UPDATE', [character.id]);
    let changed = migrated;
    for (const skill of skills) {
        const level = Math.min(Math.max(1, Number(skill.level)), Number(skill.max_level));
        if (level !== Number(skill.level)) {
            await connection.execute('UPDATE player_skills SET level=? WHERE character_id=? AND skill_id=?', [level, character.id, skill.skill_id]);
            changed = true;
        }
    }
    const [specializationFix] = await connection.execute('UPDATE player_skill_specializations SET level=LEAST(GREATEST(level,1),CASE WHEN specialization=\'overcharge\' AND skill_id IN (SELECT id FROM skill_definitions WHERE code IN (\'longsword_mastery\',\'shield_mastery\',\'staff_mastery\',\'spellbook_mastery\',\'orb_mastery\',\'dagger_mastery\',\'fistblade_mastery\')) THEN 5 WHEN specialization=\'instant\' AND skill_id IN (SELECT id FROM skill_definitions WHERE code IN (\'longsword_mastery\',\'shield_mastery\',\'staff_mastery\',\'spellbook_mastery\',\'orb_mastery\',\'dagger_mastery\',\'fistblade_mastery\')) THEN 6 ELSE 100 END) WHERE character_id=?', [character.id]);
    if (Number(specializationFix.affectedRows))
        changed = true;
    const [appraisalFix] = await connection.execute('UPDATE player_appraisal_progress SET range_level=LEAST(GREATEST(range_level,1),10),information_level=LEAST(GREATEST(information_level,1),4) WHERE character_id=?', [character.id]);
    if (Number(appraisalFix.affectedRows))
        changed = true;
    ...
    if (pointMismatch) {
        await connection.execute('UPDATE characters SET skill_points=? WHERE id=?', [Math.max(0, summary.balance), character.id]);
        changed = true;
    }
    const notes = [
...
const auditAllPlayers = async () => {
    const pool = await getPool();
    const [players] = await pool.execute('SELECT p.qq_user_id,c.name FROM characters c JOIN players p ON p.id=c.player_id ORDER BY c.id');
    const failed = [];
    const results = [];
    let completed = 0;
    for (const player of players) {
        try {
            ...
            if (fixes.length)
                results.push({ name: character.name, fixes });
        }
        catch (error) {
...
export { auditAllPlayers, auditCharacter, auditInventory, auditPlayerState, auditSkills, clearPlayerBackpack };
