...
const deconstructorQuest = async (qqUserId) => {
    ...
    const [items] = await pool.execute(`SELECT pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code='beast_core'`, [characterId]);
...
const monsterCraftClassFor = (item) => {
    const effect = jsonRecord(item.effect_json);
    if (!['hair', 'gel_skin', 'bone', 'shell', 'scale'].includes(String(effect.monster_craft_material ?? '')))
        return null;
    const monsterClass = String(effect.material_monster_class ?? 'normal');
    return ['normal', 'large', 'elite', 'boss'].includes(monsterClass) ? monsterClass : 'normal';
};
const monsterCraftValueMultiplierFor = (item) => {
...
const isDeconstructable = (item) => Boolean(monsterCraftClassFor(item)) || deconstructableCodes.includes(item.code) || constructedMaterialRecipes.has(item.code);
const deconstructionMapMaterialGain = (item) => {
    const monsterClass = monsterCraftClassFor(item);
    if (monsterClass)
        return { normal: 1, large: 2, elite: 3, boss: 4 }[monsterClass] * monsterCraftValueMultiplierFor(item);
...
const deconstructMonsterCraftMaterial = (item, bonusMultiplier, add) => {
    const monsterClass = monsterCraftClassFor(item);
    if (!monsterClass)
        return false;
    const effect = jsonRecord(item.effect_json);
    const kind = String(effect.monster_craft_material);
    const particleValue = { normal: 1, large: 2, elite: 3, boss: 4 }[monsterClass] * monsterCraftValueMultiplierFor(item) * bonusMultiplier;
    const particleCount = Math.floor(particleValue);
    const bloodBias = ['hair', 'gel_skin'].includes(kind) ? .65 : .35;
    for (let index = 0; index < particleCount; index += 1)
        add(Math.random() < bloodBias ? 'blood_residue' : 'energy_ember', 1);
    if (Math.random() < particleValue - particleCount)
        add(Math.random() < bloodBias ? 'blood_residue' : 'energy_ember', 1);
    const magicChance = { normal: 0, large: 0, elite: .2, boss: .25 }[monsterClass] * monsterCraftValueMultiplierFor(item);
    if (Math.random() < Math.min(1, magicChance * bonusMultiplier))
        add('magic_unit', 1);
    return true;
};
const constructionProficiencyGain = (category) => ({ '基材': 5, '构件': 10, '异械': 20 }[category]);
const coloredSlimeGelDust = { red_slime_gel: 'fire_element_dust', orange_slime_gel: 'metal_element_dust', yellow_slime_gel: 'thunder_element_dust', green_slime_gel: 'wood_element_dust', cyan_slime_gel: 'water_element_dust', blue_slime_gel: 'ice_element_dust', purple_slime_gel: 'dark_element_dust', black_slime_gel: 'dark_element_dust' };
...
const constructItem = async (qqUserId, recipeCode) => withTransaction(async (connection) => {
    const recipe = constructionRecipeByCode.get(recipeCode);
    if (!recipe)
    ...
    for (const ingredient of recipe.ingredients) {
        ...
        if (!material || Number(material.quantity) < ingredient.quantity)
            throw new Error(`材料不足：需要【${material?.name ?? ingredient.code}】×${ingredient.quantity}。`);
    }
    for (const ingredient of recipe.ingredients) {
        const material = materials.get(ingredient.code);
    ...
    if (recipe.outputType === 'equipment') {
        const [instance] = await connection.execute('INSERT INTO player_item_instances (character_id,item_id,quality,durability,durability_max,effect_json) VALUES (?,?,100,100,100,?)', [characterId, output.id, JSON.stringify(recipe.effect ?? {})]);
...
const chainedYield = (firstChance, decay, limit) => {
    let count = 0;
...
const deconstructItems = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    ...
    const characterId = await characterIdFor(connection, qqUserId, true);
    const progress = await deconstructorProgressFor(connection, characterId, true);
    const [rows] = await connection.execute(`SELECT i.id,i.code,i.name,i.item_category,i.item_type,i.effect_json,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.id=? FOR UPDATE`, [characterId, itemId]);
    const item = rows[0];
    if (!item || !isDeconstructable(item))
    ...
    for (let index = 0; index < quantity; index += 1) {
        ...
        if (deconstructMonsterCraftMaterial(item, bonusMultiplier, add)) {
            ...
            for (const output of forgeMaterialProfiles[item.code])
                add(output.code, chainedYield(1, Math.min(1, output.decay * bonusMultiplier), output.limit));
        }
            ...
            for (const ingredient of recipe.ingredients) {
                for (let input = 0; input < ingredient.quantity; input += 1)
                    if (Math.random() < Math.min(1, recovery * bonusMultiplier))
            ...
            for (let particle = 0; particle < particleCount; particle += 1)
                add(Math.random() < 0.5 ? 'blood_residue' : 'energy_ember', 1);
    ...
    let results = [];
    if (codes.length) {
        const [definitions] = await connection.execute(`SELECT id,code,name FROM item_definitions WHERE code IN (${codes.map(() => '?').join(',')}) FOR UPDATE`, codes);
        for (const definition of definitions) {
...
export { acceptDeconstructorQuest, claimDeconstructorQuest, claimVivianCourseBlueprints, constructItem, constructionCategories, constructionRecipesFor, deconstructItems, deconstructionItems, deconstructorProgress, deconstructorQuest };
