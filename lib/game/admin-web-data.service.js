...
const sendWebMail = async (actor, body) => {
    ...
    const item = asText(body.item, 64);
    if (!title || !content)
        throw new Error('邮件标题和正文均不能为空。');
    if (!reason)
    ...
    if (item && (!Number.isFinite(quantity) || quantity < 1 || quantity > 999_999))
        throw new Error('附件数量必须是 1 到 999999 的整数。');
    return withTransaction(async (connection) => {
        ...
        let itemName = null;
        if (item) {
            const [items] = await connection.execute('SELECT id,name FROM item_definitions WHERE id=? OR code=? OR codex_id=? OR name=? LIMIT 1', [item, item, item, item]);
            if (!items[0])
                throw new Error('未找到邮件附件物品；请填写物品编号、代号、图鉴编号或名称。');
        ...
        for (const recipient of recipients) {
            ...
            if (itemId)
                await connection.execute('INSERT INTO player_mail_attachments (mail_id,item_id,quantity) VALUES (?,?,?)', [result.insertId, itemId, quantity]);
        }
