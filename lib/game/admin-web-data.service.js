...
import { adminOperationLogs } from './admin-log.service.js';
...
const adminPlayers = async (filters = {}) => {
    ...
    if (region) {
        where.push('(r.code=? OR INSTR(r.name,?)>0)');
...
const adminSearchSuggestions = async (kind, keyword, offsetValue = 0) => {
    const type = asText(kind, 24);
    const term = asText(keyword, 64);
    const offset = Number(offsetValue);
    const empty = { entries: [], hasMore: false };
    if (!term || !Number.isSafeInteger(offset) || offset < 0 || offset > 1_000_000)
        return empty;
    const pool = await getPool();
    const page = (rows) => ({ entries: rows.slice(0, 10), hasMore: rows.length > 10 });
    if (type === 'player' || type === 'recipient') {
        const [rows] = await pool.execute(`SELECT c.name AS label,${type === 'recipient' ? 'CAST(c.id AS CHAR)' : 'c.name'} AS value,
    ...
    if (type === 'region') {
        const [rows] = await pool.execute(`SELECT name AS label,code AS value,CONCAT('区域代号：',code) AS detail
    ...
    if (type === 'item') {
        const [rows] = await pool.execute(`SELECT name AS label,CAST(id AS CHAR) AS value,
    ...
    if (type === 'mail') {
        const [rows] = await pool.execute(`SELECT m.title AS label,m.title AS value,CONCAT('邮件标题 · ',c.name) AS detail
    ...
    if (type === 'event') {
        const [rows] = await pool.execute(`SELECT x.title AS label,x.title AS value,CONCAT(x.kind,' · ',x.region_name) AS detail FROM (
    ...
    if (type === 'patrol') {
        const [rows] = await pool.execute(`SELECT d.name AS label,d.name AS value,CONCAT('巡游实体 · ',r.name,' · ',d.code) AS detail
    ...
    if (type === 'operation') {
        const [rows] = await pool.execute(`SELECT action_text AS label,action_text AS value,CONCAT('游戏内管理操作 · ',operator_qq_user_id) AS detail
    ...
    if (type === 'journal') {
        const [rows] = await pool.execute(`SELECT reason AS label,reason AS value,CONCAT('审计原因 · ',actor_ref) AS detail
      FROM operation_journals WHERE reason<>'' AND (INSTR(reason,?)>0 OR INSTR(actor_ref,?)>0)
      ORDER BY created_at DESC,id DESC LIMIT 11 OFFSET ?`, [term, term, String(offset)]);
        return page(rows);
    }
    if (type === 'portrait') {
        const [rows] = await pool.execute(`SELECT JSON_UNQUOTE(JSON_EXTRACT(a.state_json,'$.name')) AS label,
...
const adminGameOperations = (page, keyword) => adminOperationLogs({ page: pageOf(page), keyword: asText(keyword) });
const adminMails = async (keyword) => {
    const term = asText(keyword);
    const pool = await getPool();
...
const sendWebMail = async (actor, body) => {
    if (actor.role === 'viewer')
        throw new Error('只读账号不能发放邮件。');
    const characterIdText = asText(body.characterId, 24);
    const title = asText(body.title, 96);
    const content = asText(body.content, 4_000);
    const reason = asText(body.reason, 500);
    if (!title)
        throw new Error('邮件标题不能为空。');
    if (!reason)
        throw new Error('发放邮件必须填写操作原因。');
    if (characterIdText && (!/^\d+$/.test(characterIdText) || Number(characterIdText) < 1))
        throw new Error('角色编号必须是正整数，留空才会发送给全服。');
    ...
    if (rawAttachments.length > 20)
        throw new Error('一封邮件最多添加 20 种附件。');
    const attachments = rawAttachments.map((entry, index) => {
        const value = entry && typeof entry === 'object' ? entry : {};
        const item = asText(value.item, 64);
        const quantity = Math.floor(Number(value.quantity ?? 1));
        if (!item)
            throw new Error(`第 ${index + 1} 个附件未选择物品。`);
        if (!Number.isFinite(quantity) || quantity < 1 || quantity > 999_999)
            throw new Error(`第 ${index + 1} 个附件数量必须是 1 到 999999 的整数。`);
        return { item, quantity };
    });
    return withTransaction(async (connection) => {
        const [recipients] = await connection.execute(characterIdText
        ...
        if (!recipients.length)
            throw new Error(characterIdText ? '未找到该玩家角色。' : '当前没有可接收邮件的玩家角色。');
        const resolvedAttachments = [];
        for (const attachment of attachments) {
            const [items] = await connection.execute('SELECT id,name FROM item_definitions WHERE id=? OR code=? OR codex_id=? OR name=? LIMIT 1', [attachment.item, attachment.item, attachment.item, attachment.item]);
            if (!items[0])
                throw new Error(`未找到邮件附件物品：${attachment.item}`);
            resolvedAttachments.push({ id: Number(items[0].id), name: items[0].name, quantity: attachment.quantity });
        }
        for (const recipient of recipients) {
            const [result] = await connection.execute('INSERT INTO player_mails (character_id,title,content) VALUES (?,?,?)', [recipient.id, title, content]);
            for (const attachment of resolvedAttachments)
                await connection.execute('INSERT INTO player_mail_attachments (mail_id,item_id,quantity) VALUES (?,?,?)', [result.insertId, attachment.id, attachment.quantity]);
        }
...
const adminWorldEvents = async (keyword) => {
    const term = asText(keyword);
    const pool = await getPool();
    const like = `%${term}%`;
...
export { adminDashboard, adminGameOperations, adminMails, adminPatrolEntities, adminPlayerDetail, adminPlayers, adminSearchSuggestions, adminWebJournals, adminWorldEvents, adminWorldOverview, changeGlobalMultiplierFromWeb, runWebPlayerAudit, sendWebMail };
