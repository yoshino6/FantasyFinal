const hiddenProfessionsReleased = true;
const hiddenProfessionDiscoveryAffinity = 500;
...
const hiddenProfession = (code) => hiddenProfessions.find(p => p.code === code || p.name === code || p.npc === code);
const skill = (profession, suffix, name, button, mana, resource, cooldown, power, description, tier = '中位') => ({ code: 'hidden_' + suffix, name, button, profession, tier, mana, resource, cooldown, power, description });
const hiddenWeaponAttackPower = (code, weaponCount) => code === 'hidden_weapon_strike'
    ? 125
...
const hiddenSkill = (code) => hiddenSkills.find(s => s.code === code);
const isHiddenSkill = (code) => Boolean(hiddenSkill(code));
const hiddenPassiveCode = (code) => 'hidden_passive_' + code;
const hiddenVersion = 3;

export { hiddenPassiveCode, hiddenProfession, hiddenProfessionDiscoveryAffinity, hiddenProfessions, hiddenProfessionsReleased, hiddenSkill, hiddenSkills, hiddenVersion, hiddenWeaponAttackPower, isHiddenSkill };
