...
const claimChurchBlessing = (qqUserId) => withTransaction(async (connection) => {
    ...
    const [event] = await connection.execute(`INSERT INTO player_events (player_id,event_type,payload) VALUES (?,?,?)`, [character.player_id, 'social.blessing.claimed', JSON.stringify({ date, bouquet, fruit, expiresInMinutes: 60 })]);
