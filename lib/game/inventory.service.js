...
const discardMaterial = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    ...
    const [items] = await connection.execute(`SELECT i.name,pi.quantity,COALESCE(JSON_EXTRACT(i.effect_json,'$.personalOnly'),0) AS personal_only
