import { calculateDerivedStats, forgeRarityMultiplier, equipmentQualityMultiplier, forgedEquipmentBase, forgedAffixCap } from './constants.js';
import { armorSlots, armorPanelPercent } from './armor-class.js';
...
const withLegendaryEquipment = (base, profile, evolution, passive) => {
    ...
    const masteryPercent = Object.fromEntries(Object.values(panelPercentKeys).map(key => [key, Number(mastery[key] ?? 0)]));
...
const advancedMentorTrialBuild = (profession, inheritance) => {
    ...
    const trainedAttributes = Object.fromEntries(Object.entries(profile.attributes).map(([key, value]) => [key, Number(value) + Number(profile.growth[key]) * playerGrowthShares(30)]));
