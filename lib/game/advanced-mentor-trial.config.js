import { calculateDerivedStats, forgedEquipmentBase, forgedAffixCap, equipmentQualityMultiplier, forgeRarityMultiplier } from './constants.js';
import { armorClassDefenseMultiplier, armorClassMobilityModifier } from './character.service.js';
import { applyEvolutionBaseStats } from './evolution.service.js';
import { applyWeaponMasteryStats } from './weapon-mastery.service.js';
...
const addSecondary = (stats, category, key, ratio, count = 1) => {
    const cap = forgedAffixCap(category, key, 30, '传说');
    stats[key] += Math.floor(cap * equipmentQualityMultiplier(100) * ratio * count);
};
const weaponPrimary = (stats, type, value) => {
    if (type === '盾牌') {
        stats.physicalDefense += Math.floor(value);
        stats.magicDefense += Math.floor(value * .5);
        return;
    }
    if (type === '法杖' || type === '法书' || type === '法球') {
        stats.magicAttack += Math.floor(value);
        return;
    }
    if (type === '匕首') {
        stats.physicalAttack += Math.floor(value * .9);
        stats.magicAttack += Math.floor(value * .9);
        return;
    }
...
const masteryBonusesFor = (profile) => {
    const bonus = { physicalAttackPct: 0, magicAttackPct: 0, physicalDefensePct: 0, magicDefensePct: 0, accuracyPct: 0, critRatePct: 0, critDamagePct: 0, critResistPct: 0, critDamageReductionPct: 0, mpPct: 0, chantSpeedPct: 0, details: [] };
    for (const hand of [profile.mainHand, profile.offhand]) {
        if (bonus.details.some(detail => detail.startsWith(`${hand.type}精通：`)))
            continue;
        const rule = masteryRules[hand.type];
        for (const [key, value] of Object.entries(rule.effect))
            bonus[key] += value;
        bonus.details.push(rule.text);
    }
    return bonus;
};
const withLegendaryEquipment = (base, profile) => {
    const result = { ...base };
    ...
    for (const key of ['accuracy', 'evasion', 'speed'])
        result[key] = Math.max(0, Math.floor(result[key] * (1 + armorClassMobilityModifier(profile.armor, key) * 5 / 100)));
    const mastery = masteryBonusesFor(profile);
    const withMastery = applyWeaponMasteryStats(result, mastery);
    for (const key of Object.keys(withMastery))
        withMastery[key] = Math.max(0, Math.floor(withMastery[key]));
    return { stats: withMastery, mastery: mastery.details };
};
const advancedMentorTrialBuild = (profession, inheritance) => {
    const profile = profiles[profession.code];
    if (!profile)
        throw new Error(`未配置导师构筑：${profession.code}`);
    const trainedAttributes = Object.fromEntries(Object.entries(profile.attributes).map(([key, value]) => [key, Number(value) + Number(profile.growth[key]) * 29]));
    const evolutionBonus = profile.evolution.reduce((all, entry) => ({ ...all, ...Object.fromEntries(Object.entries(entry.effect).map(([key, value]) => [key, Number(all[key] ?? 0) + Number(value)])) }), {});
...
const advancedMentorTrialTraits = (build) => [
...
export { advancedMentorTrialBuild, advancedMentorTrialTraits };
