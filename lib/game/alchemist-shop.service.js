...
const categoryOf = (value) => value === '回复' || value === '特殊' ? value : '全部';
...
const validQuantity = (quantity) => { if (!Number.isInteger(quantity) || quantity < 1 || quantity > 999)
    throw new Error('数量必须是 1 至 999 之间的整数。'); return quantity; };
const alchemistShopCatalog = async (qqUserId, page = 1, category = '全部', keyword = '') => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const selected = categoryOf(category);
    const term = `%${keyword.trim()}%`;
    const filter = selected === '全部' ? '' : ' AND si.shop_category=?';
    const values = selected === '全部' ? [term] : [term, selected];
    const [counts] = await pool.execute(`SELECT COUNT(*) AS total FROM alchemist_shop_items si JOIN item_definitions i ON i.id=si.item_id WHERE si.is_active=1 AND i.name LIKE ?${filter}`, values);
    const paging = pageInfo(page, Number(counts[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT i.id,i.codex_id,i.name,i.item_category,i.description,si.buy_price,si.stock_quantity,COALESCE(pi.quantity,0) AS owned_quantity FROM alchemist_shop_items si JOIN item_definitions i ON i.id=si.item_id LEFT JOIN player_inventory pi ON pi.item_id=i.id AND pi.character_id=? WHERE si.is_active=1 AND i.name LIKE ?${filter} ORDER BY si.shop_category,i.id LIMIT ? OFFSET ?`, [character.id, ...values, PAGE_SIZE, (paging.page - 1) * PAGE_SIZE]);
...
const buyAlchemistItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = validQuantity(quantity);
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute('SELECT i.id AS item_id,i.name,si.buy_price,si.stock_quantity FROM alchemist_shop_items si JOIN item_definitions i ON i.id=si.item_id WHERE si.item_id=? AND si.is_active=1 FOR UPDATE', [itemId]);
    const item = rows[0];
    if (!item)
        throw new Error('该商品已下架。');
    const total = Number(item.buy_price) * amount;
    if (Number(item.stock_quantity) < amount)
        throw new Error(`库存不足，剩余 ${item.stock_quantity} 件。`);
    if (Number(character.copper_coins) < total)
        throw new Error(`铜币不足，需要 ${total} 铜币。`);
...
const alchemistSellCatalog = async (qqUserId, page = 1, keyword = '') => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const term = `%${keyword.trim()}%`;
    const [counts] = await pool.execute(`SELECT COUNT(*) AS total FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE ${alchemistSellable}`, [character.id, term]);
    const paging = pageInfo(page, Number(counts[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT i.id,i.name,i.item_category,pi.quantity,CEIL(i.trade_price*1.15) AS sell_price
...
const sellAlchemistItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = validQuantity(quantity);
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT i.id,i.name,i.item_category,pi.quantity,CEIL(i.trade_price*1.15) AS sell_price
    ...
    const item = rows[0];
    if (!item)
        throw new Error('晴儿只收购药剂、食物、草药与炼金相关素材。');
...
export { alchemistSellCatalog, alchemistShopCatalog, buyAlchemistItem, sellAlchemistItem };
