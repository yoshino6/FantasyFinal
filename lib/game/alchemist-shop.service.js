...
const buyAlchemistItem = async (qqUserId, itemId, quantity = 1) => buySecondaryFinished(qqUserId, 'alchemy_sweetshop', itemId, quantity);
...
const sellAlchemistItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    ...
    const [rows] = await connection.execute(`SELECT i.id,i.name,i.item_category,pi.quantity,pi.personal_bound_quantity,pi.trade_bound_quantity,CEIL(i.trade_price*1.15) AS sell_price
