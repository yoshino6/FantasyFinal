const regionalBossElements = ['水', '火', '土', '木', '风', '冰', '雷', '光', '暗'];
const resistance = (values) => values;
const regionalBossComponentDefinitions = [
    {
...
const regionalBossComponentByTemplateCode = new Map(regionalBossComponentDefinitions.map(definition => [definition.templateCode, definition]));
const regionalBossComponentsFor = (bodyCode) => regionalBossComponentDefinitions.filter(definition => definition.bodyCode === bodyCode);
const regionalBossComponentByKey = (key) => regionalBossComponentDefinitions.find(definition => definition.key === key);

export { regionalBossComponentByKey, regionalBossComponentByTemplateCode, regionalBossComponentDefinitions, regionalBossComponentsFor, regionalBossElements };
