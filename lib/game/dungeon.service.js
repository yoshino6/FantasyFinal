...
import { resolvedHitChance, strikeCorrections, opposedChance } from './combat-math.js';
...
import { assertPvpDefeatUnprotected, createPvpBattleLog, finishPvpBattleLog, recordPvpAttack, resolvePvpVictory } from './pvp.service.js';
import { isFriendRelation, recordFriendInteraction } from './social.service.js';
import { monsterCombatStats } from './adventure.service.js';
import { dungeonBlueprintDrops } from './deconstructor-catalog.js';
import { bossRandomEffectTrait } from './boss-random-effects.config.js';
import { recordCardMovement, resetCardMovementCharge } from './monster-card-exploration.service.js';

const DUNGEON_REGION_CODE = 'dark_forest_dungeon';
const FLOORS = [-10, -20, -30];
const key = (x, y) => `${x},${y}`;
const random = (items) => items[Math.floor(Math.random() * items.length)];
const skillJson = (value) => typeof value === 'string' ? value : JSON.stringify(value ?? []);
const publicForestEntrance = (x, y) => x >= -150 && x <= 149 && y >= -135 && y <= -61;
const fallbackEntrances = [[-46, -108], [40, -108], [-46, -75], [75, -110], [-80, -100]];
...
const junctionLandmark = (floor, index) => {
    const symbol = junctionSymbols[(floor * 17 + index) % junctionSymbols.length];
    const material = junctionMaterials[(floor * 19 + index * 7) % junctionMaterials.length];
...
const characterFor = async (connection, qqUserId, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.* FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [qqUserId]);
    if (!rows[0])
        throw new Error('请先发送“注册”创建角色。');
    return rows[0];
};
const ownsDungeonItem = async (connection, characterId, code) => {
    const [rows] = await connection.execute('SELECT pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code=? AND pi.quantity>0 LIMIT 1', [characterId, code]);
    return Number(rows[0]?.quantity ?? 0) > 0;
};
const regionId = async (connection, code) => {
    const [rows] = await connection.execute('SELECT id FROM map_regions WHERE code=? LIMIT 1', [code]);
    if (!rows[0])
        throw new Error('地下迷宫区域尚未初始化。');
    return Number(rows[0].id);
};
const mazeCells = (width, height) => {
    ...
    while (stack.length) {
        ...
        const candidates = directions.map(([dx, dy]) => [cx + dx, cy + dy, dx, dy])
            .filter(([nx, ny]) => nx >= 0 && nx < columns && ny >= 0 && ny < rows && !visited.has(key(nx, ny)));
        if (!candidates.length) {
            stack.pop();
            continue;
        }
        const [nx, ny, dx, dy] = random(candidates);
        visited.add(key(nx, ny));
        cells.add(key(1 + cx * 2 + dx, 1 + cy * 2 + dy));
        cells.add(key(1 + nx * 2, 1 + ny * 2));
        stack.push([nx, ny]);
    }
...
const chestQuality = (floor, required = false) => {
    const roll = Math.random();
    if (floor === 0)
        return roll < (required ? .90 : .99) ? '青铜' : roll < (required ? .99 : 1) ? '白银' : '黄金';
    if (floor === 1)
        return roll < (required ? .80 : .90) ? '青铜' : roll < (required ? .97 : 1) ? '白银' : '黄金';
    return roll < .90 ? '白银' : '黄金';
};
const randomLevel = (min, max) => min + Math.floor(Math.random() * (max - min + 1));
...
const refreshDungeonSmallMonsters = async (connection) => {
    const dungeonRegionId = await regionId(connection, DUNGEON_REGION_CODE);
    const [dungeons] = await connection.execute('SELECT id FROM dungeon_instances WHERE state=\'active\' FOR UPDATE');
    if (!dungeons.length)
        return 0;
    const allCodes = [...new Set(Object.values(dungeonSmallMonsterPlan).flatMap(plan => plan.codes))];
    const [templates] = await connection.execute(`SELECT * FROM monster_templates WHERE code IN (${allCodes.map(() => '?').join(',')})`, allCodes);
    ...
    for (const dungeon of dungeons)
        for (const z of FLOORS) {
            ...
            const [countRows] = await connection.execute(`SELECT COUNT(*) AS total FROM dungeon_monsters dm
      ...
      WHERE dm.dungeon_id=? AND dm.is_boss=0 AND dm.is_floor_leader=0 AND s.pos_z=? AND s.defeated_at IS NULL`, [dungeon.id, z]);
            ...
            if (!missing)
                continue;
            const [cells] = await connection.execute(`SELECT dc.pos_x,dc.pos_y FROM dungeon_cells dc
...
const migrateDungeonBossTraits = async (connection) => {
    const [rows] = await connection.execute(`SELECT s.id,t.code FROM dungeon_monsters dm
    ...
    for (const row of rows) {
        const trait = randomDungeonBossTrait();
        const effect = bossRandomEffectTrait(row.code, trait.code);
        await connection.execute('UPDATE monster_spawns SET traits_json=? WHERE id=?', [JSON.stringify([trait, ...(effect ? [effect] : [])]), row.id]);
    }
    const [eligible] = await connection.execute(`SELECT s.id,t.code,s.traits_json FROM dungeon_monsters dm
    JOIN monster_spawns s ON s.id=dm.spawn_id JOIN monster_templates t ON t.id=s.template_id
    ...
    for (const row of eligible) {
        ...
        const difficulty = traits.find((entry) => dungeonBossTraits.some(candidate => candidate.code === entry.code));
        const effect = difficulty && bossRandomEffectTrait(row.code, difficulty.code);
        if (effect)
            await connection.execute('UPDATE monster_spawns SET traits_json=? WHERE id=?', [JSON.stringify([...traits, effect]), row.id]);
    }
};
const nextPublicForestEntrance = (occupied) => {
    for (let attempt = 0; attempt < 160; attempt += 1) {
        const x = Math.floor(Math.random() * 281) - 140;
        const y = Math.floor(Math.random() * 75) - 135;
        if (!occupied.has(key(x, y)))
            return { x, y };
    }
    const fallback = fallbackEntrances.find(([x, y]) => !occupied.has(key(x, y)));
    if (!fallback)
        throw new Error('地下迷宫入口坐标不足，无法生成新的入口。');
    return { x: fallback[0], y: fallback[1] };
};
const repairDungeonEntrances = async (connection) => {
    const forestId = await regionId(connection, 'dark_forest');
    const [allRows] = await connection.execute('SELECT region_id,pos_x,pos_y FROM dungeon_entrances FOR UPDATE');
    const occupied = new Set(allRows.filter(row => Number(row.region_id) === forestId).map(row => key(Number(row.pos_x), Number(row.pos_y))));
    const [dungeons] = await connection.execute('SELECT id FROM dungeon_instances WHERE state=\'active\' FOR UPDATE');
    for (const dungeon of dungeons) {
        const [entrances] = await connection.execute('SELECT region_id,pos_x,pos_y FROM dungeon_entrances WHERE dungeon_id=? ORDER BY pos_y,pos_x FOR UPDATE', [dungeon.id]);
        for (const entrance of entrances) {
            const x = Number(entrance.pos_x);
            const y = Number(entrance.pos_y);
            if (Number(entrance.region_id) === forestId && publicForestEntrance(x, y))
                continue;
            ...
            if (Number(entrance.region_id) === forestId)
                occupied.delete(key(x, y));
            const replacement = nextPublicForestEntrance(occupied);
            occupied.add(key(replacement.x, replacement.y));
        ...
        const [primary] = await connection.execute('SELECT pos_x,pos_y FROM dungeon_entrances WHERE dungeon_id=? AND region_id=? ORDER BY pos_y,pos_x LIMIT 1', [dungeon.id, forestId]);
        if (primary[0])
            await connection.execute('UPDATE dungeon_instances SET entrance_region_id=?,entrance_x=?,entrance_y=? WHERE id=?', [forestId, primary[0].pos_x, primary[0].pos_y, dungeon.id]);
    }
};
const createDungeon = async (connection) => {
    const forestId = await regionId(connection, 'dark_forest');
    const dungeonRegionId = await regionId(connection, DUNGEON_REGION_CODE);
    if (!Number.isSafeInteger(forestId) || forestId <= 0 || !Number.isSafeInteger(dungeonRegionId) || dungeonRegionId <= 0)
        throw new Error('地下迷宫区域数据异常，无法重建。');
    const [existingEntryRows] = await connection.execute(`SELECT e.pos_x,e.pos_y FROM dungeon_entrances e
    ...
    for (let index = 0; index < 3; index += 1) {
        const entrance = nextPublicForestEntrance(occupiedEntries);
        occupiedEntries.add(key(entrance.x, entrance.y));
        entrances.push(entrance);
    }
    const entranceX = entrances[0].x;
    const entranceY = entrances[0].y;
    const [originRows] = await connection.execute('SELECT MAX(origin_x) AS max_origin FROM dungeon_instances FOR UPDATE');
    const originX = Math.max(10, Number(originRows[0]?.max_origin ?? -20) + 28);
    const originY = 10;
    const [result] = await connection.execute('INSERT INTO dungeon_instances (entrance_region_id,entrance_x,entrance_y,origin_x,origin_y,state) VALUES (?,?,?,?,?,\'active\')', [forestId, entranceX, entranceY, originX, originY]);
    const dungeonId = Number(result.insertId);
    for (const entrance of entrances)
        await connection.execute('INSERT INTO dungeon_entrances (dungeon_id,region_id,pos_x,pos_y) VALUES (?,?,?,?)', [dungeonId, forestId, entrance.x, entrance.y]);
    const cells = [];
    const monsterSlots = [];
    const floorBossSlots = [];
    for (const [floorIndex, z] of FLOORS.entries()) {
        if (floorIndex === 2) {
            for (let x = 1; x <= 17; x += 1) {
                ...
                if ([3, 5, 7, 11, 13, 15].includes(x))
                    monsterSlots.push({ x: originX + x, y: originY + 1, z, floor: floorIndex });
                if (type === 'boss')
                    floorBossSlots.push({ x: originX + x, y: originY + 1, z, floor: floorIndex });
            }
            continue;
        }
        const width = floorIndex === 0 ? 10 : 12;
        const height = width;
        const floorCells = mazeCells(width - 2, height);
        const start = key(1, 1);
        const bossCell = key(width - 3, height - 1);
        const bossChest = key(width - 2, height - 1);
        const stairsDown = key(width - 1, height - 1);
        ...
        const traps = candidates.splice(0, floorIndex === 0 ? 3 : 5);
        const chests = candidates.splice(0, floorIndex === 0 ? 3 : 4);
        const monsters = candidates.splice(0, floorIndex === 0 ? 13 : 20);
        ...
        for (const [x, y] of floorCells) {
            ...
            if (key(x, y) === start)
                type = floorIndex === 0 ? 'entrance' : 'stairs_up';
            else if (key(x, y) === bossCell)
                type = 'boss';
            else if (key(x, y) === bossChest)
                type = 'chest';
            else if (key(x, y) === stairsDown)
                type = 'stairs_down';
            else if (traps.some(([tx, ty]) => tx === x && ty === y)) {
                type = 'trap';
                trap = random(['毒针', '吸魔符文', '坍塌地砖']);
            }
            else if (chests.some(([tx, ty]) => tx === x && ty === y))
                type = 'chest';
            cells.push([originX + x, originY + y, z, type, trap, landmark, type === 'chest' ? chestQuality(floorIndex, key(x, y) === bossChest) : null]);
        }
        if (!floorCellSet.has(bossCell))
            cells.push([originX + width - 3, originY + height - 1, z, 'boss', null, null, null]);
        if (!floorCellSet.has(bossChest))
            cells.push([originX + width - 2, originY + height - 1, z, 'chest', null, null, chestQuality(floorIndex, true)]);
        if (!floorCellSet.has(stairsDown))
            cells.push([originX + width - 1, originY + height - 1, z, 'stairs_down', null, null, null]);
        for (const [x, y] of monsters)
            monsterSlots.push({ x: originX + x, y: originY + y, z, floor: floorIndex });
        floorBossSlots.push({ x: originX + width - 3, y: originY + height - 1, z, floor: floorIndex });
    }
    for (const [x, y, z, type, trap, landmark, quality] of cells)
        await connection.execute('INSERT INTO dungeon_cells (dungeon_id,pos_x,pos_y,pos_z,cell_type,trap_type,landmark_text,chest_quality) VALUES (?,?,?,?,?,?,?,?)', [dungeonId, x, y, z, type, trap, landmark, quality]);
    const [templates] = await connection.execute(`SELECT * FROM monster_templates WHERE code IN (${dungeonTemplateCodes.map(() => '?').join(',')},'dungeon_warden')`, dungeonTemplateCodes);
    const byCode = new Map(templates.map(template => [template.code, template]));
    for (const slot of monsterSlots) {
        const code = slot.floor === 0 ? random(firstFloorSlimes) : slot.floor === 1 ? random(secondFloorMonsters) : random(['skeleton_warrior', 'death_wight', 'undead']);
        const template = byCode.get(code);
        if (!template)
            continue;
        const level = slot.floor === 0 ? randomLevel(1, 9) : code === 'skeleton' || code === 'undead' ? randomLevel(16, 18) : code === 'skeleton_warrior' || code === 'death_wight' ? randomLevel(18, 20) : randomLevel(8, 16);
        const hp = monsterCombatStats({ ...template, level }).hpMax;
    ...
    for (const slot of floorBossSlots) {
        const code = slot.floor === 0 ? 'black_slime' : slot.floor === 1 ? random(['skeleton_general', 'death_knight']) : 'necromancer_uz';
        const boss = byCode.get(code);
        if (!boss)
            continue;
        const trait = randomDungeonBossTrait();
        const effect = bossRandomEffectTrait(code, trait.code);
        const traits = [trait, ...(effect ? [effect] : [])];
        const hp = monsterCombatStats({ ...boss, traits_json: traits }).hpMax;
...
const refreshDungeons = async (connection, options = {}) => {
    ...
    const [activeRows] = await connection.execute('SELECT COUNT(*) AS total FROM dungeon_instances WHERE state<>\'closed\'');
    const missing = Math.max(0, 1 - Number(activeRows[0]?.total ?? 0));
    for (let index = 0; index < missing; index += 1)
        await createDungeon(connection);
    await repairDungeonEntrances(connection);
    if (options.refreshMonsters)
        await refreshDungeonSmallMonsters(connection);
};
const dungeonEvents = async () => withTransaction(async (connection) => {
    const dungeonRegionId = await regionId(connection, DUNGEON_REGION_CODE);
    const [rows] = await connection.execute(`SELECT id,state,entrance_region_id,entrance_x,entrance_y
    FROM dungeon_instances WHERE state<>'closed' ORDER BY id DESC LIMIT 1`);
    const dungeon = rows[0];
    if (!dungeon)
        return [];
    const [entranceRows] = await connection.execute('SELECT pos_x AS x,pos_y AS y FROM dungeon_entrances WHERE dungeon_id=? ORDER BY pos_x,pos_y', [dungeon.id]);
    ...
    const floors = [];
    for (const z of FLOORS) {
        const [players] = await connection.execute(`SELECT COUNT(*) AS total FROM characters c
      WHERE c.current_region_id=? AND c.pos_z=? AND EXISTS (SELECT 1 FROM dungeon_cells dc WHERE dc.dungeon_id=? AND dc.pos_z=? AND dc.pos_x=c.pos_x AND dc.pos_y=c.pos_y)`, [dungeonRegionId, z, dungeon.id, z]);
        const [boss] = await connection.execute(`SELECT 1 FROM dungeon_monsters dm JOIN monster_spawns s ON s.id=dm.spawn_id
...
const dungeonEntranceAt = async (connection, regionId, x, y) => {
    const [rows] = await connection.execute(`SELECT d.*,r.name FROM dungeon_instances d JOIN map_regions r ON r.id=d.entrance_region_id
...
const dungeonCellAt = async (connection, regionId, x, y, z) => {
    const [rows] = await connection.execute('SELECT dc.* FROM dungeon_cells dc JOIN dungeon_instances d ON d.id=dc.dungeon_id WHERE d.state=\'active\' AND dc.pos_x=? AND dc.pos_y=? AND dc.pos_z=? AND (SELECT id FROM map_regions WHERE code=\'dark_forest_dungeon\' LIMIT 1)=? LIMIT 1', [x, y, z, regionId]);
    return rows[0] ?? null;
};
const rebuildDungeons = async () => withTransaction(async (connection) => {
    const dungeonRegionId = await regionId(connection, DUNGEON_REGION_CODE);
    const [inside] = await connection.execute('SELECT id FROM characters WHERE current_region_id=? FOR UPDATE', [dungeonRegionId]);
    const characterIds = inside.map(row => Number(row.id));
    let moved = 0;
    const [atKnownCells] = await connection.execute(`UPDATE characters c
    ...
    const [stranded] = await connection.execute('UPDATE characters SET current_region_id=?,pos_x=0,pos_y=-60,pos_z=0 WHERE current_region_id=?', [forestRegionId, dungeonRegionId]);
    moved += Number(stranded.affectedRows ?? 0);
    if (characterIds.length) {
        const marks = characterIds.map(() => '?').join(',');
        const [sessions] = await connection.execute(`SELECT DISTINCT cs.id FROM combat_sessions cs
...
const dungeonTrackingHint = async (qqUserId) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    if (character.secondary_profession_code !== 'omniscient')
        return null;
    const [parties] = await pool.execute('SELECT p.leader_character_id FROM party_members pm JOIN parties p ON p.id=pm.party_id WHERE pm.character_id=? LIMIT 1', [character.id]);
    if (parties[0] && Number(parties[0].leader_character_id) !== Number(character.id))
        return null;
    const dungeonRegion = await regionId(pool, DUNGEON_REGION_CODE);
    if (Number(character.current_region_id) !== dungeonRegion)
        return null;
    ...
    if (!currentCell)
        return null;
    const [bosses] = await pool.execute(`SELECT s.pos_x,s.pos_y FROM dungeon_monsters dm
    JOIN monster_spawns s ON s.id=dm.spawn_id JOIN monster_templates t ON t.id=s.template_id
    ...
    const boss = bosses[0];
    const [cells] = await pool.execute('SELECT pos_x,pos_y,cell_type FROM dungeon_cells WHERE dungeon_id=? AND pos_z=?', [currentCell.dungeon_id, character.pos_z]);
    ...
    if (!target)
        return null;
    const queue = [{ x: Number(character.pos_x), y: Number(character.pos_y), first: null }];
    const seen = new Set([key(Number(character.pos_x), Number(character.pos_y))]);
    const directions = [[0, 1, '北'], [0, -1, '南'], [1, 0, '东'], [-1, 0, '西']];
    while (queue.length) {
        const current = queue.shift();
        if (key(current.x, current.y) === target)
            return current.first ? `你发觉${current.first}方有稍强的魔力踪迹。` : null;
        for (const [dx, dy, name] of directions) {
            const next = key(current.x + dx, current.y + dy);
            if (!walkable.has(next) || seen.has(next))
                continue;
            seen.add(next);
            queue.push({ x: current.x + dx, y: current.y + dy, first: current.first ?? name });
        }
    }
    return null;
};
const enterDungeon = async (qqUserId, dungeonId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const [dungeons] = await connection.execute('SELECT * FROM dungeon_instances WHERE id=? AND state=\'active\' FOR UPDATE', [dungeonId]);
    const dungeon = dungeons[0];
    const [entrances] = await connection.execute('SELECT 1 FROM dungeon_entrances WHERE dungeon_id=? AND region_id=? AND pos_x=? AND pos_y=? LIMIT 1', [dungeonId, character.current_region_id, character.pos_x, character.pos_y]);
    ...
    if (!dungeon || Number(character.pos_z) !== 0 || (!entrances[0] && !atLegacyEntrance))
        throw new Error('你已经离开地下迷宫入口，无法进入。');
    const { authorizeDungeonEntry } = await import('./dungeon-quest.service.js');
    await authorizeDungeonEntry(connection, Number(character.id), dungeonId);
    const dungeonRegion = await regionId(connection, DUNGEON_REGION_CODE);
    const [entry] = await connection.execute('SELECT * FROM dungeon_cells WHERE dungeon_id=? AND cell_type=\'entrance\' LIMIT 1', [dungeonId]);
    if (!entry[0])
        throw new Error('这座迷宫的入口结构尚未形成。');
...
const eventForCell = async (connection, character) => {
    ...
    const withLandmark = (text) => landmark ? `${landmark}\n\n${text}` : text;
    if (cell.cell_type === 'chest' && !Number(cell.chest_opened))
        return { kind: 'chest', cellId: Number(cell.id), text: withLandmark(`墙角的尘埃下露出一只${cell.chest_quality ?? '青铜'}宝箱。锁扣尚未被人触碰。`) };
    if (cell.cell_type === 'stairs_down')
        return { kind: 'down', text: withLandmark('一段向下延伸的石阶没入黑暗，下一层的冷意正沿台阶爬上来。') };
    if (cell.cell_type === 'stairs_up')
        return { kind: 'up', text: withLandmark('石阶通向上一层。你仍能听见远处地表的风声。') };
    if (cell.cell_type === 'entrance')
        return { kind: 'leave', text: withLandmark('入口后的石阶仍在，你可以随时离开这座地下迷宫。') };
    if (cell.cell_type !== 'trap')
        return landmark ? { kind: 'landmark', text: landmark } : null;
    const [trigger] = await connection.execute('INSERT IGNORE INTO dungeon_cell_triggers (cell_id,character_id) VALUES (?,?)', [cell.id, character.id]);
    if (!Number(trigger.affectedRows))
        return null;
    if (cell.trap_type === '毒针') {
        const damage = Math.max(1, Math.ceil(Number(character.hp_max) * .08));
        await connection.execute('UPDATE characters SET current_hp=GREATEST(1,current_hp-?) WHERE id=?', [damage, character.id]);
        ...
        return { kind: 'trap', text: withLandmark(`$毒针陷阱$石缝中弹出毒针，造成 ${damage} 点伤害。`) };
    }
    if (cell.trap_type === '吸魔符文') {
        const loss = Math.max(1, Math.ceil(Number(character.mp_max) * .18));
        await connection.execute('UPDATE characters SET current_mp=GREATEST(0,current_mp-?) WHERE id=?', [loss, character.id]);
        ...
        return { kind: 'trap', text: withLandmark(`$吸魔符文$脚下的符文亮起，流失 ${loss} 点魔力。`) };
    }
    const damage = Math.max(1, Math.ceil(Number(character.hp_max) * .05));
    await connection.execute('UPDATE characters SET current_hp=GREATEST(1,current_hp-?) WHERE id=?', [damage, character.id]);
    ...
    return { kind: 'trap', text: withLandmark(`$坍塌地砖$碎石自头顶坠落，造成 ${damage} 点伤害。`) };
};
const dungeonArrivalEvent = async (connection, characterId) => {
...
const openDungeonChest = async (qqUserId, cellId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const [cells] = await connection.execute('SELECT * FROM dungeon_cells WHERE id=? AND pos_x=? AND pos_y=? AND pos_z=? AND cell_type=\'chest\' FOR UPDATE', [cellId, character.pos_x, character.pos_y, character.pos_z]);
    const cell = cells[0];
    if (!cell || Number(cell.chest_opened))
        throw new Error('宝箱已经被开启，或你已离开宝箱旁。');
    const quality = cell.chest_quality ?? '青铜';
    const z = Math.abs(Number(character.pos_z));
    const cap = z <= 10 ? 500 : z <= 20 ? 1000 : 2000;
    const upper = Math.min(cap, quality === '青铜' ? (z <= 10 ? 90 : 180) : quality === '白银' ? (z <= 20 ? 420 : 900) : 1800);
    let copper = 20 + Math.floor(Math.random() * Math.max(1, upper - 19));
    let silver = 0;
    const gold = 0;
    if (quality !== '青铜' && Math.random() < (quality === '黄金' ? .70 : .38)) {
        silver = 1 + Math.floor(Math.random() * (quality === '黄金' ? 4 : 2));
        copper = Math.max(1, copper - silver * 100);
    }
    const walletValue = copper + silver * 100 + gold * 10000;
    const pool = quality === '黄金' ? ['magic_heartcore', 'meteor_iron', 'star_copper', 'moon_silver'] : quality === '白银' ? ['beast_core', 'magic_heartcore', 'living_wood', 'meteor_iron'] : ['beast_core', 'living_wood', 'meteor_iron'];
    const itemCode = random(pool);
    const quantity = 1 + Math.floor(Math.random() * (quality === '青铜' ? 2 : 3));
    ...
    const [item] = await connection.execute('SELECT name FROM item_definitions WHERE code=?', [itemCode]);
    let blueprintName;
    if (character.secondary_profession_code === 'deconstructor') {
        ...
        const [bossRows] = await connection.execute(`SELECT 1 FROM dungeon_monsters dm JOIN monster_spawns s ON s.id=dm.spawn_id
      WHERE dm.dungeon_id=? AND dm.is_boss=1 AND s.pos_z=? AND s.defeated_at IS NOT NULL LIMIT 1`, [cell.dungeon_id, character.pos_z]);
        const chestTypes = new Set([qualityCode]);
        if (floor === 3 && qualityCode === 'gold' && bossRows[0])
            chestTypes.add('boss_gold');
        ...
        if (candidate && !await ownsDungeonItem(connection, character.id, `${candidate.code}_blueprint`)) {
            const [blueprints] = await connection.execute('SELECT id,name FROM item_definitions WHERE code=? LIMIT 1', [`${candidate.code}_blueprint`]);
            if (blueprints[0]) {
...
const changeDungeonFloor = async (qqUserId, direction) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const cell = await dungeonCellAt(connection, Number(character.current_region_id), Number(character.pos_x), Number(character.pos_y), Number(character.pos_z));
    if (!cell)
        throw new Error('你当前不在可通行的地下迷宫格子中。');
    const [dungeons] = await connection.execute('SELECT d.* FROM dungeon_instances d JOIN dungeon_cells dc ON dc.dungeon_id=d.id WHERE dc.id=? AND d.state=\'active\' FOR UPDATE', [cell.id]);
    const dungeon = dungeons[0];
    if (!dungeon)
        throw new Error('这座迷宫已经关闭。');
    if (direction === 'leave' || direction === 'escape') {
        let usedTeleporter = false;
        if (cell.cell_type !== 'entrance' || direction === 'escape') {
            const [consume] = await connection.execute(`UPDATE player_inventory pi JOIN item_definitions i ON i.id=pi.item_id
        SET pi.quantity=pi.quantity-1 WHERE pi.character_id=? AND i.code='demon_breaker_teleporter' AND pi.quantity>0`, [character.id]);
            if (Number(consume.affectedRows) !== 1)
                throw new Error(direction === 'escape' ? '脱离地下迷宫需要一枚破魔传送器。' : '只能从第一层入口离开迷宫；持有破魔传送器时可从任意位置强制脱离。');
        ...
        return { action: 'leave', x: Number(dungeon.entrance_x), y: Number(dungeon.entrance_y), z: 0, usedTeleporter };
    }
    const expected = direction === 'down' ? 'stairs_down' : 'stairs_up';
    if (cell.cell_type !== expected)
        throw new Error(direction === 'down' ? '这里没有通向下一层的石阶。' : '这里没有通向上一层的石阶。');
    if (direction === 'down') {
        const [leaders] = await connection.execute(`SELECT s.id,t.name,s.pos_x,s.pos_y FROM dungeon_monsters dm
      JOIN monster_spawns s ON s.id=dm.spawn_id JOIN monster_templates t ON t.id=s.template_id
      WHERE dm.dungeon_id=? AND s.pos_z=? AND t.monster_class='boss' AND s.defeated_at IS NULL LIMIT 1 FOR UPDATE`, [dungeon.id, character.pos_z]);
        ...
        if (leader) {
            const [leaderCell] = await connection.execute('SELECT 1 FROM dungeon_cells WHERE dungeon_id=? AND pos_x=? AND pos_y=? AND pos_z=? LIMIT 1', [dungeon.id, leader.pos_x, leader.pos_y, character.pos_z]);
            if (!leaderCell[0]) {
                const [guardCells] = await connection.execute(`SELECT * FROM dungeon_cells WHERE dungeon_id=? AND pos_z=? AND cell_type IN ('path','trap','boss')
          ORDER BY ABS(pos_x-?)+ABS(pos_y-?),id LIMIT 1`, [dungeon.id, character.pos_z, cell.pos_x, cell.pos_y]);
                if (guardCells[0])
                    await connection.execute('UPDATE monster_spawns SET pos_x=?,pos_y=? WHERE id=?', [guardCells[0].pos_x, guardCells[0].pos_y, leader.id]);
            }
            throw new Error(`【${leader.name}】仍守在通往下一层的道路上。击败它后，石阶才会开启。`);
        }
    }
    if (direction === 'down')
        recordAchievement(connection, Number(character.id), ['ACH_L13']);
    const targetZ = Number(character.pos_z) + (direction === 'down' ? -10 : 10);
    const targetType = direction === 'down' ? 'stairs_up' : Number(targetZ) === -10 ? 'entrance' : 'stairs_down';
    const [target] = await connection.execute('SELECT * FROM dungeon_cells WHERE dungeon_id=? AND pos_z=? AND cell_type=? LIMIT 1', [dungeon.id, targetZ, targetType]);
    if (!target[0])
        throw new Error('楼层之间的石阶已经坍塌。');
...
const dungeonPlayersInRange = async (connection, characterId, regionIdValue, x, y, z, range) => {
    const [rows] = await connection.execute(`SELECT c.game_id,c.name,c.pos_x,c.pos_y,hv.character_id IS NOT NULL AS in_home,
...
const pvpActionFor = async (connection, character, manual = false) => {
    ...
    if (setting?.enabled) {
        ...
        if (potionId) {
            ...
            if (items[0])
                return { type: 'item', id: Number(items[0].id), code: String(items[0].code), name: items[0].name, effect: typeof items[0].effect_json === 'string' ? JSON.parse(items[0].effect_json) : (items[0].effect_json ?? {}) };
        }
    ...
    WHERE a.character_id=? ORDER BY a.sequence_no`, [character.id]);
        if (actions.length) {
            ...
            if (picked.skill_id === null || picked.active_skill_id === null || !picked.name || !picked.category)
                return { type: 'attack' };
            return { type: 'skill', name: picked.name, category: picked.category, manaCost: Number(picked.mana_cost), power: Number(picked.power) };
        }
        return { type: 'attack' };
    }
    if (manual)
        return { type: 'attack' };
    const [quick] = await connection.execute('SELECT s.name,s.category,s.mana_cost,s.power FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id WHERE ps.character_id=? AND ps.quick_slot IS NOT NULL AND s.category IN (\'physical\',\'magic\',\'utility\') ORDER BY ps.quick_slot', [character.id]);
    if (!quick.length)
        return { type: 'attack' };
    const picked = random(quick);
    return { type: 'skill', name: picked.name, category: picked.category, manaCost: Number(picked.mana_cost), power: Number(picked.power) };
};
const resolvePvpAction = async (connection, actor, target, action) => {
    if (action.type === 'item') {
        ...
        actor.current_hp = hp;
        actor.current_mp = mp;
        return `【${actor.name}】使用【${action.name}】，HP ${oldHp}→${hp}｜MP ${oldMp}→${mp}。`;
    }
    const skill = action.type === 'skill' && Number(actor.current_mp) >= action.manaCost ? action : null;
    if (action.type === 'skill' && !skill)
        return resolvePvpAction(connection, actor, target, { type: 'attack' });
    if (skill) {
    ...
    if (skill?.category === 'utility') {
        ...
        return `【${actor.name}】释放技能「${skill.name}」，但该辅助技能尚未在 PvP 对抗中形成直接伤害。`;
    }
    const magic = skill?.category === 'magic';
    const attack = magic ? Number(actor.magic_attack) : Number(actor.physical_attack);
    const defense = magic ? Number(target.magic_defense) : Number(target.physical_defense);
    const label = skill ? `释放技能「${skill.name}」` : '普通攻击';
    const armorSets = await armorSetsFor(connection, [Number(actor.id), Number(target.id)]);
    if (Math.random() >= resolvedHitChance(opposedChance(Number(actor.accuracy), Number(target.evasion)), 0, 1, 0, strikeCorrections({ armorSet: armorSets.get(Number(actor.id)) }, { armorSet: armorSets.get(Number(target.id)) }))) {
        await recordPvpAttack(connection, actor, target, label, 0, 'miss');
        return `【${actor.name}】${label}，但【${target.name}】闪避了攻击。`;
    }
    const damage = Math.max(1, Math.floor(attack * attack / Math.max(1, attack + defense) * (skill ? skill.power / 100 : 1)));
    const hp = Math.max(0, Number(target.current_hp) - damage);
    const defeated = hp <= 0;
    if (defeated) {
        const settlement = await resolvePvpVictory(connection, Number(actor.id), Number(target.id));
        target.current_hp = 1;
...
const dungeonPvP = async (qqUserId, targetGameId) => withTransaction(async (connection) => {
    const attacker = await characterFor(connection, qqUserId, true);
    const dungeonRegion = await regionId(connection, DUNGEON_REGION_CODE);
    if (Number(attacker.current_region_id) !== dungeonRegion)
        throw new Error('只能在地下迷宫内进行 PvP。');
    ...
    if (!target || Number(target.id) === Number(attacker.id) || Number(target.current_region_id) !== dungeonRegion || Number(target.pos_z) !== Number(attacker.pos_z) || Math.abs(Number(target.pos_x) - Number(attacker.pos_x)) + Math.abs(Number(target.pos_y) - Number(attacker.pos_y)) > 1)
        throw new Error('目标不在你相邻的地下迷宫格子中。');
    await assertPvpDefeatUnprotected(connection, Number(target.id));
    if (await isFriendRelation(connection, Number(attacker.id), Number(target.id)))
...
const interactDungeonPlayer = async (qqUserId, targetGameId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const [targets] = await connection.execute('SELECT * FROM characters WHERE game_id=? AND npc_code IS NULL FOR UPDATE', [targetGameId]);
    const target = targets[0];
    const range = 0;
    if (!target || Number(target.id) === Number(character.id) || Number(target.current_region_id) !== Number(character.current_region_id) || Number(target.pos_z) !== Number(character.pos_z) || Math.abs(Number(target.pos_x) - Number(character.pos_x)) + Math.abs(Number(target.pos_y) - Number(character.pos_y)) > range)
        throw new Error('目标不在同一坐标。');
    const affinity = await recordFriendInteraction(connection, Number(character.id), Number(target.id));
    if (!affinity)
        return { name: target.name, isFriend: false, text: `你向【${target.name}】打了个招呼。对方也朝你点了点头。\n\n若想记录更深的同行记忆，可以先互相成为好友。` };
...
const closeDungeonForBossSpawns = async (connection, spawnIds, members = []) => {
    if (!spawnIds.length)
        return false;
    const [rows] = await connection.execute(`SELECT dungeon_id FROM dungeon_monsters WHERE is_boss=1 AND spawn_id IN (${spawnIds.map(() => '?').join(',')}) LIMIT 1 FOR UPDATE`, spawnIds);
    const dungeonId = Number(rows[0]?.dungeon_id ?? 0);
    if (!dungeonId)
        return false;
    const [dungeons] = await connection.execute('SELECT * FROM dungeon_instances WHERE id=? AND state=\'active\' FOR UPDATE', [dungeonId]);
    const dungeon = dungeons[0];
    ...
    for (const m of contributors)
        recordAchievement(connection, Number(m.id), [{ metric: 'ACH_L13', cooperationKey: 'dungeon:' + dungeonId }, { metric: 'ACH_L14', cooperationKey: 'dungeon:' + dungeonId }], 'dungeon:' + dungeonId);
    const dungeonRegion = await regionId(connection, DUNGEON_REGION_CODE);
...
export { changeDungeonFloor, closeDungeonForBossSpawns, dungeonArrivalEvent, dungeonCellAt, dungeonEntranceAt, dungeonEvents, dungeonPlayersInRange, dungeonPvP, dungeonTrackingHint, enterDungeon, interactDungeonPlayer, openDungeonChest, rebuildDungeons, refreshDungeons };
