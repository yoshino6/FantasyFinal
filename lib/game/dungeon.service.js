...
import { correctedHitChance, strikeCorrections, opposedChance } from './combat-math.js';
...
const resolvePvpAction = async (connection, actor, target, action) => {
    ...
    if (Math.random() >= correctedHitChance(opposedChance(Number(actor.accuracy), Number(target.evasion)), strikeCorrections({ armorSet: armorSets.get(Number(actor.id)) }, { armorSet: armorSets.get(Number(target.id)) }))) {
