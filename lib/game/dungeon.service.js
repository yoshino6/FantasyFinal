...
const migrateDungeonBossTraits = async (connection) => {
    const [rows] = await connection.execute(`SELECT s.id FROM dungeon_monsters dm
    JOIN monster_spawns s ON s.id=dm.spawn_id
    WHERE (dm.is_boss=1 OR dm.is_floor_leader=1) AND JSON_UNQUOTE(JSON_EXTRACT(s.traits_json,'$[0].code'))='dungeon_boss'`);
    for (const row of rows)
        await connection.execute('UPDATE monster_spawns SET traits_json=? WHERE id=?', [JSON.stringify([randomDungeonBossTrait()]), row.id]);
};
...
const createDungeon = async (connection) => {
    ...
    for (const slot of floorBossSlots) {
        ...
        const hp = monsterCombatStats({ ...boss, traits_json: [trait] }).hpMax;
