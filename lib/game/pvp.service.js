...
import { resolveStrike, strikeCorrections, directDamageVariance } from './combat-math.js';
...
const characterFor = async (connection, qqUserId) => {
    ...
    if (heart)
        heartPerception.set(rows[0], heart.delta.perception * playerGrowthShares(Number(rows[0].level)) + heart.offset.perception);
    return rows[0];
};
...
const resolveAction = async (connection, actor, target, action, sessionId, targetCooldowns, actorCooldowns, actorDevices = new Set(), targetDevices = new Set(), context) => {
    ...
    if (action.type === 'device') {
        ...
        const pseudo = { type: 'skill', id: 0, code: `device_${action.skill.code}`, name: `${action.deviceName}·${action.skill.name}`, category: magic ? 'magic' : 'physical', requiredWeaponType: null, manaCost: 0, power: Number(action.skill.power ?? 100), cooldown: 0 };
