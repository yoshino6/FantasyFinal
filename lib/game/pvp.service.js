...
const resolveAction = async (connection, actor, target, action, sessionId, targetCooldowns, actorCooldowns, actorDevices = new Set(), targetDevices = new Set(), context) => {
    ...
    if (action.type === 'device') {
        ...
        const pseudo = { type: 'skill', id: 0, code: `device_${action.skill.code}`, name: `${action.deviceName}·${action.skill.name}`, category: magic ? 'magic' : 'physical', requiredWeaponType: null, manaCost: 0, power: aoeSkillPower(`device_${action.skill.code}`, Number(action.skill.power ?? 100)), cooldown: 0 };
    ...
    const element = String(elementRows[0]?.element ?? '无');
