...
import { validateFolioCast, folioEndTurn, resolveFolioStrike } from './folio-combat.js';
import { playerGrowthShares } from './growth-rules.js';
import { aoeSkillPower } from './aoe-damage.config.js';
import { heartGrowthAdjustment } from './heart-question.service.js';
...
import { assertNoNegotiation } from './negotiation.service.js';
import { hiddenReorder, HiddenBattleError, executeHiddenCombat, hiddenDeviceSnapshot, hiddenNativeDevice, hiddenEndTurn } from './hidden-combat.js';
import { hiddenBattleContext, submitHiddenDraft, initializeHiddenBattleUnits } from './hidden-battle.service.js';
import { isHiddenSkill } from './hidden-profession.config.js';
import { hiddenResourceView, gainHiddenResource } from './hidden-combat-state.js';
...
import { normalSkillSpecializationFacts } from './achievement-hooks.js';
import { recordAchievement } from './achievement-events.js';
import { achievementBookSkillUsed } from './achievement-state.js';
...
import { isInHome } from './home.service.js';
import { isFriendRelation } from './social.service.js';
import { strikeCorrections, directDamageVariance } from './combat-math.js';
import { createPvpCombatRules } from './pvp-combat-rule-adapter.js';
import { readRuleState, maskRuleBattleLog, ruleStatusSummary, additivePercentFactor } from './combat-rule-registry.js';
import { residentSkillByCode } from './resident-skill.config.js';
import { initializeCombatDeviceEnergy, combatDeviceSlotsFor, activeDeviceSkillByCode, restoreCombatDeviceEnergy } from './device.service.js';
import { isAdvancedProfessionSkillCode } from './advanced-profession.config.js';
import { equippedEnchantmentEffects, cardElementDamageMultiplier, cardIncomingDamageMultiplier } from './equipment-enchantment-effects.js';
import { resolveDirectAttackElement } from './combat-element.js';
import { resetCardMovementCharge } from './monster-card-exploration.service.js';
import { recordPvpAchievements } from './achievement-pvp.js';

const random = (items) => items[Math.floor(Math.random() * items.length)];
const heartPerception = new WeakMap();
const perceptionRange = (character) => {
...
const activeDeviceCodesFor = async (connection, characterId) => {
    const [rows] = await connection.execute(`SELECT i.code FROM player_active_devices ad
...
const characterFor = async (connection, qqUserId) => {
    const [rows] = await connection.execute('SELECT c.* FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1 FOR UPDATE', [qqUserId]);
    if (!rows[0])
        throw new Error('请先发送“注册”创建角色。');
    const heart = await heartGrowthAdjustment(connection, Number(rows[0].id));
    if (heart)
        heartPerception.set(rows[0], heart.delta.perception * playerGrowthShares(Number(rows[0].level)));
    return rows[0];
};
const townRegion = async (connection) => {
    const [rows] = await connection.execute('SELECT id,name FROM map_regions WHERE code=\'baina_town\' LIMIT 1');
    if (!rows[0])
        throw new Error('百纳镇区域尚未初始化。');
    return rows[0];
};
const wanted = async (connection, characterId, regionId) => {
    const [rows] = await connection.execute('SELECT id FROM player_warrants WHERE wanted_character_id=? AND city_region_id=? AND status=\'active\' LIMIT 1 FOR UPDATE', [characterId, regionId]);
    return Number(rows[0]?.id ?? 0);
};
const activeWarrants = async (connection, characterId) => {
    const [rows] = await connection.execute('SELECT id FROM player_warrants WHERE wanted_character_id=? AND status=\'active\' FOR UPDATE', [characterId]);
    return rows.map(row => Number(row.id));
};
const assertPvpDefeatUnprotected = async (connection, characterId) => {
    await connection.execute('DELETE FROM player_pvp_defeat_protections WHERE character_id=? AND expires_at<=NOW()', [characterId]);
    const [rows] = await connection.execute('SELECT 1 FROM player_pvp_defeat_protections WHERE character_id=? AND expires_at>NOW() FOR UPDATE', [characterId]);
    if (rows[0])
        throw new Error('目标正处于战败保护中，需等待对方下一次游戏指令后才能再次攻击。');
};
const takePvpDefeatNotice = async (qqUserId) => withTransaction(async (connection) => {
    const [characters] = await connection.execute(`SELECT c.id FROM characters c
    ...
    const [protections] = await connection.execute(`SELECT attacker_name,notice_text,defeated_at
...
const completePvpDefeatProtection = async (qqUserId) => withTransaction(async (connection) => {
    const [characters] = await connection.execute(`SELECT c.id FROM characters c
    JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1 FOR UPDATE`, [qqUserId]);
    const character = characters[0];
    if (!character)
...
const recordPvpAttack = async (connection, attacker, defender, actionName, damage, outcome, lootText = null) => {
...
const createPvpBattleLog = async (connection, attacker, defender, battleType, id = randomUUID()) => {
...
const finishPvpBattleLog = async (connection, id, outcome, winner = null, lootText = null, escapedCharacterId = null) => {
    const [finished] = await connection.execute(`UPDATE player_pvp_battle_logs SET outcome=?,winner_character_id=?,winner_name=?,loot_text=?,ended_at=NOW() WHERE id=? AND outcome='ongoing'`, [outcome, winner?.id ?? null, winner?.name ?? null, lootText, id]);
    if (!finished.affectedRows)
        return;
    const [financeRows] = await connection.execute('SELECT attacker_character_id,defender_character_id,battle_type FROM player_pvp_battle_logs WHERE id=? LIMIT 1', [id]);
    const financeBattle = financeRows[0];
    if (financeBattle)
        for (const characterId of [Number(financeBattle.attacker_character_id), Number(financeBattle.defender_character_id)]) {
    ...
    if (financeBattle && outcome !== 'draw') {
        ...
        if (affected !== null && [Number(financeBattle.attacker_character_id), Number(financeBattle.defender_character_id)].includes(affected))
            await (await import('./finance-settlement.js')).recordFinanceSignal(connection, { sourceKey: `pvp:${id}`, factionCode: 'adventurer_guild', characterId: affected, eventType: outcome === 'escaped' ? 'pvp.escaped' : lootText ? 'pvp.robbed' : 'pvp.defeated', score: -1 });
    }
    if (winner && ['attacker_win', 'defender_win'].includes(outcome))
        await recordPvpAchievements(connection, id, Number(winner.id));
};
const pvpBattleHistory = async (qqUserId, page = 1, filter = '全部', keyword = '') => {
    const pool = await getPool();
    const [characters] = await pool.execute('SELECT c.id FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1', [qqUserId]);
    const character = characters[0];
    if (!character)
    ...
    const values = filter === '全部' ? [character.id, character.id] : [character.id];
    const search = keyword.trim() ? ' AND (attacker_name LIKE ? OR defender_name LIKE ? OR battle_type LIKE ? OR outcome LIKE ?)' : '';
    if (keyword.trim())
        values.push(`%${keyword.trim()}%`, `%${keyword.trim()}%`, `%${keyword.trim()}%`, `%${keyword.trim()}%`);
    const [countRows] = await pool.execute(`SELECT COUNT(*) AS total FROM player_pvp_battle_logs WHERE ${where}${search}`, values);
    const total = Number(countRows[0]?.total ?? 0);
    const totalPages = Math.max(1, Math.ceil(total / 5));
    const currentPage = Math.min(Math.max(1, page), totalPages);
    const [rows] = await pool.execute(`SELECT * FROM player_pvp_battle_logs WHERE ${where}${search} ORDER BY started_at DESC,id DESC LIMIT 5 OFFSET ?`, [...values, String((currentPage - 1) * 5)]);
...
const recordWarrantSighting = async (connection, characterId, regionId, x, y) => {
...
const recordWarrantVictim = async (connection, warrantId, targetCharacterId) => {
    if (warrantId)
        await connection.execute('INSERT IGNORE INTO player_warrant_victims (warrant_id,target_character_id) VALUES (?,?)', [warrantId, targetCharacterId]);
};
const actionFor = async (connection, character, manual = false) => {
    const [settings] = await connection.execute('SELECT * FROM player_pvp_auto_battle_settings WHERE character_id=? FOR UPDATE', [character.id]);
    ...
    if (setting?.enabled) {
        ...
        if (potionId) {
            const [items] = await connection.execute('SELECT i.id,i.code,i.name,i.effect_json FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.item_id=? AND pi.quantity>0 AND i.item_type=\'consumable\' LIMIT 1 FOR UPDATE', [character.id, potionId]);
            if (items[0])
                return { type: 'item', id: Number(items[0].id), code: items[0].code, name: items[0].name, effect: record(items[0].effect_json) };
        }
        const [actions] = await connection.execute(`SELECT a.skill_id,
    ...
    WHERE a.character_id=? ORDER BY a.sequence_no`, [character.id]);
        if (actions.length) {
            ...
            if (picked.skill_id === null || picked.active_skill_id === null || !picked.code || !picked.name || !picked.category)
                return { type: 'attack' };
    ...
    if (manual)
        return { type: 'attack' };
    const [quick] = await connection.execute('SELECT s.id,s.code,s.name,s.category,s.required_weapon_type,s.mana_cost,s.power,s.cooldown_turns FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id WHERE ps.character_id=? AND ps.quick_slot IS NOT NULL AND s.category IN (\'physical\',\'magic\',\'utility\') ORDER BY ps.quick_slot', [character.id]);
    if (!quick.length)
        return { type: 'attack' };
...
const creditItem = async (connection, characterId, itemId, quantity) => {
...
const recordPvpLootSale = async (connection, holderId, itemId, quantity, saleCopper) => {
    let remaining = Math.max(0, Math.floor(quantity));
    let unallocatedValue = Math.max(0, Math.floor(saleCopper));
    const [rows] = await connection.execute(`SELECT * FROM pvp_stolen_loot
    ...
    for (const row of rows) {
        ...
        remaining -= moved;
        unallocatedValue -= value;
    }
};
const transferStolenItem = async (connection, fromId, toId, itemId, quantity) => {
    ...
    if (remaining)
        await connection.execute('INSERT INTO pvp_stolen_loot (original_owner_character_id,holder_character_id,item_id,quantity,held_quantity) VALUES (?,?,?,?,?)', [fromId, toId, itemId, remaining, remaining]);
};
const transferStolenCopper = async (connection, fromId, toId, copper) => {
    let remaining = copper;
    const [rows] = await connection.execute('SELECT * FROM pvp_stolen_loot WHERE holder_character_id=? AND item_id IS NULL AND returned_at IS NULL AND copper_amount>0 ORDER BY acquired_at,id FOR UPDATE', [fromId]);
    for (const row of rows) {
        if (!remaining)
            break;
        const moved = Math.min(remaining, Number(row.copper_amount));
        if (moved === Number(row.copper_amount))
            await connection.execute('UPDATE pvp_stolen_loot SET holder_character_id=? WHERE id=?', [toId, row.id]);
        else {
    ...
    if (remaining)
        await connection.execute('INSERT INTO pvp_stolen_loot (original_owner_character_id,holder_character_id,copper_amount) VALUES (?,?,?)', [fromId, toId, remaining]);
};
const collectCityDebts = async (connection, characterId, cityRegionId) => {
    const [characterRows] = await connection.execute('SELECT copper_coins FROM characters WHERE id=? FOR UPDATE', [characterId]);
    let available = Number(characterRows[0]?.copper_coins ?? 0);
    if (!characterRows[0])
        return { collected: 0, remaining: 0 };
    const [debts] = await connection.execute(`SELECT * FROM player_city_debts
...
const settlePvpDefeat = async (connection, winnerId, loserId) => {
    const [loserRows] = await connection.execute('SELECT c.*,r.name AS region_name,r.code AS region_code FROM characters c JOIN map_regions r ON r.id=c.current_region_id WHERE c.id=? FOR UPDATE', [loserId]);
    const loser = loserRows[0];
    if (!loser)
        return { captured: false, restitution: undefined, text: '' };
    ...
    const [stolen] = await connection.execute('SELECT * FROM pvp_stolen_loot WHERE holder_character_id=? AND returned_at IS NULL FOR UPDATE', [loserId]);
    const restitutionId = stolen.length ? randomUUID() : null;
    let returnedItems = 0;
    let returnedCopper = 0;
    let debtCopper = 0;
    const [balanceRows] = await connection.execute('SELECT copper_coins FROM characters WHERE id=? FOR UPDATE', [loserId]);
    ...
    for (const loot of stolen) {
        ...
        if (loot.item_id && Number(loot.quantity) > 0) {
            ...
            if (heldQuantity)
                await connection.execute('UPDATE player_inventory SET quantity=GREATEST(0,quantity-?) WHERE character_id=? AND item_id=?', [heldQuantity, loserId, loot.item_id]);
        ...
        let rowDebt = 0;
        if (owedCopper > 0) {
            chargedCopper = Math.min(availableCopper, owedCopper);
            rowDebt = owedCopper - chargedCopper;
            if (chargedCopper) {
            ...
            if (rowDebt && restitutionId)
                await connection.execute('INSERT INTO player_city_debts (debtor_character_id,original_owner_character_id,city_region_id,restitution_id,amount_copper) VALUES (?,?,?,?,?)', [loserId, loot.original_owner_character_id, debtTown.id, restitutionId, rowDebt]);
            returnedCopper += chargedCopper;
            debtCopper += rowDebt;
        }
    ...
    const rewards = warrants.length
        ? (await connection.execute('SELECT * FROM player_warrant_rewards WHERE warrant_id IN (' + warrants.map(() => '?').join(',') + ') AND claimed_at IS NULL FOR UPDATE', warrants))[0]
        : [];
    let bountyItems = 0;
    let bountyCopper = 0;
    for (const reward of rewards) {
        if (reward.reward_item_id && Number(reward.quantity) > 0) {
            await creditItem(connection, winnerId, Number(reward.reward_item_id), Number(reward.quantity));
            bountyItems += Number(reward.quantity);
        }
        if (Number(reward.copper_amount) > 0) {
            await connection.execute('UPDATE characters SET copper_coins=copper_coins+? WHERE id=?', [reward.copper_amount, winnerId]);
            bountyCopper += Number(reward.copper_amount);
        }
    ...
    const restitution = restitutionId ? { id: restitutionId, itemCount: returnedItems, copper: returnedCopper, debt: debtCopper } : undefined;
...
const settleCityPursuitDefeat = async (connection, loserId, cityRegionId) => {
    const [loserRows] = await connection.execute('SELECT * FROM characters WHERE id=? FOR UPDATE', [loserId]);
    const loser = loserRows[0];
    if (!loser)
        return { text: '' };
    const warrants = await activeWarrants(connection, loserId);
    if (warrants.length)
        await connection.execute('UPDATE player_warrants SET status=\'captured\',captured_by_character_id=NULL,captured_at=NOW() WHERE wanted_character_id=? AND status=\'active\'', [loserId]);
    await connection.execute('UPDATE characters SET current_hp=1,activity_status=\'detained\',rest_started_at=NULL,detained_until=DATE_ADD(NOW(),INTERVAL 12 HOUR) WHERE id=?', [loserId]);
    const [stolen] = await connection.execute('SELECT * FROM pvp_stolen_loot WHERE holder_character_id=? AND returned_at IS NULL FOR UPDATE', [loserId]);
    const restitutionId = stolen.length ? randomUUID() : null;
    let returnedItems = 0;
    ...
    for (const loot of stolen) {
        ...
        const owedCopper = Number(loot.copper_amount) + Number(loot.sale_copper_amount);
        const chargedCopper = Math.min(availableCopper, owedCopper);
        const rowDebt = owedCopper - chargedCopper;
        if (chargedCopper) {
            await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=?', [chargedCopper, loserId]);
            await connection.execute('UPDATE characters SET copper_coins=copper_coins+? WHERE id=?', [chargedCopper, loot.original_owner_character_id]);
            availableCopper -= chargedCopper;
        ...
        if (rowDebt && restitutionId)
            await connection.execute('INSERT INTO player_city_debts (debtor_character_id,original_owner_character_id,city_region_id,restitution_id,amount_copper) VALUES (?,?,?,?,?)', [loserId, loot.original_owner_character_id, cityRegionId, restitutionId, rowDebt]);
        returnedCopper += chargedCopper;
        debtCopper += rowDebt;
        await connection.execute('UPDATE pvp_stolen_loot SET returned_at=NOW(),restitution_id=?,restitution_charged_copper=?,restitution_debt_copper=? WHERE id=?', [restitutionId, chargedCopper, rowDebt, loot.id]);
    }
...
const stealFromLoser = async (connection, winner, loser, extra = false) => {
    const [items] = await connection.execute('SELECT pi.item_id,i.name,pi.quantity-pi.trade_bound_quantity-pi.personal_bound_quantity AS quantity,i.trade_price FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.quantity>pi.trade_bound_quantity+pi.personal_bound_quantity AND i.is_tradeable=1 AND i.trade_price>0 AND i.item_category NOT IN (\'特殊\',\'地图\',\'货币\') FOR UPDATE', [loser.id]);
    ...
    for (let count = 0; count < (extra ? 2 : 1) && weighted.length; count += 1) {
        ...
        const quantity = Math.max(1, Math.min(Number(item.quantity), extra ? 2 : 1));
        ...
        if (!item.quantity)
            weighted = weighted.filter(candidate => candidate !== item);
    ...
    const [coins] = await connection.execute('SELECT copper_coins FROM characters WHERE id=? FOR UPDATE', [loser.id]);
...
const resolvePvpVictory = async (connection, winnerId, loserId) => {
    const [winners] = await connection.execute('SELECT * FROM characters WHERE id=? FOR UPDATE', [winnerId]);
    const [losers] = await connection.execute('SELECT * FROM characters WHERE id=? FOR UPDATE', [loserId]);
...
const resolveAction = async (connection, actor, target, action, sessionId, targetCooldowns, actorCooldowns, actorDevices = new Set(), targetDevices = new Set(), context) => {
    if (action.type === 'device_charge') {
        if (!sessionId)
            return { text: `【${actor.name}】尝试为【${action.deviceName}】充能，但当前不在可充能战斗中。`, defeated: false };
        const [rows] = await connection.execute(`SELECT current_energy,max_energy FROM combat_device_energy
      WHERE battle_kind='pvp' AND session_id=? AND character_id=? AND instance_id=? FOR UPDATE`, [sessionId, actor.id, action.instanceId]);
        const energy = rows[0];
        if (!energy)
            return { text: `【${actor.name}】尝试为【${action.deviceName}】充能，但该异械未在本场生效。`, defeated: false };
        const next = Math.min(Number(energy.max_energy), Number(energy.current_energy) + 30);
        if (context && next - Number(energy.current_energy) >= 30)
            gainHiddenResource(context.get(Number(actor.id)), context.rule.turn, 10);
    ...
    if (action.type === 'device') {
        if (!sessionId)
            return { text: `【${actor.name}】启动【${action.deviceName}】失败。`, defeated: false };
        const configured = activeDeviceSkillByCode.get(action.skill.code);
        if (!configured || configured.deviceCode !== action.deviceCode)
            return { text: `【${actor.name}】启动【${action.deviceName}】失败：回路配置无效。`, defeated: false };
        ...
        const heal = (ratio) => {
            const before = Number(recipient.current_hp);
            recipient.current_hp = Math.min(Number(recipient.hp_max), before + Math.floor(Number(recipient.hp_max) * ratio));
        ...
        if (action.skill.effect === 'weave_repair') {
            for (const code of ['device_slow', 'device_exposed', 'device_bind', 'device_stun', 'device_burn', 'device_poison'])
                delete recipientCooldowns?.[code];
            return { text: `【${actor.name}】启动【${action.deviceName}】·缝补程序，${heal(.18)}，并净化 1 个负面状态。`, defeated: false };
        }
        if (action.skill.effect === 'autonomous_repair') {
            const restored = Number(recipientCooldowns?.device_barrier ?? 0) > 0 ? await restoreCombatDeviceEnergy(connection, sessionId, Number(recipient.id), 20, undefined, 'pvp') : 0;
            return { text: `【${actor.name}】启动【${action.deviceName}】·应急重构，${heal(.30)}${restored ? `，并为 ${restored} 件异械各恢复至多 20 点充能` : ''}。`, defeated: false };
        }
        ...
        if (action.skill.effect === 'physical_evade_once')
            return { text: `【${actor.name}】启动【${action.deviceName}】，获得一次物理闪避。`, defeated: false };
        if (action.skill.effect === 'easter_egg') {
            ...
            for (const code of picked)
                recipientCooldowns[code] = code === 'device_rocket_boost' ? Math.min(100, Number(recipientCooldowns[code] ?? 0) + 30) : 3;
            return { text: `【${actor.name}】启动【${action.deviceName}】·彩蛋投掷，【${recipient.name}】随机获得 5 种效果。`, defeated: false };
        }
        if (action.skill.effect === 'precision_aim') {
            recipientCooldowns.device_precision_aim = 99;
            return { text: `【${actor.name}】启动【${action.deviceName}】·精准瞄准，【${recipient.name}】获得命中与暴击提升，直到下次受击。`, defeated: false };
        }
        if (action.skill.effect === 'gravity_tether') {
            recipientCooldowns.device_slow = 2;
            recipientCooldowns.device_evasion_down = 2;
            return { text: `【${actor.name}】启动【${action.deviceName}】·引力牵引，【${recipient.name}】速度、闪避降低 30%。`, defeated: false };
        }
        if (action.skill.effect === 'fold_barrier') {
            recipientCooldowns.device_barrier = actorDevices.has('fold_barrier_generator') ? 3 : 2;
            return { text: `【${actor.name}】启动【${action.deviceName}】·折叠壁垒，【${recipient.name}】获得 15% 减伤。`, defeated: false };
        }
        if (action.skill.effect === 'phase_decoy') {
            recipientCooldowns.device_phase_decoy = 99;
            return { text: `【${actor.name}】启动【${action.deviceName}】·相位替身，【${recipient.name}】的下次直接伤害降低 80%。`, defeated: false };
        }
        if (action.skill.effect === 'counter_spider') {
            ...
            if (removable) {
                delete recipientCooldowns[removable];
                return { text: `【${actor.name}】启动【${action.deviceName}】·拆解射线，移除了【${recipient.name}】的一项正面状态。`, defeated: false };
            }
            recipientCooldowns.device_exposed = 2;
            return { text: `【${actor.name}】启动【${action.deviceName}】·拆解射线，【${recipient.name}】获得易伤 15%。`, defeated: false };
        }
        if (action.skill.effect === 'reactor_overcharge')
            actor.current_hp = Math.max(1, Number(actor.current_hp) - Math.floor(Number(actor.current_hp) * .15));
        ...
        const pseudo = { type: 'skill', id: 0, code: `device_${action.skill.code}`, name: `${action.deviceName}·${action.skill.name}`, category: magic ? 'magic' : 'physical', requiredWeaponType: null, manaCost: 0, power: aoeSkillPower(`device_${action.skill.code}`, Number(action.skill.power ?? 100)), cooldown: 0, element: deviceElement };
        ...
        if (!result.defeated && action.skill.effect === 'shock_pile' && Math.random() < .65)
            targetCooldowns.device_stun = 2;
        if (!result.defeated && action.skill.effect === 'frost_pulse')
            targetCooldowns.device_slow = 2;
        if (!result.defeated && (action.skill.effect === 'coil_cannon'))
            targetCooldowns.device_exposed = 2;
        return result;
    }
    if (action.type === 'item' && !combatItemEffect(action.effect, true))
        return { text: '该道具不适用于 PVP，请在背包查看使用方式，未消耗道具。', defeated: false };
    if (action.type === 'item' && context && (action.effect.alchemyOutput || action.effect.skillReset)) {
        const [inventory] = await connection.execute('SELECT quantity FROM player_inventory WHERE character_id=? AND item_id=? FOR UPDATE', [actor.id, action.id]);
        if (Number(inventory[0]?.quantity ?? 0) <= 0)
            return { text: '背包中已没有该道具。', defeated: false };
        ...
        if (result.consumed) {
            await (await import('./achievement.service.js')).consumeAchievementRewardItem(connection, Number(actor.id), action.code, 1);
            await connection.execute('UPDATE player_inventory SET quantity=quantity-1 WHERE character_id=? AND item_id=? AND quantity>0', [actor.id, action.id]);
            await connection.execute('DELETE FROM player_inventory WHERE character_id=? AND item_id=? AND quantity<=0', [actor.id, action.id]);
        }
        return { text: `【${actor.name}】使用【${action.name}】：${result.message}`, defeated: Number(target.current_hp) <= 0 };
    }
    if (action.type === 'item') {
        const oldHp = Number(actor.current_hp);
        const oldMp = Number(actor.current_mp);
        const hp = Math.min(Number(actor.hp_max), oldHp + Number(action.effect.heal ?? 0) + Math.floor(Number(actor.hp_max) * Math.max(0, Number(action.effect.healPct ?? 0)) / 100));
        const mp = Math.min(Number(actor.mp_max), oldMp + Number(action.effect.restoreMp ?? 0) + Math.floor(Number(actor.mp_max) * Math.max(0, Number(action.effect.restoreMpPct ?? 0)) / 100));
        if (hp === oldHp && mp === oldMp)
            return { text: '当前无需回复，未消耗道具。', defeated: false };
        await (await import('./achievement.service.js')).consumeAchievementRewardItem(connection, Number(actor.id), action.code, 1);
        await connection.execute('UPDATE player_inventory SET quantity=quantity-1 WHERE character_id=? AND item_id=? AND quantity>0', [actor.id, action.id]);
        await connection.execute('DELETE FROM player_inventory WHERE character_id=? AND item_id=? AND quantity<=0', [actor.id, action.id]);
        await connection.execute('UPDATE characters SET current_hp=?,current_mp=? WHERE id=?', [hp, mp, actor.id]);
        actor.current_hp = hp;
        actor.current_mp = mp;
    ...
    if (action.type === 'skill' && !skill)
        return resolveAction(connection, actor, target, { type: 'attack' }, sessionId, targetCooldowns, actorCooldowns, actorDevices, targetDevices, context);
    if (skill) {
        actor.current_mp = Number(actor.current_mp) - skill.manaCost;
        await connection.execute('UPDATE characters SET current_mp=? WHERE id=?', [actor.current_mp, actor.id]);
    }
    ...
    if (skill?.category === 'utility') {
        await recordPvpAttack(connection, actor, target, `技能「${skill.name}」`, 0, 'utility');
        return { text: `【${actor.name}】释放技能「${skill.name}」，但该辅助技能尚未在 PvP 对抗中形成直接伤害。`, defeated: false };
    }
    const sourceUnit = context?.get(Number(actor.id));
    let targetUnit = context?.get(Number(target.id));
    ...
    if (sourceUnit && targetUnit) {
        ...
        if (redirected.key === sourceUnit.key) {
            target = actor;
            targetCooldowns = actorCooldowns;
            targetDevices = actorDevices;
            targetUnit = sourceUnit;
        }
    }
    const magic = skill?.category === 'magic';
    let attack = magic ? Number(actor.magic_attack) : Number(actor.physical_attack);
    let defense = magic ? Number(target.magic_defense) : Number(target.physical_defense);
    const label = skill ? `释放技能「${skill.name}」` : '普通攻击';
    if (sourceUnit && targetUnit) {
        if (context.rule.passive(sourceUnit, 'G01'))
            attack = Math.max(sourceUnit.attack, sourceUnit.magic);
        const swap = skill && await context.rule.consume(sourceUnit, magic ? 'swap_magic' : 'swap_physical');
        if (swap)
            attack = magic ? Math.min(sourceUnit.attack, sourceUnit.magic) : Math.max(sourceUnit.attack, sourceUnit.magic);
    ...
    if (!strike.hit) {
        await recordPvpAttack(connection, actor, target, label, 0, 'miss');
        return { text: `【${actor.name}】${label}，但【${target.name}】闪避了攻击。`, defeated: false };
    }
    const exposed = Number(targetCooldowns?.device_exposed ?? 0) > 0 ? .15 : 0;
    const barrier = Number(targetCooldowns?.device_barrier ?? 0) > 0 ? .15 : 0;
    const phase = Number(targetCooldowns?.device_phase_decoy ?? 0) > 0 ? .8 : 0;
    if (phase)
        delete targetCooldowns.device_phase_decoy;
    const [elementRows] = skill && skill.id > 0 ? await connection.execute('SELECT element,range_type,target_scope FROM skill_definitions WHERE id=?', [skill.id]) : [[]];
    const [weaponRows] = !skill ? await connection.execute(`SELECT COALESCE(ii.effect_json,i.effect_json) AS effect_json
    ...
    WHERE pe.character_id=? AND pe.slot='weapon' LIMIT 1`, [actor.id]) : [[]];
    ...
    if (sourceUnit && targetUnit) {
        damage = await context.rule.incoming(sourceUnit, targetUnit, damage, element, Boolean(magic), Boolean(skill), true, true);
        const absorbed = await context.rule.take(targetUnit, damage);
        await context.rule.afterHit(sourceUnit, targetUnit, damage - absorbed, element, Boolean(skill), absorbed, Boolean(actorCooldowns?.__extraTurn), Boolean(magic));
    }
    const hp = targetUnit ? targetUnit.hp : Math.max(0, Number(target.current_hp) - damage);
    const defeated = hp <= 0;
    const critText = strike.crit ? '暴击' : '';
    if (context && (defeated || Number(actor.current_hp) <= 0)) {
        target.current_hp = hp;
        await recordPvpAttack(connection, actor, target, label, damage, 'defeat');
        return { text: `【${actor.name}】${label}，造成 ${damage} 点伤害。`, defeated: true };
    }
    if (!defeated && targetDevices.has('inverse_buffer') && Number(targetCooldowns?.device_inverse_triggered ?? 0) <= 0 && hp * 100 <= Number(target.hp_max) * 30) {
        targetCooldowns.device_inverse_triggered = 1;
        targetCooldowns.device_barrier = Math.max(Number(targetCooldowns.device_barrier ?? 0), 2);
    }
    if (!defeated) {
        await connection.execute('UPDATE characters SET current_hp=? WHERE id=?', [hp, target.id]);
        target.current_hp = hp;
        await recordPvpAttack(connection, actor, target, label, damage, 'hit');
        return { text: `【${actor.name}】${label}，对【${target.name}】造成 ${damage} 点${critText}${magic ? '魔法' : '物理'}伤害（HP ${hp}）。`, defeated: false };
    }
...
const cityPvp = async (qqUserId, targetGameId, confirmed = false) => withTransaction(async (connection) => {
    const attacker = await characterFor(connection, qqUserId);
    const town = await townRegion(connection);
    if (Number(attacker.current_region_id) !== Number(town.id))
        throw new Error('只有在百纳镇内才能发起城镇 PvP。');
    if (await isInHome(connection, Number(attacker.id)))
        throw new Error('你正在自己的家园中，无法主动发起 PvP。');
    if (attacker.activity_status === 'detained')
        throw new Error(detentionMessage(attacker.detained_until));
    const [targets] = await connection.execute('SELECT * FROM characters WHERE game_id=? AND npc_code IS NULL FOR UPDATE', [targetGameId]);
    const target = targets[0];
    if (!target || Number(target.id) === Number(attacker.id) || Number(target.current_region_id) !== Number(town.id) || Number(target.pos_z) !== Number(attacker.pos_z) || Math.abs(Number(target.pos_x) - Number(attacker.pos_x)) + Math.abs(Number(target.pos_y) - Number(attacker.pos_y)) > 1)
        throw new Error('目标不在你相邻的城镇格子中。');
    await assertNoNegotiation(connection, Number(attacker.id));
    await assertNoNegotiation(connection, Number(target.id));
    await assertPvpDefeatUnprotected(connection, Number(target.id));
    if (target.activity_status === 'detained')
        throw new Error('目标已被守卫关押。');
    const attackerWarrant = await wanted(connection, attacker.id, Number(town.id));
    const targetWarrant = await wanted(connection, target.id, Number(town.id));
    if (await isInHome(connection, Number(target.id)) && !targetWarrant)
        throw new Error('目标正在自己的家园中，无法攻击或打劫。');
    ...
    if (unlawfulAttack && confirmed) {
        const [confirmations] = await connection.execute('SELECT 1 FROM player_pvp_attack_confirmations WHERE attacker_character_id=? AND target_character_id=? AND expires_at>NOW() FOR UPDATE', [attacker.id, target.id]);
        if (!confirmations[0])
            throw new Error('攻击确认已失效，请重新选择攻击目标。');
        await connection.execute('DELETE FROM player_pvp_attack_confirmations WHERE attacker_character_id=?', [attacker.id]);
    }
    if (unlawfulAttack) {
        await connection.execute('INSERT INTO player_warrants (wanted_character_id,city_region_id,status) VALUES (?,?,\'active\')', [attacker.id, town.id]);
        await recordWarrantSighting(connection, Number(attacker.id), Number(town.id), Number(attacker.pos_x), Number(attacker.pos_y));
    }
    if (!targetWarrant)
        await recordWarrantVictim(connection, await wanted(connection, Number(attacker.id), Number(town.id)), Number(target.id));
...
const fieldPvp = async (qqUserId, targetGameId) => withTransaction(async (connection) => {
    const attacker = await characterFor(connection, qqUserId);
    if (await isInHome(connection, Number(attacker.id)))
        throw new Error('你正在自己的家园中，无法主动发起 PvP。');
    if (attacker.activity_status === 'detained')
    ...
    if (!target || Number(target.id) === Number(attacker.id) || Number(target.current_region_id) !== Number(attacker.current_region_id) || Number(target.pos_z) !== Number(attacker.pos_z) || distance > perceptionRange(attacker))
        throw new Error('目标已经离开你的感知范围。');
    await assertNoNegotiation(connection, Number(attacker.id));
    await assertNoNegotiation(connection, Number(target.id));
    await assertPvpDefeatUnprotected(connection, Number(target.id));
    if (target.activity_status === 'detained')
        throw new Error('目标已被守卫关押。');
    if (await isInHome(connection, Number(target.id)))
        throw new Error('目标正在自己的家园中，无法攻击或打劫。');
    await refreshPvpPanel(connection, [attacker, target]);
    await resetCardMovementCharge(connection, [Number(attacker.id), Number(target.id)]);
...
const activePvpBattle = async (connection, characterId, lock = false) => {
    const [rows] = await connection.execute(`SELECT * FROM player_pvp_battle_sessions
    WHERE state='active' AND (attacker_character_id=? OR defender_character_id=?) LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [characterId, characterId]);
    return rows[0];
};
const battleActionFromSlot = async (connection, character, type, slot, skillId) => {
    if (type === 'attack')
        return { type: 'attack' };
    if (type === 'escape')
        return null;
    if (type === 'auto')
        return actionFor(connection, character);
    if (type === 'skill') {
        const [rows] = await connection.execute(skillId
        ...
        if (!rows[0])
            throw new Error(skillId ? '二转技能尚未学习。' : `技能${'①②③④'.charAt(Math.max(0, (slot ?? 1) - 1)) || slot}未配置。`);
        const skill = rows[0];
    ...
    const [rows] = await connection.execute(`SELECT i.id,i.code,i.name,i.effect_json FROM player_quick_items qi
    ...
    if (!rows[0])
        throw new Error(`道具${'①②③④'.charAt(Math.max(0, (slot ?? 1) - 1)) || slot}未配置。`);
    return { type: 'item', id: Number(rows[0].id), code: rows[0].code, name: rows[0].name, effect: record(rows[0].effect_json) };
};
const deviceActionFromSlot = async (connection, sessionId, character, cooldowns, slot, skillCode, targetKind) => {
    ...
    if (!device)
        throw new Error(`异械${'①②③④'.charAt(slot - 1) || slot}未配置或尚未生效。`);
    ...
    if (Number(cooldowns[cooldownKey] ?? 0) > 0)
        throw new Error(`【${skill.name}】冷却中，还需${cooldowns[cooldownKey]}回合。`);
    if (device.currentEnergy < skill.energyCost)
        return { type: 'device_charge', instanceId: device.instanceId, deviceName: device.deviceName };
...
const tickCooldowns = (value) => Object.fromEntries(Object.entries(record(value)).map(([code, turns]) => {
    if (code.startsWith('__'))
        return [code, turns];
    if (code === 'device_rocket_boost' || code === 'device_inverse_triggered')
        return [code, Number(turns)];
    return [code, Math.max(0, Number(turns) - 1)];
}));
const readyBattleAction = async (_connection, character, action, cooldowns, automatic) => {
    ...
    if (automatic)
        return { type: 'attack' };
    if (Number(character.current_mp) < action.manaCost)
        throw new Error('魔力不足，无法释放该技能。');
...
const sessionView = async (connection, battle, character) => {
    const attackerId = Number(battle.attacker_character_id);
    const ownAttacker = Number(character.id) === attackerId;
    const ownHp = ownAttacker ? Number(battle.attacker_hp) : Number(battle.defender_hp);
    const ownMp = ownAttacker ? Number(battle.attacker_mp) : Number(battle.defender_mp);
    const enemyId = ownAttacker ? Number(battle.defender_character_id) : attackerId;
    const [others] = await connection.execute('SELECT * FROM characters WHERE id=? LIMIT 1', [enemyId]);
    const enemy = others[0];
    const enemyHp = ownAttacker ? Number(battle.defender_hp) : Number(battle.attacker_hp);
    const enemyMp = ownAttacker ? Number(battle.defender_mp) : Number(battle.attacker_mp);
    const enemyState = readRuleState(record(ownAttacker ? battle.defender_cooldowns : battle.attacker_cooldowns).__rules);
    const hidden = enemyState.statuses.some(e => e.code === 'nightmare' && e.until >= Number(battle.turn_no));
    const cooldowns = ownAttacker ? record(battle.attacker_cooldowns) : record(battle.defender_cooldowns);
    const [skills] = await connection.execute('SELECT ps.quick_slot,s.code FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id WHERE ps.character_id=? AND ps.quick_slot IS NOT NULL', [character.id]);
    const [advancedSkillRows] = await connection.execute(`SELECT ps.skill_id AS id,s.code,s.name
    FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id
    WHERE ps.character_id=? AND s.category IN ('physical','magic','utility')
    ORDER BY ps.learned_at,s.id`, [character.id]);
    const [items] = await connection.execute('SELECT qi.quick_slot FROM player_quick_items qi JOIN player_inventory pi ON pi.character_id=qi.character_id AND pi.item_id=qi.item_id AND pi.quantity>0 WHERE qi.character_id=?', [character.id]);
...
const refreshPvpPanel = async (connection, fighters) => {
    for (const fighter of [...fighters].sort((a, b) => Number(a.id) - Number(b.id))) {
        ...
        const [rows] = await connection.execute('SELECT * FROM characters WHERE id=?', [fighter.id]);
        if (rows[0])
            Object.assign(fighter, rows[0]);
        await neutralTalentSnapshot(connection, fighter, true);
    }
};
const startPvpBattle = async (qqUserId, targetGameId, confirmed = false) => withTransaction(async (connection) => {
    const attacker = await characterFor(connection, qqUserId);
    const [regions] = await connection.execute('SELECT code FROM map_regions WHERE id=? LIMIT 1', [attacker.current_region_id]);
    ...
    if (!defender || Number(defender.id) === Number(attacker.id) || Number(defender.current_region_id) !== Number(attacker.current_region_id) || Number(defender.pos_z) !== Number(attacker.pos_z) || distance > range)
        throw new Error('目标已经离开你的感知范围。');
    await assertNoNegotiation(connection, Number(attacker.id));
    await assertNoNegotiation(connection, Number(defender.id));
    await assertPvpDefeatUnprotected(connection, Number(defender.id));
    if (await isFriendRelation(connection, Number(attacker.id), Number(defender.id)))
        throw new Error('游戏内好友之间无法互相攻击。');
    if (await isInHome(connection, Number(attacker.id)))
        throw new Error('你正在自己的家园中，无法主动发起 PvP。');
    if (attacker.activity_status === 'detained')
        throw new Error(detentionMessage(attacker.detained_until));
    if (defender.activity_status === 'detained')
        throw new Error('目标已被守卫关押。');
    if (regionCode === 'baina_town') {
        const attackerWarrant = await wanted(connection, Number(attacker.id), Number(attacker.current_region_id));
        const defenderWarrant = await wanted(connection, Number(defender.id), Number(attacker.current_region_id));
        if (await isInHome(connection, Number(defender.id)) && !defenderWarrant)
            throw new Error('目标正在自己的家园中，无法攻击或打劫。');
        const unlawfulAttack = !attackerWarrant && !defenderWarrant;
        if (unlawfulAttack && !confirmed) {
            await connection.execute('INSERT INTO player_pvp_attack_confirmations (attacker_character_id,target_character_id,expires_at) VALUES (?,?,DATE_ADD(NOW(),INTERVAL 1 MINUTE)) ON DUPLICATE KEY UPDATE target_character_id=VALUES(target_character_id),expires_at=VALUES(expires_at)', [attacker.id, defender.id]);
            return { needsConfirmation: true, target: defender.name };
        }
        if (unlawfulAttack) {
            const [confirmedRows] = await connection.execute('SELECT 1 FROM player_pvp_attack_confirmations WHERE attacker_character_id=? AND target_character_id=? AND expires_at>NOW() FOR UPDATE', [attacker.id, defender.id]);
            if (!confirmedRows[0])
                throw new Error('攻击确认已失效，请重新选择攻击目标。');
            await connection.execute('DELETE FROM player_pvp_attack_confirmations WHERE attacker_character_id=?', [attacker.id]);
        ...
        if (!defenderWarrant)
            await recordWarrantVictim(connection, await wanted(connection, Number(attacker.id), Number(attacker.current_region_id)), Number(defender.id));
    }
    else if (await isInHome(connection, Number(defender.id)))
        throw new Error('目标正在自己的家园中，无法攻击或打劫。');
    const [occupied] = await connection.execute(`SELECT 1 FROM player_pvp_battle_sessions WHERE state='active' AND (attacker_character_id IN (?,?) OR defender_character_id IN (?,?)) LIMIT 1 FOR UPDATE`, [attacker.id, defender.id, attacker.id, defender.id]);
    if (occupied[0])
        throw new Error('其中一方正在进行玩家对战。');
    ...
    const id = randomUUID();
    await connection.execute('INSERT INTO player_pvp_battle_sessions (id,attacker_character_id,defender_character_id,attacker_hp,attacker_mp,defender_hp,defender_mp,attacker_cooldowns,defender_cooldowns) VALUES (?,?,?,?,?,?,?,JSON_OBJECT(),JSON_OBJECT())', [id, attacker.id, defender.id, attacker.current_hp, attacker.current_mp, defender.current_hp, defender.current_mp]);
    await initializeCombatDeviceEnergy(connection, id, Number(attacker.id), 'pvp');
    await initializeCombatDeviceEnergy(connection, id, Number(defender.id), 'pvp');
...
const startAmbushPvpBattle = async (attackerCharacterId, defenderCharacterId, spawnId, delivery) => withTransaction(async (connection) => {
    const [fighters] = await connection.execute('SELECT * FROM characters WHERE id IN (?,?) AND npc_code IS NULL FOR UPDATE', [attackerCharacterId, defenderCharacterId]);
    ...
    if (!attacker || !defender || Number(attacker.id) === Number(defender.id))
        throw new Error('伏击目标已经离开战场。');
    await assertNoNegotiation(connection, Number(attacker.id));
    await assertNoNegotiation(connection, Number(defender.id));
    await assertPvpDefeatUnprotected(connection, Number(defender.id));
    if (attacker.activity_status !== 'active' || defender.activity_status !== 'active')
        throw new Error('伏击条件已失效，战场中的一方无法继续战斗。');
    const [occupied] = await connection.execute(`SELECT 1 FROM player_pvp_battle_sessions
    ...
    if (occupied[0])
        throw new Error('伏击目标已进入另一场玩家对战。');
    const id = randomUUID();
    await refreshPvpPanel(connection, [attacker, defender]);
    await resetCardMovementCharge(connection, [Number(attacker.id), Number(defender.id)]);
...
const pvpBattleStatus = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId);
    const battle = await activePvpBattle(connection, Number(character.id), true);
    if (!battle)
        throw new Error('当前不在玩家对战中。');
    return sessionView(connection, battle, character);
});
const selectPvpBattleOption = async (qqUserId, option) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId);
    const battle = await activePvpBattle(connection, Number(character.id), true);
    if (!battle)
        throw new Error('当前不在玩家对战中。');
    ...
    if (option.element) {
        if (!['风', '雷', '火'].includes(option.element))
    ...
    if (option.targetId !== undefined) {
        ...
        if (option.targetId !== expected)
            throw new Error('该目标不在本场对战中。');
        if (option.side === 'member')
            cooldowns.__selectedAlly = expected;
        else
            delete cooldowns.__selectedAlly;
    }
    await connection.execute(`UPDATE player_pvp_battle_sessions SET ${column}=? WHERE id=?`, [JSON.stringify(cooldowns), battle.id]);
    battle[column] = cooldowns;
    return sessionView(connection, battle, character);
});
const continuePvpChant = async (qqUserId) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT b.attacker_cooldowns FROM player_pvp_battle_sessions b JOIN characters c ON c.id=b.attacker_character_id JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? AND b.state='active' LIMIT 1`, [qqUserId]);
    if (!readRuleState(record(rows[0]?.attacker_cooldowns).__rules).cast)
        return null;
    return pvpCombatAction(qqUserId, 'auto', undefined, undefined, undefined, true);
};
const pvpCombatAction = async (qqUserId, type, slot, deviceSkillCode, targetKind, automaticChant = false, skillId, hiddenTicket) => withTransaction(async (connection) => {
    const requester = await characterFor(connection, qqUserId);
    const battle = await activePvpBattle(connection, Number(requester.id), true);
    if (!battle)
        throw new Error('当前不在玩家对战中。');
    if (Number(requester.id) !== Number(battle.attacker_character_id))
        throw new Error('对方正在发起攻击，你会按 PVP 自动战斗配置进行反击。');
    if (type === 'escape') {
        await connection.execute("UPDATE player_pvp_battle_sessions SET state='escaped' WHERE id=?", [battle.id]);
        await finishPvpBattleLog(connection, battle.id, 'escaped', null, null, Number(requester.id));
        return { ended: true, log: `战斗<${battle.turn_no}>回合\n➤【${requester.name}】撤离了战斗。`, settlement: '你脱离了玩家对战。', requesterId: Number(requester.id), winnerId: null, winnerName: null };
    }
    const [fighters] = await connection.execute('SELECT * FROM characters WHERE id IN (?,?) ORDER BY id FOR UPDATE', [battle.attacker_character_id, battle.defender_character_id]);
    const attacker = fighters.find(row => Number(row.id) === Number(battle.attacker_character_id));
    const defender = fighters.find(row => Number(row.id) === Number(battle.defender_character_id));
    if (!attacker || !defender)
        throw new Error('对战对象已失效。');
    attacker.current_hp = Number(battle.attacker_hp);
    attacker.current_mp = Number(battle.attacker_mp);
    defender.current_hp = Number(battle.defender_hp);
    defender.current_mp = Number(battle.defender_mp);
    const attackerCooldowns = record(battle.attacker_cooldowns);
    ...
    if (ownCasting && !automaticChant && type !== 'auto')
        throw new Error('你正在吟唱，技能将自动释放。');
    if (!bonusPhase && attackerDevices.has('rocket_propeller'))
        attackerCooldowns.device_rocket_boost = Math.min(100, Number(attackerCooldowns.device_rocket_boost ?? 0) + 10);
    if (!bonusPhase && defenderDevices.has('rocket_propeller'))
        defenderCooldowns.device_rocket_boost = Math.min(100, Number(defenderCooldowns.device_rocket_boost ?? 0) + 10);
    const log = [];
    let ended = false;
    let winnerId = null;
    let winnerName = null;
    ...
    let settlement = '';
    let lootText = null;
    const context = await createPvpCombatRules(connection, [attacker, defender], [attackerCooldowns, defenderCooldowns], Number(battle.turn_no), log, unit => { if (!bonusPhase)
        unit.cooldowns.__bonusAction = 1; });
    const frozenAction = (id) => {
        const cast = context.get(id).state.cast;
        const skill = cast && residentSkillByCode(cast.code);
        if (cast?.action)
            return { ...cast.action, manaCost: 0 };
        return skill && cast ? { type: 'skill', id: cast.skillId, code: skill.code, name: skill.name, category: skill.category, requiredWeaponType: null, manaCost: 0, power: skill.power, cooldown: cast.cooldown } : undefined;
    };
    ...
    const effectiveSpeed = (fighter, state) => context.rule.speed(context.get(Number(fighter.id))) * (1 + Number(state.device_rocket_boost ?? 0) / 100) * (1 - (Number(state.device_slow ?? 0) > 0 ? .3 : 0));
    const orderedTurns = [
    ...
    for (const turn of orderedTurns) {
        if (ended || Number(turn.actor.current_hp) <= 0 || Number(turn.target.current_hp) <= 0)
            break;
        const unit = context.get(Number(turn.actor.id));
        const target = context.get(Number(turn.target.id));
        if (turn.extra) {
            delete turn.cooldowns.__bonusAction;
            turn.cooldowns.__extraTurn = 1;
        }
        else
            delete turn.cooldowns.__extraTurn;
        for (const fighter of context.rule.units)
            fighter.castSpecialization = undefined;
        if (!await context.rule.beforeAction(unit))
            continue;
        if (Number(turn.cooldowns.device_stun ?? 0) > 0) {
            log.push(actionLog(`【${turn.actor.name}】被眩晕，无法行动。`));
            continue;
        }
        const casting = unit.state.cast;
        if (queuedChants.has(Number(turn.actor.id)) && !casting) {
            queuedChants.delete(Number(turn.actor.id));
            log.push(`➤【${unit.name}】的吟唱已被打断，本次行动结束。`);
            continue;
        }
        queuedChants.delete(Number(turn.actor.id));
        ...
        if (rawAction?.type === 'skill' && !casting) {
            const [definitions] = await connection.execute('SELECT code,category,tier,power,mana_cost,cooldown_turns,chant_turns FROM skill_definitions WHERE id=?', [rawAction.id]);
            const [levels] = await connection.execute('SELECT specialization,level FROM player_skill_specializations WHERE character_id=? AND skill_id=?', [turn.actor.id, rawAction.id]);
            ...
            if (blocked) {
                if (!turn.automatic)
                    throw new Error('沉默期间不能使用技能，或所选技能为被动。');
                rawAction = { type: 'attack' };
            }
            else
                rawAction = { ...rawAction, manaCost: context.rule.manaCost(unit, Math.ceil(rawAction.manaCost * (unit.state.memory.debtSkill === rawAction.code ? 1.4 : 1)), rawAction.code) };
        }
        ...
        if (rawAction?.type === 'skill' && isHiddenSkill(rawAction.code)) {
            try {
                ...
                if (turn.automatic && !automaticChoice)
                    throw new HiddenBattleError('尚未配置该隐藏技能的自动选择。');
                ...
                if (!turn.automatic || !(error instanceof HiddenBattleError))
                    throw error;
                log.push('　➥隐藏技能条件不足，改用普通攻击。');
                rawAction = { type: 'attack' };
            }
        }
        if (!hiddenExecuted) {
            ...
            if (action.type === 'skill') {
                ...
                if (resident && ['D01', 'C06', 'F04', 'I04'].includes(resident.id)) {
                    if (turn.automatic) {
                        log.push('➤【' + unit.name + '】缺少可支援队友，放弃本次辅助。');
                        continue;
                    }
                    throw new Error('这项技能需要另一名存活友方，不能在单挑中使用。');
                }
                const folio = folioSkillByCode(action.code);
                if (folio && !casting)
                    validateFolioCast(context.rule, unit, ['ally', 'allies', 'self'].includes(folio.scope) ? unit : target, folio);
                ...
                if (!casting) {
                    unit.mp -= paid;
                    await context.rule.paid(unit, paid, { category: action.category, cooldown });
                ...
                if (chant && !casting) {
                    unit.state.cast = { code: action.code, skillId: action.id, paid, target: target.key, cooldown, releaseTurn: Number(battle.turn_no) + chant, action };
                    log.push('【' + unit.name + '】开始了' + action.name + '技能吟唱。。。');
                    continue;
                }
                if (casting && casting.releaseTurn > Number(battle.turn_no)) {
                    log.push('【' + unit.name + '】继续吟唱。。。');
                    continue;
                }
                delete unit.state.cast;
                turn.cooldowns[action.code] = cooldown + 1;
                ...
                if (specializationFacts.length)
                    recordAchievement(connection, Number(turn.actor.id), specializationFacts, `pvp-specialization-use:${battle.id}:${battle.turn_no}:${turn.actor.id}:${turn.extra ? 'extra' : 'base'}:${action.id}`);
                ...
                if (action.type === 'device' && action.skill.effect === 'physical_evade_once')
                    turn.cooldowns.device_physical_evasion = 2;
                ...
                if (action.type === 'device')
                    hiddenNativeDevice(context.rule, unit, action.skill, beforeHiddenDevice);
                if (action.type === 'device')
                    turn.cooldowns[`device_${action.instanceId}_${action.skill.code}`] = action.skill.cooldownTurns + 1;
                log.push(actionLog(result.text));
            }
        }
        if (Number(attacker.current_hp) <= 0 || Number(defender.current_hp) <= 0) {
            const winner = Number(attacker.current_hp) <= 0 ? defender : attacker;
            ...
            restitutionId = result.restitution?.id;
            settlement = `【${winner.name}】获得了胜利。\n${result.text}`;
            lootText = result.lootText ?? null;
        }
        if (!turn.extra && Number(turn.actor.id) === Number(defender.id) && turn.cooldowns.__bonusAction)
            orderedTurns.push({ ...turn, extra: true });
    }
    const awaitingBonus = !ended && Boolean(attackerCooldowns.__bonusAction);
    if (awaitingBonus) {
        attackerCooldowns.__bonusPhase = 1;
        log.push('【' + attacker.name + '】获得额外行动，请选择一次普攻或技能。');
    }
    else {
        delete attackerCooldowns.__bonusPhase;
        if (!ended) {
            await alchemyEndTurn(context.rule);
            await hiddenEndTurn(context.rule);
            await folioEndTurn(context.rule);
        }
        context.rule.end();
    ...
    const nextAttackerCooldowns = awaitingBonus ? attackerCooldowns : tickCooldowns(attackerCooldowns);
    const nextDefenderCooldowns = awaitingBonus ? defenderCooldowns : tickCooldowns(defenderCooldowns);
    ...
    else if (winnerId === Number(attacker.id))
        await connection.execute('UPDATE characters SET current_hp=?,current_mp=? WHERE id=?', [Math.max(1, Number(attacker.current_hp)), Math.max(0, Number(attacker.current_mp)), attacker.id]);
    else if (winnerId === Number(defender.id))
        await connection.execute('UPDATE characters SET current_hp=?,current_mp=? WHERE id=?', [Math.max(1, Number(defender.current_hp)), Math.max(0, Number(defender.current_mp)), defender.id]);
    if (ended)
        await finishPvpBattleLog(connection, battle.id, winnerId === Number(attacker.id) ? 'attacker_win' : 'defender_win', winnerId === Number(attacker.id) ? attacker : defender, lootText);
    const ambushDelivery = battle.ambush_spawn_id ? { scope: battle.ambush_delivery_scope === 'group' ? 'group' : 'c2c', targetId: battle.ambush_delivery_target_id || '', botId: battle.ambush_delivery_bot_id || undefined } : undefined;
...
const restitutionDetail = async (qqUserId, restitutionId) => {
    const pool = await getPool();
    ...
    if (!character)
        throw new Error('请先注册角色。');
    const [rows] = await pool.execute(`SELECT sl.*,o.name AS owner_name,i.name AS item_name FROM pvp_stolen_loot sl
...
const cityWantedAlert = async (qqUserId) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT w.id,c.name,c.game_id,c.pos_x,c.pos_y,c.pos_z,r.name AS region_name FROM characters c
...
const townPassiveWantedAlert = async (qqUserId) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT w.id,c.name,c.game_id,c.pos_x,c.pos_y,c.pos_z,r.name AS region_name
...
const futureCooldown = (value) => value ? new Date(String(value)).getTime() > Date.now() : false;
const reserveWarrantEntryNotice = async (warrantId, groupOpenId) => withTransaction(async (connection) => {
    const [rows] = await connection.execute('SELECT available_at,updated_at FROM player_warrant_notice_cooldowns WHERE notice_type=\'entry\' AND warrant_id=? AND group_openid=? AND recipient_qq_user_id=\'\' FOR UPDATE', [warrantId, groupOpenId]);
    if (rows[0] && new Date(rows[0].updated_at).getTime() + 10 * 60 * 1000 > Date.now())
        return false;
...
const reservePassiveWarrantNotice = async (warrantId, groupOpenId, recipientQqUserId) => withTransaction(async (connection) => {
    const [rows] = await connection.execute(`SELECT notice_type,available_at FROM player_warrant_notice_cooldowns
    ...
    if (rows.some(row => futureCooldown(row.available_at)))
        return false;
...
const playerPvpStatus = async (qqUserId) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT c.detained_until,
...
const personalPvpPanel = async (qqUserId) => {
    const pool = await getPool();
    const [characters] = await pool.execute(`SELECT c.id,c.name,c.game_id,c.detained_until
    FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1`, [qqUserId]);
    const character = characters[0];
    if (!character)
        throw new Error('请先注册角色。');
    const [warrants] = await pool.execute(`SELECT w.id,r.name AS region_name,w.created_at,
    ...
    const [battles] = await pool.execute('SELECT COUNT(*) AS total FROM player_pvp_battle_logs WHERE attacker_character_id=? OR defender_character_id=?', [character.id, character.id]);
...
const personalPvpEnemies = async (qqUserId) => {
    const pool = await getPool();
    const [characters] = await pool.execute('SELECT c.id FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1', [qqUserId]);
    const character = characters[0];
    if (!character)
        throw new Error('请先注册角色。');
    const [rows] = await pool.execute(`SELECT holder.id AS holder_id,holder.name AS holder_name,holder.game_id AS holder_game_id,
...
const warrantsFor = async () => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT w.id,c.name,c.game_id,r.name AS region_name,c.pos_x,c.pos_y,
...
const townWarrantsFor = async (qqUserId, filter = '全部') => {
    const pool = await getPool();
    const [viewerRows] = await pool.execute(`SELECT c.current_region_id AS region_id,r.name AS region_name,r.code AS region_code
    ...
    if (!viewer)
        throw new Error('请先注册角色。');
    if (viewer.region_code !== 'baina_town')
        throw new Error('请先前往城镇，再查看当地的通缉令。');
    const [rows] = await pool.execute(`SELECT w.id,c.name,r.name AS region_name,c.current_region_id,c.pos_x,c.pos_y,c.pos_z,w.last_seen_at,w.last_seen_x,w.last_seen_y,w.pursuit_defeats,
...
const exposedWarrantsFor = async () => (await warrantsFor()).filter(warrant => warrant.exposed);
const addWarrantReward = async (qqUserId, warrantId, itemId, quantity, copper) => withTransaction(async (connection) => {
    const issuer = await characterFor(connection, qqUserId);
    const [warrants] = await connection.execute('SELECT wanted_character_id FROM player_warrants WHERE id=? AND status=\'active\' FOR UPDATE', [warrantId]);
    const warrant = warrants[0];
    if (!warrant)
        throw new Error('该通缉令已失效。');
    const [eligible] = await connection.execute('SELECT 1 FROM pvp_stolen_loot WHERE original_owner_character_id=? AND holder_character_id=? AND returned_at IS NULL LIMIT 1 FOR UPDATE', [issuer.id, warrant.wanted_character_id]);
    if (!eligible[0])
        throw new Error('只有被该通缉者夺走失物的玩家可以追加赏金。');
    if (copper > 0) {
        const [spent] = await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=? AND copper_coins>=?', [copper, issuer.id, copper]);
        if (!Number(spent.affectedRows))
            throw new Error('铜币不足。');
    }
    if (itemId) {
        const [items] = await connection.execute('SELECT pi.quantity,i.name FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.item_id=? AND i.is_tradeable=1 FOR UPDATE', [issuer.id, itemId]);
        if (!items[0] || Number(items[0].quantity) < quantity)
            throw new Error('用于悬赏的物品数量不足或不可交易。');
        await connection.execute('UPDATE player_inventory SET quantity=quantity-? WHERE character_id=? AND item_id=?', [quantity, issuer.id, itemId]);
        await connection.execute('DELETE FROM player_inventory WHERE character_id=? AND item_id=? AND quantity<=0', [issuer.id, itemId]);
    }
    const [reward] = await connection.execute('INSERT INTO player_warrant_rewards (warrant_id,issuer_character_id,reward_item_id,quantity,copper_amount) VALUES (?,?,?,?,?)', [warrantId, issuer.id, itemId, quantity, copper]);
    if (copper > 0 || itemId && quantity > 0)
        await recordCharacterOperation(connection, { characterId: Number(issuer.id), kind: 'pvp.warrant_reward_added', source: { system: 'player_warrant_rewards', id: Number(reward.insertId), step: 'added' }, outcome: '追加', summary: '向通缉令追加赏金', detail: { warrantId, rewardId: Number(reward.insertId), itemId, quantity, copper } });
    return { copper, quantity };
});

export { addWarrantReward, assertPvpDefeatUnprotected, cityPvp, cityWantedAlert, collectCityDebts, completePvpDefeatProtection, continuePvpChant, createPvpBattleLog, exposedWarrantsFor, fieldPvp, finishPvpBattleLog, personalPvpEnemies, personalPvpPanel, playerPvpStatus, pvpBattleHistory, pvpBattleStatus, pvpCombatAction, recordPvpAttack, recordPvpLootSale, recordWarrantSighting, reservePassiveWarrantNotice, reserveWarrantEntryNotice, resolvePvpVictory, restitutionDetail, selectPvpBattleOption, settleCityPursuitDefeat, settlePvpDefeat, startAmbushPvpBattle, startPvpBattle, takePvpDefeatNotice, townPassiveWantedAlert, townWarrantsFor, warrantsFor };
