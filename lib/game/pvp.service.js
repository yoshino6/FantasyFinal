...
import { resolveStrike, directDamageVariance } from './combat-math.js';
...
const activeDeviceCodesFor = async (connection, characterId) => {
    ...
    WHERE ad.character_id=?`, [characterId]);
...
const actionFor = async (connection, character, manual = false) => {
    ...
    if (setting?.enabled) {
        ...
        if (potionId) {
            const [items] = await connection.execute('SELECT i.id,i.name,i.effect_json FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.item_id=? AND pi.quantity>0 AND i.item_type=\'consumable\' LIMIT 1 FOR UPDATE', [character.id, potionId]);
            if (items[0])
                return { type: 'item', id: Number(items[0].id), name: items[0].name, effect: record(items[0].effect_json) };
        }
...
const stealFromLoser = async (connection, winner, loser, extra = false) => {
    const [items] = await connection.execute('SELECT pi.item_id,i.name,pi.quantity,i.trade_price FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.quantity>0 AND i.is_tradeable=1 AND i.trade_price>0 AND i.item_category NOT IN (\'特殊\',\'地图\',\'货币\') FOR UPDATE', [loser.id]);
...
const resolveAction = async (connection, actor, target, action, sessionId, targetCooldowns, actorCooldowns, actorDevices = new Set(), targetDevices = new Set(), context) => {
    ...
    if (sourceUnit && targetUnit) {
        ...
        attack *= 1 + (context.rule.value(sourceUnit, magic ? 'magic' : 'attack') - context.rule.value(sourceUnit, magic ? 'magic_down' : 'attack_down')) / 100;
    ...
    const battleCryBonus = Number(actorCooldowns?.device_battle_cry ?? 0) > 0 ? 20 : 0;
    const accuracy = Number(actor.accuracy) * (1 + (Number(actorCooldowns?.device_precision_aim ?? 0) > 0 ? 100 : 0) / 100) * (1 + (isDeviceDamage && actorDevices.has('precision_scope') ? .1 : 0));
    ...
    const strike = resolveStrike(attack * (skill ? skill.power / 100 : 1) * (1 + (deviceDamageBonus + battleCryBonus) / 100), defense, accuracy, evasion, critRate, Number(target.crit_resist_bp), Number(actor.crit_damage_bp), Number(target.crit_damage_reduction_bp), setup.forceHit, false, 0, setup.hitBonus * 100, setup.hitFactor);
    if (!strike.hit) {
    ...
    let damage = directDamageVariance(Math.max(1, Math.floor(strike.damage * (skill?.specialized?.damageFactor ?? 1) * (1 + exposed) * (1 - barrier) * (1 - phase) * setup.powerFactor)));
    if (sourceUnit && targetUnit) {
        damage = await context.rule.incoming(sourceUnit, targetUnit, damage, '无', Boolean(magic), Boolean(skill), true, true);
        const absorbed = await context.rule.take(targetUnit, damage);
        await context.rule.afterHit(sourceUnit, targetUnit, damage - absorbed, '无', Boolean(skill), absorbed, Boolean(actorCooldowns?.__extraTurn));
    }
...
const battleActionFromSlot = async (connection, character, type, slot) => {
    if (type === 'attack')
        return { type: 'attack' };
    if (type === 'escape')
        return null;
    ...
    if (type === 'skill') {
        const [rows] = await connection.execute(`SELECT s.id,s.code,s.name,s.category,s.required_weapon_type,s.mana_cost,s.power,s.cooldown_turns
      FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id WHERE ps.character_id=? AND ps.quick_slot=? LIMIT 1`, [character.id, slot ?? 0]);
        if (!rows[0])
            throw new Error(`技能${'①②③④'.charAt(Math.max(0, (slot ?? 1) - 1)) || slot}未配置。`);
        const skill = rows[0];
    ...
    const [rows] = await connection.execute(`SELECT i.id,i.name,i.effect_json FROM player_quick_items qi
...
const pvpCombatAction = async (qqUserId, type, slot, deviceSkillCode, targetKind, automaticChant = false) => withTransaction(async (connection) => {
    ...
    const requestedAction = frozenAction(Number(attacker.id)) ?? (type === 'device'
        ...
        : await battleActionFromSlot(connection, attacker, type, slot));
    ...
    for (const turn of orderedTurns) {
        ...
        if (action.type === 'skill') {
            ...
            const chant = action.chant ?? resident?.chant ?? 0;
            if (chant && !casting) {
