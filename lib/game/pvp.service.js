...
const finishPvpBattleLog = async (connection, id, outcome, winner = null, lootText = null) => {
...
const pvpCombatAction = async (qqUserId, type, slot, deviceSkillCode, targetKind, automaticChant = false, skillId, hiddenTicket) => withTransaction(async (connection) => {
    ...
    if (type === 'escape') {
        ...
        await finishPvpBattleLog(connection, battle.id, 'escaped');
...
const cityWantedAlert = async (qqUserId) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT w.id,c.name,c.game_id,c.pos_x,c.pos_y,r.name AS region_name FROM characters c
...
const townPassiveWantedAlert = async (qqUserId) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT w.id,c.name,c.game_id,c.pos_x,c.pos_y,r.name AS region_name
...
const townWarrantsFor = async (qqUserId, filter = '全部') => {
    ...
    const [rows] = await pool.execute(`SELECT w.id,c.name,r.name AS region_name,c.current_region_id,c.pos_x,c.pos_y,w.last_seen_at,w.last_seen_x,w.last_seen_y,w.pursuit_defeats,
