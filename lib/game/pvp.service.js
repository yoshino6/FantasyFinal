...
import { readRuleState, maskRuleBattleLog, ruleStatusSummary } from './combat-rule-registry.js';
...
const resolveAction = async (connection, actor, target, action, sessionId, targetCooldowns, actorCooldowns, actorDevices = new Set(), targetDevices = new Set(), context) => {
    ...
    const [elementRows] = skill && skill.id > 0 ? await connection.execute('SELECT element FROM skill_definitions WHERE id=?', [skill.id]) : [[]];
    ...
    for (const turn of orderedTurns) {
        ...
        if (rawAction?.type === 'skill' && !casting) {
            ...
            if (blocked) {
                ...
                rawAction = { ...rawAction, manaCost: context.rule.manaCost(unit, Math.ceil(rawAction.manaCost * (unit.state.memory.debtSkill === rawAction.code ? 1.4 : 1))) };
        }
