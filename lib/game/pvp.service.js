...
const pvpBattleHistory = async (qqUserId, page = 1, filter = '全部', keyword = '') => {
    ...
    const [rows] = await pool.execute(`SELECT * FROM player_pvp_battle_logs WHERE ${where}${search} ORDER BY started_at DESC,id DESC LIMIT 5 OFFSET ?`, [...values, (currentPage - 1) * 5]);
