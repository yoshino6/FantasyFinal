...
const heartOffsetAfterChoice = (before, favor, repel, gain, loss, levelAtChoice) => {
    const shares = playerGrowthShares(levelAtChoice);
    return { ...before, [favor]: (units(before[favor]) - units(gain) * shares) / 10, [repel]: (units(before[repel]) + units(loss) * shares) / 10 };
};

export { calculateHeartGrowthChange, heartOffsetAfterChoice };
