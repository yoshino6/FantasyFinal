...
class CombatRules {
    ...
    speed(unit) { return folioStat(unit, 'speed', unit.speed, this.turn) * (1 + (this.value(unit, 'speed') - this.value(unit, 'slow')) / 100); }
    manaCost(unit, base) {
    ...
    async restore(source, target, hp, mp = 0, echo = false, maxHealing = Infinity, activeHealing = false) {
        ...
        target.hp = Math.min(target.hpMax, target.hp + Math.max(0, Math.floor(Math.min(maxHealing, hp * healing))));
            ...
            case 'L04':
                if (foe) {
                    if (foe.state.cast) {
                        ...
                        this.log.push(`　&打断&${combatUnitLabel(foe)}的吟唱中断，返还一半已支付 MP。`);
                    }
