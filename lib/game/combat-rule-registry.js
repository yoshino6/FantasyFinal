...
import { tenacityContest, bossControlChanceMultiplier, opposedChance, strikeCorrections, correctedHitChance, correctedCritChance, correctedCritBonus } from './combat-math.js';
...
class CombatRules {
    ...
    async secondary(source, target, damage, label, element = '无', areaHit = false) {
        ...
        const { damage: dealt } = await this.takeHit(target, incoming, 1, areaHit, source);
    ...
    async takeHit(target, damage, shieldMultiplier = 1, areaHit = false, source) {
        const apply = async (amount) => ({ damage: amount, absorbed: await this.takeUnlinked(target, amount, shieldMultiplier, source) });
        return this.hooks.linkDamage ? this.hooks.linkDamage(target, damage, apply, areaHit) : apply(damage);
    }
    ...
    async takeUnlinked(target, damage, shieldMultiplier = 1, source) {
        ...
        await this.hooks.afterDamage?.(target, hpBefore - target.hp, Boolean(shield && !this.status(target, 'shield')), shield);
    ...
    async strike(source, original, power, element, magic, extra = false, forceHit = false, secondaryScale = 1, options = {}) {
        ...
        const raw = attack * attack / (attack + Math.max(1, defense)) * (critical ? 1 + correctedCritBonus(opposedChance(source.critDamage, target.critReduction), correction) : 1) * (.9 + this.random() * .2) * this.elementFactor(source, target, element) * secondaryScale * setup.powerFactor * (this.hooks.directMultiplier?.(source, target, element, magic, single, options.damageType ?? '打击') ?? 1);
