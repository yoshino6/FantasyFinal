...
const legacyEffectIsDebuff = (effect) => negative.has(effect.code) || ['control', 'damage_over_time'].includes(effect.effect_type);
...
const createCombatRules = async (connection, sessionId, turn, members, targets, statsForTarget, effects, log, weather, absorb, onExtra, openingPve = true) => {
    ...
    const mutationCodes = new Map();
    for (const row of mutationRows) {
        const id = Number(row.character_id);
        const codes = mutationCodes.get(id) ?? [];
        codes.push(String(row.mutation_code));
        mutationCodes.set(id, codes);
    }
    const make = (row, kind) => {
...
export { createCombatRules, legacyEffectIsDebuff, ruleAppraisalLevels };
