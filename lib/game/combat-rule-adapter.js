...
const createCombatRules = async (connection, sessionId, turn, members, targets, statsForTarget, effects, log, weather, absorb, onExtra) => {
    ...
    const rule = new CombatRules(units, turn, log, {
        ...
        legacyEffects: unit => { const { kind, id } = identity(unit); return effects().filter(effect => effect.target_kind === kind && Number(effect.target_id) === id && !removed.has(Number(effect.id))).map(effect => ({ code: effect.code === 'alchemy_confusion' ? 'confusion' : effect.code, value: Number(effect.value), until: turn + Number(effect.remaining_turns), source: '', debuff: negative.has(effect.code) || ['control', 'damage_over_time'].includes(effect.effect_type), stacks: Number(effect.stacks), legacyId: Number(effect.id) })); },
    ...
    rule.start();
    const takeDamage = async (kind, id, incoming) => {
        const unit = get(kind, id);
        const ownBefore = rule.value(unit, 'shield');
        const absorbed = await rule.take(unit, incoming);
