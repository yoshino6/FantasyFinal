...
import { achievementSecondaryLevel, achievementActivity } from './achievement-hooks.js';
...
import { automatonMemoryKinds, automatonBondStages } from './automaton-events.js';
import { automatonSkills } from './automaton-skill-catalog.js';
import { withTransaction } from '../database/pool.js';
import { craftJson, craftCharacterId, createCraftRequest, craftRequestFor, recordAlchemyJournal, completeCraftRequest } from './alchemy-journal.service.js';
import { grantInventory, productionBinding, consumeInventory } from './inventory-binding.js';
import { migrateAutomatonGrowth, createAutomaton, cultivateAutomaton, feedDefinition, respecAutomaton } from './automaton.js';
...
const assertAutomatonSafe = async (connection, characterId) => {
    ...
    if (rows.length)
        throw new Error('战斗中不能认主、培养、重调、维修或更换机巧配置。');
...
const automatonFor = async (connection, characterId, id, claimed = true) => {
    ...
    if (!row.combat_id && row.recover_at && new Date(row.recover_at).getTime() <= Date.now()) {
        if (!state.hp)
            await recordAutomatonEvent(connection, id, characterId, `recovery:${new Date(row.recover_at).toISOString()}`, 'recovery', { name: state.name, source: '休整后自动恢复' }, new Date(row.recover_at));
...
const recordAutomatonEvent = async (connection, id, characterId, key, kind, data, occurredAt = null) => {
    ...
    if (automatonMemoryKinds[kind])
        recordAchievement(connection, characterId, ['ACH_J15'], 'automaton-memory:' + id + ':' + key);
};
const recordAutomatonFirstEvent = async (connection, id, characterId, kind, data, identity = kind) => {
    ...
    if (automatonMemoryKinds[kind])
        recordAchievement(connection, characterId, ['ACH_J15'], 'automaton-memory:' + id + ':' + identity);
};
const grantOpeningAutomaton = async (connection, characterId) => {
    ...
    if (existing[0])
        return { id: Number(existing[0].id), state: craftJson(existing[0].state_json) };
    const [count] = await connection.execute('SELECT COUNT(*) total FROM player_automatons WHERE owner_id=? FOR UPDATE', [characterId]);
    if (Number(count[0]?.total ?? 0) >= 3)
        throw new Error('机巧认主名额已满，请先处理现有机巧后再继续剧情。');
    const state = createAutomaton(randomBytes(32).toString('hex'));
    state.name = '无主机偶';
    state.intimacy = 10;
    state.origin = { kind: 'opening', code: 'I02' };
    await connection.execute('UPDATE player_automatons SET following=0 WHERE owner_id=?', [characterId]);
    const [insert] = await connection.execute("INSERT INTO player_automatons(holder_id,creator_id,owner_id,bound_kind,following,state_json) VALUES(?,?,?,'personal',1,?)", [characterId, characterId, characterId, JSON.stringify(state)]);
...
const automatonEventPage = (user, id, page = 1) => withTransaction(async (connection) => {
    ...
    return { page, pages, state, events: events.map(e => ({ kind: String(e.kind), time: new Date(e.created_at), data: craftJson(e.data_json) })) };
});
const automatonIntimacy = async (connection, characterId, state, kind, automatonId) => {
    ...
    if (!Number(rows[0][kind])) {
        ...
        state.intimacy = Math.min(1000, state.intimacy + (kind === 'victory' ? 4 : 3));
        await connection.execute(`UPDATE automaton_daily SET ${kind}=1 WHERE character_id=? AND day_key=?`, [characterId, day]);
        if (automatonId)
            for (const stage of automatonBondStages.filter(s => s.value > before && s.value <= state.intimacy))
                await recordAutomatonFirstEvent(connection, automatonId, characterId, 'bond_milestone', { name: state.name, intimacy: stage.value, stage: stage.name }, 'bond:' + stage.value);
    }
};
const automatonList = (user) => withTransaction(async (connection) => {
    ...
    for (const row of rows) {
        ...
        if (row.combat_id) {
            ...
            if (snapshots[0])
                battle = craftJson(snapshots[0].state_json);
        }
...
const automatonRecipes = [
    ...
    ...automatonFeeds.map(f => ({ code: `automaton_feed_${f.code}`, name: f.name, profession: 'alchemist', chance: .9, ingredients: [...Object.entries(f.main).map(([code, quantity]) => ({ code, quantity, role: '主材' })), ...Object.entries(f.aux).map(([code, quantity]) => ({ code, quantity, role: '辅材' })), { code: 'magic_unit', quantity: 2, role: '催化剂' }] }))
];
const automatonCraftChance = (recipe, level) => recipe.profession === 'alchemist'
    ? Math.min(1, (recipe.chance * 100 + Math.max(0, secondaryProfessionBonus(level) - secondaryProfessionBonus(4))) / 100)
    : recipe.chance;
const recipeFor = (code) => { const r = automatonRecipes.find(r => r.code === code); if (!r)
    throw new Error('未知造物配方。'); return r; };
const assertProfession = async (connection, character, recipe) => {
    ...
    if (character.secondary_profession_code !== recipe.profession || Number(rows[0]?.level ?? 0) < 4)
        throw new Error(`需要${recipe.profession === 'alchemist' ? '炼金师' : '解构师'}达到 4 级。`);
    if (recipe.profession === 'alchemist')
...
const automatonRecipeCatalog = (user, kind) => withTransaction(async (connection) => {
    ...
    if (character.secondary_profession_code !== profession)
        throw new Error(kind === '构造' ? '请从解构师构造面板进入。' : '炼金配方仅对当前炼金师开放。');
    ...
    for (const recipe of automatonRecipes.filter(r => r.profession === profession && (kind === '育成' ? r.code.startsWith('automaton_feed_') : !r.code.startsWith('automaton_feed_')))) {
        ...
        for (const part of recipe.ingredients) {
            ...
            ingredients.push({ ...part, name: item.name, owned: Number(item.quantity) });
        }
        recipes.push({ ...recipe, chance: automatonCraftChance(recipe, level), ingredients });
    }
    return recipes;
});
const previewAutomatonCraft = (user, code, batches = 1) => withTransaction(async (connection) => {
    ...
    if (!Number.isInteger(batches) || batches < 1 || batches > (code === 'automaton' ? 1 : 100))
        throw new Error('点灵每次一具，其他配方每次 1～100 批。');
    const ingredients = [], owned = {};
    for (const part of recipe.ingredients) {
        const item = await itemFor(connection, character.id, part.code);
        owned[part.code] = Number(item.quantity);
        ingredients.push({ ...part, id: Number(item.id), name: item.name });
        if (Number(item.quantity) < part.quantity * batches + (part.code === 'sky_dust' && Number(character.realm_stage) < 2 ? 1 : 0))
            throw new Error(`【${item.name}】不足。`);
    }
...
const confirmAutomatonCraft = (user, token) => withTransaction(async (connection) => {
    ...
    if (snapshot.version !== 'automaton-v4' || snapshot.level !== recipeLevel || snapshot.chance !== chance)
        throw new Error('职业等级或配方条件已变化，请重新放入配方确认。');
    const batches = [], lines = [];
    for (let index = 0; index < snapshot.batches; index++) {
        for (const part of recipe.ingredients) {
            const item = await itemFor(connection, character.id, part.code);
            if (Number(item.quantity) < await talentMaterialPayment(connection, character.id, Number(item.id), part.quantity, 'craft') + (part.code === 'sky_dust' && Number(character.realm_stage) < 2 ? 1 : 0))
                throw new Error(`【${item.name}】不足，本次批量未扣料。`);
        }
        const success = Math.random() < chance, used = { unbound: 0, trade: 0, personal: 0 }, consumed = [];
        for (const ingredient of snapshot.ingredients) {
            ...
            consumed.push({ ...ingredient, quantity: payment.paid });
        }
        const outputs = [];
        if (success) {
            if (snapshot.code === 'automaton') {
                const state = createAutomaton(randomBytes(32).toString('hex'));
                const [insert] = await connection.execute('INSERT INTO player_automatons (holder_id,creator_id,bound_kind,state_json) VALUES (?,?,?,?)', [character.id, character.id, used.personal ? 'personal' : 'none', JSON.stringify(state)]);
                if (!currentSecondaryShop())
                    recordAchievement(connection, Number(character.id), ['ACH_J04'], 'automaton-birth:' + insert.insertId);
                ...
                outputs.push({ id: Number(item.id), code: item.code, name: item.name, quantity: 1, role: 'output' });
            }
        }
    ...
    if (successes > 0 && !currentSecondaryShop())
...
const cultivationInput = (args) => {
    ...
    if (bottles.some(b => !Number.isInteger(b.count) || b.count < 1) || bottles.reduce((s, b) => s + b.count, 0) > 100)
        throw new Error('每次投入合计 1～100 瓶，混合格式：blade:2,shell:1。');
...
const previewAutomatonMutation = (user, id, action, args = []) => withTransaction(async (connection) => {
    ...
    if (action === '重调') {
        ...
        if (!Number.isInteger(from) || !Number.isInteger(to) || from < 2 || to > state.level || to < from)
            throw new Error('重调范围须为已完成的 2～当前等级。');
        ...
        respecAutomaton(state, levels, [{ code: feed.code, xp }]);
    ...
    if (action === '重调') {
        ...
        await assertFeedStock(connection, character.id, [{ code: feedDefinition(args[2]).code, count: Math.ceil((xp + Math.max(100, Math.ceil(xp * .1))) / 100) }]);
    }
    if (action === '装配')
        validateAutomatonLoadout(state.level, state.learned, args);
    if (action === '命名')
        cleanText(args.join(' '), 12);
    if (action === '认主') {
        const [counts] = await connection.execute('SELECT COUNT(*) total FROM player_automatons WHERE owner_id=?', [character.id]);
        if (row.owner_id || Number(counts[0].total) >= 3)
            throw new Error('已经认主或已达到三具上限。');
        description += '\n认主后永久绑定，不能再次交易。';
    }
    if (action === '维修') {
        if (state.hp >= Math.floor(state.stats[0]) && state.mp >= Math.floor(state.stats[1]))
            throw new Error('状态已满，无需维修。');
        ...
        if (Number(kit.quantity) < paid)
            throw new Error('缺少维修包。');
        description += `\n消耗维修包×${paid}，完全恢复生命、魔力并结束停机。`;
    }
    if (action === '休眠归档')
        description += '\n隐藏日常列表并收起，仍占认主名额，可恢复展示。';
    const token = await createCraftRequest(connection, character.id, 'automaton_mutate', { id, revision: Number(row.revision), action, args });
    return { token, description };
});
const cleanText = (text, max) => {
    const size = [...new Intl.Segmenter('zh', { granularity: 'grapheme' }).segment(text)].length;
    if (size < 1 || size > max || /[\p{Cc}\p{Cf}\r\n@<>\[\]`]|https?:|www\.|^\s*[/＃#]|系统[:：]/iu.test(text))
        throw new Error(`内容须为 1～${max} 个可见字，不含链接、提及、换行或指令格式。`);
    return text;
};
const confirmAutomatonMutation = (user, token) => withTransaction(async (connection) => {
    ...
    if (action === '认主') {
        ...
        if (recent.length)
            throw new Error('切换随行后需等待 60 秒。');
        if (action === '随行' && !state.hp)
            throw new Error('暂时停机，等待恢复或使用维修包。');
        if (action === '随行') {
        ...
        if (action === '随行')
            await recordAutomatonFirstEvent(connection, id, character.id, 'first_follow', { name: state.name });
    }
    else if (action === '培养') {
        const { bottles, stop } = cultivationInput(args);
        ...
        if (remainder)
            state.reserve.push({ code: feed.code, xp: remainder });
    }
    else if (action === '维修') {
        if (state.hp >= Math.floor(state.stats[0]) && state.mp >= Math.floor(state.stats[1]))
            throw new Error('状态已满，不消耗维修包。');
        ...
        if (!state.hp)
            await recordAutomatonEvent(connection, id, character.id, `recovery:${token}`, 'recovery', { name: state.name, source: '主人使用维修包修复' });
        state.hp = Math.floor(state.stats[0]);
        state.mp = Math.floor(state.stats[1]);
        ...
        if (state.named && Number(state.renameCount) >= 3)
            throw new Error('每日最多改名三次。');
        const name = cleanText(args.join(' '), 12);
        if (name === state.name)
            throw new Error('名字没有变化。');
        if (state.named)
            state.renameCount = Number(state.renameCount ?? 0) + 1;
        state.named = true;
        state.name = name;
    }
        ...
        if (state.archived)
            await connection.execute('UPDATE player_automatons SET following=0 WHERE id=?', [id]);
    }
    await saveAutomaton(connection, row, state);
    ...
    if (action === '培养')
        for (const level of state.levels.filter(l => l.level > before.level && l.level % 10 === 0))
            await recordAutomatonFirstEvent(connection, id, character.id, 'breakthrough', { name: state.name, level: level.level, skills: level.skills, gain: level.gain }, `breakthrough:${level.level}`);
    if (action === '培养')
        for (const level of state.levels.filter(l => l.level > before.level && l.level % 10 !== 0 && l.skills.length))
            await recordAutomatonFirstEvent(connection, id, character.id, 'skill_learned', { name: state.name, level: level.level, skills: level.skills }, `skill_learned:${level.level}`);
    if (action === '维修')
        recordAchievement(connection, Number(character.id), ['ACH_J12'], 'automaton:' + token);
    if (action === '培养' && state.level > before.level) {
        recordAchievement(connection, Number(character.id), ['ACH_J09']);
        if (state.learned.some(s => !before.learned.includes(s)))
            recordAchievement(connection, Number(character.id), ['ACH_J14']);
    }
