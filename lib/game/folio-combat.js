import { folioBuffs, folioDebuffs } from './folio-effect.config.js';
import { canDispelCombatEffect } from './combat-dispel-policy.js';
import { resolveStrike, tenacityContest } from './combat-math.js';

const resolveFolioStrike = (rules, source, target, magic, ...args) => {
...
const folioCorrection = (source, target, turn) => Math.min(100, folioValue(source, 'hit', turn) + folioValue(target, 'exposed_hit', turn));
const folioDuration = (source, turns) => Math.min(4, Math.max(1, Math.floor(turns * (source.castSpecialization?.effectFactor ?? 1) + 1e-9)));
const folioBenefit = (source, value, cap) => Math.min(cap, value * (source.castSpecialization?.effectFactor ?? 1));
const addFolioStatus = (rules, source, target, code, value, turns, debuff = false, data) => {
...
const cleansePriority = (e) => ['sleep', 'stun', 'fear', 'alchemy_stun', 'hidden_stun', 'hidden_freeze'].includes(e.code) ? 0 : ['silence', 'blind', 'confusion'].includes(e.code) ? 1 : /poison|burn|bleed/.test(e.code) ? 2 : 3;
const folioCleanseCandidates = (rules, target, dotOnly = false) => rules.effects(target).filter(e => e.debuff && canDispelCombatEffect(e.code, 'ordinary', Boolean(e.mechanism)) && (!dotOnly || /^(poison|burn|bleed|bleeding|folio_burn)$/.test(e.code))).sort((a, b) => cleansePriority(a) - cleansePriority(b) || b.until - a.until || a.code.localeCompare(b.code));
const folioTargets = (rules, source, target, skill, keys) => {
    ...
    if (keys)
        return [...new Set(keys)].map(key => pool.find(u => u.key === key)).filter((u) => Boolean(u)).slice(0, skill.targetCount);
    ...
    return [first, ...remaining].filter((u) => Boolean(u)).slice(0, skill.targetCount);
};
const validateFolioCast = (rules, source, target, skill, keys) => {
...
const castFolioSkill = async (rules, source, target, skill, extra = false) => {
    ...
    const effect = async (unit, code, value, cap, turns, debuff = false, data) => {
    ...
    if (skill.category === 'utility') {
        ...
        for (const unit of targets) {
            if (/^s0[1-6]$/.test(id)) {
                ...
                const healing = { s02: .6, s04: .4, s06: .5 }[id];
                if (healing)
            ...
            for (const [code, value, cap] of folioBuffs[id] ?? [])
    ...
    for (let segment = 0; segment < skill.parts.length; segment++)
        await rules.areaDamage(targets, async (unit) => {
            ...
            if (!hit || segment > 0 || unit.hp <= 0)
                return;
            if (folioDebuffs[id] && (!['p20', 'm14', 'm23'].includes(id) || main)) {
                const [code, value, cap, turns] = folioDebuffs[id];
...
const folioEndTurn = async (rules) => {
...
export { addFolioStatus, castFolioSkill, folioBenefit, folioCleanseCandidates, folioCorrection, folioDuration, folioEndTurn, folioStat, folioStatusNames, folioTargets, folioValue, resolveFolioStrike, validateFolioCast };
