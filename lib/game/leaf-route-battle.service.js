...
import { leafRouteTransport } from './leaf-route.service.js';
import { resetCardMovementCharge } from './monster-card-exploration.service.js';

const json = (v) => typeof v === 'string' ? JSON.parse(v) : v ?? {};
const leafEnemyScale = (stage, wave) => stage === 4 ? { hp: .8, attack: .75, defense: 1 } : wave === 0 ? { hp: .55, attack: .75, defense: .6 } : { hp: 2, attack: .85, defense: 1.1 };
const spawnEnemies = async (c, character, stage, wave, level) => {
    const [templates] = await c.execute("SELECT * FROM monster_templates WHERE code='windchime_bird'");
    const [dummy] = await c.execute("SELECT id FROM monster_templates WHERE code='npc_sparring_dummy'");
    if (!templates[0] || !dummy[0])
        throw Error('航务战斗基础数据尚未初始化。');
    const stats = (await import('./adventure.service.js')).monsterCombatStats({ ...templates[0], level, monster_class: 'normal' }), scale = leafEnemyScale(stage, wave), spawns = [];
...
const attach = async (c, session, id, enemies) => {
...
const startLeafRouteBattle = async (c, character, row) => {
    ...
    const resource = (await import('./advanced-resource.config.js')).advancedResourceForProfession(String(professions[0]?.profession_code ?? ''));
    if (resource)
...
const advanceLeafWave = async (c, session) => {
    const [rows] = await c.execute('SELECT * FROM player_leaf_route_battles WHERE session_id=? FOR UPDATE', [session]);
    const row = rows[0];
    if (!row || row.state !== 'active' || Number(row.stage) !== 8 || Number(row.wave) !== 0)
        return false;
    const [characters] = await c.execute('SELECT * FROM characters WHERE id=?', [row.character_id]);
...
const finishLeafRouteBattle = async (c, session, result) => {
    const [rows] = await c.execute('SELECT * FROM player_leaf_route_battles WHERE session_id=? FOR UPDATE', [session]);
    const row = rows[0];
    if (!row)
        return null;
...
export { advanceLeafWave, finishLeafRouteBattle, leafEnemyScale, startLeafRouteBattle };
