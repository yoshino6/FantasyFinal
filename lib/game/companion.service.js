...
const companionAction = async (user, companionId, action, value = '') => withTransaction(async (connection) => {
    ...
    if (action === 'release') {
        ...
        await track('companion.released', `放归${pet.name}`);
    ...
    if (action === 'name') {
        ...
        if (value.trim() !== String(pet.name))
            await track('companion.renamed', `将${pet.name}改名为${value.trim()}`, { newName: value.trim() });
    ...
    if (action === 'feed') {
        ...
        await track('companion.fed', `喂养${pet.name}`, { intimacyGained: intimacy });
    ...
    if (action === 'specialty') {
        ...
        if (value !== String(pet.specialty))
            await track('companion.specialty_changed', `调整${pet.name}的出行专长`, { before: String(pet.specialty), after: value });
        return '已调整出行专长。';
    }
    if (action === 'talk' && pet.template_code === 'golden_rabbit' && Number(pet.intimacy) >= 50) {
        ...
        if (used.affectedRows) {
            ...
            await track('companion.bread_shared', '与黄金兔分享面包');
