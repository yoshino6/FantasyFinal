...
const shopCatalog = async (qqUserId, page = 1, keyword = '') => {
    ...
    const [rows] = await pool.execute(`SELECT i.id,i.codex_id,i.name,i.item_type,i.trade_price,i.rarity,i.item_category,i.description,si.buy_price,si.stock_quantity,COALESCE(pi.quantity,0) AS owned_quantity
...
const buyShopItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    ...
    const [rows] = await connection.execute('SELECT i.id,i.name,i.item_type,i.item_category,i.description,i.trade_price,i.rarity,si.buy_price,si.stock_quantity FROM guild_shop_items si JOIN item_definitions i ON i.id=si.item_id WHERE si.item_id=? AND si.is_active=1 AND si.buy_price>0 FOR UPDATE', [itemId]);
    const item = rows[0];
    if (!item)
    ...
    recordAchievement(connection, Number(character.id), [{ metric: 'ACH_K08', value: totalPrice, life: true }]);
