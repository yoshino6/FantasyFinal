...
import { dawnWeapons, crimsonArmor } from './opening-chest.config.js';

const grantOpeningRouteReward = async (c, id, route, choice) => {
    let kind = choice.rewardKind;
    if (!kind)
        throw new Error('这条路线的奖励配置尚未齐全。');
    if (kind === '照料') {
        const [companions] = await c.execute('SELECT id FROM player_companions WHERE character_id=? LIMIT 1', [id]);
        if (!companions.length)
            kind = '药袋';
    }
    let copper = choice.rewardCopper ?? (kind === '工料' ? 100 : ['药袋', '餐食', '照料'].includes(kind) ? 150 : 180);
    const items = kind === '工料' ? [['home_wood', 3], ['home_stone', 3], ['home_metal', 3]]
        : kind === '药袋' ? [['healing_herb', 3], ['novice_mp_potion_small', 2]]
            : kind === '照料' ? [['opening_companion_feed', 3]] : [];
    if (kind === '餐食') {
        const [meals] = await c.execute("SELECT i.id FROM item_definitions i JOIN guild_restaurant_menu m ON m.item_id=i.id WHERE i.code='mushroom_cream_soup'");
        if (meals.length)
            items.push(['mushroom_cream_soup', 1]);
        else
            copper = 180;
    }
    if (choice.bonusItem)
        items.push([choice.bonusItem, 1]);
    for (const item of choice.rewardItems ?? [])
        items.push([item.code, item.quantity]);
    ...
    for (const [code, quantity] of items) {
        ...
        const [rows] = await c.execute('SELECT name FROM item_definitions WHERE code=?', [code]);
    ...
    if (choice.rewardEquipment) {
        ...
        const equipment = await (await import('./opening-chest.service.js')).grantOpeningEquipment(c, id, code);
...
export { grantOpeningRouteReward };
