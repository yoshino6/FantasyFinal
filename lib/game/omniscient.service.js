import { achievementSecondaryLevel } from './achievement-hooks.js';
import { talentProficiency } from './talent-rewards.js';
import { currentSecondaryShop, shopProgressFor } from './secondary-shop-context.js';
...
const characterFor = async (connection, qqUserId, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.id,c.level,c.name,c.secondary_profession_code,c.current_region_id,c.pos_x,c.pos_y,c.pos_z FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [qqUserId]);
    if (!rows[0])
        throw new Error('请先注册角色。');
    return rows[0];
};
const omniscientQuest = async (qqUserId) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const [rows] = await pool.execute(`SELECT q.status,op.slime_observed,op.wolf_king_observed
    ...
    const row = rows[0];
    const slimeObserved = Boolean(row?.slime_observed);
    const wolfKingObserved = Boolean(row?.wolf_king_observed);
    const completed = row?.status === 'accepted' && slimeObserved && wolfKingObserved;
    if (completed)
        await pool.execute("UPDATE player_side_quests SET status='completed',completed_at=COALESCE(completed_at,NOW()) WHERE character_id=? AND quest_code=?", [character.id, questCode]);
    ...
    return { status, slimeObserved, wolfKingObserved };
};
const acceptOmniscientQuest = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    if (Number(character.level) < 10)
        throw new Error('secondary_profession_level_required');
    if (character.secondary_profession_code && character.secondary_profession_code !== 'omniscient')
        throw new Error('你已经拥有其他副职业，无法再选择全知者。');
...
const recordOmniscientObservation = async (connection, characterId, targetCodes) => {
    const sawSlime = targetCodes.includes('forest_slime');
    const sawWolfKing = targetCodes.includes('shadow_wolf_king');
    if (!sawSlime && !sawWolfKing)
        return;
    ...
    if (!quests[0] || !['accepted', 'completed'].includes(quests[0].status))
        return;
    ...
    const [progress] = await connection.execute('SELECT slime_observed,wolf_king_observed FROM player_omniscient_quest_progress WHERE character_id=? FOR UPDATE', [characterId]);
    if (Number(progress[0]?.slime_observed) && Number(progress[0]?.wolf_king_observed)) {
        const [completed] = await connection.execute("UPDATE player_side_quests SET status='completed',completed_at=NOW() WHERE character_id=? AND quest_code=? AND status='accepted'", [characterId, questCode]);
        if (Number(completed.affectedRows) > 0)
            await recordCharacterOperation(connection, { characterId, kind: 'quest.omniscient_completed', source: { system: 'omniscient_quest', id: characterId, step: 'completed' }, actorRole: 'system', outcome: '完成', summary: '完成全知者观察委托', detail: { questCode, observed: ['forest_slime', 'shadow_wolf_king'] } });
    }
};
const claimOmniscientQuest = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const [quests] = await connection.execute('SELECT status FROM player_side_quests WHERE character_id=? AND quest_code=? FOR UPDATE', [character.id, questCode]);
    if (quests[0]?.status !== 'completed')
        throw new Error('任务尚未完成。');
    const [gifts] = await connection.execute('SELECT id,name FROM item_definitions WHERE code=\'luowen_gift\' LIMIT 1 FOR UPDATE');
    if (!gifts[0])
        throw new Error('洛文的赠礼尚未初始化，请重启机器人后重试。');
...
const omniscientProgress = async (qqUserId) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const shop = await shopProgressFor(pool, Number(character.id), 'omniscient');
    if (shop)
        return { ...shop, rangeBonus: 3, informationBonus: 3, dropBonusPct: shop.bonus };
    if (character.secondary_profession_code !== 'omniscient')
        throw new Error('尚未转职全知者。');
    ...
    const [rows] = await pool.execute('SELECT level,proficiency FROM player_secondary_professions WHERE character_id=? AND profession_code=\'omniscient\'', [character.id]);
...
const awardOmniscientProficiency = async (connection, characterId, targetLevels) => {
    ...
    if (characters[0]?.secondary_profession_code !== 'omniscient')
        return null;
    ...
    const [rows] = await connection.execute('SELECT level,proficiency FROM player_secondary_professions WHERE character_id=? AND profession_code=\'omniscient\' FOR UPDATE', [characterId]);
    ...
    const materialValue = materialValueMultiplierForLevel(averageLevel);
    const gain = await talentProficiency(connection, characterId, Math.floor(materialValue) + (Math.random() < materialValue % 1 ? 1 : 0), { profession: 'omniscient', successfulBase: 0 });
    proficiency += gain;
...
const fallbackTrailProfile = (name) => ({ aura: '难以辨析的强大魔力正在远处盘踞', signs: [`地面留有属于【${name}】的凌乱痕迹，尚未被风完全掩去。`, '附近的草木与碎石都被某种强大力量扰乱了原本的位置。', '残余魔力在此处格外浓重，连呼吸都像被轻轻压住。'], interruption: '原本清晰的魔力残响突然断开，仿佛目标已经离开此地。' });
const pick = (items) => items[Math.floor(Math.random() * items.length)];
const clueVariant = (profile, stage) => {
...
const initialVariant = (profile, bearing) => pick([
...
const nextVariant = (bearing) => pick([
...
const interruptionVariant = (profile) => pick([
...
const trailNodes = (boss) => {
    const centerX = (Number(boss.min_x) + Number(boss.max_x)) / 2;
    const centerY = (Number(boss.min_y) + Number(boss.max_y)) / 2;
    let vx = centerX - Number(boss.pos_x);
    let vy = centerY - Number(boss.pos_y);
    const length = Math.hypot(vx, vy) || 1;
    vx /= length;
    vy /= length;
    const rotate = (degrees) => { const radians = degrees * Math.PI / 180; return { x: vx * Math.cos(radians) - vy * Math.sin(radians), y: vx * Math.sin(radians) + vy * Math.cos(radians) }; };
    const radius = Math.max(12, Math.min(72, Math.min(Number(boss.max_x) - Number(boss.min_x), Number(boss.max_y) - Number(boss.min_y)) * .32));
    const side = Number(boss.id) % 2 ? 1 : -1;
    const clamp = (value, low, high) => Math.max(low + 2, Math.min(high - 2, Math.round(value)));
    const count = 3 + Number(boss.id) % 4;
    const raw = Array.from({ length: count }, (_, index) => {
        const progress = count === 1 ? 1 : index / (count - 1);
        const distance = .95 - progress * .72;
        const angle = 78 - progress * 60;
    ...
    let next = { x: Number(boss.pos_x), y: Number(boss.pos_y) };
    for (let index = raw.length - 1; index >= 0; index -= 1) {
        const candidate = raw[index];
        const distance = Math.abs(candidate.x - next.x) + Math.abs(candidate.y - next.y);
...
const omniscientTraces = async (qqUserId) => {
    const pool = await getPool();
    const preview = await characterFor(pool, qqUserId);
    if (preview.secondary_profession_code !== 'omniscient' && !currentSecondaryShop())
        return null;
    return withTransaction(async (connection) => {
        const character = await characterFor(connection, qqUserId, true);
        await shopProgressFor(connection, Number(character.id), 'omniscient');
        const [materials] = await connection.execute(`SELECT i.name FROM map_resource_pools rp JOIN item_definitions i ON i.id=rp.item_id
    LEFT JOIN blacksmith_refinement_materials rm ON rm.item_id=i.id WHERE rp.region_id=? AND rm.item_id IS NOT NULL ORDER BY rm.max_gain DESC,rp.spawn_density ASC,i.id DESC LIMIT 1`, [character.current_region_id]);
        ...
        const [bossRows] = await connection.execute(`SELECT s.id,t.code,t.name,s.pos_x,s.pos_y,r.min_x,r.max_x,r.min_y,r.max_y,
    ...
    ORDER BY s.spawned_at DESC,s.id DESC LIMIT 1 FOR UPDATE`, [character.current_region_id]);
        ...
        if (!boss) {
            const [previous] = await connection.execute(`SELECT spawn_id,boss_code,boss_name FROM player_omniscient_boss_traces WHERE character_id=? AND region_id=? AND interrupted=0 AND completed=0 ORDER BY updated_at DESC LIMIT 1 FOR UPDATE`, [character.id, character.current_region_id]);
            if (!previous[0])
                return materialText;
        ...
        const [traceRows] = await connection.execute('SELECT stage,interrupted,completed FROM player_omniscient_boss_traces WHERE character_id=? AND spawn_id=? FOR UPDATE', [character.id, boss.id]);
        const profile = trailProfiles[boss.code] ?? fallbackTrailProfile(boss.name);
        const trace = traceRows[0];
        const nodes = trailNodes(boss);
        const previousStage = Math.max(0, Number(trace.stage));
        let stage = previousStage;
        if (Number(trace.completed))
            return [materialText, `【踪迹】${boss.name} 位于（${boss.pos_x}，${boss.pos_y}）`].filter(Boolean).join('\n');
        if (Number(trace.interrupted))
            return [materialText, `【线索·中断】${interruptionVariant(profile)}`].filter(Boolean).join('\n');
        if (Number(boss.occupied)) {
            if (!Number(trace.interrupted))
                await connection.execute('UPDATE player_omniscient_boss_traces SET interrupted=1 WHERE character_id=? AND spawn_id=?', [character.id, boss.id]);
            return [materialText, `【线索·中断】${interruptionVariant(profile)}`].filter(Boolean).join('\n');
        }
        while (stage < nodes.length && Number(character.pos_x) === nodes[stage].x && Number(character.pos_y) === nodes[stage].y)
            stage += 1;
        const completedLines = Array.from({ length: Math.max(0, stage - previousStage) }, (_, index) => `【线索·${previousStage + index + 1}】${clueVariant(profile, previousStage + index)}`);
        ...
        if (stage !== previousStage)
            await connection.execute('UPDATE player_omniscient_boss_traces SET stage=? WHERE character_id=? AND spawn_id=?', [stage, character.id, boss.id]);
...
export { acceptOmniscientQuest, awardOmniscientProficiency, claimOmniscientQuest, omniscientProgress, omniscientQuest, omniscientTraces, recordOmniscientObservation };
