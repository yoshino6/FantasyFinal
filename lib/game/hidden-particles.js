...
const diminishingParticle = (count) => 2 * (1 - 2 ** -Math.max(0, count));
const elemental = new Set(hiddenParticles.slice(0, 9).map(particle => particle.code));
const validateHiddenParticles = (particles) => {
    if (particles.length < 2 || particles.length > 4 || particles.some(code => !hiddenParticles.some(particle => particle.code === code)))
        throw new Error('每次调配需要2～4颗有效粒子，每颗各消耗1个。');
    return particles;
};
const hiddenMixProbability = (count, catalyst, kettle = false) => {
    if (![2, 3, 4].includes(count))
        throw new Error('投料数量必须为2～4颗。');
    let failure = 15 + (count - 2) * 2 - (kettle ? 5 : 0), great = kettle ? 30 : 15;
    if (catalyst === 'stable') {
        failure = Math.max(2, failure - 10);
        great = Math.max(5, great - 5);
    }
    if (catalyst === 'excite') {
        failure += 10;
        great += 15;
...
const rollHiddenMix = (count, catalyst, kettle = false, random = Math.random) => {
...
const hiddenMix = (input, outcome = 'success', kettle = false) => {
    const particles = validateHiddenParticles(input), primary = particles[0];
    const counts = Object.fromEntries(hiddenParticles.map(particle => [particle.code, particles.filter(code => code === particle.code).length]));
    ...
    const radiusScale = targets ** -0.65, growth = [0, 0, 1, 1.12, 1.198][count];
    const arc = counts.magic_unit - Number(primary === 'magic_unit'), blood = counts.blood_residue - Number(primary === 'blood_residue');
    const codes = new Set(particles.filter(code => elemental.has(code)));
    if (primary === 'energy_ember' || primary === 'magic_unit' || primary === 'blood_residue')
        codes.add(primary);
    if (primary !== 'wind_element_dust')
        codes.delete('wind_element_dust');
    const damageElements = ['fire_element_dust', 'ice_element_dust', 'thunder_element_dust', 'dark_element_dust', 'wind_element_dust', 'energy_ember', 'magic_unit'];
    if (counts.magic_unit && ![...codes].some(code => damageElements.includes(code)))
        codes.add('magic_unit');
    if (counts.blood_residue && ![...codes].some(code => ['water_element_dust', 'wood_element_dust', 'blood_residue'].includes(code)))
        codes.add('blood_residue');
    const branchCount = codes.size, baseWeight = branchCount === 1 ? 1 : 1 / (branchCount + 1);
    const branches = [...codes].sort().map(code => {
        const weight = branchCount === 1 ? 1 : code === primary ? baseWeight * 2 : baseWeight;
        const addition = elemental.has(code) ? .16 * diminishingParticle(counts[code] - 1) : 0;
        ...
        const branch = { code, element: hiddenParticles.find(p => p.code === code).element, weight, damage: 0, heal: 0, regeneration: 0, shield: 0, slow: 0, defenseDown: 0, burn: 0, damageScale, healScale, stateScale };
        if (code === 'water_element_dust')
            branch.heal = 12;
        if (code === 'fire_element_dust') {
            branch.damage = 110;
            branch.burn = 16;
        }
        if (code === 'metal_element_dust')
            branch.shield = 14;
        if (code === 'wood_element_dust')
            branch.regeneration = 16;
        if (code === 'wind_element_dust')
            branch.damage = 95;
        if (code === 'ice_element_dust') {
            branch.damage = 95;
            branch.slow = 15;
        }
        if (code === 'thunder_element_dust')
            branch.damage = 110;
        if (code === 'light_element_dust')
            branch.shield = 8;
        if (code === 'dark_element_dust') {
            branch.damage = 90;
            branch.defenseDown = 12;
        }
        if (code === 'energy_ember' || code === 'magic_unit')
            branch.damage = 100;
        if (code === 'blood_residue')
            branch.heal = 10;
        if ((code === 'ice_element_dust' || code === 'thunder_element_dust') && counts[code] >= 2)
            branch.control = { code: code === 'ice_element_dust' ? 'freeze' : 'stun', chance: Math.min(great ? 75 : 55, 35 + 10 * diminishingParticle(counts[code] - 2) + (great ? 20 : 0)) * weight };
        return branch;
    });
...
const hiddenBranchDamage = (mix, branch, magic, defense) => {
...
const hiddenBranchHealing = (mix, branch, hpMax) => hpMax * (branch.heal + branch.regeneration) / 100 * branch.weight * branch.healScale * mix.radiusScale * mix.numericScale;

export { diminishingParticle, hiddenBranchDamage, hiddenBranchHealing, hiddenMix, hiddenMixProbability, hiddenParticles, rollHiddenMix, validateHiddenParticles };
