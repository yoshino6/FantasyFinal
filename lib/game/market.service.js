...
import { moveMaterialCosts, takeMaterialCosts, addMaterialCosts } from './talent-material-recovery.js';
...
const settleMatch = async (connection, sell, buy, quantity, price, reference) => {
    ...
    if (sellerWeek.gross + gross > cap)
        throw new Error('卖家本周交易额度已满，请选择其他订单。');
...
const marketSellable = async (qqUserId, page = 1, keyword = '') => {
    ...
    const paging = pageInfo(page, number(countRows[0]?.total));
    const [rows] = await pool.execute(`SELECT i.id,i.name,i.item_category,i.description,i.trade_price,i.stack_limit,(pi.quantity-pi.trade_bound_quantity-pi.personal_bound_quantity) AS quantity,ms.reference_price
...
const createOrder = async (qqUserId, itemId, unitPrice, quantity, side) => withTransaction(async (connection) => {
    ...
    if (number(openRows[0]?.total) + number(instances[0]?.total) >= 60)
    ...
    if (side === 'sell' && week.gross + number(held[0]?.reserved) + number(instances[0]?.reserved) + price * amount > weeklyCap)
        throw new Error(`本周寄售额将超过 ${weeklyCap} 铜币的交易额度。`);
    let reserved = 0;
    let materialCost = { quantity: 0, paid: {} };
    if (side === 'sell') {
    ...
    if (side === 'sell')
        await addMaterialCosts(connection, 'market', Number(order.id), itemId, materialCost);
    if (side === 'sell')
    ...
    if (side === 'sell')
        recordAchievement(connection, Number(character.id), ['ACH_K03']);
...
export { MARKET_PAGE_SIZE, MARKET_TYPES, cancelMarketOrder, createMarketBuyOrder, createMarketSellOrder, marketCatalog, characterFor as marketCharacterFor, feeForSale as marketFeeForSale, marketFeeProfile, marketItemDetail, marketOrders, marketSellable, weeklySales as marketWeeklySales };
