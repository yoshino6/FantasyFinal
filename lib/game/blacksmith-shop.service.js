...
const validQuantity = (quantity) => {
    if (!Number.isInteger(quantity) || quantity < 1 || quantity > 99)
        throw new Error('数量必须是 1 至 99 之间的整数。');
    return quantity;
};
const validMaterialQuantity = (quantity) => {
...
const shopCategoryFilter = (category) => {
    if (weaponShopTypes.includes(category))
        return { selected: category, filter: " AND i.item_category IN ('武器','副手') AND i.weapon_type=?", values: [category] };
    if (armorShopCategories.includes(category))
        return { selected: category, filter: ' AND i.item_category=?', values: [category] };
    if (armorShopTypes.includes(category))
        return { selected: category, filter: ` AND i.item_category IN (${armorShopCategories.map(() => '?').join(',')}) AND i.weapon_type=?`, values: [...armorShopCategories, category] };
    if (category === '防具')
        return { selected: category, filter: ` AND i.item_category IN (${armorShopCategories.map(() => '?').join(',')})`, values: armorShopCategories };
    if (category === '武器')
        return { selected: category, filter: " AND i.item_category IN ('武器','副手')", values: [] };
    return { selected: '全部', filter: '', values: [] };
};
const blacksmithShopCatalog = async (qqUserId, page = 1, category = '全部', keyword = '') => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const term = `%${keyword.trim()}%`;
    const categoryFilter = shopCategoryFilter(category);
    const values = [term, ...categoryFilter.values];
    const [countRows] = await pool.execute(`SELECT COUNT(*) AS total FROM blacksmith_shop_items si JOIN item_definitions i ON i.id=si.item_id WHERE si.is_active=1 AND i.name LIKE ?${categoryFilter.filter}`, values);
    const paging = pageInfo(page, Number(countRows[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT i.id,i.codex_id,i.name,i.item_category,i.weapon_type,i.required_level,i.description,si.buy_price,si.stock_quantity,COUNT(ii.id) AS owned_quantity
...
const blacksmithSellCatalog = async (qqUserId, page = 1, keyword = '') => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const term = `%${keyword.trim()}%`;
    ...
    const [countRows] = await pool.execute(`SELECT COUNT(*) AS total FROM ${source}`, [character.id, term, character.id, term]);
    const paging = pageInfo(page, Number(countRows[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT * FROM ${source} ORDER BY acquired_at DESC,sale_id DESC LIMIT ? OFFSET ?`, [character.id, term, character.id, term, PAGE_SIZE, (paging.page - 1) * PAGE_SIZE]);
...
const buyBlacksmithEquipment = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = validQuantity(quantity);
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT i.id,i.name,si.buy_price,si.stock_quantity FROM blacksmith_shop_items si JOIN item_definitions i ON i.id=si.item_id WHERE si.item_id=? AND si.is_active=1 AND i.item_type='equipment' FOR UPDATE`, [itemId]);
    const item = rows[0];
    if (!item)
        throw new Error('该装备已下架。');
    ...
    for (let index = 0; index < amount; index += 1)
        await connection.execute('INSERT INTO player_item_instances (character_id,item_id,quality,durability,durability_max) VALUES (?,?,0,100,100)', [character.id, item.id]);
...
const sellBlacksmithEquipment = async (qqUserId, instanceId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT ii.id AS instance_id,ii.item_id,i.name,i.item_category,i.required_level,ii.quality,COALESCE(si.sell_price,GREATEST(1,FLOOR(i.required_level*20))) AS sell_price
    FROM player_item_instances ii JOIN item_definitions i ON i.id=ii.item_id
    LEFT JOIN player_equipment pe ON pe.character_id=ii.character_id AND pe.instance_id=ii.id
    LEFT JOIN blacksmith_shop_items si ON si.item_id=i.id
...
const sellBlacksmithMaterial = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = validMaterialQuantity(quantity);
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT i.id,i.name,pi.quantity,CEIL(i.trade_price*1.15) AS sell_price
    FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id
    ...
    const item = rows[0];
    if (!item)
        throw new Error('小北只收购装备、怪材、锻材与粒子。');
...
export { blacksmithSellCatalog, blacksmithShopCatalog, buyBlacksmithEquipment, sellBlacksmithEquipment, sellBlacksmithMaterial };
