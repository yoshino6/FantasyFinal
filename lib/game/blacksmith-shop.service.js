...
import { buySecondaryFinished } from './secondary-shop.service.js';
...
const blacksmithSellCatalog = async (qqUserId, page = 1, keyword = '') => {
    ...
    const paging = pageInfo(page, Number(countRows[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT * FROM ${source} ORDER BY acquired_at DESC,sale_id DESC LIMIT ? OFFSET ?`, [character.id, term, character.id, term, String(PAGE_SIZE), String((paging.page - 1) * PAGE_SIZE)]);
...
const buyBlacksmithEquipment = async (qqUserId, itemId, quantity = 1) => buySecondaryFinished(qqUserId, 'blacksmith', itemId, quantity);
...
const sellBlacksmithMaterial = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    ...
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT i.id,i.name,pi.quantity,FLOOR(i.trade_price*.5) AS sell_price
    FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id
    ...
    const item = rows[0];
    if (!item)
        throw new Error('小北只收购装备、怪材、锻材、粒子与天空粉尘。');
