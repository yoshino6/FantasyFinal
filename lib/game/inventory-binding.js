const consumeBinding = (stock, quantity, unboundOnly = false) => {
    if (!Number.isSafeInteger(quantity) || quantity < 1 || Object.values(stock).some(n => !Number.isSafeInteger(n) || n < 0))
        throw new Error('物品数量无效。');
    if ((unboundOnly ? stock.unbound : stock.unbound + stock.trade + stock.personal) < quantity)
        throw new Error(unboundOnly ? '未绑定数量不足。' : '物品数量不足。');
...
const consumeInventory = async (connection, characterId, itemId, quantity, unboundOnly = false) => {
    const [rows] = await connection.execute('SELECT quantity,trade_bound_quantity,personal_bound_quantity FROM player_inventory WHERE character_id=? AND item_id=? FOR UPDATE', [characterId, itemId]);
    ...
    const [updated] = await connection.execute(`UPDATE player_inventory
...
const productionBinding = (used, quantity, irreversible) => used.personal ? { personal: quantity, trade: 0, unbound: 0 } : used.trade && !irreversible ? { personal: 0, trade: quantity, unbound: 0 } : { personal: 0, trade: 0, unbound: quantity };

export { consumeBinding, consumeInventory, grantInventory, productionBinding };
