...
const hunterDailySpecial = async (pool) => {
  ...
  `);
    if (existing[0])
        return { itemId: existing[0].item_id === null ? null : Number(existing[0].item_id), name: existing[0].name, discountPct: Number(existing[0].discount_pct) };
    const [choices] = await pool.execute('SELECT i.id,i.name FROM hunter_lodge_items hs JOIN item_definitions i ON i.id=hs.item_id WHERE hs.is_active=1 ORDER BY i.id');
...
const hunterCatalog = async (qqUserId, page = 1, keyword = '') => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const special = await hunterDailySpecial(pool);
    const term = `%${keyword.trim()}%`;
    const [countRows] = await pool.execute('SELECT COUNT(*) AS total FROM hunter_lodge_items hs JOIN item_definitions i ON i.id=hs.item_id WHERE hs.is_active=1 AND i.name LIKE ?', [term]);
    const paging = pageInfo(page, Number(countRows[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT i.id,i.codex_id,i.name,i.item_category,i.description,hs.buy_price,hs.stock_quantity,
...
const hunterSellCatalog = async (qqUserId, page = 1, keyword = '') => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const term = `%${keyword.trim()}%`;
    ...
    const paging = pageInfo(page, Number(countRows[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT i.id,i.name,i.item_category,pi.quantity,CEIL(i.trade_price*1.20) AS sell_price
...
const buyHunterItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = quantityOf(quantity);
    const character = await characterFor(connection, qqUserId, true);
    const special = await hunterDailySpecial(connection);
    const [rows] = await connection.execute(`SELECT i.id,i.name,hs.buy_price,hs.stock_quantity,COALESCE(gs.sell_price,0) AS sell_price FROM hunter_lodge_items hs JOIN item_definitions i ON i.id=hs.item_id LEFT JOIN guild_shop_items gs ON gs.item_id=i.id WHERE hs.item_id=? AND hs.is_active=1 FOR UPDATE`, [itemId]);
    const item = rows[0];
    if (!item)
        throw new Error('该兽材已经售罄。');
    if (Number(item.stock_quantity) < amount)
        throw new Error(`库存不足，剩余 ${item.stock_quantity} 份。`);
    ...
    if (Number(character.copper_coins) < price)
...
const sellHunterItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = quantityOf(quantity);
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT i.id,i.name,i.item_type,i.is_tradeable,i.trade_price,i.item_category,pi.quantity,CEIL(i.trade_price*1.20) AS sell_price
    FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id
    WHERE pi.character_id=? AND pi.item_id=? AND pi.quantity>0 FOR UPDATE`, [character.id, itemId]);
    const item = rows[0];
    if (!item || item.item_type !== 'consumable' || !item.is_tradeable || !Number(item.trade_price))
        throw new Error('雷恩只收购可交易的药剂、食物等消耗品。');
...
export { buyHunterItem, hunterCatalog, hunterDailySpecial, hunterSellCatalog, sellHunterItem };
