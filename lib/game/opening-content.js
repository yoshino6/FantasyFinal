...
const openingRouteVersions = [...generatedSevenOpeningRoutes, ...generatedOpeningRoutes, ...forestRoutes].map(applyOpeningRewrite).map(applyOpeningNarrativeExpansion).map(applyOpeningRewardPolicy).map(route => ({ ...route,
...
const openingRoutes = openingRouteVersions.filter((route, index, all) => all.findIndex(r => r.code === route.code) === index);
