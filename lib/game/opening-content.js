import { generatedOpeningRoutes } from './opening-content.generated.js';
import { generatedSevenOpeningRoutes } from './opening-seven.generated.js';
import { finalOpeningRewrites } from './opening-rewrites-final.config.js';
import { openingNarrativeExpansions, openingExpandedLesson } from './opening-narrative-expanded.config.js';
import { applyOpeningRewardPolicy } from './opening-reward-policy.js';
import { retainOpeningRoute } from './opening-retained.config.js';
import { polishRetainedOpening } from './opening-prose-polish.config.js';
export { talentDefinitions } from './talent.config.js';

const forestChoice = (code, label, text, quest, task, rewardName, pack) => ({ code, label, pages: [{ title: quest, text }], quest, task, rewardCode: '', rewardName, rewardUse: '初行的支持', future: quest, farewell: '', pack });
const forestRoutes = [
...
const openingLessonText = (route, choice) => openingNewcomerText(openingExpandedLesson(route.code, choice.code)
    ?? route.lessonText
    ?? routeLessonTexts[`${route.code}${choice.code}`]
    ?? routeLessonTexts[route.code]
    ?? '你已抵达安全区。请根据自己亲眼见到的事，完成这一次交接。');
const applyOpeningRewrite = (route) => {
...
const applyOpeningNarrativeExpansion = (route) => {
...
const retainedRouteVersions = [...generatedSevenOpeningRoutes, ...generatedOpeningRoutes, ...forestRoutes].map(applyOpeningRewrite).map(applyOpeningNarrativeExpansion).map(applyOpeningRewardPolicy).map(retainOpeningRoute).filter((route) => Boolean(route)).map(polishRetainedOpening).map(route => ({ ...route,
...
const openingRouteVersions = retainedRouteVersions.filter((route, index, all) => all.findIndex(r => r.code === route.code) === index);
const openingRoutes = [...openingRouteVersions];
const openingRouteByCode = (code, version) => openingRouteVersions.find(route => route.code === code && (version === undefined || route.version === version));
const openingNewcomerText = (text) => text
...
const openingNarrativeText = (text) => text
...
const firstMeetingIntroductionOffsets = {
...
const firstMeetingCompanionAliases = {
    D02: { 乌禾: '柜后的送葬学徒' },
    W02: { 小苇: '药师的学徒' }, Y03: { 萨芙: '第一位魔族客人' }
};
const firstMeetingIntroductions = {
    F01: '',
    F02: '',
    F03: '',
...
const openingFirstMeetingIntroduction = (route) => {
    if (route.version < 3)
        return '';
    if (route.code in firstMeetingIntroductions)
        return firstMeetingIntroductions[route.code];
    if (!route.person)
        return '';
...
const openingFirstMeetingText = (route, text, pageIndex = 0) => {
    ...
    const before = paragraphs.slice(0, offset).map(paragraph => {
        ...
        for (const [name, alias] of Object.entries(firstMeetingCompanionAliases[route.code] ?? {}))
...
export { openingFirstMeetingIntroduction, openingFirstMeetingText, openingLessonText, openingNarrativeText, openingNewcomerText, openingRouteByCode, openingRouteVersions, openingRoutes };
