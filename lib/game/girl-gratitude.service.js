...
const finishedGoblinQuest = async (connection, characterId) => {
    ...
    const [leaf] = await connection.execute("SELECT 1 FROM player_opening_stories WHERE character_id=? AND route_code='M01' AND destination_code='floating_leaf_town' LIMIT 1", [characterId]);
...
const writeStage = async (connection, characterId, stage) => {
...
const girlGratitudeMainQuest = async (qqUserId) => withTransaction(async (connection) => {
    ...
    const [leaf] = await connection.execute("SELECT 1 FROM player_opening_stories WHERE character_id=? AND route_code='M01' AND destination_code='floating_leaf_town' LIMIT 1", [character.id]);
    if (leaf[0])
        return stage >= 6
...
const startGirlGratitude = (qqUserId) => withTransaction(async (connection) => {
    ...
    const [leaf] = await connection.execute("SELECT 1 FROM player_opening_stories WHERE character_id=? AND route_code='M01' AND destination_code='floating_leaf_town' LIMIT 1", [character.id]);
    if (leaf[0])
        throw new Error('你的浮叶镇路线由菲萝缇陪同游历世界树，请回公馆与她会合。');
    if (!await finishedGoblinQuest(connection, character.id))
...
const teleportToWorldTree = (qqUserId) => withTransaction(connection => teleportToWorldTreeIn(connection, qqUserId));
const teleportToWorldTreeIn = async (connection, qqUserId) => {
    const character = await characterFor(connection, qqUserId, true);
    await requireAt(connection, character, gateCode);
    const [regions] = await connection.execute("SELECT r.id,n.pos_x,n.pos_y,n.pos_z FROM map_regions r JOIN map_npcs n ON n.region_id=r.id AND n.code='world_tree_gate' WHERE r.code='world_tree' AND r.is_enabled=1 AND r.is_owner_only=0 LIMIT 1 FOR UPDATE");
    const destination = regions[0];
    if (!destination)
        throw new Error('世界树界门暂未开放。');
    await (await import('./progression-map.service.js')).ensureMapRegions(connection, character.id, ['world_tree']);
    ...
    const [leaf] = await connection.execute("SELECT 1 FROM player_opening_stories WHERE character_id=? AND route_code='M01' AND destination_code='floating_leaf_town' LIMIT 1", [character.id]);
    if (leaf[0] || await stageFor(connection, character.id, true) !== 1)
        return '界门的银蓝色光纹从脚边升起。片刻后，你已站在世界树的根桥旁，返程界门就在身后。';
...
const returnToBainaTown = (qqUserId) => withTransaction(connection => returnToBainaTownIn(connection, qqUserId));
const returnToBainaTownIn = async (connection, qqUserId) => {
    const character = await characterFor(connection, qqUserId, true);
    await requireAt(connection, character, worldTreeGateCode);
    const [regions] = await connection.execute("SELECT r.id,n.pos_x,n.pos_y,n.pos_z FROM map_regions r JOIN map_npcs n ON n.region_id=r.id AND n.code='world_gate' WHERE r.code='baina_town' AND r.is_enabled=1 AND r.is_owner_only=0 LIMIT 1 FOR UPDATE");
    const destination = regions[0];
    if (!destination)
        throw new Error('百纳镇界门暂未开放。');
    await (await import('./progression-map.service.js')).ensureMapRegions(connection, character.id, ['baina_town']);
    await connection.execute('UPDATE characters SET current_region_id=?,pos_x=?,pos_y=?,pos_z=? WHERE id=?', [destination.id, destination.pos_x, destination.pos_y, destination.pos_z, character.id]);
...
const receiveGirlGratitudeGift = (qqUserId) => withTransaction(async (connection) => {
    ...
    const [leaf] = await connection.execute("SELECT 1 FROM player_opening_stories WHERE character_id=? AND route_code='M01' AND destination_code='floating_leaf_town' LIMIT 1", [character.id]);
    if (leaf[0])
        throw new Error('菲萝缇在市场等你，请继续本路线的同行。');
...
export { continueGirlGratitude, girlGratitudeMainQuest, girlGratitudePending, girlGratitudeStage, receiveGirlGratitudeGift, returnToBainaTown, returnToBainaTownIn, startGirlGratitude, teleportToWorldTree, teleportToWorldTreeIn };
