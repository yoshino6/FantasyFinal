const normal = (rewardKind, rewardName, rewardUse, extra = {}) => ({ rewardKind, rewardName, rewardUse, bonusItem: undefined, rewardItems: undefined, rewardCopper: undefined, rewardEquipment: undefined, ...extra });
const rewards = {
...
const applyOpeningRewardPolicy = (route) => ({ ...route, choices: route.choices.map(choice => {
        const change = rewards[`${route.code}-${choice.code}`];
        return change ? { ...choice, ...change } : choice;
    }) });

export { applyOpeningRewardPolicy };
