...
const emptyTalentData = () => ({ settings: {}, counters: {}, flags: {}, remainders: {}, jobs: [] });
const talentSchema = [
...
const talentDay = (now = new Date()) => new Intl.DateTimeFormat('en-CA', { timeZone: 'Asia/Shanghai', year: 'numeric', month: '2-digit', day: '2-digit' }).format(now);
...
const applyTalentPanel = async (connection, id, stats, elements, neutralStats = stats) => {
    ...
    if (modifier.mastery)
        elements.mastery[modifier.mastery] = Number(elements.mastery[modifier.mastery] ?? 0) + 50;
    if (modifier.resistance)
        elements.resistance[modifier.resistance] = Number(elements.resistance[modifier.resistance] ?? 0) + 50;
...
const neutralTalentSnapshot = async (connection, row, scaleCurrent = false) => {
    ...
    const fields = { mpMax: 'mp_max', physicalAttack: 'physical_attack', magicAttack: 'magic_attack', physicalDefense: 'physical_defense', magicDefense: 'magic_defense', accuracy: 'accuracy', evasion: 'evasion', speed: 'speed', critRateBp: 'crit_rate_bp', critDamageBp: 'crit_damage_bp', critResistBp: 'crit_resist_bp', critDamageReductionBp: 'crit_damage_reduction_bp', tenacity: 'tenacity', tenacityPierce: 'tenacity_pierce' };
    for (const [key, field] of Object.entries(fields))
        if (base.stats?.[key] !== undefined)
            row[field] = Number(base.stats[key]);
    if (scaleCurrent)
        row.current_mp = Math.min(Number(row.mp_max), Math.max(0, Math.floor(Number(row.mp_max) * mpRatio)));
    row.hp_max = Number(base.hpMax);
    if (scaleCurrent)
        row.current_hp = ratio <= 0 ? 0 : Math.max(1, Math.floor(row.hp_max * Math.min(1, ratio)));
...
const initializeTalentPersistence = async (pool) => {
    ...
    if (affinityColumns.some(column => /unsigned/i.test(String(column.COLUMN_TYPE))))
        await pool.query('ALTER TABLE player_npc_affinity MODIFY COLUMN affinity BIGINT NOT NULL DEFAULT 0');
    for (const sql of talentSchema)
        await pool.query(sql);
    for (const column of ['children_json JSON NULL', 'children_scale DECIMAL(20,8) NOT NULL DEFAULT 1'])
        try {
    ...
    if (!names.has('talent_cost_clamp_v2'))
    ...
    if (!names.has('talent_cost_delete'))
        await pool.query(`CREATE TRIGGER talent_cost_delete AFTER DELETE ON player_inventory FOR EACH ROW DELETE FROM talent_material_costs WHERE owner_type='stock' AND owner_id=OLD.character_id AND item_id=OLD.item_id`);
    ...
    if (!names.has('talent_product_delete'))
        await pool.query(`CREATE TRIGGER talent_product_delete AFTER DELETE ON player_inventory FOR EACH ROW DELETE FROM player_talent_products WHERE character_id=OLD.character_id AND item_id=OLD.item_id`);
};
