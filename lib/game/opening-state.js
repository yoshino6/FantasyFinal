import { openingRouteByCode, openingRoutes } from './opening-content.js';
import { openingSpawnRegions, openingStartRouteCodes, openingHubs } from './opening-world.config.js';
import { drawOpeningRoute, chooseWeighted } from './opening-route-draw.js';
import { validWorldSitePoint, pointBelongsToRegion } from './world-site-geometry.js';

const assertOpeningFree = async (connection, characterId, forestSpawnId) => {
    const [rows] = await connection.execute("SELECT state FROM player_opening_stories WHERE character_id=? AND state<>'completed' LIMIT 1", [characterId]);
    if (!rows.length)
        return;
    if (Number.isSafeInteger(forestSpawnId) && Number(forestSpawnId) > 0) {
        const [pending] = await connection.execute(`SELECT o.character_id FROM player_opening_stories o
...
const openingWorldFor = async (connection, lock = false) => {
    const [rows] = await connection.execute(`SELECT * FROM opening_world WHERE id=1${lock ? ' FOR UPDATE' : ''}`);
    if (!rows[0])
        throw new Error('接引记录尚未初始化。');
    return rows[0];
};
const openingSafeHubs = async (connection, lock = false) => {
    const codes = Object.keys(openingHubs);
    const [rows] = await connection.execute(`SELECT r.id,r.code,n.code AS guild_code,n.pos_x,n.pos_y,n.pos_z
    ...
    return rows.filter(row => openingHubs[String(row.code)]?.guild === row.guild_code
        && pointBelongsToRegion(areas, Number(row.id), { x: Number(row.pos_x), y: Number(row.pos_y), z: Number(row.pos_z) }));
};
const availableOpeningSpawns = async (connection) => {
    const [rows] = await connection.execute('SELECT * FROM map_regions WHERE newbie_spawn_enabled=1 AND is_enabled=1 AND is_owner_only=0 ORDER BY id');
    const [areas] = await connection.execute('SELECT a.*,r.danger_level FROM map_region_areas a JOIN map_regions r ON r.id=a.region_id');
    const hubs = await openingSafeHubs(connection);
    const enabled = new Set(hubs.map(h => String(h.code)));
    if (!enabled.size)
        throw new Error('安全接引点暂未备齐，请稍后重新选择恩赐；你的选择尚未扣除或提交。');
    const candidates = rows.flatMap(region => {
        const configuration = openingSpawnRegions[String(region.code)];
        if (!configuration)
            return [];
        ...
        if (!routes.length)
            return [];
        try {
            return [{ region, configuration, routes, point: validWorldSitePoint(areas, Number(region.id), { x: Number(region.min_x), y: Number(region.min_y), z: Number(region.min_z) }) }];
        }
        catch {
            return [];
        }
    ...
    if (!candidates.length)
        throw new Error('暂时没有已开放且可安全降临的地图，请稍后重试。');
    return { candidates, areas, enabled };
};
const openingSpawnPoint = (candidate, areas, random = Math.random) => {
    ...
    for (let attempt = 0; attempt < 64; attempt++) {
        ...
        if (pointBelongsToRegion(areas, Number(candidate.region.id), trial)) {
            point = trial;
            break;
        }
    }
...
const chooseOpeningSpawn = async (connection, random = Math.random) => {
...
export { assertOpeningFree, availableOpeningSpawns, chooseOpeningSpawn, chooseWeighted, openingSafeHubs, openingSpawnPoint, openingWorldFor };
