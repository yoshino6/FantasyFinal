...
const kingbeastCombatMultipliers = (unit) => {
    ...
    return {
        attack: (role === 'spearman' && symbiosis ? .75 : 1) * (enrage ? 1.30 : 1) * (role === 'king' && Boolean(cooldowns.kingbeast_castling_attack) ? 1.20 : 1),
        defense: role === 'guard' && symbiosis ? .75 : 1,
        accuracy: (enrage ? 1.20 : 1) * (royalSignal ? 1.30 : 1),
        speed: (enrage ? 1.15 : 1) * (royalSignal ? 1.30 : 1),
        tenacity: enrage ? 1.30 : 1
    };
};
...
const kingbeastPanelSummary = (units, turn, lastSummonTurn) => {
    ...
    if (kingbeastSymbiosisActive(units))
        states.push('矛盾共生：双侍卫受抑，王座双核心最终伤害-33%');
