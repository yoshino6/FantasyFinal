...
const isSecondaryFinishedProduct = (shop, item) => shop === 'alchemy_sweetshop' ? item.item_type === 'consumable' && (alchemyCodes.has(item.code) || noviceAlchemyCodes.has(item.code) || item.code === 'alchemy_skill_reset_elixir' && secondaryShopBasicLevel.alchemy_sweetshop >= 3) : shop === 'oddworkshop' ? ['device', 'consumable'].includes(item.item_type) && deviceCodes.has(item.code) : shop === 'blacksmith' ? (item.code === 'forge_repair_kit' || item.item_type === 'equipment' && item.code.startsWith('shop_') && Number(item.required_level ?? Infinity) <= secondaryShopBasicLevel.blacksmith * 5 && item.rarity === '普通' && item.item_category !== '异械') : false;
