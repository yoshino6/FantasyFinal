...
const discoverSecondaryFinished = async (user, shopName, codex) => {
    ...
    return withTransaction(async (connection) => {
        ...
        const [discovered] = await connection.execute('INSERT IGNORE INTO player_item_codex (character_id,item_id) VALUES (?,?)', [id, item.id]);
        if (discovered.affectedRows)
            await recordCharacterOperation(connection, { characterId: id, kind: 'item.codex_discovered', source: { system: 'player_item_codex', id: Number(item.id), step: 'discovered' }, outcome: '发现', summary: `在${secondaryShopNames[shop]}发现「${item.name}」`, detail: { itemId: Number(item.id), itemCode: String(item.code), shopCode: shop } });
