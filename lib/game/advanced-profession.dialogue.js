import { renameAdvancedMentorText, inheritancePassiveFor, advancedProfessionByCode } from './advanced-profession.config.js';
import { spiritDefinitions } from './spirit-summoner.config.js';

const relation = (affinity) => affinity >= 500 ? '他看见你时，语气比从前松缓了些。' : affinity >= 200 ? '他认出了你，停下手里的事认真回应。' : affinity >= 50 ? '他向你点头，示意可以聊几句。' : '他先打量了你一眼，随后平静地开口。';
const period = () => { const hour = new Date().getHours(); return hour < 11 ? 'morning' : hour < 18 ? 'afternoon' : 'evening'; };
...
const advancedProfessionReveal = (profession) => {
    ...
    if (inheritance)
        lines.push(`传承被动【${inheritance.name}】：${inheritance.ownDescription}`, `旁修效果：${inheritance.studyDescription}（Lv.30 后可在其他导师处学习；只能装备一条已学旁修。）`);
...
export { advancedProfessionReveal, mentorChatDialogue, mentorDialogues, mentorSuccessDialogue };
