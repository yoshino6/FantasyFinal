...
import { calculatePanelStats } from './panel-stat-formula.js';
import { advancedProfessionByMentor, registeredAdvancedProfessionByCode, cachedAdvancedPassiveEffectFor } from './advanced-profession.config.js';
...
const weights = { warrior: [25, 12, 25, 8, 14, 16], mage: [12, 25, 8, 27, 12, 16], rogue: [13, 10, 20, 10, 25, 22], priest: [22, 27, 8, 21, 10, 12], archer: [14, 8, 18, 10, 24, 26] };
const professionGrowth = { warrior: ['constitution', 'strength'], mage: ['spirit', 'intelligence'], rogue: ['agility', 'perception'], priest: ['constitution', 'spirit'], archer: ['agility', 'perception'] };
const recalculateNpcSparProfileStats = (profile) => {
    const archetype = profile.profession || 'warrior';
    if (!weights[archetype])
        throw new Error(`无法识别域民职业：${archetype}`);
    const birthAttributes = Object.fromEntries(attributes.map((key, i) => [key, weights[archetype][i]]));
    const fixedGrowth = Object.fromEntries(attributes.map((key, i) => [key, Math.round((weights[archetype][i] / 10 + (professionGrowth[profile.profession]?.includes(key) ? 1.2 : 0)) * 10) / 10]));
    const trainedAttributes = Object.fromEntries(attributes.map(key => [key, Math.round((birthAttributes[key] + fixedGrowth[key] * playerGrowthShares(profile.level)) * 10) / 10]));
    ...
    const armorType = { warrior: '重甲', mage: '布甲', rogue: '皮甲', priest: '轻甲', archer: '轻甲' }[archetype];
...
const buildNpcSparProfile = (npc, playerLevel, worldStage) => {
    ...
    const baseNameToCode = { 战士: 'warrior', 法师: 'mage', 盗贼: 'rogue', 牧师: 'priest', 射手: 'archer' };
    ...
    return { code, name: mentor?.mentor.name ?? sparBuildingPersonas[code] ?? dynamic?.displayName ?? npc.name, level, band, worldStage,
        profession, advancedCode, advancedName: registeredAdvancedProfessionByCode(advancedCode ?? '')?.name ?? '', advancedEffect: registeredAdvancedProfessionByCode(advancedCode ?? '')?.passive.effect ?? {},
...
export { buildNpcSparProfile, canSparNpc, canonicalSparNpc, carriedSparSkills, recalculateNpcSparProfileStats, sparBuildingPersonas, sparRegionBands };
