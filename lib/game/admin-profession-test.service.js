...
const baseCodes = { '战士': 'warrior', '法师': 'mage', '盗贼': 'rogue', '牧师': 'priest', '射手': 'archer' };
...
const professionTestArmorSetFor = (advancedCode, base) => advancedArmorProfiles[advancedCode] ?? (equipmentProfiles[base] ?? equipmentProfiles.warrior).set;
...
const equipEpicTestSet = async (connection, characterId, base, advancedCode) => {
    const profile = equipmentProfiles[base] ?? equipmentProfiles.warrior;
    const armorSet = professionTestArmorSetFor(advancedCode, base);
    const recipes = [...epicForgeRecipes.filter(recipe => recipe.setCode === armorSet), ...profile.weapons.map(code => epicForgeRecipes.find(recipe => recipe.code === code))].filter((recipe) => Boolean(recipe));
    if (recipes.length !== 7)
...
const hasEpicTestSet = async (connection, characterId, base, advancedCode) => {
    const profile = equipmentProfiles[base] ?? equipmentProfiles.warrior;
...
export { adminTestProfession, professionTestArmorSetFor, professionTestOptions };
