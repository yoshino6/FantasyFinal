...
import { combatItemEffect, itemEffect } from './item-use-policy.js';
...
const quickItemConfig = async (qqUserId) => {
    ...
    const [items] = await pool.execute(`SELECT i.id,i.code,i.effect_json,i.name,i.item_category,i.codex_id,pi.quantity
...
const setQuickItem = async (qqUserId, slot, itemId) => withTransaction(async (connection) => {
    ...
    const [items] = await connection.execute(`SELECT i.id,i.code,i.effect_json,i.name,i.item_category,pi.quantity
    FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id
    ...
    if (!combatItemEffect(itemEffect(item)))
        throw new Error('该物品不适用于战斗快捷栏，请从背包使用。');
    const [configured] = await connection.execute('SELECT quick_slot,item_id FROM player_quick_items WHERE character_id=? AND (quick_slot=? OR item_id=?) FOR UPDATE', [characterId, slot, itemId]);
    if (configured.length === 1 && Number(configured[0].quick_slot) === slot && Number(configured[0].item_id) === itemId)
...
const clearQuickItem = async (qqUserId, slot) => withTransaction(async (connection) => {
    ...
    if (!characters[0])
        throw new Error('请先发送“注册”创建角色。');
    await assertCombatLoadoutMutable(connection, Number(characters[0].id));
    const [deleted] = await connection.execute('DELETE FROM player_quick_items WHERE character_id=? AND quick_slot=?', [characters[0].id, slot]);
    if (Number(deleted.affectedRows) > 0)
        await recordCharacterOperation(connection, { characterId: Number(characters[0].id), kind: 'item.quick_slot_changed', source: { system: 'quick_item_config', id: randomUUID(), step: 'clear' }, outcome: '清除', summary: `清除道具快捷栏 ${slot}`, detail: { slot } });
});
const toggleQuickItem = async (qqUserId, itemId) => withTransaction(async (connection) => {
    ...
    if (configured[0]) {
        ...
        await recordCharacterOperation(connection, { characterId, kind: 'item.quick_slot_changed', source: { system: 'quick_item_config', id: randomUUID(), step: 'toggle_off' }, outcome: '取消', summary: `取消道具 ${itemId} 的快捷栏`, detail: { slot: Number(configured[0].quick_slot), itemId } });
        return { enabled: false };
    }
    const [items] = await connection.execute(`SELECT i.id,i.code,i.effect_json,i.name,i.item_category,i.codex_id,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id
    WHERE pi.character_id=? AND pi.item_id=? AND pi.quantity>0 AND i.item_type='consumable' AND i.effect_json IS NOT NULL FOR UPDATE`, [characterId, itemId]);
    if (!items[0])
        throw new Error('背包中没有该可用道具。');
    if (!combatItemEffect(itemEffect(items[0])))
        throw new Error('该物品不适用于战斗快捷栏，请从背包使用。');
