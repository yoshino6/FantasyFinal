...
import { characterIdFor, blacksmithProgressFor, forgeEquipmentCapsFor, forgePrimaryKeys, tierForgeMaterial, forgeName, addBlacksmithProficiency, forgeFee, forgeRequirements, randomSecondaryAffixPool, secondaryAffixCount, pickRandomSecondaryAffix, randomForgeValue, specialAffixChance } from './blacksmith.service.js';
import { lowForgeMaterials, allocateLowMaterials, fusionQualityLoss, refinementGain, breakthroughs, qualityDescription, refinementCounts, rerollRatios, verifiedLegacyLayers } from './equipment-workshop-rules.js';
...
import { talentMaterialPayment, consumeTalentMaterial, fixedTalentMaterials } from './talent-production.js';
...
const load = async (c, owner, id) => {
    const [rows] = await c.execute(`SELECT ii.*,i.code,i.name,i.item_type,i.item_category,i.weapon_type,i.rarity,i.required_level,i.effect_json AS definition_effect,
    ...
    const row = rows[0];
    if (!row || row.item_type !== 'equipment' || row.item_category === '异械' || !refinementCounts[row.rarity])
        throw new Error('请选择自己背包中的常规装备。');
    if (row.equipped)
        throw new Error('请先卸下装备。');
    if (row.market_listing_id)
        throw new Error('请先取回寄售中的装备。');
    await assertCombatLoadoutMutable(c, owner);
    await assertHiddenInstanceMutable(c, owner, id);
    return row;
};
const layersFor = async (c, row) => {
    if (row.layers_json)
        return object(row.layers_json);
    ...
    if (!row.legacy)
        return { base: { ...current }, frozen: {}, randomSpecial };
    const [ledger] = await c.execute('SELECT effect_json FROM equipment_fusions WHERE instance_id=? ORDER BY id', [row.id]);
...
const requirementsFor = (row, mode) => {
    const level = Number(row.required_level), quality = Number(row.quality);
    if (mode === 'refine') {
        if (quality >= 100 && !breakthroughs[row.rarity])
            throw new Error('该装备已臻圆满，无法继续常规精炼。');
    ...
    if (mode === 'fusion' && (row.rarity === '史诗' || ![5, 10, 15, 20, 25].includes(level)))
        throw new Error('当前仅能熔铸升级尚未达到本阶段顶点的常规装备；史诗需专属升阶配方。');
    if (mode === 'reroll' && !rerollRatios[row.rarity])
        throw new Error('普通装备尚无副词条，请先精炼突破。');
    ...
    if (row.rarity === '史诗' && !epic)
        throw new Error('未找到该史诗装备的打造配方。');
...
const workshopCosts = async (c, owner, requirements, selected = [], discounted = false) => {
    const costs = [];
    for (const requirement of requirements) {
        const codes = requirement.code === 'living_wood' ? [...lowForgeMaterials] : [requirement.code];
        const [rows] = await c.execute(`SELECT i.id,i.code,i.name,COALESCE(pi.quantity,0) AS quantity FROM item_definitions i
      LEFT JOIN player_inventory pi ON pi.item_id=i.id AND pi.character_id=? WHERE i.code IN (${codes.map(() => '?').join(',')}) FOR UPDATE`, [owner, ...codes]);
        if (discounted)
            for (const row of rows) {
                ...
                while (low < high) {
                    ...
                    if (await talentMaterialPayment(c, owner, Number(row.id), mid, 'craft', !currentSecondaryShop()) <= Number(row.quantity))
                        low = mid;
                    else
                        high = mid - 1;
                }
        ...
        if (requirement.code === 'living_wood') {
            ...
            if (allocation.remaining)
                throw new Error(`低级主材不足，还需 ${allocation.remaining} 个（活纹木胚、根心、河壳、潮壳可混用）。`);
        ...
        for (const code of codes) {
            ...
            if (quantity > 0) {
                const existing = costs.find(r => r.id === Number(row.id));
                if (existing)
                    existing.quantity += quantity;
                else
                    costs.push({ id: Number(row.id), code, name: row.name, quantity });
                remaining -= quantity;
            }
        }
        if (remaining)
            throw new Error(`${requirement.code === 'living_wood' ? '低级主材（活纹木胚、根心、河壳、潮壳可混用）' : rows[0]?.name ?? requirement.code}不足，还需 ${remaining} 个。`);
    }
    if (discounted)
        for (const cost of costs) {
            cost.recipeQuantity = cost.quantity;
            cost.quantity = await talentMaterialPayment(c, owner, cost.id, cost.recipeQuantity, 'craft', !currentSecondaryShop());
        }
    return costs;
};
const workshopList = async (user) => withTransaction(async (c) => {
    const owner = await characterIdFor(c, user, true);
    await blacksmithProgressFor(c, owner, true);
    const [rows] = await c.execute(`SELECT ii.id,i.name,i.rarity,i.required_level,ii.quality FROM player_item_instances ii JOIN item_definitions i ON i.id=ii.item_id
...
const workshopPreview = async (user, id, mode) => withTransaction(async (c) => {
    const owner = await characterIdFor(c, user, true);
    await blacksmithProgressFor(c, owner, true);
    const row = await load(c, owner, id);
    if (!await layersFor(c, row) && (mode === 'reroll' || mode === 'refine' && Number(row.quality) >= 100))
        throw new Error('这件装备的旧版熔铸记录与现有属性不一致，需核验后才能洗练或突破；仍可精炼品质与熔铸升级。');
    ...
    for (const material of materials)
        costs.push({ id: Number(material.item.id), code: material.item.code, name: material.item.name, recipeQuantity: material.quantity, quantity: await talentMaterialPayment(c, owner, Number(material.item.id), material.quantity, 'craft', !currentSecondaryShop()) });
...
const rollAffixes = (row, effect, budget, rarity, keep) => {
    ...
    while (count < secondaryAffixCount(rarity) && available.length) {
        const key = pickRandomSecondaryAffix(available);
        effect[key] = randomForgeValue(caps[key]);
        count++;
    }
    if (!keep && randomSpecial && Math.random() < specialAffixChance(budget))
        effect[row.item_category === '武器' && row.weapon_type !== '盾牌' ? 'damageBonusPct' : 'damageReductionPct'] = Math.round(randomForgeValue(5) * 10) / 10;
};
const workshopExecute = async (user, token, expectedMode) => withTransaction(async (c) => {
    const owner = await characterIdFor(c, user, true);
    await blacksmithProgressFor(c, owner, true);
    const [quotes] = await c.execute('SELECT *,expires_at<=NOW() AS expired FROM equipment_workshop_quotes WHERE token=? AND character_id=? FOR UPDATE', [token, owner]);
    const quote = quotes[0];
    if (!quote || quote.source !== source() || quote.mode !== expectedMode)
        throw new Error('确认凭据与本次操作或入口不符，请重新预览。');
    if (quote.result_json)
        return object(quote.result_json);
    if (quote.expired)
        throw new Error('预览已过期，请重新放入装备。');
    const row = await load(c, owner, Number(quote.instance_id)), snapshot = object(quote.snapshot_json);
    if (fingerprint(row) !== snapshot.hash)
        throw new Error('装备状态已经变化，请重新预览。');
    requirementsFor(row, expectedMode);
    const costs = object(quote.costs_json);
    const [paid] = await c.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=? AND copper_coins>=?', [quote.fee, owner, quote.fee]);
    ...
    for (const cost of costs) {
        if (cost.recipeQuantity !== undefined) {
            ...
            if (expected !== cost.quantity)
                throw new Error('材料减耗状态已变化，请重新预览。');
            const used = await consumeTalentMaterial(c, owner, cost.id, cost.recipeQuantity, 'craft', !currentSecondaryShop());
            if (used.paid !== cost.quantity)
                throw new Error('材料结算发生变化，请重新预览。');
            if (used.binding.personal)
                bound = 'personal';
        }
        else {
            const used = await consumeInventory(c, owner, cost.id, cost.quantity);
            if (used.personal)
                bound = 'personal';
        }
    }
    ...
    if (!layers && (expectedMode === 'reroll' || snapshot.breakthrough))
        throw new Error('旧版词条来源尚未核验。');
    ...
    if (expectedMode === 'fusion') {
        quality = Math.round((quality - fusionQualityLoss(rarity, quality)) * 10) / 10;
        level += 5;
        outcome = '装备等级提升，火候有所回落，原有词条完整保留。';
    }
    if (expectedMode === 'reroll') {
        rollAffixes(row, effect, budget, rarity, false);
        quality = Math.min(100, quality + 2 + Math.floor(Math.random() * 3));
        outcome = '副词条已重新淬炼，火候略有精进。';
    }
    if (expectedMode === 'refine' && !snapshot.breakthrough) {
        const gain = refinementGain();
        quality = Math.min(100, Math.round((quality + gain) * 10) / 10);
        outcome = gain >= 7 ? '炉火相合，品质获得了可喜的提升。' : '精炼成功，装备火候进一步精进。';
    }
    if (snapshot.breakthrough) {
        ...
        if (failures + 1 >= rule.pity || Math.random() < rule.chance) {
            ...
            for (const [key, cap] of Object.entries(oldCaps))
                if (typeof effect[key] === 'number' && cap > 0)
                    effect[key] = Math.round(Math.min(1, effect[key] / cap) * (caps[key] ?? cap) * 100) / 100;
            rarity = rule.next;
            failures = 0;
            rollAffixes(row, effect, budget, rarity, true);
            outcome = '突破成功，装备迈入了新的稀有度。';
        }
        else {
            failures++;
            outcome = '突破未成，装备仍保持圆满火候，积累的淬炼心得将在后续尝试中发挥作用。';
        }
    }
    const nextLayers = layers ? { base: { ...effect }, frozen: layers.frozen, randomSpecial: layers.randomSpecial ?? false } : null;
    if (layers)
        for (const [key, value] of Object.entries(layers.frozen))
            effect[key] = Math.round((Number(effect[key] ?? 0) + value) * 10000) / 10000;
    if (expectedMode !== 'refine' || rarity !== row.rarity) {
        if (rarity !== '史诗') {
            const main = level <= 20 ? (costs.filter(r => lowForgeMaterials.includes(r.code)).sort((a, b) => b.quantity - a.quantity + (b.quantity === a.quantity ? (lowForgeMaterials.indexOf(a.code) - lowForgeMaterials.indexOf(b.code)) : 0))[0]?.code ?? 'living_wood') : tierForgeMaterial(row.item_category, row.weapon_type, level);
            name = forgeName(row.weapon_type, level, effect, main, row.item_category, rarity, budget);
        }
        const [definition] = await c.execute(`INSERT INTO item_definitions(code,name,description,obtain_source,item_type,item_category,weapon_type,rarity,required_level,weight,trade_price,stack_limit,stackable,is_tradeable,effect_json)
      SELECT ?,?,description,obtain_source,item_type,item_category,weapon_type,?,?,weight,trade_price,stack_limit,stackable,is_tradeable,? FROM item_definitions WHERE id=?`, [`workshop_${randomUUID()}`, name, rarity, level, JSON.stringify(effect), row.item_id]);
    ...
    await recalculateCharacterStats(c, owner);
    await addBlacksmithProficiency(c, owner);
    if (expectedMode === 'refine' && quality > Number(row.quality) && !currentSecondaryShop())
        recordAchievement(c, owner, ['ACH_I16']);
...
export { workshopCosts, workshopExecute, workshopList, workshopPreview };
