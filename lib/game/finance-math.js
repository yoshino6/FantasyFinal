...
const financeBusinessDate = (date = new Date()) => {
    const p = shanghaiParts(date);
    return `${p.year}${p.month}${p.day}`;
};
const financePeriodKey = (date = new Date()) => {
    const p = shanghaiParts(date);
...
const tryConstrainedFinancePrices = (stocks, directions) => {
    ...
    const desired = stocks.map(item => {
        ...
        const min = Math.max(1, Math.ceil(item.priceMilli * .95), Math.ceil(anchor * .8), minQuote * 1000 - 500, direction === 1 ? quote * 1000 + 500 : 1);
        const max = Math.min(Math.floor(item.priceMilli * 1.05), Math.floor(anchor * 1.2), maxQuote * 1000 + 499, direction === -1 ? quote * 1000 - 501 : Number.MAX_SAFE_INTEGER);
        if (min > max)
            return null;
        return { ...item, target: item.priceMilli * (1 + .004 * Math.max(-20, Math.min(20, item.score - mean))), min, max };
    });
    if (desired.some(item => item === null))
        return null;
...
const stableFinancePrices = (stocks) => {
    ...
    if (!result)
        throw new Error('证券参考价无法在限幅内保持总值守恒。');
    return result;
};
