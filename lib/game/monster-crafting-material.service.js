const materialValueMultiplierForLevel = (level) => Math.pow(1.2, Math.floor((Math.max(1, Math.floor(level)) - 1) / 10));
const purificationOutputMultiplierForLevel = (level) => Math.max(1, Math.floor(level)) <= 10 ? 1 : 1.2;
const kindMeta = {
...
const purifiedTierTwoValues = { hair: 32, gel_skin: 32, bone: 34, shell: 36, scale: 38 };
const purifiedCraftMaterialTierForLevel = (level) => Math.min(5, Math.max(1, Math.ceil(Math.max(1, Math.floor(level)) / 20)));
const uniqueMonsterMaterialNames = {
...
const monsterCraftMaterialKinds = (name, monsterClass) => {
    const count = monsterClass === 'boss' ? 5 : monsterClass === 'elite' ? 4 : monsterClass === 'large' ? 3 : 2;
    const primary = /蛇|蚺|蟒|蜥|龙|鳄|鳗|鲨|鱼人|蛟/.test(name) ? ['scale', 'bone']
                        ...
                        : /鸟|隼|鹰|鹭|鹳|鹫|鸥|蝠/.test(name) ? ['hair', 'bone']
                            : /蛙|蟾/.test(name) ? ['gel_skin', 'bone']
                                : /兔|鼠|鹿|羊|狼|熊|狗|犬|狐|豺|猪|犀|獭|白角/.test(name) ? ['hair', 'gel_skin']
                                    : ['gel_skin', 'bone'];
    const allKinds = ['hair', 'gel_skin', 'bone', 'shell', 'scale'];
    return [...primary, ...allKinds.filter(kind => !primary.includes(kind))].slice(0, count);
};
const monsterCraftMaterialKind = (name) => monsterCraftMaterialKinds(name)[0];
const monsterCraftMaterialCode = (monsterCode, kind) => `monster_${monsterCode}_${kind}`;
const beastCoreCode = () => 'beast_core';
const beastCoreName = () => '兽核';
const meatChunkCode = () => 'meat_chunk';
const meatChunkName = () => '新鲜肉块';
const monsterDropsMeat = (name) => /兔|鼠|鹿|羊|狼|熊|狗|犬|狐|豺|猪|犀|獭|白角|蛙|蟾|蛇|蚺|蟒|蜥|龙|鳄|鳗|鲨|鱼人|蛟|鸟|隼|鹰|鹭|鹳|鹫|鸥|蟹|龟/.test(name);
const meatChunkQuantity = (monsterClass) => monsterClass === 'normal' ? 1 : monsterClass === 'large' ? 2 : 3;
const monsterCraftMaterialName = (monsterCode, monsterName, kind, materialIndex) => {
    ...
    const conciseSuffix = { hair: '绒', gel_skin: '革', bone: '骨', shell: '甲', scale: '鳞' };
    if (materialIndex >= 2)
        return `${stem}${conciseSuffix[kind]}`;
...
const purifiedCraftMaterialCode = (kind, level = 1) => {
    const tier = purifiedCraftMaterialTierForLevel(level);
    return `${kindMeta[kind].refinedCode}${tier === 1 ? '' : `_t${tier}`}`;
};
const purifiedCraftMaterialTierForCode = (code) => {
    const match = code.match(/_t([1-5])$/);
    return Math.max(1, Number(match?.[1] ?? 1));
};
const purifiedCraftMaterialKindForCode = (code) => (Object.entries(kindMeta).find(([, meta]) => code === meta.refinedCode || code.startsWith(`${meta.refinedCode}_t`))?.[0] ?? undefined);
const purifiedCraftMaterialBaseCode = (code) => {
    const kind = purifiedCraftMaterialKindForCode(code);
    return kind ? kindMeta[kind].refinedCode : undefined;
};
const purifiedCraftMaterialName = (kind, level = 1) => {
    const tier = purifiedCraftMaterialTierForLevel(level);
    return kindMeta[kind].refinedNames[tier - 1];
};
const purifiedCraftMaterialDisplayName = (code) => {
    const kind = purifiedCraftMaterialKindForCode(code);
    return kind ? purifiedCraftMaterialName(kind, purifiedCraftMaterialTierForCode(code) * 20) : undefined;
};
const purifiedMaterialForArmor = (armorType, equipmentLevel = 1) => {
    const kind = (Object.entries(kindMeta).find(([, meta]) => meta.armorType === armorType)?.[0] ?? 'gel_skin');
    return purifiedCraftMaterialCode(kind, equipmentLevel);
};
const purifiedCraftMaterialValue = (code) => {
    const kind = purifiedCraftMaterialKindForCode(code);
    return kind ? purifiedTierTwoValues[kind] * Math.pow(2, purifiedCraftMaterialTierForCode(code) - 2) : 0;
};
const purifiedCraftOutputFor = (code, sourceLevel = 1) => {
    const match = code.match(/^(?:monster_.+|map_.+)_(hair|gel_skin|bone|shell|scale)(?:_l\d+)?$/);
    return match ? purifiedCraftMaterialCode(match[1], sourceLevel) : undefined;
};
const resolvedMonsterMaterialDropCode = (drop, _monsterLevel) => {
    const kind = String(drop.material_kind ?? '');
    if (['hair', 'gel_skin', 'bone', 'shell', 'scale'].includes(kind) && typeof drop.material_monster === 'string')
        return monsterCraftMaterialCode(drop.material_monster, kind);
    if (drop.dynamic_material === 'beast_core')
        return beastCoreCode();
    if (drop.dynamic_material === 'meat_chunk')
        return meatChunkCode();
    return String(drop.code ?? '');
};
const allPurifiedCraftMaterials = () => ['hair', 'gel_skin', 'bone', 'shell', 'scale'].flatMap(kind => kindMeta[kind].refinedNames.map((_name, index) => {
    const tier = index + 1;
    return {
...
const allBeastCoreMaterials = () => [{ code: beastCoreCode(), name: beastCoreName(), description: '蕴含野性魔力的兽核，可用作基础怪物素材。' }];
const allMeatChunkMaterials = () => [{ code: meatChunkCode(), name: meatChunkName(), description: '来自可食用怪物的新鲜肉块，可作为食材使用。' }];

export { allBeastCoreMaterials, allMeatChunkMaterials, allPurifiedCraftMaterials, beastCoreCode, beastCoreName, materialValueMultiplierForLevel, meatChunkCode, meatChunkName, meatChunkQuantity, monsterCraftMaterialCode, monsterCraftMaterialKind, monsterCraftMaterialKinds, monsterCraftMaterialName, monsterDropsMeat, purificationOutputMultiplierForLevel, purifiedCraftMaterialBaseCode, purifiedCraftMaterialCode, purifiedCraftMaterialDisplayName, purifiedCraftMaterialName, purifiedCraftMaterialTierForCode, purifiedCraftMaterialTierForLevel, purifiedCraftMaterialValue, purifiedCraftOutputFor, purifiedMaterialForArmor, resolvedMonsterMaterialDropCode };
