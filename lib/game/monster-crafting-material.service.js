...
const purifiedCraftMaterialCode = (kind) => kindMeta[kind].refinedCode;
const purifiedCraftMaterialName = (kind) => kindMeta[kind].refinedName;
const purifiedCraftMaterialDisplayName = (code) => {
    const kind = (Object.entries(kindMeta).find(([, meta]) => meta.refinedCode === code)?.[0] ?? undefined);
    return kind ? purifiedCraftMaterialName(kind) : undefined;
};
const purifiedMaterialForArmor = (armorType) => {
...
const purifiedCraftOutputFor = (code) => {
    const match = code.match(/^(?:monster_.+|map_.+)_(hair|gel_skin|bone|shell|scale)(?:_l\d+)?$/);
    return match ? purifiedCraftMaterialCode(match[1]) : undefined;
};
...
const allPurifiedCraftMaterials = () => ['hair', 'gel_skin', 'bone', 'shell', 'scale'].map(kind => ({
...
export { allBeastCoreMaterials, allMeatChunkMaterials, allPurifiedCraftMaterials, beastCoreCode, beastCoreName, materialValueMultiplierForLevel, meatChunkCode, meatChunkName, meatChunkQuantity, monsterCraftMaterialCode, monsterCraftMaterialKind, monsterCraftMaterialKinds, monsterCraftMaterialName, monsterDropsMeat, purifiedCraftMaterialCode, purifiedCraftMaterialDisplayName, purifiedCraftMaterialName, purifiedCraftMaterialValue, purifiedCraftOutputFor, purifiedMaterialForArmor, resolvedMonsterMaterialDropCode };
