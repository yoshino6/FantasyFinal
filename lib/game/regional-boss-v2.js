...
class RegionalBossBattle {
    ...
    async settleOrders() {
        ...
        else if (result.effective.length && s.fullFires < 2) {
            s.fullFires++;
            this.boss.hp = Math.min(this.boss.hpMax, this.boss.hp + Math.floor(this.boss.hpMax * .03));
            this.rules.log.push('&满额开炉&炉火为瓦尔克重新锻合伤口。');
        }
    ...
    async bossTurn(victim) {
        ...
        if (s.overloadAt && this.rules.turn >= s.overloadAt) {
            ...
            if (this.boss.bossEffects?.includes('everburning_embers'))
                for (const unit of this.players())
                    this.rules.add(unit, 'burn', 5, 3, this.boss, true);
        ...
        if (slot === 1) {
            ...
            await this.strike(players, 80, '炉渣喷流', '火', true);
            for (const unit of this.players())
                this.rules.add(unit, 'burn', 3, 2, this.boss, true);
        }
        else {
        ...
        if (this.boss.bossEffects?.includes('soul_chain_forging') && slot === 1) {
            for (const unit of this.players().filter(unit => s.obeyed[unit.key] === false && (s.scars[unit.key] ?? 0) > 0 && unit !== inspectionTarget))
                await this.mechanism(unit, .04, '拘魂连锻');
        }
        s.heat += 10;
        await this.checkOverload();
    }
    async gruenTurn(victim) {
        ...
        if (s.warningAt) {
            ...
            if (s.pressure < 30) {
                ...
                if (this.boss.bossEffects?.includes('leyline_recast') && !s.recastUsed) {
                    s.recastUsed = true;
                    this.rules.add(this.boss, 'shield', this.boss.hpMax * .20, 2, this.boss);
                    this.rules.log.push('&地脉复铸&崩落的岩层重新聚合，覆盖住裸露的山心。');
                }
            }
            else {
...
export { RegionalBossBattle, furnaceOrderNames, installRegionalV2Damage, isRegionalV2, newRegionalState, nextFurnaceOrder, obeysFurnaceOrder, pressureChange, readRegionalState, regionalIncomingFactor, regionalOutgoingFactor, regionalStateSummary, regionalV2Codes, settleFurnaceOrders };
