...
import { dynamicWorldRegions, generatedDynamicEncounterTemplates, dynamicNpcDisplayName } from './world-dynamics.content.js';
import { dynamicNpcProfile, dynamicNpcVoiceAnchor } from './dynamic-npc-dialogue.service.js';
import { patrolKindFor, buildPatrolEncounter, patrolObjective, patrolPositionMatches } from './patrol-encounters.js';
import { validWorldSitePoint, pointBelongsToRegion } from './world-site-geometry.js';
import { guildContributionReward } from './skill-access.config.js';

const weatherEffectAudit = {
...
const weatherElementMultiplier = (modifiers, element, channel = 'direct', shelteredCharacterId) => {
    if (!weatherEffectAudit[channel].inheritsElement)
        return 1;
...
const json = (value, fallback) => { try {
    return typeof value === 'string' ? JSON.parse(value) : (value ?? fallback);
}
catch {
    return fallback;
} };
const hash = (text) => { let value = 2166136261; for (const char of text) {
...
const anomalyModifiers = { leaflight_rain: { memberSpeedPct: 3, hint: '叶脉祝福：我方先攻 +3%' }, spore_tide: { memberSpeedPct: -5, hint: '荧孢附着：我方先攻 -5%' }, rootglow_fog: { memberSpeedPct: -8, hint: '根辉迷向：我方先攻 -8%' }, amber_petals: { memberSpeedPct: 4, hint: '花雨轻步：我方先攻 +4%' }, silk_rain: { memberSpeedPct: -3, hint: '河岸湿滑：我方先攻 -3%' }, glass_salt_fog: { memberSpeedPct: -4, hint: '盐雾刺眼：我方先攻 -4%' }, echo_wind: { targetSpeedPct: 5, hint: '回风示警：野怪先攻 +5%' }, ironflower_ash: { memberSpeedPct: -6, hint: '灰雨灼甲：我方先攻 -6%' }, glowspore_tide: { targetSpeedPct: 6, hint: '孢潮躁动：野怪先攻 +6%' }, star_mud_rain: { memberSpeedPct: -5, hint: '星泥滞足：我方先攻 -5%' }, mirror_snow_aurora: { targetSpeedPct: 7, hint: '镜雪折影：野怪先攻 +7%' }, thunderthread_cloud: { targetSpeedPct: 8, hint: '雷丝惊兽：野怪先攻 +8%' }, eclipse_drizzle: { memberSpeedPct: -7, hint: '蚀光失距：我方先攻 -7%' } };
const writeLedger = async (connection, eventType, outcome, sourceKey, payload, actorCharacterId, regionId, correlationId) => {
    const [result] = await connection.execute('INSERT INTO game_event_ledger (event_type,outcome,actor_character_id,region_id,correlation_id,source_key,payload_json) VALUES (?,?,?,?,?,?,?)', [eventType, outcome, actorCharacterId ?? null, regionId ?? null, correlationId ?? null, sourceKey, JSON.stringify(payload)]);
    if ((eventType === 'site.commission' && outcome === 'completed') || (eventType === 'encounter.resolved' && outcome === 'resolved'))
        await (await import('./finance-settlement.js')).recordFinanceWorldEvent(connection, Number(result.insertId), eventType, outcome, actorCharacterId ?? null, regionId ?? null, payload);
    return Number(result.insertId);
};
const isRoutePoint = (value) => {
    if (!value || typeof value !== 'object')
        return false;
    const point = value;
    return typeof point.name === 'string' && Number.isFinite(point.x) && Number.isFinite(point.y) && Number.isFinite(point.z);
};
const pointInRegion = (bounds, index, offset = 0) => {
...
const retiredDynamicNpcCodes = ['courier_morningdew'];
const initializeDynamicWorldSystem = async (pool) => {
    const connection = pool ?? await getPool();
    for (const statement of schema)
        await connection.query(statement);
    const [areas] = await connection.execute('SELECT a.*,r.danger_level FROM map_region_areas a JOIN map_regions r ON r.id=a.region_id');
    ...
    for (const template of templates)
        await connection.execute(`INSERT INTO dynamic_encounter_templates (code,title,region_codes_json,weather_codes_json,min_exposure,weight,definition_json,is_enabled)
    ...
    const legacyWorldlines = [['forest_patrol', '密林巡游'], ['river_trade', '晨露河运'], ['rediron_furnace', '赤铁炉监'], ['marsh_signal', '沼泽星讯'], ['thunder_watch', '雷崖观测'], ['eclipse_memory', '蚀界残忆'], ['canopy_whispers', '冠层低语'], ['rootglow_mystery', '根辉之谜']];
    for (const [code, name] of legacyWorldlines)
        await connection.execute(`INSERT INTO worldline_states (code,stage,state_json) VALUES (?,0,JSON_OBJECT('name',?,'updatedByEncounter',false))
    ...
    for (const region of dynamicWorldRegions) {
      ...
      ON DUPLICATE KEY UPDATE state_json=JSON_SET(state_json,'$.name',?,'$.regionCode',?)`, [region.worldline, `${region.name}世界线`, region.code, `${region.name}世界线`, region.code]);
        const [boundsRows] = await connection.execute('SELECT id,min_x,max_x,min_y,max_y,min_z,max_z FROM map_regions WHERE code=? LIMIT 1', [region.code]);
        ...
        for (const npc of region.npcs) {
            ...
            const [oldPositions] = await connection.execute('SELECT n.region_id,n.pos_x,n.pos_y,n.pos_z,d.route_json FROM map_npcs n JOIN world_dynamic_npc_states d ON d.code=n.code AND d.region_id=n.region_id WHERE n.code=?', [npc.code]);
        ...
        ON DUPLICATE KEY UPDATE name=VALUES(name),route_json=VALUES(route_json),state_json=JSON_SET(state_json,'$.role',?,'$.homeSite',?,'$.homeRegionId',?,'$.homeStatus',?),next_action_at=IF(status IN ('patrolling','responding'),next_action_at,LEAST(next_action_at,DATE_ADD(NOW(),INTERVAL 8 MINUTE)))`, [npc.code, displayName, bounds.id, npc.status, JSON.stringify(route), JSON.stringify({ role: npc.role, homeSite: home.building.code, homeRegionId: bounds.id, homeStatus: npc.status, currentPoint: current.name }), npc.role, home.building.code, bounds.id, npc.status]);
            if (oldPositions.length)
                await connection.execute("UPDATE map_npcs SET name=?,description=? WHERE code=? AND interaction_kind='npc'", [displayName, `${npc.description}\n\n常驻地：${home.building.name}。`, npc.code]);
            else
        ...
        ON DUPLICATE KEY UPDATE name=VALUES(name),description=VALUES(description),interaction_kind='npc'`, [bounds.id, npc.code, displayName, `${npc.description}\n\n常驻地：${home.building.name}。`, current.x, current.y, current.z]);
            ...
            const oldHome = old ? json(old.route_json, [])[0] : undefined;
            if (old && (oldHome && Number(old.region_id) === Number(bounds.id) && Number(old.pos_x) === oldHome.x && Number(old.pos_y) === oldHome.y && Number(old.pos_z) === oldHome.z || !pointBelongsToRegion(areas, Number(old.region_id), { x: Number(old.pos_x), y: Number(old.pos_y), z: Number(old.pos_z) }))) {
                ...
                if (Number(old.pos_x) !== current.x || Number(old.pos_y) !== current.y || Number(old.region_id) !== Number(bounds.id))
                    await writeLedger(connection, 'npc.presence', 'returned', npc.code, { reason: '修复建筑与巡查坐标归属', before: old, position: current }, null, Number(bounds.id));
            }
        }
        if (region.bossCode)
            await connection.execute(`INSERT INTO world_boss_gates (worldline_code,stage_required,boss_code,state) VALUES (?,24,?,'locked')
      ON DUPLICATE KEY UPDATE stage_required=VALUES(stage_required)`, [region.worldline, region.bossCode]);
    }
};
const nextWeather = (current, forecast, roll, abrupt) => {
    if (!forecast.length)
        return 'clear';
    if (abrupt)
        return forecast[roll % forecast.length];
    const index = Math.max(0, forecast.indexOf(current));
    return forecast[clamp(index + [-1, 0, 1][roll % 3], 0, forecast.length - 1)];
};
const weatherModifiersFor = (weatherCode, anomalyCode, intensity) => {
    const base = { memberSpeedPct: 0, targetSpeedPct: 0, hint: '天气平稳：无额外先攻修正', elementBonuses: {} };
    if (weatherCode === 'mist' || weatherCode === 'frost_fog') {
        base.memberSpeedPct = -3 * intensity;
        base.hint = `能见度受限：我方先攻 ${base.memberSpeedPct}%`;
    }
    if (weatherCode === 'rain') {
        base.memberSpeedPct = -2 * intensity;
        base.hint = `地面湿滑：我方先攻 ${base.memberSpeedPct}%`;
        base.elementBonuses = { 水: 8, 火: -6 };
    }
    if (weatherCode === 'wind') {
        base.targetSpeedPct = 2 * intensity;
        base.hint = `风势扰动：野怪先攻 +${base.targetSpeedPct}%`;
    }
    if (weatherCode === 'storm') {
        base.memberSpeedPct = -4 * intensity;
        base.targetSpeedPct = 2 * intensity;
        base.hint = `雷暴压境：我方先攻 ${base.memberSpeedPct}%｜野怪先攻 +${base.targetSpeedPct}%`;
        base.elementBonuses = { 雷: 10, 风: 4 };
    }
    if (weatherCode === 'snow' || weatherCode === 'ash_rain') {
        base.memberSpeedPct = -3 * intensity;
        base.hint = `地表阻滞：我方先攻 ${base.memberSpeedPct}%`;
    }
    if (anomalyCode)
        Object.assign(base, anomalyModifiers[anomalyCode] ?? {});
    return base;
};
const playerContext = async (connection, qqUserId, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.id,c.current_region_id AS region_id,r.code AS region_code,r.name AS region_name,c.pos_x,c.pos_y,c.pos_z
    ...
    if (!rows[0])
        throw new Error('请先完成冒险者登记。');
    return rows[0];
};
const weatherStateForRegion = async (connection, regionId) => {
    const [regions] = await connection.execute('SELECT code,name FROM map_regions WHERE id=? LIMIT 1', [regionId]);
    const region = regions[0];
    if (!region)
        throw new Error('区域天气数据异常：找不到地图。');
    ...
    const [rows] = await connection.execute(`SELECT p.climate_code,p.forecast_json,p.anomaly_pool_json,s.weather_code,s.intensity,s.phase,s.anomaly_code,s.revision,s.transition_due_at,s.cause_code
    ...
    if (new Date(row.transition_due_at).getTime() <= Date.now()) {
        const forecast = json(row.forecast_json, fallback.forecast);
        const anomalies = json(row.anomaly_pool_json, fallback.anomalies);
        const seed = hash(`${regionId}:${row.revision}:${row.weather_code}`);
        const abrupt = seed % 1000 < 12;
        const next = nextWeather(row.weather_code, forecast, seed >>> 4, abrupt);
        const anomaly = anomalies.length && seed % 100 < 7 ? anomalies[(seed >>> 9) % anomalies.length] : null;
        const intensity = clamp(Number(row.intensity) + ((seed >>> 15) % 3 - 1), 1, 3);
        const phase = abrupt ? 'forming' : next === row.weather_code ? 'steady' : intensity > Number(row.intensity) ? 'forming' : 'easing';
        const revision = Number(row.revision) + 1;
        const cause = abrupt ? 'rare_abrupt_front' : anomaly ? 'regional_anomaly' : 'gradual_front';
        const minutes = 70 + seed % 51;
...
const weatherForPlayer = async (qqUserId) => withTransaction(async (connection) => {
    ...
    const detail = weatherText[weather.anomalyCode ?? weather.weatherCode] ?? weatherText[weather.weatherCode] ?? weatherText.clear;
...
const siteCapabilityFor = (siteType) => {
    if (['courier_post', 'ferry', 'rest_stop', 'wayfinder', 'civic_hall'].includes(siteType))
        return 'commission';
    if (['observatory', 'lighthouse', 'watchpost'].includes(siteType))
        return 'forecast';
    if (['workshop', 'mining_station', 'industrial', 'laboratory', 'garden'].includes(siteType))
        return 'exchange';
    if (['shelter', 'inn', 'ranch'].includes(siteType))
        return 'shelter';
    return 'clues';
};
const siteActionLabel = { commission: '询问区域委托', forecast: '查看天气预警', exchange: '交换区域材料', clues: '回看已发现线索', shelter: '获得天气庇护' };
const siteDailyLimit = { commission: 1, forecast: 3, exchange: 2, clues: 4, shelter: 1 };
const weatherPhaseLabel = { forming: '天气渐强', steady: '天气平稳', easing: '天气渐缓' };
const regionMaterialCode = { world_tree: 'root_heart', dark_forest: 'living_wood', dark_forest_deep: 'living_wood', worldtree_meadow: 'root_heart', morningdew_riverbank: 'river_shell', gravelwind_shore: 'tide_shell', ridge_foothills: 'ridge_core', rediron_pass: 'fire_crystal', mistalgae_marsh: 'marsh_heart', fallenstar_swamp: 'star_mud_core', frostcrown_plateau: 'frost_crystal', thundercliff: 'thunder_core', eclipse_ruins: 'eclipse_core' };
const currentWorldSite = async (connection, player, siteCode, lock = false) => {
    const [rows] = await connection.execute(`SELECT s.code,s.name,n.description,s.site_type,s.state_json,r.name AS region_name,JSON_UNQUOTE(JSON_EXTRACT(s.state_json,'$.relatedWorldline')) AS worldline_code,w.stage AS worldline_stage,r.danger_level AS region_danger
    ...
    if (!rows[0])
        throw new Error('请先来到该特色建筑门前。');
    return rows[0];
};
const consumeSiteDailyUse = async (connection, characterId, siteCode, action) => {
    const [rows] = await connection.execute('SELECT usage_count FROM player_world_site_daily WHERE character_id=? AND site_code=? AND action_code=? AND usage_date=CURDATE() FOR UPDATE', [characterId, siteCode, action]);
    const used = Number(rows[0]?.usage_count ?? 0);
    const limit = siteDailyLimit[action];
    if (used >= limit)
        throw new Error(`该站点的「${siteActionLabel[action]}」今日已达 ${limit} 次上限。`);
...
const siteAttendantAtFrontDesk = async (connection, player, siteCode) => {
    const [rows] = await connection.execute(`SELECT d.code,d.name FROM world_dynamic_npc_states d
...
const worldSiteView = async (qqUserId, siteCode) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    ...
    const state = json(site.state_json, {});
    const access = state.access === 'private' ? 'private' : 'public';
...
const worldSiteKnock = async (qqUserId, siteCode) => {
    const site = await worldSiteView(qqUserId, siteCode);
    if (site.access === 'public' || site.attendant)
        return { answered: true, site };
    return { answered: false, site, text: `你轻叩${site.name}的门。门后没有回应，常驻域民似乎外出未归。` };
};
const worldSiteForAttendant = async (qqUserId, npcCode) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const [rows] = await connection.execute(`SELECT d.code,d.name,s.code AS site_code,s.site_type FROM world_dynamic_npc_states d
...
const commissionSiteForAttendant = async (qqUserId, npcCode) => {
    const site = await worldSiteForAttendant(qqUserId, npcCode);
    return site?.capability === 'commission' ? site : null;
};
const acceptWorldSiteCommissionFromAttendant = async (qqUserId, siteCode, npcCode) => {
    const attendant = await commissionSiteForAttendant(qqUserId, npcCode);
    if (!attendant || attendant.siteCode !== siteCode)
        throw new Error('请先在该站点前台与常驻域民交谈。');
...
const commissionPlanFor = (seed, context) => commissionPlans[seed % commissionPlans.length](context);
const worldlineCommissionDailyLimit = 6;
const commissionBackdropFor = (regionCode, regionName) => ({
...
const commissionRewardFor = (dangerLevel, worldlineStage, plan) => 16 + Math.max(0, dangerLevel) * 4 + Math.min(8, Math.max(0, worldlineStage)) * 2 + plan.rewardBonus;
const npcAffinityFor = async (connection, characterId, npcCode) => {
    const [rows] = await connection.execute('SELECT affinity FROM player_npc_affinity WHERE character_id=? AND npc_code=? LIMIT 1', [characterId, npcCode]);
    return Number(rows[0]?.affinity ?? 0);
};
const speakerReason = (reason, issuerName) => reason.replaceAll(issuerName, '我')
    .replaceAll('；他', '；我').replaceAll('；她', '；我').replaceAll('。他', '。我').replaceAll('。她', '。我');
const conciseCommissionObjective = (targetName, handoff) => {
...
const commissionTargetFor = async (connection, regionId, sourceSiteCode, seed) => {
    const [rows] = await connection.execute(`SELECT s.code,s.name,n.pos_x,n.pos_y,n.pos_z FROM world_site_states s JOIN map_npcs n ON n.code=s.code AND n.region_id=s.region_id AND n.interaction_kind='building'
    WHERE s.region_id=? AND s.code<>? ORDER BY s.code`, [regionId, sourceSiteCode]);
    if (!rows[0])
        throw new Error('该区域暂时没有可安排的巡检站点。');
    return rows[seed % rows.length];
};
const playerWorldSiteCommissions = async (qqUserId) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId);
    const [rows] = await connection.execute(`SELECT c.id,c.source_site_code,c.target_site_code,c.title,c.objective_text,c.reward_copper,c.status,t.name AS target_name,r.name AS region_name,n.pos_x,n.pos_y,n.pos_z
...
const completeWorldSiteCommissionsAtSite = async (qqUserId, siteCode) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const site = await currentWorldSite(connection, player, siteCode, true);
    const [rows] = await connection.execute(`SELECT c.id,c.title,JSON_UNQUOTE(JSON_EXTRACT(s.state_json,'$.relatedWorldline')) AS worldline_code
    ...
    if (!rows.length)
        return [];
    if (!await siteAttendantAtFrontDesk(connection, player, site.code))
        return ['前台域民暂时外出。请在任务栏选择“提交交接”留下回站通知，待其回来后再次提交或进入建筑即可交接。'];
    ...
    for (const row of rows) {
        ...
        let regionalDailyLimitReached = false;
        if (row.worldline_code) {
            const [daily] = await connection.execute(`INSERT IGNORE INTO player_worldline_daily_contributions (character_id,worldline_code,contribution_date,commission_id)
        VALUES (?,?,CURDATE(),?)`, [player.id, row.worldline_code, row.id]);
            if (Number(daily.affectedRows)) {
                ...
                const [progressRows] = await connection.execute('SELECT contribution_count FROM worldline_daily_commission_progress WHERE worldline_code=? AND progress_date=CURDATE() FOR UPDATE', [row.worldline_code]);
                if (Number(progressRows[0]?.contribution_count ?? 0) < worldlineCommissionDailyLimit) {
                    ...
                    regionalDailyLimitReached = true;
            }
        }
...
const claimWorldSiteCommission = async (qqUserId, commissionId) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const [rows] = await connection.execute('SELECT title,reward_copper,status FROM player_world_site_commissions WHERE id=? AND character_id=? FOR UPDATE', [commissionId, player.id]);
    const commission = rows[0];
    if (!commission)
        throw new Error('未找到这份站点委托。');
    if (commission.status !== 'completed')
        throw new Error('请先抵达委托指定的站点完成巡检。');
...
const submitWorldSiteCommission = async (qqUserId, commissionId) => {
    const ready = await withTransaction(async (connection) => {
        const player = await playerContext(connection, qqUserId, true);
        const [rows] = await connection.execute('SELECT target_site_code,status FROM player_world_site_commissions WHERE id=? AND character_id=? FOR UPDATE', [commissionId, player.id]);
        const row = rows[0];
        if (!row)
            throw new Error('未找到你的这份委托。');
        if (row.status !== 'accepted')
            return { code: row.target_site_code, ready: false, text: row.status === 'completed' ? '交接已经完成，请在任务栏领取报酬。' : '这份委托已经结清。' };
        const site = await currentWorldSite(connection, player, row.target_site_code, true);
        if (await siteAttendantAtFrontDesk(connection, player, site.code))
            return { code: site.code, ready: true, text: '' };
...
const useWorldSite = async (qqUserId, siteCode, action) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const site = await currentWorldSite(connection, player, siteCode, true);
    const capability = siteCapabilityFor(site.site_type);
    if (capability !== action)
        throw new Error(`此站点提供「${siteActionLabel[capability]}」，不能执行该操作。`);
    const attendant = await siteAttendantAtFrontDesk(connection, player, site.code);
    if (!attendant)
        throw new Error('前台域民暂时外出，待其归来后才能办理这项事务。');
    const usage = await consumeSiteDailyUse(connection, Number(player.id), site.code, action);
    const weather = await weatherStateForRegion(connection, Number(player.region_id));
    let text = '';
    let reward = 0;
    let materialName = '';
    let clues = [];
    let shelterUntil = null;
    let commission = null;
    if (action === 'commission') {
        const dayKey = new Date().toISOString().slice(0, 10);
        const seed = hash(`${player.id}:${site.code}:${dayKey}`);
        ...
        const [insert] = await connection.execute('INSERT INTO player_world_site_commissions (character_id,source_site_code,target_site_code,title,objective_text,reward_copper) VALUES (?,?,?,?,?,?)', [player.id, site.code, target.code, title, objective, reward]);
        ...
        const materialCode = regionMaterialCode[player.region_code];
        if (!materialCode)
            throw new Error('这个地区暂未登记可供交换的区域材料。');
        const [items] = await connection.execute('SELECT id,name FROM item_definitions WHERE code=? LIMIT 1', [materialCode]);
        const item = items[0];
        if (!item)
            throw new Error('区域材料定义缺失。');
        const [inventoryRows] = await connection.execute('SELECT quantity FROM player_inventory WHERE character_id=? AND item_id=? FOR UPDATE', [player.id, item.id]);
        if (Number(inventoryRows[0]?.quantity ?? 0) < 3)
            throw new Error(`交换需要 3 个${item.name}。`);
        await connection.execute('UPDATE player_inventory SET quantity=quantity-3 WHERE character_id=? AND item_id=?', [player.id, item.id]);
        await connection.execute('DELETE FROM player_inventory WHERE character_id=? AND item_id=? AND quantity<=0', [player.id, item.id]);
        reward = 30 + Math.min(20, Number(site.worldline_stage ?? 0));
        materialName = item.name;
        await connection.execute('UPDATE characters SET copper_coins=copper_coins+? WHERE id=?', [reward, player.id]);
        recordAchievement(connection, Number(player.id), [{ metric: 'ACH_K09', value: reward, life: true }]);
        text = `你交付了区域材料，站点按当日行情结算铜币 ×${reward}。`;
    }
    else if (action === 'clues') {
        const [rows] = await connection.execute('SELECT flag_code,value_int FROM player_destiny_flags WHERE character_id=? AND (flag_code LIKE ? OR flag_code LIKE ?) ORDER BY updated_at DESC LIMIT 8', [player.id, `${player.region_code}_%`, `${site.worldline_code ?? ''}%`]);
        clues = rows.map((row, index) => `区域线索 ${index + 1}（已记录 ${Number(row.value_int)} 次）`);
        text = clues.length ? `档案阁为你调出已发现线索：${clues.join('、')}。` : '档案阁暂未找到你在本区域留下的可回看线索；完成奇遇后再来试试。';
    }
    else {
      ...
      ON DUPLICATE KEY UPDATE region_id=VALUES(region_id),site_code=VALUES(site_code),expires_at=VALUES(expires_at)`, [player.id, player.region_id, site.code]);
        shelterUntil = new Date(Date.now() + 35 * 60_000);
        text = '你在站点补充了防护物资。接下来 35 分钟内，于本区域开启的战斗会获得天气庇护。';
    }
    ...
    if (action !== 'commission')
        await recordCharacterOperation(connection, { characterId: Number(player.id), kind: `world_site.${action}`, source: { system: 'world_site_action', id: randomUUID(), step: 'settled' }, outcome: siteActionLabel[action], summary: `在${site.name}${siteActionLabel[action]}`, detail: { siteCode: site.code, siteType: site.site_type, usage, rewardCopper: reward, materialName, clues, shelterUntil: shelterUntil?.toISOString() ?? null } });
    return { siteName: site.name, action, usage, text, reward, materialName, clues, shelterUntil, commission };
});
const snapshotCombatEnvironment = async (connection, sessionId, regionId, characterIds = []) => {
    const [existing] = await connection.execute('SELECT region_id,weather_code,intensity,anomaly_code,modifiers_json FROM combat_environment_snapshots WHERE session_id=? LIMIT 1', [sessionId]);
    if (existing[0])
        return { regionId: Number(existing[0].region_id), weatherCode: existing[0].weather_code, intensity: Number(existing[0].intensity), anomalyCode: existing[0].anomaly_code, modifiers: json(existing[0].modifiers_json, weatherModifiersFor(existing[0].weather_code, existing[0].anomaly_code, Number(existing[0].intensity))) };
    const weather = await weatherStateForRegion(connection, regionId);
    const modifiers = weatherModifiersFor(weather.weatherCode, weather.anomalyCode, weather.intensity);
    if (characterIds.length) {
        const placeholders = characterIds.map(() => '?').join(',');
        const [shelters] = await connection.execute(`SELECT character_id FROM player_weather_shelters WHERE region_id=? AND expires_at>NOW() AND character_id IN (${placeholders})`, [regionId, ...characterIds]);
        if (shelters.length)
            modifiers.shelteredCharacterIds = shelters.map(row => Number(row.character_id));
    }
...
const combatEnvironmentFor = async (connection, sessionId) => {
    const [rows] = await connection.execute('SELECT region_id,weather_code,intensity,anomaly_code,modifiers_json FROM combat_environment_snapshots WHERE session_id=? LIMIT 1', [sessionId]);
    const row = rows[0];
    if (!row)
        return null;
    const modifiers = json(row.modifiers_json, weatherModifiersFor(row.weather_code, row.anomaly_code, Number(row.intensity)));
...
const recordExplorationMovement = async (qqUserId) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const cellX = Math.floor(Number(player.pos_x) / 8);
    const cellY = Math.floor(Number(player.pos_y) / 8);
    const cellZ = Number(player.pos_z);
    const [rows] = await connection.execute('SELECT window_started_at,effective_exposure,last_region_id,last_cell_x,last_cell_y,last_cell_z FROM player_exploration_budgets WHERE character_id=? FOR UPDATE', [player.id]);
    const row = rows[0];
    const expired = !row || Date.now() - new Date(row.window_started_at).getTime() >= 10 * 60_000;
    const changed = expired || !row || Number(row.last_region_id) !== Number(player.region_id) || Number(row.last_cell_x) !== cellX || Number(row.last_cell_y) !== cellY || Number(row.last_cell_z) !== cellZ;
    const exposure = changed ? Math.min(12, (expired ? 0 : Number(row?.effective_exposure ?? 0)) + 1) : Number(row?.effective_exposure ?? 0);
    if (!row)
        await connection.execute('INSERT INTO player_exploration_budgets (character_id,effective_exposure,last_region_id,last_cell_x,last_cell_y,last_cell_z) VALUES (?,?,?,?,?,?)', [player.id, exposure, player.region_id, cellX, cellY, cellZ]);
    else if (changed)
        await connection.execute('UPDATE player_exploration_budgets SET window_started_at=IF(?,NOW(),window_started_at),effective_exposure=?,last_region_id=?,last_cell_x=?,last_cell_y=?,last_cell_z=? WHERE character_id=?', [expired ? 1 : 0, exposure, player.region_id, cellX, cellY, cellZ, player.id]);
    if (changed)
        await writeLedger(connection, 'exploration.exposure', 'recorded', 'grid_crossing', { cellX, cellY, cellZ, exposure }, Number(player.id), Number(player.region_id));
    return { exposure, changed };
});
const activeEncounterFor = async (connection, characterId, lock = false) => {
    const [expired] = await connection.execute("SELECT id,region_id,template_code,node_code FROM player_encounter_instances WHERE character_id=? AND status='active' AND expires_at<=NOW() FOR UPDATE", [characterId]);
    ...
    for (const row of expired)
        await writeLedger(connection, 'encounter.expired', 'expired', row.template_code, { node: row.node_code }, characterId, Number(row.region_id), row.id);
    const [rows] = await connection.execute(`SELECT i.id,i.region_id,i.template_code,i.node_code,i.context_json,i.expires_at,t.title,t.definition_json FROM player_encounter_instances i JOIN dynamic_encounter_templates t ON t.code=i.template_code WHERE i.character_id=? AND i.status='active' ORDER BY i.opened_at DESC LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [characterId]);
    return rows[0] ?? null;
};
const definitionForInstance = (row) => {
    const context = json(row.context_json, {});
    return context.definitionSnapshot ?? json(row.definition_json, { opening: '一段未能辨明的奇遇正在消散。', choices: [] });
};
const renderEncounter = (row, regionName) => {
    const definition = definitionForInstance(row);
    const context = json(row.context_json, {});
    const node = definition.nodes?.[row.node_code] ?? { text: definition.opening ?? '一段未能辨明的奇遇正在消散。', choices: definition.choices ?? [] };
...
const recordBudgetProbe = async (connection, player) => {
    const cellX = Math.floor(Number(player.pos_x) / 8);
    const cellY = Math.floor(Number(player.pos_y) / 8);
    const cellZ = Number(player.pos_z);
    const [rows] = await connection.execute('SELECT last_region_id,last_cell_x,last_cell_y,last_cell_z FROM player_exploration_budgets WHERE character_id=? FOR UPDATE', [player.id]);
    const row = rows[0];
    ...
    if (!row)
        await connection.execute('INSERT INTO player_exploration_budgets (character_id,effective_exposure,last_region_id,last_cell_x,last_cell_y,last_cell_z) VALUES (?,?,?,?,?,?)', [player.id, 1, player.region_id, cellX, cellY, cellZ]);
    else if (!sameCell)
        await connection.execute('UPDATE player_exploration_budgets SET effective_exposure=LEAST(12,effective_exposure+1),last_region_id=?,last_cell_x=?,last_cell_y=?,last_cell_z=? WHERE character_id=?', [player.region_id, cellX, cellY, cellZ, player.id]);
};
const currentDynamicEncounter = async (qqUserId) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const active = await activeEncounterFor(connection, Number(player.id), true);
    return active ? renderEncounter(active, player.region_name) : null;
});
const patrolAtPlayer = async (connection, player) => {
    const [rows] = await connection.execute(`SELECT d.code,d.action_revision,d.status,d.state_json,d.next_action_at,r.danger_level
...
const patrolEncounterPreview = async (qqUserId, npcCode) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const npc = (await patrolAtPlayer(connection, player)).find(row => row.code === npcCode);
    const profile = dynamicNpcProfile(npcCode);
    if (!npc || !profile)
        return null;
    if (npc.status === 'responding')
        return { revision: Number(npc.action_revision), text: `${profile.displayName}正在处理附近的公共奇遇。你可以查看现场，再决定如何协助。`, responding: true };
    const state = json(npc.state_json, {});
...
const startPatrolEncounter = async (qqUserId, npcCode, revision) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const active = await activeEncounterFor(connection, Number(player.id), true);
    if (active) {
        const patrol = json(active.context_json, {}).patrol;
        if (patrol?.npcCode === npcCode && patrol.revision === revision)
            return renderEncounter(active, player.region_name);
        throw new Error('请先完成当前奇遇；可在“/奇遇”查看进展。');
    }
    const npc = (await patrolAtPlayer(connection, player)).find(row => row.code === npcCode);
    const profile = dynamicNpcProfile(npcCode);
    if (!npc || !profile || npc.status !== 'patrolling' || Number(npc.action_revision) !== revision)
        throw new Error('这段巡查已经改变，请到域民身边重新询问行程。');
    const [previous] = await connection.execute('SELECT 1 FROM player_patrol_encounters WHERE character_id=? AND npc_code=? AND (visit_revision=? OR accepted_date=CURDATE()) LIMIT 1', [player.id, npcCode, revision]);
    if (previous.length)
        throw new Error('你已经参与过这位域民今日的巡查，明日再来听新的见闻吧。');
    const [daily] = await connection.execute('SELECT COUNT(*) AS total,COALESCE(SUM(created_at>DATE_SUB(NOW(),INTERVAL 20 MINUTE)),0) AS recent FROM player_patrol_encounters WHERE character_id=? AND (accepted_date=CURDATE() OR created_at>DATE_SUB(NOW(),INTERVAL 20 MINUTE))', [player.id]);
    if (Number(daily[0]?.total) >= 3)
        throw new Error('你今日已经接手三次巡查，先消化这些见闻吧。');
    if (Number(daily[0]?.recent))
        throw new Error('刚接手的巡查还需要整理，距上次参与满二十分钟后再来。');
    const [targets] = await connection.execute(`SELECT s.name,n.pos_x AS x,n.pos_y AS y,n.pos_z AS z FROM world_site_states s JOIN map_npcs n ON n.code=s.code AND n.region_id=s.region_id AND n.interaction_kind='building'
    ...
    if (!target)
        throw new Error('附近暂时没有适合交接的公共落脚处。');
    const [parties] = await connection.execute('SELECT party_id FROM party_members WHERE character_id=? LIMIT 1', [player.id]);
    const teamKey = parties[0] ? `party:${parties[0].party_id}` : `solo:${player.id}`;
    const [claims] = await connection.execute('SELECT COALESCE(SUM(rewarded),0) AS total,COALESCE(SUM(rewarded=1 AND team_key=?),0) AS team_count FROM player_patrol_encounters WHERE npc_code=? AND visit_revision=?', [teamKey, npcCode, revision]);
    ...
    const [worldlines] = await connection.execute('SELECT stage FROM worldline_states WHERE code=?', [region.worldline]);
    const weather = await weatherStateForRegion(connection, Number(player.region_id));
    ...
    const patrol = { npcCode, revision, kind, rewarded, origin: { regionId: Number(player.region_id), name: `${profile.name}的巡查现场`, x: Number(player.pos_x), y: Number(player.pos_y), z: Number(player.pos_z) }, target: { regionId: Number(player.region_id), name: target.name, x: Number(target.x), y: Number(target.y), z: Number(target.z) } };
    const templateCode = `patrol_${npcCode}_${kind}`;
    const id = randomUUID();
    ...
    return renderEncounter({ id, region_id: Number(player.region_id), template_code: templateCode, node_code: 'opening', context_json: { patrol, regionName: player.region_name, titleSnapshot: content.title, definitionSnapshot: content.definition }, expires_at: new Date(Date.now() + 30 * 60_000), title: content.title, definition_json: content.definition }, player.region_name);
});
const offerDynamicEncounter = async (qqUserId) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const active = await activeEncounterFor(connection, Number(player.id), true);
    if (active)
        return renderEncounter(active, player.region_name);
    await recordBudgetProbe(connection, player);
    const [budgetRows] = await connection.execute('SELECT effective_exposure,trigger_count,cooldown_until FROM player_exploration_budgets WHERE character_id=? FOR UPDATE', [player.id]);
    const budget = budgetRows[0];
    if (budget.cooldown_until && new Date(budget.cooldown_until).getTime() > Date.now())
        return null;
    const weather = await weatherStateForRegion(connection, Number(player.region_id));
    const visibleWeather = weather.anomalyCode ?? weather.weatherCode;
    const [templateRows] = await connection.execute('SELECT code,title,region_codes_json,weather_codes_json,min_exposure,weight,definition_json FROM dynamic_encounter_templates WHERE is_enabled=1');
    const candidates = templateRows.map(row => ({ code: row.code, title: row.title, regions: json(row.region_codes_json, []), weather: json(row.weather_codes_json, []), minExposure: Number(row.min_exposure), weight: Math.max(1, Number(row.weight)), definition: json(row.definition_json, { choices: [] }) }))
    ...
    if (!candidates.length || hash(`${player.id}:${budget.trigger_count}:${weather.revision}`) % 100 >= 38)
        return null;
    let pick = hash(`${player.id}:${budget.trigger_count}:template`) % candidates.reduce((sum, template) => sum + template.weight, 0);
    const template = candidates.find(item => (pick -= item.weight) < 0) ?? candidates[candidates.length - 1];
    const id = randomUUID();
    const weatherName = (weatherText[visibleWeather] ?? weatherText.clear).name;
    ...
    if (definition.publicScene) {
        ...
        const [scenes] = await connection.execute(`SELECT id FROM world_scene_instances WHERE template_code=? AND region_id=? AND cell_x=? AND cell_y=? AND cell_z=? AND status='active' AND expires_at>NOW() LIMIT 1 FOR UPDATE`, [template.code, player.region_id, cellX, cellY, cellZ]);
        ...
        VALUES (?,?,?,?,?,?,?,DATE_ADD(NOW(),INTERVAL 15 MINUTE),?)`, [sceneId, template.code, player.region_id, cellX, cellY, cellZ, player.id, JSON.stringify({ weatherName, weatherRevision: weather.revision, definitionSnapshot: definition })]);
            ...
            for (const npcCode of definition.relatedNpcCodes ?? [])
                await connection.execute('UPDATE world_dynamic_npc_states SET next_action_at=NOW() WHERE code=?', [npcCode]);
    ...
    const [created] = await connection.execute('SELECT i.id,i.region_id,i.template_code,i.node_code,i.context_json,i.expires_at,t.title,t.definition_json FROM player_encounter_instances i JOIN dynamic_encounter_templates t ON t.code=i.template_code WHERE i.id=? LIMIT 1', [id]);
    return renderEncounter(created[0], player.region_name);
});
const applyWorldlineEffects = async (connection, worldline, actorCharacterId, regionId, encounterId) => {
    const [worldlines] = await connection.execute('SELECT stage FROM worldline_states WHERE code=? FOR UPDATE', [worldline]);
    const stage = Number(worldlines[0]?.stage ?? 0);
    const siteState = stage >= 12 ? 'active' : stage >= 5 ? 'stirring' : 'dormant';
    const [sites] = await connection.execute(`UPDATE world_site_states SET state_json=JSON_SET(state_json,'$.state',?,'$.worldlineStage',?)
    WHERE JSON_UNQUOTE(JSON_EXTRACT(state_json,'$.relatedWorldline'))=?`, [siteState, stage, worldline]);
    const [gates] = await connection.execute(`UPDATE world_boss_gates SET state='ready',activated_at=COALESCE(activated_at,NOW())
    WHERE worldline_code=? AND state='locked' AND stage_required<=?`, [worldline, stage]);
    if (Number(sites.affectedRows) || Number(gates.affectedRows))
        await writeLedger(connection, 'worldline.updated', Number(gates.affectedRows) ? 'boss_ready' : 'site_updated', worldline, { stage, siteState, sites: Number(sites.affectedRows), bossGates: Number(gates.affectedRows) }, actorCharacterId, regionId, encounterId);
    return { stage, siteState, bossReady: Number(gates.affectedRows) > 0 };
};
const grantHiddenReward = async (connection, characterId, encounterId, choice) => {
    if (!choice.rewardCode || !choice.rewardChance || hash(`${encounterId}:${choice.code}:reward`) % 10_000 >= Math.floor(choice.rewardChance * 10_000))
        return null;
    const [inserted] = await connection.execute('INSERT IGNORE INTO player_encounter_rewards (character_id,reward_code,encounter_id) VALUES (?,?,?)', [characterId, choice.rewardCode, encounterId]);
    const [items] = await connection.execute('SELECT id,name FROM item_definitions WHERE code=? LIMIT 1', [choice.rewardCode]);
    const item = items[0];
    if (!item)
        return null;
    if (!Number(inserted.affectedRows))
        return { name: item.name, duplicate: true };
...
const settleSceneWitnesses = async (connection, sceneId, copper, regionId, templateCode) => {
    const [sceneRows] = await connection.execute('SELECT discoverer_character_id,status FROM world_scene_instances WHERE id=? FOR UPDATE', [sceneId]);
    ...
    if (new Set(assistants.map(p => p.contribution_code)).size >= 2)
        for (const p of assistants)
            recordAchievement(connection, Number(p.character_id), [{ metric: 'ACH_G12', cooperationKey: 'scene:' + sceneId }], 'scene-cooperate:' + sceneId);
    if (assistants.length && participants.some(p => p.role === 'witness'))
        for (const id of [Number(scene.discoverer_character_id), ...participants.map(p => Number(p.character_id))])
            recordAchievement(connection, id, [{ metric: 'ACH_G25', cooperationKey: 'scene:' + sceneId }], 'scene-roles:' + sceneId);
    ...
    for (const participant of participants) {
        ...
        if (participant.role === 'assistant' && participant.contribution_code)
            recordAchievement(connection, Number(participant.character_id), [{ metric: 'ACH_D24', cooperationKey: 'scene:' + sceneId }, { metric: 'ACH_D25', distinct: templateCode }], 'scene-assist:' + sceneId);
        if (participant.contribution_code)
            await connection.execute('UPDATE world_scene_contributions SET settled_at=NOW() WHERE scene_id=? AND character_id=? AND settled_at IS NULL', [sceneId, participant.character_id]);
...
const nearbyDynamicScenes = async (qqUserId) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const cellX = Math.floor(Number(player.pos_x) / 8);
    const cellY = Math.floor(Number(player.pos_y) / 8);
    const cellZ = Number(player.pos_z);
    await connection.execute("UPDATE world_scene_instances SET status='expired' WHERE status='active' AND expires_at<=NOW()");
    const [rows] = await connection.execute(`SELECT s.id,t.title,c.name AS discoverer_name,s.expires_at,COUNT(p.character_id) AS participant_count,s.cell_x,s.cell_y,s.cell_z
...
const joinDynamicScene = async (qqUserId, sceneId) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const cellX = Math.floor(Number(player.pos_x) / 8);
    const cellY = Math.floor(Number(player.pos_y) / 8);
    const cellZ = Number(player.pos_z);
    ...
    const [scenes] = await connection.execute('SELECT template_code,region_id,cell_x,cell_y,cell_z,status,expires_at FROM world_scene_instances WHERE id=? FOR UPDATE', [sceneId]);
    const scene = scenes[0];
    if (!scene || scene.status !== 'active' || new Date(scene.expires_at).getTime() <= Date.now())
        throw new Error('这个公共奇遇已经结束。');
    if (Number(scene.region_id) !== Number(player.region_id) || Math.abs(Number(scene.cell_x) - cellX) > 1 || Math.abs(Number(scene.cell_y) - cellY) > 1 || Number(scene.cell_z) !== cellZ)
        throw new Error('请先移动到奇遇附近一格以内，再参与。');
    const [existing] = await connection.execute('SELECT role FROM world_scene_participants WHERE scene_id=? AND character_id=? LIMIT 1 FOR UPDATE', [sceneId, player.id]);
    if (existing[0])
        return { role: existing[0].role, alreadyJoined: true };
    const [countRows] = await connection.execute('SELECT COUNT(*) AS total FROM world_scene_participants WHERE scene_id=? FOR UPDATE', [sceneId]);
    const [partyRows] = await connection.execute('SELECT party_id FROM party_members WHERE character_id=? LIMIT 1', [player.id]);
    const partyId = partyRows[0]?.party_id;
    const [partyWitnesses] = partyId ? await connection.execute(`SELECT 1 FROM world_scene_participants p JOIN party_members pm ON pm.character_id=p.character_id WHERE p.scene_id=? AND pm.party_id=? AND p.role IN ('discoverer','witness','assistant') LIMIT 1`, [sceneId, partyId]) : [[]];
...
const contributionText = { escort: '护送', decode: '破译', supply: '物资支援' };
const contributeDynamicScene = async (qqUserId, sceneId, contribution) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const cellX = Math.floor(Number(player.pos_x) / 8);
    const cellY = Math.floor(Number(player.pos_y) / 8);
    const cellZ = Number(player.pos_z);
    await connection.execute("UPDATE world_scene_instances SET status='expired' WHERE id=? AND status='active' AND expires_at<=NOW()", [sceneId]);
    const [scenes] = await connection.execute('SELECT template_code,region_id,cell_x,cell_y,cell_z,status FROM world_scene_instances WHERE id=? FOR UPDATE', [sceneId]);
    const scene = scenes[0];
    if (!scene || scene.status !== 'active' || Number(scene.region_id) !== Number(player.region_id) || Math.abs(Number(scene.cell_x) - cellX) > 1 || Math.abs(Number(scene.cell_y) - cellY) > 1 || Number(scene.cell_z) !== cellZ)
        throw new Error('请作为现场参与者，在公共奇遇一格范围内提供协作。');
    const [participants] = await connection.execute('SELECT role FROM world_scene_participants WHERE scene_id=? AND character_id=? FOR UPDATE', [sceneId, player.id]);
    const participant = participants[0];
    if (!participant || participant.role === 'observer')
        throw new Error('请先以见证者身份加入现场；旁观者不能贡献或领取协作奖励。');
    const [own] = await connection.execute('SELECT contribution_code FROM world_scene_contributions WHERE scene_id=? AND character_id=? FOR UPDATE', [sceneId, player.id]);
    if (own[0])
        return { contribution: own[0].contribution_code, alreadyContributed: true, text: `你已完成${contributionText[own[0].contribution_code]}贡献，结算会保持幂等。` };
    const [cooldowns] = await connection.execute('SELECT cooldown_until FROM player_scene_contribution_cooldowns WHERE character_id=? AND contribution_code=? FOR UPDATE', [player.id, contribution]);
    if (cooldowns[0] && new Date(cooldowns[0].cooldown_until).getTime() > Date.now())
        throw new Error(`「${contributionText[contribution]}」仍在角色冷却中，不能连续堆叠贡献。`);
    const [parties] = await connection.execute('SELECT party_id FROM party_members WHERE character_id=? LIMIT 1', [player.id]);
    const teamKey = parties[0] ? `party:${parties[0].party_id}` : `solo:${player.id}`;
    const [teamRows] = await connection.execute('SELECT 1 FROM world_scene_contributions WHERE scene_id=? AND team_key=? LIMIT 1 FOR UPDATE', [sceneId, teamKey]);
    if (teamRows[0])
        throw new Error('同一队伍在同一公共现场只能登记一份协作贡献。');
    let cost = 0;
    if (contribution === 'supply') {
        cost = 8;
        const [coins] = await connection.execute('SELECT copper_coins FROM characters WHERE id=? FOR UPDATE', [player.id]);
        if (Number(coins[0]?.copper_coins ?? 0) < cost)
            throw new Error('物资支援需要 8 铜币。');
        await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=?', [cost, player.id]);
    }
    ...
    if (participant.role === 'witness')
        await connection.execute("UPDATE world_scene_participants SET role='assistant' WHERE scene_id=? AND character_id=?", [sceneId, player.id]);
...
const resolveDynamicEncounter = async (qqUserId, choiceCode, requestedId) => withTransaction(async (connection) => {
    const player = await playerContext(connection, qqUserId, true);
    const active = await activeEncounterFor(connection, Number(player.id), true);
    if (!active)
        throw new Error('当前没有可选择的奇遇。');
    if (requestedId && requestedId !== active.id)
        throw new Error('这不是你当前有效的奇遇实例。');
    const definition = definitionForInstance(active);
    const node = definition.nodes?.[active.node_code] ?? { text: definition.opening ?? '', choices: definition.choices ?? [] };
    const selected = node.choices.find(item => item.code === choiceCode);
    if (!selected)
        throw new Error('无效的奇遇选项。');
    const choice = { ...selected };
    const patrol = json(active.context_json, {}).patrol;
    if (patrol) {
        const objective = patrolObjective(patrol, active.node_code);
        if (choice.code !== 'leave' && !patrolPositionMatches(player, objective.location))
            throw new Error(`请先抵达${objective.location.name}（${objective.location.x}, ${objective.location.y}, ${objective.location.z}），再完成这一步。`);
        if (!choice.nextNode && Number(choice.stage) > 0 && choice.worldline) {
            const [progress] = await connection.execute('INSERT IGNORE INTO patrol_world_progress (npc_code,visit_revision,encounter_id) VALUES (?,?,?)', [patrol.npcCode, patrol.revision, active.id]);
            choice.stage = 0;
            if (progress.affectedRows) {
                ...
                const [quota] = await connection.execute('UPDATE worldline_daily_commission_progress SET contribution_count=contribution_count+1 WHERE worldline_code=? AND progress_date=CURDATE() AND contribution_count<?', [choice.worldline, worldlineCommissionDailyLimit]);
    ...
    if (choice.nextNode) {
        const nextNode = definition.nodes?.[choice.nextNode];
        if (!nextNode)
            throw new Error('奇遇分支配置缺失，已记录异常。');
    ...
    const copper = Math.max(0, Number(choice.copper ?? 0));
    const stage = Number(choice.stage ?? 0);
    const worldline = choice.worldline ?? '';
    const flag = choice.flag ?? `${active.template_code}_${choice.code}`;
    await connection.execute("UPDATE player_encounter_instances SET status='resolved',resolved_at=NOW() WHERE id=? AND status='active'", [active.id]);
    if (copper > 0)
        await connection.execute('UPDATE characters SET copper_coins=copper_coins+? WHERE id=?', [copper, player.id]);
    ...
    let worldlineResult = null;
    if (worldline && (!patrol || stage !== 0)) {
        await connection.execute(`UPDATE worldline_states SET stage=stage+?,state_json=JSON_SET(state_json,'$.lastDecision',?,'$.lastActor',?,'$.updatedByEncounter',true) WHERE code=?`, [stage, choice.code, player.id, worldline]);
        worldlineResult = await applyWorldlineEffects(connection, worldline, Number(player.id), Number(player.region_id), active.id);
    }
    const reward = await grantHiddenReward(connection, Number(player.id), active.id, choice);
    const context = json(active.context_json, {});
    const sharedWitnesses = context.sceneId ? await settleSceneWitnesses(connection, context.sceneId, copper, Number(player.region_id), active.template_code) : 0;
    ...
    if (choice.code !== 'leave')
        achievementActivity(connection, Number(player.id));
    if (patrol && choice.code !== 'leave') {
        const facts = ['ACH_F20'];
        if (active.node_code === 'handoff' && choice.code === 'deliver') {
            if (patrol.kind === 'escort')
                facts.push('ACH_F21');
            if (patrol.kind === 'rescue')
                facts.push('ACH_F22');
        }
        recordAchievement(connection, Number(player.id), facts, 'patrol:' + active.id);
    }
    if (patrol)
        await writeLedger(connection, 'patrol.resolved', 'completed', patrol.npcCode, { patrol, choice: choice.code, copper, stage, position: { x: player.pos_x, y: player.pos_y, z: player.pos_z } }, Number(player.id), Number(player.region_id), active.id);
...
const advanceDynamicNpcs = async () => withTransaction(async (connection) => {
    await connection.execute("UPDATE world_scene_instances SET status='expired' WHERE status='active' AND expires_at<=NOW()");
    const [sceneRows] = await connection.execute(`SELECT s.id,s.region_id,s.cell_x,s.cell_y,s.cell_z,s.payload_json,t.definition_json
    ...
    for (const scene of sceneRows) {
        const snapshot = json(scene.payload_json, {}).definitionSnapshot ?? json(scene.definition_json, {});
        for (const npcCode of snapshot.relatedNpcCodes ?? [])
            deployments.set(npcCode, { sceneId: scene.id, regionId: Number(scene.region_id), x: Number(scene.cell_x) * 8 + 4, y: Number(scene.cell_y) * 8 + 4, z: Number(scene.cell_z) });
    }
    const [rows] = await connection.execute('SELECT code,name,region_id,status,route_json,state_json,action_revision FROM world_dynamic_npc_states WHERE next_action_at<=NOW() FOR UPDATE');
    for (const npc of rows) {
        const route = json(npc.route_json, []);
        const home = route[0];
        const previous = json(npc.state_json, {});
        const deployment = deployments.get(npc.code);
        const revision = Number(npc.action_revision) + 1;
        const homeRegionId = Number(previous.homeRegionId ?? npc.region_id);
        const homeStatus = String(previous.homeStatus ?? npc.status) === 'patrolling' ? 'stationed' : String(previous.homeStatus ?? npc.status);
        const homeDestination = isRoutePoint(home) ? { regionId: homeRegionId, x: home.x, y: home.y, z: home.z } : null;
        ...
        if (deployment) {
            ...
            nextRouteIndex = nextPatrolOffset + 1;
            nextMinutes = 12;
            outcome = 'patrolling';
            currentPoint = patrolPoint.name;
        }
        else {
            destination = homeDestination;
            nextStatus = homeStatus;
            nextRouteIndex = 0;
            nextMinutes = 8;
            outcome = npc.status === 'patrolling' ? 'returned' : 'stationed';
            currentPoint = home?.name ?? '常驻点';
        }
        ...
        if (outcome !== 'stationed' || Number(npc.region_id) !== destination.regionId)
            await writeLedger(connection, 'npc.presence', outcome, npc.code, { name: npc.name, sceneId, position: { x: destination.x, y: destination.y, z: destination.z }, homeRegionId, currentPoint }, null, destination.regionId, sceneId);
    }
    return rows.length;
});
const activateReadyWorldlineBosses = async () => {
    const pool = await getPool();
    const [gates] = await pool.execute(`SELECT worldline_code,boss_code,state FROM world_boss_gates
    WHERE state='ready' OR (state='cooldown' AND (last_attempt_at IS NULL OR last_attempt_at<=DATE_SUB(NOW(),INTERVAL 30 MINUTE)))`);
    let activated = 0;
    for (const gate of gates) {
        try {
            const result = await (await import('./adventure.service.js')).activateWorldlineBoss(gate.boss_code);
...
const worldAdminSnapshot = async () => {
    const pool = await getPool();
    await advanceDynamicNpcs();
    const [weather] = await pool.execute('SELECT r.code AS region_code,r.name AS region_name,s.weather_code,s.intensity,s.anomaly_code,s.transition_due_at FROM region_weather_states s JOIN map_regions r ON r.id=s.region_id ORDER BY r.danger_level,r.id');
    const [worldlines] = await pool.execute('SELECT code,stage,state_json FROM worldline_states ORDER BY code');
    const [npcs] = await pool.execute('SELECT n.code,n.name,r.name AS region_name,n.status,n.state_json,n.action_revision,m.pos_x AS x,m.pos_y AS y,m.pos_z AS z FROM world_dynamic_npc_states n JOIN map_regions r ON r.id=n.region_id JOIN map_npcs m ON m.code=n.code AND m.interaction_kind=\'npc\' ORDER BY r.name,n.code');
    const [sites] = await pool.execute('SELECT s.code,s.name,r.name AS region_name,s.site_type,s.state_json FROM world_site_states s JOIN map_regions r ON r.id=s.region_id ORDER BY r.name,s.code');
    const [gates] = await pool.execute('SELECT g.worldline_code,g.boss_code,b.name AS boss_name,g.state,g.stage_required FROM world_boss_gates g LEFT JOIN monster_templates b ON b.code=g.boss_code ORDER BY g.worldline_code,g.boss_code');
    const [counts] = await pool.execute(`SELECT (SELECT COUNT(*) FROM player_encounter_instances WHERE status='active' AND expires_at>NOW()) AS active_encounters, (SELECT COUNT(*) FROM world_scene_instances WHERE status='active' AND expires_at>NOW()) AS active_scenes, (SELECT COUNT(*) FROM dynamic_encounter_templates WHERE is_enabled=1) AS template_count, (SELECT COUNT(*) FROM game_event_ledger WHERE created_at>=CURDATE()) AS ledger_today`);
    return { weather: weather.map(row => ({ regionCode: row.region_code, regionName: row.region_name, name: (weatherText[row.anomaly_code ?? row.weather_code] ?? weatherText.clear).name, intensity: Number(row.intensity), transitionDueAt: new Date(row.transition_due_at) })), worldlines: worldlines.map(row => ({ code: row.code, stage: Number(row.stage), state: json(row.state_json, {}) })), npcs: npcs.map(row => ({ code: row.code, name: row.name, regionName: row.region_name, status: row.status, revision: Number(row.action_revision), position: { x: Number(row.x), y: Number(row.y), z: Number(row.z) }, state: json(row.state_json, {}) })), sites: sites.map(row => ({ code: row.code, name: row.name, regionName: row.region_name, siteType: row.site_type, state: json(row.state_json, {}) })), bossGates: gates.map(row => ({ worldline: row.worldline_code, bossCode: row.boss_code, bossName: row.boss_name ?? '未命名首领', state: row.state, stageRequired: Number(row.stage_required) })), activeEncounters: Number(counts[0]?.active_encounters ?? 0), activeScenes: Number(counts[0]?.active_scenes ?? 0), templateCount: Number(counts[0]?.template_count ?? 0), ledgerToday: Number(counts[0]?.ledger_today ?? 0) };
};
const validateContentTemplate = async (connection, row) => {
    const errors = [];
    const definition = json(row.definition_json, {});
    if (!/^[a-z0-9_]{3,64}$/.test(row.code))
        errors.push('模板代号格式无效');
    if (!row.title.trim() || row.title.length > 96)
        errors.push('标题不能为空且不得超过 96 字');
    const regions = json(row.region_codes_json, []);
    const weathers = json(row.weather_codes_json, []);
    if (!regions.length || !weathers.length)
        errors.push('至少配置一个区域与一种天气');
    const nodes = definition.nodes ?? {};
    const nodeCodes = Object.keys(nodes);
    const openingCode = nodes.opening ? 'opening' : null;
    if (definition.nodes && !openingCode)
        errors.push('分支模板必须包含 opening 节点');
    if (!definition.nodes && !(definition.opening && definition.choices?.length))
        errors.push('模板必须包含开场和至少一个选择');
    const reachable = new Set();
    const terminalNodes = new Set();
    const examineChoices = (nodeCode, choices) => {
        if (!choices.length)
            errors.push(`节点 ${nodeCode} 没有选择`);
        const seen = new Set();
        for (const choice of choices) {
            if (!/^[a-z0-9_]{1,48}$/.test(choice.code) || seen.has(choice.code))
                errors.push(`节点 ${nodeCode} 的选项代号重复或无效`);
            seen.add(choice.code);
            if (!choice.label?.trim() || choice.label.length > 30 || !choice.text?.trim() || choice.text.length > 500)
                errors.push(`节点 ${nodeCode} 的选项文案长度无效`);
            if (choice.nextNode) {
                if (!nodes[choice.nextNode])
                    errors.push(`节点 ${nodeCode} 指向不存在的 ${choice.nextNode}`);
            }
            else
                terminalNodes.add(nodeCode);
        }
    };
    if (definition.nodes)
        for (const [code, node] of Object.entries(nodes)) {
            if (!node.text?.trim() || node.text.length > 800)
                errors.push(`节点 ${code} 文案长度无效`);
            examineChoices(code, node.choices ?? []);
        }
    else
        examineChoices('opening', definition.choices ?? []);
    if (definition.nodes && openingCode) {
        const queue = [openingCode];
        while (queue.length) {
            const code = queue.shift();
            if (reachable.has(code))
                continue;
            reachable.add(code);
            for (const choice of nodes[code]?.choices ?? [])
                if (choice.nextNode)
                    queue.push(choice.nextNode);
        }
        for (const code of nodeCodes)
            if (!reachable.has(code))
                errors.push(`节点 ${code} 不可达`);
    }
    if (!terminalNodes.size)
        errors.push('未找到终局选择');
    const rewardCodes = new Set();
    const worldlineCodes = new Set();
    for (const choices of definition.nodes ? Object.values(nodes).map(node => node.choices) : [definition.choices ?? []])
        for (const choice of choices) {
            if (choice.rewardCode)
                rewardCodes.add(choice.rewardCode);
            if (choice.worldline)
                worldlineCodes.add(choice.worldline);
        }
    const exists = async (table, field, code) => { const [rows] = await connection.execute(`SELECT 1 FROM ${table} WHERE ${field}=? LIMIT 1`, [code]); return Boolean(rows[0]); };
    for (const code of rewardCodes)
        if (!await exists('item_definitions', 'code', code))
            errors.push(`奖励物品不存在：${code}`);
    for (const code of worldlineCodes)
        if (!await exists('worldline_states', 'code', code))
            errors.push(`世界线不存在：${code}`);
    for (const code of definition.relatedSiteCodes ?? [])
        if (!await exists('world_site_states', 'code', code))
            errors.push(`站点不存在：${code}`);
    for (const code of definition.relatedNpcCodes ?? [])
        if (!await exists('world_dynamic_npc_states', 'code', code))
            errors.push(`NPC 不存在：${code}`);
    return { errors, definition };
};
const worldContentPreview = async (code) => withTransaction(async (connection) => {
    const [rows] = await connection.execute(code ? 'SELECT code,title,region_codes_json,weather_codes_json,min_exposure,weight,definition_json,is_enabled FROM dynamic_encounter_templates WHERE code=?' : 'SELECT code,title,region_codes_json,weather_codes_json,min_exposure,weight,definition_json,is_enabled FROM dynamic_encounter_templates ORDER BY code LIMIT 80', code ? [code] : []);
    if (code && !rows[0])
        throw new Error('未找到该内容模板。');
    const results = [];
    for (const row of rows) {
        const validation = await validateContentTemplate(connection, row);
        results.push({ code: row.code, title: row.title, enabled: Boolean(row.is_enabled), weight: Number(row.weight), regions: json(row.region_codes_json, []), weather: json(row.weather_codes_json, []), errors: validation.errors, valid: !validation.errors.length });
    }
    return results;
});
const updateWorldContent = async (code, updater) => withTransaction(async (connection) => {
    const [rows] = await connection.execute('SELECT code,title,region_codes_json,weather_codes_json,min_exposure,weight,definition_json,is_enabled FROM dynamic_encounter_templates WHERE code=? FOR UPDATE', [code]);
    const row = rows[0];
    if (!row)
        throw new Error('未找到该内容模板。');
    const validation = await validateContentTemplate(connection, row);
    if (validation.errors.length)
        throw new Error(`模板未通过校验：${validation.errors.join('；')}`);
    await updater(connection, row);
    await writeLedger(connection, 'content.control', 'updated', code, { historicalInstancesUntouched: true });
});
const setWorldContentEnabled = async (code, enabled) => updateWorldContent(code, async (connection) => { await connection.execute('UPDATE dynamic_encounter_templates SET is_enabled=? WHERE code=?', [enabled ? 1 : 0, code]); });
const setWorldContentWeight = async (code, weight) => { if (!Number.isInteger(weight) || weight < 1 || weight > 1000)
    throw new Error('权重必须是 1 到 1000 的整数。'); return updateWorldContent(code, async (connection) => { await connection.execute('UPDATE dynamic_encounter_templates SET weight=? WHERE code=?', [weight, code]); }); };
const worldLedger = async (limit = 20) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT l.id,l.event_type,l.outcome,l.source_key,c.name AS actor_name,r.name AS region_name,l.payload_json,l.created_at FROM game_event_ledger l LEFT JOIN characters c ON c.id=l.actor_character_id LEFT JOIN map_regions r ON r.id=l.region_id ORDER BY l.id DESC LIMIT ?`, [String(clamp(limit, 1, 100))]);
    return rows.map(row => ({ id: Number(row.id), eventType: row.event_type, outcome: row.outcome, sourceKey: row.source_key, actorName: row.actor_name, regionName: row.region_name, payload: json(row.payload_json, {}), createdAt: new Date(row.created_at) }));
};
const settleDynamicWorld = async () => {
    const pool = await getPool();
    const [regions] = await pool.execute('SELECT region_id FROM region_weather_states WHERE transition_due_at<=NOW()');
    let transitions = 0;
    for (const region of regions)
        await withTransaction(async (connection) => { const before = await connection.execute('SELECT revision FROM region_weather_states WHERE region_id=? FOR UPDATE', [region.region_id]); const previous = Number(before[0][0]?.revision ?? 0); const current = await weatherStateForRegion(connection, Number(region.region_id)); if (current.revision !== previous)
            transitions += 1; });
...
export { acceptWorldSiteCommissionFromAttendant, advanceDynamicNpcs, claimWorldSiteCommission, combatEnvironmentFor, commissionSiteForAttendant, completeWorldSiteCommissionsAtSite, contributeDynamicScene, currentDynamicEncounter, initializeDynamicWorldSystem, joinDynamicScene, nearbyDynamicScenes, offerDynamicEncounter, patrolEncounterPreview, playerWorldSiteCommissions, recordExplorationMovement, resolveDynamicEncounter, setWorldContentEnabled, setWorldContentWeight, settleDynamicWorld, snapshotCombatEnvironment, startPatrolEncounter, submitWorldSiteCommission, useWorldSite, weatherEffectAudit, weatherElementMultiplier, weatherForPlayer, worldAdminSnapshot, worldContentPreview, worldLedger, worldSiteForAttendant, worldSiteKnock, worldSiteView };
