...
import { generatedDynamicEncounterTemplates, dynamicWorldRegions, dynamicNpcDisplayName } from './world-dynamics.content.js';
...
const resolveDynamicEncounter = async (qqUserId, choiceCode, requestedId) => withTransaction(async (connection) => {
    ...
    const choice = node.choices.find(item => item.code === choiceCode);
    if (!choice)
...
const advanceDynamicNpcs = async () => withTransaction(async (connection) => {
    ...
    for (const npc of rows) {
        ...
        const homeStatus = String(previous.homeStatus ?? npc.status);
        const homeDestination = isRoutePoint(home) ? { sceneId: null, regionId: homeRegionId, x: home.x, y: home.y, z: home.z } : null;
        const destination = deployment ?? homeDestination;
        if (!destination) {
        ...
        if (outcome !== 'stationed' || Number(npc.region_id) !== destination.regionId)
            await writeLedger(connection, 'npc.presence', outcome, npc.code, { name: npc.name, sceneId: deployment?.sceneId ?? null, position: { x: destination.x, y: destination.y, z: destination.z }, homeRegionId }, null, destination.regionId, deployment?.sceneId ?? null);
    }
    return rows.length;
});
...
const worldAdminSnapshot = async () => {
    ...
    const [npcs] = await pool.execute('SELECT n.code,n.name,r.name AS region_name,n.status,n.state_json,n.action_revision FROM world_dynamic_npc_states n JOIN map_regions r ON r.id=n.region_id ORDER BY r.name,n.code');
...
export { acceptWorldSiteCommissionFromAttendant, advanceDynamicNpcs, claimWorldSiteCommission, combatEnvironmentFor, commissionSiteForAttendant, completeWorldSiteCommissionsAtSite, contributeDynamicScene, currentDynamicEncounter, initializeDynamicWorldSystem, joinDynamicScene, nearbyDynamicScenes, offerDynamicEncounter, playerWorldSiteCommissions, recordExplorationMovement, resolveDynamicEncounter, setWorldContentEnabled, setWorldContentWeight, settleDynamicWorld, snapshotCombatEnvironment, useWorldSite, weatherEffectAudit, weatherElementMultiplier, weatherForPlayer, worldAdminSnapshot, worldContentPreview, worldLedger, worldSiteForAttendant, worldSiteKnock, worldSiteView };
