...
const paging = (page, total) => ({ page: Math.min(Math.max(1, page), Math.max(1, Math.ceil(total / PAGE_SIZE))), totalPages: Math.max(1, Math.ceil(total / PAGE_SIZE)) });
const amountOf = (value) => { if (!Number.isInteger(value) || value < 1 || value > 999)
    throw new Error('数量必须是 1 至 999 之间的整数。'); return value; };
...
const bookshopSellCatalog = async (qqUserId, page = 1, keyword = '') => {
    ...
    const info = paging(page, Number(countRows[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT i.id,i.name,i.item_category,pi.quantity,CEIL(i.trade_price*1.20) AS price FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE ${where} ORDER BY i.item_category,i.name LIMIT ? OFFSET ?`, [character.id, term, String(PAGE_SIZE), String((info.page - 1) * PAGE_SIZE)]);
...
const buyBookshopItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = amountOf(quantity);
    const character = await characterFor(connection, qqUserId, true);
    const [rows] = await connection.execute("SELECT i.name,i.code,i.is_tradeable,JSON_UNQUOTE(JSON_EXTRACT(i.effect_json,'$.skillBook')) AS skill_code,bs.buy_price,bs.stock_quantity FROM bookshop_items bs JOIN item_definitions i ON i.id=bs.item_id WHERE bs.item_id=? AND bs.is_active=1 FOR UPDATE", [itemId]);
    const item = rows[0];
    if (!item)
        throw new Error('这本书已下架。');
    if (Number(item.stock_quantity) < amount)
        throw new Error(`库存不足，剩余 ${item.stock_quantity} 本。`);
    if (item.code.startsWith('skill_book_resident_')) {
        if (amount !== 1)
            throw new Error('技能书每次只能购买一本。');
      ...
      LEFT JOIN player_skill_discoveries d ON d.skill_id=s.id AND d.character_id=? WHERE s.code=? AND (ps.skill_id IS NOT NULL OR d.skill_id IS NOT NULL) LIMIT 1`, [character.id, character.id, item.skill_code]);
        if (known[0])
            throw new Error('你已经领悟这项技能，无需重复购买技能书。');
    }
...
const sellBookshopItem = async (qqUserId, itemId, quantity = 1) => withTransaction(async (connection) => {
    const amount = amountOf(quantity);
    const character = await characterFor(connection, qqUserId, true);
    ...
    const item = rows[0];
    if (!item || !item.is_tradeable || !Number(item.trade_price) || !['书籍', '卷宗', '技能书'].includes(item.item_category))
        throw new Error('店主只收购可交易的书籍、卷宗与技能书。');
    if (Number(item.quantity) < amount)
        throw new Error(`背包数量不足，当前仅有 ${item.quantity} 本。`);
...
const readSkillBook = async (qqUserId, itemId) => withTransaction(async (connection) => {
    ...
    if (discovered[0] || learned[0])
        throw new Error(`你已经领悟技能「${skill.name}」。`);
...
export { bookshopCatalog, bookshopSellCatalog, buyBookshopItem, readSkillBook, sellBookshopItem };
