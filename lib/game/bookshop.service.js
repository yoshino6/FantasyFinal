...
const bookshopSellCatalog = async (qqUserId, page = 1, keyword = '') => {
    ...
    const info = paging(page, Number(countRows[0]?.total ?? 0));
    const [rows] = await pool.execute(`SELECT i.id,i.name,i.item_category,pi.quantity,CEIL(i.trade_price*1.20) AS price FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE ${where} ORDER BY i.item_category,i.name LIMIT ? OFFSET ?`, [character.id, term, PAGE_SIZE, (info.page - 1) * PAGE_SIZE]);
