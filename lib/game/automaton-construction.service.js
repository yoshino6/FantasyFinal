...
import { automatonCharacter, assertAutomatonSafe, automatonRecipes } from './automaton.service.js';
import { constructionRecipeByCode } from './deconstructor-catalog.js';
import { constructItemFor } from './deconstructor.service.js';
import { createCraftRequest, craftRequestFor, completeCraftRequest } from './alchemy-journal.service.js';
import { alchemyFingerprint } from './alchemy-journal.js';

const planAutomatonComponents = (stock, bodies) => {
    if (!Number.isInteger(bodies) || bodies < 1 || bodies > 100)
        throw new Error('素体目标为 1～100 具。');
    const available = new Map(stock), missing = new Map(), used = new Map(), steps = [];
    const need = (code, count, depth = 0) => {
        if (depth > 20)
            throw new Error('构造链存在循环。');
        const take = Math.min(count, available.get(code) ?? 0);
        available.set(code, (available.get(code) ?? 0) - take);
        if (take)
            used.set(code, (used.get(code) ?? 0) + take);
        count -= take;
        if (!count)
            return;
        ...
        if (recipe.outputType !== 'material' || recipe.recommendedSecondaryLevel > 4)
            throw new Error('素体前置配方超出四级制造范围。');
        for (let n = 0; n < count; n++) {
            for (const part of recipe.ingredients)
                need(part.code, part.quantity, depth + 1);
            steps.push(code);
        }
    };
    for (const part of automatonRecipes.find(r => r.code === 'automaton_body').ingredients)
        need(part.code, part.quantity * bodies);
...
const planFor = async (connection, user, bodies) => {
    ...
    if (character.secondary_profession_code !== 'deconstructor' || Number(progress[0]?.level ?? 0) < 4)
        throw new Error('解构师四级解锁素体构造链。');
    const [rows] = await connection.execute('SELECT i.code,i.name,COALESCE(p.quantity,0) quantity,p.binding_revision FROM item_definitions i LEFT JOIN player_inventory p ON p.item_id=i.id AND p.character_id=? WHERE i.item_type=\'material\' ORDER BY i.id FOR UPDATE', [character.id]);
    ...
    const fingerprint = alchemyFingerprint(rows.map(r => [r.code, r.quantity, r.binding_revision]));
    return { character, plan, names, fingerprint };
};
...
const confirmAutomatonComponents = (user, token) => withTransaction(async (connection) => {
    ...
    if (plan.missing.length || fingerprint !== request.snapshot.fingerprint)
        throw new Error('构造链库存已变化，请重新预览。');
    for (const code of plan.steps) {
        const result = await constructItemFor(connection, user, code);
        if (!result.success)
            throw new Error('四级素体前置构造出现异常，本批已回滚。');
    }
