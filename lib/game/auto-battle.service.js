...
const beginAutoBattleQuickSetup = async (qqUserId, mode = 'pve') => withTransaction(async (connection) => {
    ...
    if (!rows[0])
        throw new Error('请先发送“注册”创建角色。');
    const tables = autoTables(mode);
    await connection.execute(`DELETE FROM ${tables.actions} WHERE character_id=?`, [rows[0].id]);
...
const finishAutoBattleQuickSetup = async (qqUserId, mode = 'pve') => { const characterId = await characterIdFor(qqUserId); const pool = await getPool(); await pool.execute(`DELETE FROM ${autoTables(mode).quick} WHERE character_id=?`, [characterId]); };
...
const pendingPartyAutoBattleActions = async (qqUserId) => {
    ...
    const [targets] = await pool.execute(`SELECT ct.spawn_id,ct.is_defeated,s.traits_json,ct.cooldowns
