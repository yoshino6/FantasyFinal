...
const recordAutoConfiguration = async (connection, characterId, mode, summary, detail) => recordCharacterOperation(connection, { characterId, kind: 'combat.auto_config_changed', source: { system: 'auto_battle_config', id: randomUUID(), step: 'changed' }, outcome: '调整', summary, detail: { mode, ...detail } });
...
const setAutoBattleEnabled = async (qqUserId, enabled, mode = 'pve') => withTransaction(async (connection) => {
    ...
    if (Number(changed.affectedRows) > 0)
        await recordAutoConfiguration(connection, characterId, mode, `${enabled ? '启用' : '停用'}自动战斗`, { setting: 'enabled', enabled });
    return enabled;
});
...
const setAutoPotionEnabled = async (qqUserId, enabled, mode = 'pve') => withTransaction(async (connection) => {
    ...
    if (Number(changed.affectedRows) > 0)
        await recordAutoConfiguration(connection, characterId, mode, `${enabled ? '启用' : '停用'}自动药剂`, { setting: 'auto_potion_enabled', enabled });
    return enabled;
});
const autoBattleSkills = async (qqUserId, page = 1, keyword = '') => {
    ...
    const choices = [{ id: null, name: '普通攻击' }, ...rows.map(row => ({ id: Number(row.id), name: row.name }))].slice((safePage - 1) * 10, safePage * 10);
...
const assertSkill = async (connection, characterId, skillId) => {
    ...
    if (!rows[0])
        throw new Error('只能配置已经学习的主动技能。');
};
const saveAutoBattleAction = async (qqUserId, sequence, skillId, mode = 'pve') => withTransaction(async (connection) => {
    const characterId = await mutableCharacterIdFor(connection, qqUserId);
    if (!Number.isInteger(sequence) || sequence < 1 || sequence > 30)
        throw new Error('出招位置需在 1 至 30 之间。');
    await assertSkill(connection, characterId, skillId);
    const [changed] = await connection.execute(`INSERT INTO ${autoTables(mode).actions} (character_id,sequence_no,skill_id) VALUES (?,?,?) ON DUPLICATE KEY UPDATE skill_id=VALUES(skill_id)`, [characterId, sequence, skillId]);
    if (Number(changed.affectedRows) > 0)
        await recordAutoConfiguration(connection, characterId, mode, `设置自动出招 ${sequence}`, { setting: 'action', sequence, skillId });
});
const deleteAutoBattleAction = async (qqUserId, sequence, mode = 'pve') => withTransaction(async (connection) => {
    ...
    if (Number(deleted.affectedRows) > 0)
        await recordAutoConfiguration(connection, characterId, mode, `删除自动出招 ${sequence}`, { setting: 'action', sequence, deleted: true });
});
...
const saveQuickAutoBattleAction = async (qqUserId, skillId, mode = 'pve') => withTransaction(async (connection) => {
    ...
    await recordAutoConfiguration(connection, characterId, mode, `快速配置自动出招 ${sequence}`, { setting: 'quick_action', sequence, skillId });
    return sequence + 1;
});
const finishAutoBattleQuickSetup = async (qqUserId, mode = 'pve') => withTransaction(async (connection) => {
    ...
    if (Number(finished.affectedRows) > 0)
        await recordAutoConfiguration(connection, characterId, mode, '完成快速配置自动战斗', { setting: 'quick_setup', phase: 'finished' });
});
const autoPotionItems = async (qqUserId, page = 1, keyword = '') => {
    const characterId = await characterIdFor(qqUserId);
    const pool = await getPool();
    const [rows] = await pool.execute('SELECT i.id,i.name FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.quantity>0 AND i.item_type=\'consumable\' AND i.name LIKE ? ORDER BY i.id', [characterId, `%${keyword}%`]);
    const total = Math.max(1, Math.ceil(rows.length / 10));
    const safePage = Math.max(1, Math.min(total, page));
    return { items: rows.slice((safePage - 1) * 10, safePage * 10).map(row => ({ id: Number(row.id), name: row.name })), page: safePage, total };
};
const setAutoPotionThreshold = async (qqUserId, kind, threshold, mode = 'pve') => withTransaction(async (connection) => {
    ...
    if (!Number.isInteger(threshold) || threshold < 1 || threshold > 99)
        throw new Error('门槛需要是 1 至 99 的整数百分比。');
    ...
    if (Number(changed.affectedRows) > 0)
        await recordAutoConfiguration(connection, characterId, mode, `设置${kind.toUpperCase()}自动药剂门槛`, { setting: `${kind}_threshold`, threshold });
});
const setAutoPotionItem = async (qqUserId, kind, itemId, mode = 'pve') => withTransaction(async (connection) => {
    const characterId = await mutableCharacterIdFor(connection, qqUserId);
    if (itemId !== null) {
        const [items] = await connection.execute('SELECT 1 FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.id=? AND i.item_type=\'consumable\'', [characterId, itemId]);
        if (!items[0])
            throw new Error('背包中没有该可使用道具。');
    }
    ...
    if (Number(changed.affectedRows) > 0)
        await recordAutoConfiguration(connection, characterId, mode, `设置${kind.toUpperCase()}自动药剂`, { setting: `${kind}_item_id`, itemId });
});
const availableAutoPotion = async (pool, state) => {
    if (!Number(state.auto_potion_enabled))
        return null;
    ...
    if (!preferred)
        return null;
    const [items] = await pool.execute('SELECT 1 FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.item_id=? AND pi.quantity>0 AND i.item_type=\'consumable\' LIMIT 1', [state.character_id, preferred]);
...
const configuredAutoAction = async (pool, state) => {
    ...
    if (visibleResidentBuff(readRuleState(cooldowns?.__rules), selected.code, Number(state.turn_no)))
        return { type: 'attack' };
...
const nextAutoBattleAction = async (qqUserId) => {
    ...
    if (!settings[0] || !settings[0].enabled || storyBattle[0])
        return null;
...
const pendingPartyAutoBattleActions = async (qqUserId) => {
    ...
    const jsonObject = (value) => { if (!value)
        return {}; try {
    ...
    const component = (target) => {
        const traits = Array.isArray(target.traits_json) ? target.traits_json : (() => { try {
            return JSON.parse(String(target.traits_json ?? '[]'));
        }
        catch {
            return [];
        } })();
        return Array.isArray(traits) ? traits.find((trait) => trait?.code === 'boss_component') : undefined;
    };
    const kingbeast = (target) => {
        ...
        return Array.isArray(traits) ? traits.find((trait) => trait?.code === 'kingbeast_encounter') : undefined;
    };
    const kingbeastSelectableTargets = (rows) => rows.filter(target => {
        if (Number(target.is_defeated))
            return false;
    ...
    const automaticTargetFor = (member) => {
        ...
        if (forcedKingbeastTarget)
            return Number(forcedKingbeastTarget.spawn_id);
        const componentRows = aliveTargets.map(target => ({ target, trait: component(target) })).filter((entry) => Boolean(entry.trait));
    ...
    if (regionalTarget) {
        ...
        JOIN effect_definitions e ON e.id=ce.effect_id WHERE mine.character_id=? AND ce.remaining_turns>0`, [characterId]);
            const profiles = await Promise.all(party.map(async (row) => {
                ...
                const rule = readRuleState(cd.__rules);
                ...
                if (preferred.type === 'skill' && !ready.some(skill => Number(skill.id) === preferredSkillId))
                    preferred = { type: 'attack' };
                ...
                return { key: `member:${row.character_id}`, hp: Number(row.current_hp), hpMax: Number(row.hp_max),
                    ...
                    cleanse: ready.find(skill => ['purifying_light', 'saint_healer_absolution_hand', 'saint_healer_revival_sanctuary', 'summoner_returning_veil'].includes(String(skill.code))) ? { type: 'skill', skillId: Number(ready.find(skill => ['purifying_light', 'saint_healer_absolution_hand', 'saint_healer_revival_sanctuary', 'summoner_returning_veil'].includes(String(skill.code))).id) } : undefined,
