...
import { visibleResidentBuff, readRuleState } from './combat-rule-registry.js';
...
const nextAutoBattleAction = async (qqUserId) => {
    ...
    const [storyBattle] = await pool.execute(`SELECT 1 FROM combat_members cm JOIN combat_sessions cs ON cs.id=cm.session_id AND cs.state='active' AND cs.mode<>'spar'
