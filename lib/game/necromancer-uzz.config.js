const uzzTemplateCode = 'necromancer_uz';
const uzzOrdinarySummonCodes = [
    'uzz_skeleton_berserker',
    'uzz_skeleton_archer',
    'uzz_pain_wraith',
    'uzz_skeleton_mage'
];
const uzzBoneDragonCode = 'uzz_frost_bone_dragon';
const uzzPhaseTwoTransition = {
    code: 'uzz_phase_two',
    title: '第二阶段·亡者开门',
...
const uzzPhaseTwoTransitionLog = () => [
...
const uzzPhaseTwoTransitionDue = (phaseTwoLocked, currentHp, maxHp) => !phaseTwoLocked && currentHp / Math.max(1, maxHp) < .5;
const uzzSkills = {
...
const isUzzOrdinarySummonCode = (code) => uzzOrdinarySummonCodes.includes(String(code));
const isUzzBoneDragonCode = (code) => String(code) === uzzBoneDragonCode;
const uzzNextSummonSlot = (lastSummonSlot, lastClearSlot) => Math.max(lastSummonSlot > 0 ? lastSummonSlot + 7 : 1, lastClearSlot === undefined ? 1 : lastClearSlot + 6);
const uzzSummonDue = (currentSlot, lastSummonSlot, lastClearSlot) => currentSlot >= uzzNextSummonSlot(lastSummonSlot, lastClearSlot);
const uzzRotationSkill = (phaseTwo, cursor) => {
...
const uzzUndeadConstitutionMultiplier = (magic, element) => {
    if (magic)
        return element === '光' ? 1.5 : .65;
    return 1.25 * (element === '光' ? 1.5 : 1);
};
const uzzDomainMagicMultiplier = (magic, element) => magic && element !== '冰' ? .75 : 1;
const uzzSpeedFactor = (domainActive, slowPct = 0, breathSlowPct = 0) => Math.max(.3, 1 - ((domainActive ? 20 : 0) + Math.max(0, slowPct) + Math.max(0, breathSlowPct)) / 100);
const uzzSoulDrainTransfer = (victimCurrentMp, bossCurrentMp, bossMaxMp, bossCurrentHp, bossMaxHp) => {
...
export { isUzzBoneDragonCode, isUzzOrdinarySummonCode, uzzBoneDragonCode, uzzDomainMagicMultiplier, uzzNextSummonSlot, uzzOrdinarySummonCodes, uzzPhaseTwoTransition, uzzPhaseTwoTransitionDue, uzzPhaseTwoTransitionLog, uzzRotationSkill, uzzSkills, uzzSoulDrainTransfer, uzzSpeedFactor, uzzSummonDue, uzzTemplateCode, uzzUndeadConstitutionMultiplier };
