...
import { purifiedCraftMaterialValue } from './monster-crafting-material.service.js';
...
const rareParticles = new Set(['light_element_dust', 'dark_element_dust']);
const ordinaryElementParticles = new Set(['wood_element_dust', 'metal_element_dust', 'water_element_dust', 'ice_element_dust', 'fire_element_dust', 'thunder_element_dust', 'wind_element_dust']);
const isRareParticle = (code) => rareParticles.has(code);
...
const exclusionReasonFor = (item, effect) => {
    if (particleCodes.includes(item.code) || item.item_category === '粒子')
        return '粒子是分解产物，不能再次分解。';
    if (['copper_coin', 'silver_coin', 'gold_coin'].includes(item.code) || item.item_category === '货币' || effect.currency)
        return '货币不能分解。';
    if (item.code.includes('blueprint') || item.item_category === '图纸' || effect.constructionBlueprint)
        return '图纸不能分解。';
    if (item.code.startsWith('skill_book_') || effect.skillBook)
        return '技能书不能分解。';
    if (item.item_category === '地图' || effect.map)
        return '地图不能分解。';
    if (['evolution_active_sample', 'evolution_stable_medium', 'evolution_catalyst'].includes(item.code) || item.item_category === '育成' || effect.evolutionMaterial)
        return '专用成长资源暂不开放分解。';
    if (item.item_category === '神材' || item.item_category === '世界印记' || effect.artifactMaterial)
        return '唯一成长材料与世界印记不能分解。';
    if (item.item_category === '怪物卡片' || effect.monsterCard)
        return '怪物卡片不能分解。';
    if (item.code === 'automaton_body' || item.code === 'pure_soul_trace' || item.code.startsWith('automaton_feed_') || effect.automatonProduct)
        return '机巧人工材料尚未配置成本回拆。';
    if (item.code === 'resonance_crystal' || ['任务', '剧情', '礼物'].includes(item.item_category) || ['xiaowei_gift', 'qinger_gift'].includes(item.code))
        return '任务、纪念或专用资源不能分解。';
    if (excludedCodes.has(item.code) || excludedCategories.has(item.item_category))
        return '该物品属于受保护的专用资源。';
    if (item.item_type !== 'material' && item.code !== 'healing_herb')
        return item.item_type === 'equipment' ? '成品装备不在材料分解范围。' : '成品道具不在材料分解范围。';
    return null;
};
const deconstructionBudget = (value) => {
    const points = [[1, .3], [2, .6], [5, 1.5], [10, 3], [20, 5], [40, 6], [100, 8], [200, 13], [400, 20], [800, 34]];
    if (!Number.isFinite(value) || value <= 0)
        return 0;
    if (value <= 1)
        return .3 * value;
    for (let index = 1; index < points.length; index += 1) {
        const [rightValue, rightBudget] = points[index];
        const [leftValue, leftBudget] = points[index - 1];
        if (value <= rightValue)
            return leftBudget + (rightBudget - leftBudget) * (value - leftValue) / (rightValue - leftValue);
    }
    return 34 * Math.sqrt(value / 800);
};
const normalize = (weights) => {
    const entries = Object.entries(weights).filter((entry) => particleCodes.includes(entry[0]) && Number(entry[1]) > 0);
    const total = entries.reduce((sum, [, weight]) => sum + weight, 0);
    return Object.fromEntries(entries.map(([code, weight]) => [code, weight / total]));
};
const inferredElements = (text) => {
    const scores = {};
    const add = (code, score = 1) => { scores[code] = (scores[code] ?? 0) + score; };
    if (/木|根|藤|枝|叶|草|苔|藻|芽|花/u.test(text))
        add('wood_element_dust');
    if (/土|岩|石|矿|金属|铁|铜|银|合锭|甲|壳/u.test(text))
        add('metal_element_dust');
    if (/水|潮|河|海|露|涟|雾|沼|泽/u.test(text))
        add('water_element_dust');
    if (/冰|霜|寒|雪/u.test(text))
        add('ice_element_dust');
    if (/火|炎|熔|炉|赤|炽/u.test(text))
        add('fire_element_dust');
    if (/雷|电|磁/u.test(text))
        add('thunder_element_dust');
    if (/风|空鸣|羽|翎/u.test(text))
        add('wind_element_dust');
    if (/光|辉|曦|圣/u.test(text))
        add('light_element_dust', .5);
    if (/暗|蚀|夜|幽/u.test(text))
        add('dark_element_dust', .5);
    return scores;
};
const inferredWeights = (item) => {
    const text = `${item.name} ${item.description ?? ''}`;
    const scores = inferredElements(text);
    const add = (code, score) => { scores[code] = (scores[code] ?? 0) + score; };
    if (/血|肉|皮|革|筋|毛|绒|鬃|膜|胶/u.test(text))
        add('blood_residue', 1);
    if (/魔力|灵魂|法力|奥术|回响|共鸣|核心|晶核|残印|军徽/u.test(text)) {
        add('energy_ember', .7);
        add('magic_unit', .3);
    }
    if (Object.keys(scores).length)
        return normalize(scores);
    if (item.item_category === '食材')
        return { blood_residue: .85, energy_ember: .15 };
    if (item.item_category === '草药')
        return { wood_element_dust: .8, water_element_dust: .2 };
    if (['锻材', '稀有锻材', '区域锻材', 'Boss部件', '建材'].includes(item.item_category))
        return { metal_element_dust: .7, energy_ember: .3 };
    if (item.item_category === '炼材')
        return { energy_ember: .6, magic_unit: .4 };
    if (item.item_category === '怪材')
        return { blood_residue: .5, energy_ember: .5 };
    return { energy_ember: .7, magic_unit: .3 };
};
const dynamicMonsterWeightOverride = (name, kind) => {
    if (/^(藤心木|蔓节片)$/u.test(name))
        return { wood_element_dust: .8, energy_ember: .2 };
    if (/^(露凝胶|晨露核|泡凝胶|涟漪核)$/u.test(name))
        return { water_element_dust: .7, energy_ember: .3 };
    if (/^(浮灯胶|微光核)$/u.test(name))
        return { water_element_dust: .5, light_element_dust: .3, energy_ember: .2 };
    if (/^(砾岩芯|碎岩片|石脉芯|脉岩片)$/u.test(name))
        return { metal_element_dust: .8, energy_ember: .2 };
    if (/^(赤铁灵膜|火晶核|熔岩芯)$/u.test(name))
        return { fire_element_dust: .7, metal_element_dust: .2, energy_ember: .1 };
    if (/^(磁石芯|磁甲片)$/u.test(name))
        return { metal_element_dust: .6, thunder_element_dust: .3, energy_ember: .1 };
    if (/^(雾藻膜|藻核)$/u.test(name))
        return { wood_element_dust: .5, water_element_dust: .4, energy_ember: .1 };
    if (/^(沼火膜|鬼灯核)$/u.test(name))
        return { dark_element_dust: .5, fire_element_dust: .3, energy_ember: .2 };
...
const monsterWeights = (name, kind) => {
    ...
    if (!elementEntries.length)
        return { blood_residue: blood, energy_ember: 1 - blood };
    const result = { blood_residue: blood * .3, energy_ember: (1 - blood) * .3 };
    for (const [code, weight] of elementEntries)
        result[code] = (result[code] ?? 0) + weight * .7;
    return normalize(result);
};
const refinedWeights = (code) => {
    if (/^spellcloth_bolt(?:_t[2-5])?$/.test(code))
        return { wood_element_dust: .5, blood_residue: .2, magic_unit: .3 };
    if (/^tanned_spirit_leather(?:_t[2-5])?$/.test(code))
        return { blood_residue: .5, energy_ember: .3, magic_unit: .2 };
    if (/^bone_steel_plate(?:_t[2-5])?$/.test(code))
        return { metal_element_dust: .6, energy_ember: .2, magic_unit: .2 };
    if (/^cast_shell_plate(?:_t[2-5])?$/.test(code))
        return { metal_element_dust: .7, energy_ember: .2, magic_unit: .1 };
    if (/^laminated_scale_plate(?:_t[2-5])?$/.test(code))
        return { metal_element_dust: .5, blood_residue: .2, magic_unit: .3 };
    if (/^refined_/.test(code))
        return undefined;
    return undefined;
};
const materialValue = (item) => {
    const explicit = explicitValues[item.code];
    if (explicit)
        return explicit;
    const forge = forgeMaterialValue[item.code];
    if (forge)
        return forge;
    const purified = purifiedCraftMaterialValue(item.code);
    if (purified)
        return purified;
    const trade = Math.max(0, Number(item.trade_price ?? 0));
    if (trade)
        return trade;
    return 0;
};
const profileProficiencyForBudget = (budget) => budget < 5 ? 1 : budget < 10 ? 2 : budget < 20 ? 3 : budget < 34 ? 4 : 5;
const deconstructionProfileFor = (item) => {
    const effect = jsonRecord(item.effect_json);
    if (exclusionReasonFor(item, effect))
        return null;
    ...
    if (['hair', 'gel_skin', 'bone', 'shell', 'scale'].includes(kind)) {
        ...
        const monsterClass = ['normal', 'large', 'elite', 'boss'].includes(rawMonsterClass)
            ? rawMonsterClass
            : 'normal';
        const classValue = { normal: 1, large: 2, elite: 3, boss: 4 }[monsterClass] ?? 1;
        ...
        return { kind: 'monster', weights: monsterWeights(item.name, kind), particleValue: classValue * levelMultiplier, magicChance: { normal: 0, large: 0, elite: .2, boss: .25 }[monsterClass] * levelMultiplier, proficiency: classValue * levelMultiplier };
    }
    if (ordinaryProfiles[item.code])
        return { kind: 'ordinary', ...ordinaryProfiles[item.code], proficiency: 1 };
    if (magicalBeastMaterials.has(item.code))
        return { kind: 'magical_beast', proficiency: 2 };
    if (forgeProfiles[item.code])
        return { kind: 'forge', outputs: forgeProfiles[item.code], proficiency: { living_wood: 1, meteor_iron: 2, star_copper: 3, moon_silver: 4, sun_gold: 5 }[item.code] ?? 3 };
    if (coloredSlimes[item.code])
        return { kind: 'slime', element: coloredSlimes[item.code], proficiency: 1 };
    if (fixedChanceProfiles[item.code])
        return { kind: 'fixed_chance', outputs: fixedChanceProfiles[item.code], proficiency: 1 };
    const explicit = explicitWeights[item.code];
    const refined = refinedWeights(item.code);
    if (!explicit && !refined && !genericProductionCategories.has(item.item_category))
        return null;
...
const deconstructionBlockReason = (item) => {
    const exclusion = exclusionReasonFor(item, jsonRecord(item.effect_json));
    if (exclusion)
        return exclusion;
    if (deconstructionProfileFor(item))
        return null;
    if (item.item_category === '特殊')
        return '特殊物品尚未配置安全的分解规则。';
    if (['基材', '构件'].includes(item.item_category))
        return '人工构造材料缺少成本账本配置，暂不可分解。';
    if (!genericProductionCategories.has(item.item_category))
        return '该类别未纳入普通生产材料分解范围。';
    return '缺少可靠的材料估值，暂不可分解。';
};
const chainedExpectation = (firstChance, decay, limit) => {
    let expected = 0;
    let reach = 1;
    let chance = Math.max(0, Math.min(1, firstChance));
    for (let index = 0; index < limit; index += 1) {
        const hit = reach * chance;
        expected += hit;
        reach = hit;
        chance *= Math.max(0, Math.min(1, decay));
    }
    return expected;
};
const rareParticleChance = (candidateExpected, ordinaryChance) => Math.max(0, Math.min(.25 * candidateExpected, .15, .25 * ordinaryChance));
const weightedPreviews = (weights, budget, bonusMultiplier) => {
    const candidates = Object.entries(normalize(weights)).map(([code, weight]) => ({ code, expected: budget * bonusMultiplier * weight / particleValues[code] }));
...
const deconstructionPreviewFor = (profile, bonusMultiplier = 1) => {
    if (profile.kind === 'weighted')
        return weightedPreviews(profile.weights, profile.budget, bonusMultiplier);
    if (profile.kind === 'monster') {
        ...
        if (expected > 0)
            outputs.push({ code: 'magic_unit', name: particleNames.magic_unit, expected, min: Math.floor(expected), max: Math.ceil(expected), rare: false });
        return outputs;
    }
    ...
    if (profile.kind === 'slime') {
        const rare = isRareParticle(profile.element);
        const elementExpected = rare ? rareParticleChance(1, 1) : 1;
        return [
...
const chainedRoll = (firstChance, decay, limit, random) => {
    let count = 0;
...
const add = (outputs, code, amount) => { if (amount > 0)
    outputs.set(code, (outputs.get(code) ?? 0) + amount); };
const rollExpected = (outputs, preview, random) => {
    const base = Math.floor(preview.expected);
    add(outputs, preview.code, base + (random() < preview.expected - base ? 1 : 0));
};
const rollDeconstructionProfile = (profile, bonusMultiplier = 1, random = Math.random) => {
    const outputs = new Map();
    if (profile.kind === 'weighted' || profile.kind === 'monster' || profile.kind === 'fixed_chance') {
        for (const preview of deconstructionPreviewFor(profile, bonusMultiplier))
            rollExpected(outputs, preview, random);
        return outputs;
    }
    if (profile.kind === 'ordinary') {
    ...
    if (profile.kind === 'magical_beast') {
        ...
        for (let index = 0; index < count; index += 1)
            add(outputs, random() < .5 ? 'blood_residue' : 'energy_ember', 1);
    ...
    if (profile.kind === 'slime') {
        ...
        if (random() < elementChance)
            add(outputs, profile.element, 1);
        if (random() < .8)
            add(outputs, 'blood_residue', 1);
        if (random() < .4)
            add(outputs, 'energy_ember', 1);
        return outputs;
    }
    const previews = deconstructionPreviewFor(profile, bonusMultiplier);
    for (const output of profile.outputs) {
        if (isRareParticle(output.code)) {
            const preview = previews.find(entry => entry.code === output.code);
            if (random() < preview.expected)
                add(outputs, output.code, 1);
        }
        else
            add(outputs, output.code, chainedRoll(1, Math.min(1, output.decay * bonusMultiplier), output.limit, random));
    }
    return outputs;
};
const deconstructionProficiencyFor = (profile) => profile.proficiency;
const compactNumber = (value, digits) => {
    const fixed = value.toFixed(digits);
    return fixed.includes('.') ? fixed.replace(/0+$/u, '').replace(/\.$/u, '') : fixed;
};
const deconstructionPreviewText = (profile, bonusMultiplier = 1) => deconstructionPreviewFor(profile, bonusMultiplier)
    .filter(output => output.expected > 0)
    .map(output => {
    const range = output.min === output.max ? `×${output.min}` : `×${output.min}～${output.max}`;
    if (output.min === output.max)
        return `${output.name}${range}`;
    if (output.max === 1) {
...
export { chainedExpectation, deconstructionBlockReason, deconstructionBudget, deconstructionPreviewFor, deconstructionPreviewText, deconstructionProficiencyFor, deconstructionProfileFor, isRareParticle, particleCodes, particleNames, particleValues, rareParticleChance, rollDeconstructionProfile };
