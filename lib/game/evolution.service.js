...
import { realmLevelCap, experienceRequiredForLevel } from './constants.js';
...
import { mutationByCode, dynamicDamageMutationCodes, dynamicDamageReductionMutationCodes, mutationCatalog as mutationCatalog$1 } from './mutation.config.js';
...
const ensureEvolutionProfile = async (connection, character, lock = false) => {
    ...
    if (profile)
        await migrateMutationCatalog(connection, Number(character.id));
    return profile;
};
const repairEvolutionProgress = async (connection, characterId) => {
    ...
    if (profile && Number(profile.unlocked_level) !== evolutionCap)
        await connection.execute('UPDATE player_evolution_profiles SET unlocked_level=?,updated_at=NOW() WHERE character_id=?', [evolutionCap, character.id]);
...
const evolutionStatBonuses = async (connection, characterId) => {
    ...
    return mergeBonus(jsonObject(profile.fixed_bonus_json), ...rows.map(row => {
        ...
        if (configured && isConditionalMutation(configured.description) && !dynamicDamageMutationCodes.has(configured.code) && !dynamicDamageReductionMutationCodes.has(configured.code)) {
            for (const key of Object.keys(configured.effect))
                delete effect[key];
        }
        if (configured && dynamicDamageMutationCodes.has(configured.code))
...
const injectEvolutionOnConnection = async (connection, qqUserId, code, requestedPart, requestedSymbiosisTrait, adminTest = false) => {
    ...
    if (Number(character.level) !== currentCap || (!adminTest && Number(character.experience) < experienceRequiredForLevel(currentCap)))
    ...
    if (code === 'shaping') {
        ...
        if (adminTest && selectedIds.length !== required)
            selectedIds.splice(0, selectedIds.length, ...candidates.slice(0, required).map(trait => Number(trait.id)));
        if (selectedIds.length !== required)
            throw new Error(required ? `请先在自我定义中选择 ${required} 项定型特征。` : '尚无稳定观测可供定型；可直接完成最后一次注射。');
        if (adminTest)
            await connection.execute('UPDATE player_evolution_profiles SET final_traits_json=? WHERE character_id=?', [JSON.stringify(selectedIds), character.id]);
...
const simulateEvolutionToLevel30 = async (connection, qqUserId) => {
    ...
    if (startingLevel > 30 || Number(character.realm_stage) > 3)
        throw new Error('测试二转只支持不高于 Lv.30、开化及以前的角色。');
    if (startingLevel < 20) {
        ...
        for (let level = startingLevel + 1; level <= 20; level++)
            await recordCharacterOperation(connection, { characterId: Number(character.id), kind: 'realm.level_up', source: { system: 'admin_profession_test', id: level, step: 'reached' }, actorRole: 'admin', outcome: '升级', summary: `管理测试模拟升至 Lv${level}`, detail: { fromLevel: level - 1, toLevel: level, adminTest: true } });
    ...
    if (cap < 20 || cap > 30 || Math.max(20, startingLevel) !== cap)
        throw new Error('角色等级与已有生长结进度不一致，请先核查进化档案。');
    for (let level = cap; level < 30; level++) {
        const code = level === 29 ? 'shaping' : level <= 23 ? ['conservative', 'aggressive', 'harmonic'][Math.floor(Math.random() * 3)] : level <= 26 ? ['conservative', 'aggressive', 'harmonic', 'perception', 'symbiosis'][Math.floor(Math.random() * 5)] : ['conservative', 'aggressive', 'harmonic', 'perception', 'symbiosis', 'metamorphosis'][Math.floor(Math.random() * 6)];
...
const claimEvolutionObservation = async (qqUserId) => withTransaction(async (connection) => {
    ...
    const [ordinaryRows] = codes.length
        ? await connection.execute(`SELECT code,item_type,rarity FROM item_definitions WHERE code IN (${codes.map(() => '?').join(',')})`, codes)
        : [[]];
    ...
    for (const [code, rawQuantity] of Object.entries(items)) {
        ...
        if (ordinaryCodes.has(code) && ordinaryBonusPct)
            remainder = Math.max(0, exactQuantity - quantity);
        if (!quantity)
            continue;
...
const setMutationPaused = async (qqUserId, mutationId, paused) => withTransaction(async (connection) => {
    ...
    const [mutationEvent] = await connection.execute(`INSERT INTO player_events (player_id,event_type,payload) SELECT player_id,?,? FROM characters WHERE id=?`, [kind, JSON.stringify({ mutationId, code: mutation.mutation_code }), character.id]);
...
const stabilizeMutation = async (qqUserId, mutationId) => withTransaction(async (connection) => {
    ...
    if (!['deviation', 'paused'].includes(mutation.mutation_state) && !hasClearableNegative)
        throw new Error('这份变异没有可清除的附带负面。');
    ...
    const [mutationEvent] = await connection.execute(`INSERT INTO player_events (player_id,event_type,payload) SELECT player_id,'evolution.mutation_stabilized',? FROM characters WHERE id=?`, [JSON.stringify({ mutationId, code: mutation.mutation_code }), character.id]);
...
const archiveMutation = async (qqUserId, mutationId) => withTransaction(async (connection) => {
    ...
    const [mutationEvent] = await connection.execute(`INSERT INTO player_events (player_id,event_type,payload) SELECT player_id,'evolution.mutation_archived',? FROM characters WHERE id=?`, [JSON.stringify({ mutationId, code: mutation.mutation_code }), character.id]);
