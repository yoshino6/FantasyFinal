...
const simulateEvolutionToLevel30 = async (connection, qqUserId) => {
    const character = await characterFor(connection, qqUserId, true);
    if (Number(character.level) > 30 || Number(character.realm_stage) > 3)
    ...
    if (Number(character.level) < 20) {
        ...
        for (let level = Number(character.level) + 1; level <= 20; level++)
    ...
    if (cap < 20 || cap > 30 || Math.max(20, Number(character.level)) !== cap)
