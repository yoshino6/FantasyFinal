...
const recordWebOperation = async (input, connection) => {
    ...
    if (input.target)
        await db.execute(`INSERT INTO operation_targets
...
const webOperationJournal = async (page = 1, keyword = '') => {
    ...
    const [countRows] = await pool.execute(`SELECT COUNT(*) AS total FROM operation_journals j LEFT JOIN operation_targets t ON t.operation_id=j.id ${where}`, values);
    const total = Number(countRows[0]?.total ?? 0);
    const totalPages = Math.max(1, Math.ceil(total / 30));
    const safePage = Math.min(currentPage, totalPages);
    const [rows] = await pool.execute(`SELECT j.id,j.correlation_id,j.actor_ref,j.action_type,j.status,j.risk_level,j.reason,j.created_at,t.target_kind,t.target_id
...
export { recordWebOperation, webOperationJournal };
