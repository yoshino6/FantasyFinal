...
import { advancedProfessionInheritanceCodes, worldTreeAdvancedProfessions, activeSkillCodesForAdvancedProfession, advancedProfessionByCode, advancedProfessionByMentor, inheritancePassiveFor, advancedInheritanceSkillCode } from './advanced-profession.config.js';
...
import { hiddenSkills, hiddenProfessions, hiddenPassiveCode } from './hidden-profession.config.js';
...
const advancedProfessionView = async (qqUserId, mentorCode) => {
    ...
    const [cores] = await pool.execute('SELECT pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code=? LIMIT 1', [character.id, profession.route.materialCode]);
...
const beginAdvancedProfession = async (qqUserId, code, replaceActiveQuest = false) => withTransaction(async (connection) => {
    ...
    if (otherQuest) {
        ...
        await recordCharacterOperation(connection, { characterId: Number(character.id), kind: 'profession.advanced_quest_abandoned', source: { system: 'advanced_profession_quest', id: randomUUID(), step: 'abandoned' }, outcome: '中断', summary: `中断${advancedProfessionByCode(otherQuest.profession_code)?.name ?? otherQuest.profession_code}试炼`, detail: { professionCode: otherQuest.profession_code, stage: Number(otherQuest.stage) } });
    }
...
const advanceAdvancedProfessionStage = async (qqUserId, code) => withTransaction(async (connection) => {
    ...
    await recordCharacterOperation(connection, { characterId: Number(character.id), kind: 'profession.advanced_quest_stage', source: { system: 'advanced_profession_quest', id: randomUUID(), step: 'stage_2' }, outcome: '推进', summary: `提交${profession.name}试炼第一段见闻`, detail: { professionCode: code, stage: 2, storyKills: Number(quest.story_kills) } });
    return profession;
});
const submitAdvancedProfessionProof = async (qqUserId, code) => withTransaction(async (connection) => {
    ...
    const [cores] = await connection.execute('SELECT pi.item_id,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code=? FOR UPDATE', [character.id, profession.route.materialCode]);
    if (Number(cores[0]?.quantity ?? 0) < profession.second.materialCount)
        throw new Error(`还需 ${profession.second.materialCount - Number(cores[0]?.quantity ?? 0)} 个【${profession.route.materialName}】。`);
...
const beginAdvancedProfessionTrial = async (qqUserId, code) => withTransaction(async (connection) => {
    ...
    if (existing[0]) {
        const [active] = await connection.execute("SELECT 1 FROM combat_targets ct JOIN combat_sessions cs ON cs.id=ct.session_id WHERE ct.spawn_id=? AND cs.state='active' LIMIT 1", [existing[0].id]);
        if (active.length)
            throw new Error('导师正在战斗，请先结束当前试炼。');
        if (Number((typeof existing[0].traits_json === 'string' ? JSON.parse(existing[0].traits_json) : existing[0].traits_json).find((trait) => trait.code === 'advanced_mentor_build')?.build?.version ?? 0) < build.version)
...
const completeAdvancedProfessionTrial = async (connection, characterId, trialCode) => {
    ...
    for (const code of [profession.passive.code, advancedInheritanceSkillCode(profession.code)])
...
export { advanceAdvancedProfessionStage, advancedProfessionView, beginAdvancedProfession, beginAdvancedProfessionTrial, completeAdvancedProfessionTrial, isWorldTreeAdvancedMentor, recordAdvancedProfessionKills, revokeAdvancedProfessionSkills, submitAdvancedProfessionProof };
