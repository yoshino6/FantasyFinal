...
import { calculateHeartGrowthChange, heartOffsetAfterChoice } from './heart-question-rules.js';
...
const heartGrowthAdjustment = async (connection, characterId) => {
    const [rows] = await connection.execute('SELECT delta_json,offset_json,birth_json FROM character_heart_growth WHERE character_id=?', [characterId]);
...
const applyHeartGrowthToRow = async (connection, characterId, row) => {
    ...
    for (const key of attributes) {
        adjusted[key] = (Number(row[key] ?? 0) + profile.offset[key]);
...
const createHeartQuestionsForLevels = async (connection, characterId, fromLevel, toLevel, realmStage) => {
    if (realmStage < 2 || toLevel <= fromLevel)
        return;
...
const activeHeartQuestion = async (userId) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT q.* FROM character_heart_questions q JOIN characters c ON c.id=q.character_id JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? AND q.status='active' ORDER BY q.id LIMIT 1`, [userId]);
    return rows[0] ? ticketView(rows[0]) : null;
};
const pendingHeartQuestionCount = async (userId) => {
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT COUNT(*) AS count FROM character_heart_questions q JOIN characters c ON c.id=q.character_id JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? AND q.status IN ('active','queued','deferred')`, [userId]);
    return Number(rows[0]?.count ?? 0);
};
const openHeartQuestion = async (userId) => withTransaction(async (connection) => {
    const [rows] = await connection.execute('SELECT c.id FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1 FOR UPDATE', [userId]);
    const characterId = Number(rows[0]?.id ?? 0);
    if (!characterId)
        return null;
    const [tickets] = await connection.execute("SELECT * FROM character_heart_questions WHERE character_id=? AND status IN ('active','queued','deferred') ORDER BY CASE status WHEN 'active' THEN 0 WHEN 'queued' THEN 1 ELSE 2 END,id LIMIT 1 FOR UPDATE", [characterId]);
...
const openQueuedHeartQuestion = async (userId) => withTransaction(async (connection) => {
    ...
    if (!characterId)
        return null;
    const [active] = await connection.execute("SELECT * FROM character_heart_questions WHERE character_id=? AND status='active' ORDER BY id LIMIT 1 FOR UPDATE", [characterId]);
    if (active[0])
        return ticketView(active[0]);
    const [tickets] = await connection.execute("SELECT * FROM character_heart_questions WHERE character_id=? AND status='queued' ORDER BY id LIMIT 1 FOR UPDATE", [characterId]);
    const ticket = tickets[0];
    if (!ticket)
        return null;
    await connection.execute("UPDATE character_heart_questions SET status='active' WHERE id=?", [ticket.id]);
...
export { activeHeartQuestion, answerHeartQuestion, answerTestHeartQuestions, applyHeartGrowthToRow, createHeartQuestionsForLevels, ensureHeartGrowth, heartAttributeCorrection, heartAttributeNames, heartGrowthAdjustment, openHeartQuestion, openQueuedHeartQuestion, pendingHeartQuestionCount, skipHeartQuestion };
