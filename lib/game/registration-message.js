...
const completedRegistrationFormat = async (user) => {
    ...
    const [rows] = await pool.execute(`SELECT ev.payload FROM player_events ev
    ...
    if (story && gift && record?.region)
        return giftSelectionFormat({ giftName: gift.name, regionName: record.region });
