...
import { openingStatus } from './opening.service.js';
import { openingFormat } from './opening-message.js';
import { talentDefinitions } from './talent.config.js';

const giftSelectionFormat = (character) => Format.create()
...
const completedRegistrationFormat = async (user) => {
    const story = await openingStatus(user);
    if (story && story.state !== 'armed')
        return openingFormat(story);
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT ev.payload,r.name AS current_region_name FROM player_events ev
    ...
    if (story && gift && rows[0]?.current_region_name)
        return giftSelectionFormat({ giftName: gift.name, regionName: String(rows[0].current_region_name) });
...
export { completedRegistrationFormat, giftSelectionFormat };
