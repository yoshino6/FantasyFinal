...
const currentMainQuest = async (qqUserId, skipLamplight = false) => {
    const opening = await openingMainQuest(qqUserId);
    if (opening)
        return opening;
    if (!skipLamplight) {
        const lamplight = await (await import('./lamplight.service.js')).lamplightMainQuest(qqUserId);
        if (lamplight)
            return lamplight;
    }
    ...
    if (!Number(character.owns_forest_map))
        return {
            title: '【主线·探索的准备】',
    ...
    if (gratitudeCompleted && Number(character.realm_stage) === 2 && level >= 20 && experience >= experienceRequiredForLevel(20)) {
        ...
        if (evolutionStage === 0)
            return { title: '【主线·未知的枷锁】', description: '二十级的经验已然圆满，力量却像被一道从未见过的门槛拦住。去冒险者公会问问，或许莫妮卡能找到些线索。', action: { label: '[前往 冒险者公会]', command: '/前往 -2 -181' } };
        if (evolutionStage === 1)
            return { title: '【主线·未知的枷锁】', description: '莫妮卡没有见过这种停滞。她只在一张陈旧借阅条上找到一点模糊线索：世界图书馆或许藏有答案。前往世界树，在图书馆里继续追查。', action: { label: '[前往 世界图书馆]', command: '/前往 -4 5' } };
        if (evolutionStage < 5)
...
const evolutionCharacterFor = async (connection, qqUserId, lock = false) => {
    ...
    if (!character)
        throw new Error('请先注册角色。');
    const [lamplight] = await connection.execute("SELECT 1 FROM player_lamplight_progress WHERE character_id=? AND (phase IN ('world','completed') OR phase='join' AND node_index>=3)", [character.id]);
    if (!lamplight.length) {
        if (Number(character.goblin_quest_stage) < 11)
            throw new Error('先完成「失踪的少女」，再处理这道新的瓶颈。');
        if (Number(character.gratitude_stage) < 6)
            throw new Error('先完成与梨子喵在世界树上的同行，再处理这道新的瓶颈。');
    }
