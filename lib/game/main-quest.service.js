...
import { girlGratitudeMainQuest } from './girl-gratitude.service.js';
import { activateEvolutionProfile } from './evolution.service.js';
import { guildCareerMainQuest } from './career-quest.service.js';
import { openingMainQuest } from './opening.service.js';
import { floatingStoryMainQuest } from './floating-leaf.service.js';
import { floatingRescueTexts } from './floating-leaf-content.js';
...
const barrierStage = (value) => Math.min(4, Math.max(0, Number(value) || 0));
const relativeDirection = (fromX, fromY, toX, toY) => {
...
const goblinQuestFor = async (connection, characterId, lock = false) => {
    const [rows] = await connection.execute(`SELECT stage,goblin_kills,region_id,pos_x,pos_y,pos_z,encounter_id,boss_spawn_id
    FROM player_goblin_king_quest WHERE character_id=? LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [characterId]);
    return rows[0] ?? null;
};
const evolutionQuestFor = async (connection, characterId, lock = false) => {
    const [rows] = await connection.execute(`SELECT stage FROM player_main_quest_progress WHERE character_id=? AND quest_code=? LIMIT 1${lock ? ' FOR UPDATE' : ''}`, [characterId, evolutionQuestCode]);
    return Number(rows[0]?.stage ?? 0);
};
const writeEvolutionStage = (connection, characterId, stage) => connection.execute('INSERT INTO player_main_quest_progress (character_id,quest_code,stage) VALUES (?,?,?) ON DUPLICATE KEY UPDATE stage=VALUES(stage),updated_at=NOW()', [characterId, evolutionQuestCode, stage]);
const recordQuestStage = (connection, characterId, quest, stage, summary, detail = {}) => recordCharacterOperation(connection, {
...
const requireQuestNpc = async (connection, characterId, code) => {
    const [rows] = await connection.execute('SELECT 1 FROM characters c JOIN map_npcs n ON n.region_id=c.current_region_id AND n.pos_x=c.pos_x AND n.pos_y=c.pos_y AND n.pos_z=c.pos_z WHERE c.id=? AND n.code=? LIMIT 1', [characterId, code]);
    if (!rows[0])
        throw new Error('请先前往对应人物处，再继续推进主线。');
};
const ordinaryTrait = { code: 'ordinary', name: '普通的' };
const questBossTrait = (ownerCharacterId, encounterId) => ({ code: 'main_quest_goblin_king', name: '', owner_character_id: ownerCharacterId, encounter_id: encounterId });
const evolutionBossTrait = (ownerCharacterId) => ({ code: 'main_quest_evolution', name: '', owner_character_id: ownerCharacterId });
...
const monsterHp = (template, level, bossCore) => monsterCombatStats({ ...template, level, traits_json: [evolutionBossTrait(0)]  }).hpMax;
const createGoblinKingEncounter = async (connection, ownerCharacterId, quest) => {
    if (quest.region_id === null || quest.pos_x === null || quest.pos_y === null || quest.pos_z === null || !quest.encounter_id)
        throw new Error('讨伐坐标尚未准备好。');
    const codes = ['goblin_king', 'habadragon', 'goblin_royal_guard', 'goblin_royal_spearman'];
    const [rows] = await connection.execute(`SELECT id,code,monster_class,level,constitution,spirit,strength,intelligence,agility,perception,constitution_growth,spirit_growth,strength_growth,intelligence_growth,agility_growth,perception_growth,skill_sequence
    FROM monster_templates WHERE code IN (${codes.map(() => '?').join(',')}) FOR UPDATE`, codes);
    const byCode = new Map(rows.map(row => [row.code, row]));
    if (codes.some(code => !byCode.has(code)))
        throw new Error('哥布林国王的遭遇配置尚未完成。');
    const create = async (code, role, level, isBossCore) => {
        const template = byCode.get(code);
        const traits = [...([ordinaryTrait] ), questBossTrait(ownerCharacterId, quest.encounter_id), { code: 'kingbeast_encounter', name: '', groupId: quest.encounter_id, role }];
    ...
    await create('habadragon', 'dragon', 18);
    const kingSpawnId = await create('goblin_king', 'king', 18);
...
const currentMainQuest = async (qqUserId, _skipLamplight = false) => {
    const maps = await (await import('./progression-map.service.js')).repairProgressionMaps(qqUserId);
    ...
    if (/^\/前往地图 /.test(command) || /^\/前往 -?\d+ -?\d+ 0$/.test(command)) {
        ...
        const [positions] = await connection.execute('SELECT r.code FROM characters c JOIN players p ON p.id=c.player_id JOIN map_regions r ON r.id=c.current_region_id WHERE p.qq_user_id=? LIMIT 1', [qqUserId]);
    ...
    if (maps.granted.length || maps.stored.length || maps.unavailable.length)
        quest.description += '\n\n' + (await import('./progression-map.service.js')).progressionMapReceipt(maps);
    return quest;
};
const currentMainQuestAfterMapRepair = async (qqUserId, _skipLamplight = false) => {
    ...
    if (['awaiting_arrival', 'arrival_story', 'guild_story'].includes(character.forest_status ?? ''))
        return {
            title: '【主线·前往百纳镇】',
            description: '森林史莱姆的战斗已经结束。继续入镇剧情，跟随梨子喵认识百纳镇，前往冒险者公会。',
            action: { label: '[继续 剧情]', command: '/继续剧情' }
        };
    if (['joined', 'declined'].includes(character.forest_status ?? ''))
        return {
            title: '【主线·讨伐森林史莱姆】',
            description: '与莱昂、伊芙和希娅并肩击败眼前的森林史莱姆。战斗结束后，他们会带你返回百纳镇。',
            action: { label: '[继续战斗]', command: '/继续剧情' }
        };
    if (character.forest_status !== 'completed')
        return {
            title: '【主线·寻找出路】',
    ...
    if (floatingQuest)
        return floatingQuest;
    const worldtreeQuest = await (await import('./worldtree-witness.service.js')).worldtreeWitnessMainQuest(qqUserId);
    if (worldtreeQuest)
        return worldtreeQuest;
    ...
    if (Number(character.realm_stage) === 1 && level >= 10 && experience >= experienceRequiredForLevel(10)) {
        ...
        if (stage === 0)
            return { title: '【主线·无形的禁锢】', description: '体内的力量已经积蓄到极限，却被一道看不见的屏障牢牢挡住。\n前往百纳镇的糖水屋，向老板请教这种异常。', action: { label: '[前往 糖水屋]', command: '/前往 -12 -196 0' } };
        if (stage === 1)
            return { title: '【主线·寻访晴儿】', description: '前台小姐姐建议你去找炼金师晴儿。\n前往糖水屋，询问这道无形的禁锢。', action: { label: '[前往 糖水屋]', command: '/前往 -12 -196 0' } };
        if (stage === 2 && !Number(character.owns_sky_dust))
            return { title: '【主线·追寻天空粉尘】', description: '击败幽影狼王，收集一份【天空粉尘】。\n它或许能帮助你感悟这方世界。' };
        if (stage === 2 || stage === 3)
            return { title: '【主线·归还天空粉尘】', description: '你已获得【天空粉尘】。\n回到糖水屋，把它交给晴儿看看。', action: { label: '[前往 糖水屋]', command: '/前往 -12 -196 0' } };
    ...
    if (gratitudeCompleted && Number(character.realm_stage) === 2 && level >= 20 && experience >= experienceRequiredForLevel(20)) {
        ...
        if (evolutionStage === 0)
            return { title: '【主线·未知的枷锁】', description: gratitudeCompleted ? '二十级的经验已然圆满，力量却被新的门槛拦住。梨子喵送我的百纳镇地图还在包里；去那里的冒险者公会请教莫妮卡，或许能找到线索。' : '二十级的经验已然圆满，力量却像被一道从未见过的门槛拦住。去冒险者公会问问，或许莫妮卡能找到些线索。', action: gratitudeCompleted && character.region_code !== 'baina_town' ? { label: '[前往 百纳镇]', command: '/前往地图 map_baina_town' } : { label: '[前往 冒险者公会]', command: '/前往 -2 -181 0' } };
        if (evolutionStage === 1)
            return { title: '【主线·未知的枷锁】', description: '莫妮卡没有见过这种停滞。她只在一张陈旧借阅条上找到一点模糊线索：世界图书馆或许藏有答案。前往世界树，在图书馆里继续追查。', action: character.region_code === 'world_tree' ? { label: '[前往 世界图书馆]', command: '/前往 -4 5 0' } : { label: '[前往 世界树]', command: '/前往地图 map_world_tree' } };
        if (evolutionStage < 5)
            return { title: '【主线·寻访大学者】', description: '世界图书馆的线索仍未拼全。依次探查大厅、阅览室、资料室与休息室，循着那封封存信件留下的脉络寻找【噶】。', action: { label: '[前往 世界图书馆]', command: '/前往 -4 5 0' } };
        if (evolutionStage === 5)
            return { title: '【主线·寻访大学者】', description: '所有线索都指向世界图书馆的无尽回廊。那里每一道门后都是不同的知识与岁月。前往无尽回廊，寻访【噶】的研究室。', action: { label: '[前往 世界图书馆]', command: '/前往 -4 5 0' } };
        if (evolutionStage === 6)
            return { title: '【主线·信念的试炼】', description: '大学者【噶】已压制力量，等待你以战斗证明自己的信念。战败后可再次挑战。', action: { label: '[前往 世界图书馆]', command: '/前往 -4 5 0' } };
    ...
    if (Number(character.realm_stage) === 3 && Number(character.evolution_stage) >= 8) {
        ...
        if (level >= cap && experience >= experienceRequiredForLevel(cap))
            return { title: '【主线·开化·生长结】', description: `Lv.${cap} 的生长结已经形成。前往世界树上的演化研究室，制作并注射一支进化针剂。`, action: { label: '[前往 演化研究室]', command: '/前往 -6 7 0' } };
    ...
    const questDirection = goblinQuest?.pos_x !== null && goblinQuest?.pos_x !== undefined && goblinQuest.pos_y !== null && goblinQuest.pos_y !== undefined
        ...
        : '';
    if (stage === 0)
        return { title: '【主线·失踪的少女】', description: '最近哥布林频繁骚扰百纳镇，镇上更接连传出少女失踪的消息。直到有一天，梨子喵进森林打猎后也迟迟未归。\n\n前往冒险者公会，了解失踪事件的情报。', action: { label: '[前往 冒险者公会]', command: '/前往 -2 -181 0' } };
    if (stage === 1)
        return { title: '【主线·失踪的少女】', description: '公会的人手一时无法抽调。我不能坐等梨子喵在森林里失去消息。密林深处的异动让我觉得，那里恐怕藏着一头难以应付的大家伙。\n\n前往异工坊，向唯薇安请教深入森林的办法。', action: { label: '[前往 异工坊]', command: '/前往 6 -189 0' } };
    if (stage === 2)
        return { title: '【主线·失踪的少女】', description: '唯薇安确认，天位制裁仪能压低未知统领的力量。为了救出梨子喵，我决定以特价购下这台仿品。\n\n前往异工坊，购买【天位制裁仪（仿品）】。', action: { label: '[购买 天位制裁仪（200铜币）]', command: '/购买天位制裁仪' } };
    if (stage === 3)
        return { title: '【主线·失踪的少女】', description: '天位制裁仪（仿品）已经到手。我要立刻进入幽暗密林深处，循着哥布林活动的痕迹寻找梨子喵。\n\n这次行动只能由我带领的队伍触发。', action: { label: '[前往 幽暗密林深处]', command: '/前往地图 map_dark_forest_deep' } };
    if (stage === 4)
        return { title: '【主线·失踪的少女】', description: `【线索】从${questDirection}传来打斗声。\n\n可疑坐标位于你当前的${questDirection}：(${goblinQuest?.pos_x}, ${goblinQuest?.pos_y})。\n\n只有我带领的队伍能够发现这处遭遇。`, action: location };
    if (stage >= 5 && stage < 10)
        return { title: '【主线·失踪的少女】', description: `梨子喵还在哥布林的包围中苦战。我要撑住她，击退这些围剿闯入者的哥布林。\n\n遭遇坐标：幽暗密林深处（${goblinQuest?.pos_x}, ${goblinQuest?.pos_y}）。`, action: location };
    if (stage === 10)
        return { title: '【主线·失踪的少女】', description: `天位制裁仪已经消耗，哥布林国王与红鳞幼龙的等级被压制至 Lv.18。\n两名御前侍卫均为 Lv.16。\n\n讨伐坐标：幽暗密林深处（${goblinQuest?.pos_x}, ${goblinQuest?.pos_y}）\n战败不会使副本或任务消失，可再次挑战。`, action: location };
    return { title: '【主线·失踪的少女】', description: '哥布林大军已经溃散，所有失踪的少女都已获救。\n\n带她们回冒险者公会报平安。', action: { label: '[前往 冒险者公会]', command: '/前往 -2 -181 0' } };
};
const startGoblinKingQuest = async (qqUserId) => withTransaction(async (connection) => {
    const [rows] = await connection.execute('SELECT c.id,c.level,c.realm_stage FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? FOR UPDATE', [qqUserId]);
    const character = rows[0];
    if (!character)
        throw new Error('请先注册角色。');
    if (Number(character.level) < 11 || Number(character.realm_stage) < 2)
        throw new Error('完成天空粉尘突破并达到 Lv.11 后，才能接受这项征召。');
    ...
    if (current && Number(current.stage) > 0)
        throw new Error('你已经接受了「失踪的少女」。');
    ...
    await (await import('./progression-map.service.js')).ensureProgressionMaps(connection, Number(character.id));
...
const consultVivianForJudicator = async (qqUserId) => withTransaction(async (connection) => {
    const [rows] = await connection.execute('SELECT c.id FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? FOR UPDATE', [qqUserId]);
    const character = rows[0];
    if (!character)
        throw new Error('请先注册角色。');
    ...
    if (!quest || Number(quest.stage) !== 1)
        throw new Error('你暂时不需要向唯薇安询问这件事。');
...
const buyCelestialJudicator = async (qqUserId) => withTransaction(async (connection) => {
    const [rows] = await connection.execute('SELECT c.id,c.copper_coins FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? FOR UPDATE', [qqUserId]);
    const character = rows[0];
    if (!character)
        throw new Error('请先注册角色。');
    await requireQuestNpc(connection, Number(character.id), 'oddworkshop');
    ...
    if (!quest || Number(quest.stage) !== 2)
        throw new Error('先向唯薇安了解天位制裁仪。');
    if (Number(character.copper_coins) < 200)
        throw new Error(`铜币不足，还需要 ${200 - Number(character.copper_coins)} 铜币。`);
    const [regions] = await connection.execute(`SELECT deep.id,deep.min_x,deep.max_x,deep.min_y,deep.max_y,town.min_y AS town_min_y
    ...
    for (let attempt = 0; attempt < 80; attempt++) {
        ...
        const [occupied] = await connection.execute('SELECT 1 FROM monster_spawns WHERE region_id=? AND pos_x=? AND pos_y=? AND pos_z=0 AND defeated_at IS NULL LIMIT 1', [region.id, x, y]);
        if (!occupied[0]) {
    ...
    if (!foundCoordinate)
        throw new Error('暂时无法确定安全的讨伐坐标，请稍后再试。');
...
const goblinKingArrival = async (connection, characterId, regionId, regionCode, x, y, z) => {
    const quest = await goblinQuestFor(connection, characterId, true);
    if (!quest)
        return null;
    const [origins] = await connection.execute("SELECT 1 FROM player_opening_stories WHERE character_id=? AND route_code='M01' AND destination_code='floating_leaf_town' LIMIT 1", [characterId]);
    ...
    if (Number(quest.stage) === 3 && regionCode === 'dark_forest_deep') {
        ...
        if (leaf)
            return { clue: true, chapter: 1, text: `${floatingRescueTexts.trail}\n\n【线索】营地在${direction}，坐标（${quest.pos_x}，${quest.pos_y}）。` };
    ...
    if (Number(quest.region_id) !== regionId || Number(quest.pos_x) !== x || Number(quest.pos_y) !== y || Number(quest.pos_z) !== z)
        return null;
    if (Number(quest.stage) === 4) {
        ...
        if (leaf)
            return { clue: false, chapter: 2, text: floatingRescueTexts.found };
    ...
    if (![5, 6, 7].includes(Number(quest.stage)))
        return null;
    if (leaf)
        return { clue: false, chapter: Number(quest.stage) === 5 ? 2 : 3, text: Number(quest.stage) === 5 ? floatingRescueTexts.found : floatingRescueTexts.drums };
...
const continueGoblinKingArrival = async (qqUserId) => withTransaction(async (connection) => {
    const [rows] = await connection.execute('SELECT c.id,c.current_region_id,c.pos_x,c.pos_y,c.pos_z FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? FOR UPDATE', [qqUserId]);
    const character = rows[0];
    if (!character)
        throw new Error('请先注册角色。');
    const quest = await goblinQuestFor(connection, Number(character.id), true);
    if (!quest || ![5, 6, 7].includes(Number(quest.stage)))
        throw new Error('当前没有可继续的讨伐剧情。');
    if (Number(quest.region_id) !== Number(character.current_region_id) || Number(quest.pos_x) !== Number(character.pos_x) || Number(quest.pos_y) !== Number(character.pos_y) || Number(quest.pos_z) !== Number(character.pos_z))
        throw new Error('请回到讨伐坐标，再继续剧情。');
    const [origins] = await connection.execute("SELECT 1 FROM player_opening_stories WHERE character_id=? AND route_code='M01' AND destination_code='floating_leaf_town' LIMIT 1", [character.id]);
    const leaf = Boolean(origins[0]);
    ...
    const [devices] = await connection.execute('SELECT pi.item_id,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code=? FOR UPDATE', [character.id, celestialJudicatorCode]);
    if (!devices[0]?.quantity)
        throw new Error('缺少天位制裁仪（仿品），无法压制哥布林国王。');
...
const completeGoblinKingQuest = async (connection, targets) => {
    const trait = targets.flatMap(target => {
        try {
            return Array.isArray(target.traits_json) ? target.traits_json : JSON.parse(String(target.traits_json ?? '[]'));
        }
        catch {
            return [];
        }
    }).find((item) => item?.code === 'main_quest_goblin_king');
    const ownerId = Number(trait?.owner_character_id ?? 0);
    if (!ownerId)
        return false;
    const [completed] = await connection.execute('UPDATE player_goblin_king_quest SET stage=11,completed_at=NOW() WHERE character_id=? AND stage=10', [ownerId]);
    ...
    const [leaf] = await connection.execute("SELECT 1 FROM player_opening_stories WHERE character_id=? AND route_code='M01' AND destination_code='floating_leaf_town' LIMIT 1", [ownerId]);
    if (leaf[0]) {
        const [owned] = await connection.execute("SELECT 1 FROM player_inventory p JOIN item_definitions i ON i.id=p.item_id WHERE p.character_id=? AND i.code='map_baina_town' AND p.quantity>0", [ownerId]);
        if (!owned.length)
            await (await import('./opening.service.js')).grantOpeningItem(connection, ownerId, 'map_baina_town');
    }
    return true;
};
const evolutionCharacterFor = async (connection, qqUserId, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.id,c.name,c.level,c.experience,c.realm_stage,
    ...
    if (!character)
        throw new Error('请先注册角色。');
    if (Number(character.goblin_quest_stage) < 11)
        throw new Error('先完成哥布林国王讨伐，再处理这道新的瓶颈。');
    if (Number(character.gratitude_stage) < 6)
        throw new Error('先完成世界树的同行剧情，再处理这道新的瓶颈。');
    if (Number(character.realm_stage) !== 2 || Number(character.level) < 20 || Number(character.experience) < experienceRequiredForLevel(20))
        throw new Error('你的积累尚未触及这道新的灵阶枷锁。');
    return character;
};
const evolutionQuestStage = async (qqUserId) => withTransaction(async (connection) => {
    const [rows] = await connection.execute('SELECT c.id FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? LIMIT 1', [qqUserId]);
    const character = rows[0];
    if (!character)
        throw new Error('请先注册角色。');
    return evolutionQuestFor(connection, Number(character.id));
});
const advanceEvolutionQuest = async (qqUserId, source) => withTransaction(async (connection) => {
    ...
    const expected = { guild: 0, hall: 1, reading: 2, archive: 3, rest: 4 };
    if (stage !== expected[source])
...
const openGaStudy = async (qqUserId) => withTransaction(async (connection) => {
    const character = await evolutionCharacterFor(connection, qqUserId, true);
    const stage = await evolutionQuestFor(connection, Number(character.id), true);
    if (stage < 5 || stage > 6)
        throw new Error('你暂时还没有找到大学者【噶】的研究室。');
    await requireQuestNpc(connection, Number(character.id), 'world_library');
    const [templates] = await connection.execute(`SELECT id,code,monster_class,level,constitution,spirit,strength,intelligence,agility,perception,constitution_growth,spirit_growth,strength_growth,intelligence_growth,agility_growth,perception_growth,skill_sequence
    FROM monster_templates WHERE code='scholar_ga' LIMIT 1 FOR UPDATE`);
    const template = templates[0];
    if (!template)
        throw new Error('大学者【噶】的战斗配置尚未完成。');
    const [existing] = await connection.execute(`SELECT s.id FROM monster_spawns s WHERE s.defeated_at IS NULL
    ...
    if (existing[0]) {
        const [active] = await connection.execute('SELECT 1 FROM combat_targets ct JOIN combat_sessions cs ON cs.id=ct.session_id WHERE ct.spawn_id=? AND cs.state=? LIMIT 1 FOR UPDATE', [existing[0].id, 'active']);
        if (!active[0])
            await connection.execute('UPDATE monster_spawns SET current_hp=?,traits_json=? WHERE id=?', [monsterHp(template, 22, true), JSON.stringify([evolutionBossTrait(Number(character.id))]), existing[0].id]);
        if (stage === 5) {
    ...
    const [positions] = await connection.execute('SELECT region_id,pos_x,pos_y,pos_z FROM map_npcs WHERE code=? LIMIT 1', ['world_library']);
    const position = positions[0];
    if (!position)
        throw new Error('世界图书馆尚未开放。');
...
const completeEvolutionQuest = async (connection, targets) => {
    const trait = targets.flatMap(target => {
        try {
            return Array.isArray(target.traits_json) ? target.traits_json : JSON.parse(String(target.traits_json ?? '[]'));
        }
    ...
    }).find((item) => item?.code === 'main_quest_evolution');
    if (!trait?.owner_character_id)
        return false;
    const characterId = Number(trait.owner_character_id);
    if (await evolutionQuestFor(connection, characterId, true) !== 6)
        return false;
...
const contemplateEvolutionSeed = async (qqUserId) => withTransaction(async (connection) => {
    const character = await evolutionCharacterFor(connection, qqUserId, true);
    if (await evolutionQuestFor(connection, Number(character.id), true) !== 7)
        throw new Error('进化之种还没有准备好回应你的感悟。');
    const [items] = await connection.execute('SELECT pi.item_id,pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code=? FOR UPDATE', [character.id, 'evolution_seed']);
    if (!items[0]?.quantity)
        throw new Error('背包中没有进化之种。');
    if (Number(items[0].quantity) <= 1)
        await connection.execute('DELETE FROM player_inventory WHERE character_id=? AND item_id=?', [character.id, items[0].item_id]);
    else
    ...
    const [awakenedEvent] = await connection.execute(`INSERT INTO player_events (player_id,event_type,payload)
...
const contemplateSkyDust = async (qqUserId) => withTransaction(async (connection) => {
    ...
    if (!character)
        throw new Error('请先注册角色。');
    if (Number(character.realm_stage) !== 1)
        throw new Error('你已经完成初心境界的突破。');
    if (Number(character.level) < 10 || Number(character.experience) < experienceRequiredForLevel(10))
        throw new Error('你的积累尚未触及初心境界的枷锁。');
    if (!Number(character.owns_sky_dust))
        throw new Error('背包中没有可供感悟的天空粉尘。');
    if (barrierStage(character.barrier_stage) !== 4)
        throw new Error('先带着天空粉尘回去向晴儿请教，再开始窥探吧。');
...
const advanceRealmBarrier = async (qqUserId, source) => withTransaction(async (connection) => {
    const [rows] = await connection.execute(`
    ...
    FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? FOR UPDATE`, [qqUserId]);
    const character = rows[0];
    if (!character || Number(character.realm_stage) !== 1 || Number(character.level) < 10 || Number(character.experience) < experienceRequiredForLevel(10))
    ...
    return { previous: current, stage: next };
});

export { advanceEvolutionQuest, advanceRealmBarrier, buyCelestialJudicator, completeEvolutionQuest, completeGoblinKingQuest, consultVivianForJudicator, contemplateEvolutionSeed, contemplateSkyDust, continueGoblinKingArrival, currentMainQuest, evolutionQuestStage, goblinKingArrival, openGaStudy, startGoblinKingQuest };
