...
import { experienceRequiredForLevel, calculateDerivedStats } from './constants.js';
...
const monsterHp = (template, level, bossCore) => {
    const values = Object.fromEntries(attributes.map(attribute => [attribute, Math.floor(Number(template[attribute]) + Math.max(0, level - 1) * Number(template[`${attribute}_growth`]))]));
...
const currentMainQuest = async (qqUserId) => {
    ...
    if (!Number(character.has_appraisal))
        return {
            title: '【主线·初识鉴识】',
