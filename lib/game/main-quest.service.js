...
const currentMainQuest = async (qqUserId, _skipLamplight = false) => {
    ...
    if (gratitudeCompleted && Number(character.realm_stage) === 2 && level >= 20 && experience >= experienceRequiredForLevel(20)) {
        ...
        if (evolutionStage === 0)
            return { title: '【主线·未知的枷锁】', description: gratitudeCompleted ? '二十级的经验已然圆满，力量却被新的门槛拦住。梨子喵送我的百纳镇地图还在包里；去那里的冒险者公会请教莫妮卡，或许能找到线索。' : '二十级的经验已然圆满，力量却像被一道从未见过的门槛拦住。去冒险者公会问问，或许莫妮卡能找到些线索。', action: gratitudeCompleted && character.region_code !== 'baina_town' ? { label: '[前往 百纳镇]', command: '/前往地图 map_baina_town' } : { label: '[前往 冒险者公会]', command: '/前往 -2 -181' } };
        if (evolutionStage === 1)
            return { title: '【主线·未知的枷锁】', description: '莫妮卡没有见过这种停滞。她只在一张陈旧借阅条上找到一点模糊线索：世界图书馆或许藏有答案。前往世界树，在图书馆里继续追查。', action: character.region_code === 'world_tree' ? { label: '[前往 世界图书馆]', command: '/前往 -4 5' } : { label: '[前往 世界树]', command: '/前往地图 map_world_tree' } };
        if (evolutionStage < 5)
            return { title: '【主线·寻访大学者】', description: '世界图书馆的线索仍未拼全。依次探查大厅、阅览室、资料室与休息室，循着那封封存信件留下的脉络寻找【噶】。', action: { label: '[前往 世界图书馆]', command: '/前往 -4 5' } };
        if (evolutionStage === 5)
            return { title: '【主线·寻访大学者】', description: '所有线索都指向世界图书馆的无尽回廊。那里每一道门后都是不同的知识与岁月。前往无尽回廊，寻访【噶】的研究室。', action: { label: '[前往 世界图书馆]', command: '/前往 -4 5' } };
        if (evolutionStage === 6)
            return { title: '【主线·信念的试炼】', description: '大学者【噶】已压制力量，等待你以战斗证明自己的信念。战败后可再次挑战。', action: { label: '[前往 世界图书馆]', command: '/前往 -4 5' } };
    ...
    if (Number(character.realm_stage) === 3 && Number(character.evolution_stage) >= 8) {
        ...
        if (level >= cap && experience >= experienceRequiredForLevel(cap))
            return { title: '【主线·开化·生长结】', description: `Lv.${cap} 的生长结已经形成。前往世界树上的演化研究室，制作并注射一支进化针剂。`, action: { label: '[前往 演化研究室]', command: '/前往 -6 7' } };
