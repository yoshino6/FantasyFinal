import { dynamicWorldRegions, dynamicNpcDisplayName } from './world-dynamics.content.js';

const profiles = new Map(dynamicWorldRegions.flatMap(region => region.npcs.map(npc => {
    ...
    return [npc.code, { ...npc, displayName, regionName: region.name, homeName: home.name, homeSiteCode: home.code, meeting: `【${npc.role}】${displayName}停下脚步。${firstImpression}。` }];
})));
const legacyAliases = {
...
for (const [legacyCode, currentCode] of Object.entries(legacyAliases)) {
    const profile = profiles.get(currentCode);
    if (profile)
        profiles.set(legacyCode, profile);
}
const voice = (morning, afternoon, evening, caution, warmth, trust) => ({ morning, afternoon, evening, caution, warmth, trust });
const voices = {
...
const choose = (items, variant) => items[variant === undefined ? Math.floor(Math.random() * items.length) : Math.abs(variant) % items.length] ?? items[0];
const dynamicNpcProfile = (code) => profiles.get(code) ?? null;
const dynamicNpcVoiceAnchor = (code, affinity = 0, variant = 0) => {
    const npc = dynamicNpcProfile(code);
    if (!npc)
        return null;
    ...
    const notes = personalNotes[npc.code] ?? ['他把手边的物件逐一归位。', '他认真听完每一段来话。', '他把未完的事情逐一记下。'];
...
const dynamicNpcChatDialogue = (code, affinity, now = new Date(), variant) => {
    const npc = dynamicNpcProfile(code);
    if (!npc)
        return null;
    const hour = now.getHours();
    const period = hour < 11 ? 'morning' : hour < 18 ? 'afternoon' : 'evening';
    const relation = relationFor(affinity);
    const persona = voices[npc.code] ?? voice('把手边的物件逐一归位', '沿着巡游线确认每一处记号', '在灯下收好今日的记录', '陌生人的脚步也该有来处', '你肯把经过说完整，便值得我停一停', '若路真要断了，我会陪你找到另一头');
    const habit = speechHabits[npc.code] ?? { gesture: '他把手边的物件逐一归位', catchphrase: '路总得有人认真看着' };
    const notes = personalNotes[npc.code] ?? ['他把手边的物件逐一归位。', '他认真听完每一段来话。', '他把未完的事情逐一记下。'];
    const scenes = {
...
const auditDynamicNpcDialogues = () => {
    const expected = dynamicWorldRegions.flatMap(region => region.npcs.map(npc => npc.code));
    const missingVoices = expected.filter(code => !voices[code]);
    const missingHabits = expected.filter(code => !speechHabits[code]);
    const missingNotes = expected.filter(code => !personalNotes[code] || personalNotes[code].length < 3);
    ...
    for (const code of expected)
        for (const note of personalNotes[code] ?? []) {
            if (noteOwners.has(note))
                duplicateNotes.push(`${code} 与 ${noteOwners.get(note)}`);
            else
                noteOwners.set(note, code);
        }
    for (const code of expected)
        for (const [hour, affinity] of [[8, 0], [14, 50], [20, 200], [8, 500], [14, 2000]])
            for (const variant of [0, 1, 2]) {
                const text = dynamicNpcChatDialogue(code, affinity, new Date(2026, 0, 1, hour), variant) ?? '';
                if (samples.has(text))
                    duplicates.push(`${code} 与 ${samples.get(text)}`);
                else
                    samples.set(text, code);
                if (forbiddenPhrases.some(phrase => text.includes(phrase)))
                    forbidden.push(`${code}@${hour}/${affinity}`);
            }
...
export { auditDynamicNpcDialogues, dynamicNpcChatDialogue, dynamicNpcProfile, dynamicNpcVoiceAnchor };
