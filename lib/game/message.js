...
const npcInteractionMarkdown = (title, name, text, detailCode, detailsUnlocked = false) => {
    const markdown = Format.createMarkdown().addTitle(title).addNewline().addNewline().addText(`【${name}】`);
    if (detailCode && detailsUnlocked)
        markdown.addText('   ').addButton('[详情]', { data: `/域民详情 ${detailCode}`, autoEnter: false });
...
export { audienceFormat, audienceText, dangerFormat, dangerText, destinationFormat, destinationText, giftFormat, giftText, heavenFormat, heavenText, messageFormat, npcInteractionMarkdown, questionFormat, questionText, randomStoryText, sendWithTextFallback, storyFormat };
