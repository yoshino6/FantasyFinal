...
import { openingRouteByCode, openingNarrativeText, openingFirstMeetingText, openingNewcomerText, openingLessonText } from './opening-content.js';
import { openingHubs } from './opening-world.config.js';
import { openingWorldFor, openingSafeHubs } from './opening-state.js';
import { grantInventory, consumeInventory } from './inventory-binding.js';
import { grantOpeningExperience } from './opening-progress.service.js';
import { openingSpecialArrival } from './opening-special-arrivals.js';

const json = (value) => typeof value === 'string' ? JSON.parse(value) : value ?? {};
const openingCharacter = async (connection, user, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.*,r.code AS region_code FROM characters c JOIN players p ON p.id=c.player_id JOIN map_regions r ON r.id=c.current_region_id WHERE p.qq_user_id=?${lock ? ' FOR UPDATE' : ''}`, [user]);
    if (!rows[0])
        throw new Error('请先完成转生。');
    return rows[0];
};
const grantOpeningItem = async (connection, id, code, quantity = 1) => {
    const [items] = await connection.execute('SELECT id FROM item_definitions WHERE code=?', [code]);
    if (!items[0])
        throw new Error(`物品尚未初始化：${code}`);
...
const consumeCode = async (connection, id, code) => {
    const [rows] = await connection.execute('SELECT id FROM item_definitions WHERE code=?', [code]);
    if (!rows[0])
        throw new Error('所需物品不存在。');
    await consumeInventory(connection, id, Number(rows[0].id), 1);
};
const loadStory = async (connection, id, lock = false) => {
    const [rows] = await connection.execute(`SELECT * FROM player_opening_stories WHERE character_id=?${lock ? ' FOR UPDATE' : ''}`, [id]);
    return rows[0];
};
const erisPages = [{ title: '神界·又一次敲门', text: '再睁眼时，椅子上坐着银发的厄里斯。她没有露出惊讶，只先将一杯温水推到你面前。\n\n“欢迎回来。我知道，这句话现在并不合适。”\n\n她核对了你尚未消散的接引印。“旧接引点的保护还没有全部修好。那只守门兽也正在学习把欢迎动作改成挥帽子。这次先由我送你平安回去。”' },
...
const selectedChoice = (row) => openingRouteByCode(row.route_code, row.story_version).choices.find(c => c.code === row.branch_code);
const scenePages = (row) => {
    const route = openingRouteByCode(row.route_code, row.story_version);
    if (!route)
        throw new Error('这段初行故事的版本暂不可读取，请联系管理员。');
    const flags = json(row.flags_json);
    const choice = selectedChoice(row);
    if (row.state === 'armed')
        return [{ title: '初行之路', text: '天赋的光芒已经散去。先打开操作面板，看看自己落在何处；当你首次移动或寻怪时，眼前的相遇才会真正开始。' }];
    ...
    if (row.state === 'arrival') {
        if (row.destination_code !== route.destination)
            return [{ title: '接应改道', text: `引灯人先一步收到停航的消息，立刻请沿路值守改接安全通道。你没有被留在原地等待；${row.route_code === 'F02' && row.branch_code === 'B' ? '瑟芙菈收起王印，将你一路送到新的接引灯下，才把邀请函交回你手中。' : ''}\n\n${openingHubs[row.destination_code].description}\n\n原定的${openingHubs[route.destination].name}暂不接待，接引记录与奖励一同转到${openingHubs[row.destination_code].name}。` }];
        if (row.route_code === 'F02' && row.branch_code === 'B')
            return [
...
const view = (row) => {
    ...
    const page = ps[Math.min(Number(row.page_index), ps.length - 1)];
    const choice = selectedChoice(row);
    const flags = json(row.flags_json);
    const state = row.state === 'reading' && Number(row.page_index) >= ps.length - 1 ? 'choice' : row.state;
    let choices = route.choices.map(c => ({ code: c.code, label: c.label }));
    if (row.route_code === 'A01' && flags.eris)
        choices = [{ code: 'A', label: '接受厄里斯的安全返还' }, { code: 'B', label: '带事故函去地上的女神办事桌' }, { code: 'C', label: '留下厄里斯的个人受理印' }];
...
const openingStatus = async (user) => {
    const pool = await getPool();
    const [chars] = await pool.execute('SELECT c.id FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=?', [user]);
    if (!chars[0])
        return null;
    const row = await loadStory(pool, Number(chars[0].id));
    return row ? view(row) : null;
};
const beginOpening = async (user, entry = 'continue') => withTransaction(async (connection) => {
    const character = await openingCharacter(connection, user, true);
    const row = await loadStory(connection, Number(character.id), true);
    ...
    if (row.state === 'armed') {
        ...
        const flags = { ...json(row.flags_json), eris: world.current_goddess === 'eris' };
...
const keepRecord = async (connection, row, code, name, use, future) => {
...
const settleArrival = async (connection, row) => {
    const choice = selectedChoice(row);
    const flags = json(row.flags_json);
    let code = choice.rewardCode;
    let name = choice.rewardName;
    ...
    if (row.route_code === 'A01' && row.branch_code === 'B' && !flags.eris && world.current_goddess !== 'aqua') {
        flags.eris = true;
        flags.handoff = true;
        await connection.execute("UPDATE player_opening_stories SET state='choice',branch_code=NULL,page_index=0,flags_json=? WHERE character_id=?", [JSON.stringify(flags), row.character_id]);
        return false;
    }
    const safeHubs = await openingSafeHubs(connection, true);
    let destination = safeHubs.find(h => h.code === row.destination_code);
    if (!destination) {
        const [characters] = await connection.execute('SELECT pos_x,pos_y FROM characters WHERE id=?', [row.character_id]);
        ...
        if (!destination)
            throw new Error('所有安全接引点暂时关闭或无法使用。你仍受剧情保护，重新开放后可从本页继续。');
        row.destination_code = String(destination.code);
    ...
    if (row.route_code === 'S03' && row.branch_code === 'A') {
        const companion = await (await import('./companion.service.js')).grantWindbirdChick(connection, Number(row.character_id));
        name = `旅程变化：${companion}破壳后选择与你同行`;
    }
    else if (row.route_code === 'I02' && ['A', 'B'].includes(String(row.branch_code))) {
        const automaton = await (await import('./automaton.service.js')).grantOpeningAutomaton(connection, Number(row.character_id));
        name = `旅程变化：${automaton.state.name} #${automaton.id} 主动认主，以“${automaton.state.personality.coreName}”人格开始随行`;
    }
    else if (choice.rewardKind) {
        name = await (await import('./opening-rewards.service.js')).grantOpeningRouteReward(connection, Number(row.character_id), openingRouteByCode(row.route_code, row.story_version), choice);
    }
    else if (row.route_code === 'F01' && row.branch_code === 'A') {
        await (await import('./companion.service.js')).grantGoldenRabbit(connection, Number(row.character_id));
    }
        ...
        if (row.branch_code === 'A')
            await (await import('./opening-chest.service.js')).grantCrimsonArmor(connection, Number(row.character_id));
        else {
            await connection.execute('UPDATE characters SET copper_coins=copper_coins+10000 WHERE id=?', [row.character_id]);
            await keepRecord(connection, row, code, '魔界邀请函', '鉴物员查看后归还，凭它留下魔界见闻', '魔界的正式邀请');
        }
    }
    else if (choice.pack) {
        await grantOpeningItem(connection, Number(row.character_id), 'healing_herb', choice.pack === 'R医' ? 3 : 1);
        if (choice.pack === 'R契')
            await grantOpeningItem(connection, Number(row.character_id), 'opening_companion_feed', 3);
        await (await import('./opening-pack.service.js')).grantOpeningPackExtras(connection, Number(row.character_id), choice.pack);
    }
    else {
        if (row.route_code === 'A01' && flags.eris) {
            if (row.branch_code === 'A') {
                code = 'opening_eris_return';
                name = '厄里斯核验的返还单';
            }
            if (row.branch_code === 'B') {
                code = 'opening_aqua_letter';
                name = '给地上女神的未读事故函';
            }
        }
    ...
    flags.rewardName = name;
    flags.rewardCode = code;
...
const advanceOpening = async (user, revision, action) => withTransaction(async (connection) => {
    if (!Number.isSafeInteger(revision) || revision < 0 || !['next', 'lesson', 'A', 'B', 'C', 'treat'].includes(action))
        throw new Error('剧情操作无效，请重新打开当前剧情。');
    const character = await openingCharacter(connection, user, true);
    const row = await loadStory(connection, Number(character.id), true);
    if (!row)
        throw new Error('没有进行中的初行剧情。');
    const [prior] = await connection.execute('SELECT action_key,result_json FROM player_opening_actions WHERE character_id=? AND revision=?', [character.id, revision]);
    if (prior[0])
        return prior[0].action_key === action ? json(prior[0].result_json) : view(row);
    ...
    if (row.state === 'armed') {
        if (action !== 'next')
            throw new Error('请先从操作面板移动或寻怪，再展开初行故事。');
        return view(row);
    }
    else if (row.state === 'choice') {
        if (!route.choices.some(c => c.code === action))
            throw new Error('请从眼前的选项中选择。');
        const world = await openingWorldFor(connection);
        const flags = json(row.flags_json);
        if (row.route_code === 'A01' && !flags.eris && world.current_goddess === 'eris') {
            flags.eris = true;
            flags.handoff = true;
            await connection.execute("UPDATE player_opening_stories SET state='choice',flags_json=?,page_index=0 WHERE character_id=?", [JSON.stringify(flags), character.id]);
        }
        else {
        ...
        if (action !== 'lesson')
            throw new Error('请先读完当前的公会剧情。');
        if (row.route_code === 'F01' && row.branch_code === 'B') {
            const [opened] = await connection.execute("SELECT 1 FROM opening_chest_requests WHERE character_id=? AND chest_code='opening_golden_chest' AND state='complete' LIMIT 1", [character.id]);
            if (!opened.length)
                throw new Error('请先打开黄金宝箱，再进行装备教学。');
        }
        if (row.route_code === 'F01' && row.branch_code === 'A')
            await (await import('./companion.service.js')).openingFeedRabbit(connection, Number(character.id));
        await grantOpeningExperience(connection, Number(character.id), 'register');
        if (route.code === 'F01' || route.code === 'F02' && row.branch_code === 'A' || selectedChoice(row)?.pack || selectedChoice(row)?.rewardKind)
            await grantOpeningExperience(connection, Number(character.id), 'lesson');
        ...
        if (row.state === 'branch' && row.route_code === 'F02' && row.branch_code === 'B' && Number(row.page_index) === ps.length - 1) {
            if (action !== 'treat')
                throw new Error('请确认敷上微光草药后继续。');
            await consumeCode(connection, Number(character.id), 'healing_herb');
        }
        else if (action !== 'next')
            throw new Error('请按当前剧情继续。');
        if (row.state === 'reading' && Number(row.page_index) + 1 >= ps.length - 1)
            await connection.execute("UPDATE player_opening_stories SET state='choice',page_index=? WHERE character_id=?", [ps.length - 1, character.id]);
        else if (Number(row.page_index) < ps.length - 1)
            await connection.execute('UPDATE player_opening_stories SET page_index=page_index+1 WHERE character_id=?', [character.id]);
        else if (row.state === 'reading')
            await connection.execute("UPDATE player_opening_stories SET state='choice' WHERE character_id=?", [character.id]);
        else if (row.state === 'branch')
            await settleArrival(connection, row);
        else
            await connection.execute("UPDATE player_opening_stories SET state='lesson',page_index=0 WHERE character_id=?", [character.id]);
    }
    else
        throw new Error('当前剧情已经结束，或尚未开始。');
    await connection.execute('UPDATE player_opening_stories SET revision=revision+1 WHERE character_id=?', [character.id]);
    const updated = (await loadStory(connection, Number(character.id)));
    const result = view(updated);
    if (json(updated.flags_json).handoff && updated.state === 'choice')
        result.text = '厄里斯接过了接引名册：“前辈刚刚随另一位旅人下去了。你的物品与恩赐都在，请重新选择这次返还的方式。”';
...
const openingMainQuest = async (user) => {
    const status = await openingStatus(user);
    if (!status)
        return null;
    if (status.state === 'completed')
        return (await import('./opening-progress.service.js')).openingFollowupQuest(user);
...
export { advanceOpening, beginOpening, grantOpeningItem, openingCharacter, openingMainQuest, openingStatus };
