...
const scenePages = (row) => {
    ...
    if (row.state === 'arrival') {
        ...
        if (row.destination_code !== route.destination)
            return [{ title: '安全改道', text: `原定通往${openingHubs[route.destination].name}的道路临时关闭。初行保护在最近的安全节点开启了单向传送门，将我直接送到${openingHubs[row.destination_code].name}公会门前。\n\n${openingHubs[row.destination_code].description}\n\n路线奖励仍按我亲历的选择结算，没有因改道增加额外剧情。${mapReceived}` }];
        const pages = (choice?.arrival ?? route.arrival).map(page => ({ ...page, text: openingNewcomerText(page.text) }));
        if (mapReceived && pages.length)
            pages[pages.length - 1] = { ...pages[pages.length - 1], text: pages[pages.length - 1].text + mapReceived };
        return pages;
    }
    if (row.state === 'lesson')
...
const view = (row) => {
    ...
    const pageIndex = row.state === 'choice' ? ps.length - 1 : Math.min(Number(row.page_index), ps.length - 1);
    const page = ps[pageIndex];
    const choice = selectedChoice(row);
    const flags = json(row.flags_json);
...
const settleArrival = async (connection, row) => {
    ...
    if (row.route_code === 'S03' && row.branch_code === 'A') {
        ...
        code = 'opening_s03_rescue_reward';
        name = '2000铜币';
    }
...
const completeOpening = async (connection, row) => {
...
const advanceOpening = async (user, revision, action) => withTransaction(async (connection) => {
    ...
    if (row.state === 'completed')
        return view(row);
    const [prior] = await connection.execute('SELECT action_key FROM player_opening_actions WHERE character_id=? AND revision=?', [character.id, revision]);
    if (prior[0])
        return view(row);
    ...
    if (row.state === 'armed') {
        ...
        if (row.state === 'branch' && row.route_code === 'F02' && row.branch_code === 'B' && Number(row.page_index) === ps.length - 1) {
            ...
            await consumeCode(connection, Number(character.id), 'healing_herb');
            await recordCharacterOperation(connection, { characterId: Number(character.id), kind: 'opening.treated', source: { system: 'opening_story', id: character.id, step: `treat:${revision}` }, outcome: '处理', summary: '在初行中使用草药处理伤势', detail: { routeCode: row.route_code, branchCode: row.branch_code, consumedItemCode: 'healing_herb' } });
        }
        else if (action !== 'next')
        ...
        if (row.state === 'reading' && Number(row.page_index) + 1 >= ps.length - 1)
            ...
            if (row.route_code === 'F03') {
                const flags = json(row.flags_json);
                flags.forestBattlePending = row.branch_code === 'A' ? 'join' : 'depart';
...
const completeOpeningForestBattleStart = async (user) => withTransaction(async (connection) => {
    const character = await openingCharacter(connection, user, true);
    const row = await loadStory(connection, Number(character.id), true);
    if (!row || row.route_code !== 'F03')
        throw new Error('当前没有待开始的三人冒险团战斗。');
    const flags = json(row.flags_json);
    const choice = selectedChoice(row);
    if (row.state === 'completed')
        return String(flags.rewardName ?? choice?.rewardName ?? '');
    if (row.state !== 'branch' || !choice || !flags.forestBattlePending)
        throw new Error('三人冒险团尚未准备好迎战。');
    if (!row.reward_claimed) {
        ...
        flags.rewardName = choice.rewardName;
        flags.rewardCode = choice.rewardCode;
    }
    delete flags.forestBattlePending;
    flags.forestBattleStarted = true;
...
export { advanceOpening, beginOpening, completeOpeningForestBattleStart, grantOpeningItem, openingCharacter, openingMainQuest, openingStatus };
