...
const settleTalentPurification = async (connection, characterId, input) => {
    ...
    if (risk) {
        ...
        if (items.length !== 2 || !items.every(ordinaryTalentItem))
            throw new Error('孤注提纯仅支持普通主材和普通产物，请先关闭孤注制作。');
    }
    const payment = await consumeTalentMaterial(connection, characterId, input.id, input.quantity, 'craft', input.personal);
    const failed = { personal: 0, trade: 0, unbound: 0 };
    ...
    for (let index = 0; index < input.quantity; index++) {
        if (Math.random() * 100 < input.success * (risk ? .7 : 1)) {
            ...
            if (Math.random() < input.valueMultiplier % 1)
                outputQuantity++;
        }
