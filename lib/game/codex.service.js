...
const codexKinds = ['装备', '道具', '材料', '怪物', '技能'];
const categories = {
...
const categoryExpression = (kind) => {
    if (kind === '装备')
        return "CASE WHEN i.item_category='副手' THEN '武器' WHEN i.item_category IN ('头部','眼部') THEN '头肩' ELSE i.item_category END";
    if (kind === '道具' || kind === '材料')
        return 'i.item_category';
    if (kind === '怪物')
        return "CASE t.monster_class WHEN 'normal' THEN '普通' WHEN 'large' THEN '大型' WHEN 'elite' THEN '精英' WHEN 'boss' THEN '首领' ELSE '特殊' END";
...
const characterFor = async (qqUserId) => {
    const [rows] = await (await getPool()).execute('SELECT c.id,c.level FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? AND c.npc_code IS NULL LIMIT 1', [qqUserId]);
    if (!rows[0])
        throw new Error('请先完成注册。');
    return rows[0];
};
const codexCategories = (kind) => categories[kind];
const codexList = async (qqUserId, kind, category = '全部', page = 1, keyword = '') => {
    ...
    if (keyword.trim()) {
        conditions.push(`${config.name} LIKE ?`);
        params.push(`%${keyword.trim()}%`);
    }
    const where = conditions.join(' AND ');
    const [countRows] = await pool.execute(`SELECT COUNT(*) AS total ${config.from} WHERE ${where}`, params);
    const total = Number(countRows[0]?.total ?? 0);
    const pageSize = 5;
    const totalPages = Math.max(1, Math.ceil(total / pageSize));
    const currentPage = Math.min(Math.max(1, Math.floor(page)), totalPages);
    const [rows] = await pool.execute(`SELECT ${config.id} AS id,${config.name} AS name,${expression} AS category,${config.detail} AS detailId ${config.from} WHERE ${where} ORDER BY ${config.id} LIMIT ? OFFSET ?`, [...params, String(pageSize), String((currentPage - 1) * pageSize)]);
...
const skillCodexDetail = async (qqUserId, skillId) => {
    const character = await characterFor(qqUserId);
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT s.id,s.code,s.codex_id,s.name,s.category,s.skill_kind,s.element,s.range_type,s.target_scope,s.power,s.mana_cost,s.cooldown_turns,s.chant_turns,s.description,
...
const monsterCodexDetail = async (qqUserId, templateId) => {
    const character = await characterFor(qqUserId);
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT t.id,t.name,t.monster_class,t.level,t.skill_sequence,t.weakness_json,t.resistance_json
    ...
    if (!monster)
        throw new Error('尚未遭遇该怪物。');
    const [appraisalRows] = await pool.execute(`SELECT ap.range_level,ap.information_level FROM player_skills ps
    ...
    const rangeLevel = Number(appraisal?.range_level ?? 0);
    const informationLevel = Number(appraisal?.information_level ?? 0);
    const className = { normal: '普通', large: '大型', elite: '精英', boss: '首领' };
    ...
    if (skillCodes.length) {
        const [skillRows] = await pool.query(`SELECT code,name FROM skill_definitions WHERE code IN (${skillCodes.map(() => '?').join(',')})`, skillCodes);
        const names = new Map(skillRows.map(row => [row.code, row.name]));
        skills = skillCodes.map(code => names.get(code) ?? code);
    }
...
export { codexCategories, codexKinds, codexList, monsterCodexDetail, skillCodexDetail };
