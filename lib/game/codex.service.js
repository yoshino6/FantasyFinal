...
const codexList = async (qqUserId, kind, category = '全部', page = 1, keyword = '') => {
    ...
    const [rows] = await pool.execute(`SELECT ${config.id} AS id,${config.name} AS name,${expression} AS category,${config.detail} AS detailId ${config.from} WHERE ${where} ORDER BY ${config.id} LIMIT ? OFFSET ?`, [...params, pageSize, (currentPage - 1) * pageSize]);
