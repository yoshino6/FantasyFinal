...
import { lamplightNode } from './lamplight.config.js';

const json = (value) => typeof value === 'string' ? JSON.parse(value) : value ?? {};
const lamplightBattleProfile = (node, stage) => {
    ...
    const profile = { ...base, name, level, band: [level, level], profession: node.gate === 'barrier' ? 'warrior' : 'mage', advancedCode: undefined, advancedName: '', advancedEffect: {},
        equipment: { ...base.equipment, level, quality: 100 }, evolution: {}, injections: 0 };
    return { ...profile, ...recalculateNpcSparProfileStats(profile) };
};
const startLamplightBattle = async (c, character, node, stage) => {
    await (await import('./character.service.js')).recalculateCharacterStats(c, Number(character.id));
    const [current] = await c.execute('SELECT * FROM characters WHERE id=?', [character.id]);
    Object.assign(character, current[0]);
    const profile = lamplightBattleProfile(node, stage), session = randomUUID();
    const [templates] = await c.execute("SELECT id FROM monster_templates WHERE code='npc_sparring_dummy' LIMIT 1");
    if (!templates[0])
        throw Error('主线对抗所需的基础战斗数据尚未初始化。');
    ...
    const [spawn] = await c.execute(`INSERT INTO monster_spawns (template_id,region_id,pos_x,pos_y,pos_z,level,current_hp,skill_sequence,traits_json,defeated_at) VALUES (?,?,?,?,?,?,?,?,?,NOW())`, [templates[0].id, character.current_region_id, character.pos_x, character.pos_y, character.pos_z, profile.level, profile.stats.hpMax, JSON.stringify(profile.rotation), JSON.stringify([{ code: 'npc_sparring', name: '', profile }, { code: 'lamplight_encounter', name: '', owner_character_id: character.id, node: node.code, stage }])]);
    ...
    const [professions] = await c.execute('SELECT profession_code FROM player_advanced_professions WHERE character_id=?', [character.id]);
...
const finishLamplightBattle = async (c, session, result) => {
    const [rows] = await c.execute('SELECT * FROM player_lamplight_battles WHERE session_id=? FOR UPDATE', [session]);
    const row = rows[0];
    if (!row)
        return null;
    if (row.state !== 'active')
    ...
    const [states] = await c.execute('SELECT * FROM player_lamplight_progress WHERE character_id=? FOR UPDATE', [row.character_id]);
    const state = states[0];
    if (!state)
        throw Error('主线记录缺失，请保留战斗现场。');
    const flags = json(state.flags_json);
    if (lamplightNode(state)?.code !== row.node_code)
        throw Error('对抗与当前主线不一致，请保留记录核查。');
    if (snap.kind === 'boss' && Number(row.stage) !== Number(flags.bossStage ?? 0))
        throw Error('中枢对抗阶段不一致，请保留记录核查。');
    if (result === 'victory') {
        if (snap.kind === 'boss')
            flags.bossStage = Math.max(Number(flags.bossStage ?? 0), Number(row.stage) + 1);
        else if (snap.kind === 'research')
            flags.research = true;
        else if (snap.kind === 'barrier' && !flags.barrierWon) {
            flags.barrierWon = true;
            await grantOpeningItem(c, Number(row.character_id), 'sky_dust');
        }
    }
    ...
    await (await import('./automaton-combat.service.js')).finishCombatAutomatons(c, session, result === 'victory');
    ...
    await (await import('./character.service.js')).recalculateCharacterStats(c, Number(row.character_id));
...
export { finishLamplightBattle, lamplightBattleProfile, startLamplightBattle };
