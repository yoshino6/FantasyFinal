...
const floatingRescueStart = async (user) => withTransaction(async (c) => {
    ...
    await (await import('./progression-map.service.js')).ensureProgressionMaps(c, character.id);
...
const floatingStoryMainQuest = async (user) => {
    ...
    if (Number(c.realm_stage) === 1 && Number(c.level) >= 10 && Number(c.experience) >= experienceRequiredForLevel(10)) {
        if (!c.barrier_stage)
            return { title: '【主线·无形的禁锢】', description: '十级的经验已满，力量却停在看不见的边界。返回浮叶镇风枝会馆前台，向菈芮说明异状。', action: c.region_code !== 'floating_leaf_town' ? returnToLeaf : Number(c.pos_x) === 12 && Number(c.pos_y) === 0 ? { label: '[前往 前台]', command: '/初行公会 前台' } : { label: '[前往 风枝会馆]', command: '/前往 12 0 30' } };
        if (Number(c.barrier_stage) === 1)
            return { title: '【主线·观风台】', description: '菈芮让我到浮叶镇观风台查看风向刻纹，找出这道无形边界的形状。', action: c.region_code !== 'floating_leaf_town' ? returnToLeaf : Number(c.pos_x) === 13 && Number(c.pos_y) === 2 ? { label: '[观察 刻纹]', command: '/浮叶瓶颈 observatory' } : { label: '[前往 观风台]', command: '/前往 13 2 30' } };
    ...
    if (Number(c.goblin_stage) < 11) {
        const [location] = await pool.execute('SELECT pos_x,pos_y,pos_z FROM player_goblin_king_quest WHERE character_id=?', [c.id]);
    ...
    if (Number(c.goblin_stage) === 11)
        return { title: '【主线·返镇复命】', description: c.region_code === 'floating_leaf_town' ? '获救的人已回到浮叶镇。到公馆向菲萝缇复命，领取约定的一千铜币。' : '梨子喵和三人冒险团帮我把获救者送到安全商道。带浮叶镇的孩子们乘接驳回公馆。', action: c.region_code !== 'floating_leaf_town' ? { label: '[护送返镇]', command: '/浮叶返程' } : Number(c.pos_x) === 15 && Number(c.pos_y) === 0 ? { label: '[公馆复命]', command: '/浮叶复命' } : { label: '[前往 公馆]', command: '/前往 15 0 30' } };
