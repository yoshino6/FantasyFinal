...
const beginInheritanceStudy = async (qqUserId, sourceProfessionCode) => withTransaction(async (connection) => {
    ...
    if (!rows[0]) {
        ...
        await recordCharacterOperation(connection, { characterId: Number(character.id), kind: 'profession.inheritance_study_started', source: { system: 'inheritance_study', id: randomUUID(), step: 'started' }, outcome: '开始', summary: `开始旁修${profession.name}传承`, detail: { sourceProfessionCode } });
    }
...
const equipInheritanceStudy = async (qqUserId, sourceProfessionCode, equipped) => withTransaction(async (connection) => {
    ...
    if (Boolean(rows[0].equipped) !== equipped)
        await recordCharacterOperation(connection, { characterId: Number(character.id), kind: 'profession.inheritance_study_equipped', source: { system: 'inheritance_study', id: randomUUID(), step: 'equipped' }, outcome: equipped ? '装备' : '卸下', summary: `${equipped ? '装备' : '卸下'}${sourceProfessionCode}旁修传承`, detail: { sourceProfessionCode, equipped } });
