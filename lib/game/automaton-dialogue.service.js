...
import { automatonCharacter, automatonFor, recordAutomatonEvent, automatonIntimacy, saveAutomaton, assertAutomatonSafe } from './automaton.service.js';
...
const forgetAutomatonMemory = (user, id, memoryId) => withTransaction(async (connection) => { const character = await automatonCharacter(connection, user); await automatonFor(connection, character.id, id); const [deleted] = await connection.execute('DELETE FROM automaton_memories WHERE id=? AND automaton_id=? AND character_id=?', [memoryId, id, character.id]); if (Number(deleted.affectedRows) > 0)
    await recordCharacterOperation(connection, { characterId: Number(character.id), kind: 'automaton.memory_forgotten', source: { system: 'automaton_memory', id: memoryId, step: 'forgotten' }, outcome: '取消收藏', summary: '取消收藏机巧语录', detail: { automatonId: id, memoryId } }); });
