...
const talentActivityCommands = (number) => Object.entries(activityTalentNumbers)
    .filter(([, numbers]) => numbers.includes(number))
    .map(([command]) => command);
const canUseTalentActivity = (number, action, arg = '') => {
...
const talentActivity = async (c, actor, talent, data, action, arg, value) => {
    ...
    if (action === '调查') {
        if (arg !== '机关')
            throw new Error('废墟搜寻已由拾荒王冠的探索与战斗触发取代。');
...
export { canUseTalentActivity, talentActivity, talentActivityActions, talentActivityCommands, talentNpcLinks };
