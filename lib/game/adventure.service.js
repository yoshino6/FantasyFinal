...
import { readRegionalState, regionalStateSummary, isRegionalV2, RegionalBossBattle, installRegionalV2Damage, newRegionalState, regionalIncomingFactor } from './regional-boss-v2.js';
...
const monsterCombatStats = (monster) => {
    ...
    const traits = traitList(monster.traits_json).map(trait => bossTraitDefinitionFor(trait.code) ?? trait);
...
const inspectCombat = async (qqUserId) => {
    ...
    for (const raw of targets) {
        ...
        if (observer.informationLevel >= 2) {
            ...
            lines.push(`词条：${visibleTraits.map(trait => trait.name).join('、') || '无'}｜物攻 ${stats.physicalAttack}｜魔攻 ${stats.magicAttack}｜物防 ${stats.physicalDefense}｜魔防 ${stats.magicDefense}｜命中 ${stats.accuracy}｜闪避 ${stats.evasion}｜韧性 ${stats.tenacity}｜破韧 ${stats.tenacityPierce}`);
...
const summonCombatSpirit = async (connection, sessionId, member, definition) => {
    ...
    const hpMax = Math.max(1, Math.floor(Number(member.hp_max) * definition.statScale.hp));
    ...
    for (const spirit of spirits) {
        ...
        if (spirit.spirit_code === 'ember') {
            ...
            for (const ally of allies) {
                ...
                const amount = receivedHealingAmount(Math.max(1, Math.floor(Math.max(stats.magicAttack * .72, overload ? Number(ally.hp_max) * .08 : 0))), (await modifiersFor(connection, Number(ally.id))).healingReceivedPct);
...
const currentEncounter = async (qqUserId) => {
    const character = await characterFor(qqUserId);
    const pool = await getPool();
    const [rows] = await pool.execute(`SELECT s.id,s.template_id,t.name,t.monster_class,COALESCE(s.level,t.level) AS level,s.current_hp,s.traits_json,COALESCE(s.skill_sequence,t.skill_sequence) AS skill_sequence,${monsterAttributeColumns},t.experience,t.drops_json,t.weakness_json,t.resistance_json,t.element_mastery_json,t.element_resistance_json FROM monster_spawns s JOIN monster_templates t ON t.id=s.template_id WHERE s.region_id=? AND s.pos_x=? AND s.pos_y=? AND s.pos_z=? AND s.defeated_at IS NULL AND ${visiblePursuitCondition('s')}`, [character.current_region_id, character.pos_x, character.pos_y, character.pos_z, character.id, character.id, character.id, character.id, character.id]);
    const appraisal = await appraisalProfileFor(pool, [character.id]);
...
const combatActionInTransaction = async (connection, qqUserId, action, slot, skillId, itemId, deviceSkillCode, targetKind, targetId, automaticChant = false, hiddenTicket, hiddenAutomatic = false, skipPlayerTurn = false) => {
    ...
    const inheritanceHarmfulSkill = async (skillId) => {
        if (!skillId)
            return false;
        const [rows] = await connection.execute(`SELECT 1 FROM skill_effects se JOIN effect_definitions e ON e.id=se.effect_id
        ...
        try {
            ...
            if (turn.kind === 'member') {
                ...
                if (!bonusPhase && persistentArtifact.hpRegenPct) {
                    const oldHp = Number(member.current_hp);
                    const amount = receivedHealingAmount(Math.max(1, Math.floor(Number(member.hp_max) * persistentArtifact.hpRegenPct / 100)), persistentArtifact.healingReceivedPct);
          ...
          WHERE energy.battle_kind='pve' AND energy.session_id=? AND energy.character_id=? AND energy.instance_id=? FOR UPDATE`, [session.combat_id, member.id, Number(choice.deviceInstanceId)]);
                    ...
                    if (deviceSkill.effect === 'physical_evade_once')
                        ...
                        for (const ally of allyTargets) {
                            ...
                            ally.current_hp = Math.min(Number(ally.hp_max), before + Math.floor(Number(ally.hp_max) * ratio));
                ...
                if (skillCode === 'healing_prayer' || skillCode === 'healing_light' || skillCode === 'saint_healer_mending_prayer' || skillCode === 'saint_healer_absolution_hand') {
                    ...
                    const lowHpForSaintInheritance = hpRatioBeforeHeal < .4;
                    ...
                    if (lowHpForSaintInheritance && saintValue) {
                        await refreshCombatSpiritEffect(connection, session.combat_id, 'member', Number(ally.id), 'barrier', saintValue, 1);
                        log.push(`&余辉援护&为低血的${combatUnitLabel(ally)}追加 ${saintValue}% 减伤壁垒(1)。`);
                    }
                ...
                let elementalistRestored = false;
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.code IN (${debuffCodes.map(() => '?').join(',')})`, [session.combat_id, struckTarget.id, ...debuffCodes]);
                    ...
                    const inheritanceDamage = resonanceDamage + dawnDamage + nightDamage + spellbladeDamage;
                    ...
                    if (modifiers.lifestealPct && (choice.type === 'attack' || damageType === '刺击')) {
                        ...
                        if (rawLifesteal)
                            member.current_hp = Math.min(Number(member.hp_max), Number(member.current_hp) + receivedHealingAmount(rawLifesteal, modifiers.healingReceivedPct));
                    }
                    ...
                    const venomStatusBonusPct = venomCrit;
                    const effectCaster = venomStatusBonusPct ? { ...member, tenacity_pierce: Math.round(Number(member.tenacity_pierce ?? 0) * (1 + venomStatusBonusPct / 100)) } : member;
                    if (skillId)
                    ...
                    if (harmfulSkill && !warlordMarked && hasInheritance(Number(member.id), 'war_lord')) {
                        markTarget(struckTarget, `heritage_round_warlord_${member.id}`);
                        warlordMarked = true;
                        log.push(`&共鸣号令&【${targetName(struckTarget)}】被标记，等待队友下一次技能集火。`);
                    }
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.code IN (${freshDebuffCodes.map(() => '?').join(',')})`, [session.combat_id, struckTarget.id, ...freshDebuffCodes]);
                        ...
                        if (appliedDebuffs.length && !elementalistRestored) {
                            const restoredPct = inheritanceValue(Number(member.id), 'elementalist');
                            if (restoredPct) {
                                const oldMp = Number(member.current_mp);
                                const restored = Math.max(1, Math.floor(Number(member.mp_max) * restoredPct / 100));
                                member.current_mp = Math.min(Number(member.mp_max), oldMp + restored);
                                elementalistRestored = true;
                                log.push(`&异相共鸣&为新施加的减益恢复 ${member.current_mp - oldMp} MP。`);
                            }
                        }
                    }
          ...
          ON DUPLICATE KEY UPDATE value=VALUES(value),remaining_turns=VALUES(remaining_turns)`, [session.combat_id, monsterTarget.id]);
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.effect_type IN ('damage_over_time','stat_modifier','control') AND e.code<>'life_shield'`, [session.combat_id, monsterTarget.id]);
                ...
                const splitAdvancedGuard = async (recipient, rawDamage, allowTransfer) => {
                    ...
                    const sourceId = Number(jsonObject(recipient.cooldowns).advanced_guard_source ?? 0);
                    ...
                    const transferredRaw = Math.max(0, Math.floor(rawDamage * Math.min(80, rate) / 100));
                ...
                const afterAdvancedDamage = async (recipient, rawDamage, barrier, counterActive, primaryTarget, split) => {
                    ...
                    if (barrier > 0 && cooldowns.advanced_aegis_echo && rawDamage > 0) {
                        ...
                        const restored = Math.max(1, Math.floor(Number(recipient.hp_max) * .07));
                ...
                if (pounce) {
                    for (let index = 0; index < 3 && !victim.is_defeated; index += 1) {
                        ...
                        const damageReduction = Math.min(90, artifactReduction + victimModifiers.damageReductionPct + epicIncomingSkillReduction(victim));
                ...
                for (const affectedVictim of affected) {
                    ...
                    const damageReduction = Math.min(90, artifactReduction + affectedModifiers.damageReductionPct + epicIncomingSkillReduction(affectedVictim));
