...
import { kingbeastGroupId, kingbeastSymbiosisActive, isHiddenFusedKing, kingbeastForcedSingleTarget, kingbeastPanelSummary, kingbeastPassiveSummary, kingbeastCombatMultipliers, kingbeastSelectableTargets, isKingbeastPrimaryCore, kingbeastSummonDue, withoutKingbeastPhaseTransitionLogs, kingbeastTransition, kingbeastCoreDamageMultiplier, bossPhaseTransitionLogsAfterRound, kingbeastPhaseTransition, kingbeastPhaseTransitionLog } from './kingbeast.config.js';
...
const internalMonsterTraitCodes = ['npc_sparring', 'aeson_duel', 'boss_summon_inheritance', 'uzz_summon'];
const traitList = (value) => jsonArray(value).map(item => jsonObject(item)).filter(trait => trait.code && (trait.name || internalMonsterTraitCodes.includes(trait.code)));
...
const awardRealmExperience = async (connection, character, rawExperience, options = {}) => {
    ...
    let talentRoom = -Number(character.experience);
    ...
    let experience = repaired.corrected ? 0 : Number(character.experience);
    ...
    for (let reached = currentLevel + 1; reached <= level; reached++)
        await recordCharacterOperation(connection, { characterId: Number(character.id), kind: 'realm.level_up', source: { system: 'character_level', id: reached, step: 'reached' }, actorRole: 'system', outcome: '升级', summary: `角色升至 Lv${reached}`, detail: { fromLevel: reached - 1, toLevel: reached, realmStage: Number(character.realm_stage), derived: true } });
    if (gainedPoints > 0)
        await createHeartQuestionsForLevels(connection, Number(character.id), currentLevel, level, Number(character.realm_stage));
    if (gainedPoints > 0)
...
const spawnBoss = async (pool, region, template, forcedTrait, ignoreRespawn = false) => {
    ...
    if (template.code === 'goblin_king') {
      ...
      FROM monster_templates t WHERE t.code IN ('goblin_king','habadragon','goblin_royal_guard','goblin_royal_spearman')`, []);
        ...
        const create = async (entry, role) => {
            ...
            const traits = [...(['king', 'dragon'].includes(role) ? [bossTrait] : []), { code: 'kingbeast_encounter', name: '', groupId: encounterId, role }];
    ...
    const traits = forcedTrait ? [forcedTrait] : randomMonsterTraits(template);
...
const adminStartBossTest = async (qqUserId, code, traitName) => withTransaction(async (connection) => {
    ...
    const createTestSpawn = async (entry, role, includeBossTrait = true) => {
        const attributes = randomMonsterBaseAttributes(entry);
        const traits = [...(includeBossTrait ? [trait] : []), { code: 'boss_test', name: '测试·', session_id: sessionId }, ...(role ? [{ code: 'kingbeast_encounter', name: '', groupId: `kingbeast-test-${sessionId}`, role }] : [])];
    ...
    return { bossName: template.boss_name, bossCode: template.code, level, trait: trait.name, participants: participants.map(participant => participant.name), x: arenaX, y: arenaY };
});
...
const summonKingbeastCourtPair = async (connection, sessionId, source, members, entryTurn) => {
    ...
    if (!location)
        return 0;
    let count = 0;
    for (const code of ['goblin_royal_spearman', 'goblin_royal_guard']) {
...
const spawnThreeheadMotherHeads = async (connection, sessionId, bodies, members) => {
    for (const body of bodies.filter(item => String(item.growth_template_code ?? item.template_code ?? '') === threeheadMotherTemplateCode)) {
        ...
        for (const [index, role] of ['flame', 'gale'].entries()) {
            ...
            const traits = [{ code: 'summoned', name: '蛇母战斗蛇首' }];
            const [result] = await connection.execute('INSERT INTO monster_spawns (template_id,region_id,pos_x,pos_y,pos_z,level,constitution,spirit,strength,intelligence,agility,perception,current_hp,skill_sequence,traits_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [
...
const combatActionInTransaction = async (connection, qqUserId, action, slot, skillId, itemId, deviceSkillCode, targetKind, targetId, automaticChant = false, hiddenTicket, hiddenAutomatic = false, skipPlayerTurn = false) => {
    ...
    if (kingbeastSource && kingbeastSummonDue(Number(session.turn_no), Number(combatSessionCooldowns.kingbeast_last_summon_turn ?? 0), livingCourtCount, Boolean(combatSessionCooldowns.__bonusPhase))) {
    ...
    rules.hooks.directMultiplier = (source, target, element, magic, single, damageType) => {
        ...
        return physical * body * uzzTarget * frostPower * kingbeastReduction * weatherElementMultiplier$1(element, source.side === 'member' ? Number(source.key.split(':')[1]) : undefined) * (1 - Math.min(80, rules.value(target, 'barrier')) / 100);
    };
          ...
          ON DUPLICATE KEY UPDATE value=VALUES(value),remaining_turns=VALUES(remaining_turns)`, [session.combat_id, monsterTarget.id]);
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.effect_type IN ('damage_over_time','stat_modifier','control') AND e.code<>'life_shield'`, [session.combat_id, monsterTarget.id]);
                ...
                if (skill && [uzzSkills.skeletonCall, uzzSkills.necromanticCall].includes(skill.code)) {
                    ...
                    const summoned = await spawnUzzSummons(connection, session.combat_id, monsterTarget, members, phaseTwo ? ['uzz_pain_wraith', 'uzz_skeleton_mage'] : ['uzz_skeleton_berserker', 'uzz_skeleton_archer'], 'ordinary');
                ...
                if (pounce) {
                    for (let index = 0; index < 3 && !victim.is_defeated; index += 1) {
                        ...
                        const damage = timeGuarded ? 0 : directDamageVariance(Math.max(1, Math.floor(strike.damage * elemental * weatherElement * uzzFieldDamageMultiplier * (1 + exposed / 100) * (1 - phaseDecoy / 100) * (1 - counter.damageReductionPct / 100) * (1 - Math.min(80, barrier) / 100) * (1 - Math.min(90, guard) / 100) * (1 - damageReduction / 100))));
                ...
                const affected = skill?.target_scope === '全体' ? members.filter(member => !member.is_defeated) : isAeson && skill?.code === 'aeson_snakebind' ? [victim, ...members.filter(member => !member.is_defeated && Number(member.id) !== Number(victim.id)).sort((a, b) => Number(a.current_hp) / Math.max(1, Number(a.hp_max)) - Number(b.current_hp) / Math.max(1, Number(b.hp_max))).slice(0, 1)] : [victim];
                await consumePrecisionAim(victim);
                const strike = await resolveMonsterStrike(victim);
                ...
                for (const affectedVictim of affected) {
                    ...
                    const dealt = effectValue('member', Number(affectedVictim.id), 'time_guard') > 0 ? 0 : directDamageVariance(Math.max(1, Math.floor(affectedStrike.damage * elemental * weatherElementMultiplier$1(String(skill?.element ?? '')) * uzzFieldDamageMultiplier * (1 + exposed / 100) * (1 - phaseDecoy / 100) * (1 - counter.damageReductionPct / 100) * (1 - Math.min(80, barrier) / 100) * (1 - Math.min(90, guard) / 100) * (1 - damageReduction / 100))));
