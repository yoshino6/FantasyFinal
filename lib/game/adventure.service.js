...
import { skillSpecialization, specializationMaximum, specializationOptions, specializeEffectValue, manaTransferCost } from './skill-specialization.js';
import { canDispelCombatEffect, nativeCleanseLimit } from './combat-dispel-policy.js';
...
import { recalculateCharacterStats, refreshCharacterStamina } from './character.service.js';
...
import { resolveStrike, directDamageVariance, tenacityContest, opposedChance } from './combat-math.js';
...
import { advanceEvolutionObservationMining, evolutionStatBonuses, repairEvolutionProgress, symbiosisTraits, advanceEvolutionObservationBattle } from './evolution.service.js';
...
import { isAdvancedProfessionSkillCode, advancedProfessionByCode, isCachedAdvancedPassiveEffect } from './advanced-profession.config.js';
...
const monsterCombatStats = (monster) => {
    ...
    const specialQuestBossHpMultiplier = isKingbeastCore(monster) || traits.some(trait => trait.code === 'main_quest_evolution') ? 3 : undefined;
...
const awardRealmExperience = async (connection, character, rawExperience) => {
    ...
    let remaining = Math.max(0, Math.floor(rawExperience * await globalExperienceMultiplier(connection)));
    ...
    return { experience: gainedExperience, level, gainedPoints, realmLocked, realmCapReached, realmStage: Number(character.realm_stage) };
};
...
const normalizeRegionalBossTraits = async (pool) => {
    ...
    const [rows] = await pool.execute(`SELECT DISTINCT s.id,COALESCE(s.level,t.level) AS level,
...
const decayWorldBossTraits = async (pool) => {
    const [rows] = await pool.execute(`SELECT DISTINCT s.id,COALESCE(s.level,t.level) AS level,s.current_hp,
    COALESCE(s.constitution,t.constitution) AS constitution,COALESCE(s.spirit,t.spirit) AS spirit,COALESCE(s.strength,t.strength) AS strength,
    COALESCE(s.intelligence,t.intelligence) AS intelligence,COALESCE(s.agility,t.agility) AS agility,COALESCE(s.perception,t.perception) AS perception,
    t.constitution_growth,t.spirit_growth,t.strength_growth,t.intelligence_growth,t.agility_growth,t.perception_growth,s.traits_json
    FROM monster_spawns s JOIN monster_templates t ON t.id=s.template_id
...
const finalAttribute = (character, attribute) => Number(character[attribute]) + Number(character[`${attribute}_growth`]) * Math.max(0, Number(character.level) - 1);
...
const movementSpeedFrom = (agility, level, speedPenalty) => explorationScale(Math.max(1, agility - speedPenalty / 8), level);
const activeSkillUpgradeCost = (level) => Math.floor(Math.max(1, level) / 10) + 1;
...
const modifiersFor = async (connection, characterId) => {
    ...
    const [deviceRows] = await connection.execute(`SELECT i.name,i.weapon_type,'device' AS slot,COALESCE(ii.effect_json,i.effect_json) AS effect_json,COALESCE(ii.quality,100) AS quality FROM player_active_devices ad JOIN player_item_instances ii ON ii.id=ad.instance_id AND ii.character_id=ad.character_id JOIN item_definitions i ON i.id=ii.item_id WHERE ad.character_id=? AND i.item_category='异械'`, [characterId]);
...
const bossTestLoadout = (professionCode, level) => {
    ...
    const baseArmor = Math.floor(forgedEquipmentBase(level, '防具'));
    const profile = {
    ...
    ].map(([slot, category, name]) => ({ slot, category, weaponType: '皮甲', name: `测试·Lv.${level}${name}`, effect: { physicalDefense: baseArmor, magicDefense: baseArmor } }));
    return [
...
const adminStartBossTest = async (qqUserId, code, traitName) => withTransaction(async (connection) => {
    ...
    for (const item of loadout) {
      ...
      ON DUPLICATE KEY UPDATE name=VALUES(name),description=VALUES(description),weapon_type=VALUES(weapon_type),required_level=VALUES(required_level),effect_json=VALUES(effect_json)`, [item.code, item.name, `首领测试专用的 Lv.${level} 普通品质装备。`, item.category, item.weaponType, level, JSON.stringify(item.effect)]);
        const [definitions] = await connection.execute('SELECT id FROM item_definitions WHERE code=? LIMIT 1', [item.code]);
        const [instance] = await connection.execute('INSERT INTO player_item_instances (character_id,item_id,quality,durability,durability_max,effect_json) VALUES (?,?,100,100,100,?)', [owner.id, definitions[0].id, JSON.stringify(item.effect)]);
...
const inventoryView = async (qqUserId, category) => {
    ...
    const [stacked] = await pool.execute('SELECT i.id,i.code,i.codex_id,i.name,i.item_category,pi.quantity,i.description FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.quantity>0 AND i.item_type=? AND i.stackable=1 ORDER BY i.name', [character.id, itemType]);
    const [instances] = await pool.execute('SELECT ii.id,i.codex_id AS definition_codex_id,i.name,i.item_category,ii.quality,ii.durability,ii.durability_max,i.description FROM player_item_instances ii JOIN item_definitions i ON i.id=ii.item_id WHERE ii.character_id=? AND i.item_type=? ORDER BY ii.acquired_at DESC', [character.id, itemType]);
    const [recent] = await pool.execute(`SELECT code,codex_id,item_type,item_category,name FROM (
...
const itemCodex = async (qqUserId, codexId) => {
    const character = await characterFor(qqUserId);
    const [rows] = await (await getPool()).execute(`SELECT DISTINCT i.id,i.codex_id,i.name,i.item_type,i.item_category,i.description,i.obtain_source,i.weight,i.effect_json FROM item_definitions i
...
const equipment = async (qqUserId) => {
    const character = await characterFor(qqUserId);
    const [rows] = await (await getPool()).execute(`SELECT pe.slot,i.name,pe.instance_id
    FROM player_equipment pe JOIN item_definitions i ON i.id=pe.item_id
    WHERE pe.character_id=?
...
const skillList = async (qqUserId) => {
    ...
    const [skills] = await pool.execute('SELECT s.id,s.name,s.category,s.tier,ps.level,ps.quick_slot,ps.passive_linked,ps.learned_at FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id WHERE ps.character_id=? ORDER BY ps.learned_at,s.id', [character.id]);
...
const skillDetail = async (qqUserId, skillId) => {
    ...
    for (const effect of effectRows)
        effect.value = specializeEffectValue(effect.code, Number(effect.value), specialized.effectFactor);
...
const upgradeSkillSpecialization = async (qqUserId, skillId, specialization) => withTransaction(async (connection) => {
    ...
    if (!weaponMastery && !['passive', 'bound'].includes(skill.category)) {
        const [effects] = await connection.execute('SELECT e.code,COALESCE(se.value_override,e.default_value) AS value FROM skill_effects se JOIN effect_definitions e ON e.id=se.effect_id WHERE se.skill_id=?', [skillId]);
    ...
    const cost = weaponMastery ? masteryUpgradeCost(level) : activeSkillUpgradeCost(Number(skill.level));
    if (Number(character.skill_points) < cost)
        throw new Error(`技能点不足，升级需要 ${cost} 点。`);
    await connection.execute('UPDATE characters SET skill_points=skill_points-? WHERE id=?', [cost, character.id]);
...
const moveToPosition = async (connection, qqUserId, x, y, restrictToPerception, speedLimit, destinationRegionId, allowRestingArrival = false) => {
    ...
    return { character: moved, kind: 'event', text: texts[0]?.description ?? '四周一片寂静，暂时没有发现异常。' };
};
...
const moveToMap = async (qqUserId, mapCode) => {
    const pool = await getPool();
    const character = await characterFor(qqUserId);
    const [rows] = await pool.execute(`SELECT i.code,r.name,r.min_x,r.max_x,r.min_y,r.max_y
    ...
    if (map.min_x === null || map.max_x === null || map.min_y === null || map.max_y === null)
        throw new Error('该地图区域暂未开放。');
    if (map.name === character.region_name)
...
const huntMonster = async (qqUserId) => withTransaction(async (connection) => {
    ...
    const [encounter] = await connection.execute('SELECT 1 FROM monster_spawns WHERE region_id=? AND pos_x=? AND pos_y=? AND pos_z=? AND defeated_at IS NULL LIMIT 1 FOR UPDATE', [character.current_region_id, character.pos_x, character.pos_y, character.pos_z]);
    if (encounter[0])
        throw new Error('当前格子存在敌对生物，请先选择战斗、交涉或躲避。');
    ...
    const [targets] = await connection.execute(`SELECT id,pos_x,pos_y,pos_z FROM monster_spawns
...
const mineResource = async (qqUserId, resourceId) => withTransaction(async (connection) => {
    ...
    await ensureForestGuideFreeAction(connection, Number(character.id));
    ensureActionAvailable(character);
    const [miningRows] = await connection.execute(`SELECT m.resource_id,rs.item_id,i.code,i.name,m.finishes_at FROM player_resource_mining m
    ...
    if (mining) {
        ...
        const quantity = await resourceYield(connection, Number(mining.item_id), mining.code);
...
const chooseTarget = async (qqUserId, spawnId, ambush = false, retreatPosition) => withTransaction(async (connection) => {
    let character = await characterFor(qqUserId);
...
const negotiationFailureOpening = async (qqUserId, spawnId) => {
    const started = await chooseTarget(qqUserId, spawnId);
    return withTransaction(async (connection) => {
        const session = await activeCombatFor(connection, started.character.id);
        if (!session)
        ...
        const victim = attacker && threatTarget(members, new Map(members.map(member => [Number(member.id), 1])));
        if (!attacker || !victim)
            return { started, log: '交涉失败，敌人露出敌意。' };
        const [templateRows] = await connection.execute('SELECT code FROM monster_templates WHERE id=?', [Number(attacker.template_id ?? 0)]);
        ...
        if (sequence.length) {
            ...
            const [skillRows] = await connection.execute(`SELECT id,code,name,category,element,power,mana_cost,cooldown_turns FROM skill_definitions WHERE code IN (${placeholders})`, sequence);
        ...
        if (skill?.code === 'wolfking_summon_shadow_wolf') {
            attacker.current_mp -= Number(skill.mana_cost);
            cooldowns[skill.code] = Number(skill.cooldown_turns);
        ...
        const monster = monsterCombatStatsForPlayer(attacker, Number(victim.level));
        const attack = skill?.category === 'magic' ? monster.magicAttack : monster.physicalAttack;
        const kind = skill?.category === 'magic' ? '魔法' : '物理';
        const multiplier = Number(skill?.power ?? 100) / 100;
        ...
        const damage = strike.hit ? directDamageVariance(strike.damage) : 0;
        const oldHp = Number(victim.current_hp);
        if (strike.hit) {
            victim.current_hp = Math.max(0, oldHp - damage);
            if (!victim.current_hp)
                victim.is_defeated = 1;
            if (skill)
                await applySkillEffects(connection, session.combat_id, Number(skill.id), attacker, 'target', victim, 'member', 'on_hit', actionLog);
        }
...
const encounterAction = async (qqUserId, spawnId, action) => {
    ...
    if (action === 'avoid') {
        ...
        WHERE character_id=? AND encounter_region_id=? AND encounter_x=? AND encounter_y=? AND encounter_z=? LIMIT 1 FOR UPDATE`, [character.id, character.current_region_id, character.pos_x, character.pos_y, character.pos_z]);
          ...
          VALUES (?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE encounter_region_id=VALUES(encounter_region_id),encounter_x=VALUES(encounter_x),encounter_y=VALUES(encounter_y),encounter_z=VALUES(encounter_z),retreat_region_id=VALUES(retreat_region_id),retreat_x=VALUES(retreat_x),retreat_y=VALUES(retreat_y),retreat_z=VALUES(retreat_z)`, [member.id, retreat.retreat_region_id, retreat.retreat_x, retreat.retreat_y, retreat.retreat_z, character.current_region_id, character.pos_x, character.pos_y, character.pos_z]);
            ...
            const strike = resolveStrike(monster.physicalAttack, Number(victim.physical_defense), monster.accuracy, Number(victim.evasion), monster.crit, Number(victim.crit_resist_bp), monster.critDamage, Number(victim.crit_damage_reduction_bp));
    ...
    if (action === 'persuade') {
        ...
        const success = await withTransaction(async (connection) => {
            ...
            const partyExperienceBonus = { 1: 0, 2: .1, 3: .2, 4: .35 }[partySize] ?? 0;
            const partyDropBonus = { 1: 0, 2: .6, 3: 1, 4: 1.5 }[partySize] ?? 0;
            const rewardLines = [];
            const dropsByMember = new Map();
            const modifiersByMember = new Map();
            for (const member of rewardMembers) {
                const modifiers = await modifiersFor(connection, Number(member.id));
                modifiersByMember.set(Number(member.id), modifiers);
            ...
            for (const member of members.filter(member => !member.npc_code && !eligibility.get(Number(member.id))))
                rewardLines.push(`【${member.name}】体力不足，未获得经验与战利品。`);
            const randomRecipient = () => rewardMembers[random(0, rewardMembers.length - 1)];
            for (const drop of resolvedDrops(target.drops_json)) {
                if (!drop.code || !rewardMembers.length)
                    continue;
                const recipient = randomRecipient();
                const modifiers = modifiersByMember.get(Number(recipient.id));
                const traitBonus = percentBonus(traitList(target.traits_json), 'dropPct') / 100;
                if (Math.random() >= Math.min(1, Number(drop.chance ?? 1) * .2 * (1 + traitBonus + partyDropBonus) + modifiers.dropBonus))
                    continue;
                const [items] = await connection.execute('SELECT id,name,item_type FROM item_definitions WHERE code=?', [resolvedDropCode(drop, Number(target.level))]);
                const item = items[0];
                if (!item)
                    continue;
                for (let index = 0; index < dropQuantity(drop); index += 1) {
                    ...
                    if (item.item_type === 'equipment')
                        await connection.execute('INSERT INTO player_item_instances (character_id,item_id) VALUES (?,?)', [owner.id, item.id]);
                    else
                        await connection.execute('INSERT INTO player_inventory (character_id,item_id,quantity) VALUES (?,?,1) ON DUPLICATE KEY UPDATE quantity=quantity+1,acquired_at=NOW()', [owner.id, item.id]);
                    const list = dropsByMember.get(Number(owner.id)) ?? [];
                    list.push(item.name);
                    dropsByMember.set(Number(owner.id), list);
                }
            }
            for (const member of rewardMembers) {
                const drops = dropsByMember.get(Number(member.id));
                if (drops?.length)
                    rewardLines.push(`【${member.name}】获得 ${drops.join('、')}`);
            }
...
const inspectCombat = async (qqUserId) => {
    ...
    if (profile.informationLevel >= 1)
        [effects] = await pool.execute('SELECT ce.id,ce.target_kind,ce.target_id,e.code,e.name,e.effect_type,ce.value,ce.stacks,ce.remaining_turns FROM combat_status_effects ce JOIN effect_definitions e ON e.id=ce.effect_id WHERE ce.session_id=?', [session.combat_id]);
    ...
    for (const raw of targets) {
        ...
        if (component) {
            ...
            if (count)
                lines.push(`多部位核心：存活部位 ${count}/3｜当前部位减伤 ${bossBodyDamageReductionPct(target, targets)}%。`);
        }
...
const negotiationChance = (negotiation, monsterResistance, monsterClass, inDungeon) => {
...
const negotiationSuccessText = (code, name) => ({
...
}[code] ?? `${name} 感受到你的善意，收起敌意，转身离开了此地。`);
...
const activeCombatEffects = async (connection, sessionId) => {
    const [rows] = await connection.execute(`SELECT ce.id,ce.target_kind,ce.target_id,e.code,e.name,e.effect_type,ce.value,ce.stacks,ce.remaining_turns
...
const useCombatConsumable = async (connection, sessionId, member, targets, itemId, itemName, rawEffect) => {
    ...
    if (isAlchemyControlDebuff(effect) && !effectiveTargets.length) {
        ...
        if (overLevel)
            return { consumed: false, message: `【${itemName}】的适用等级为 Lv.${applicableLevel}及以下；【${overLevel.name}】当前为 Lv.${overLevel.level}，无法生效。` };
    }
    const messages = [];
    ...
    const heal = rawHeal ? receivedHealingAmount(rawHeal, (await modifiersFor(connection, Number(member.id))).healingReceivedPct) : 0;
    ...
    if (effect.throwable && target) {
        ...
        const shield = await absorbLifeShield(connection, sessionId, 'target', Number(target.id), Number(target.hp_max), damage);
        const hpDamage = damage - shield.absorbed;
        target.current_hp = Math.max(0, oldTargetHp - hpDamage);
    ...
    if (Number(effect.experienceBonusPct)) {
        const value = Math.max(0, Number(effect.experienceBonusPct));
    ...
    if (Number(effect.partyDropBonusPct)) {
        const value = Math.max(0, Number(effect.partyDropBonusPct));
        const count = Math.max(1, Math.floor(Number(effect.battleCount ?? 1)));
...
const applySkillEffects = async (connection, sessionId, skillId, caster, casterKind, target, targetKind, timing, log, beneficialEffectBonusPct = 0, controlChanceMultiplier = 1, ruleBridge) => {
    ...
    if (casterKind === 'member') {
        ...
        shieldPower = specialized.powerFactor;
    }
    ...
    for (const effect of effects) {
        ...
        let appliedDuration = Number(effect.duration);
        ...
        if (effect.stackable) {
            ...
            if (currentStacks < maxStacks)
                await connection.execute('INSERT INTO combat_status_effects (session_id,target_kind,target_id,effect_id,effect_level,value,remaining_turns) VALUES (?,?,?,?,?,?,?)', [sessionId, effectTargetKind, targetId, effect.id, effect.effect_level, appliedValue, appliedDuration]);
            log.push(effectMessage(effect, appliedValue, appliedDuration, stacks, effectMarkerForTarget(effectTargetKind)));
        }
        else if (existing[0]) {
            await connection.execute('UPDATE combat_status_effects SET effect_level=?,value=?,stacks=1,remaining_turns=GREATEST(remaining_turns,?) WHERE id=?', [effect.effect_level, appliedValue, appliedDuration, existing[0].id]);
...
const processTurnEffects = async (connection, sessionId, targetKind, targetId, members, targets, log, absorb = absorbLifeShield) => {
    ...
    for (const effect of effects) {
        ...
        if (effect.effect_type === 'damage_over_time' || effect.effect_type === 'heal_over_time') {
            ...
            const amount = effect.effect_type === 'heal_over_time' ? reducedHealingAmount(baseAmount, healingCut) : nonPoisonBossDotCap !== undefined ? Math.min(baseAmount, nonPoisonBossDotCap) : bossDotCap === undefined ? baseAmount : Math.min(baseAmount, bossDotCap);
    ...
    for (const body of bodies) {
        ...
        for (let index = 0; index < definitions.length; index += 1) {
            ...
            if (!template)
                continue;
            const stats = {
                hpMax: Math.max(1, Math.floor(bodyStats.hpMax * definition.hpRatio)), mpMax: 0,
                physicalAttack: Math.max(1, Math.floor(bodyStats.physicalAttack * definition.physicalAttackRatio)), magicAttack: Math.max(1, Math.floor(bodyStats.magicAttack * definition.magicAttackRatio)),
                physicalDefense: Math.max(1, Math.floor(bodyStats.physicalDefense * definition.physicalDefenseRatio)), magicDefense: Math.max(1, Math.floor(bodyStats.magicDefense * definition.magicDefenseRatio)),
                accuracy: bodyStats.accuracy, evasion: bodyStats.evasion, crit: bodyStats.crit, critResist: bodyStats.critResist, critDamage: bodyStats.critDamage, critReduction: bodyStats.critReduction,
                tenacity: bodyStats.tenacity, tenacityPierce: bodyStats.tenacityPierce, speed: Math.max(1, Math.floor(bodyStats.speed * definition.speedRatio)), perception: bodyStats.perception
            };
            const traits = [
...
const refreshCombatSpiritEffect = async (connection, sessionId, targetKind, targetId, effectCode, value, turns) => {
    ...
    if (existing[0])
        await connection.execute('UPDATE combat_status_effects SET value=GREATEST(value,?),remaining_turns=GREATEST(remaining_turns,?) WHERE id=?', [value, turns, existing[0].id]);
    else
        await connection.execute('INSERT INTO combat_status_effects (session_id,target_kind,target_id,effect_id,effect_level,value,remaining_turns) VALUES (?,?,?,?,1,?,?)', [sessionId, targetKind, targetId, definition.id, value, turns]);
};
...
const lifeShieldAbsorptionText = (result) => result.absorbed ? `｜生命护盾吸收${result.absorbed}${result.broken ? '（破碎）' : `（余${result.remaining}）`}` : '';
...
const resolveCombatSpirits = async (connection, sessionId, members, targets, log, only, absorb = absorbLifeShield) => {
    ...
    for (const spirit of spirits) {
        ...
        if (spirit.spirit_code === 'ember') {
            ...
            for (const target of spiritTargets) {
                ...
                if (!strike.hit)
                    log.push(`&灵契·${spirit.spirit_name}&追击【${target.name}】，但攻击被闪避。`);
                else {
                    ...
                    const damage = directDamageVariance(Math.max(1, Math.floor(strike.damage * elemental * weather * partAoeMultiplier * (isBossComponent(target) ? 1 : bossBodyDamageMultiplier(target, targets)))));
                    const shield = await absorb(connection, sessionId, 'target', Number(target.id), Number(target.hp_max), damage);
                    const hpDamage = damage - shield.absorbed;
            ...
            for (const ally of allies) {
                ...
                log.push(`&灵契·${spirit.spirit_name}&为【${ally.name}】恢复 ${ally.current_hp - oldHp} HP(${oldHp}→${ally.current_hp})`);
            }
        }
...
const finishPartyVictory = async (connection, sessionId, members, targets) => {
    ...
    const staminaSpent = await consumeVictoryStamina(connection, members, Math.max(1, targets.filter(target => !isSummonedMonster(target)).length));
    ...
    for (const member of playerMembers) {
        ...
        const reward = { name: member.name, experience, staminaSpent: staminaSpent.get(Number(member.id)), realmLocked: experienceGain.realmLocked, realmCapReached: experienceGain.realmCapReached, realmStage: experienceGain.realmStage, levelText: gainedPoints ? `升级至 Lv.${newLevel}，获得 ${gainedPoints} 技能点` : undefined, drops, learned };
        rewards.push(reward);
    ...
    for (const target of rewardTargets)
        for (const drop of resolvedDrops(target.drops_json)) {
            if (!drop.code || !rewardMembers.length)
                continue;
            const recipient = randomRecipient();
            const modifiers = modifiersByMemberId.get(Number(recipient.id));
            ...
            if (Math.random() > Math.min(1, baseDropChance * globalDrop * (1 + traitDropBonus + partyDropBonus + omniscientDropBonus + elixirDropBonus) + modifiers.dropBonus))
                continue;
            for (let index = 0; index < dropQuantity(drop); index += 1)
                await grantDrop(randomRecipient(), resolvedDropCode(drop, Number(target.level)), 1);
        }
...
const currentEncounter = async (qqUserId) => {
    ...
    if (!spawns.length)
        return null;
    await pool.query(`INSERT IGNORE INTO player_monster_codex (character_id,monster_template_id) VALUES ${spawns.map(() => '(?,?)').join(',')}`, spawns.flatMap(spawn => [character.id, Number(spawn.template_id)]));
...
const persistBattleMembers = async (connection, members, forceRest = false) => {
    for (const member of members) {
        ...
        const hp = defeated ? 1 : Math.max(1, Number(member.current_hp));
...
const combatAction = async (qqUserId, action, slot, skillId, itemId, deviceSkillCode, targetKind, targetId, automaticChant = false) => withTransaction(async (connection) => {
    const character = await characterFor(qqUserId);
    if (await repairInvalidCombatFor(connection, Number(character.id)))
        throw new Error('检测到上一场战斗已失效，已自动结束。请重新寻怪或移动。');
    const session = await activeCombatFor(connection, character.id);
    ...
    if (!continuingNpcPartyBattle && action === 'skill') {
        ...
        const manaCost = skills[0].code === 'resident_d01' ? manaTransferCost(Number(actor.current_mp)) : ruleManaCost(ruleState, linked.map(row => row.code), Math.ceil(baseManaCost * (ruleState.memory.debtSkill === skills[0].code ? 1.4 : 1)), Number(session.turn_no));
    ...
    if (!continuingNpcPartyBattle) {
        const pending = { type: pendingType, chantRelease: automaticChant, ...(Number(jsonObject(actor.cooldowns).__selectedAlly) ? { targetKind: 'member', targetId: Number(jsonObject(actor.cooldowns).__selectedAlly) } : { targetKind: 'target', targetId: Number(actor.selected_target_id) }), ...(slot ? { slot } : {}), ...(skillId ? { skillId } : {}), ...(itemId ? { itemId } : {}), ...(selectedDevice ? { deviceInstanceId: selectedDevice.instanceId, deviceSkillCode: selectedDevice.skillCode } : {}), ...(targetKind ? { targetKind } : {}), ...(targetId ? { targetId } : {}) };
    ...
    const rewardSummonerSupport = async (reason) => {
        for (const member of members.filter(item => resourceFor(Number(item.id))?.profession_code === 'spirit_summoner' && !item.is_defeated))
            await gainResource(member, 15, reason);
    };
    ...
    const specializedNativeHealing = async (caster, recipient, amount) => {
    ...
    const effectValue = (kind, targetId, code) => residentValue(kind, targetId, code) + effects.filter(effect => effect.target_kind === kind && Number(effect.target_id) === targetId && effect.code === code).reduce((sum, effect) => sum + Number(effect.value) * Number(effect.stacks), 0);
    const targetHealingAmount = (target, amount) => reducedHealingAmount(amount, effectValue('target', Number(target.id), 'advanced_healing_cut'));
    ...
    const triggerEpicIncomingSkill = async (recipient) => {
        ...
        if (hasEpicWeaponEffect(epicFor(recipient), 'epic_mountaingate_shield') && Number(cooldowns.epic_mountaingate_ready_turn ?? 0) <= turnNo) {
            ...
            if (shield.added)
                log.push(`&山门盾牌&【${recipient.name}】受击后获得${shield.added}点生命护盾(1)。`);
        }
    ...
    const triggerEpicEffectiveHeal = async (caster, recipient, hpBefore, lowBeforeHeal) => {
        ...
        if (epicSetActive(caster, 'mistmother_cocoon', 3)) {
            ...
            if (bonus > 0) {
                ...
                log.push(`&雾生灵潮&【${recipient.name}】治疗额外恢复${bonus} HP。`);
            }
        }
        ...
        if (hasEpicWeaponEffect(loadout, 'epic_marshmoon_orb') && lowBeforeHeal && Number(recipientCooldowns[`epic_marshmoon_heal_${caster.id}`] ?? 0) !== turnNo) {
            ...
            if (bonus > 0) {
                recipient.current_hp += bonus;
                log.push(`&沼月回潮&【${recipient.name}】额外恢复${bonus} HP。`);
            }
        }
    ...
    for (const turn of turns) {
        ...
        const controlled = turn.kind === 'spirit' || extraTurn ? false : await processTurnEffects(connection, session.combat_id, turn.kind, turn.id, members, targets, log, absorbRuleShield);
        effects = await activeCombatEffects(connection, session.combat_id);
        if (!canRuleAct)
            continue;
        ...
        if (turn.kind === 'member') {
          ...
          WHERE energy.battle_kind='pve' AND energy.session_id=? AND energy.character_id=? AND energy.instance_id=? FOR UPDATE`, [session.combat_id, member.id, Number(choice.deviceInstanceId)]);
                ...
                if (!deviceSkill || !energy || deviceSkill.deviceCode !== energy.code) {
                    log.push(`➤【${member.name}】启动异械\n　➥异械配置已失效。`);
                    continue;
                }
                if (Number(energy.current_energy) < deviceSkill.energyCost) {
                    log.push(`➤【${member.name}】启动【${energy.name}】\n　➥能量不足，本回合应改为充能。`);
                    continue;
                }
                const cooldowns = jsonObject(member.cooldowns);
                ...
                const applyDamage = async (enemy, magic, multiplier, element = '') => {
                    ...
                    const strike = resolveStrike(attack * multiplier / 100 * (1 + damageBonus / 100), defense, Number(member.accuracy) * (1 + accuracyBonus / 100), stats.evasion, Number(member.crit_rate_bp), stats.critResist, Number(member.crit_damage_bp), stats.critReduction, false, !magic && persistentArtifact.physicalForceCrit);
                    if (!strike.hit) {
                    ...
                    const damage = directDamageVariance(Math.max(1, Math.floor(strike.damage * elemental * (1 + exposed / 100) * (1 - Math.min(80, barrier) / 100) * partAoeMultiplier * (isBossComponent(enemy) ? 1 : bossBodyDamageMultiplier(enemy, targets)))));
                    const oldHp = Number(enemy.current_hp);
                    const shield = await absorbRuleShield(connection, session.combat_id, 'target', Number(enemy.id), Number(enemy.hp_max), damage);
                    enemy.current_hp = Math.max(0, oldHp - (damage - shield.absorbed));
                    if (!enemy.current_hp)
                        enemy.is_defeated = 1;
                    await connection.execute('UPDATE combat_threat SET threat=threat+? WHERE session_id=? AND spawn_id=? AND character_id=?', [damage, session.combat_id, enemy.id, member.id]);
                ...
                if (deviceSkill.effect === 'physical_evade_once')
                    ...
                    for (const enemy of enemyTargets)
                        await applyDamage(enemy, false, deviceSkill.power ?? 100);
                    ...
                    for (const enemy of enemyTargets) {
                        await applyDamage(enemy, false, 120);
                        if (Math.random() < .65)
                            await applyAdvancedStatus('target', Number(enemy.id), 'stun', 1, 1);
                    }
                else if (deviceSkill.effect === 'frost_pulse')
                    for (const enemy of enemyTargets) {
                        await applyDamage(enemy, true, 100, '冰');
                        await applyAdvancedStatus('target', Number(enemy.id), 'slow', 25, 2);
                    }
                    ...
                    for (const enemy of enemyTargets) {
                        await applyDamage(enemy, false, 180, '雷');
                        await applyAdvancedStatus('target', Number(enemy.id), 'exposed', 25, 2);
                    }
                    ...
                    for (const enemy of enemyTargets)
                        await applyDamage(enemy, true, 230, '火');
                }
            ...
            if (effectValue('member', Number(member.id), 'alchemy_confusion') > 0) {
                ...
                log.push(`#混乱#【${member.name}】的攻击目标变得随机。`);
            }
            ...
            if (counterReady > 0) {
                ...
                if (Math.random() * 100 < counterReady) {
                    ...
                    const counterStrike = resolveStrike(Number(member.physical_attack) * .9, counterMonster.physicalDefense, Number(member.accuracy), counterMonster.evasion, Number(member.crit_rate_bp), counterMonster.critResist, Number(member.crit_damage_bp), counterMonster.critReduction, false, false);
                    if (counterStrike.hit) {
                        ...
                        log.push(`&反击架势&【${member.name}】反击【${targetName(target)}】，造成 ${dealt} 点物理伤害${lifeShieldAbsorptionText(shield)}(${oldHp}→${target.current_hp})。`);
                    }
                    else
                        log.push(`&反击架势&【${member.name}】的反击被【${targetName(target)}】闪避。`);
                }
                else
                    log.push(`&反击架势&【${member.name}】未能抓住反击空隙。`);
            }
            ...
            if (choice.type === 'skill') {
                ...
                if (choice.chantRelease && !casting) {
                    log.push(`➤【${member.name}】的吟唱已被打断，本次行动结束。`);
                    continue;
                }
            ...
            if (skillCode === 'summoner_contract_spirit' || skillCode === 'summoner_spirit_tether' || skillCode === 'summoner_star_pact') {
                ...
                if (!spirits.length) {
                    ...
                    const requirement = advancedResourceRequirementForSkill(skillCode);
                    if (requirement)
                        await gainResource(member, requirement.amount, '灵体未响应，返还消耗');
                    log.push('　&灵契&场上没有可回应的灵体，未消耗魔力。');
                    await triggerBulwarkFormation(member);
                    continue;
                }
                if (skillCode === 'summoner_contract_spirit') {
                    ...
                    for (const current of spirits)
                        await resolveCombatSpirits(connection, session.combat_id, members, targets, log, current, absorbRuleShield);
            ...
            if (skillCode === 'warlord_triumph_banner') {
                ...
                for (const ally of members.filter(item => !item.is_defeated)) {
                    ...
                    if (targets.filter(item => !item.is_defeated).length >= 3) {
                        const oldHp = Number(ally.current_hp);
                        ally.current_hp = Math.min(Number(ally.hp_max), oldHp + await specializedNativeHealing(member, ally, Math.floor(Number(ally.hp_max) * .08)));
                    }
                }
            ...
            if (skillCode === 'saint_healer_revival_sanctuary') {
                ...
                for (const ally of members.filter(item => !item.is_defeated)) {
                    const oldHp = Number(ally.current_hp);
                    ally.current_hp = Math.min(Number(ally.hp_max), oldHp + await specializedNativeHealing(member, ally, Math.floor(Number(ally.hp_max) * .18 * (1 + modifiers.healingBonusPct / 100))));
                    if (skillId)
            ...
            if (skillCode === 'healing_prayer' || skillCode === 'healing_light' || skillCode === 'saint_healer_mending_prayer' || skillCode === 'saint_healer_absolution_hand') {
                ...
                ally.current_hp = Math.min(Number(ally.hp_max), oldHp + await specializedNativeHealing(member, ally, amount));
                log.push(`➤【${member.name}】${label}`);
                log.push(`　➥【${ally.name}】恢复 ${ally.current_hp - oldHp} HP(${oldHp}→${ally.current_hp})`);
                if (skillId)
                ...
                if (lowHpForSaintInheritance && saintValue) {
                    ...
                    log.push(`&余辉援护&为低血的【${ally.name}】追加 ${saintValue}% 减伤壁垒(1)。`);
                }
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.code IN (${debuffCodes.map(() => '?').join(',')})`, [session.combat_id, struckTarget.id, ...debuffCodes]);
                ...
                const bodyReductionMultiplier = isBossComponent(struckTarget) ? 1 : bossBodyDamageMultiplier(struckTarget, targets);
                ...
                const shield = await ruleTakeDamage('target', Number(struckTarget.id), damage);
                const hpDamage = damage - shield.absorbed;
                await rules.afterHit(ruleUnit('member', Number(member.id)), ruleUnit('target', Number(struckTarget.id)), hpDamage, element, Boolean(skillId), shield.absorbed, extraTurn);
                await connection.execute('UPDATE combat_threat SET threat=threat+? WHERE session_id=? AND spawn_id=? AND character_id=?', [damage, session.combat_id, struckTarget.id, member.id]);
                ...
                if (skillCode === 'ranger_hundred_hunt' && !struckTarget.is_defeated) {
                    ...
                    if (ally) {
                        ...
                        const cooperation = resolveStrike(Number(ally.physical_attack) * .7, monster.physicalDefense, Number(ally.accuracy), monster.evasion, 0, monster.critResist, Number(ally.crit_damage_bp), monster.critReduction, false, false);
                        if (cooperation.hit) {
                            const oldHp = Number(struckTarget.current_hp);
                            ...
                            log.push(`　&协同追击&【${ally.name}】追加 ${followUp} 点基础物理伤害${lifeShieldAbsorptionText(shield)}(${oldHp}→${struckTarget.current_hp})。`);
                        }
                    }
                }
                ...
                if (mistTideReady) {
                    ...
                    member.cooldowns = cooldowns;
                    log.push(`&三首潮汐&【${member.name}】获得潮汐${stacks}/3。`);
                }
            ...
            if (effectValue('target', Number(monsterTarget.id), 'advanced_taunt') > 0) {
                ...
                if (tauntSource) {
                    victim = tauntSource;
                    log.push(`$盾墙嘲讽$【${targetName(monsterTarget)}】被迫锁定【${victim.name}】。`);
                }
            }
            ...
            if (isBossComponent(monsterTarget)) {
                ...
                const componentHit = async (target, scale, label) => {
                    ...
                    const strike = resolveStrike(Math.max(1, stats.physicalAttack * scale), Number(target.physical_defense), stats.accuracy, Number(target.evasion), 0, Number(target.crit_resist_bp), stats.critDamage, Number(target.crit_damage_reduction_bp), false, false);
                    if (!strike.hit) {
                        log.push(`　➥【${target.name}】闪避了${label}`);
                        return false;
                    }
                    ...
                    if (!target.current_hp)
                        target.is_defeated = 1;
                    log.push(`　➥${label}对【${target.name}】造成 ${dealt} 点物理伤害${lifeShieldAbsorptionText(shield)}(${oldHp}→${target.current_hp})`);
                    return true;
                };
          ...
          ON DUPLICATE KEY UPDATE value=VALUES(value),remaining_turns=VALUES(remaining_turns)`, [session.combat_id, monsterTarget.id]);
            ...
            const preserveAdvancedUndying = async (recipient) => {
                ...
                log.push(`&濒危不倒&【${recipient.name}】被不灭穹顶托住，生命保留为1。`);
                return true;
            };
            ...
            const afterAdvancedDamage = async (recipient, rawDamage, barrier, counterActive, primaryTarget, split) => {
                ...
                if (barrier > 0 && cooldowns.advanced_aegis_echo && rawDamage > 0) {
                    ...
                    if (recipient.current_hp > oldHp)
                        log.push(`　&光幕回响&【${recipient.name}】的壁垒碎光回流，恢复 ${recipient.current_hp - oldHp} HP(${oldHp}→${recipient.current_hp})。`);
                }
                if (primaryTarget && rawDamage > 0 && cooldowns.advanced_bulwark_mountain && !recipient.is_defeated) {
                    ...
                    if (hits >= 3) {
                        ...
                        log.push(`　&不动如山&【${recipient.name}】扛住三次攻击，反击架势已就绪。`);
                    }
                    recipient.cooldowns = cooldowns;
                }
            };
            ...
            if (pounce) {
                for (let index = 0; index < 3 && !victim.is_defeated; index += 1) {
                    ...
                    if ((skill?.category ?? 'physical') !== 'magic' && effectValue('member', Number(victim.id), 'device_physical_evade') > 0) {
                        ...
                        log.push(`　#物理闪避#【${victim.name}】避开了这次物理攻击。`);
                        continue;
                    }
                    ...
                    const oldHp = Number(victim.current_hp);
                    const shield = await ruleTakeDamage('member', Number(victim.id), dealt);
                    const hpDamage = dealt - shield.absorbed;
            ...
            for (const affectedVictim of affected) {
                ...
                if ((skill?.category ?? 'physical') !== 'magic' && effectValue('member', Number(affectedVictim.id), 'device_physical_evade') > 0) {
                    ...
                    log.push(`　#物理闪避#【${affectedVictim.name}】避开了这次物理攻击。`);
                    continue;
                }
                ...
                if (!affectedStrike.hit) {
                    ...
                    log.push(`　➥【${affectedVictim.name}】闪避了攻击`);
                    continue;
                }
                ...
                const shield = await ruleTakeDamage('member', Number(affectedVictim.id), finalDealt);
                ...
                if (king && !king.is_defeated) {
                    ...
                    for (const commandVictim of commandTargets.filter(member => !member.is_defeated)) {
                        ...
                        if (!commandStrike.hit) {
                            log.push(`　➥【${commandVictim.name}】闪避了王令雷击`);
                            continue;
                        }
    ...
    if (session.mode === 'spar' && (targets.every(target => target.is_defeated) || members.every(member => member.is_defeated) || aliveMembers.every(member => jsonObject(member.pending_action).type === 'escape'))) {
    ...
    if (victoryTargets.length && victoryTargets.every(target => target.is_defeated)) {
        ...
        await persistBattleMembers(connection, members);
        const clearedDungeon = await closeDungeonForBossSpawns(connection, victoryTargets.map(target => Number(target.id)));
