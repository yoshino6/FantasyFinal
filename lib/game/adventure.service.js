...
import { talentMovementFactor, talentMovementArrived, talentGatherReward, talentBeginGather } from './talent-exploration.js';
...
import { calculateDerivedStats, virtualEquipmentStats, realmEnergyDissipationText, realmLevelCap, experienceRequiredForLevel, forgedEquipmentBase } from './constants.js';
...
import { advancedResourceRequirementForSkill, advancedResourceForProfession } from './advanced-resource.config.js';
...
const traitList = (value) => jsonArray(value).map(item => jsonObject(item)).filter(trait => trait.code && (trait.name || ['npc_sparring', 'aeson_duel'].includes(trait.code)));
...
const materializeMonster = (monster, revealTraits = false) => {
    ...
    const displayTraits = traits.filter(trait => !['npc_sparring', 'aeson_duel', 'domain_resident', 'advanced_profession_trial', 'advanced_mentor_build', 'advanced_mentor_equipment', 'advanced_mentor_evolution'].includes(trait.code) && !(isBossComponent(monster) && trait.code === 'summoned'));
...
const materializeMonsters = (monsters, revealTraits = false) => monsters.map(monster => materializeMonster(monster, revealTraits)).sort((left, right) => monsterCombatStats(right).perception - monsterCombatStats(left).perception || Number(left.id) - Number(right.id));
...
const finalAttribute = (character, attribute) => Number(character[attribute]) + Number(character[`${attribute}_growth`]) * playerGrowthShares(Number(character.level));
...
const bossTestLoadout = (professionCode, level) => {
...
const adminStartBossTest = async (qqUserId, code, traitName) => withTransaction(async (connection) => {
    const [owners] = await connection.execute(`SELECT c.id,c.name,c.profession_code,c.current_region_id,c.pos_x,c.pos_y,c.pos_z
    ...
    const [participants] = partyRows[0]
        ? await connection.execute('SELECT c.id,c.name,c.profession_code,c.current_region_id,c.pos_x,c.pos_y,c.pos_z FROM party_members pm JOIN characters c ON c.id=pm.character_id WHERE pm.party_id=? ORDER BY pm.joined_at,c.id FOR UPDATE', [partyRows[0].party_id])
        : [owners];
    ...
    for (const item of loadout) {
      ...
      ON DUPLICATE KEY UPDATE name=VALUES(name),description=VALUES(description),weapon_type=VALUES(weapon_type),required_level=VALUES(required_level),effect_json=VALUES(effect_json)`, [item.code, item.name, `首领测试专用的 Lv.${level} 普通品质装备。`, item.category, item.weaponType, level, JSON.stringify({ ...item.effect, balanceVersion: 3 })]);
        const [definitions] = await connection.execute('SELECT id FROM item_definitions WHERE code=? LIMIT 1', [item.code]);
        const [instance] = await connection.execute('INSERT INTO player_item_instances (character_id,item_id,quality,durability,durability_max,effect_json) VALUES (?,?,100,100,100,?)', [owner.id, definitions[0].id, JSON.stringify({ ...item.effect, balanceVersion: 3 })]);
    ...
    const [skillCounts] = await connection.execute('SELECT COUNT(*) AS total FROM player_skills WHERE character_id=?', [owner.id]);
...
const skillList = async (qqUserId) => {
    ...
    const [skills] = await pool.execute('SELECT s.id,s.code,s.name,s.category,s.tier,ps.level,ps.quick_slot,ps.passive_linked,ps.learned_at FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id WHERE ps.character_id=? ORDER BY ps.learned_at,s.id', [character.id]);
...
const learnSkill = async (qqUserId, skillId) => withTransaction(async (connection) => {
    const character = await characterFor(qqUserId);
    const [skills] = await connection.execute(`SELECT s.name,s.code,s.learn_cost FROM player_skill_discoveries d JOIN skill_definitions s ON s.id=d.skill_id
    ...
    if (Number(character.skill_points) < Number(skill.learn_cost))
        throw new Error(`技能点不足，学习「${skill.name}」需要 ${skill.learn_cost} 点。`);
...
const mapCodeByRegion = {
    '百纳镇': 'map_baina_town',
    '世界树': 'map_world_tree',
    '幽暗密林': 'map_dark_forest',
    '幽暗密林深处': 'map_dark_forest_deep'
};
const hasRegionMap = async (connection, characterId, regionName) => {
...
const nearbyPoints = async (qqUserId) => {
    ...
    const dungeonCell = character.region_name === '地下迷宫'
        ...
        : null;
    const point = (type, item) => ({ type, name: item.name, x: Number(item.x), y: Number(item.y), distance: Math.abs(Number(item.x) - Number(character.pos_x)) + Math.abs(Number(item.y) - Number(character.pos_y)), code: item.code, interaction: item.interaction });
    ...
    const [landmarks] = mapUnlocked ? await pool.execute(`SELECT n.code,n.name,n.pos_x AS x,n.pos_y AS y,s.site_type AS siteType
    ...
    const [homes] = mapUnlocked ? await pool.execute(`SELECT 'player_home' AS code,CONCAT('我的小屋·',h.house_level,'级') AS name,h.plot_x AS x,h.plot_y AS y
    ...
    const mapMarkers = sortMapMarkers([...homes, ...visibleLandmarks]).map(item => ({ code: item.code, name: item.name, x: Number(item.x), y: Number(item.y), siteType: item.siteType, kind: 'landmark' }));
...
const moveToPosition = async (connection, qqUserId, x, y, restrictToPerception, speedLimit, destinationRegionId, allowRestingArrival = false) => {
    ...
    await (await import('./lamplight-access.js')).assertLamplightRegionAccess(connection, Number(character.id), region.code, partyRows[0] ? Number(partyRows[0].party_id) : undefined);
    ...
    if (region.code === 'baina_town') {
        const [unlocked] = await connection.execute('SELECT 1 FROM player_story_progress WHERE character_id=? AND story_code=\'forest_guide\' AND status=\'completed\' LIMIT 1', [character.id]);
        if (!unlocked[0])
            throw new Error('前方的百纳镇尚未找到入口，先在密林中继续前进吧。');
    }
    if (partyRows[0])
...
const moveTo = async (qqUserId, x, y, options = {}) => {
    if (!Number.isInteger(x) || !Number.isInteger(y))
        throw new Error('目标坐标必须为整数。');
    ...
    return withTransaction(async (connection) => {
        ...
        await assertOwnerOnlyRegionAccess(connection, qqUserId, Boolean(region.is_owner_only), Boolean(region.is_enabled), partyRows[0] ? Number(partyRows[0].party_id) : undefined);
        await (await import('./lamplight-access.js')).assertLamplightRegionAccess(connection, Number(character.id), region.code, partyRows[0] ? Number(partyRows[0].party_id) : undefined);
        const [ownerRows] = await connection.execute("SELECT 1 FROM game_permissions WHERE qq_user_id=? AND role='owner' LIMIT 1", [qqUserId]);
        ...
        if (!sameRegion && !targetMapUnlocked && !Boolean(region.is_release_managed && region.is_enabled) && !(isOwner && Boolean(region.is_owner_only)))
            throw new Error('尚未解锁目标区域地图，无法前往。');
        ...
        const [landmarks] = await connection.execute('SELECT name FROM map_npcs WHERE region_id=? AND pos_x=? AND pos_y=? AND pos_z=? AND interaction_kind=\'building\' LIMIT 1', [region.id, x, y, character.pos_z]);
        ...
        return { kind: 'travel', regionName: region.name, destinationName: landmarks[0]?.name, destinationKind, x, y, seconds, remaining: seconds };
    });
};
...
const combatTargets = async (connection, sessionId, revealTraits = false) => {
    const [rows] = await connection.execute(`SELECT s.id,s.template_id,t.name,t.monster_class,COALESCE(s.level,t.level) AS level,s.current_hp,s.traits_json,COALESCE(s.skill_sequence,t.skill_sequence) AS skill_sequence,${monsterAttributeColumns},t.experience,t.drops_json,t.weakness_json,t.resistance_json,t.element_mastery_json,t.element_resistance_json,ct.current_mp,ct.cooldowns,ct.is_defeated
...
const battleStatus = async (qqUserId) => {
    ...
    const [targetRows] = await pool.execute(`SELECT s.id,s.template_id,t.name,t.monster_class,COALESCE(s.level,t.level) AS level,s.current_hp,s.traits_json,COALESCE(s.skill_sequence,t.skill_sequence) AS skill_sequence,${monsterAttributeColumns},t.weakness_json,t.resistance_json,t.element_mastery_json,t.element_resistance_json,ct.current_mp,ct.cooldowns,ct.is_defeated FROM combat_targets ct JOIN monster_spawns s ON s.id=ct.spawn_id JOIN monster_templates t ON t.id=s.template_id WHERE ct.session_id=? ORDER BY s.id`, [session.combat_id]);
...
const inspectCombat = async (qqUserId) => {
    const character = await characterFor(qqUserId);
    const pool = await getPool();
    const [sessions] = await pool.execute(`SELECT cs.id AS combat_id,cs.turn_no FROM combat_members cm JOIN combat_sessions cs ON cs.id=cm.session_id WHERE cm.character_id=? AND cs.state='active' LIMIT 1`, [character.id]);
    const session = sessions[0];
    if (!session)
    ...
    const [targets] = await pool.execute(`SELECT s.id,s.template_id,t.name,t.monster_class,COALESCE(s.level,t.level) AS level,s.current_hp,s.traits_json,COALESCE(s.skill_sequence,t.skill_sequence) AS skill_sequence,${monsterAttributeColumns},t.experience,t.drops_json,t.weakness_json,t.resistance_json,t.element_mastery_json,t.element_resistance_json,ct.current_mp,ct.cooldowns,ct.is_defeated
    ...
    for (const raw of targets) {
        ...
        if (observer.informationLevel >= 2) {
            ...
            lines.push(`词条：${traitList(target.traits_json).map(trait => trait.name).join('、') || '无'}｜物攻 ${stats.physicalAttack}｜魔攻 ${stats.magicAttack}｜物防 ${stats.physicalDefense}｜魔防 ${stats.magicDefense}｜命中 ${stats.accuracy}｜闪避 ${stats.evasion}｜韧性 ${stats.tenacity}｜破韧 ${stats.tenacityPierce}`);
...
const monsterDetail = async (qqUserId, spawnId) => {
    ...
    const [rows] = await pool.execute(`SELECT s.id,s.template_id,t.name,t.monster_class,COALESCE(s.level,t.level) AS level,s.current_hp,s.traits_json,COALESCE(s.skill_sequence,t.skill_sequence) AS skill_sequence,${monsterAttributeColumns},t.experience,t.drops_json
    FROM monster_spawns s JOIN monster_templates t ON t.id=s.template_id
...
const switchCombatTarget = async (qqUserId, targetId, targetKind = 'target') => withTransaction(async (connection) => {
    ...
    if (targetKind === 'member')
        ...
        await connection.execute("UPDATE combat_members SET selected_target_id=?,cooldowns=JSON_REMOVE(COALESCE(cooldowns,JSON_OBJECT()),'$.__selectedAlly') WHERE session_id=? AND character_id=?", [targetId, session.combat_id, character.id]);
...
const summonShadowWolves = async (connection, sessionId, boss, members) => {
    ...
    for (let index = 0; index < 2; index += 1) {
        ...
        const spawned = { ...template, ...base, level: 10, traits_json: [] };
        const stats = monsterCombatStats(spawned);
...
const resolveCombatSpirits = async (connection, sessionId, members, targets, log, only, absorb = absorbLifeShield, ruleBridge) => {
    ...
    for (const spirit of spirits) {
        ...
        if (spirit.spirit_code === 'ember') {
            const spiritTargets = overload ? activeTargets : [activeTargets.find(item => Number(item.id) === Number(owner.selected_target_id)) ?? activeTargets[0]].filter(Boolean);
...
const summonNecromancerMinions = async (connection, sessionId, boss, members) => {
    ...
    for (let index = 0; index < 2; index += 1) {
        const base = randomMonsterBaseAttributes(template);
        const spawned = { ...template, ...base, level: 16, traits_json: [] };
        const stats = monsterCombatStats(spawned);
...
const combatActionInTransaction = async (connection, qqUserId, action, slot, skillId, itemId, deviceSkillCode, targetKind, targetId, automaticChant = false, hiddenTicket, hiddenAutomatic = false, skipPlayerTurn = false) => {
    ...
    const targetName = (target) => { const observer = appraisalForTarget(appraisal, Number(target.level)); if (readRuleState(jsonObject(target.cooldowns).__rules).statuses.some(effect => effect.code === 'nightmare' && effect.until >= Number(session.turn_no)))
        return '信息被雾遮蔽'; if (session.mode === 'spar' || traitList(target.traits_json).some(trait => trait.code === 'aeson_duel'))
    ...
    const resolveKingbeastState = () => {
        ...
        if (fallenDuringFusion.length) {
            ...
            if (survivor) {
                ...
                cooldowns.royal_beast_enrage = 3;
        ...
        const living = cores.filter(target => !target.is_defeated);
        const fallen = cores.filter(target => target.is_defeated);
        if (fallen.length === 1 && living.length === 1) {
            const survivor = living[0];
            const cooldowns = jsonObject(survivor.cooldowns);
            if (!cooldowns.royal_beast_enrage) {
                cooldowns.royal_beast_enrage = 3;
                survivor.cooldowns = cooldowns;
                log.push(`$${kingbeastRole(survivor) === 'king' ? '孤王狂怒' : '孤龙狂怒'}$同伴倒下，存活者的怒火彻底失控！\n命中 +200%｜暴击 +200%｜暴伤 +200%｜攻击吸血 40%，持续 3 回合。`);
            }
        }
    };
    ...
    rules.hooks.directMultiplier = (source, target, element, magic, single, damageType) => {
        ...
        return physical * body * weatherElementMultiplier$1(element, source.side === 'member' ? Number(source.key.split(':')[1]) : undefined) * (1 - Math.min(80, rules.value(target, 'barrier')) / 100);
    };
    ...
    for (const turn of turns) {
        ...
        try {
          ...
          WHERE energy.battle_kind='pve' AND energy.session_id=? AND energy.character_id=? AND energy.instance_id=? FOR UPDATE`, [session.combat_id, member.id, Number(choice.deviceInstanceId)]);
                ...
                let target = targets.find(item => Number(item.id) === Number(choice.targetKind === 'target' ? choice.targetId : member.selected_target_id) && !item.is_defeated) ?? targets.find(item => !item.is_defeated);
                if (!target)
                    continue;
                if (effectValue('member', Number(member.id), 'alchemy_confusion') > 0) {
                    const candidates = targets.filter(item => !item.is_defeated);
                    target = candidates[random(0, candidates.length - 1)] ?? target;
                ...
                if (counterReady > 0) {
                    ...
                    if (Math.random() * 100 < counterReady) {
                        ...
                        if (counterStrike.hit) {
                            ...
                            const dealt = directDamageVariance(Math.max(1, Math.floor(counterStrike.damage * bodyMultiplier)));
                ...
                if (skillCode === 'summoner_contract_spirit' || skillCode === 'summoner_spirit_tether' || skillCode === 'summoner_star_pact') {
                    ...
                    if (skillCode === 'summoner_contract_spirit') {
                        ...
                        for (const current of spirits)
                            await resolveCombatSpirits(connection, session.combat_id, members, targets, log, current, absorbRuleShield, rules);
                ...
                if (expanded)
                    skillTargetScope = '全体';
                const affectedTargets = skillTargetScope === '全体' ? targets.filter(item => !item.is_defeated) : [target];
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.code IN (${debuffCodes.map(() => '?').join(',')})`, [session.combat_id, struckTarget.id, ...debuffCodes]);
                    ...
                    if (skillCode === 'ranger_hundred_hunt' && !struckTarget.is_defeated) {
                        ...
                        if (ally) {
                            ...
                            if (cooperation.hit) {
                                const oldHp = Number(struckTarget.current_hp);
                                const followUp = directDamageVariance(Math.max(1, cooperation.damage));
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.code IN (${freshDebuffCodes.map(() => '?').join(',')})`, [session.combat_id, struckTarget.id, ...freshDebuffCodes]);
                ...
                const [templateRows] = await connection.execute('SELECT code FROM monster_templates WHERE id=?', [Number(monsterTarget.template_id)]);
                const templateCode = templateRows[0]?.code ?? '';
                ...
                if (fusedKingbeast && kingbeastRole(monsterTarget) === 'king')
                    continue;
                const sequence = isWolfKing ? ['wolfking_summon_shadow_wolf', 'wolfking_trample', 'wolfking_rending_pounce', 'wolfking_bite', 'wolfking_shadow_curse', 'wolfking_fang_devour', 'boss_mana_charge'] : stringList(monsterTarget.skill_sequence);
            ...
            WHERE ct.session_id=? AND ct.is_defeated=0 AND s.defeated_at IS NULL AND t.code='shadow_wolf' LIMIT 1`, [session.combat_id]);
                    ...
                    if (!needsManaCharge && isKingbeast) {
                        ...
                        if (fusedKingbeast && role === 'dragon') {
                            ...
                            if (eliteAlive < 2 && Number(cooldowns.royal_elite_summons ?? 0) < 2)
                                skill = choose('goblin_king_call_elites') ?? skill;
                            else if (!cooldowns.kingbeast_regal_used) {
                                skill = choose('goblin_king_regal_conduct') ?? skill;
                                cooldowns.kingbeast_regal_used = 1;
                            }
                            else if (members.filter(member => !member.is_defeated && Number(member.current_hp) / Math.max(1, Number(member.hp_max)) < .6).length >= 2)
                            ...
                            if ((hpRatio < .6 || kingbeastCores().some(target => target.is_defeated)) && choose('habadragon_bloodjaw'))
                    ...
                    if (isDungeonBoss) {
                        ...
                        if (templateCode === 'black_slime') {
                            ...
                            const alive = members.filter(member => !member.is_defeated);
                            const markedMember = (key) => alive.find(member => Number(jsonObject(member.cooldowns)[key] ?? 0) >= Number(session.turn_no));
                            const reapingTarget = markedMember('regional_uzz_reap_until');
                            const brandedTarget = markedMember('regional_uzz_gravebrand_until');
                            ...
                            if ((hpRatio <= .58 || selfDebuffed) && !cooldowns.regional_uzz_phylactery_used) {
                                skill = choose('uzz_phylactery_turn') ?? skill;
                                cooldowns.regional_uzz_phylactery_used = 1;
                            }
                            else if (!minionsAlive && Number(cooldowns.regional_uzz_revenant_calls ?? 0) < 2) {
                                skill = choose('uzz_revenant_call') ?? skill;
                                cooldowns.regional_uzz_revenant_calls = Number(cooldowns.regional_uzz_revenant_calls ?? 0) + 1;
                            }
                            else if (reapingTarget) {
                                skill = choose('uzz_soul_reaping') ?? skill;
                                victim = reapingTarget;
                            }
                            else if (brandedTarget) {
                                skill = choose('uzz_choir_of_graves') ?? skill;
                            }
                            else {
                                const sequence = ['uzz_gravebrand', 'uzz_marrow_lash'];
                                const index = Math.max(0, Number(cooldowns.regional_uzz_rotation ?? 0)) % sequence.length;
                                skill = choose(sequence[index]) ?? skill;
                                cooldowns.regional_uzz_rotation = index + 1;
                                if (skill?.code === 'uzz_marrow_lash' && lowest)
                                    victim = lowest;
                            }
                        }
                    }
          ...
          ON DUPLICATE KEY UPDATE value=VALUES(value),remaining_turns=VALUES(remaining_turns)`, [session.combat_id, monsterTarget.id]);
                ...
                if (skill?.code === 'goblin_king_regal_conduct' || skill?.code === 'goblin_royal_signal_flag') {
                    ...
                    for (const ally of targets.filter(target => !target.is_defeated && String(kingbeastTrait(target)?.groupId ?? '') === groupId)) {
                        const allyCooldowns = jsonObject(ally.cooldowns);
                        allyCooldowns[skill.code === 'goblin_king_regal_conduct' ? 'royal_regal' : 'royal_signal'] = skill.code === 'goblin_king_regal_conduct' ? 3 : 2;
                        ally.cooldowns = allyCooldowns;
                    }
                    log.push(`➤【${targetName(monsterTarget)}】释放技能「${skill.name}」\n　$${skill.code === 'goblin_king_regal_conduct' ? '王权导律' : '王旗电令'}$在场王庭单位获得强化。`);
                    continue;
                }
                if (skill?.code === 'goblin_king_call_elites') {
                    monsterTarget.current_mp -= Number(skill.mana_cost);
                    cooldowns[skill.code] = Number(skill.cooldown_turns) + 1;
                    cooldowns.royal_elite_summons = Number(cooldowns.royal_elite_summons ?? 0) + 1;
                    monsterTarget.cooldowns = cooldowns;
                    const code = Math.random() < .5 ? 'goblin_royal_guard' : 'goblin_royal_spearman';
                    const [templates] = await connection.execute(`SELECT t.id,t.level,t.monster_class,t.skill_sequence,${templateMonsterAttributeColumns} FROM monster_templates t WHERE t.code=?`, [code]);
                    const template = templates[0];
                    const groupId = String(kingbeastTrait(monsterTarget)?.groupId ?? '');
                    if (template) {
                        const attributes = randomMonsterBaseAttributes(template);
                        const mainQuestTrait = mainQuestGoblinKingTrait(monsterTarget);
                        const summonedLevel = mainQuestTrait ? 18 : 28;
                        const traits = [{ code: 'summoned', name: '召唤的' }, ...(mainQuestTrait ? [mainQuestTrait] : []), { code: 'kingbeast_encounter', name: '', groupId, role: code === 'goblin_royal_guard' ? 'guard' : 'spearman' }];
                        const spawned = { ...template, ...attributes, level: summonedLevel, traits_json: traits };
                        const stats = monsterCombatStats(spawned);
                        const [locations] = await connection.execute('SELECT region_id,pos_x,pos_y,pos_z FROM monster_spawns WHERE id=?', [monsterTarget.id]);
                        const location = locations[0];
                        ...
                        await connection.execute('INSERT INTO combat_targets (session_id,spawn_id,current_mp,cooldowns) VALUES (?,?,?,JSON_OBJECT())', [session.combat_id, created.insertId, stats.mpMax]);
                        for (const member of members)
                            await connection.execute('INSERT INTO combat_threat (session_id,spawn_id,character_id,threat) VALUES (?,?,?,1)', [session.combat_id, created.insertId, member.id]);
                    }
                    log.push(`➤【${targetName(monsterTarget)}】释放技能「${skill.name}」\n　$王庭再征$新的王庭精锐踏入战场。`);
                    continue;
                }
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.effect_type IN ('damage_over_time','stat_modifier','control') AND e.code<>'life_shield'`, [session.combat_id, monsterTarget.id]);
                ...
                const royalEnrage = Number(cooldowns.royal_beast_enrage ?? 0) > 0;
                ...
                const monsterAttack = (skill?.category === 'magic' ? monster.magicAttack * (royalRegal ? 1.35 : 1) : monster.physicalAttack) * (1 + mentorDamageBonus / 100) * (aesonInspired ? 1.6 : 1) * (aesonBerserk ? 1.3 : 1);
                ...
                const shieldCounterDamage = async (defender, rawDamage) => {
                    ...
                    const reflected = Math.max(1, Math.floor(rawDamage * .5));
                ...
                if (spiritVictim) {
                    ...
                    if (!strike.hit)
                        ...
                        const dealt = directDamageVariance(Math.max(1, strike.damage));
                ...
                if (pounce) {
                    for (let index = 0; index < 3 && !victim.is_defeated; index += 1) {
                        ...
                        const damage = timeGuarded ? 0 : directDamageVariance(Math.max(1, Math.floor(strike.damage * elemental * weatherElement * (1 + exposed / 100) * (1 - phaseDecoy / 100) * (1 - counter.damageReductionPct / 100) * (1 - Math.min(80, barrier) / 100) * (1 - Math.min(90, guard) / 100) * (1 - damageReduction / 100))));
                ...
                for (const affectedVictim of affected) {
                    ...
                    const dealt = effectValue('member', Number(affectedVictim.id), 'time_guard') > 0 ? 0 : directDamageVariance(Math.max(1, Math.floor(affectedStrike.damage * elemental * weatherElementMultiplier$1(String(skill?.element ?? '')) * (1 + exposed / 100) * (1 - phaseDecoy / 100) * (1 - counter.damageReductionPct / 100) * (1 - Math.min(80, barrier) / 100) * (1 - Math.min(90, guard) / 100) * (1 - damageReduction / 100))));
                    ...
                    if ((royalEnrage || skill?.code === 'habadragon_bloodjaw') && finalDealt > 0) {
                        const oldHp = Number(monsterTarget.current_hp);
                        const restored = Math.min(Number(monsterTarget.hp_max) - oldHp, targetHealingAmount(monsterTarget, Math.max(1, Math.floor(finalDealt * (royalEnrage ? .4 : .35)))));
                        monsterTarget.current_hp += restored;
                        if (restored)
                            log.push(`&${royalEnrage ? '狂暴吸血' : '裂颚汲战'}&恢复 ${restored} HP(${oldHp}→${monsterTarget.current_hp})`);
                    }
