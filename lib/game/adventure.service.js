...
import { readRuleState, ruleStatusSummary, displayedRuleName, ruleManaCost, visibleResidentBuff, maskRuleBattleLog } from './combat-rule-registry.js';
...
import { repairEvolutionProgress, evolutionStatBonuses, advanceEvolutionObservationMining, symbiosisTraits, advanceEvolutionObservationBattle } from './evolution.service.js';
...
const settleResourceMining = async (connection, character, cancel = false, resourceId) => {
    const [rows] = await connection.execute(`SELECT m.resource_id,rs.item_id,rs.region_id AS resource_region_id,rs.pos_x,rs.pos_y,rs.pos_z,i.code,i.name,i.item_category,m.started_at,m.finishes_at FROM player_resource_mining m
    ...
    while (new Date(mining.finishes_at).getTime() <= now) {
        ...
        const [nextRows] = await connection.execute(`SELECT rs.id AS resource_id,rs.item_id,rs.region_id AS resource_region_id,rs.pos_x,rs.pos_y,rs.pos_z,i.code,i.name,i.item_category FROM resource_spawns rs JOIN item_definitions i ON i.id=rs.item_id
...
const useCombatConsumable = async (connection, sessionId, member, targets, itemId, itemName, rawEffect, ruleBridge) => {
    ...
    if (restoreMp) {
        member.current_mp = Math.min(Number(member.mp_max), oldMp + restoreMp);
...
const finishPartyVictory = async (connection, sessionId, members, targets) => {
    ...
    const grantDrop = async (recipient, code, quantity) => {
        const [items] = await connection.execute('SELECT id,code,name,item_type,codex_id FROM item_definitions WHERE code=?', [code]);
        const item = items[0];
...
const combatActionInTransaction = async (connection, qqUserId, action, slot, skillId, itemId, deviceSkillCode, targetKind, targetId, automaticChant = false, hiddenTicket, hiddenAutomatic = false, skipPlayerTurn = false) => {
    ...
    const triggerSummonerShelter = async (victim) => {
        if (Number(victim.current_hp) / Math.max(1, Number(victim.hp_max)) > .5)
            return;
        for (const owner of members.filter(member => !member.is_defeated)) {
            ...
            const key = 'heritage_once_spirit_summoner';
            if (cooldowns[key])
                continue;
            cooldowns[key] = 1;
    ...
    const triggerAegisEcho = async (victim, hadBarrier) => {
        if (!hadBarrier)
            return;
        const value = Math.max(...members.map(member => inheritanceValue(Number(member.id), 'aegis_priest')), 0);
        if (!value)
            return;
        const cooldowns = jsonObject(victim.cooldowns);
    ...
    const consumeSpellbladeSupport = (member) => {
        const cooldowns = jsonObject(member.cooldowns);
    ...
    rules.hooks.directMultiplier = (source, target, element, magic, single, damageType) => {
        ...
        return physical * body * uzzTarget * frostPower * kingbeastReduction * randomEffect * weatherElementMultiplier$1(element, source.side === 'member' ? Number(source.key.split(':')[1]) : undefined) * (1 - Math.min(80, rules.value(target, 'barrier')) / 100);
    };
        ...
        try {
          ...
          WHERE energy.battle_kind='pve' AND energy.session_id=? AND energy.character_id=? AND energy.instance_id=? FOR UPDATE`, [session.combat_id, member.id, Number(choice.deviceInstanceId)]);
                ...
                if (skillCode === 'aegis_shared_vow') {
                    ...
                    for (const ally of recipients) {
                        await applyAdvancedStatus('member', Number(ally.id), 'barrier', 20, 2, true);
                        markAegisBarrier(member, ally, 2);
                    }
                ...
                if (skillCode === 'saint_healer_resonant_mass') {
                    ...
                    for (const ally of members.filter(item => !item.is_defeated)) {
                        ...
                        const amount = receivedHealingAmount(Math.max(1, Math.floor((Number(member.magic_attack) * .65 + Number(ally.hp_max) * .05) * activeHealingMultiplier(modifiers.healingBonusPct, modifiers.activeHealingBonusPct))), received.healingReceivedPct);
                ...
                if (skillCode === 'saint_healer_revival_sanctuary') {
                    ...
                    for (const ally of members.filter(item => !item.is_defeated)) {
                        const oldHp = Number(ally.current_hp);
                        ally.current_hp = Math.min(Number(ally.hp_max), oldHp + await specializedNativeHealing(member, ally, Math.floor(Number(ally.hp_max) * .18 * activeHealingMultiplier(modifiers.healingBonusPct, modifiers.activeHealingBonusPct)), true));
                        if (skillId)
                            await applySkillEffects(connection, session.combat_id, skillId, member, 'member', ally, 'member', 'on_cast', [], 0, 1, rules);
                        await triggerEpicEffectiveHeal(member, ally, oldHp, oldHp / Math.max(1, Number(ally.hp_max)) < .5);
                ...
                if (skillCode === 'aegis_luminous_echo') {
                    for (const ally of members.filter(item => !item.is_defeated)) {
                        await applyAdvancedStatus('member', Number(ally.id), 'barrier', 10, 2, true);
                        markAegisBarrier(member, ally, 2);
                        await applyAdvancedStatus('member', Number(ally.id), 'regeneration', 5, 1, true);
                        const cooldowns = jsonObject(ally.cooldowns);
                ...
                if (skillCode === 'ranger_guiding_smoke') {
                    for (const ally of members.filter(item => !item.is_defeated)) {
                        if (skillId)
                            await applySkillEffects(connection, session.combat_id, skillId, member, 'member', ally, 'member', 'on_cast', [], 0, 1, rules);
                        await applyAdvancedStatus('member', Number(ally.id), 'alchemy_evasion', 20, 2, true);
                    }
                    for (const enemy of targets.filter(item => !item.is_defeated))
                        await applyAdvancedStatus('target', Number(enemy.id), 'imbalance', 15, 2, true);
                ...
                if (skillCode === 'healing_prayer' || skillCode === 'healing_light' || skillCode === 'saint_healer_mending_prayer' || skillCode === 'saint_healer_absolution_hand') {
                    ...
                    if (spellbladeSupport)
                        log.push(`&攻势换挡&本次治疗强化 ${spellbladeSupport}%，下一次伤害技能已蓄势。`);
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.code IN (${debuffCodes.map(() => '?').join(',')})`, [session.combat_id, struckTarget.id, ...debuffCodes]);
                    ...
                    const dawnKey = `heritage_dawn_${member.id}`;
                    const dawnDamage = skillId && targetCooldowns[dawnKey] ? inheritanceValue(Number(member.id), 'dawn_inquisitor') : 0;
                    ...
                    const inheritanceDamage = resonanceDamage + dawnDamage + nightDamage + spellbladeDamage + elementGuideDamage;
                    ...
                    const rangerOwner = huntBonus > 0 ? members.find(item => Number(item.id) === rangerSourceId && !item.is_defeated) : undefined;
                    if (rangerOwner)
                        await gainResource(rangerOwner, 15, '队友命中追猎目标');
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.code IN (${freshDebuffCodes.map(() => '?').join(',')})`, [session.combat_id, struckTarget.id, ...freshDebuffCodes]);
                    ...
                    if (dawnTrigger) {
                        const dawnValue = inheritanceValue(Number(member.id), 'dawn_inquisitor');
                        if (dawnValue) {
                            const cooldowns = jsonObject(struckTarget.cooldowns);
                            cooldowns[dawnKey] = 2;
                            struckTarget.cooldowns = cooldowns;
                            log.push(`&晨钟裁意&${dawnTrigger}，下一次对该目标的技能直击伤害 +${dawnValue}%（至下回合结束）。`);
                        }
                    }
                    const rangerValue = inheritanceValue(Number(member.id), 'trickster_ranger');
                    const joinedHit = Object.keys(jsonObject(struckTarget.cooldowns)).some(key => key.startsWith('heritage_round_hit_') && key !== `heritage_round_hit_${member.id}`);
                    if (!rangerTriggered && rangerValue && joinedHit) {
                        await refreshCombatSpiritEffect(connection, session.combat_id, 'target', Number(struckTarget.id), 'slow', rangerValue, 1);
                        rangerTriggered = true;
                        log.push(`&猎线回响&【${targetName(struckTarget)}】减速 ${rangerValue}%(1)。`);
                    }
                });
                if (skillId && hasInheritance(Number(member.id), 'spellblade')) {
                    const cooldowns = jsonObject(member.cooldowns);
          ...
          ON DUPLICATE KEY UPDATE value=VALUES(value),remaining_turns=VALUES(remaining_turns)`, [session.combat_id, monsterTarget.id]);
            ...
            WHERE ce.session_id=? AND ce.target_kind='target' AND ce.target_id=? AND e.effect_type IN ('damage_over_time','stat_modifier','control') AND e.code<>'life_shield'`, [session.combat_id, monsterTarget.id]);
                ...
                const rewardAegisBarrierFaith = async (recipient, barrier, rawDamage) => {
                    if (barrier <= 0 || rawDamage <= 0)
                        return;
                    const cooldowns = jsonObject(recipient.cooldowns);
                    const sourceId = Number(cooldowns.advanced_aegis_barrier_source ?? 0);
                    ...
                    if (!priest || cooldowns.advanced_aegis_barrier_faith)
                        return;
                    cooldowns.advanced_aegis_barrier_faith = 1;
                    recipient.cooldowns = cooldowns;
