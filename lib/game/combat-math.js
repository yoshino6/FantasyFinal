...
const correctedCritChance = (chance, correction = {}) => Math.max(0, Math.min(1, chance + Math.max(0, Number(correction.actualCritRatePct ?? 0)) / 100)) * (1 - correctionRate(correction.critAvoidanceCorrectionPct));
