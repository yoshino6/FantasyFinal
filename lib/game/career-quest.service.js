...
import { advancedProfessionByCode, worldTreeAdvancedProfessions } from './advanced-profession.config.js';
import { openingHubs } from './opening-world.config.js';

const careerCharacterFor = async (pool, qqUserId) => {
    const [rows] = await pool.execute(`SELECT c.id,c.level,c.adventurer_registered,c.profession_code,c.current_region_id,
...
const guildCareerQuestFor = async (pool, character) => {
    if (Number(character.adventurer_registered) && character.profession_code)
        return null;
    const local = openingHubs[character.region_code];
    if (local && character.region_code !== 'baina_town')
        return {
    ...
    if (!Number(character.adventurer_registered))
        return {
            title: '【主线·成为冒险者】',
...
const guildCareerMainQuest = async (qqUserId) => {
    const pool = await getPool();
    return guildCareerQuestFor(pool, await careerCharacterFor(pool, qqUserId));
};
const advancedProfessionMainQuest = async (qqUserId) => {
    const pool = await getPool();
    const character = await careerCharacterFor(pool, qqUserId);
    if (Number(character.level) < 25)
        return null;
    await (await import('./progression-map.service.js')).repairProgressionMaps(qqUserId);
    const [[quests], [completed]] = await Promise.all([
        pool.execute('SELECT profession_code,stage,story_kills,proof_kills FROM player_advanced_profession_quests WHERE character_id=? AND stage IN (1,2,3) ORDER BY stage DESC,profession_code ASC LIMIT 1', [character.id]),
        pool.execute('SELECT profession_code FROM player_advanced_professions WHERE character_id=? LIMIT 1', [character.id])
    ]);
    const active = quests[0];
    if (!active && completed[0])
        return null;
    if (character.adventurer_registered && character.profession_code && ['floating_leaf_town', 'frost_dragon_inn'].includes(character.region_code))
        return { title: '【主线·二转之路】', description: '二转导师在世界树。先回当地公会后勤区，乘安全接驳到世界树，再拜访导师。', action: { label: '[前往 安全接驳]', command: '/初行公会 接驳' } };
    const profession = active ? advancedProfessionByCode(active.profession_code) : undefined;
    const mentorAction = (route) => character.region_code === 'world_tree'
    ...
    if (!active) {
        ...
        if (prerequisite)
            return { ...prerequisite, title: '【主线·二转之路】', description: '你已达到 Lv.25，可以开始寻找二转导师。先完成冒险者注册与主职业选择，再决定要深入哪一条道路。\n\n' + prerequisite.description };
    ...
    if (!profession)
        throw new Error('当前二转试炼的职业配置不存在。');
...
export { advancedProfessionMainQuest, guildCareerMainQuest };
