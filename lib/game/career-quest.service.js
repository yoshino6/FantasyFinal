...
const advancedProfessionMainQuest = async (qqUserId) => {
    ...
    const [[quests], [completed], [cores]] = await Promise.all([
        ...
        pool.execute(`SELECT pi.quantity FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.code='ridge_core' LIMIT 1`, [character.id])
    ]);
