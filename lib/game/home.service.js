...
const consumeMaterials = async (connection, characterId, materials) => {
    for (const [code, quantity] of Object.entries(materials)) {
        ...
        if (!rows[0] || Number(rows[0].quantity) < quantity)
            throw new Error(`材料不足：${code} 需要 ${quantity}。`);
...
const upgradeHome = async (qqUserId) => withTransaction(async (connection) => {
    ...
    return { level: Number(home.house_level) + 1, cost };
});
...
const craftFurniture = async (qqUserId, code, floor, _legacySlotKey) => withTransaction(async (connection) => {
    ...
    for (const material of recipe) {
        ...
        if (!owned[0] || Number(owned[0].quantity) < Number(material.quantity))
            throw new Error(`材料不足：${material.name}×${material.quantity}。`);
    }
    for (const material of recipe) {
        await connection.execute('UPDATE player_inventory SET quantity=quantity-? WHERE character_id=? AND item_id=?', [material.quantity, character.id, material.item_id]);
...
const depositHomeStorage = async (qqUserId, itemId, quantity) => withTransaction(async (connection) => {
    ...
    if (!capacity)
        throw new Error('尚未摆放储物箱，暂时没有可用仓储空间。');
    const [rows] = await connection.execute('SELECT pi.item_id,i.name,pi.quantity,i.weight FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.item_id=? AND pi.quantity>0 AND i.stackable=1 FOR UPDATE', [character.id, itemId]);
    const item = rows[0];
    if (!item)
