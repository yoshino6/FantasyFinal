...
import { talentMaterialPayment, consumeTalentMaterial } from './talent-production.js';
...
import { slotsPerFloor, homeCosts, BAINA_RESIDENCE_CODE, BAINA_GUILD_POSITION, homePlotDistance } from './home.constants.js';
import { backfillHomeFloorLayout, findFurniturePlacement, occupyFurnitureCells } from './home-layout.service.js';
...
const assertAtResidence = async (connection, character) => {
    const [rows] = await connection.execute(`SELECT 1 FROM map_npcs WHERE code=? AND region_id=? AND pos_x=? AND pos_y=? AND pos_z=? LIMIT 1`, [BAINA_RESIDENCE_CODE, character.current_region_id, character.pos_x, character.pos_y, character.pos_z]);
    if (!rows[0])
        throw new Error('请先前往百纳镇的百纳居。');
};
const assertFree = async (connection, character, allowResting = false) => {
    if (character.activity_status === 'detained')
        throw new Error('你正在被守卫关押。');
    if (character.activity_status === 'unconscious')
        throw new Error('你已昏迷，暂时无法进入家园。');
    if (!allowResting && character.activity_status === 'resting')
        throw new Error('请先结束休息。');
    ...
    const [[travel], [mining], [combat], [pvp], [party]] = await Promise.all([
        ...
        connection.execute(`SELECT 1 FROM player_pvp_battle_sessions WHERE state='active' AND (attacker_character_id=? OR defender_character_id=?) LIMIT 1`, [character.id, character.id]),
        connection.execute('SELECT 1 FROM party_members WHERE character_id=? LIMIT 1', [character.id])
    ]);
    if (travel[0])
        throw new Error('移动或寻怪尚未结束。');
    if (mining[0])
        throw new Error('开采尚未结束。');
    if (combat[0] || pvp[0])
        throw new Error('战斗尚未结束。');
    if (party[0])
        throw new Error('请先离开队伍后再进入家园。');
};
const addItem = async (connection, characterId, itemId, quantity) => {
...
const consumeMaterials = async (connection, characterId, materials) => {
    for (const [code, quantity] of Object.entries(materials)) {
        ...
        if (!rows[0] || Number(rows[0].quantity) < paid)
            throw new Error(`材料不足：${code} 实际需要 ${paid}。`);
...
const homeEffects = (furniture) => furniture.reduce((result, item) => {
    for (const [key, value] of Object.entries(json(item.effect_json)))
...
const homePanel = async (qqUserId) => {
    ...
    if (!home)
        return { character, home: null, inHome: false, furniture: [], effects: {}, materials: [] };
...
const choosePlot = async (connection, townId) => {
    const candidates = [];
    for (let x = -25; x <= 24; x++)
        for (let y = -185; y <= -136; y++) {
            const distance = Math.abs(x - BAINA_GUILD_POSITION.x) + Math.abs(y - BAINA_GUILD_POSITION.y);
            if (distance >= homePlotDistance.min && distance <= homePlotDistance.max)
                candidates.push([x, y]);
        }
...
const purchaseHome = async (qqUserId) => withTransaction(async (connection) => {
    ...
    if (await homeFor(connection, character.id, true))
        throw new Error('你已经拥有一间小屋。');
    const plot = await choosePlot(connection, character.current_region_id);
    const [paid] = await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=? AND copper_coins>=?', [homeCosts.purchase.copper, character.id, homeCosts.purchase.copper]);
    if (!Number(paid.affectedRows))
        throw new Error(`铜币不足，需要 ${homeCosts.purchase.copper} 铜币。`);
    const homeName = `${character.name}的小屋`;
    const [created] = await connection.execute('INSERT INTO player_homes (character_id,home_name,town_region_id,plot_x,plot_y,plot_z) VALUES (?,?,?,?,?,?)', [character.id, homeName, character.current_region_id, plot.x, plot.y, plot.z]);
    const [homeEvent] = await connection.execute('INSERT INTO player_events (player_id,event_type,payload) VALUES (?,\'home.purchased\',JSON_OBJECT(\'homeId\',?,\'x\',?,\'y\',?))', [character.player_id, created.insertId, plot.x, plot.y]);
...
const enterHome = async (qqUserId) => withTransaction(async (connection) => {
    ...
    if (character.region_code !== 'baina_town' || Number(character.pos_x) !== Number(home.plot_x) || Number(character.pos_y) !== Number(home.plot_y) || Number(character.pos_z) !== Number(home.plot_z))
        throw new Error('请先前往自己的小屋地块。');
    await assertFree(connection, character);
    if (await isInHome(connection, Number(character.id)))
        throw new Error('你已经在自己的家园中。');
...
const leaveHome = async (qqUserId) => withTransaction(async (connection) => {
    const character = await characterFor(connection, qqUserId, true);
    const home = await homeFor(connection, character.id, true);
    if (!home || !await isInHome(connection, character.id))
        throw new Error('你当前不在自己的家园中。');
    const { settleHomeRestExperience } = await import('./adventure.service.js');
...
const renameHome = async (qqUserId, input) => withTransaction(async (connection) => {
    ...
    if (Array.from(name).length < 2 || Array.from(name).length > 32 || /[\r\n]/.test(name))
        throw new Error('小屋名称需为 2～32 个字符，且不能包含换行。');
    const character = await characterFor(connection, qqUserId, true);
    const home = await homeFor(connection, character.id, true);
    if (!home)
        throw new Error('你还没有小屋。');
    if (name === home.home_name)
        return { name };
    await connection.execute('UPDATE player_homes SET home_name=? WHERE id=?', [name, home.id]);
    const [renameEvent] = await connection.execute('INSERT INTO player_events (player_id,event_type,payload) VALUES (?,\'home.renamed\',JSON_OBJECT(\'homeId\',?,\'name\',?))', [character.player_id, home.id, name]);
...
const upgradeHome = async (qqUserId) => withTransaction(async (connection) => {
    ...
    if (!cost)
        throw new Error('房屋已经达到最高等级。');
    await consumeMaterials(connection, character.id, { ...cost.materials });
    const [paid] = await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=? AND copper_coins>=?', [cost.copper, character.id, cost.copper]);
    if (!Number(paid.affectedRows))
        throw new Error(`铜币不足，需要 ${cost.copper} 铜币。`);
...
const expandHome = async (qqUserId, floor) => withTransaction(async (connection) => {
    ...
    if (Number(home.floor_count) + 1 !== target || Number(home.house_level) < target || (target === 3 && Number(home.floor_count) < 2))
        throw new Error(target === 2 ? '需要房屋 Lv.2 且尚未扩建二层。' : '需要房屋 Lv.3 且已拥有二层。');
    await consumeMaterials(connection, character.id, { ...cost.materials });
    const [paid] = await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=? AND copper_coins>=?', [cost.copper, character.id, cost.copper]);
    if (!Number(paid.affectedRows))
        throw new Error(`铜币不足，需要 ${cost.copper} 铜币。`);
...
const listFurniture = async (qqUserId, floor) => {
    ...
    if (!home)
        throw new Error('你还没有小屋。');
    if (floor)
        await withTransaction(connection => backfillHomeFloorLayout(connection, Number(home.id), floor, Number(home.house_level)));
    ...
    recipes[0].forEach(row => recipesByCode.set(row.furniture_code, [...(recipesByCode.get(row.furniture_code) ?? []), { name: row.name, quantity: Number(row.quantity) }]));
...
const craftFurniture = async (qqUserId, code, floor, _legacySlotKey) => withTransaction(async (connection) => {
    ...
    if (!home)
        throw new Error('你还没有小屋。');
    if (!Number.isInteger(floor) || floor < 1 || floor > Number(home.floor_count))
    ...
    if (!definition || Number(definition.required_house_level) > Number(home.house_level))
        throw new Error('该家具尚未解锁。');
    ...
    const [[usedRows], [same]] = await Promise.all([
        connection.execute('SELECT d.floor_slot_cost FROM player_home_furniture f JOIN home_furniture_definitions d ON d.code=f.furniture_code WHERE f.home_id=? AND f.floor_no=? FOR UPDATE', [home.id, floor]),
        connection.execute('SELECT COUNT(*) AS count FROM player_home_furniture WHERE home_id=? AND floor_no=? AND furniture_code=? FOR UPDATE', [home.id, floor, code])
    ]);
    const used = usedRows.reduce((sum, item) => sum + Number(item.floor_slot_cost), 0);
    if (used + Number(definition.floor_slot_cost) > slotsPerFloor(Number(home.house_level)))
        throw new Error('这一层的家具槽位不足。');
    if (Number(same[0]?.count ?? 0) >= Number(definition.max_per_floor))
        throw new Error(`每层最多放置 ${definition.max_per_floor} 个${definition.name}。`);
    ...
    for (const material of recipe) {
        ...
        if (Number(owned[0]?.quantity ?? 0) < paid)
            throw new Error(`材料不足：${material.name}×${paid}。`);
    }
    for (const material of recipe) {
    ...
    const [created] = await connection.execute('INSERT INTO player_home_furniture (home_id,furniture_code,floor_no,slot_key,grid_x,grid_y,rotation,layout_version) VALUES (?,?,?,?,?,?,?,2)', [home.id, code, floor, slotKey, placement.x, placement.y, placement.rotation]);
...
const tradeHomeOffer = async (qqUserId, offerId, quantity) => withTransaction(async (connection) => {
    ...
    if (!offer)
        throw new Error('该报价已失效。');
    if (offer.input_item_id) {
        const [owned] = await connection.execute('SELECT quantity FROM player_inventory WHERE character_id=? AND item_id=? FOR UPDATE', [character.id, offer.input_item_id]);
        const need = Number(offer.input_quantity) * quantity;
        if (!owned[0] || Number(owned[0].quantity) < need)
            throw new Error('兑换材料不足。');
        await connection.execute('UPDATE player_inventory SET quantity=quantity-? WHERE character_id=? AND item_id=?', [need, character.id, offer.input_item_id]);
        await connection.execute('DELETE FROM player_inventory WHERE character_id=? AND item_id=? AND quantity<=0', [character.id, offer.input_item_id]);
    }
    else {
        const price = Number(offer.copper_price) * quantity;
        const [paid] = await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=? AND copper_coins>=?', [price, character.id, price]);
        if (!Number(paid.affectedRows))
            throw new Error(`铜币不足，需要 ${price} 铜币。`);
    }
    const gained = Number(offer.output_quantity) * quantity;
    await addItem(connection, character.id, Number(offer.output_item_id), gained);
...
const homeStorageView = async (qqUserId, scope, category) => {
    ...
    if (!capacity)
        throw new Error('尚未摆放储物箱，暂时没有可用仓储空间。');
    const pool = await getPool();
    const itemType = category === '装备' ? 'equipment' : category === '道具' ? 'consumable' : 'material';
    const stackedQuery = scope === 'storage'
        ? pool.execute('SELECT i.id,i.code,i.codex_id,i.name,i.item_category,hs.quantity,i.weight,i.description FROM player_home_storage_items hs JOIN item_definitions i ON i.id=hs.item_id WHERE hs.home_id=? AND hs.quantity>0 AND i.item_type=? ORDER BY i.name', [panel.home.id, itemType])
        : pool.execute('SELECT i.id,i.code,i.codex_id,i.name,i.item_category,pi.quantity,i.weight,i.description FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND i.item_type=? AND i.stackable=1 ORDER BY i.name', [panel.character.id, itemType]);
    const instanceQuery = scope === 'storage'
        ? pool.execute('SELECT ii.id,i.codex_id AS definition_codex_id,i.name,i.item_category,ii.quality,ii.durability,ii.durability_max,i.description FROM player_home_storage_instances hs JOIN player_item_instances ii ON ii.id=hs.instance_id JOIN item_definitions i ON i.id=ii.item_id WHERE hs.home_id=? AND i.item_type=? ORDER BY hs.stored_at DESC', [panel.home.id, itemType])
        : pool.execute('SELECT ii.id,i.codex_id AS definition_codex_id,i.name,i.item_category,ii.quality,ii.durability,ii.durability_max,i.description FROM player_item_instances ii JOIN item_definitions i ON i.id=ii.item_id WHERE ii.character_id=? AND i.item_type=? ORDER BY ii.acquired_at DESC', [panel.character.id, itemType]);
...
const depositHomeStorage = async (qqUserId, itemId, quantity) => withTransaction(async (connection) => {
    if (!Number.isInteger(itemId) || itemId < 1 || !Number.isInteger(quantity) || quantity < 1)
        throw new Error('物品编号和数量必须为正整数。');
    const character = await characterFor(connection, qqUserId, true);
    ...
    if (!capacity)
        throw new Error('尚未摆放储物箱，暂时没有可用仓储空间。');
    const [rows] = await connection.execute('SELECT pi.item_id,i.name,pi.quantity,i.weight,COALESCE(JSON_EXTRACT(i.effect_json,\'$.personalOnly\'),0) AS personal_only FROM player_inventory pi JOIN item_definitions i ON i.id=pi.item_id WHERE pi.character_id=? AND pi.item_id=? AND pi.quantity>0 AND i.stackable=1 FOR UPDATE', [character.id, itemId]);
    const item = rows[0];
    if (!item)
        throw new Error('背包中没有可放入的该物品。');
    if (Number(item.personal_only))
        throw new Error('足迹永久道具必须保留在背包中，无法转存。');
    ...
    if (usedWeight + addedWeight > capacity + 0.000001)
        throw new Error(`仓储容量不足，还可放入 ${Math.max(0, capacity - usedWeight).toFixed(2)} kg。`);
...
export { craftFurniture, depositHomeStorage, enterHome, expandHome, homePanel, homeRestExperiencePerMinute, homeRestRecoveryBonus, homeStorageView, isInHome, leaveHome, listFurniture, listHomeShop, purchaseHome, removeFurniture, renameHome, tradeHomeOffer, upgradeHome };
