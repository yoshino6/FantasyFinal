const bossCommonEffects = [
    ...
    { code: 'all_methods_calamity', name: '万法同劫', summary: '所有敌方单体技能改为敌方全体并逐人结算' },
...
const exclusive = (code, name, summary) => ({ code, name, summary });
const bossExclusiveEffects = {
    ...
    gruen_mountainheart: [exclusive('mountainheart_resonance', '山心共振', '每破坏1部件，本体伤害+25%、速度+15%、穿透+20%'), exclusive('leyline_recast', '地脉复铸', '全部件破坏后预警1行动并以40%生命复铸全部部件，每场一次')],
    valk_forge_overseer: [exclusive('everburning_embers', '永燃余烬', '过载保留2炉温，每点炉温提高过载伤害并延长灼烧'), exclusive('soul_chain_forging', '拘魂连锻', '锁链标记3名低血目标，铁砧裁决引发标记间等额传伤')],
    ...
    askr_stormroc: [exclusive('wind_thunder_chorus', '风雷合鸣', '风刃留印，雷枪无视50%魔防、伤害+100%并大幅强化眩晕'), exclusive('sky_rending_wings', '裂空振翼', '每2次实际行动立即追加一次全威力横扫并叠加速度')],
...
const bossRandomEffectTrait = (bossCode, difficultyCode, random = Math.random) => {
    const slots = difficultySlots[difficultyCode];
    if (!slots)
        return undefined;
...
const readBossRandomEffect = (traits) => {
    let values = [];
    ...
    const value = values.find(item => item && typeof item === 'object' && item.code === 'boss_random_effect');
...
const bossRandomEffectDefinitions = (trait) => {
    if (!trait)
        return [];
    const allExclusive = Object.values(bossExclusiveEffects).flat();
    return [...trait.common.map(code => bossCommonEffects.find(item => item.code === code)), ...trait.exclusive.map(code => allExclusive.find(item => item.code === code))].filter((item) => Boolean(item));
};
const hasBossRandomEffect = (traits, code) => {
    const trait = readBossRandomEffect(traits);
    return Boolean(trait && (trait.common.includes(code) || trait.exclusive.includes(code)));
};

export { bossCommonEffects, bossExclusiveEffects, bossRandomEffectDefinitions, bossRandomEffectTrait, hasBossRandomEffect, readBossRandomEffect };
