...
import { newWorldWeaponProfile, newWorldLevels, newWorldEquipmentCodes, newWorldItems } from './new-world.config.js';

const journeyCharacter = async (connection, user, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.id,c.name,c.level,c.profession_code
    FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=?${lock ? ' FOR UPDATE' : ''}`, [user]);
    if (!rows[0])
        throw new Error('请先创建角色，再开启新世界旅途。');
    return rows[0];
};
const advancedProfession = async (connection, id) => {
    const [rows] = await connection.execute('SELECT profession_code FROM player_advanced_professions WHERE character_id=? LIMIT 1', [id]);
    return rows[0]?.profession_code;
};
const newWorldPanel = async (user) => {
    const pool = await getPool();
    const character = await journeyCharacter(pool, user);
    const [claims] = await pool.execute('SELECT reward_level FROM player_new_world_claims WHERE character_id=?', [character.id]);
...
const claimNewWorldOnConnection = async (connection, user, requestedLevel) => {
    if (!newWorldLevels.includes(requestedLevel))
        throw new Error('请选择1、10、20或30级奖励。');
    const level = requestedLevel;
    const character = await journeyCharacter(connection, user, true);
    const id = Number(character.id);
    if (Number(character.level) < level)
        throw new Error(`达到${level}级后才能领取这份奖励。`);
    const [claims] = await connection.execute('SELECT reward_level FROM player_new_world_claims WHERE character_id=? AND reward_level=? FOR UPDATE', [id, level]);
    if (claims.length)
        throw new Error(`你已经领取过${level}级奖励。`);
    const equipment = level === 10 || level === 20;
    const rewards = equipment
        ? newWorldEquipmentCodes(level, character.profession_code, await advancedProfession(connection, id)).map(code => [code, 1])
        : newWorldItems[level];
    ...
    for (const [code, quantity] of rewards) {
        const [items] = await connection.execute('SELECT id,name,item_type,required_level,rarity FROM item_definitions WHERE code=?', [code]);
        const item = items[0];
...
const claimNewWorld = (user, level) => withTransaction(connection => claimNewWorldOnConnection(connection, user, level));

export { claimNewWorld, claimNewWorldOnConnection, newWorldPanel };
