...
const addThreeheadDot = (rules, source, target, code, stacks) => {
    ...
    const effect = rules.add(target, code, code === 'mother_poison' ? 6 : code === 'mother_burn' ? 3 : 100, 3, source, true, existing?.data ?? JSON.stringify({ appliedTurn: rules.turn }));
...
const reductionFactor = (rules, head, solo) => 1 - Math.min(80, (rules.status(head, 'mother_wind_barrier') ? 25 : 0) + (solo ? 35 : 0)) / 100;
const installThreeheadMotherDamage = (rules) => {
    ...
    rules.hooks.afterDamage = async (unit, damage, shieldBroken, originalShield, source) => {
        ...
        if (source?.side !== 'member' || source.hp <= 0 || !threeheadMotherRole(unit) || heads().filter(head => head.hp > 0).length !== 1)
            return;
...
const attack = async (rules, source, targets, power, element, magic, single, onHit) => {
    const hit = async (target) => { const landed = await rules.strike(source, target, power, element, magic, false, false, 1, { skill: true, single }); if (landed) {
        onHit?.(target);
        if (rules.allies(source).filter(unit => threeheadMotherRole(unit)).length === 1)
            for (const code of dotCodes)
                addThreeheadDot(rules, source, target, code, 1);
    } };
    if (single)
        await hit(targets[0]);
    else
...
const executeThreeheadMotherTurn = async (rules, head, defaultVictim) => {
    ...
    if (role === 'venom') {
        if (slot === 1) {
            title('瘟疫吐息');
            await attack(rules, head, enemies, 75, '木', true, false, target => { tryAddThreeheadDot(rules, head, target, 'mother_poison', 1, 35); });
        }
        else if (slot === 2) {
            title('腐毒獠牙');
        ...
        if (slot === 1) {
            title('炎息洪流');
            await attack(rules, head, enemies, 80, '火', true, false, target => { tryAddThreeheadDot(rules, head, target, 'mother_burn', 1, 35); });
        }
        else if (slot === 2) {
            title('烈炎噬咬');
            ...
            title('烈焰风暴');
            await attack(rules, head, enemies, 100, '火', true, false, target => addThreeheadDot(rules, head, target, 'mother_burn', 2));
        }
        else {
            title('红莲撕咬');
        ...
        if (slot === 1) {
            ...
            title('狂岚呼啸');
            await attack(rules, head, enemies, 65, '风', true, false);
            await settleThreeheadDots(rules, enemies, true);
        }
        else if (slot === 3) {
            title('裂空风涡');
            await attack(rules, head, enemies, 70, '风', true, false, target => increaseThreeheadDots(rules, target, 1));
        }
        else if (unlocked) {
            title('卷蚀罡风');
            await attack(rules, head, enemies, 95, '风', true, false, target => addThreeheadDot(rules, head, target, 'mother_wind_erosion', 2));
        }
        else {
            title('裂空风涡');
            await attack(rules, head, enemies, 70, '风', true, false, target => increaseThreeheadDots(rules, target, 1));
