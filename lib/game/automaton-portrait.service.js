...
import { automatonFor, automatonCharacter, recordAutomatonEvent } from './automaton.service.js';
import { recordCharacterOperation } from './character-operation.service.js';

const portraitScope = (event) => {
    if (!event.UserId || (!event.IsPrivate && !event.ChannelId))
        throw new Error('无法识别上传会话。');
...
const assertQueueAvailable = async (c, characterId, id) => {
    const [pending] = await c.execute("SELECT id FROM automaton_portrait_reviews WHERE character_id=? AND automaton_id=? AND status='pending'", [characterId, id]);
    if (pending.length)
        throw new Error('此机巧已有待审核图片，请等待审核或恢复默认形象后重新提交。');
    const [counts] = await c.execute("SELECT COUNT(*) total FROM automaton_portrait_reviews WHERE character_id=? AND created_at>=DATE_SUB(NOW(),INTERVAL 24 HOUR)", [characterId]);
    if (Number(counts[0]?.total) >= 10)
        throw new Error('24小时内最多提交10张形象图片，请稍后再试。');
};
const beginPortraitUpload = (user, scope, id) => withTransaction(async (c) => {
...
const reservePortraitUpload = (user, scope) => withTransaction(async (c) => {
    const [players] = await c.execute('SELECT c.id FROM characters c JOIN players p ON p.id=c.player_id WHERE p.qq_user_id=? FOR UPDATE', [user]);
    if (!players[0])
        return null;
    const characterId = Number(players[0].id);
    const [rows] = await c.execute('SELECT *,expires_at<=NOW() expired FROM automaton_portrait_uploads WHERE character_id=? AND scope_key=? FOR UPDATE', [characterId, scope]);
    const row = rows[0];
    if (!row)
        return null;
    if (row.expired) {
        await c.execute('DELETE FROM automaton_portrait_uploads WHERE character_id=? AND scope_key=?', [characterId, scope]);
        return { expired: true };
    }
    if (row.status === 'processing')
        return { busy: true };
    const id = Number(row.automaton_id);
    await automatonFor(c, characterId, id);
...
const retryPortraitUpload = (upload) => withTransaction(async (c) => {
...
const submitPortraitReview = (user, upload, portrait) => withTransaction(async (c) => {
    const character = await automatonCharacter(c, user);
    const [rows] = await c.execute("SELECT token FROM automaton_portrait_uploads WHERE character_id=? AND scope_key=? AND token=? AND status='processing' AND expires_at>NOW() FOR UPDATE", [character.id, upload.scope, upload.token]);
    if (character.id !== upload.characterId || !rows.length)
        throw new Error('上传已取消、超时或被新上传替代，本次图片未保存。');
    const { state } = await automatonFor(c, character.id, upload.id);
    await assertQueueAvailable(c, character.id, upload.id);
    const [review] = await c.execute('INSERT INTO automaton_portrait_reviews(character_id,automaton_id,token,file_key,width,height) VALUES(?,?,?,?,?,?)', [character.id, upload.id, upload.token, portrait.key, portrait.width, portrait.height]);
...
const resetPortrait = (user, id) => withTransaction(async (c) => {
    const character = await automatonCharacter(c, user), { state } = await automatonFor(c, character.id, id);
    ...
    const [pending] = await c.execute("SELECT file_key FROM automaton_portrait_reviews WHERE character_id=? AND automaton_id=? AND status='pending' FOR UPDATE", [character.id, id]);
    ...
    if (state.portrait || pending.length) {
        ...
        if (state.portrait)
            await recordAutomatonEvent(c, id, character.id, resetId, 'portrait_reset', { name: state.name });
...
export { beginPortraitUpload, cancelPortraitUpload, portraitReviewStatus, portraitScope, reservePortraitUpload, resetPortrait, retryPortraitUpload, submitPortraitReview };
