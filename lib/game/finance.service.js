...
import { financeFaction } from './finance-content.js';
import { financeBusinessDate } from './finance-math.js';
import { reservedInterestCopper, copperText } from './finance-money.js';
import { npcBuyAllocation, npcSellSupport } from './finance-npc.js';

const numeric = (value) => Number(value ?? 0);
const safeInt = (value, label, min = 0, max = Number.MAX_SAFE_INTEGER) => {
...
const depositProducts = product;
const characterFor = async (connection, qqUserId, lock = false) => {
    const [rows] = await connection.execute(`SELECT c.id,c.copper_coins,c.level,c.adventurer_registered,c.created_at,c.activity_status,c.current_region_id,c.pos_x,c.pos_y,c.pos_z,
...
const assertStationary = async (connection, character) => {
    if (character.activity_status !== 'active')
        throw new Error('当前状态无法办理钱庄或证券业务。');
    if (numeric(character.is_spawn_enabled) || numeric(character.is_owner_only) || !numeric(character.is_enabled))
        throw new Error('请前往已开放的安全区。');
    const [travel] = await connection.execute('SELECT 1 FROM player_travels WHERE character_id=? LIMIT 1', [character.id]);
    if (travel.length)
        throw new Error('旅行途中不能办理钱庄或证券业务。');
    const [combat] = await connection.execute('SELECT 1 FROM combat_sessions WHERE character_id=? AND state=\'active\' LIMIT 1', [character.id]);
    if (combat.length)
        throw new Error('战斗中不能办理钱庄或证券业务。');
    const [pvp] = await connection.execute("SELECT 1 FROM player_pvp_battle_sessions WHERE (attacker_character_id=? OR defender_character_id=?) AND state='active' LIMIT 1", [character.id, character.id]);
    if (pvp.length)
        throw new Error('玩家对战中不能办理钱庄或证券业务。');
};
const assertAtBuilding = async (connection, character, code) => {
    await assertStationary(connection, character);
    const [rows] = await connection.execute('SELECT 1 FROM map_npcs WHERE region_id=? AND code=? AND pos_x=? AND pos_y=? AND pos_z=? AND interaction_kind=\'building\' LIMIT 1', [character.current_region_id, code, character.pos_x, character.pos_y, character.pos_z]);
    if (!rows.length)
        throw new Error('请站在对应建筑入口办理。');
};
const bankAccount = async (connection, characterId) => {
    await connection.execute('INSERT IGNORE INTO finance_bank_accounts (character_id) VALUES (?)', [characterId]);
    const [rows] = await connection.execute('SELECT demand_copper FROM finance_bank_accounts WHERE character_id=? FOR UPDATE', [characterId]);
    return rows[0];
};
const poolBalance = async (connection, code) => {
    const [rows] = await connection.execute('SELECT copper FROM finance_pools WHERE code=? FOR UPDATE', [code]);
    if (!rows[0])
        throw new Error(`钱庄资金池 ${code} 未初始化。`);
    return safeInt(rows[0].copper, '资金池余额');
};
const ledger = async (connection, characterId, key, kind, source, target, copper) => {
    if (!copper)
        return;
...
const splitFee = async (connection, characterId, key, fee) => {
    if (!fee)
        return;
    const interest = Math.floor(fee * .4), risk = Math.floor(fee * .4), burned = fee - interest - risk;
    if (interest) {
        await connection.execute("UPDATE finance_pools SET copper=copper+? WHERE code='interest'", [interest]);
        await ledger(connection, characterId, `${key}:interest`, 'trade_fee', `bank:${characterId}`, 'pool:interest', interest);
    }
    if (risk) {
        await connection.execute("UPDATE finance_pools SET copper=copper+? WHERE code='clearing'", [risk]);
        await ledger(connection, characterId, `${key}:risk`, 'trade_fee', `bank:${characterId}`, 'pool:clearing', risk);
    }
    if (burned) {
        await connection.execute("UPDATE finance_pools SET copper=copper+? WHERE code='fees_burned'", [burned]);
        await ledger(connection, characterId, `${key}:burn`, 'trade_fee', `bank:${characterId}`, 'pool:fees_burned', burned);
    }
};
const bankSummary = async (qqUserId) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const [accounts] = await pool.execute('SELECT demand_copper FROM finance_bank_accounts WHERE character_id=?', [character.id]);
    const [openDeposits] = await pool.execute('SELECT id,product_code,principal_copper,reserved_interest_copper,matures_ms,status FROM finance_bank_deposits WHERE character_id=? AND status=\'open\' ORDER BY id DESC', [character.id]);
    const [recentDeposits] = await pool.execute('SELECT id,product_code,principal_copper,reserved_interest_copper,matures_ms,status FROM finance_bank_deposits WHERE character_id=? AND status<>\'open\' ORDER BY id DESC LIMIT 3', [character.id]);
...
const bankTransfer = async (qqUserId, amount, direction) => withTransaction(async (connection) => {
    ...
    if (direction === 'in') {
        const [deduct] = await connection.execute('UPDATE characters SET copper_coins=copper_coins-? WHERE id=? AND copper_coins>=?', [copper, character.id, copper]);
        if (!deduct.affectedRows)
            throw new Error('随身银币不足。');
        ...
        if (safeInt(account.demand_copper, '活期余额') < copper)
...
const openBankDeposit = async (qqUserId, code, amount) => withTransaction(async (connection) => {
    const terms = product[code];
    if (!terms)
        throw new Error('未知存单期限。');
    const copper = safeInt(amount, '本金', terms.min, 99_999_999);
    const character = await characterFor(connection, qqUserId, true);
    await assertAtBuilding(connection, character, 'silver_bell_bank');
    const account = await bankAccount(connection, character.id);
    const [openCount] = await connection.execute('SELECT COUNT(*) n FROM finance_bank_deposits WHERE character_id=? AND status=\'open\'', [character.id]);
    if (numeric(openCount[0]?.n) >= 3)
        throw new Error('每名角色最多同时持有 3 张未结定存；其余资金可留在活期。');
    if (numeric(account.demand_copper) < copper)
        throw new Error('活期余额不足。');
    ...
    const [result] = await connection.execute('INSERT INTO finance_bank_deposits (character_id,product_code,principal_copper,reserved_interest_copper,opened_ms,matures_ms) VALUES (?,?,?,?,?,?)', [character.id, code, copper, interest, now, due]);
    await connection.execute('UPDATE finance_bank_accounts SET demand_copper=demand_copper-? WHERE character_id=?', [copper, character.id]);
    if (interest)
        await connection.execute("UPDATE finance_pools SET copper=copper-? WHERE code='interest'", [interest]);
...
const settleBankDeposit = async (qqUserId, depositId, early = false) => withTransaction(async (connection) => {
    const id = safeInt(depositId, '存单编号', 1);
    const character = await characterFor(connection, qqUserId, true);
    await assertAtBuilding(connection, character, 'silver_bell_bank');
    await bankAccount(connection, character.id);
    const [rows] = await connection.execute('SELECT id,product_code,principal_copper,reserved_interest_copper,matures_ms,status FROM finance_bank_deposits WHERE id=? AND character_id=? FOR UPDATE', [id, character.id]);
    const deposit = rows[0];
    if (!deposit || deposit.status !== 'open')
        throw new Error('未找到可结算的存单。');
    const matured = Date.now() >= numeric(deposit.matures_ms);
    if (!matured && !early)
        throw new Error('这张存单尚未到期。');
    if (!matured && deposit.product_code === 'hundred')
        throw new Error('百日定存不可提前支取。');
    ...
    if (!matured && interest + penalty)
        await connection.execute("UPDATE finance_pools SET copper=copper+? WHERE code='interest'", [interest + penalty]);
    ...
    if (!matured && interest)
        await ledger(connection, character.id, `deposit:${id}:interest_return`, 'interest_return', `term:${id}`, 'pool:interest', interest);
    if (penalty)
        await ledger(connection, character.id, `deposit:${id}:penalty`, 'term_penalty', `term:${id}`, 'pool:interest', penalty);
...
const financeCatalog = async (qqUserId) => {
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    const [account] = await pool.execute('SELECT demand_copper FROM finance_bank_accounts WHERE character_id=?', [character.id]);
    const [rows] = await pool.execute('SELECT code,name,share_name,total_shares,treasury_shares,price_milli,status FROM finance_instruments ORDER BY status,code');
    const [holdings] = await pool.execute('SELECT instrument_code,shares FROM finance_holdings WHERE character_id=?', [character.id]);
    const [npcHoldings] = await pool.execute('SELECT instrument_code,shares FROM finance_npc_portfolio');
    const [liquidity] = await pool.execute("SELECT code,copper FROM finance_pools WHERE code IN ('clearing','npc_capital')");
    const [obligations] = await pool.execute(`SELECT COALESCE(SUM(h.shares * GREATEST(1,ROUND(i.price_milli/1000))),0) copper
...
const financeTrade = async (qqUserId, code, sharesInput, side, expectedPrice) => {
    await (await import('./finance-settlement.js')).settleFinancePeriod();
    return withTransaction(async (connection) => {
        const shares = safeInt(sharesInput, '份额数量', 1, 100);
        const faction = financeFaction(code);
        if (!faction || faction.status !== 'open')
            throw new Error('该份额尚未开放。');
        const character = await characterFor(connection, qqUserId, true);
        await assertAtBuilding(connection, character, 'canopy_exchange');
        if (numeric(character.level) < 10 || !numeric(character.adventurer_registered))
            throw new Error('请先完成冒险者登记并达到 Lv.10。');
        if (Date.now() - new Date(character.created_at).getTime() < 72 * 3600000)
            throw new Error('角色注册满 72 小时后，才能交易势力份额。');
        const account = await bankAccount(connection, character.id);
        const [rows] = await connection.execute('SELECT * FROM finance_instruments WHERE code=? FOR UPDATE', [code]);
        const instrument = rows[0];
        if (!instrument || instrument.status !== 'open')
            throw new Error('该份额当前不能交易。');
        ...
        const [positions] = await connection.execute('SELECT shares FROM finance_holdings WHERE character_id=? AND instrument_code=? FOR UPDATE', [character.id, code]);
        ...
        if (safeInt(expectedPrice, '报价', 1) !== price)
            throw new Error(`报价已变化，当前每份 ${copperText(price)}，请刷新行情后重新确认。`);
        const gross = safeInt(price * shares, '成交额', 1, 99_999_999);
        const fee = Math.ceil(gross * (side === 'buy' ? .01 : .015));
        if (side === 'sell' && gross <= fee)
            throw new Error('成交额不足以支付手续费，请增加份额后再卖出。');
        const clearing = await poolBalance(connection, 'clearing');
        const npcCapital = await poolBalance(connection, 'npc_capital');
        const [npcPositions] = await connection.execute('SELECT shares FROM finance_npc_portfolio WHERE instrument_code=? FOR UPDATE', [code]);
        const npcShares = numeric(npcPositions[0]?.shares);
        let fromNpc = 0, fromTreasury = 0, support = 0;
        if (side === 'buy') {
            if (position + shares > Math.floor(numeric(instrument.total_shares) * .05))
                throw new Error('单势力持仓上限为总份额的 5%。');
            ({ fromNpc, fromTreasury } = npcBuyAllocation(shares, npcShares, numeric(instrument.treasury_shares)));
            if (numeric(account.demand_copper) < gross + fee)
                throw new Error('钱庄活期余额不足。');
        ...
        const [trade] = await connection.execute('INSERT INTO finance_trades (character_id,instrument_code,side,shares,unit_copper,gross_copper,fee_copper) VALUES (?,?,?,?,?,?,?)', [character.id, code, side, shares, price, gross, fee]);
        ...
        await (await import('./finance-settlement.js')).assertFinanceShareSupply(connection, code);
...
const financeMissionList = async (qqUserId, code) => {
    const faction = financeFaction(code);
    if (!faction)
        throw new Error('未知势力。');
    const pool = await getPool();
    const character = await characterFor(pool, qqUserId);
    if (faction.building) {
        const [buildings] = await pool.execute('SELECT 1 FROM map_npcs WHERE code=? AND region_id=? AND pos_x=? AND pos_y=? AND pos_z=? LIMIT 1', [faction.building, character.current_region_id, character.pos_x, character.pos_y, character.pos_z]);
        if (!buildings.length)
            throw new Error(`请前往${faction.name}的分所或对应建筑。`);
    }
    const date = financeBusinessDate();
    const [rows] = await pool.execute('SELECT title,status,completed_count FROM finance_missions WHERE business_date=? AND instrument_code=?', [date, code]);
    const [completions] = await pool.execute('SELECT 1 FROM finance_mission_completions WHERE business_date=? AND instrument_code=? AND character_id=? LIMIT 1', [date, code, character.id]);
    const [acceptances] = await pool.execute('SELECT 1 FROM finance_mission_acceptances WHERE business_date=? AND instrument_code=? AND character_id=? LIMIT 1', [date, code, character.id]);
...
const acceptFinanceMission = async (qqUserId, code) => withTransaction(async (connection) => {
    const faction = financeFaction(code);
    if (!faction?.building || !faction.source || faction.status !== 'open')
        throw new Error('该势力今日没有开放的委托。');
    const character = await characterFor(connection, qqUserId, true);
    ...
    const [result] = await connection.execute('INSERT IGNORE INTO finance_mission_acceptances (business_date,instrument_code,character_id) VALUES (?,?,?)', [date, code, character.id]);
    if (result.affectedRows)
        await recordCharacterOperation(connection, { characterId: numeric(character.id), kind: 'finance.mission_accepted', source: { system: 'finance_mission', id: `${date}:${code}`, step: 'accepted' }, outcome: '接取', summary: `接取${faction.name}委托：${faction.mission}`, detail: { businessDate: date, instrumentCode: code, factionName: faction.name, title: faction.mission } });
    return { faction, acceptedNow: !!result.affectedRows };
});

export { acceptFinanceMission, bankSummary, bankTransfer, depositProducts, financeCatalog, financeMissionList, financeTrade, openBankDeposit, settleBankDeposit };
