...
const recordSkillPointChange = async (connection, characterId, amount, kind, skillId = null, detail = null) => {
    const metric = { learn_skill: 'ACH_A06', upgrade_skill: 'ACH_A07', upgrade_specialization: 'ACH_A08', upgrade_appraisal: 'ACH_A21' };
    if (amount < 0 && metric[kind])
        recordAchievement(connection, characterId, [metric[kind]]);
    if (!amount && !['legacy_opening_balance', 'legacy_level_reset'].includes(kind))
        return;
...
const skillAllocationPlan = async (connection, characterId) => {
    const [characters] = await connection.execute('SELECT level,skill_points FROM characters WHERE id=? FOR UPDATE', [characterId]);
    if (!characters[0])
        throw new Error('角色不存在。');
    const [skills] = await connection.execute('SELECT ps.*,s.name,s.code,s.category,s.learn_cost FROM player_skills ps JOIN skill_definitions s ON s.id=ps.skill_id WHERE ps.character_id=? ORDER BY ps.skill_id FOR UPDATE', [characterId]);
    const [history] = await connection.execute('SELECT * FROM player_skill_point_ledger WHERE character_id=? ORDER BY id FOR UPDATE', [characterId]);
    const [libraryGrants] = await connection.execute('SELECT skill_id FROM player_library_free_skills WHERE character_id=? FOR UPDATE', [characterId]);
    ...
    const mode = history.some(row => row.change_kind !== 'legacy_opening_balance') ? 'ledger' : 'level';
    const level = Math.max(1, Math.trunc(Number(characters[0].level)));
    const available = Number(characters[0].skill_points);
    const [specializations] = await connection.execute('SELECT * FROM player_skill_specializations WHERE character_id=? ORDER BY skill_id,specialization FOR UPDATE', [characterId]);
    const [appraisal] = await connection.execute('SELECT * FROM player_appraisal_progress WHERE character_id=? FOR UPDATE', [characterId]);
    const changes = [];
    const preserved = [];
    const [professionGifts] = await connection.execute(`SELECT s.id FROM characters c JOIN profession_definitions p ON p.code=c.profession_code JOIN skill_definitions s ON JSON_CONTAINS(p.skill_codes_json,JSON_QUOTE(s.code)) WHERE c.id=?`, [characterId]);
    for (const skill of skills) {
        ...
        if (mode === 'level') {
            ...
            const range = skill.code === 'appraisal' ? Math.max(0, Number(appraisal[0]?.range_level ?? 1) - 1) : 0;
            const information = skill.code === 'appraisal' ? Math.max(0, Number(appraisal[0]?.information_level ?? 1) - 1) : 0;
            if (remove || Number(skill.level) > 1 || Object.keys(specialization).length || range || information)
                changes.push({ skillId: Number(skill.skill_id), name: String(skill.name), remove, nextLevel: 1, rows: [], amount: 0, specialization, range, information });
            if (!remove)
                preserved.push(`${skill.name}的基础能力`);
            continue;
        }
        ...
        for (const row of upgrades) {
            if (row.change_kind === 'upgrade_specialization') {
                const type = /(?:overcharge|instant|efficient|potent)$/.exec(String(row.detail))?.[0];
                if (!type)
                    invalid = true;
                else
                    counts[type] = (counts[type] ?? 0) + 1;
            }
            if (row.change_kind === 'upgrade_appraisal') {
                if (String(row.detail).endsWith('慧眼'))
                    range++;
                else if (String(row.detail).endsWith('识珠'))
                    information++;
                else
                    invalid = true;
            }
        ...
        for (const [type, count] of Object.entries(counts))
            if (count > Number(specializations.find(row => Number(row.skill_id) === Number(skill.skill_id) && row.specialization === type)?.level ?? 1) - 1)
                invalid = true;
        if (range > Number(appraisal[0]?.range_level ?? 1) - 1 || information > Number(appraisal[0]?.information_level ?? 1) - 1 || upgrades.length > Number(skill.level) - 1)
            invalid = true;
        if (invalid) {
            preserved.push(String(skill.name));
            continue;
        }
        ...
        if (!remove && Number(skill.level) - upgrades.length > 1)
            preserved.push(`${skill.name}的旧进度`);
    ...
    const [automatic] = await connection.execute('SELECT * FROM player_auto_battle_actions WHERE character_id=? ORDER BY sequence_no', [characterId]);
    const [pvpAutomatic] = await connection.execute('SELECT * FROM player_pvp_auto_battle_actions WHERE character_id=? ORDER BY sequence_no', [characterId]);
...
const resetSkillPointAllocation = async (connection, characterId, token = randomUUID()) => {
    const plan = await skillAllocationPlan(connection, characterId);
    if (!plan.canReset)
        return { restoredPoints: 0, availablePoints: plan.targetPoints, removedSkills: 0, mode: plan.mode };
    for (const change of plan.changes) {
        if (change.remove) {
            ...
            for (const [type, count] of Object.entries(change.specialization))
                await connection.execute('UPDATE player_skill_specializations SET level=level-? WHERE character_id=? AND skill_id=? AND specialization=?', [count, characterId, change.skillId, type]);
        }
        if (change.range || change.information)
            await connection.execute('UPDATE player_appraisal_progress SET range_level=range_level-?,information_level=information_level-? WHERE character_id=?', [change.range, change.information, characterId]);
        for (const table of ['player_auto_battle_actions', 'player_pvp_auto_battle_actions'])
            await connection.execute(`UPDATE ${table} SET skill_id=NULL WHERE character_id=? AND skill_id=?`, [characterId, change.skillId]);
        if (change.rows.length)
            await connection.execute(`UPDATE player_skill_point_ledger SET refund_token=? WHERE character_id=? AND id IN (${change.rows.map(() => '?').join(',')}) AND refund_token IS NULL`, [token, characterId, ...change.rows]);
    }
    if (plan.mode === 'level') {
    ...
    if (plan.mode === 'level')
        await recordSkillPointChange(connection, characterId, plan.targetPoints - plan.available, 'legacy_level_reset', null, `无明细按 Lv.${plan.level} 重置 ${token}`);
    else if (plan.targetPoints < plan.available)
        await recordSkillPointChange(connection, characterId, plan.targetPoints - plan.available, 'allocation_reconcile', null, `洗练余额限制为 Lv.${plan.level} ${token}`);
    else if (plan.refund) {
...
export { ensureSkillPointLedger, recordSkillPointChange, resetSkillPointAllocation, skillAllocationPlan, skillPointLedgerSummary };
