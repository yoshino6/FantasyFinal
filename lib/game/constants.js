...
const forgedEquipmentBase = (level, category, slot) => {
    ...
    if (category === '武器')
        return body * .5;
    if (['upper', 'lower', '上装', '下装'].includes(slot ?? ''))
        return body * .24;
    if (['shoulder', 'waist', 'feet', '头肩', '腰部', '脚部'].includes(slot ?? ''))
        return body * 13 / 75;
    return body / 5;
};
const forgedPrimaryStats = (category, subtype, level, rarity, slot = category) => {
    ...
    if (!weapon)
        return { physicalDefense: value, magicDefense: value };
    if (subtype === '盾牌')
        return { physicalDefense: value, magicDefense: value * .5 };
    if (subtype === '匕首')
        return { physicalAttack: value * .9, magicAttack: value * .9 };
