...
import { serveBasicOpeningMeal } from './opening-keepsakes.service.js';
...
const aquaPermitLevel = (battles) => Math.min(3, 1 + Math.floor(Math.max(0, battles) / 10));
const aquaSupport = async (c, rules, owner, actionKey) => {
    if (owner.hp <= 0 || owner.companion || owner.opening?.pve === false || owner.state.memory.opening_aqua_action === actionKey)
        return;
    const [rows] = await c.execute(`SELECT r.flags_json FROM opening_world w JOIN player_opening_relations r ON r.character_id=w.aqua_character_id AND r.npc_code='aqua' WHERE w.id=1 AND w.aqua_character_id=? AND w.aqua_location='follow' AND w.aqua_stage>=3`, [Number(owner.key.split(':')[1])]);
    if (!rows[0])
        return;
    owner.state.memory.opening_aqua_action = actionKey;
    const count = Number(owner.state.memory.opening_aqua_actions ?? 0) + 1;
    owner.state.memory.opening_aqua_actions = count;
    if (count % 3)
        return;
    ...
    if (owner.hp < owner.hpMax * .6 && rules.once(owner, 'opening_aqua_heal', true)) {
        const heal = Math.min(owner.hpMax - owner.hp, Math.floor(owner.hpMax * (.04 + level * .01)));
        owner.hp += heal;
        rules.log.push(`　&阿库娅·在地恩惠&“这时候就该依靠本女神啦！”恢复 ${heal} HP。`);
        return;
    }
    const removable = (e) => !e.mechanism && ['poison', 'burn', 'bleed', 'bleeding', 'slow', 'blind'].includes(e.code);
    if (rules.effects(owner).some(removable) && rules.once(owner, 'opening_aqua_cleanse', true)) {
        await rules.dispel(owner, owner, true, 1, removable);
        rules.log.push('　&阿库娅·净水涤尘&“小小的污秽，还想挡住女神？”一类普通减益被净化。');
    }
};
const advanceAquaPermit = async (c, id) => {
...
const aquaView = async (user) => {
    ...
    const [records] = await c.execute("SELECT flags_json FROM player_opening_relations WHERE character_id=? AND npc_code='aqua'", [character.id]);
    return ownerView(world, parse(records[0]?.flags_json));
};
const aquaAction = async (user, revision, action) => withTransaction(async (c) => {
    if (!Number.isSafeInteger(revision) || revision < 0)
        throw new Error('请重新打开女神办事桌。');
    const character = await openingCharacter(c, user, true), id = Number(character.id);
    const replay = await openingReplay(c, id, 'aqua', revision);
    if (replay?.action === action)
        return replay.result;
    const context = await requireGuildService(c, id);
    const world = await openingWorldFor(c, true);
    if (Number(world.aqua_character_id) !== id)
        throw new Error('这份同行约定属于另一位旅人。你仍可在办事桌交信、查阅自己的回执。');
    await c.execute("INSERT IGNORE INTO player_opening_relations (character_id,npc_code,flags_json) VALUES (?,'aqua','{}')", [id]);
    const [records] = await c.execute("SELECT flags_json FROM player_opening_relations WHERE character_id=? AND npc_code='aqua' FOR UPDATE", [id]);
    const flags = parse(records[0].flags_json);
    const current = ownerView(world, flags);
    if (Number(world.revision) !== revision || replay)
        return current;
    if (!current.choices.some(choice => choice.code === action))
        throw new Error('请从当前章节选择。');
    if (context.code !== 'world_tree')
        throw new Error('阿库娅的落地登记与同行安排在世界树公会办理。');
    let result = '';
    if (action === 'purify') {
        flags.identity = '净水特殊协作员';
        world.aqua_stage = 1;
        ...
        flags.firstHome = action;
        world.aqua_stage = 2;
        if (action === 'meal')
            result = '餐盘刚落桌，阿库娅便把登记纸挪到不会沾汤的位置：“神明也得体察当地饮食！”\n\n' + await serveBasicOpeningMeal(c, id);
        else if (action === 'room') {
            ...
            result = '你把同行凭据交给绫页归档。阿库娅要求把签名放大一点，绫页认真另附了一张签名纸。她捧着回执，终于肯承认这里办事还算周到。';
    }
        ...
        flags.jobSettled = true;
        world.aqua_stage = 3;
    }
    else if (action === 'follow') {
        await c.execute('UPDATE player_companions SET is_out=0 WHERE character_id=?', [id]);
        world.aqua_location = 'follow';
        result = '你们约好继续调查。阿库娅收好文件来到身旁，普通随从回到休整处，出行支援位由她占用。';
    }
    else {
        world.aqua_location = 'world_tree';
        result = '阿库娅同意暂驻世界树：“先说好，是我主动留下来帮忙的！”绫页将她的行程牌翻回在岗一面。同行关系保留，你可以安排普通随从出行。';
    }
...
export { advanceAquaPermit, aquaAction, aquaPermitLevel, aquaSupport, aquaView };
