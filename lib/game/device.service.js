...
import { activeDeviceCodes } from './deconstructor-catalog.js';
...
const ensureDeviceLoadoutMutable = async (connection, characterId) => {
    const [pveRows] = await connection.execute(`SELECT 1 FROM combat_members cm JOIN combat_sessions cs ON cs.id=cm.session_id
    WHERE cm.character_id=? AND cs.state='active' LIMIT 1 FOR UPDATE`, [characterId]);
    if (pveRows[0])
        throw new Error('战斗已经开始，异械生效与快捷配置将在下场战斗生效。');
    const [pvpRows] = await connection.execute(`SELECT 1 FROM player_pvp_battle_sessions
...
export { activateDevice, activeDeviceDefinitionByCode, activeDeviceDefinitions, activeDeviceList, activeDeviceSkillByCode, clearDeviceQuickSlot, combatDeviceSlotsFor, deactivateDevice, deviceQuickConfig, initializeCombatDeviceEnergy, restoreCombatDeviceEnergy, setDeviceQuickSlot };
