...
import { constructionRecipeByCode, activeDeviceCodes } from './deconstructor-catalog.js';
...
import { aoeDescription, aoeSkillPower } from './aoe-damage.config.js';

const activeDeviceDefinitions = [
...
for (const definition of activeDeviceDefinitions)
    for (const skill of definition.skills) {
        skill.description = aoeDescription(`device_${skill.code}`, skill.description);
        if (skill.power !== undefined)
            skill.power = aoeSkillPower(`device_${skill.code}`, skill.power);
    }
const activeDeviceDefinitionByCode = new Map(activeDeviceDefinitions.map(definition => [definition.code, definition]));
const activeDeviceSkillByCode = new Map(activeDeviceDefinitions.flatMap(definition => definition.skills.map(skill => [skill.code, { ...skill, deviceCode: definition.code }])));
const passiveDeviceActualEffects = {
...
const activeDeviceList = async (qqUserId) => {
...
const deviceDetail = async (qqUserId, instanceId) => {
    const device = (await activeDeviceList(qqUserId)).find(item => item.id === instanceId);
    if (!device)
        throw new Error('未找到该异械。');
    return device;
};
const deviceSkillDetail = async (qqUserId, instanceId, skillCode) => {
    ...
    if (!skill)
        throw new Error('该异械不具备此主动技。');
    return { device, skill, maxEnergy: device.activeDefinition.maxEnergy };
};
const activateDevice = async (qqUserId, instanceId) => withTransaction(async (connection) => {
  ...
  `, [instanceId, characterId]);
    ...
    const [activated] = await connection.execute('INSERT IGNORE INTO player_active_devices (character_id,instance_id) VALUES (?,?)', [characterId, instanceId]);
    if (Number(activated.affectedRows) > 0)
        await recordCharacterOperation(connection, { characterId, kind: 'device.activated', source: { system: 'device_activation', id: randomUUID(), step: 'activated' }, outcome: '启用', summary: `启用异械${device.name}`, detail: { instanceId, deviceName: device.name } });
    return device.name;
});
const deactivateDevice = async (qqUserId, instanceId) => withTransaction(async (connection) => {
    const characterId = await characterIdFor(connection, qqUserId, true);
    await assertCombatLoadoutMutable(connection, characterId);
    const [rows] = await connection.execute(`
...
const deviceQuickConfig = async (qqUserId) => {
...
const setDeviceQuickSlot = async (qqUserId, slot, instanceId) => withTransaction(async (connection) => {
    if (!Number.isInteger(slot) || slot < 1 || slot > 4)
        throw new Error('异械栏位仅限 ① 至 ④。');
    const characterId = await characterIdFor(connection, qqUserId, true);
    await assertCombatLoadoutMutable(connection, characterId);
    const [rows] = await connection.execute(`SELECT i.name,i.code FROM player_active_devices ad
    ...
    const device = rows[0];
    if (!device || !activeDeviceDefinitionByCode.has(device.code))
        throw new Error('只能配置已生效的主动异械。');
    const [sameTypeRows] = await connection.execute(`SELECT 1 FROM player_device_quick_slots qs JOIN player_item_instances ii ON ii.id=qs.instance_id
    ...
    if (sameTypeRows[0])
        throw new Error('同型号主动异械只提供一组技能，请先取消其原有快捷配置。');
...
const clearDeviceQuickSlot = async (qqUserId, slot) => withTransaction(async (connection) => {
    if (!Number.isInteger(slot) || slot < 1 || slot > 4)
        throw new Error('异械栏位仅限 ① 至 ④。');
    const characterId = await characterIdFor(connection, qqUserId, true);
    await assertCombatLoadoutMutable(connection, characterId);
    const [deleted] = await connection.execute('DELETE FROM player_device_quick_slots WHERE character_id=? AND quick_slot=?', [characterId, slot]);
    if (Number(deleted.affectedRows) > 0)
        await recordCharacterOperation(connection, { characterId, kind: 'device.quick_slot_changed', source: { system: 'device_quick_slot', id: randomUUID(), step: 'clear' }, outcome: '清除', summary: `清除异械栏 ${slot}`, detail: { slot } });
});
const initializeCombatDeviceEnergy = async (connection, sessionId, characterId, battleKind = 'pve') => {
    const [rows] = await connection.execute(`SELECT ad.instance_id,i.code,
    ...
    for (const row of rows) {
        const definition = activeDeviceDefinitionByCode.get(row.code);
        if (!definition)
            continue;
...
const combatDeviceSlotsFor = async (connection, sessionId, characterId, lock = false, battleKind = 'pve') => {
    const [rows] = await connection.execute(`SELECT qs.quick_slot,qs.instance_id,i.code,i.name,energy.current_energy,energy.max_energy
...
const restoreCombatDeviceEnergy = async (connection, sessionId, characterId, amount, exceptInstanceId, battleKind = 'pve') => {
    const params = [amount, battleKind, sessionId, characterId];
    ...
    if (exceptInstanceId != null) {
        query += ' AND instance_id<>?';
        params.push(exceptInstanceId);
    }
    const [result] = await connection.execute(query, params);
    return Number(result.affectedRows ?? 0);
};

export { activateDevice, activeDeviceDefinitionByCode, activeDeviceDefinitions, activeDeviceList, activeDeviceSkillByCode, clearDeviceQuickSlot, combatDeviceSlotsFor, deactivateDevice, deviceDetail, deviceQuickConfig, deviceSkillDetail, initializeCombatDeviceEnergy, restoreCombatDeviceEnergy, setDeviceQuickSlot };
