...
import { ensureHeartGrowth, applyHeartGrowthToRow, heartGrowthAdjustment } from './heart-question.service.js';
...
import { registeredAdvancedProfessionByCode, cachedAdvancedPassiveEffectFor, advancedElementMasteryBonusFor } from './advanced-profession.config.js';
...
import { flushAchievements, achievementStatBonus, unlockAccountAchievement } from './achievement.service.js';
import { recordAchievement, takeAchievementEvents } from './achievement-events.js';
import { equippedEnchantmentEffects, equippedEnchantments, cardIndependentPanelPercent } from './equipment-enchantment-effects.js';
...
const virtualNpcTier = (npcCode) => {
    if (!npcCode)
        return null;
    if (/^npc_forest_(warrior|mage|priest)(?:_[0-9]+)?$/.test(npcCode))
        return 'elite';
    return 'large';
};
...
const equipmentExtraAttributes = async (connection, characterId) => {
    ...
    const result = Object.fromEntries(keys.map(key => [key, Math.round(effects.reduce((total, row) => total + Number(jsonRecord(row.effect_json)[key] ?? 0) * (key === 'damageBonusPct' || key === 'damageReductionPct' ? 1 : equipmentQualityMultiplier(Number(row.quality))), 0) * 10) / 10]));
    const enchantment = await equippedEnchantmentEffects(connection, characterId);
    for (const key of keys)
        result[key] = Number(result[key] ?? 0) + Number(enchantment[key] ?? 0);
    return result;
};
const effectiveCharacterAttributes = async (connection, character, characterId) => {
    ...
    const base = finalAttributes(await applyHeartGrowthToRow(connection, characterId, character));
    const achievementBonus = await achievementStatBonus(connection, characterId);
    for (const key of attributes)
        base[key] += achievementBonus[key] ?? 0;
    const enchantment = await equippedEnchantmentEffects(connection, characterId);
    const bonus = (key) => equipmentRows.reduce((total, row) => total + Number(jsonRecord(row.effect_json)[key] ?? 0) * equipmentQualityMultiplier(Number(row.quality)) * (row.slot === 'offhand' ? mastery.offhandAttributeMultiplier : 1), 0) + Number(enchantment[key] ?? 0);
    const percent = (key) => equipmentRows.reduce((total, row) => total + Number(jsonRecord(row.effect_json)[`${key}Pct`] ?? 0) * equipmentQualityMultiplier(Number(row.quality)) * (row.slot === 'offhand' ? mastery.offhandAttributeMultiplier : 1), 0) + Number(enchantment[`${key}Pct`] ?? 0);
...
const withEquipmentElements = async (connection, characterId, baseMastery, baseResistance, offhandMultiplier = .5) => {
    ...
    WHERE pe.character_id=?`, [characterId]);
    const enchantment = await equippedEnchantmentEffects(connection, characterId);
    const bonus = (prefix, element) => rows.reduce((total, row) => total + Number(jsonRecord(row.effect_json)[`${prefix}_${element}`] ?? 0) * equipmentQualityMultiplier(Number(row.quality)) * (row.slot === 'offhand' ? offhandMultiplier : 1), 0) + Number(enchantment[`${prefix}_${element}`] ?? 0);
...
const recalculateCharacterStats = async (connection, characterId) => {
    ...
    WHERE pe.character_id=?`, [characterId]);
    const enchantment = await equippedEnchantmentEffects(connection, characterId);
    const equipmentAttributeBonus = (key) => equipmentAttributeRows.reduce((total, row) => total + Number(jsonRecord(row.effect_json)[key] ?? 0) * equipmentQualityMultiplier(Number(row.quality)) * (row.slot === 'offhand' ? masteryBonuses.offhandAttributeMultiplier : 1), 0) + Number(enchantment[key] ?? 0);
    const equipmentAttributePercent = (key) => equipmentAttributeRows.reduce((total, row) => total + Number(jsonRecord(row.effect_json)[`${key}Pct`] ?? 0) * equipmentQualityMultiplier(Number(row.quality)) * (row.slot === 'offhand' ? masteryBonuses.offhandAttributeMultiplier : 1), 0) + Number(enchantment[`${key}Pct`] ?? 0);
    ...
    for (const [element, value] of Object.entries(advancedMastery))
        elemental.mastery[element] = Number(elemental.mastery[element] ?? 0) + value;
...
const getPlayer = async (connection, qqUserId, nickname) => {
    ...
    const [rows] = await connection.execute('SELECT id, status, qq_nickname FROM players WHERE qq_user_id = ? FOR UPDATE', [qqUserId]);
    return rows[0];
};
...
const completeHeavenRebirth = async (qqUserId) => withTransaction(async (connection) => {
    const player = await getPlayer(connection, qqUserId);
    const [finished] = await connection.execute('SELECT payload FROM player_events WHERE player_id=? AND event_type=\'registration.heaven_rebirth\' ORDER BY id DESC LIMIT 1', [player.id]);
    if (finished[0])
        return { replayed: true, achievementName: '宁静的彼岸' };
    if (await completedRegistration(connection, player.id))
        throw new Error('你已经完成异世界转生。');
    const session = await getSession(connection, player.id, true);
    if (!session || session.expires_at <= new Date())
        throw new Error('注册会话已过期，请重新发送“注册”。');
    if (session.stage !== 'heaven')
        throw new Error('请先在命运的岔路选择前往天堂。');
...
const chooseGift = async (qqUserId, giftCode, nickname) => withTransaction(async (connection) => {
    ...
    await connection.execute('DELETE FROM registration_sessions WHERE id = ?', [session.id]);
    const [createdEvent] = await connection.execute('INSERT INTO player_events (player_id, event_type, payload) VALUES (?, \'character.created\', ?)', [player.id, JSON.stringify({ characterId, region: region.name, x, y, z, giftCode, giftName: gift.name })]);
...
const getCharacter = async (qqUserId) => {
    ...
    return {
        ...
        extraAttributes,
        ...activeEffects,
        growth: Object.fromEntries(attributes.map(key => [key, Number(row[`${key}Growth`]) + Number(heartProfile?.delta[key] ?? 0)]))
    };
};
...
const changeCharacterName = async (qqUserId, input) => withTransaction(async (connection) => {
    ...
    const [before] = await connection.execute('SELECT name FROM characters WHERE id=? FOR UPDATE', [characterId]);
    if (before[0]?.name === name)
        return { name, usedCard: false };
...
const changeCharacterGender = async (qqUserId, gender) => withTransaction(async (connection) => {
    ...
    const characterId = await characterIdForChange(connection, qqUserId);
    const [before] = await connection.execute('SELECT gender FROM characters WHERE id=? FOR UPDATE', [characterId]);
    if (before[0]?.gender === gender)
        return { gender, usedCard: false };
...
const registerAdventurer = async (qqUserId) => withTransaction(async (connection) => {
    ...
    if (rows[0].adventurer_registered) {
        await (await import('./guild-map.service.js')).ensureRegistrationMapExchange(connection, Number(rows[0].id));
        await (await import('./progression-map.service.js')).ensureProgressionMaps(connection, Number(rows[0].id));
        return false;
    }
...
export { adventurerProfile, armorClassDefenseMultiplier, askWhereAmI, beginRegistration, changeCharacterGender, changeCharacterName, chooseDestination, chooseGift, chooseProfession, completeHeavenRebirth, continueRegistration, effectiveCharacterAttributes, equipmentExtraAttributes, getCharacter, hasCharacter, recalculateCharacterStats, refreshCharacterStamina, registerAdventurer, withVirtualNpcEquipment };
