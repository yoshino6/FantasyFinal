...
import { adminDashboard, adminPlayers, adminPlayerDetail, runWebPlayerAudit, adminWorldOverview, changeGlobalMultiplierFromWeb, adminWorldEvents, adminPatrolEntities, adminGameOperations, adminMails, sendWebMail, adminWebJournals } from '../game/admin-web-data.service.js';
...
const registerAdminWebRoutes = (router) => {
    ...
    router.post('/api/admin/session', async (ctx) => {
        try {
            ...
            const config = configFor();
            ctx.cookies.set(cookieName, login.token, { httpOnly: true, sameSite: 'strict', secure: Boolean(config.publicBaseUrl), maxAge: config.sessionAbsoluteMinutes * 60_000, overwrite: true });
    ...
    router.delete('/api/admin/session', async (ctx) => {
        ...
        ctx.cookies.set(cookieName, '', { httpOnly: true, sameSite: 'strict', secure: Boolean(configFor().publicBaseUrl), maxAge: 0, overwrite: true });
