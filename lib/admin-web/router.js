...
import fileUrl from '../assets/game/story/pear-admin-cover.png.js';
import { adminCoverPath, adminPage, loginPage } from './page.js';
import { sessionForAdminWeb, loginAdminWeb, logoutAdminWeb, webAdminAccounts, createWebAdminAccount, setWebAdminEnabled } from '../game/admin-web.service.js';
import { adminDashboard, adminPlayers, adminSearchSuggestions, adminPlayerDetail, runWebPlayerAudit, adminWorldOverview, changeGlobalMultiplierFromWeb, adminWorldEvents, adminPatrolEntities, adminGameOperations, adminMails, sendWebMail, adminWebJournals } from '../game/admin-web-data.service.js';
...
const clientIp = (ctx) => {
    ...
    if (config.trustedProxyIps.includes(remote))
        return String(ctx.get('x-forwarded-for')).split(',')[0].trim().slice(0, 64) || remote;
...
const apiError = (ctx, status, message) => { ctx.status = status; ctx.type = 'application/json'; ctx.body = { message }; };
...
const registerAdminWebRoutes = (router) => {
    ...
    router.post('/api/admin/session', async (ctx) => {
        try {
            if (!secureAdminRequest(ctx)) {
                apiError(ctx, 404, '未找到接口。');
                return;
            ...
            const login = await loginAdminWeb({ username: body.username, password: body.password, ip: clientIp(ctx) });
            const config = configFor();
            ctx.cookies.set(cookieName, login.token, { httpOnly: true, sameSite: 'strict', secure: secureCookie(config), maxAge: config.sessionAbsoluteMinutes * 60_000, overwrite: true });
            ...
            ctx.type = 'application/json';
            ctx.body = { username: login.session.username, role: login.session.role };
        }
        catch (error) {
            apiError(ctx, 400, error instanceof Error ? error.message : '登录失败。');
        }
    });
    router.delete('/api/admin/session', async (ctx) => {
        const session = await auth(ctx, true);
        if (!session)
            return;
        await logoutAdminWeb(sessionToken(ctx));
        ctx.cookies.set(cookieName, '', { httpOnly: true, sameSite: 'strict', secure: secureCookie(configFor()), maxAge: 0, overwrite: true });
    ...
    router.get('/api/admin/portraits/:id/image', async (ctx) => {
        if (!await auth(ctx))
            return;
        ctx.set('Cache-Control', 'no-store');
        ctx.set('X-Content-Type-Options', 'nosniff');
