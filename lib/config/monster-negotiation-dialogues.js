import { creatureVoices } from './monster-negotiation.js';
import { moodBand } from '../game/negotiation-rules.js';
...
const negotiationDialogue = (family, ppm, kind, subtype = '', itemName = '', lastKey = '', random = Math.random) => {
    const band = moodBand(ppm).code;
    const prefix = `${family}:${band}:${kind}:${subtype}`;
    ...
    if (kind === 'like' || kind === 'neutral' || kind === 'dislike') {
        ...
        text = voice.protection[variant];
    else if (kind === 'approach')
        text = voice.approach[variant] + moods[band][variant];
    else if (kind === 'blocked')
        text = voice.approach[variant] + (variant ? '熟悉的旧面孔让局面骤然绷紧；它仍记得上次谈判如何结束，不肯再听解释。' : '它认出了你们中的旧人，再多礼物也不能让这次谈判重新开始。');
    else if (kind === 'talk_failed')
        text = voice.approach[variant] + (variant ? '你试着继续解释，却没有得到认可；它的耐心正在消退。' : '你的提议没有打动它，短暂的沉默让气氛紧了几分。') + moods[band][variant];
    else if (kind === 'refused')
        text = voice.approach[variant] + (variant ? '它已经心满意足，没有再接下礼物。你可以直接与它交谈。' : '它没有继续索取，愿意听你们说定最后的条件。礼物仍留在背包中。');
    else if (kind === 'success')
        text = voice.approach[variant] + (variant ? '这次回应已经足够清楚：它接受了提议，同意和平结束这场遭遇。' : '你们终于读懂了彼此的意思。它不再阻拦，双方达成了协议。');
    else
        text = voice.approach[variant] + (variant ? '你们暂且收住话头。它记住了这次往来，态度不会因离开而重置。' : '谈判暂告一段落，已经发生的好意与冒犯仍会留在记忆中。');
    return { key: `${prefix}:${variant}`, text };
};

export { negotiationDialogue };
