import { purifiedMaterialForArmor, purifiedCraftMaterialCode } from '../game/monster-crafting-material.service.js';

const epicBlueprintDropChance = {
    '武器': .02, '上装': .04, '下装': .04, '头肩': .06, '腰部': .06, '脚部': .06
};
const slots = [
    ['头肩', 6, 2, 1], ['上装', 12, 4, 2], ['腰部', 5, 2, 1], ['下装', 10, 4, 2], ['脚部', 7, 2, 1]
];
const rareForgeMaterials = [
    { code: 'meteor_iron', name: '陨铁锻锭', description: '自天外坠落后反复锻净的沉重铁锭，是史诗装备的基础骨架。', hourlyAttempts: 5, attemptChance: 1, perRegionActiveCap: 30, minRegionLevel: 20, miningSeconds: 15 * 60, yields: [1, 2, 3], weights: [.4, .4, .2] },
    { code: 'star_copper', name: '星铜锻锭', description: '带有微弱星辉的高导性锻锭，用于固定史诗装备的精炼结构。', hourlyAttempts: 1, attemptChance: 1, perRegionActiveCap: 10, minRegionLevel: 20, miningSeconds: 30 * 60, yields: [1, 2, 3], weights: [.6, .3, .1] },
    { code: 'moon_silver', name: '月银锻锭', description: '在月光下仍保持柔韧的银白锻锭，是史诗装备的稀有结合材。', hourlyAttempts: 1, attemptChance: .5, perRegionActiveCap: 3, minRegionLevel: 20, miningSeconds: 60 * 60, yields: [1, 2, 3], weights: [.8, .15, .05] },
    { code: 'sun_gold', name: '曜金合锭', description: '以极高温度熔合的金色核心锭，能令史诗装备承受完整的力量回路。', hourlyAttempts: 1, attemptChance: .2, perRegionActiveCap: 1, minRegionLevel: 20, miningSeconds: 120 * 60, yields: [1, 2, 3], weights: [.95, .04, .01] }
];
const regionalForgeMaterials = [
...
const commonArmorMaterials = (profile, regionalAmount, craftAmount, partAmount) => [
...
const weaponMaterialFor = (weaponType) => weaponType === '匕首' ? purifiedCraftMaterialCode('gel_skin', 30) : weaponType === '法杖' || weaponType === '法书' || weaponType === '法球' ? purifiedCraftMaterialCode('hair', 30) : weaponType === '盾牌' ? purifiedCraftMaterialCode('shell', 30) : purifiedCraftMaterialCode('bone', 30);
const epicForgeRecipes = setProfiles.flatMap(profile => [
    ...
    ...profile.weaponEntries.map(([name, subtype, shortCode, weaponEffect]) => ({
        ...
        category: '武器', subtype, weaponEffect, materials: [
...
const epicSetProfile = (code) => setProfiles.find(profile => profile.code === code) ?? null;
const epicRecipeByBlueprint = (code) => epicForgeRecipes.find(recipe => recipe.blueprintCode === code) ?? null;
const epicRecipesByBoss = (bossCode) => epicForgeRecipes.filter(recipe => recipe.bossCode === bossCode);

export { epicBlueprintDropChance, epicForgeRecipes, epicRecipeByBlueprint, epicRecipesByBoss, epicSetProfile, rareForgeMaterials, regionalForgeMaterials };
