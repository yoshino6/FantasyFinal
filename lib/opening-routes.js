const registerOpeningRoutes = (group) => {
    group.use('选择道路', () => import('./response/opening-road.js').then(m => ({ default: m.openingRoadPanelHandler })));
    group.use({ path: '道路选定', schema: { usage: '/道路选定 <版本> <路线>', args: [{ name: 'revision', rules: [{ required: true, type: 'number', min: 0 }] }, { name: 'code', rules: [{ required: true }] }] } }, () => import('./response/opening-road.js').then(m => ({ default: m.openingRoadChoiceHandler })));
