...
import { itemCodex as itemCodex$1 } from '../game/adventure.service.js';
...
import { discoverSecondaryFinished } from '../game/secondary-shop.service.js';
...
const equipmentSections = (effectJson, category, subtype) => {
    ...
    const attributeLabel = (key) => attributeLabels[key] ?? (key.startsWith('elementMastery_') ? `${key.slice('elementMastery_'.length)}元素精通` : key.startsWith('elementResistance_') ? `${key.slice('elementResistance_'.length)}元素抗性` : '');
    ...
    const effectLabels = { ignoreDefensePct: '无视目标物理防御', lifestealPct: '造成伤害后恢复生命', magicDamagePct: '魔法伤害提高', manaCostReduction: '技能魔力消耗降低', damageBonusPct: '造成伤害提高', damageReductionPct: '受到伤害降低', minimumHitRatePct: '攻击命中率最低' };
...
const consumableEffects = (effectJson) => {
    const effect = jsonObject(effectJson);
    const lines = [];
    if (Number(effect.heal ?? 0))
        lines.push(`立即恢复 ${Number(effect.heal)} 点生命。`);
    if (Number(effect.restoreMp ?? 0))
        lines.push(`立即恢复 ${Number(effect.restoreMp)} 点魔力。`);
    if (Number(effect.healPct ?? 0))
        lines.push(`立即恢复最大生命的 ${Number(effect.healPct)}%。`);
    if (Number(effect.restoreMpPct ?? 0))
        lines.push(`立即恢复最大魔力的 ${Number(effect.restoreMpPct)}%。`);
    if (effect.cleanse)
        lines.push('清除自身可净化的异常状态。');
    const status = jsonObject(effect.status);
    const statusNames = { regeneration: '再生', mana_regeneration: '回流', barrier: '减伤', battle_cry: '战吼', precision: '精准', critical_focus: '凝神', sprint: '迅行', alchemy_guard: '坚守', alchemy_evasion: '轻灵', burn: '灼烧', bind: '束缚', stun: '眩晕', exposed: '易伤', imbalance: '失衡', alchemy_confusion: '混乱' };
    if (String(status.code))
        lines.push(`施加${effect.target === 'enemy' ? '目标' : '自身'}「${statusNames[String(status.code)] ?? String(status.code)}」${Number(status.turns) ? `，持续 ${Number(status.turns)} 回合` : ''}${Number(status.chance) ? `（${Number(status.chance)}% 基础概率）` : ''}。`);
    const throwable = jsonObject(effect.throwable);
    if (Number(throwable.damageScale ?? 0))
        lines.push(`对当前目标造成${String(throwable.element ?? '无')}属性直击伤害${effect.trueHit ? '（必中）' : ''}${effect.noCrit ? '（不可暴击）' : ''}。`);
    if (Number(effect.experienceBonusPct ?? 0))
        lines.push(`战斗经验获取提高 ${Number(effect.experienceBonusPct)}%。`);
    if (Number(effect.partyDropBonusPct ?? 0))
        lines.push(`所在队伍打怪掉率提高 ${Number(effect.partyDropBonusPct)}%。`);
    if (Number(effect.playerAffinity ?? 0))
        lines.push(`赠送给好友后，好感增加 ${Number(effect.playerAffinity)} 点。`);
    if (Number(effect.giftDailyLimit ?? 0))
        lines.push(`每日最多可用于赠礼 ${Number(effect.giftDailyLimit)} 次。`);
    if (effect.starOathRing)
        lines.push('星誓仪式需要双方各准备一枚。');
    if (Number(effect.battleCount ?? 0))
        lines.push(`持续 ${Number(effect.battleCount)} 场战斗。`);
    if (Number(effect.perBattleLimit ?? 0))
        lines.push(`每场战斗最多使用 ${Number(effect.perBattleLimit)} 次。`);
    return { effect, lines };
};
const skillBookDetails = async (effectJson) => {
    const code = String(jsonObject(effectJson).skillBook ?? '');
    if (!code)
        return null;
    const pool = await getPool();
    const [skills] = await pool.execute('SELECT id,name,category,skill_kind,element,range_type,target_scope,power,mana_cost,cooldown_turns,chant_turns,description FROM skill_definitions WHERE code=? LIMIT 1', [code]);
    const skill = skills[0];
    if (!skill)
        return null;
    const [effects] = await pool.execute('SELECT e.name,se.value_override,se.duration_override,se.target_scope FROM skill_effects se JOIN effect_definitions e ON e.id=se.effect_id WHERE se.skill_id=? ORDER BY e.id', [skill.id]);
    return { skill, effects };
};
const foodDetails = async (itemId) => {
    const pool = await getPool();
    const [rows] = await pool.execute('SELECT buff_json,duration_minutes FROM guild_restaurant_menu WHERE item_id=? AND is_active=1 LIMIT 1', [itemId]);
    return rows[0] ?? null;
};
const foodBuffText = (value) => {
    const buff = jsonObject(value);
...
var itemCodex = async () => {
    ...
    try {
        ...
        if (route.param('shop'))
            await discoverSecondaryFinished(event.current.UserId, String(route.param('shop')), codexId);
        const item = await itemCodex$1(event.current.UserId, codexId);
        ...
        if (item.item_type === 'consumable') {
            ...
            if (details.effect.alchemyOutput)
                details.lines = [item.description];
            appendItemUse(markdown, item);
            markdown.addNewline().addText(`使用方式：${itemUsePolicy(item).reason}`);
            markdown.addText(`\n使用等级：Lv.${item.required_level}\n\n效果：\n`);
            for (const effect of details.lines.length ? details.lines : ['使用后会产生特殊效果。'])
            ...
            if (book) {
                ...
                if (book.effects.length)
                    markdown.addText('特殊效果：\n').addBlockquote(book.effects.map((effect) => `${effect.name}${effect.value_override !== null ? `：${effect.value_override}` : ''}${effect.duration_override !== null ? `（${effect.duration_override}回合）` : ''}${effect.target_scope === 'self' ? '（自身）' : ''}`).join('｜')).addNewline();
