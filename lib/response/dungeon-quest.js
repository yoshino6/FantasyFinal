...
import { entranceStory, consultDungeonAtGuild, consultDungeonAtWorkshop, buyOddWorkshopItem, dungeonSecretProgress } from '../game/dungeon-quest.service.js';
import { messageFormat } from '../game/message.js';

const chapterFormat = (stage, text, buttons) => Format.create()
    .addMarkdown(Format.createMarkdown().addTitle(`地下的秘密（${stage}/6）`).addNewline().addNewline().addBlockquote(text))
    .addButtonGroup(buttons);
const dungeonSecretGuildHandler = async () => {
    ...
    try {
        ...
        await message.send({ format: messageFormat('无法询问地下迷宫', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const dungeonSecretWorkshopHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        ...
        await message.send({ format: messageFormat('无法询问破魔传送器', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const dungeonSecretEntranceHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        const dungeonId = Number(route.param('id'));
        const result = await entranceStory(event.current.UserId, dungeonId);
        const buttons = Format.createButtonGroup().addRow();
        if (result.stage >= 5)
            buttons.addButton('进入', `/下迷宫 ${dungeonId}`, { type: 'command', autoEnter: true, style: 'blue' });
        else if (result.stage < 2)
            buttons.addButton('前往 冒险者公会', '/前往 -2 -181 0', { type: 'command', autoEnter: false, style: 'blue' });
        else if (result.stage < 4)
            buttons.addButton('前往 异工坊', '/前往 6 -189 0', { type: 'command', autoEnter: false, style: 'blue' });
        else if (result.stage === 4)
            buttons.addButton('再次查看石门', `/地下的秘密 ${dungeonId}`, { type: 'command', autoEnter: true, style: 'blue' });
        ...
        await message.send({ format: messageFormat('地下的秘密', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const oddWorkshopBuyFormat = async (qqUserId) => secondaryShopFormat(qqUserId, 'oddworkshop');
const oddWorkshopBuyHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
...
const oddWorkshopPurchaseHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
...
const dungeonSecretStatusHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
...
export { dungeonSecretEntranceHandler, dungeonSecretGuildHandler, dungeonSecretStatusHandler, dungeonSecretWorkshopHandler, oddWorkshopBuyFormat, oddWorkshopBuyHandler, oddWorkshopPurchaseHandler };
