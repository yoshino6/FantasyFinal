...
import { specializationDescriptions } from '../game/skill-specialization.js';
...
import { isAdvancedProfessionSkillCode, advancedProfessionPassiveCodes, worldTreeAdvancedProfessions, hasBattleOnlyAdvancedPassiveEffect } from '../game/advanced-profession.config.js';
...
const skillDetailHandler = async () => {
    ...
    try {
        ...
        if (skill.learned && skill.category !== 'passive' && skill.category !== 'bound') {
            const names = { overcharge: '过充', instant: '瞬息', efficient: '节能', potent: '强效' };
            ...
            appendSkillEffectDetails(markdown, skill);
            {
                ...
                Object.keys(names).forEach((key, index) => {
                    ...
                    if (level < skill.specializationMaxLevel && skill.specializationUpgradeCost !== null)
                        markdown.addButton(`[升级(SP${skill.specializationUpgradeCost})]`, { data: `/升级专精 ${skill.id} ${names[key]}`, autoEnter: false });
        ...
        if (skill.category === 'passive' || skill.category === 'bound') {
            ...
            if (skill.passiveSpecializable) {
                ...
                if (skill.learned && level < skill.specializationMaxLevel && skill.specializationUpgradeCost !== null)
                    markdown.addNewline().addButton(`[升级强效(SP${skill.specializationUpgradeCost})]`, { data: `/升级专精 ${skill.id} 强效`, autoEnter: false });
            }
            else
            ...
            if (advancedProfessionPassiveCodes.has(skill.code)) {
                ...
                if (hasBattleOnlyAdvancedPassiveEffect(profession?.code))
