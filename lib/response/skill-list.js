...
import { talentByCode } from '../game/talent.config.js';
import { useTalentDetailMessage } from '../game/talent-detail-message.js';
import { useGameMessage } from '../game/use-game-message.js';
import { skillList as skillList$1, learnSkill, togglePassiveLink, skillDetail, toggleSkillShortcut, upgradeAppraisal, upgradeSkill, upgradeSkillSpecialization } from '../game/adventure.service.js';
import { messageFormat } from '../game/message.js';
import { specializationPerLevelLines, specializationNumberText, passiveSpecializationPerLevelLine, passiveSpecializationTotalLine } from '../game/skill-specialization-presentation.js';
import { balancedSkillDescription } from '../game/combat-skill-balance.config.js';
import { craftsmanshipEffect } from '../game/blacksmith.service.js';
import { advancedResourceRequirementForSkill, advancedResourceForProfession, advancedSkillDescriptions } from '../game/advanced-resource.config.js';
import { folioSkillByCode } from '../game/active-folio-skills.config.js';
import { folioEffectPreview } from '../game/folio-effect.config.js';
import { isAdvancedProfessionSkillCode, advancedBoundSkillDefinitions, advancedProfessionPassiveCodes, worldTreeAdvancedProfessions, hasBattleOnlyAdvancedPassiveEffect } from '../game/advanced-profession.config.js';

const categoryNames = { physical: '物理', magic: '魔法', utility: '辅助', passive: '被动', bound: '绑定', special: '特殊' };
const effectValueText = (value) => Number(value) % 1 === 0 ? String(Number(value)) : Number(value).toFixed(1);
const effectDescription = (effect) => {
    const value = effectValueText(effect.value);
    const duration = Number(effect.duration) ? `，持续${effect.duration}回合` : '';
    switch (effect.code) {
        ...
        case 'vulnerability':
        case 'armor_shatter':
        case 'sword_break': return `命中时，使目标物理防御降低${value}%${duration}。`;
...
const skillListFormat = async (qqUserId, view, page = 1, keyword = '') => {
    const data = await skillList$1(qqUserId);
    const markdown = Format.createMarkdown().addTitle('技能列表');
    if (data.isOmniscient)
        markdown.addText(' ').addButton('[贯注]', { data: '/技能贯注', autoEnter: false });
    ...
    if (view === '已学习') {
        ...
        if (!shortcuts.length)
            markdown.addBlockquote('暂无').addNewline();
        else
            for (const skill of shortcuts)
                markdown.addBlockquote(`技能${'①②③④'.charAt(Number(skill.quick_slot) - 1)} ${skill.name} `).addButton('[取消快捷]', { data: `/技能快捷 ${skill.id}`, autoEnter: false }).addNewline();
        ...
        if (!linkedPassives.length)
            markdown.addBlockquote('暂无').addNewline();
        else
            for (const skill of linkedPassives)
                markdown.addBlockquote(`【${skill.name}】${isAdvancedProfessionSkillCode(skill.code) ? '' : `Lv.${skill.level} `}`).addButton('[卸下]', { data: `/链接被动 ${skill.id}`, autoEnter: false }).addNewline();
        ...
        if (!displayed.length)
            markdown.addBlockquote(normalizedKeyword ? '没有找到符合条件的已学技能。' : '暂无').addNewline();
        ...
        if (!displayed.length)
            markdown.addBlockquote(normalizedKeyword ? '没有找到符合条件的技能。' : '尚无可学习的技能。').addNewline();
        for (const skill of displayed)
            markdown.addBlockquote(`【${skill.name}】${skill.tier}｜SP:${skill.learn_cost} `).addButton('[详情]', { data: `/技能详情 ${skill.id}`, autoEnter: false }).addText(' ').addButton('[学习]', { data: `/学习技能 ${skill.id}`, autoEnter: false }).addNewline();
    }
    markdown.addNewline().addNewline().addText(`当前第（${currentPage}/${totalPages}）页`);
    const command = (target) => `/技能分页 ${view} ${target}${normalizedKeyword ? ` ${normalizedKeyword}` : ''}`;
...
var skillList = async () => {
    ...
    const view = String(route.param('view') ?? '已学习') === '未学习' ? '未学习' : '已学习';
    try {
        await message.send({ format: await skillListFormat(event.current.UserId, view) });
    }
    catch (error) {
        await message.send({ format: messageFormat('技能不可用', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const skillPageHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    const view = String(route.param('view')) === '未学习' ? '未学习' : '已学习';
    try {
        await message.send({ format: await skillListFormat(event.current.UserId, view, Number(route.param('page')), String(route.param('keyword') ?? '')) });
    }
    catch (error) {
        await message.send({ format: messageFormat('技能不可用', error instanceof Error ? error.message : '请稍后重试。') });
    }
...
const skillSearchHandler = async () => {
    ...
    const view = String(route.param('view')) === '未学习' ? '未学习' : '已学习';
    try {
        await message.send({ format: await skillListFormat(event.current.UserId, view, 1, String(route.param('keyword') ?? '')) });
    }
    catch (error) {
        await message.send({ format: messageFormat('技能搜索失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const passiveLinkHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await togglePassiveLink(event.current.UserId, Number(route.param('id')));
        await message.send({ format: await skillListFormat(event.current.UserId, '已学习') });
    }
    catch (error) {
        await message.send({ format: messageFormat('链接失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const skillInfusionHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        const data = await skillList$1(event.current.UserId);
        if (!data.isOmniscient)
            throw new Error('只有副职业「全知者」能够贯注技能。');
        ...
        await message.send({ format: messageFormat('无法贯注', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const skillDetailHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    ...
    try {
        ...
        if (talent) {
            ...
            if (skill.learned)
                markdown.addNewline().addButton('[天赋操作]', { data: '/天赋', autoEnter: false });
        ...
        if (skill.code === 'appraisal' && skill.learned && skill.appraisal) {
            const eyeCost = skill.appraisal.rangeLevel;
            const pearlCost = skill.appraisal.informationLevel + 1;
            ...
            if (skill.appraisal.rangeLevel < 10)
                markdown.addButton(`[升级(SP${eyeCost})]`, { data: '/升级鉴识 慧眼', autoEnter: false });
            ...
            if (skill.appraisal.informationLevel < 4)
                markdown.addButton(`[升级(SP${pearlCost})]`, { data: '/升级鉴识 识珠', autoEnter: false });
        ...
        if (skill.weaponMastery && skill.learned) {
            const proficiency = Number(skill.specializations.overcharge ?? 1);
            const focus = Number(skill.specializations.instant ?? 1);
            const masteryText = {
            ...
            }[skill.code] ?? { weapon: '对应', stat: '属性', base: 0, step: 0 };
            ...
            if (skill.masteryProficiencyCost !== null)
                markdown.addButton(`[升级(SP${skill.masteryProficiencyCost})]`, { data: `/升级专精 ${skill.id} 娴熟`, autoEnter: false });
            ...
            if (skill.masteryFocusCost !== null)
                markdown.addButton(`[升级(SP${skill.masteryFocusCost})]`, { data: `/升级专精 ${skill.id} 随心`, autoEnter: false });
        ...
        if (skill.learned && skill.category !== 'passive' && skill.category !== 'bound') {
            const names = { overcharge: '过充', potent: '强效', instant: '瞬息', efficient: '节能' };
            ...
            const resource = advancedResourceForProfession(resourceRequirement?.professionCode);
            const markdown = Format.createMarkdown().addTitle('技能详情').addNewline().addNewline().addText(`【${skill.name}】${`Lv.${skill.level}`}\n`)
            ...
            if (resourceRequirement && resource)
                markdown.addBlockquote(`专属资源：消耗${resourceRequirement.amount}${resource.name}。${resource.summary}`).addNewline();
            appendSkillEffectDetails(markdown, skill);
            {
                markdown.addNewline().addText('专精 · 下一级变化\n');
                Object.keys(names).forEach((key, index) => {
                    if (!skill.specializationChoices.includes(key))
                        return;
                    const level = Number(skill.specializations[key] ?? 1);
                    markdown.addText(`${'①②③④'.charAt(index)}${names[key]} Lv.${level}/${skill.specializationMaxLevel} `);
                    const cost = skill.specializationUpgradeCosts[key];
                    if (cost != null)
                        markdown.addButton(`[升级(SP${cost})]`, { data: `/升级专精 ${skill.id} ${names[key]}`, autoEnter: false });
                    markdown.addNewline();
                    for (const line of perLevel[key])
        ...
        if (skill.code.startsWith('talent_'))
            markdown.addNewline().addButton('[天赋操作]', { data: '/天赋', autoEnter: false });
        if (skill.category === 'passive' || skill.category === 'bound') {
            ...
            if (skill.passiveSpecializable) {
                ...
                if (skill.learned && level < skill.specializationMaxLevel && skill.specializationUpgradeCost !== null)
                    markdown.addButton(`[升级强效(SP${skill.specializationUpgradeCost})]`, { data: `/升级专精 ${skill.id} 强效`, autoEnter: false }).addNewline();
                markdown.addNewline().addText('总体变化（仅专精）\n').addBlockquote(passiveSpecializationTotalLine(skill.passiveFactor));
            }
            else
                markdown.addNewline().addBlockquote('专精：固定机制被动不放大权限、次数或资源返还；装备精通和鉴识保留专用成长。');
            const bound = advancedBoundSkillDefinitions.find(entry => entry.code === skill.code);
            if (bound)
                markdown.addNewline().addBlockquote(`本职${bound.kind}：常驻生效，不占普通被动槽；切换二转时随职业更换。`);
            if (bound?.kind === '传承')
                markdown.addNewline().addBlockquote('传承按本职规则触发，不会因列入技能列表而重复结算。');
                ...
                if (!profession || hasBattleOnlyAdvancedPassiveEffect(profession.code))
                    markdown.addNewline().addBlockquote('战斗规则：此效果仅在战斗结算时生效，不会增加角色详情中的基础属性。');
                else
                    markdown.addNewline().addBlockquote('属性规则：此效果已在角色属性重算时写入角色详情，不会在战斗中重复叠加。');
            }
        }
        else {
        ...
        if (appraisalText)
            markdown.addNewline().addText(appraisalText.trim());
        if (costText)
        ...
        if (!skill.learned)
            buttons.addButton('学习', `/学习技能 ${skill.id}`, { type: 'command', autoEnter: true, style: 'blue' });
        else if (skill.code === 'appraisal' && skill.appraisal) {
            if (skill.appraisal.rangeLevel < 10)
                buttons.addButton('升级慧眼', '/升级鉴识 慧眼', { type: 'command', autoEnter: true, style: 'blue' });
            if (skill.appraisal.informationLevel < 4)
                buttons.addButton('升级识珠', '/升级鉴识 识珠', { type: 'command', autoEnter: true, style: 'blue' });
        }
        else if (!fixedAdvancedSkill && skill.nextUpgradeCost !== null)
            buttons.addButton('升级', `/升级技能 ${skill.id}`, { type: 'command', autoEnter: true, style: 'blue' });
        await message.send({ format: Format.create().addMarkdown(markdown).addButtonGroup(buttons) });
    }
    catch (error) {
        await message.send({ format: messageFormat('无法查看技能', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const learnSkillHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        const result = await learnSkill(event.current.UserId, Number(route.param('id')));
        await message.send({ format: Format.create().addMarkdown(Format.createMarkdown().addTitle('学习技能').addNewline().addNewline().addText(`已学习「${result.name}」，消耗 ${result.cost} 技能点。`)).addButtonGroup(Format.createButtonGroup().addRow().addButton('技能列表', '/技能列表', { type: 'command', autoEnter: true, style: 'blue' })) });
    }
    catch (error) {
        await message.send({ format: messageFormat('学习失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const skillShortcutHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        const result = await toggleSkillShortcut(event.current.UserId, Number(route.param('id')));
        await message.send({ format: messageFormat('技能快捷', result.slot ? `「${result.name}」已设为技能${'①②③④'.charAt(result.slot - 1)}。` : `已取消「${result.name}」的快捷设置。`) });
    }
    catch (error) {
        await message.send({ format: messageFormat('设置失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const upgradeSkillHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        const result = await upgradeSkill(event.current.UserId, Number(route.param('id')));
        await message.send({ format: messageFormat('技能升级', `「${result.name}」已提升至 Lv.${result.level}，消耗 ${result.cost} 技能点。`) });
    }
    catch (error) {
        await message.send({ format: messageFormat('升级失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const upgradeSpecializationHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    const specialization = { 过充: 'overcharge', 瞬息: 'instant', 节能: 'efficient', 强效: 'potent', 娴熟: 'overcharge', 随心: 'instant' }[String(route.param('specialization'))];
    try {
        const result = await upgradeSkillSpecialization(event.current.UserId, Number(route.param('id')), specialization);
        await message.send({ format: messageFormat('技能升级', `「${result.name}」已提升至 Lv.${result.skillLevel}\n消耗 ${result.cost} 技能点。`) });
    }
    catch (error) {
        await message.send({ format: messageFormat('升级失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const upgradeAppraisalHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    ...
    try {
        ...
        await message.send({ format: messageFormat('鉴识升级失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};

export { skillList as default, learnSkillHandler, passiveLinkHandler, skillDetailHandler, skillInfusionHandler, skillPageHandler, skillSearchHandler, skillShortcutHandler, upgradeAppraisalHandler, upgradeSkillHandler, upgradeSpecializationHandler };
