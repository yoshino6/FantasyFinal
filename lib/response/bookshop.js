...
import { addNpcAffinity, grantNpcAffinity, requireNpcAtCurrentPosition } from '../game/adventure.service.js';
import { buyBookshopItem, sellBookshopItem, readSkillBook, bookshopCatalog, bookshopSellCatalog } from '../game/bookshop.service.js';
import { messageFormat } from '../game/message.js';
import { npcChatDialogue } from '../game/npc-dialogue.service.js';
import { omniscientQuest, acceptOmniscientQuest, claimOmniscientQuest, omniscientProgress, omniscientTraces } from '../game/omniscient.service.js';

const code = 'bookshop';
const requireBookshop = (qqUserId) => requireNpcAtCurrentPosition(qqUserId, code);
const scene = () => {
    const hour = new Date().getHours();
    if (hour < 11)
        return '晨光从三层书屋的高窗斜斜洒下，书页与木架都泛着温暖的颜色。白须店主戴着老花镜，正把厚重的百科全书贴得很近，一字一句地读。';
    if (hour < 18)
        return '午后书屋里坐满翻阅资料的少年与冒险者。书架一直延伸到楼梯尽头，老人缓缓走过其间，不时为客人抽出一本更合适的书。';
    return '夜色落在百味书屋的玻璃窗上，三层楼仍亮着柔黄的灯。老人伏在柜台后翻着旧卷宗，偶尔抬头，耐心替晚来的冒险者指路。';
};
const bookshopFormat = async (user, text, chatting = false) => {
    ...
    if (chatting)
        return Format.create().addMarkdown(markdown).addButtonGroup(Format.createButtonGroup().addRow().addButton('切磋', '/切磋 bookshop', { type: 'command', autoEnter: true, style: 'blue' }).addButton('继续闲聊', '/书屋闲聊', { type: 'command', autoEnter: true, style: 'blue' }));
...
const buyFormat = async (qqUserId, page = 1, keyword = '') => {
    const shop = await bookshopCatalog(qqUserId, page, keyword);
    const markdown = Format.createMarkdown().addTitle('百味书屋·购买').addNewline().addNewline().addBlockquote('洛文扶正老花镜，慈和地笑了笑：“知识不分年纪，也不该只躺在书架上。挑一本合眼缘的，慢慢读。”').addNewline().addNewline();
    if (!shop.items.length)
        markdown.addText('没有找到符合条件的书籍。');
    for (const [index, item] of shop.items.entries())
        markdown.addText(`${'①②③④⑤'.charAt(index)}【${item.category}】`).addButton(item.name, { data: `/物品图鉴 ${item.codexId}`, autoEnter: false }).addText(' ').addButton('[购买]', { data: `/购买书屋物品 ${item.id} `, autoEnter: false }).addNewline().addBlockquote(`价格：铜币×${item.price}｜剩余：${item.stockQuantity}｜${item.ownedQuantity ? `已拥有${item.ownedQuantity}` : '未拥有'}`).addNewline().addBlockquote(`简介：${item.description}`).addNewline().addNewline();
    markdown.addText(`当前第（${shop.page}/${shop.totalPages}）页｜持有铜币：${shop.copper}`);
    return Format.create().addMarkdown(markdown).addButtonGroup(pageButtons(shop.page, shop.totalPages, '书屋购买页', '书屋购买搜索', shop.keyword));
};
const sellFormat = async (qqUserId, page = 1, keyword = '') => {
    const shop = await bookshopSellCatalog(qqUserId, page, keyword);
    const markdown = Format.createMarkdown().addTitle('百味书屋·出售').addNewline().addNewline().addBlockquote('洛文轻轻拂去柜台上的纸屑：“书籍、卷宗与手札我都收。它们或许会在下一位读者手里，继续找到新的答案。”').addNewline().addNewline();
    if (!shop.items.length)
        markdown.addText('背包中没有可出售的书籍、卷宗或技能书。');
    ...
    markdown.addText(`当前第（${shop.page}/${shop.totalPages}）页｜持有铜币：${shop.copper}`);
    return Format.create().addMarkdown(markdown).addButtonGroup(pageButtons(shop.page, shop.totalPages, '书屋出售页', '书屋出售搜索', shop.keyword));
};
const bookshopHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    await requireBookshop(event.current.UserId);
    await message.send({ format: await bookshopFormat(event.current.UserId) });
}
catch (error) {
    await message.send({ format: messageFormat('无法进入书屋', error instanceof Error ? error.message : '请稍后重试。') });
} };
const bookshopBuyHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireBookshop(event.current.UserId);
...
const bookshopBuySearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireBookshop(event.current.UserId);
...
const bookshopPurchaseHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireBookshop(event.current.UserId);
    const amount = String(route.param('quantity') ?? '').trim();
    const result = await buyBookshopItem(event.current.UserId, Number(route.param('id')), amount ? Number(amount) : 1);
...
const bookshopSellHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireBookshop(event.current.UserId);
...
const bookshopSellSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireBookshop(event.current.UserId);
...
const bookshopSellItemHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireBookshop(event.current.UserId);
    const amount = String(route.param('quantity') ?? '').trim();
    const result = await sellBookshopItem(event.current.UserId, Number(route.param('id')), amount ? Number(amount) : 1);
...
const bookshopChatHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    await requireBookshop(event.current.UserId);
    const affinity = await addNpcAffinity(event.current.UserId, code, 'chat');
    await message.send({ format: await bookshopFormat(event.current.UserId, npcChatDialogue('bookshop', affinity.affinity), true) });
}
catch (error) {
    await message.send({ format: messageFormat('无法闲聊', error instanceof Error ? error.message : '请稍后重试。') });
} };
const readBookHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    const result = await readSkillBook(event.current.UserId, Number(route.param('id')));
    await message.send({ format: messageFormat('研读完成', `你读完了【${result.book}】。\n领悟技能【${result.skill}】，可前往“技能列表·未学习”消耗技能点学习。`) });
}
catch (error) {
    await message.send({ format: messageFormat('研读失败', error instanceof Error ? error.message : '请稍后重试。') });
} };
const omniscientAboutHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireBookshop(event.current.UserId);
        const quest = await omniscientQuest(event.current.UserId);
...
const omniscientProfessionSelectHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireBookshop(event.current.UserId);
        const quest = await omniscientQuest(event.current.UserId);
        if (quest.status !== 'none')
            throw new Error('你已经接取或完成了全知者任务。');
...
const acceptOmniscientQuestHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireBookshop(event.current.UserId);
        await acceptOmniscientQuest(event.current.UserId);
        const markdown = Format.createMarkdown().addTitle('接受任务').addNewline().addNewline().addText('已接受【副职业·全知者入门】\n挑战并观察：森林史莱姆、幽影狼王\n可随时通过 ').addButton('/任务', { data: '/任务', autoEnter: false }).addText(' 查看进度。');
        ...
        if (error instanceof Error && error.message === 'secondary_profession_level_required') {
            await message.send({ format: Format.create().addMarkdown(Format.createMarkdown().addTitle('洛文的劝告').addNewline().addNewline().addBlockquote('洛文缓缓摇头，把一册野外笔记推回书架。\n“眼下的你还缺少几次真正的险境。先去磨炼吧，等你到了 Lv.10，能在危机中保持观察，再回来找我。”')) });
            return;
        }
        await message.send({ format: messageFormat('接取失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const claimOmniscientQuestHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireBookshop(event.current.UserId);
        const result = await claimOmniscientQuest(event.current.UserId);
        await grantNpcAffinity(event.current.UserId, code, 200);
...
const omniscientProfessionFormat = async (qqUserId) => {
    const progress = await omniscientProgress(qqUserId);
    const maxed = progress.required === 0;
    const filled = maxed ? 10 : Math.floor(Math.max(0, Math.min(1, progress.proficiency / progress.required)) * 10);
...
const omniscientInsightHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        ...
        await message.send({ format: messageFormat('明鉴失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const omniscientTraceHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await omniscientProgress(event.current.UserId);
        const trace = await omniscientTraces(event.current.UserId);
        ...
        await message.send({ format: messageFormat('识踪失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const omniscientIngenuityHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        const progress = await omniscientProgress(event.current.UserId);
        ...
        await message.send({ format: messageFormat('巧思失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};

export { acceptOmniscientQuestHandler, bookshopBuyHandler, bookshopBuySearchHandler, bookshopChatHandler, bookshopHandler, bookshopPurchaseHandler, bookshopSellHandler, bookshopSellItemHandler, bookshopSellSearchHandler, claimOmniscientQuestHandler, omniscientAboutHandler, omniscientIngenuityHandler, omniscientInsightHandler, omniscientProfessionFormat, omniscientProfessionSelectHandler, omniscientTraceHandler, readBookHandler };
