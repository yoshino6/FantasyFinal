import { armorPieceDescription } from '../game/armor-class.js';
...
const equipmentSections = (effectJson, quality, primaryJson, category, subtype) => {
    ...
    const attributes = Object.entries(effect)
        ...
        const armorScale = key === 'physicalDefense' || key === 'magicDefense' ? armorClassDefenseMultiplier() : 1;
    ...
    if (armorText)
        effects.unshift('甲类逐件乘算：' + armorText);
...
const enchantmentSlotText = (enchantment) => enchantment.allowedSlots
    .map(slot => equipmentSlotName(slot))
    .filter(Boolean)
    .join(' / ') || '未知';
const detailMarkdown = (item, heading = '装备详情') => {
    ...
    if (item.enchantment)
        markdown.addBlockquote(`${item.enchantment.cardName}（版本 ${item.enchantment.cardVersion}）
