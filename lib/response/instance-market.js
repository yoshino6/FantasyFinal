...
const enchantmentSlotText = (value) => (Array.isArray(value) ? value : [])
    .map(slot => equipmentSlotName(String(slot)))
    .filter(Boolean)
    .join(' / ') || '未知';
...
const listFormat = async (user, page, category, keyword) => {
    ...
    for (const [index, item] of data.items.entries()) {
        ...
        if (item.kind === 'automaton') {
            ...
            if (enchantment?.cardName)
                markdown.addBlockquote(`附魔：${escapeAutomatonText(String(enchantment.cardName))}｜可附魔部位：${enchantmentSlotText(enchantment.allowedSlots)}｜${escapeAutomatonText(String(enchantment.effectText ?? ''))}`).addNewline();
