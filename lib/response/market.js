...
import { createMarketSellOrder, createMarketBuyOrder, cancelMarketOrder, marketFeeProfile, marketCatalog, marketItemDetail, marketSellable, marketOrders, MARKET_TYPES } from '../game/market.service.js';
import { messageFormat } from '../game/message.js';
import { girlGratitudeStage } from '../game/girl-gratitude.service.js';

const marketCode = 'canopy_exchange';
const sequence = '①②③④⑤';
const access = async (qqUserId) => {
    await requireNpcAtCurrentPosition(qqUserId, marketCode);
    if (await girlGratitudeStage(qqUserId) < 6)
        throw new Error('先完成梨子喵的谢礼之约，万叶联会才会向你开放。');
};
const fail = async (message, title, error) => message.send({ format: messageFormat(title, error instanceof Error ? error.message : '请稍后重试。') });
const listButtons = (page, totalPages, type, keyword) => {
    ...
    buttons.addRow();
    for (const typeName of MARKET_TYPES)
        buttons.addButton(typeName, `/万叶市场 1 ${typeName}`, { type: 'command', autoEnter: true, style: typeName === type ? 'blue' : undefined });
...
const marketHomeFormat = async (qqUserId) => {
...
const marketListFormat = async (qqUserId, page, type, keyword) => {
    ...
    if (!market.items.length)
        markdown.addText('暂时没有符合条件的公开订单。').addNewline().addNewline();
...
const sellListFormat = async (qqUserId, page, keyword) => {
    ...
    if (!catalog.items.length)
        markdown.addText('背包中没有可寄售的常规堆叠物品。').addNewline().addNewline();
...
const detailFormat = async (qqUserId, itemId) => {
    ...
    if (item.lowestSell !== null)
        buttons.addButton('按最低价买入', `/万叶求购 ${item.id} ${item.lowestSell} `, { type: 'command', autoEnter: false });
    buttons.addRow().addButton('返回 市场', '/万叶市场 1 全部', { type: 'command', autoEnter: true });
    return Format.create().addMarkdown(markdown).addButtonGroup(buttons);
};
const ordersFormat = async (qqUserId) => {
    ...
    if (!result.orders.length)
        markdown.addText('还没有提交过市场订单。').addNewline().addNewline();
    result.orders.forEach((order, index) => {
        const side = order.side === 'sell' ? '寄售' : '求购';
        const state = { open: '等待成交', partial: '部分成交', filled: '已完成', cancelled: '已撤销', expired: '已到期', frozen: '审核中' }[order.status] ?? order.status;
        ...
        if (order.status === 'open' || order.status === 'partial')
            markdown.addButton('[撤销订单]', { data: `/万叶撤单 ${order.id}`, autoEnter: false }).addNewline();
        markdown.addNewline();
    });
...
const marketHomeHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    await message.send({ format: await marketHomeFormat(event.current.UserId) });
}
catch (error) {
    await fail(message, '无法进入万叶联市', error);
} };
const marketListHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    await message.send({ format: await marketListFormat(event.current.UserId, Number(route.param('page') ?? 1), String(route.param('type') ?? '全部'), String(route.param('keyword') ?? '')) });
}
catch (error) {
    await fail(message, '市场暂不可用', error);
} };
const marketSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    await message.send({ format: await marketListFormat(event.current.UserId, 1, '全部', String(route.param('keyword') ?? '')) });
}
catch (error) {
    await fail(message, '搜索失败', error);
} };
const marketSellListHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    await message.send({ format: await sellListFormat(event.current.UserId, Number(route.param('page') ?? 1), String(route.param('keyword') ?? '')) });
}
catch (error) {
    await fail(message, '寄售列表不可用', error);
} };
const marketSellSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    await message.send({ format: await sellListFormat(event.current.UserId, 1, String(route.param('keyword') ?? '')) });
}
catch (error) {
    await fail(message, '搜索失败', error);
} };
const marketDetailHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    await message.send({ format: await detailFormat(event.current.UserId, Number(route.param('id'))) });
}
catch (error) {
    await fail(message, '无法查看物品', error);
} };
const marketSellHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    const result = await createMarketSellOrder(event.current.UserId, Number(route.param('id')), Number(route.param('price')), Number(route.param('quantity')));
    const suffix = result.remaining ? `，剩余 ${result.remaining} 件等待成交。` : '，已全部成交。';
    await message.send({ format: messageFormat('寄售订单已提交', `【${result.name}】×${result.quantity}，单价铜币×${result.price}${suffix}`) });
    await message.send({ format: await marketHomeFormat(event.current.UserId) });
}
catch (error) {
    await fail(message, '寄售失败', error);
} };
const marketBuyHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    const result = await createMarketBuyOrder(event.current.UserId, Number(route.param('id')), Number(route.param('price')), Number(route.param('quantity')));
    const suffix = result.remaining ? `，剩余 ${result.remaining} 件继续公开求购。` : '，已全部成交。';
    await message.send({ format: messageFormat('求购订单已提交', `【${result.name}】×${result.quantity}，单价铜币×${result.price}${suffix}`) });
    await message.send({ format: await marketHomeFormat(event.current.UserId) });
}
catch (error) {
    await fail(message, '求购失败', error);
} };
const marketOrdersHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    await message.send({ format: await ordersFormat(event.current.UserId) });
}
catch (error) {
    await fail(message, '订单列表不可用', error);
} };
const marketCancelHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await access(event.current.UserId);
    const result = await cancelMarketOrder(event.current.UserId, Number(route.param('id')));
    const fee = result.fee ? `快速撤单费用：铜币×${result.fee}\n` : '';
    await message.send({ format: messageFormat('订单已撤销', fee + (result.side === 'sell' ? `已退回托管物品×${result.quantity}。` : `已退回未成交部分的铜币。`)) });
    await message.send({ format: await ordersFormat(event.current.UserId) });
}
catch (error) {
    await fail(message, '撤单失败', error);
} };
const marketFeeHandler = async () => {
    ...
    try {
        ...
        await fail(message, '无法查看手续费', error);
    }
};

export { marketBuyHandler, marketCancelHandler, marketDetailHandler, marketFeeHandler, marketHomeFormat, marketHomeHandler, marketListHandler, marketOrdersHandler, marketSearchHandler, marketSellHandler, marketSellListHandler, marketSellSearchHandler };
