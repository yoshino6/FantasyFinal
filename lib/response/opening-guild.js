...
import { openingGuildView, enterOpeningGuild, openingGuildAction, openingTransport, openingKeepsakes } from '../game/opening-guild.service.js';
import { rootGuildPeople, guildLessons } from '../game/opening-guild.config.js';
import { nearbyPoints } from '../game/adventure.service.js';
import { openingHubs } from '../game/opening-world.config.js';
import { progressionMapReceipt } from '../game/progression-map.service.js';

const openingGuildServiceFormat = (text) => {
    const markdown = Format.createMarkdown().addTitle('公会交接').addNewline().addNewline();
    for (const paragraph of text.trim().split(/\r?\n\s*\r?\n/)) {
        if (/^(已领取|已兑换|已享用|这次是复习|凭单已核销|急救券已登记|补给券已登记|你已领过)/.test(paragraph))
            markdown.addText(paragraph);
        else
            markdown.addBlockquote(paragraph.replace(/\r?\n/g, '\n> '));
        markdown.addNewline().addNewline();
    }
    return Format.create().addMarkdown(markdown);
};
const openingGuildFormat = async (user, area = '大厅') => {
    const view = await openingGuildView(user);
    const section = ['大厅', '次页'].includes(area) ? '' : `·${area === '休息' ? '休息区' : area}`;
    const md = Format.createMarkdown().addTitle(`${view.hub.name}·${view.hub.guildName}${section}`).addNewline().addNewline();
    const buttons = Format.createButtonGroup();
    const add = (label, command, autoEnter = true) => buttons.addButton(label, command, { type: 'command', autoEnter, style: 'blue' });
    if (!view.at) {
        md.addText(`公会入口位于（${view.place.pos_x}，${view.place.pos_y}，${view.place.pos_z}）。`).addNewline().addBlockquote('办事员与补给都在柜台等候，请先抵达入口。');
        buttons.addRow();
        add('前往公会', `/前往 ${view.place.pos_x} ${view.place.pos_y} ${view.place.pos_z}`, false);
        add('查看地图', '/地图');
    }
    else if (!view.inside) {
        ...
        return (await import('./adventure.js')).guildFrontDeskFormat(user);
    }
    else if (area === '人物') {
        md.addBlockquote(view.hub.description);
        if (view.code === 'world_tree')
            for (const [index, person] of rootGuildPeople.entries()) {
                md.addNewline().addNewline().addText(`${person.name} · ${person.role}`);
                if (index % 2 === 0)
                    buttons.addRow();
                add(person.name.split('·').at(-1), `/初行服务 chat ${person.code}`);
            }
        else
            md.addNewline().addNewline().addBlockquote(`“${view.code === 'frost_dragon_inn' ? '我已经把结实的椅子摆好了。别急，尾巴也给你们让开。' : view.code === 'floating_leaf_town' ? '云上也有人会迷路，不必为了刚到这里而难为情。' : '欢迎回来，需要什么帮助？'}”`);
        buttons.addRow();
        add('返回休息区', '/初行公会 休息区');
    }
    else if (area === '礼包') {
        ...
        buttons.addRow();
        add('核对路线礼包', '/初行服务 pack');
        add('补领选职武器', '/初行服务 profession_weapon');
        buttons.addRow();
        add('返回委托板', '/初行公会 委托板');
    }
    else if (area === '集结区') {
        ...
        buttons.addRow();
        add('创建队伍', '/组队 创建');
        add('加入队伍', '/组队 加入 ', false);
        buttons.addRow();
        add('领取地图', '/初行公会 地图', false);
        add('补领通行地图', '/初行服务 map_reclaim', false);
        buttons.addRow();
        add('返回大厅', '/初行公会');
    }
    else if (area === '后勤区') {
        ...
        if (view.mapRepair.granted.length || view.mapRepair.stored.length || view.mapRepair.unavailable.length)
            md.addNewline().addNewline().addText(progressionMapReceipt(view.mapRepair));
        ...
        for (const map of view.maps) {
            ...
            if (map.codexId)
                md.addButton('[地图详情]', { data: `/物品图鉴 ${map.codexId}`, autoEnter: false });
            if (view.registered && map.canExchange)
                md.addText(' ').addButton('[用登记额度兑换]', { data: `/初行服务 map_exchange ${map.code}`, autoEnter: false });
        }
        if (!view.maps.length)
            md.addNewline().addNewline().addText('周边暂时没有已开放的地图资料。');
        buttons.addRow();
        add('初行见闻', '/初行见闻');
        add('地图教学', '/初行服务 lesson map');
        buttons.addRow();
        add('公会商店', '/工会商店', false);
        buttons.addRow();
        add('返回集结区', '/初行公会 集结区');
        ...
        buttons.addRow();
        add('工艺教学', '/初行服务 lesson craft');
        add('凭单练习', '/初行服务 craft_practice');
        buttons.addRow();
        add('返回后勤区', '/初行公会 后勤区');
    }
        ...
        buttons.addRow();
        add('随从名册', '/随从');
        add('契约教学', '/初行服务 lesson contract');
        buttons.addRow();
        add('领取基础疗养', '/初行服务 heal_companion');
        add('购买灵契饲料', '/商店搜索 灵契饲料');
        buttons.addRow();
        add('返回后勤区', '/初行公会 后勤区');
    }
    else if (area === '教学') {
        md.addBlockquote('公会把教具放在窗边。这里的练习不会刷出高等级野怪，也不要求你交出仅有的行装。');
        for (const lesson of guildLessons) {
            buttons.addRow();
            add(lesson.title, `/初行服务 lesson ${lesson.code}`);
        }
        buttons.addRow();
        add('返回委托板', '/初行公会 委托板');
    }
        ...
        if (view.code === 'world_tree')
            for (const [code, hub] of Object.entries(openingHubs)) {
                if (['world_tree', 'baina_town'].includes(code) || code === 'floating_leaf_town' && !view.world.leaf_route_open)
                    continue;
                buttons.addRow();
                add(hub.name, `/初行接驳 ${code}`);
            }
        else if (view.code !== 'baina_town') {
            buttons.addRow();
            add('前往世界树', '/初行接驳 world_tree');
        }
        else
            md.addNewline().addText('百纳镇与世界树的通行请向原有界门驿站办理。');
        buttons.addRow();
        add('返回后勤区', '/初行公会 后勤区');
    }
    else {
...
const openingGuildHandler = (mode = 'view') => async () => {
    ...
    try {
        ...
        if (mode === 'enter' || mode === 'leave')
            await enterOpeningGuild(event.current.UserId, mode === 'enter');
        if (mode === 'service') {
            ...
            if (action === 'chat') {
                ...
                await message.send({ format: Format.create().addMarkdown(npcInteractionMarkdown(`冒险者公会·${area}`, person.name.split('·').at(-1), text, person.code, nearby.npcDetailsUnlocked))
                        .addButtonGroup(Format.createButtonGroup().addRow().addButton('继续闲聊', `/初行服务 chat ${person.code}`, { type: 'command', autoEnter: true, style: 'blue' }).addButton('返回休息区', '/初行公会 休息区', { type: 'command', autoEnter: true })) });
                return;
            }
            ...
            const serviceAreas = { pack: '礼包', profession_weapon: '礼包', map_exchange: '地图', map_reclaim: '地图', craft_practice: '工艺', recover: '休息区', heal_companion: '兽栏' };
            const lessonAreas = { contract: '兽栏', craft: '工艺', map: '地图' };
        ...
        if (mode === 'keepsakes') {
            const view = await openingKeepsakes(event.current.UserId);
            const md = Format.createMarkdown().addTitle('初行见闻·未解之事').addNewline().addNewline();
            for (const item of view.items) {
                md.addText(`【${item.name}】${item.used ? ' · 本次服务已使用' : ''}`).addNewline().addBlockquote(`现在：${item.use}`).addNewline().addBlockquote(`未解之事：${item.future}`).addNewline();
                if (item.recordOnly)
                    md.addButton('[人物资料]', { data: `/初行人物 ${item.route}`, autoEnter: false });
                else
                    md.addButton('[查看经办窗口]', { data: `/初行凭物 ${item.code}`, autoEnter: false });
                md.addNewline().addNewline();
            }
            if (!view.items.length)
                md.addBlockquote('你还没有留下特殊凭物。一路遇见的人与事，会慢慢填满这本见闻。\n\n');
            for (const event of view.events)
                md.addBlockquote(`世界纪事：${event.text}`).addNewline();
            await message.send({ format: Format.create().addMarkdown(md).addButtonGroup(Format.createButtonGroup().addRow().addButton('返回休息区', '/初行公会 休息区', { type: 'command', autoEnter: true }).addButton('继续剧情', '/继续剧情', { type: 'command', autoEnter: true })) });
            return;
        }
        await message.send({ format: await openingGuildFormat(event.current.UserId, returnArea) });
    }
    catch (error) {
        await message.send({ format: messageFormat('公会', error instanceof Error ? error.message : '请稍后再试。') });
    }
};

export { openingGuildFormat, openingGuildHandler, openingGuildServiceFormat };
