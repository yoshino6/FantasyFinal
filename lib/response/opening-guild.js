...
const openingGuildFormat = async (user, area = '大厅') => {
    ...
    if (!view.at) {
        ...
        buttons.addRow();
        add('前往公会', `/前往 ${view.place.pos_x} ${view.place.pos_y}`);
        add('查看地图', '/地图');
    }
    else if (!view.inside) {
        ...
        if (view.code === 'world_tree')
            ...
            md.addNewline().addNewline().addBlockquote(`“${view.code === 'sleepwhale_market' ? '请慢慢看，停在柜台前也不会按分钟收费。' : view.code === 'frost_dragon_inn' ? '我已经把结实的椅子摆好了。别急，尾巴也给你们让开。' : view.code === 'floating_leaf_town' ? '云上也有人会迷路，不必为了刚到这里而难为情。' : view.code === 'snowlamp_hollow' ? '手暖起来，字也会写得顺些。' : '欢迎回来，需要什么帮助？'}”`);
        buttons.addRow();
        add('返回休息区', '/初行公会 休息区');
    }
    else if (area === '礼包') {
        ...
        for (const map of view.maps) {
            ...
            if (map.canExchange)
                md.addText(' ').addButton('[兑换地图]', { data: `/初行服务 map_exchange ${map.code}`, autoEnter: false });
        }
        if (!view.maps.length)
...
const openingGuildHandler = (mode = 'view') => async () => {
    ...
    try {
        ...
        if (mode === 'service') {
            ...
            const serviceAreas = { pack: '礼包', profession_weapon: '礼包', map_exchange: '地图', craft_practice: '工艺', recover: '休息区', heal_companion: '兽栏' };
