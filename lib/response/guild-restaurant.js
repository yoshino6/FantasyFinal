...
import { requireNpcAtCurrentPosition, addNpcAffinity } from '../game/adventure.service.js';
import { enjoyRestaurantMeal, foodBuffText, restaurantMenu } from '../game/guild-restaurant.service.js';
...
const menuFormat = async (qqUserId, page = 1, keyword = '') => {
    ...
    const markdown = Format.createMarkdown().addTitle('百纳镇·冒险者公会·餐厅').addNewline().addNewline()
...
const requireRestaurant = (qqUserId) => requireNpcAtCurrentPosition(qqUserId, 'guild_counter');
var guildRestaurant = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
...
const restaurantMenuHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    await requireRestaurant(event.current.UserId);
...
const restaurantSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    await requireRestaurant(event.current.UserId);
...
const enjoyMealHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    await requireRestaurant(event.current.UserId);
