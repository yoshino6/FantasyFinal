...
const barrierActive = async (qqUserId) => ['【主线·无形的禁锢】', '【主线·寻访晴儿】', '【主线·追寻天空粉尘】', '【主线·归还天空粉尘】', '【主线·窥探世间】'].includes((await currentMainQuest(qqUserId)).title);
...
const alchemistBarrierHandler = async () => {
    ...
    try {
        ...
        if (progress.stage === 2 && (progress.previous === 0 || progress.previous === 1)) {
...
const alchemyFormat = async (qqUserId, page = 1, keyword = '') => {
    ...
    if (creationQuest.pending)
        markdown.addBlockquote('新支线·瓶中之外的世界：炼金已达四级，去糖水屋找晴儿，听听她尚未讲完的那堂课。').addNewline().addButton('[前往 糖水屋]', { data: '/前往 -12 -196 0', autoEnter: false }).addNewline().addNewline();
