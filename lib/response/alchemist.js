...
const alchemistShopButtons = (page, totalPages, category, keyword = '') => Format.createButtonGroup()
...
const alchemistBuyFormat = async (qqUserId, page = 1, category = '全部', keyword = '') => {
    const shop = await alchemistShopCatalog(qqUserId, page, category, keyword);
    const markdown = Format.createMarkdown().addTitle('糖水屋·购买').addNewline().addNewline().addBlockquote(keyword ? `晴儿找出了与「${keyword}」有关的药剂。` : '晴儿轻轻摆好一列色泽澄澈的药剂。“回复品和秘药都在这里，出门前记得按自己的需要准备。”').addNewline().addNewline();
...
const bulkPurificationConfirmFormat = async (qqUserId) => {
    ...
    if (!preview.materials.length)
        ...
        for (const material of preview.materials)
            markdown.addBlockquote(`【${material.name}】×${material.quantity} → 【${material.outputName}】预计${material.expectedOutput.toFixed(1)}份`).addNewline();
    }
...
const alchemyFormat = async (qqUserId, page = 1, keyword = '') => {
    ...
    appendSlot('反应剂', 'reagent', state.reagentId, state.reagentName, state.reagentQuantity);
    ...
    if (state.processing)
        markdown.addText('操作：').addBlockquote('炼金反应正在进行中，请等待本次结果。');
    else
...
const purificationHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
...
const sweetshopPurificationHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
...
const purificationContinueHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    await message.send({ format: await purificationFormat(event.current.UserId) });
}
catch (error) {
    await message.send({ format: messageFormat('无法提纯', error instanceof Error ? error.message : '请稍后重试。') });
} };
const purificationPageHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const purificationSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const purificationPutHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const purificationClearHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
...
const purificationExecuteHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    const result = await executePurification(event.current.UserId);
...
const bulkPurificationPreviewHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
...
const bulkPurificationExecuteHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    const result = await executeBulkPurification(event.current.UserId);
...
const alchemyHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    await activatePersonalAlchemy(event.current.UserId);
...
const sweetshopAlchemyHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    await activateSweetshopAlchemy(event.current.UserId);
    await message.send({ format: await alchemyFormat(event.current.UserId) });
}
catch (error) {
...
const alchemyContinueHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    await message.send({ format: await alchemyFormat(event.current.UserId) });
}
catch (error) {
    await message.send({ format: messageFormat('无法炼金', error instanceof Error ? error.message : '请稍后重试。') });
} };
const alchemyMaterialPageHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    await message.send({ format: await alchemyFormat(event.current.UserId, Number(route.param('page')), String(route.param('keyword') ?? '')) });
}
catch (error) {
    await message.send({ format: messageFormat('无法查看材料', error instanceof Error ? error.message : '请稍后重试。') });
} };
const alchemyMaterialSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    await message.send({ format: await alchemyFormat(event.current.UserId, 1, String(route.param('keyword'))) });
}
catch (error) {
    await message.send({ format: messageFormat('无法搜索材料', error instanceof Error ? error.message : '请稍后重试。') });
} };
const alchemySelectHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    const label = String(route.param('role'));
    const role = label === '主材' ? 'main' : label === '辅材' ? 'auxiliary' : label === '反应剂' ? 'reagent' : null;
    if (!role)
...
const alchemyFormulaListHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
...
const alchemyFormulaPageHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const alchemyFormulaSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const saveAlchemyFormulaHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    const formula = await saveAlchemyFormula(event.current.UserId);
...
const renameAlchemyFormulaHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const loadAlchemyFormulaHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const deleteAlchemyFormulaHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const alchemyAddHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    const label = String(route.param('role'));
    const role = label === '主材' ? 'main' : label === '辅材' ? 'auxiliary' : label === '反应剂' ? 'reagent' : null;
    if (!role)
        throw new Error('请选择主材、辅材或反应剂。');
...
const alchemyRemoveHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
...
const sendAlchemyResult = async (message, stages) => {
    await message.send({ format: messageFormat('炼金结果', stages.join('\n\n')) });
};
const alchemyExecuteHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    const result = await executeAlchemy(event.current.UserId);
    if (result.needsConfirmation) {
        const markdown = Format.createMarkdown().addTitle('炼金确认').addNewline().addNewline().addText('⚠️ 素材等级能量相差过大\n继续炼金风险极高，是否继续？');
        const buttons = Format.createButtonGroup().addRow().addButton('确认炼金', '/确认炼金', { type: 'command', autoEnter: true, style: 'blue' }).addButton('取消', '/取消炼金', { type: 'command', autoEnter: true });
        await message.send({ format: Format.create().addMarkdown(markdown).addButtonGroup(buttons) });
        return;
    }
    await sendAlchemyResult(message, result.stages ?? []);
    await message.send({ format: await alchemyFormat(event.current.UserId) });
}
catch (error) {
    await message.send({ format: messageFormat('无法炼金', error instanceof Error ? error.message : '请稍后重试。') });
} };
const alchemyConfirmHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    const result = await executeAlchemy(event.current.UserId, true);
    await sendAlchemyResult(message, result.stages ?? []);
    await message.send({ format: await alchemyFormat(event.current.UserId) });
}
catch (error) {
...
const alchemyCancelHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    await cancelAlchemyConfirmation(event.current.UserId);
    await message.send({ format: messageFormat('已取消炼金', '材料尚未消耗。') });
    await message.send({ format: await alchemyFormat(event.current.UserId) });
}
catch (error) {
    await message.send({ format: messageFormat('无法取消炼金', error instanceof Error ? error.message : '请稍后重试。') });
} };
const alchemistTradeHandler = async () => { const [event] = useEvent(); const [message] = useMessage(); try {
    await requireAlchemist(event.current.UserId);
...
const alchemistSellPageHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    await requireAlchemist(event.current.UserId);
...
const alchemistSellSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    await requireAlchemist(event.current.UserId);
...
const alchemistSellItemHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useMessage(); try {
    await requireAlchemist(event.current.UserId);
    const requested = String(route.param('quantity') ?? '').trim();
