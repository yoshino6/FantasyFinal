...
import { dynamicNpcVoiceAnchor } from '../game/dynamic-npc-dialogue.service.js';
import { weatherForPlayer, currentDynamicEncounter, startPatrolEncounter, resolveDynamicEncounter, nearbyDynamicScenes, joinDynamicScene, contributeDynamicScene, patrolEncounterPreview } from '../game/world-dynamics.service.js';
...
const dynamicEncounterHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const dynamicEncounterChoiceHandler = async () => {
    ...
    try {
        ...
        if (result.patrol) {
            if (!result.completed) {
                await message.send({ format: encounterFormat({ ...result, opening: `${result.choice.text}\n\n${result.opening}` }) });
                return;
            }
...
const patrolMeetingFormat = async (qqUserId, npcCode, name, continuingChat = false, affinity = 0) => {
    const preview = await patrolEncounterPreview(qqUserId, npcCode);
    if (!preview)
        return null;
    ...
    if (voice)
        markdown.addNewline().addNewline().addBlockquote(`“${voice.catchphrase}。${affinity >= 200 ? voice.trust : affinity >= 50 ? voice.warmth : voice.caution}。”`);
...
const patrolEncounterHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await message.send({ format: encounterFormat(await startPatrolEncounter(event.current.UserId, String(route.param('code')), Number(route.param('revision')))) });
    }
    catch (error) {
        await fail(message, error, '巡游奇遇');
    }
};

export { contributeDynamicSceneHandler, dynamicEncounterChoiceHandler, dynamicEncounterHandler, encounterFormat, joinDynamicSceneHandler, nearbyDynamicSceneHandler, patrolEncounterHandler, patrolMeetingFormat, weatherHandler };
