...
import { continueGirlGratitude, girlGratitudeStage, teleportToWorldTree, returnToBainaTown, receiveGirlGratitudeGift, startGirlGratitude } from '../game/girl-gratitude.service.js';
...
const girlGratitudeContinueHandler = async () => {
    ...
    try {
        ...
        if (result.exchange)
            buttons.addButton('前往 万叶联市', '/前往 4 2 0', { type: 'command', autoEnter: false, style: 'blue' });
...
const worldExchangeFormat = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    if (await (await import('../game/floating-leaf.service.js')).floatingLeafOrigin(event.current.UserId)) {
