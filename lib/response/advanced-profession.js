...
const baseName = (code) => ({ warrior: '战士', mage: '法师', rogue: '盗贼', priest: '牧师', archer: '射手' }[code] ?? code);
...
const advancedProfessionDetailFormat = async (qqUserId, mentorCode) => {
    const view = await advancedProfessionView(qqUserId, mentorCode);
    const { profession, active, activeQuest, completedCode, retrainRemainingSeconds, materialQuantity } = view;
