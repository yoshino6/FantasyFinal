import { useEvent, logger, Format, useRoute } from 'alemonjs';
import { useGameMessage } from '../game/use-game-message.js';
import { travelStatus, completeTravel, resourceMiningStatus, battleStatus, nearbyPoints, blockedDungeonDirections, movementProfile, setMapLandmarksVisible, setNearbyPlayersVisible, startRest, resumeAction } from '../game/adventure.service.js';
import { sendWithTextFallback, messageFormat } from '../game/message.js';
...
const areaText = (character, verb) => Number(character.adventurer_registered)
...
const outsidePanel = (title, location, speed, range, x, y, description, points, resting = false, landmarks = [], speaker = '', speedLimit = speed, perceptionObscured = false, showLandmarks = true, showPlayers = true, mapUnlocked = false, activityStatus, emphasizeSpeaker = false) => {
    ...
    if (speaker) {
        if (emphasizeSpeaker)
            for (const line of speaker.split('\n'))
                markdown.addText('> ').addBold(line).addNewline();
        else
            markdown.addText(speaker).addNewline();
        markdown.addNewline();
    }
    markdown.addBlockquote(description).addNewline().addNewline();
    if (resting)
        markdown.addText(activityStatus === 'detained' ? '状态：关押中' : activityStatus === 'unconscious' ? '状态：昏迷中' : '状态：休息中（每秒恢复 1% 生命与魔力）').addNewline();
    markdown.addText(`移动速度：（${speed}/${speedLimit}）`).addButton('[调整移速]', { data: '/调整移速 ', autoEnter: false }).addNewline().addText(`感知范围：${range}`);
    if (perceptionObscured)
        markdown.addNewline().addBlockquote('压抑的黑暗干扰了你的感知').addNewline();
    ...
    if (mapUnlocked || landmarks.length) {
        ...
        if (showLandmarks)
            for (const landmark of entranceLandmarks) {
                const seconds = Math.max(1, Math.ceil((Math.abs(landmark.x - x) + Math.abs(landmark.y - y)) / speedLimit));
                markdown.addText('> ').addButton(markerName(landmark), { data: `/前往 ${landmark.x} ${landmark.y} ${landmark.z}`, autoEnter: false }).addText(`（${landmark.x}, ${landmark.y}, ${landmark.z}）[预计${durationText(seconds)}]`).addNewline();
            }
    }
    ...
    if (!visiblePoints.length)
        ...
        for (const point of visiblePoints) {
            ...
            if (point.type === '域民')
                markdown.addText(`（${point.x}, ${point.y}, ${point.z}）`);
            if (point.movementState)
                markdown.addText(` [${point.movementState}]`);
            if (point.observedOnly)
                markdown.addText(' [仅观察]');
            else {
                if (point.type === '怪物' && point.code)
                    markdown.addText(' ').addButton('[交互]', { data: `/怪物交互 ${point.code}`, autoEnter: false });
                else if (point.distance === 0 && point.interaction?.type === '建筑')
                    markdown.addText(' ').addButton('[进入]', { data: `/建筑进入 ${point.interaction.id}`, autoEnter: false });
                else if (point.type === '玩家' && point.distance === 0 && point.code)
                    markdown.addText(' ').addButton('[互动]', { data: `/玩家互动 ${point.code}`, autoEnter: false });
                else if (point.type === '玩家' && point.distance > 0)
                    markdown.addText(' ').addButton('[前往]', { data: `/前往 ${point.x} ${point.y} ${point.z}`, autoEnter: false });
                else if (point.type !== '玩家' && point.distance === 0 && point.interaction)
                    markdown.addText(' ').addButton('[互动]', { data: `/坐标互动 ${point.interaction.type} ${point.interaction.id}`, autoEnter: false });
                else if (point.type !== '玩家' && point.distance > 0 && speedLimit >= point.distance)
                    markdown.addText(' ').addButton('[前往]', { data: `/前往 ${point.x} ${point.y} ${point.z}`, autoEnter: false });
                if (point.type === '怪物' && point.code)
                    markdown.addText(' ').addButton('[攻击]', { data: `/怪物攻击 ${point.code}`, autoEnter: false });
                if (point.isTrialTarget && point.code)
                    markdown.addText(' ').addButton('[试炼]', { data: `/怪物攻击 ${point.code}`, autoEnter: false });
                if (point.type === '玩家' && point.code) {
                    if (point.pvpAvailable)
                        markdown.addText(' ').addButton('[攻击]', { data: `/玩家攻击 ${point.code}`, autoEnter: false });
                    else
                        markdown.addText(' [好友]');
                }
            }
            if (point.trackable && point.type === '怪物' && point.code)
                markdown.addText(' ').addButton('[追迹]', { data: `/追迹 ${point.code}`, autoEnter: false });
            markdown.addNewline();
        }
    }
    return Format.create().addMarkdown(markdown);
};
const panelButtons = (resting = false, _autoBattleEnabled = false, blockedDirections = []) => {
    ...
    const moveButton = (direction) => ({ label: blocked(direction) ? '石墙' : direction, command: blocked(direction) ? '/面板' : `/移动 ${direction}`, style: blocked(direction) || resting ? undefined : 'blue' });
...
const limitedViewButtons = (buttons) => buttons.addRow()
...
const miningPanel = (mining) => Format.create()
...
var panel = async () => {
    ...
    try {
        ...
        if (travel)
            await completeTravel(event.current.UserId);
        ...
        try {
            ...
            if (!(error instanceof Error) || !error.message.includes('当前不在战斗中'))
                throw error;
...
const showRestPanel = async (message, qqUserId, text) => {
    ...
    const [nearby, movement, trace] = await Promise.all([nearbyPoints(qqUserId), movementProfile(qqUserId), omniscientTraces(qqUserId)]);
    const resting = nearby.character.activity_status !== 'active';
...
const restHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const resumeActionHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
export { currentLocationText, panel as default, mapLandmarkVisibilityHandler, movedLocationText, nearbyPlayersVisibilityHandler, outsidePanel, panelButtons, restHandler, resumeActionHandler };
