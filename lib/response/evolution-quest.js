...
import { librarySkillCatalog, discoverLibrarySkill } from '../game/library-skills.service.js';
...
const librarySkillsHandler = async () => {
    ...
    try {
        ...
        for (const skill of skills.slice((page - 1) * 5, page * 5)) {
            ...
            if (!skill.learned && !skill.discovered)
                markdown.addButton('[免费领悟]', { data: `/图书馆领悟 ${skill.id}`, autoEnter: false }).addNewline();
            markdown.addNewline();
        }
        ...
        await message.send({ format: messageFormat('馆藏暂不可用', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const librarySkillDiscoverHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
...
export { contemplateEvolutionSeedHandler, evolutionGuildHandler, evolutionInvestigationHandler, gaStudyHandler, librarySkillDiscoverHandler, librarySkillsHandler, worldLibraryAreaHandler, worldLibraryFormat };
