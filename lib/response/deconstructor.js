...
const oddWorkshopFormat = async (qqUserId, dialogue) => {
    ...
    const buttons = Format.createButtonGroup()
        ...
        .addRow().addButton('分解', '/店铺分解', { type: 'command', autoEnter: true, style: 'blue' }).addButton('构造', '/店铺构造', { type: 'command', autoEnter: true, style: 'blue' })
        .addRow().addButton('离开', `/建筑离开 ${workshopCode}`, { type: 'command', autoEnter: true });
    if (dungeonSecret.stage === 2)
        buttons.addRow().addButton('关于 地下的秘密', '/异工坊 地下的秘密', { type: 'command', autoEnter: true, style: 'blue' });
    if (mainQuest.title === '【主线·失踪的少女】' && mainQuest.description.startsWith('公会的人手'))
        buttons.addRow().addButton('询问 天位制裁仪', '/唯薇安 天位制裁仪', { type: 'command', autoEnter: true, style: 'blue' });
    if (mainQuest.title === '【主线·失踪的少女】' && mainQuest.description.startsWith('唯薇安确认'))
        buttons.addRow().addButton('购买 天位制裁仪（200铜币）', '/购买天位制裁仪', { type: 'command', autoEnter: true, style: 'blue' });
...
const deconstructionFormat = async (qqUserId, category = '材料', page = 1, keyword = '') => {
    ...
    if (!entries.length)
        markdown.addBlockquote('当前分类没有可分解的物品。').addNewline();
    for (const [index, item] of entries.entries())
        markdown.addBlockquote(`${numberMark.charAt(index)}【${item.category}】${item.name}×${item.quantity} `).addButton('[分解]', { data: `/分解物品 ${item.id} `, autoEnter: false }).addText('+数量').addNewline();
