...
import { addNpcAffinity, nearbyPoints, grantNpcAffinity, requireNpcAtCurrentPosition } from '../game/adventure.service.js';
import { claimVivianCourseBlueprints, acceptDeconstructorQuest, claimDeconstructorQuest, constructItem, deconstructItems, deconstructorQuest, deconstructorProgress, constructionRecipesFor, deconstructionItems } from '../game/deconstructor.service.js';
import { sellOddWorkshopItem, oddWorkshopSellCatalog } from '../game/oddworkshop-shop.service.js';
...
const requireWorkshop = (qqUserId) => requireNpcAtCurrentPosition(qqUserId, workshopCode);
const oddWorkshopFormat = async (qqUserId, dialogue) => {
    ...
    if (nearby.npcDetailsUnlocked)
        markdown.addText(' ').addButton('[详情]', { data: '/域民详情 oddworkshop', autoEnter: false });
    ...
    if (dungeonSecret.stage === 2)
        buttons.addRow().addButton('关于 地下的秘密', '/异工坊 地下的秘密', { type: 'command', autoEnter: false, style: 'blue' });
    if (mainQuest.title === '【主线·失踪的少女】' && mainQuest.description.startsWith('公会的人手'))
        buttons.addRow().addButton('询问 天位制裁仪', '/唯薇安 天位制裁仪', { type: 'command', autoEnter: false, style: 'blue' });
    if (mainQuest.title === '【主线·失踪的少女】' && mainQuest.description.startsWith('唯薇安确认'))
        buttons.addRow().addButton('购买 天位制裁仪（200铜币）', '/购买天位制裁仪', { type: 'command', autoEnter: false, style: 'blue' });
    await appendHiddenQuestButton(buttons, qqUserId, workshopCode);
    return Format.create().addMarkdown(markdown).addButtonGroup(buttons);
};
const oddWorkshopHandler = async () => {
    ...
    try {
        ...
        await message.send({ format: messageFormat('无法进入异工坊', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const oddWorkshopChatHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireWorkshop(event.current.UserId);
        ...
        if (course.affinityAwarded.length)
            markdown.addNewline().addNewline().addText(`唯薇安另交给你：${course.affinityAwarded.map(name => `【${name}图纸】`).join('、')}`);
        if (detailsUnlocked)
...
const oddWorkshopTradeHandler = (action) => async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireWorkshop(event.current.UserId);
        if (action === 'sell') {
            await message.send({ format: await oddWorkshopSellFormat(event.current.UserId) });
            return;
        }
        if (action === 'buy') {
            const { oddWorkshopBuyFormat } = await import('./dungeon-quest.js');
            await message.send({ format: await oddWorkshopBuyFormat(event.current.UserId) });
            return;
        }
    }
    catch (error) {
        await message.send({ format: messageFormat(action === 'buy' ? '无法购买' : '无法出售', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const oddWorkshopSellButtons = (page, totalPages, keyword = '') => Format.createButtonGroup()
...
const oddWorkshopSellFormat = async (qqUserId, page = 1, keyword = '') => {
    ...
    if (!shop.items.length)
        markdown.addText('背包中没有唯薇安会收购的物品。');
...
const oddWorkshopSellPageHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireWorkshop(event.current.UserId);
    await message.send({ format: await oddWorkshopSellFormat(event.current.UserId, Number(route.param('page') ?? 1), String(route.param('keyword') ?? '')) });
}
catch (error) {
    await message.send({ format: messageFormat('出售列表不可用', error instanceof Error ? error.message : '请稍后重试。') });
} };
const oddWorkshopSellSearchHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireWorkshop(event.current.UserId);
    await message.send({ format: await oddWorkshopSellFormat(event.current.UserId, 1, String(route.param('keyword'))) });
}
catch (error) {
    await message.send({ format: messageFormat('搜索失败', error instanceof Error ? error.message : '请稍后重试。') });
} };
const oddWorkshopSellItemHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await requireWorkshop(event.current.UserId);
    const requested = String(route.param('quantity') ?? '').trim();
    const result = await sellOddWorkshopItem(event.current.UserId, Number(route.param('id')), requested ? Number(requested) : 1);
    await addNpcAffinity(event.current.UserId, workshopCode, 'sell');
...
const deconstructorAboutHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireWorkshop(event.current.UserId);
...
const deconstructorProfessionSelectHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireWorkshop(event.current.UserId);
        const quest = await deconstructorQuest(event.current.UserId);
        if (quest.status !== 'none')
            throw new Error('你已经接取或完成了解构师任务。');
...
const acceptDeconstructorQuestHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireWorkshop(event.current.UserId);
        await acceptDeconstructorQuest(event.current.UserId);
        const markdown = Format.createMarkdown().addTitle('接受任务').addNewline().addNewline().addText('已接受【副职业·解构师入门】\n收集：兽核×1\n可随时通过 ').addButton('/任务', { data: '/任务', autoEnter: false }).addText(' 查看进度。');
...
const claimDeconstructorQuestHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await requireWorkshop(event.current.UserId);
...
const deconstructorProfessionFormat = async (qqUserId) => {
...
const numberMark = '①②③④⑤⑥⑦⑧⑨⑩';
const deconstructionFormat = async (qqUserId, category = '材料', page = 1, keyword = '') => {
    ...
    if (!entries.length)
        markdown.addBlockquote('当前分类没有持有的物品。').addNewline();
    for (const [index, item] of entries.entries()) {
        markdown.addBlockquote(`${numberMark.charAt(index)}【${item.category}】${item.name}×${item.quantity} `);
        if (item.canDeconstruct)
            markdown.addButton('[分解]', { data: `/分解物品 ${item.id} `, autoEnter: false }).addText('+数量');
        else
            markdown.addText(`状态：${item.blockReason}`);
        markdown.addNewline();
        if (item.preview)
            markdown.addBlockquote(`每份可能获得：${item.preview}`).addNewline();
        if (item.notice)
            markdown.addBlockquote(item.notice).addNewline();
    }
    ...
    const pageCommand = (target) => `/分解页 ${category} ${target}${keyword ? ` ${keyword}` : ''}`;
    const buttons = Format.createButtonGroup()
...
const constructionFormat = async (qqUserId, category = '基材', page = 1, keyword = '') => {
    ...
    if (progress.level >= 4)
        markdown.addNewline().addButton('[灵枢素体]', { data: '/机巧 构造', autoEnter: false }).addNewline();
    if (!entries.length)
        markdown.addBlockquote('当前分类没有已掌握图纸的构造配方。').addNewline();
    for (const [index, recipe] of entries.entries()) {
        markdown.addText(`${numberMark.charAt(index)}【${recipe.name}】 `);
        if (recipe.unlocked && recipe.codexId)
            markdown.addButton('[详情]', { data: `/物品图鉴 ${recipe.codexId}`, autoEnter: false });
        ...
        for (const ingredient of recipe.ingredients) {
            if (ingredient.codexId)
                markdown.addButton(`【${ingredient.name}】`, { data: `/物品图鉴 ${ingredient.codexId}`, autoEnter: false });
            else
                markdown.addText(`【${ingredient.name}】`);
            markdown.addText(` ${ingredient.owned}/${ingredient.quantity} `);
        }
        markdown.addNewline();
        if (recipe.blueprintName)
            markdown.addText(`图纸：【${recipe.blueprintName}】（已持有）`).addNewline();
        else
            markdown.addText('构造链：已随异械图纸解锁（基材与构件无需图纸）').addNewline();
    ...
    const previous = Math.max(1, currentPage - 1);
    const next = Math.min(totalPages, currentPage + 1);
    const pageCommand = (target) => `/构造页 ${category} ${target}${keyword ? ` ${keyword}` : ''}`;
    const buttons = Format.createButtonGroup()
...
const deconstructorFeatureHandler = (feature) => async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        const quest = await deconstructorQuest(event.current.UserId);
        if (!currentSecondaryShop() && quest.status !== 'claimed')
            throw new Error('成为解构师后才能使用这项能力。');
        if (feature === '分解') {
        ...
        await message.send({ format: messageFormat(`无法${feature}`, error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const deconstructionPageHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await message.send({ format: await deconstructionFormat(event.current.UserId, String(route.param('category')), Number(route.param('page')), String(route.param('keyword') ?? '')) });
    }
    catch (error) {
        await message.send({ format: messageFormat('无法查看分解列表', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const deconstructionSearchHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await message.send({ format: await deconstructionFormat(event.current.UserId, String(route.param('category')), 1, String(route.param('keyword'))) });
    }
    catch (error) {
        await message.send({ format: messageFormat('无法搜索分解物品', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const constructionPageHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await message.send({ format: await constructionFormat(event.current.UserId, String(route.param('category')), Number(route.param('page')), String(route.param('keyword') ?? '')) });
    }
    catch (error) {
        await message.send({ format: messageFormat('无法查看构造配方', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const constructionSearchHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await message.send({ format: await constructionFormat(event.current.UserId, String(route.param('category')), 1, String(route.param('keyword'))) });
    }
    catch (error) {
        await message.send({ format: messageFormat('无法搜索构造配方', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const deconstructItemHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        ...
        await message.send({ format: messageFormat('无法分解', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const constructItemHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        ...
        await message.send({ format: messageFormat('无法构造', error instanceof Error ? error.message : '请稍后重试。') });
    }
};

export { acceptDeconstructorQuestHandler, claimDeconstructorQuestHandler, constructItemHandler, constructionPageHandler, constructionSearchHandler, deconstructItemHandler, deconstructionPageHandler, deconstructionSearchHandler, deconstructorAboutHandler, deconstructorFeatureHandler, deconstructorProfessionFormat, deconstructorProfessionSelectHandler, oddWorkshopChatHandler, oddWorkshopFormat, oddWorkshopHandler, oddWorkshopSellItemHandler, oddWorkshopSellPageHandler, oddWorkshopSellSearchHandler, oddWorkshopTradeHandler };
