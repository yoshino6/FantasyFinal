...
import { friendList, oathStatus } from '../game/social.service.js';

const churchCode = 'saint_church';
const requireChurch = (qqUserId) => requireNpcAtCurrentPosition(qqUserId, churchCode);
const churchFormat = async (qqUserId, dialogue) => {
    ...
    if (nearby.npcDetailsUnlocked)
        markdown.addText(' ').addButton('[详情]', { data: '/域民详情 saint_church', autoEnter: false });
    markdown.addNewline().addNewline().addBlockquote(scene);
    if (oath?.status === 'ceremony_pending')
        markdown.addNewline().addNewline().addText(`【${oath.name}】已接受星光申请。请与对方一同站在祭台前，打开“星誓”开始正式仪式。`);
    if (oathCandidates.length) {
        markdown.addNewline().addNewline().addText('同行的星光在祭台前轻轻回应：').addNewline();
        for (const candidate of oathCandidates)
            markdown.addText(`【${candidate.name}】好感：${candidate.affinity}/500`).addNewline();
    }
    const buttons = Format.createButtonGroup()
        .addRow().addButton('星誓', '/星誓', { type: 'command', autoEnter: false }).addButton('祈福', '/祈福', { type: 'command', autoEnter: false });
    for (const candidate of oathCandidates)
        buttons.addRow().addButton(`发起星誓·${candidate.name}`, `/发起星誓 ${candidate.game_id}`, { type: 'command', autoEnter: true, style: 'blue' });
    buttons
...
const churchHandler = async () => {
    ...
    try {
        ...
        await message.send({ format: messageFormat('无法进入圣恩教堂', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const churchChatHandler = async () => {
    const [event] = useEvent();
    const [message] = useMessage();
    try {
        await requireChurch(event.current.UserId);
...
export { churchChatHandler, churchFormat, churchHandler as default };
