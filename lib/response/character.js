...
import { evolutionLabAvailable } from '../game/evolution.service.js';
import { pendingHeartQuestionCount } from '../game/heart-question.service.js';

const elementOrder = ['水', '火', '木', '土', '风', '冰', '雷', '光', '暗'];
const numberText = (value) => Number.isInteger(value) ? String(value) : value.toFixed(1);
const extraLabels = {
...
const elementEntries = (character, key, start, end) => elementOrder.slice(start, end).map(element => [element, String(Number(character[key][element] ?? 0))]);
const appendDetails = (markdown, character) => {
    ...
    const extraAttributes = Object.entries(character.extraAttributes).flatMap(([key, raw]) => {
        const label = extraLabels[key];
        const value = Number(raw);
        if (!label || !value)
            return [];
        const actual = label.inverse ? -value : value;
        return [[label.name, `${actual > 0 ? '+' : ''}${numberText(actual)}${label.unit}`]];
    });
    if (extraAttributes.length) {
        markdown.addNewline().addText('额外').addNewline();
        for (let index = 0; index < extraAttributes.length; index += 3)
            appendQuotedAttributes(markdown, extraAttributes.slice(index, index + 3));
    }
...
const overviewFormat = (character, evolutionUnlocked, pendingHeartQuestions) => {
    ...
    if (pendingHeartQuestions)
        markdown.addButton('[窥尘问心]', { data: '/窥尘问心', autoEnter: false });
    ...
    if (character.activeBuffs.length)
        character.activeBuffs.forEach((buff, index) => markdown.addBlockquote(`${index + 1}. ${buff.replace(/剩余(\d+)秒/, (_all, seconds) => `剩余${durationText(Number(seconds))}`)}`).addNewline());
    else
    ...
    if (evolutionUnlocked)
        buttons.addButton('进化面板', '/进化面板', { type: 'command', autoEnter: true, style: 'blue' });
    return Format.create().addMarkdown(markdown).addButtonGroup(buttons);
};
const detailFormat = (character) => {
...
const loadCharacter = async (message, qqUserId, detail) => {
    try {
        const character = await getCharacter(qqUserId);
        if (!character) {
            await message.send({ format: messageFormat('尚未注册', '发送“注册”开始异世界之旅。') });
            return;
        }
...
const characterDetailHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    await loadCharacter(message, event.current.UserId, true);
};

export { characterDetailHandler, character as default };
