...
import { leafRouteAction, leafRouteView } from '../game/leaf-route.service.js';
import { messageFormat } from '../game/message.js';
import { leafRouteScenes } from '../game/leaf-route.config.js';

const leafRouteFormat = async (user) => {
    ...
    if (v.claimed)
        ...
        const steps = { 3: ['第一处风标', '第二处风标', '第三处风标'], 5: ['检查承重榫', '扶正风叶', '对齐铃片'], 7: ['校准青色回音', '校准黄色回音', '校准白色回音'], 10: ['排除铃片故障', '理顺测风绳', '确认登舱'] };
    ...
    if (v.started)
        md.addText('　').addButton('回顾航务记录', { data: '/航路回顾 1', autoEnter: false });
...
var leafRoute = async () => {
    ...
    try {
        ...
        await m.send({ format: messageFormat('航路任务', error.message) });
    }
};
const review = async () => {
    const [e] = useEvent();
    ...
    try {
        ...
        if (!v.started || !Number.isInteger(stage) || stage < 1 || stage > v.stage)
            throw Error('只能回顾已经抵达的剧情幕。');
        ...
        for (const paragraph of scene.text.split(/\r?\n\r?\n/))
            md.addBlockquote(paragraph.replace(/\r?\n/g, '\n> ')).addNewline().addNewline();
        if (stage > 1)
            md.addButton('上一幕', { data: '/航路回顾 ' + (stage - 1), autoEnter: false }).addText('　');
        if (stage < v.stage)
            md.addButton('下一幕', { data: '/航路回顾 ' + (stage + 1), autoEnter: false }).addText('　');
        md.addButton('当前进度', { data: '/浮叶航路', autoEnter: false });
        await m.send({ format: Format.create().addMarkdown(md) });
    }
    catch (error) {
        await m.send({ format: messageFormat('航务回顾', error.message) });
    }
};
const advance = async () => {
    const [e] = useEvent();
...
const anchor = async () => {
    ...
    const [m] = useGameMessage();
    try {
        const result = await (await import('../game/adventure.service.js')).combatAction(e.current.UserId, 'anchor');
        await (await import('./adventure.js')).sendCombatResult(m, e.current.UserId, result);
    }
    catch (error) {
        await m.send({ format: messageFormat('重新锚定', error.message) });
    }
};

export { advance, anchor, leafRoute as default, leafRouteFormat, review };
