...
import { monsterDetail as monsterDetail$1 } from '../game/adventure.service.js';
import { messageFormat } from '../game/message.js';

var monsterDetail = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useMessage();
    try {
        const monster = await monsterDetail$1(event.current.UserId, Number(route.param('id')));
        const attr = monster.attributes;
        ...
        logger.warn({ err: error }, 'monster detail failed');
        await message.send({ format: messageFormat('怪物鉴识', error instanceof Error ? error.message : '请稍后重试。') });
    }
};

export { monsterDetail as default };
