...
import { worldSiteKnock, useWorldSite, acceptWorldSiteCommissionFromAttendant, claimWorldSiteCommission, submitWorldSiteCommission, worldSiteView } from '../game/world-dynamics.service.js';
...
const submitWorldSiteCommissionHandler = async () => {
    ...
    try {
        await message.send({ format: messageFormat('委托交接', await submitWorldSiteCommission(event.current.UserId, Number(route.param('id')))).addButtonGroup(Format.createButtonGroup().addRow().addButton('任务栏', '/任务', { type: 'command', autoEnter: true })) });
    }
    catch (error) {
        await fail(message, error);
    }
};
const actionCode = { 委托: 'commission', 预报: 'forecast', 交换: 'exchange', 线索: 'clues', 庇护: 'shelter', commission: 'commission', forecast: 'forecast', exchange: 'exchange', clues: 'clues', shelter: 'shelter' };
const actionCommand = { commission: '委托', forecast: '预报', exchange: '交换', clues: '线索', shelter: '庇护' };
const worldSiteFormat = async (qqUserId, siteCode, notices = []) => {
    ...
    if (site.attendant) {
        ...
        if (nearby.npcDetailsUnlocked)
            markdown.addText(' ').addButton('[详情]', { data: `/域民详情 ${site.attendant.code}`, autoEnter: false });
    ...
    if (notices.length)
        markdown.addNewline().addNewline().addBlockquote(notices.join('\n'));
    ...
    if (site.attendant) {
        ...
        if (site.capability !== 'commission')
            buttons.addButton(site.actionLabel, `/站点行动 ${site.code} ${actionCommand[site.capability]}`, { type: 'command', autoEnter: true });
        else
            markdown.addNewline().addNewline().addBlockquote('前台委托需先与值守域民交谈，再由对方亲手交付。');
    }
...
const worldSiteActionHandler = async () => {
    ...
    try {
        ...
        if (!action)
            throw new Error('未识别的站点服务。');
        if (action === 'commission')
            throw new Error('请先与前台常驻域民交谈，再接取委托。');
...
const acceptWorldSiteCommissionHandler = async () => {
    ...
    try {
        ...
        if (!result.commission)
            throw new Error('委托交接内容暂未生成，请稍后再试。');
