...
import { useEvent, useRoute, Format, useMessage, ResultCode, logger } from 'alemonjs';
import { useGameMessage } from '../game/use-game-message.js';
import { automatonCharacter, automatonList, automatonRecipeCatalog, previewAutomatonCraft, confirmAutomatonCraft, confirmAutomatonMutation, automatonEventPage, previewAutomatonMutation } from '../game/automaton.service.js';
import { escapeAutomatonText, automatonInteractionText } from '../game/automaton-dialogue.js';
import { greetAutomaton, quoteForAutomatonMutation, finishAutomatonMutationQuote, acknowledgeAutomatonQuote } from '../game/automaton-dialogue.service.js';
...
import { previewAutomatonComponents, confirmAutomatonComponents } from '../game/automaton-construction.service.js';
import { assertAlchemyCreationUnlocked } from '../game/alchemy-creation-quest.service.js';
import { beginPortraitUpload, portraitScope, cancelPortraitUpload, resetPortrait, portraitReviewStatus } from '../game/automaton-portrait.service.js';
...
const link = (m, title, command) => m.addButton(`[${title}]`, { data: command, autoEnter: false }).addText(' ');
const automatonHandler = async (alchemyMode = false) => {
    ...
    try {
        ...
        let interaction;
        let mutationToken;
        const pageTitles = { 详情: '机巧·详情', 技能: '机巧·技能', 成长: '机巧·成长', 回忆: '机巧·回忆', 选料: '机巧·培养', 选人偶: '机巧·选择人偶', 归档列表: '机巧·休眠归档' };
        ...
        const controls = [];
        const control = (title, command, autoEnter = true) => controls.push({ title, command, autoEnter });
        ...
        const back = (m) => {
            if (alchemyMode)
                return Format.create().addMarkdown(m).addButtonGroup(Format.createButtonGroup().addRow().addButton('返回炼金', '/炼金', { type: 'command', autoEnter: true }));
            ...
            if (!action || action === '列表')
                return controls.length ? Format.create().addMarkdown(m).addButtonGroup(buttons) : Format.create().addMarkdown(m);
            buttons.addRow();
            if (Number.isSafeInteger(id) && id > 0 && !['列表', '归档列表', '选人偶', '构件预览', '构件确认', '配方', '制造确认', '取消', '确认', '构造'].includes(action))
                buttons.addButton('返回详情', `/机巧 详情 ${id}`, { type: 'command', autoEnter: true });
            buttons.addButton('返回机巧', '/机巧', { type: 'command', autoEnter: true });
            return Format.create().addMarkdown(m).addButtonGroup(buttons);
        };
        if (alchemyMode && !['点灵', '育成', '配方', '制造确认', '取消'].includes(action))
            throw new Error('请选择「炼金 点灵」或「炼金 育成」。');
        if (!alchemyMode && (action === '点灵' || action === '育成'))
            throw new Error('入口已迁移，请使用「炼金 ' + action + '」。');
        if (alchemyMode && action === '取消' && args[1] !== 'automaton_craft')
            throw new Error('只能取消本次炼金点灵或育成。');
        if (alchemyMode && action !== '取消')
        ...
        if (!action || action === '列表' || action === '归档列表') {
            ...
            if (!visible.length)
                m.addText(action === '归档列表' ? '暂无休眠归档的机巧。' : '尚未拥有机巧。可以向其他玩家购买未认主人偶，再进行认主和培养。');
            if (pages > 1)
                paginate(page, pages, p => `/机巧 ${action === '归档列表' ? '归档列表' : '列表'} ${p}`);
            if (action !== '归档列表' && items.some(p => p.state.archived))
                control('休眠归档', '/机巧 归档列表');
            format = back(m);
        }
        else if (action === '选人偶') {
            const { items } = await automatonList(user);
            ...
            if (!feed)
                throw new Error('请选择有效原液。');
            m.addText(`使用${feed.name}，请选择已认主机巧：`).addNewline();
            for (const { row, state } of items.filter(p => p.row.owner_id))
                control(state.name, `/机巧 培养 ${row.id} ${feed.code} 1`, false);
            format = back(m);
        }
        else if (action === '选料') {
            const { items } = await automatonList(user), item = items.find(p => Number(p.row.id) === id && p.row.owner_id);
            if (!item)
                throw new Error('未找到已认主机巧。');
                ...
                if (action === '构造') {
                    ...
                    for (const role of ['主材', '辅材', '催化剂']) {
                        ...
                        for (const part of ingredients)
                            m.addText(`> ${part.name}（${part.owned}/${part.quantity}）`).addNewline();
                    }
                    m.addNewline();
                }
                if (action === '构造') {
            ...
            if (result.missing.length)
                m.addText('仍缺少：' + result.missing.map(i => `${i.name}×${i.count}`).join('、')).addNewline();
            ...
            if (result.recipe.profession === 'alchemist') {
                ...
                for (const i of result.ingredients)
                    confirmation.addBlockquote(`${i.role}：${i.name}（${result.owned[i.code]}/${i.quantity * result.batches}）`).addNewline();
                ...
                for (const i of result.ingredients)
                    m.addText(`${i.role}：${i.name}×${i.quantity * result.batches}`).addNewline();
            ...
            if (result.code === 'automaton_body') {
                ...
                const consumed = new Map(), outputs = new Map();
                for (const batch of record.batches) {
                    for (const item of batch.consumed ?? record.snapshot.ingredients) {
                        ...
                        consumed.set(item.code, { name: item.name, quantity: (previous?.quantity ?? 0) + item.quantity });
                    }
                    for (const item of batch.outputs) {
                        ...
                        outputs.set(code, { name: code === 'automaton' ? '机巧人偶·未认主' : item.name, quantity: (previous?.quantity ?? 0) + item.quantity });
                    }
                }
                ...
                if (outputs.has('automaton'))
                    resultMarkdown.addNewline().addText('可通过指令 ')
                        .addButton('/机巧', { data: '/机巧', autoEnter: false }).addText(' 查看');
                if (record.batches.length > 1)
                    resultMarkdown.addNewline().addText(`本次 ${record.batches.length} 批，成功 ${successes}，失败 ${record.batches.length - successes}。`);
            ...
            if (!['automaton_craft', 'automaton_mutate', 'automaton_components'].includes(args[1]))
            ...
            if (!getAdminWebConfig().enabled)
                throw new Error('机巧形象人工审核暂未开放。');
            ...
            if (!event.current.IsPrivate)
                m.addText('请在当前群发送图片；群内未开启免@接收时，请在私聊重新点击“更换形象”后上传。').addNewline();
            ...
            for (const key of result.pendingKeys)
                await cleanupPortrait(key);
            m.addText('已恢复默认形象。');
            format = back(m);
        }
        else if (action === '详情' || action === '技能' || action === '成长' || action === '回忆') {
            const data = await automatonList(user), item = data.items.find(p => Number(p.row.id) === id);
            if (!item)
                throw new Error('未找到你的机巧。');
            ...
            if (action === '详情' && row.owner_id)
                link(m, '改名', `/机巧 命名 ${id} ${state.name}`);
            m.addNewline().addNewline();
            if (action === '详情') {
                ...
                for (let index = 0; index < labels.length; index += 2)
                    m.addText(`${labels[index]} ${Math.floor(vitals.stats[index])}${index + 1 < labels.length ? `｜${labels[index + 1]} ${Math.floor(vitals.stats[index + 1])}` : ''}`).addNewline();
                if (item.battle)
                    m.addNewline().addText(`本场有效等级 ${vitals.level}｜同步 ${item.battle.sync}/100｜${item.battle.exited ? '已退出本场战斗' : vitals.hp ? '战斗中' : '停机'}`).addNewline();
                if (row.recover_at)
                    m.addText(`恢复时间：${new Date(row.recover_at).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })}`).addNewline();
                if (review?.status === 'pending')
                    m.addNewline().addText('新形象：等待人工审核，审核通过后生效。').addNewline();
                if (review?.status === 'rejected')
                    m.addNewline().addText('形象审核未通过：' + escapeAutomatonText(review.reason)).addNewline();
                const buttons = Format.createButtonGroup().addRow();
                for (const title of ['技能', '成长', '回忆'])
                    buttons.addButton(title, `/机巧 ${title} ${id}`, { type: 'command', autoEnter: true, style: 'blue' });
                if (row.owner_id)
                    buttons.addRow().addButton('培养', `/机巧 选料 ${id}`, { type: 'command', autoEnter: true }).addButton('维修', `/机巧 维修 ${id}`, { type: 'command', autoEnter: true });
                else
                    buttons.addRow().addButton('认主', `/机巧 认主 ${id}`, { type: 'command', autoEnter: true });
                if (row.owner_id) {
                    buttons.addRow().addButton('更换形象', `/机巧 更换形象 ${id}`, { type: 'command', autoEnter: true });
                    if (state.portrait || review?.status === 'pending')
                        buttons.addButton('恢复默认形象', `/机巧 恢复形象 ${id}`, { type: 'command', autoEnter: true });
                }
                ...
                if (row.owner_id) {
                    control('修改装配', `/机巧 装配 ${id} ${state.equipped.join(' ')}`, false);
                }
                ...
                if (!state.levels.length)
                    m.addBlockquote('尚未产生升级记录。').addNewline();
                for (const l of state.levels.slice((page - 1) * 3, page * 3)) {
                    ...
                    for (let index = 0; index < labels.length; index += 2)
                        m.addBlockquote(`${labels[index]}+${l.gain[index].toFixed(2)}${index + 1 < labels.length ? `｜${labels[index + 1]}+${l.gain[index + 1].toFixed(2)}` : ''}`).addNewline();
                    m.addNewline();
                }
                if (row.owner_id && state.level >= 2)
                    control('回路重调', `/机巧 重调 ${id} 2 ${state.level} blade`, false);
                paginate(page, pages, p => `/机巧 成长 ${id} ${p}`);
            }
            else {
                if (!event.current.IsPrivate)
                    m.addText('回忆默认私有，请在私密会话查看。');
                else {
                    ...
                    if (!history.events.length)
                        m.addText('你们的共同经历正从这里开始。').addNewline();
                    for (const e of history.events) {
                        ...
                        if (Array.isArray(e.data.skills) && e.data.skills.length)
                            m.addText('领悟：' + e.data.skills.map(id => automatonSkills.find(s => s.id === id)?.name ?? id).join('、')).addNewline();
                        m.addNewline();
                    }
            ...
            if (result.quote)
                interaction = { ...result.quote, name: result.name };
            format = back(m);
        }
        else {
        ...
        if (results.length && results.every(r => r.code === ResultCode.Ok)) {
            try {
                ...
                if (quote) {
                    ...
                    if (sent.length && sent.every(r => r.code === ResultCode.Ok)) {
                        if (mutationQuote)
                            await finishAutomatonMutationQuote(mutationQuote, true);
                        else
                            await acknowledgeAutomatonQuote(quote.id);
                    }
                    else if (mutationQuote && sent.length && sent.every(r => r.code !== ResultCode.Ok))
                        await finishAutomatonMutationQuote(mutationQuote, false);
                }
            }
            catch (error) {
...
var automaton = async () => automatonHandler();
const alchemyCraftHandler = async () => automatonHandler(true);

export { alchemyCraftHandler, automaton as default };
