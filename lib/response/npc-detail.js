...
import { nearbyPoints, npcDetail as npcDetail$1 } from '../game/adventure.service.js';
import { messageFormat } from '../game/message.js';
import { npcSparringView } from '../game/npc-sparring.service.js';
import { requireCurrentGuild } from '../game/guild-context.js';
import { rootGuildPeople, rootGuildScenes } from '../game/opening-guild.config.js';
...
var npcDetail = async () => {
    ...
    try {
        ...
        if (person) {
            ...
            if (!(await nearbyPoints(event.current.UserId)).npcDetailsUnlocked)
            ...
            await message.send({ format: Format.create().addMarkdown(markdown).addButtonGroup(Format.createButtonGroup().addRow().addButton('返回', back, { type: 'command', autoEnter: true })) });
            return;
        }
        const npc = await npcDetail$1(event.current.UserId, code);
        ...
        if (spar) {
            markdown.addNewline().addBlockquote(`切磋 ${spar.used ? 1 : 0}/1`);
            buttons.addButton('切磋', `/切磋 ${spar.profile.code}`, { type: 'command', autoEnter: true, style: spar.used ? undefined : 'blue' });
        }
        ...
        await message.send({ format: messageFormat('无法查看域民资料', error instanceof Error ? error.message : '请稍后重试。') });
    }
};

export { npcDetail as default };
