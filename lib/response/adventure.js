...
import { explore, move, adjustMovementStep, nearbyPoints, movementProfile, moveTo, moveToMap, huntMonster, coordinateInteraction, cancelTravel, travelStatus, completeTravel, mineResource, resourceMiningStatus, cancelResourceMining, chooseTarget, battleStatus, moveToNearbyMonster, queueAmbush, leaveOccupiedBattle, switchCombatTarget, encounterAction, forestGuideAdvance, forestGuideChoice, forestGuideProgress, continueForestArrival, combatAction, requireNpcAtCurrentPosition, addNpcAffinity, inventory, talkToNpc, currentEncounter, blockedDungeonDirections, forceAutoBattleDefeat, continueCombatChant, negotiateEncounter, bossRandomEffectSummary, claimCombatAmbushHandoffs } from '../game/adventure.service.js';
import { hiddenSkill, isHiddenSkill } from '../game/hidden-profession.config.js';
import { Format, useEvent, logger, useRoute, MessageDirect } from 'alemonjs';
import { useGameMessage, withoutAutomatonInteractions, separateAutomatonBattleQuotes } from '../game/use-game-message.js';
import { acknowledgeAutomatonBattleText } from '../game/automaton-dialogue.service.js';
...
import { adventurerProfile, registerAdventurer, chooseProfession } from '../game/character.service.js';
...
import { startPvpBattle, pvpBattleStatus, selectPvpBattleOption, pvpCombatAction, cityWantedAlert, continuePvpChant, reserveWarrantEntryNotice, startAmbushPvpBattle } from '../game/pvp.service.js';
...
const addAdvancedBattleButton = (row, label, command, ready, autoEnter = true) => row.addButton(label, command, {
...
const battleButtons = (battle) => {
    ...
    if (battle.advancedSkills?.length) {
        ...
        if (summonSkills.length) {
            ...
            for (const [index, skill] of battle.advancedSkills.entries()) {
    ...
    if (battle.mode !== 'spar' && (battle.members?.length ?? 0) > 1)
        buttons.addButton('援助', '/药剂援助', { type: 'command', autoEnter: true });
    ...
    if (battle.appraisal.learned)
        finalRow.addButton('鉴识', '/鉴识', { type: 'command', autoEnter: true, style: 'blue' });
...
const appendBattleState = (markdown, battle) => {
    if (battle.talentNote)
        markdown.addBlockquote(battle.talentNote).addNewline();
    ...
    for (const [index, target] of battle.targets.entries()) {
        ...
        if (!target.isBossComponent && target.passiveSummary)
            markdown.addBlockquote(`被动：${target.passiveSummary}`).addNewline();
        if (target.randomEffects?.length)
            markdown.addBlockquote(`高难词条：${target.randomEffects.join('｜')}`).addNewline();
        if (target.mechanicSummary)
            markdown.addBlockquote(`机制：${target.mechanicSummary}`).addNewline();
        if (target.statusText)
            markdown.addBlockquote(`状态：${target.statusText}`).addNewline();
    }
    return markdown;
};
const encounterRandomEffectLines = (spawns) => [...new Set(spawns
...
const battleStartFormat = (text, battle, reward) => {
    ...
    if (reward)
        markdown.addBold(`已获得：${reward}`).addNewline().addNewline();
    return Format.create().addMarkdown(appendBattleState(markdown, battle)).addButtonGroup(battleButtons(battle));
};
...
const bossPhaseTransitionFormat = (transitions) => {
    ...
    for (const [index, transition] of transitions.entries()) {
        if (index)
            markdown.addNewline().addText('————————————').addNewline().addNewline();
        markdown.addBold(`【${transition.title}】`).addNewline().addNewline()
            .addText(transition.description).addNewline().addNewline();
        for (const line of transition.dialogue)
            markdown.addBlockquote(`【${line.speaker}】“${line.text}”`).addNewline();
...
const victoryFormat = (settlement) => {
    ...
    for (const reward of settlement.members) {
        ...
        if (reward.talentNotice)
            markdown.addBlockquote(reward.talentNotice).addNewline();
...
const sendCombatResult = async (message, qqUserId, result, options = {}) => {
    ...
    const separateTransitions = options.transitionMode !== 'inline';
    const sendBossTransitions = async () => {
        if (separateTransitions && presentation.bossTransitions?.length)
            await message.send({ format: bossPhaseTransitionFormat(presentation.bossTransitions) });
    };
    const displayedLog = separateTransitions ? presentation.manualLog ?? result.log : result.log;
    if (result.ended) {
        if (!options.omitFinalLog)
            await message.send({ format: finalBattleFormat(displayedLog) });
    ...
    for (const entry of await pendingPartyAutoBattleActions(qqUserId)) {
        if (entry.targetId) {
            try {
                ...
                if (!(error instanceof Error) || error.message !== '该目标已被击败或不在本场战斗中。')
                    throw error;
            }
        }
        try {
...
const resolveRemainingAutoBattle = async (qqUserId, initial) => {
    ...
    for (let round = 0; round < 200; round += 1) {
        ...
        pending = undefined;
        if (!result || result.ended || result.waiting || hasBossPhaseTransitions(result))
            return result;
    }
    return null;
};
        ...
        autoBattleTimers.set(qqUserId, setTimeout(async () => {
            try {
                ...
                if (omitted) {
                    ...
                    if (preserved.ended)
                        await sendCombatResult(message, qqUserId, preserved, { omitFinalLog: !preservesTransition, transitionMode: 'inline' });
                    else if (preservesTransition)
                        await sendCombatResult(message, qqUserId, preserved, { transitionMode: 'inline' });
                ...
                if (!result.ended && !result.waiting && visibleRounds >= AUTO_BATTLE_VISIBLE_ROUND_LIMIT) {
                    ...
                    if (finalResult?.ended)
                        await sendCombatResult(message, qqUserId, finalResult, { omitFinalLog: !hasBossPhaseTransitions(finalResult), transitionMode: 'inline' });
...
const startAutoBattle = async (message, qqUserId, openingText = '') => {
    ...
    if (await isFullPartyAutoBattle(qqUserId)) {
        ...
        if (full.result?.ended)
            await sendCombatResult(message, qqUserId, full.result, { omitFinalLog: true, transitionMode: 'inline' });
...
const sendAmbushDirect = async (handoff, format, mention = false) => {
    ...
    const results = await MessageDirect.create().sendToTarget({ target, format: split.quotes.length ? split.source : format });
    if (results.length && results.every(result => result.code === 2000)) {
        try {
            for (const quote of split.quotes) {
                ...
                if (sent.length && sent.every(result => result.code === 2000))
                    await acknowledgeAutomatonBattleText(handoff.ambusherQqUserId, quote);
            }
        }
        catch (error) {
            logger.warn({ err: error }, '伏击战报已发送，机巧互动消息暂未送达');
        }
    }
};
...
const guildFrontDeskFormat = async (qqUserId, text, continuingChat = false) => {
    ...
    if (context.code !== 'baina_town') {
        ...
        const markdown = npcInteractionMarkdown('冒险者公会·前台', context.hub.host, text?.replaceAll('莫妮卡', context.hub.host) ?? context.hub.description, context.code === 'world_tree' ? 'root_guild_clerk' : undefined, nearby.npcDetailsUnlocked);
        const buttons = Format.createButtonGroup();
        if (!continuingChat) {
            ...
            if (context.code === 'floating_leaf_town' && mainQuest.title === '【主线·无形的禁锢】')
                buttons.addRow().addButton('询问等级停滞', '/浮叶瓶颈 guild', { type: 'command', autoEnter: true, style: 'blue' });
            if (context.code === 'floating_leaf_town' && mainQuest.title === '【主线·失踪的孩子】')
                buttons.addRow().addButton('前往公馆', '/前往 15 0 30', { type: 'command', autoEnter: false, style: 'blue' });
        }
...
const exploreHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const inventoryHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    const bag = await inventory(event.current.UserId);
    const penaltyText = `移速降低 ${bag.speedPenaltyPct.toFixed(1)}%${bag.overloadPct > 0 ? `｜超重 ${bag.overloadPct.toFixed(1)}%` : ''}`;
    await message.send({ format: messageFormat('冒险背包', `负重 ${bag.weight.toFixed(2)}/${bag.capacity.toFixed(2)} kg｜${penaltyText}\n当前速度 ${bag.speed.toFixed(2)}\n\n${bag.items.length ? bag.items.map(i => `${i.equipped_slot ? `[已装备·${i.equipped_slot}] ` : i.quick_slot ? `[道具${i.quick_slot}] ` : ''}${i.name} ×${i.quantity}（${i.weight}kg）`).join('\n') : '背包为空。'}`) });
}
catch (error) {
    await fail(message, error);
} };
...
const dungeonEnterHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const dungeonFloorHandler = (direction) => async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const dungeonChestHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const dungeonPvpHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const playerInteractionHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const showMoveResult = async (message, qqUserId, result) => {
    ...
    if (result.character?.talentScavengeNotice)
    ...
    if (randomEffects.length)
        markdown.addNewline().addNewline().addTitle('高难词条').addNewline().addBlockquote(randomEffects.join('\n'));
    if (hasBoss)
...
const continueStoryHandler = async () => {
    ...
    const [message] = useGameMessage();
    try {
        if (await (await import('./opening.js')).continueOpeningIfPresent(event.current.UserId, message))
            return;
...
const buildingHandler = (action) => async () => {
    ...
    try {
        ...
        if (code === 'guild_counter' && action === 'enter')
            await message.send({ format: Format.create().addMarkdown(Format.createMarkdown().addTitle('冒险者总公会·每日委托').addNewline().addNewline().addText('领取一份已完成的正式悬赏。').addNewline().addButton('[查看今日委托]', { data: '/每日势力委托 adventurer_guild', autoEnter: false })) });
        if (Object.values((await import('../game/opening-world.config.js')).openingHubs).some(hub => hub.guild === code)) {
            ...
            if (action === 'enter' || action === 'leave' || action === 'ignore')
                await guild.enterOpeningGuild(event.current.UserId, action === 'enter');
            if (action === 'area') {
        ...
        if (action === 'enter') {
            const financeFactionAtBuilding = {
            ...
            if (factionCode) {
                ...
                if (['worldtree_council', 'rediron_office', 'ferry_office'].includes(code))
                    return;
            }
        }
        if (code === 'blacksmith') {
...
const guildRegistrationHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    ...
    const markdown = Format.createMarkdown().addTitle('冒险家 注册').addNewline().addNewline().addBlockquote('你将手轻放在水晶球上，顿时散发出一阵耀眼的白光。').addNewline().addBlockquote(context.code === 'world_tree' ? '维萝扶稳水晶球，将一张漆黑的卡片贴近光晕。银线顺着卡面舒展开来，她用指腹抚平卡角，又轻声核对了一遍你的名字。' : `${context.code === 'baina_town' ? '莫妮卡' : context.hub.host}将一张通体漆黑卡片靠近球体，光芒汇聚成一道银线注入，卡片逐渐被染成银白。`).addNewline().addBlockquote('随后一行行异世界文字在卡面上依次浮现——').addNewline().addNewline().addText('获得【冒险者 卡片】').addNewline().addText('已获得一次地图兑换机会，可到公会集结区兑换已开放的 Lv.30 及以下地图。').addNewline().addText('你随时可以发送 ').addButton('/卡片', { data: '/卡片', autoEnter: false }).addText(' 查看。').addNewline().addText('你已察觉到自身属性，发送 ').addButton('/角色', { data: '/角色', autoEnter: false }).addText(' 查看。');
    await message.send({ format: Format.create().addMarkdown(markdown).addButtonGroup(Format.createButtonGroup().addRow().addButton('前往集结区', context.code === 'baina_town' ? '/建筑区域 guild_counter 集结区' : '/初行公会 集结区', { type: 'command', autoEnter: false, style: 'blue' }).addButton('返回前台', context.code === 'baina_town' ? '/建筑区域 guild_counter 前台' : '/初行公会 前台', { type: 'command', autoEnter: false, style: 'blue' })) });
}
catch (error) {
    await fail(message, error, '注册失败');
} };
const professionHandler = (action) => async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const guildChatHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    const context = await (await import('../game/guild-context.js')).requireCurrentGuild(event.current.UserId);
    ...
    if (context.code !== 'baina_town')
        throw new Error('请在当地公会的人物交谈处与接待员交谈。');
...
const guildBarrierHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    await requireNpcAtCurrentPosition(event.current.UserId, 'guild_counter');
...
const mineResourceHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const refreshMiningHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const cancelMiningHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const moveHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const goToHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await leaveHomeForMovement(message, event.current.UserId);
...
const goToMapHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    await leaveHomeForMovement(message, event.current.UserId);
...
const huntHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const cancelTravelHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const targetHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const nearbyMonsterInteractionHandler = async () => {
    ...
    try {
        ...
        if (!await showBlockedEncounter(message, event.current.UserId))
            throw new Error('该怪物已离开当前位置或被击败。');
    }
    catch (error) {
        if (error instanceof Error && error.message.includes('当前格子存在敌对生物') && await showBlockedEncounter(message, event.current.UserId))
            return;
        if (await showOngoingActivity(message, event.current.UserId, '无法交互'))
            return;
...
const ambushHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const queueAmbushHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
    const spawnId = Number(route.param('id'));
    const channelId = String(event.current.ChannelId ?? '');
...
const leaveOccupiedBattleHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
...
const forestGuideHandler = async () => { const [event] = useEvent(); const [route] = useRoute(); const [message] = useGameMessage(); try {
...
const advancedSkillIdFor = (battle, request) => {
    ...
    if (!Number.isInteger(skillId) || !skillId || !battle.advancedSkills.some(skill => skill.id === skillId))
        throw new Error('该技能不是你当前可用的二转主动技能。');
    return skillId;
};
const summonSpiritRequest = (input) => {
    ...
    if (!spirit)
        throw new Error(`请输入灵契编号或名称：${spiritDefinitions.map((candidate, i) => `${i + 1}.${candidate.name}`).join('、')}。`);
...
const actionHandler = (action, advancedSkill = false, requestAdvancedSkill) => async () => {
    ...
    const [route] = useRoute();
    const [message] = useGameMessage();
    const slot = advancedSkill ? undefined : Number(route.param('slot')) || undefined;
    const skillRequest = advancedSkill ? requestAdvancedSkill?.(route.param('spirit')) ?? { id: Number(route.param('id')) } : undefined;
    try {
        if (advancedSkill && (!skillRequest || (!skillRequest.code && (!Number.isInteger(skillRequest.id) || !skillRequest.id))))
            throw new Error('二转技能编号无效。');
        let pveBattle = null;
        try {
        ...
        try {
            ...
            const skillId = advancedSkill ? advancedSkillIdFor(pvpBattle, skillRequest) : undefined;
            ...
            const pvpResult = await pvpCombatAction(event.current.UserId, action, slot, undefined, undefined, false, skillId);
...
const alchemyAllyHandler = async () => {
    ...
    try {
        ...
        if (slot && targetId) {
            if (!battle.members.some(member => member.id === targetId && !member.defeated))
                throw new Error('请选择本场存活队友。');
            stopAutoBattle(user);
            let result = await combatAction(user, 'item', slot, undefined, undefined, undefined, 'member', targetId);
            if (!result.ended && result.waiting)
                result = await resolvePartyAutoBattleActions(user) ?? result;
        ...
        const buttons = Format.createButtonGroup();
        if (!slot)
            for (const itemSlot of battle.itemSlots)
                buttons.addRow().addButton(`道具${'①②③④'[itemSlot - 1]}`, `/药剂援助 ${itemSlot}`, { type: 'command', autoEnter: true });
        else
            for (const member of battle.members.filter(member => !member.defeated))
                buttons.addRow().addButton(member.name, `/药剂援助 ${slot} ${member.id}`, { type: 'command', autoEnter: true });
...
const hiddenCombatConfirmHandler = async () => {
    ...
    try {
        if (ticket.battleKey.startsWith('pvp:')) {
            ...
            if (!result.ended)
                await startPvpAutoBattle(message, user);
        }
        else {
            stopAutoBattle(user);
            ...
            await sendCombatResult(message, user, result);
            if (!result.ended && !result.waiting)
                await startAutoBattle(message, user);
        }
    }
    catch (error) {
...
const defendHandler = async () => { const [event] = useEvent(); const [message] = useGameMessage(); try {
    stopAutoBattle(event.current.UserId);
