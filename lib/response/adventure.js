...
import { explore, move, adjustMovementStep, nearbyPoints, movementProfile, moveTo, moveToMap, huntMonster, coordinateInteraction, cancelTravel, travelStatus, completeTravel, mineResource, resourceMiningStatus, cancelResourceMining, chooseTarget, battleStatus, moveToNearbyMonster, queueAmbush, leaveOccupiedBattle, switchCombatTarget, encounterAction, forestGuideAdvance, forestGuideChoice, forestGuideProgress, continueForestArrival, combatAction, requireNpcAtCurrentPosition, addNpcAffinity, inventory, blockedDungeonDirections, requireNpcForOrdinaryTalk, talkToNpc, continueCombatChant, currentEncounter, forceAutoBattleDefeat, claimCombatAmbushHandoffs, negotiateEncounter, bossRandomEffectSummary } from '../game/adventure.service.js';
...
const sendCombatResult = async (message, qqUserId, result, options = {}) => {
    ...
    const sendBossTransitions = async () => {
        if (!options.inlineBossTransitions && presentation.bossTransitions?.length)
        ...
        autoBattleTimers.set(qqUserId, setTimeout(async () => {
            try {
                ...
                if (omitted) {
                    ...
                    if (preserved.ended)
                        await sendCombatResult(message, qqUserId, preserved, { omitFinalLog: true, inlineBossTransitions: true });
                ...
                if (!result.ended && !result.waiting && visibleRounds >= AUTO_BATTLE_VISIBLE_ROUND_LIMIT) {
                    ...
                    if (finalResult?.ended)
                        await sendCombatResult(message, qqUserId, finalResult, { omitFinalLog: !hasBossPhaseTransitions(finalResult), inlineBossTransitions: true });
...
const startAutoBattle = async (message, qqUserId, openingText = '') => {
    ...
    if (await isFullPartyAutoBattle(qqUserId)) {
        ...
        if (full.result?.ended)
            await sendCombatResult(message, qqUserId, full.result, { omitFinalLog: true, inlineBossTransitions: true });
...
const professionCodeByName = { 战士: 'warrior', 法师: 'mage', 盗贼: 'rogue', 牧师: 'priest', 射手: 'archer' };
