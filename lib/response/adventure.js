...
import { explore, move, adjustMovementStep, nearbyPoints, movementProfile, moveTo, moveToMap, huntMonster, coordinateInteraction, cancelTravel, travelStatus, completeTravel, mineResource, resourceMiningStatus, cancelResourceMining, chooseTarget, battleStatus, moveToNearbyMonster, queueAmbush, leaveOccupiedBattle, switchCombatTarget, encounterAction, forestGuideAdvance, forestGuideChoice, forestGuideProgress, continueForestArrival, combatAction, requireNpcAtCurrentPosition, addNpcAffinity, inventory, requireNpcForOrdinaryTalk, talkToNpc, continueCombatChant, currentEncounter, blockedDungeonDirections, forceAutoBattleDefeat, claimCombatAmbushHandoffs, negotiateEncounter, bossRandomEffectSummary } from '../game/adventure.service.js';
...
const showMoveResult = async (message, qqUserId, result) => {
    ...
    if (result.routeNotices?.length)
        await message.send({ format: messageFormat('途中见闻', result.routeNotices.join('\n\n')) });
    try {
