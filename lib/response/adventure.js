...
import { explore, move, adjustMovementStep, nearbyPoints, movementProfile, moveTo, moveToMap, huntMonster, coordinateInteraction, cancelTravel, travelStatus, completeTravel, mineResource, resourceMiningStatus, cancelResourceMining, chooseTarget, battleStatus, moveToNearbyMonster, queueAmbush, leaveOccupiedBattle, switchCombatTarget, encounterAction, forestGuideAdvance, forestGuideChoice, forestGuideProgress, continueForestArrival, combatAction, requireNpcAtCurrentPosition, addNpcAffinity, inventory, talkToNpc, continueCombatChant, currentEncounter, blockedDungeonDirections, forceAutoBattleDefeat, claimCombatAmbushHandoffs, negotiateEncounter, bossRandomEffectSummary } from '../game/adventure.service.js';
...
import { startPvpBattle, pvpBattleStatus, selectPvpBattleOption, pvpCombatAction, cityWantedAlert, continuePvpChant, startAmbushPvpBattle, reserveWarrantEntryNotice } from '../game/pvp.service.js';
...
const battleButtons = (battle) => {
    ...
    if (battle.canLeafAnchor)
        buttons.addButton('重新锚定', '/航路锚定', { type: 'command', autoEnter: false, style: battle.canAct ? 'blue' : undefined });
...
const appendBattleState = (markdown, battle) => {
    ...
    for (const [index, target] of battle.targets.entries()) {
        ...
        if (!target.hideBossMechanics && !target.isBossComponent && target.passiveSummary)
            markdown.addBlockquote(`被动：${target.passiveSummary}`).addNewline();
        if (!target.hideBossMechanics && target.randomEffects?.length)
            markdown.addBlockquote(`高难词条：${target.randomEffects.join('｜')}`).addNewline();
        if (!target.hideBossMechanics && target.mechanicSummary)
        ...
        if (target.encounterStatus)
            markdown.addBlockquote(target.encounterStatus).addNewline();
    }
    return markdown;
};
...
const sendCombatResult = async (message, qqUserId, result, options = {}) => {
    ...
    const sendBossTransitions = async () => {
        if (presentation.bossTransitions?.length)
...
const resolveFullAutoBattle = async (qqUserId) => {
    ...
    for (let round = 0; round < 100; round += 1) {
        ...
        if (result.log)
            logs.push(combatResultPresentation(result).manualLog ?? result.log);
        if (result.ended || result.waiting || hasBossPhaseTransitions(result))
        ...
        autoBattleTimers.set(qqUserId, setTimeout(async () => {
            try {
                ...
                if (omitted) {
                    ...
                    if (preserved.ended)
                        await sendCombatResult(message, qqUserId, preserved, { omitFinalLog: !preservesTransition });
                    else if (preservesTransition)
                        await sendCombatResult(message, qqUserId, preserved);
                ...
                if (!result.ended && !result.waiting && visibleRounds >= AUTO_BATTLE_VISIBLE_ROUND_LIMIT) {
                    ...
                    if (finalResult?.ended)
                        await sendCombatResult(message, qqUserId, finalResult, { omitFinalLog: !hasBossPhaseTransitions(finalResult) });
    ...
    if (await isFullPartyAutoBattle(qqUserId)) {
        ...
        if (full.result?.ended)
            ...
            if (transitions?.length)
                await message.send({ format: bossPhaseTransitionFormat(transitions) });
            scheduleAutoBattle(message, qqUserId, true);
        }
