...
import { previewEquipmentEnchantment, executeEquipmentEnchantment, enchantableEquipment, compatibleEnchantCards } from '../game/equipment-enchantment.service.js';
...
import { requireBlacksmith } from './blacksmith.js';
...
const command = (name, suffix = '') => `/${inShop() ? '店铺' : ''}${name}${suffix ? ` ${suffix}` : ''}`;
...
const equipmentListFormat = async (qqUserId, page = 1, keyword = '') => {
    ...
    if (!view.entries.length)
        markdown.addBlockquote('背包中没有符合条件的装备。').addNewline();
...
const cardListFormat = async (qqUserId, instanceId, page = 1, keyword = '') => {
    ...
    if (!view.entries.length)
        markdown.addBlockquote('背包中没有满足该部位与装备等级要求的怪物卡片。').addNewline();
...
const sendError = async (title, error) => {
    const [message] = useGameMessage();
    await message.send({ format: messageFormat(title, error instanceof Error ? error.message : '请稍后重试。') });
};
const enchantmentListHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await requireBlacksmith(event.current.UserId);
        await message.send({ format: await equipmentListFormat(event.current.UserId, Number(route.param('page') ?? 1), String(route.param('keyword') ?? '')) });
    }
    catch (error) {
        await sendError('无法查看附魔装备', error);
    }
};
const enchantmentSearchHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await requireBlacksmith(event.current.UserId);
        await message.send({ format: await equipmentListFormat(event.current.UserId, 1, String(route.param('keyword') ?? '')) });
    }
    catch (error) {
        await sendError('无法搜索附魔装备', error);
    }
};
const enchantmentPutHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await requireBlacksmith(event.current.UserId);
        await message.send({ format: await cardListFormat(event.current.UserId, Number(route.param('id'))) });
    }
    catch (error) {
        await sendError('无法放入附魔装备', error);
    }
};
const enchantmentCardPageHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await requireBlacksmith(event.current.UserId);
        await message.send({ format: await cardListFormat(event.current.UserId, Number(route.param('id')), Number(route.param('page')), String(route.param('keyword') ?? '')) });
    }
    catch (error) {
        await sendError('无法查看附魔卡片', error);
    }
};
const enchantmentCardSearchHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await requireBlacksmith(event.current.UserId);
        await message.send({ format: await cardListFormat(event.current.UserId, Number(route.param('id')), 1, String(route.param('keyword') ?? '')) });
    }
    catch (error) {
        await sendError('无法搜索附魔卡片', error);
    }
};
const enchantmentPreviewHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        ...
        if (preview.warning)
            markdown.addBlockquote(`注意：${preview.warning}`).addNewline();
        ...
        await sendError('无法预览附魔', error);
    }
};
const enchantmentExecuteHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        ...
        await sendError('附魔失败', error);
    }
};

export { enchantmentCardPageHandler, enchantmentCardSearchHandler, enchantmentExecuteHandler, enchantmentListHandler, enchantmentPreviewHandler, enchantmentPutHandler, enchantmentSearchHandler };
