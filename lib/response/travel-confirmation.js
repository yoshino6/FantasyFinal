...
const travelConfirmationFormat = (result) => {
    ...
    if (guild && portal)
        md.addNewline().addText('途中还需通过界门换乘。');
    const position = (p) => `（${p.x}, ${p.y}, ${p.z}）`;
    for (const leg of plan.legs)
        md.addNewline().addText(`${leg.kind === 'walk' ? '步行' : leg.kind === 'portal' ? '界门' : '公会接驳'}${leg.name ? `·${leg.name}` : ''}：${position(leg.from)} → ${position(leg.to)}，${durationText(leg.seconds ?? 0)}`);
...
export { travelConfirmationFormat };
