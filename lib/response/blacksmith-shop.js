...
import { buyBlacksmithEquipment, sellBlacksmithEquipment, sellBlacksmithMaterial, blacksmithShopCatalog, blacksmithSellCatalog } from '../game/blacksmith-shop.service.js';
...
const pageButtons = (page, totalPages, category, keyword = '') => {
...
const buyFormat = async (qqUserId, page = 1, category = '全部', keyword = '') => {
    ...
    const addCategoryLinks = (title, types) => {
    ...
    if (shop.category !== '防具' && !armorShopCategories.includes(shop.category) && !armorShopTypes.includes(shop.category))
        addCategoryLinks('武器分类', weaponShopTypes);
    if (shop.category !== '武器' && !weaponShopTypes.includes(shop.category)) {
        if (!armorShopCategories.includes(shop.category))
            addCategoryLinks('防具部位', armorShopCategories);
        if (!armorShopTypes.includes(shop.category))
            addCategoryLinks('防具甲类', armorShopTypes);
    }
    if (!shop.items.length)
        markdown.addText('没有找到符合条件的装备。');
    const sequence = '①②③④⑤';
