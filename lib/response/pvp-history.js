...
import { pvpBattleHistory } from '../game/pvp.service.js';
import { messageFormat } from '../game/message.js';

const outcomeText = { ongoing: '进行中', attacker_win: '进攻方胜利', defender_win: '防守方胜利', draw: '未分胜负', escaped: '撤离战斗' };
const timeText = (value) => value ? new Date(value).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai', hour12: false }) : '进行中';
var pvpHistory = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        const filter = String(route.param('filter') ?? '全部');
        const normalizedFilter = ['全部', '进攻方', '防守方'].includes(filter) ? filter : '全部';
        ...
        if (!data.entries.length)
            markdown.addBlockquote('暂无符合条件的 PvP 战斗记录。').addNewline();
        for (const [index, entry] of data.entries.entries()) {
            ...
            if (entry.loot)
                markdown.addBlockquote(`物品得失：${entry.loot}`).addNewline();
            markdown.addNewline();
        }
