...
import { addWarrantReward, townWarrantsFor } from '../game/pvp.service.js';
import { messageFormat } from '../game/message.js';

const sequence = '①②③④⑤⑥⑦⑧⑨⑩';
const warrantFormat = async (qqUserId, filter = '全部') => {
    ...
    if (!data.warrants.length)
        markdown.addBlockquote(filter === '全部' ? '这座城镇目前没有生效中的通缉。' : `当前没有「${filter}」的通缉者。`);
    for (const [index, warrant] of data.warrants.entries()) {
        ...
        if (warrant.exposed || warrant.recent)
            markdown.addButton('[前往]', { data: `/前往 ${warrant.x} ${warrant.y} ${warrant.z}`, autoEnter: false }).addText(' ');
...
var warrant = async () => {
    ...
    try {
        await message.send({ format: await warrantFormat(event.current.UserId) });
    }
    catch (error) {
        await message.send({ format: messageFormat('通缉令不可用', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const activeWarrantHandler = async () => {
    const [event] = useEvent();
    const [message] = useGameMessage();
    try {
        await message.send({ format: await warrantFormat(event.current.UserId) });
        ...
        await message.send({ format: messageFormat('通缉面板不可用', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const filterWarrantHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        await message.send({ format: await warrantFormat(event.current.UserId, String(route.param('filter'))) });
    }
    catch (error) {
        await message.send({ format: messageFormat('通缉筛选不可用', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const addRewardHandler = async () => {
    ...
    try {
        ...
        const id = Number(route.param('warrant'));
        const itemOrAmount = Number(route.param('itemOrAmount'));
        const quantity = Number(route.param('quantity') ?? 1);
        if (kind === '铜币') {
            if (!Number.isInteger(itemOrAmount) || itemOrAmount < 1)
                throw new Error('赏金金额必须是正整数。');
            await addWarrantReward(event.current.UserId, id, null, 0, itemOrAmount);
        }
        else {
            if (!Number.isInteger(itemOrAmount) || itemOrAmount < 1 || !Number.isInteger(quantity) || quantity < 1)
                throw new Error('物品编号与数量必须是正整数。');
        ...
        await message.send({ format: messageFormat('追加赏金失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};

export { activeWarrantHandler, addRewardHandler, warrant as default, filterWarrantHandler };
