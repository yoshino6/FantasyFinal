...
import { equipment as equipment$1, unequip, equipmentCandidates, equip } from '../game/adventure.service.js';
import { messageFormat } from '../game/message.js';
import { equipmentSetSummary } from '../game/equipment-set-summary.js';
...
var equipment = async () => {
    ...
    try {
        const items = await equipment$1(event.current.UserId);
        ...
        for (const slot of slotOrder) {
            ...
            if (item?.appearanceName)
                markdown.addBlockquote(`外观投影：${item.appearanceName}`).addNewline();
            markdown.addText(`\n【${slotNames[slot]}】`);
            if (item) {
                if (item.instance_id)
                    markdown.addButton(item.name, { data: `/装备详情 ${item.instance_id}`, autoEnter: false });
                else
                    markdown.addText(item.name);
                markdown.addText(' ').addButton('[卸下]', { data: `/卸下装备 ${slot}`, autoEnter: false }).addText(' ').addButton('[切换]', { data: `/选择装备 ${slot}`, autoEnter: false });
            }
            else
                markdown.addText('无 ').addButton('[装备]', { data: `/选择装备 ${slot}`, autoEnter: false });
        }
        const sets = equipmentSetSummary(items);
        markdown.addNewline().addNewline().addBold('已生效套装效果').addNewline();
        if (!sets.length)
            markdown.addBlockquote('暂无已生效套装效果。');
        for (const set of sets)
            markdown.addBlockquote(`**${set.title}**\n> ${set.effects.join('\n> ')}`).addNewline();
        ...
        await message.send({ format: messageFormat('装备不可用', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const unequipHandler = async () => {
    ...
    try {
        ...
        await message.send({ format: messageFormat('卸下失败', error instanceof Error ? error.message : '请稍后重试。') });
    }
};
const chooseEquipmentHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        const slot = String(route.param('slot'));
        const items = await equipmentCandidates(event.current.UserId, slot);
...
const equipHandler = async () => {
    ...
    try {
        const slot = String(route.param('slot'));
        const item = await equip(event.current.UserId, slot, Number(route.param('id')));
...
export { chooseEquipmentHandler, equipment as default, equipHandler, unequipHandler };
