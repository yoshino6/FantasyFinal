...
import { worldAdminSnapshot, worldLedger, worldContentPreview, setWorldContentEnabled, setWorldContentWeight } from '../game/world-dynamics.service.js';
...
const section = (markdown, title, lines, empty) => {
    markdown.addText(title).addNewline().addBlockquote(lines.length ? lines.join('\n') : empty).addNewline().addNewline();
};
const displayStatus = (value, fallback = '状态未记录') => statusNames[String(value ?? '')] ?? fallback;
const worldManagementHandler = async () => {
...
const worldLedgerHandler = async () => {
    const [event] = useEvent();
    ...
    try {
        ...
        if (!rows.length)
            markdown.addBlockquote('暂无事件记录。');
        else
            for (const row of rows)
                markdown.addBlockquote(`第 ${row.id} 条｜${row.createdAt.toLocaleString('zh-CN', { hour12: false })}\n${eventNames[row.eventType] ?? '世界事件'}｜${outcomeNames[row.outcome] ?? '状态已更新'}｜${row.actorName ?? '系统'}｜${row.regionName ?? '全局'}`).addNewline();
...
const worldContentPreviewHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
        ...
        if (!rows.length)
            markdown.addBlockquote('暂无内容模板。');
        else
            for (const row of rows)
                markdown.addText(`【${row.title}】`).addNewline().addBlockquote(`${row.enabled ? '已启用' : '已停用'}｜触发权重 ${row.weight}\n适用区域 ${row.regions.length} 处｜适用天气 ${row.weather.length} 种\n${row.valid ? '配置校验通过' : '配置存在需要处理的问题'}`).addNewline().addNewline();
        await message.send({ format: Format.create().addMarkdown(markdown).addButtonGroup(Format.createButtonGroup().addRow().addButton('世界生态', '/世界生态管理', { type: 'command', autoEnter: true, style: 'blue' })) });
    }
    catch (error) {
...
const worldContentEnabledHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
...
const worldContentWeightHandler = async () => {
    const [event] = useEvent();
    const [route] = useRoute();
    const [message] = useGameMessage();
    try {
...
export { worldContentEnabledHandler, worldContentPreviewHandler, worldContentWeightHandler, worldLedgerHandler, worldManagementHandler };
